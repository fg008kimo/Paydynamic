...
static unsigned int sqlctx = 2538491;
...
extern void sqliem( unsigned char *, signed int * );
 ...
 static char *sq0004 = 
"select seq_num ,merchant_ref ,country ,identity_id ,account_num ,account_na\
me ,bank_name ,bank_code ,branch ,phone_num ,province ,city ,amount ,payout_c\
urrency ,dest_currency ,checksum  from payout_record where (batch_id=:b0 and \
merchant_id=:b1)           ";
...
{12,4130,871,0,0,
5,0,0,1,661,0,6,248,0,0,21,21,0,1,0,2,3,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,
9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,1,0,0,1,9,0,0,
104,0,0,2,93,0,6,325,0,0,3,3,0,1,0,2,3,0,0,1,9,0,0,1,9,0,0,
131,0,0,3,0,0,17,422,0,0,1,1,0,1,0,1,9,0,0,
150,0,0,3,0,0,21,423,0,0,0,0,0,1,0,
165,0,0,4,256,0,9,515,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
188,0,0,4,0,0,13,517,0,0,16,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,
2,9,0,0,
267,0,0,4,0,0,15,659,0,0,0,0,0,1,0,
282,0,0,4,0,0,15,669,0,0,0,0,0,1,0,
};
...
-------------------------------                    ------------            --------------
Init Version                                       2010/09/24              Cody Chan
*/
...
void PayoutRecord(char    cdebug)
{
        cDebug = cdebug;
}

int Add(const hash_t *hPayoutRecord)
{
	char		*csTmp;
	char		cTmp;
...
struct { unsigned short len; unsigned char arr[16]; } hv_batch_id;
		...
		/* varchar         hv_seq_num[PD_SEQ_NUM_LEN]; */ 
...
struct { unsigned short len; unsigned char arr[3]; } hv_payout_currency;
		...
		/* varchar		hv_dest_currency[PD_CCY_ID_LEN]; */ 
struct { unsigned short len; unsigned char arr[3]; } hv_dest_currency;
...
struct { unsigned short len; unsigned char arr[12]; } hv_amount;
		...
		char		hv_status;
		/* varchar		hv_resp_code[PD_ERROR_CODE_LEN]; */ 
struct { unsigned short len; unsigned char arr[6]; } hv_resp_code;
...
/*batch_id*/
	if(GetField_CString(hPayoutRecord,"batch_id",&csTmp)){
...
/*seq_num*/
	if(GetField_CString(hPayoutRecord,"seq_num",&csTmp)){
		hv_seq_num.len = strlen(csTmp);
		memcpy(hv_seq_num.arr, csTmp, hv_seq_num.len);
		ind_seq_num = 0;
...
/*merchant ref no*/
	if(GetField_CString(hPayoutRecord,"merchant_ref",&csTmp)){
...
/*country*/
	if(GetField_CString(hPayoutRecord,"country",&csTmp)){
...
/*identity id no*/
	if(GetField_CString(hPayoutRecord,"identity_id",&csTmp)){
...
/*account_num*/
	if(GetField_CString(hPayoutRecord,"account_num",&csTmp)){
...
/*account name*/
	if(GetField_CString(hPayoutRecord,"account_name",&csTmp)){
...
/*bank_name*/
	if(GetField_CString(hPayoutRecord,"bank_name",&csTmp)){
...
/*bank_code*/
	if(GetField_CString(hPayoutRecord,"bank_code",&csTmp)){
...
/*branch*/
	if(GetField_CString(hPayoutRecord,"branch",&csTmp)){
...
/*phone_num*/
	if(GetField_CString(hPayoutRecord,"phone_num",&csTmp)){
...
/*province*/
	if(GetField_CString(hPayoutRecord,"province",&csTmp)){
...
/*city*/
	if(GetField_CString(hPayoutRecord,"city",&csTmp)){
...
/*payout currency*/
	if(GetField_CString(hPayoutRecord,"payout_currency",&csTmp)){
...
/*dest_currency*/
	if(GetField_CString(hPayoutRecord,"dest_currency",&csTmp)){
		hv_dest_currency.len = strlen(csTmp);
		memcpy(hv_dest_currency.arr, csTmp, hv_dest_currency.len);
		ind_dest_currency = 0;
DEBUGLOG(("Add:dest_currency = [%.*s]\n",hv_dest_currency.len,hv_dest_currency.arr));
	}

/*checksum*/
	if(GetField_CString(hPayoutRecord,"checksum",&csTmp)){
...
/*merchant_id*/
	if(GetField_CString(hPayoutRecord,"merchant_id",&csTmp)){
...
/*amount*/
	if (GetField_CString(hPayoutRecord,"amount",&csTmp)) {
...
/*resp_code*/
	if (GetField_CString(hPayoutRecord,"resp_code",&csTmp)) {
		hv_resp_code.len = strlen(csTmp);
		memcpy(hv_resp_code.arr, csTmp, hv_resp_code.len);
		ind_resp_code= 0;
DEBUGLOG(("Add:resp_code= [%.*s]\n",hv_resp_code.len,hv_resp_code.arr));
	}

/*status*/
	if(GetField_Char(hPayoutRecord,"status",&cTmp)){
		hv_status = cTmp;
		ind_status = 0;
DEBUGLOG(("Add:status= [%c]\n",hv_status));
	...
	/* EXEC SQL EXECUTE
		BEGIN
			:hv_return_value := sp_payout_record_insert(
					...
					:hv_dest_currency:ind_dest_currency,
					:hv_checksum:ind_checksum,
					:hv_status:ind_status,
					:hv_resp_code:ind_resp_code);
		END;
	END-EXEC; */ 

{
 ...
 sqlstm.stmt = "begin :hv_return_value := sp_payout_record_insert ( :hv_bat\
ch_id:ind_batch_id , :hv_seq_num:ind_seq_num , :hv_merchant_id:ind_merchant_i\
d , :hv_merchant_ref:ind_merchant_ref , :hv_country:ind_country , :hv_identit\
y_id:ind_identity_id , :hv_account_num:ind_account_num , :hv_account_name:ind\
_account_name , :hv_bank_name:ind_bank_name , :hv_bank_code:ind_bank_code , :\
hv_branch:ind_branch , :hv_phone_num:ind_phone_num , :hv_province:ind_provinc\
e , :hv_city:ind_city , :hv_amount:ind_amount , :hv_payout_currency:ind_payou\
t_currency , :hv_dest_currency:ind_dest_currency , :hv_checksum:ind_checksum \
, :hv_status:ind_status , :hv_resp_code:ind_resp_code ) ; END ;";
 ...
 sqlstm.sqhstv[17] = (unsigned char  *)&hv_dest_currency;
 sqlstm.sqhstl[17] = (unsigned long )5;
 sqlstm.sqhsts[17] = (         int  )0;
 sqlstm.sqindv[17] = (         short *)&ind_dest_currency;
 sqlstm.sqinds[17] = (         int  )0;
 sqlstm.sqharm[17] = (unsigned long )0;
 sqlstm.sqadto[17] = (unsigned short )0;
 sqlstm.sqtdso[17] = (unsigned short )0;
 sqlstm.sqhstv[18] = (unsigned char  *)&hv_checksum;
 sqlstm.sqhstl[18] = (unsigned long )18;
 sqlstm.sqhsts[18] = (         int  )0;
 sqlstm.sqindv[18] = (         short *)&ind_checksum;
 sqlstm.sqinds[18] = (         int  )0;
 sqlstm.sqharm[18] = (unsigned long )0;
 sqlstm.sqadto[18] = (unsigned short )0;
 sqlstm.sqtdso[18] = (unsigned short )0;
 sqlstm.sqhstv[19] = (unsigned char  *)&hv_status;
 ...
 sqlstm.sqhstv[20] = (unsigned char  *)&hv_resp_code;
 sqlstm.sqhstl[20] = (unsigned long )8;
 sqlstm.sqhsts[20] = (         int  )0;
 sqlstm.sqindv[20] = (         short *)&ind_resp_code;
...
DEBUGLOG(("Add:Ret = [%d]\n",hv_return_value));
	...
	if (hv_return_value == SP_OTHER_ERR)  {
		ERRLOG("PayoutRecord_Add: SP_OTHER_ERR TxnAbort\n");
		DEBUGLOG(("Add: SP_OTHER_ERR TxnAbort\n"));
		return PD_OTHER_ERR;
	}

	if (hv_return_value == SP_ERR)  {
		ERRLOG("PayoutRecord_Add: SP_ERR TxnAbort\n");
...
DEBUGLOG(("Delete: batch_id = [%.*s]\n",hv_batch_id.len,hv_batch_id.arr));
	...
	hv_merchant_ref.len = strlen((const char*)merchant_ref);
	memcpy(hv_merchant_ref.arr,merchant_ref,hv_merchant_ref.len);
DEBUGLOG(("Delete: merchant_ref = [%.*s]\n",hv_merchant_ref.len,hv_merchant_ref.arr));

	/* EXEC SQL EXECUTE
		BEGIN

			:hv_return_value := sp_payout_record_delete(
				:hv_batch_id,
				:hv_merchant_ref);
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "begin :hv_return_value := sp_payout_record_delete ( :hv_bat\
ch_id , :hv_merchant_ref ) ; END ;";
 sqlstm.iters = (unsigned int  )1;
...
DEBUGLOG(("Delete:Ret = [%d]\n",hv_return_value));
	...
	if (hv_return_value == SP_OTHER_ERR)  {
		ERRLOG("PayoutRecord_Delete: SP_OTHER_ERR TxnAbort\n");
DEBUGLOG(("Delete: SP_OTHER_ERR TxnAbort\n"));
		return PD_OTHER_ERR;
	}

	if (hv_return_value == SP_ERR)  {
		ERRLOG("PayoutRecord_Delete: SP_ERR TxnAbort\n");
...
DEBUGLOG(("Update: Begin\n"));
        ...
        strcpy((char*)hv_dynstmt.arr,"update payout_record set ");
...
DEBUGLOG(("\n%.*s\n", sqlca.sqlerrm.sqlerrml, sqlca.sqlerrm.sqlerrmc));
ERRLOG("PayoutRecord_Update: SP_ERR TxnAbort\n");
        /* EXEC SQL WHENEVER SQLERROR CONTINUE; */ 
...
struct { unsigned short len; unsigned char arr[4]; } v_payout_currency;
		...
		/* varchar		v_dest_currency[PD_CCY_ID_LEN+1]; */ 
struct { unsigned short len; unsigned char arr[4]; } v_dest_currency;
...
DEBUGLOG(("GetPayoutRecord  = [%.*s]\n",hv_merchant_id.len,hv_merchant_id.arr));
	...
	/* EXEC SQL DECLARE c_cursor_getpayout CURSOR FOR
		...
		and	merchant_id =:hv_merchant_id; */ 
 ...
 if (sqlca.sqlcode < 0) goto getpayout_error;
	...
	do{
		...
		INTO
			...
			:v_dest_currency:ind_dest_currency,
...

{
  ...
  sqlstm.sqadto[12] = (unsigned short )0;
  sqlstm.sqtdso[12] = (unsigned short )0;
  sqlstm.sqhstv[13] = (unsigned char  *)&v_payout_currency;
  sqlstm.sqhstl[13] = (unsigned long )6;
  sqlstm.sqhsts[13] = (         int  )0;
  sqlstm.sqindv[13] = (         short *)&ind_payout_currency;
  sqlstm.sqinds[13] = (         int  )0;
  sqlstm.sqharm[13] = (unsigned long )0;
  sqlstm.sqadto[13] = (unsigned short )0;
  sqlstm.sqtdso[13] = (unsigned short )0;
  sqlstm.sqhstv[14] = (unsigned char  *)&v_dest_currency;
  sqlstm.sqhstl[14] = (unsigned long )6;
  sqlstm.sqhsts[14] = (         int  )0;
  sqlstm.sqindv[14] = (         short *)&ind_dest_currency;
  sqlstm.sqinds[14] = (         int  )0;
  sqlstm.sqharm[14] = (unsigned long )0;
  sqlstm.sqadto[14] = (unsigned short )0;
  sqlstm.sqtdso[14] = (unsigned short )0;
...
/*seq_num*/
	if(ind_seq_num>=0){
		v_seq_num.arr[v_seq_num.len]='\0';
		PutField_CString(myHash,"seq_num",(const char*)v_seq_num.arr);
DEBUGLOG(("GetPayoutRecord seq_num=[%s]\n",v_seq_num.arr));
	}
...
DEBUGLOG(("GetPayoutRecord payout_currency= [%s]\n",v_payout_currency.arr));
	}

/*dest_currency*/
	if(ind_dest_currency>=0){
		v_dest_currency.arr[v_dest_currency.len]='\0';
		PutField_CString(myHash,"dest_currency",(const char*)v_dest_currency.arr);
DEBUGLOG(("GetPayoutRecord dest_currency= [%s]\n",v_dest_currency.arr));
	}

/*checksum*/
	if(ind_checksum>=0){
		v_checksum.arr[v_checksum.len]='\0';
		PutField_CString(myHash,"checksum",(const char*)v_checksum.arr);
DEBUGLOG(("GetPayoutRecord checksum = [%s]\n",v_checksum.arr));
	}
...
DEBUGLOG(("GetPayoutRecord amount = [%s]\n",v_amount.arr));
	}
...
DEBUGLOG(("GetPayoutRecord Normal Exit\n"));
	return  PD_OK;

getpayout_error:
DEBUGLOG(("getpayout_error code %d\n", sqlca.sqlcode));
DEBUGLOG(("\n%.*s\n", sqlca.sqlerrm.sqlerrml, sqlca.sqlerrm.sqlerrmc));
ERRLOG("PayoutRecord_Get: SP_ERR\n");
	/* EXEC SQL WHENEVER SQLERROR CONTINUE; */ 
