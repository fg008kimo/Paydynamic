...
{
    20,
    "PregenDailyReport.pc"
};


static unsigned int sqlctx = 81810875;
...
{12,4130,871,0,0,
5,0,0,1,358,0,6,146,0,0,11,11,0,1,0,2,3,0,0,1,97,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,
9,0,0,1,9,0,0,1,9,0,0,1,1,0,0,1,3,0,0,1,9,0,0,
64,0,0,2,113,0,4,232,0,0,3,2,0,1,0,2,3,0,0,1,97,0,0,1,9,0,0,
91,0,0,3,232,0,4,332,0,0,7,6,0,1,0,2,3,0,0,1,97,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,
9,0,0,1,9,0,0,
134,0,0,4,0,0,17,480,0,0,1,1,0,1,0,1,9,0,0,
153,0,0,4,0,0,21,481,0,0,0,0,0,1,0,
168,0,0,4,0,0,17,534,0,0,1,1,0,1,0,1,9,0,0,
187,0,0,4,0,0,21,535,0,0,0,0,0,1,0,
};
...
Add ChkCthExist, Revise UpdateStatus		   2019/11/20    Michael Chow
*/
...

{
 ...
 sqlstm.stmt = "begin :hv_return_value := sp_pregen_daily_report_ins ( :hv_\
report_code:ind_report_code , :hv_merchant_id:ind_merchant_id , :hv_country:i\
nd_country , :hv_ccy:ind_ccy , :hv_service_code:ind_service_code , :hv_report\
_date:ind_report_date , :hv_filename:ind_filename , :hv_status:ind_status , :\
...
int ChkCthExist(const hash_t *hRls)
{
	char *csTmp;
	int  iRecCnt;
	
	/* EXEC SQL WHENEVER SQLERROR GOTO chkcthexist_error; */ 
...
DEBUGLOG(("ChkCthExist:Begin\n"));

	if(GetField_CString(hRls, "report_code", &csTmp)) {
		memcpy(hv_report_code, csTmp, strlen(csTmp));
		hv_report_code[PD_REPORT_CODE_LEN+1] = '\0';
		ind_report_code = 0;
DEBUGLOG(("ChkCthExist:report_code = [%.*s]\n",strlen(hv_report_code), hv_report_code));
	}
	...
	if(GetField_CString(hRls,"report_date",&csTmp))
	{
		...
		strncpy((char *)hv_report_date.arr, csTmp, hv_report_date.len);
		ind_report_date = 0;
DEBUGLOG(("ChkCthExist:report_date = [%.*s]\n",hv_report_date.len,hv_report_date.arr));
	}
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "select COUNT ( 1 ) INTO :b0 FROM PREGEN_DAILY_REPORT WHERE \
PDR_REPORT_CODE = :b1:b2 AND PDR_REPORT_DATE = :b3:b4 ";
 sqlstm.iters = (unsigned int  )1;
 ...
 sqlstm.sqhsts[2] = (         int  )0;
 sqlstm.sqindv[2] = (         short *)&ind_report_date;
 sqlstm.sqinds[2] = (         int  )0;
 sqlstm.sqharm[2] = (unsigned long )0;
 sqlstm.sqadto[2] = (unsigned short )0;
 sqlstm.sqtdso[2] = (unsigned short )0;
 ...
 sqlstm.sqptdso = sqlstm.sqtdso;
 sqlcxt((void **)0, &sqlctx, &sqlstm, &sqlfpn);
 if (sqlca.sqlcode < 0) goto chkcthexist_error;
}



	iRecCnt = v_rec_cnt;
DEBUGLOG(("ChkCthExist:rec_cnt = [%d]\n", v_rec_cnt));

DEBUGLOG(("ChkCthExist:Normal Exit\n"));
		return iRecCnt;
	
chkcthexist_error:
DEBUGLOG(("chkcthexist_error code %d\n", sqlca.sqlcode));
DEBUGLOG(("\n%.*s\n", sqlca.sqlerrm.sqlerrml, sqlca.sqlerrm.sqlerrmc));
ERRLOG("PregenDailyReport_ChkCthExist: SP_INTERNAL_ERR TxnAbort\n");
	/* EXEC SQL WHENEVER SQLERROR CONTINUE; */ 
...

{
 ...
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "select COUNT ( 1 ) INTO :b0 FROM PREGEN_DAILY_REPORT WHERE \
PDR_REPORT_CODE = :b1:b2 AND PDR_MERCHANT_ID = :b3:b4 AND PDR_CURRENCY_ID = :\
b5:b6 AND PDR_COUNTRY_ID = :b7:b8 AND PDR_SERVICE_CODE = :b9:b10 AND PDR_REPO\
RT_DATE = :b11:b12 ";
 sqlstm.iters = (unsigned int  )1;
