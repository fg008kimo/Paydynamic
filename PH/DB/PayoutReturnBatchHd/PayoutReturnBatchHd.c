...
{
    22,
    "PayoutReturnBatchHd.pc"
};


static unsigned int sqlctx = 325118203;
...
extern void sqliem( unsigned char *, signed int * );
 ...
 static char *sq0003 = 
"select prh_batch_id , prh_filename , prh_record_count , prh_status from pay\
out_return_batch_hd where prh_batch_id = :b0 for update ";
...
{12,4130,871,0,0,
5,0,0,1,209,0,6,98,0,0,6,6,0,1,0,2,3,0,0,1,3,0,0,1,9,0,0,1,3,0,0,1,3,0,0,1,9,0,
0,
44,0,0,2,0,0,17,216,0,0,1,1,0,1,0,1,9,0,0,
63,0,0,2,0,0,21,217,0,0,0,0,0,1,0,
78,0,0,3,131,0,9,267,0,2049,1,1,0,1,0,1,3,0,0,
97,0,0,3,0,0,13,269,0,0,4,0,0,1,0,2,3,0,0,2,9,0,0,2,3,0,0,2,3,0,0,
128,0,0,3,0,0,15,307,0,0,0,0,0,1,0,
143,0,0,3,0,0,15,317,0,0,0,0,0,1,0,
158,0,0,4,68,0,4,337,0,0,1,0,0,1,0,2,68,0,0,
};
...
-------------------------------                    ------------            --------------
Init Version                                       2014/07/22              Dirk Wong
*/
...
void PayoutReturnBatchHd(char cdebug)
{
        cDebug = cdebug;
}
...
DEBUGLOG(("Add:record_count = [%d]\n",hv_record_count));
		...
		hv_record_count = 0;
		ind_record_count = 0;
DEBUGLOG(("Add:record_count = 0 (Default)\n"));
	}
...
DEBUGLOG(("Add:add_user = [%.*s]\n",hv_add_user.len,hv_add_user.arr));
	...
	/* EXEC SQL EXECUTE
		BEGIN
			:hv_return_value := sp_payout_ret_batch_hd_insert(
...

{
 ...
 sqlstm.stmt = "begin :hv_return_value := sp_payout_ret_batch_hd_insert ( :\
hv_batch_id:ind_batch_id , :hv_filename:ind_filename , :hv_record_count:ind_r\
ecord_count , :hv_status:ind_status , :hv_add_user:ind_add_user ) ; END ;";
...
DEBUGLOG(("Add:Ret = [%d]\n",hv_return_value));
	...
	if (hv_return_value == SP_OTHER_ERR)  {
		ERRLOG("PayoutReturnBatchHd_Add: SP_OTHER_ERR TxnAbort\n");
		DEBUGLOG(("Add: SP_OTHER_ERR TxnAbort\n"));
		return PD_OTHER_ERR;
	}

	if (hv_return_value == SP_ERR)  {
		ERRLOG("PayoutReturnBatchHd_Add: SP_ERR TxnAbort\n");
...
ERRLOG("PayoutReturnBatchHd_Add: SP_INTERNAL_ERR TxnAbort\n");
...
DEBUGLOG(("Update: Begin\n"));
        strcpy((char*)hv_dynstmt.arr,"update payout_return_batch_hd set prh_update_timestamp=sysdate");
        hv_dynstmt.len = strlen((const char*)hv_dynstmt.arr);

/*KEY: batch_id*/
	GetField_Int(hRls,"batch_id",&iBatchId);
	sprintf(csBatchId,"%d",iBatchId);
DEBUGLOG(("Update:batch_id = [%d]\n",iBatchId));
...
/*UPDATE: record_count*/
	if(GetField_Int(hRls,"record_count",&iRecordCount)){
		...
		hv_dynstmt.len = strlen((const char*)hv_dynstmt.arr);
DEBUGLOG(("Update:record_count = [%d]\n",iRecordCount));
	}
...
DEBUGLOG(("\n%.*s\n", sqlca.sqlerrm.sqlerrml, sqlca.sqlerrm.sqlerrmc));
ERRLOG("PayoutReturnBatchHd_Update: SP_INTERNAL_ERR\n");
        /* EXEC SQL WHENEVER SQLERROR CONTINUE; */ 
...
//DEBUGLOG(("GetTxnHeader status = [%c]\n",hv_status));
	...
	/* EXEC SQL DECLARE c_cursor_getpayout CURSOR FOR
		select	prh_batch_id,
			...
			prh_record_count,
			prh_status
		from	payout_return_batch_hd
		where	prh_batch_id=:hv_batch_id
		for update; */ 
 ...
 if (sqlca.sqlcode < 0) goto getpayout_error;
	...
	do{
		...
		INTO
			...
			:v_record_count:ind_record_count,
			:v_status:ind_status; */ 

{
  ...
  sqlstm.sqadto[0] = (unsigned short )0;
  sqlstm.sqtdso[0] = (unsigned short )0;
  sqlstm.sqhstv[1] = (unsigned char  *)&v_filename;
  ...
  sqlstm.sqadto[1] = (unsigned short )0;
  sqlstm.sqtdso[1] = (unsigned short )0;
  sqlstm.sqhstv[2] = (unsigned char  *)&v_record_count;
  sqlstm.sqhstl[2] = (unsigned long )sizeof(int);
  sqlstm.sqhsts[2] = (         int  )0;
  sqlstm.sqindv[2] = (         short *)&ind_record_count;
  sqlstm.sqinds[2] = (         int  )0;
  sqlstm.sqharm[2] = (unsigned long )0;
  sqlstm.sqadto[2] = (unsigned short )0;
  sqlstm.sqtdso[2] = (unsigned short )0;
...
DEBUGLOG(("GetPayoutReturnBatchHd filename = [%s]\n",v_filename.arr));
		}

//record count
		if(ind_record_count>=0){
			PutField_Int(myHash,"record_count",v_record_count);
DEBUGLOG(("GetPayoutReturnBatchHd record_count=[%d]\n",v_record_count));
		}

//status
		if(ind_status>=0){
			PutField_Int(myHash,"status",v_status);
DEBUGLOG(("GetPayoutReturnBatchHd status=[%d]\n",v_status));
		}
...
//DEBUGLOG(("GetPayoutReturnBatchHd Normal Exit\n"));
	return  PD_OK;

getpayout_error:
DEBUGLOG(("getpayout_error code %d\n", sqlca.sqlcode));
DEBUGLOG(("\n%.*s\n", sqlca.sqlerrm.sqlerrml, sqlca.sqlerrm.sqlerrmc));
ERRLOG("PayoutReturnBatchHd_Get: SP_INTERNAL_ERR\n");
	/* EXEC SQL WHENEVER SQLERROR CONTINUE; */ 
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "select max ( prh_batch_id ) INTO :b0:b1 FROM payout_return_\
batch_hd ";
 sqlstm.iters = (unsigned int  )1;
