DROP VIEW PAYOUT_FILE_SUMMARY_VIEW;

CREATE OR REPLACE FORCE VIEW PAYOUT_FILE_SUMMARY_VIEW
(
   UH_BATCH_ID,
   UH_MERCHANT_ID,
   UH_SERVICE_CODE,
...
AS
     ...
     SELECT uh_batch_id,
            ...
            NVL(time.timestamp, merchant_upload_file_header.uh_update_timestamp)  AS update_timestamp
       FROM merchant_upload_file_header,
            (  SELECT ud_batch_id, MAX (ud_update_timestamp) AS timestamp
