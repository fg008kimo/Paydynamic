...
static unsigned int sqlctx = 9064163;
...
extern void sqliem( unsigned char *, signed int * );
 ...
 static char *sq0001 = 
"select max ( at_code_in_num ) from adjustment_type where at_party_type = :b\
0            ";

 static char *sq0003 = 
"select at_dc_ind , at_desc , at_disabled , at_allow_modify , at_allow_bal_n\
egative from adjustment_type where at_party_type = :b0 and at_code = :b1     \
       ";
...
{12,4130,871,0,0,
5,0,0,1,88,0,9,59,0,2049,1,1,0,1,0,1,1,0,0,
24,0,0,1,0,0,13,61,0,0,1,0,0,1,0,2,3,0,0,
43,0,0,1,0,0,15,75,0,0,0,0,0,1,0,
58,0,0,1,0,0,15,84,0,0,0,0,0,1,0,
73,0,0,2,341,0,6,193,0,0,11,11,0,1,0,2,3,0,0,1,1,0,0,1,9,0,0,1,3,0,0,1,1,0,0,1,
9,0,0,1,3,0,0,1,3,0,0,1,3,0,0,1,9,0,0,1,1,0,0,
132,0,0,3,159,0,9,291,0,2049,2,2,0,1,0,1,1,0,0,1,9,0,0,
155,0,0,3,0,0,13,293,0,0,5,0,0,1,0,2,1,0,0,2,9,0,0,2,3,0,0,2,3,0,0,2,3,0,0,
190,0,0,3,0,0,15,345,0,0,0,0,0,1,0,
205,0,0,3,0,0,15,361,0,0,0,0,0,1,0,
220,0,0,4,283,0,6,458,0,0,9,9,0,1,0,2,3,0,0,1,1,0,0,1,9,0,0,1,1,0,0,1,9,0,0,1,
3,0,0,1,3,0,0,1,3,0,0,1,9,0,0,
271,0,0,5,89,0,6,530,0,0,3,3,0,1,0,2,3,0,0,1,1,0,0,1,9,0,0,
};
...
Handle Dual Control Flag			   2012/03/30		   Virginia Yun	
Modify GetAdjustmentTypeRec			   2013/02/22		   David Wong
Add Adjustment Nature				   2014/06/30		   Dirk Wong
*/
...
struct { unsigned short len; unsigned char arr[20]; } hv_create_user;
		...
		short		ind_dual_control = -1;
		short		ind_fee_type = -1;
		short		ind_nature = -1;
...
DEBUGLOG(("Add:disabled = [%d]\n",hv_disabled));
	...
	if(GetField_Int(hRls, "dual_control", &iTmp)) {
		hv_dual_control = iTmp;
		ind_dual_control = 0;
DEBUGLOG(("Add:dual_control = [%d]\n",hv_dual_control));
	}

	if(GetField_Int(hRls, "fee_type", &iTmp)) {
		hv_fee_type = iTmp;
		ind_fee_type = 0;
DEBUGLOG(("Add:fee_type = [%d]\n",hv_fee_type));
	}
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "begin :hv_return_value := sp_adjustment_type_insert ( :hv_p\
arty_type:ind_party_type , :hv_code:ind_code , :hv_code_in_num:ind_code_in_nu\
m , :hv_dc_ind:ind_dc_ind , :hv_desc:ind_desc , :hv_dual_control:ind_dual_con\
trol , :hv_fee_type:ind_fee_type , :hv_disabled:ind_disabled , :hv_create_use\
r:ind_create_user , :hv_nature:ind_nature ) ; END ;";
 sqlstm.iters = (unsigned int  )1;
 ...
 sqlstm.sqhstv[6] = (unsigned char  *)&hv_dual_control;
 sqlstm.sqhstl[6] = (unsigned long )sizeof(int);
 sqlstm.sqhsts[6] = (         int  )0;
 sqlstm.sqindv[6] = (         short *)&ind_dual_control;
 sqlstm.sqinds[6] = (         int  )0;
 sqlstm.sqharm[6] = (unsigned long )0;
 sqlstm.sqadto[6] = (unsigned short )0;
 sqlstm.sqtdso[6] = (unsigned short )0;
 sqlstm.sqhstv[7] = (unsigned char  *)&hv_fee_type;
 sqlstm.sqhstl[7] = (unsigned long )sizeof(int);
 sqlstm.sqhsts[7] = (         int  )0;
 sqlstm.sqindv[7] = (         short *)&ind_fee_type;
 ...
 sqlstm.sqhstv[10] = (unsigned char  *)&hv_nature;
 sqlstm.sqhstl[10] = (unsigned long )1;
 sqlstm.sqhsts[10] = (         int  )0;
 sqlstm.sqindv[10] = (         short *)&ind_nature;
 ...
 if (sqlca.sqlcode < 0) goto add_error;
        ...
        if (hv_return_value == SP_OTHER_ERR)  {
                ERRLOG("AdjustmentType_Add: SP_OTHER_ERR TxnAbort\n");
                DEBUGLOG(("Add: SP_OTHER_ERR TxnAbort\n"));
                return PD_OTHER_ERR;
        }

        if (hv_return_value == SP_ERR)  {
                ERRLOG("AdjustmentType_Add: SP_ERR TxnAbort\n");
...
DEBUGLOG(("Update:Adj Type: update_user = [%.*s]\n",hv_update_user.len, hv_update_user.arr));
		...
		hv_dual_control = iTmp;
		ind_dual_control = 0;
DEBUGLOG(("Update Adj Type: dual_control = [%d]\n", hv_dual_control));
        }

        if (GetField_Int(hRls,"fee_type",&iTmp)) {
		hv_fee_type = iTmp;
		ind_fee_type = 0;
DEBUGLOG(("Update Adj Type: fee_type = [%d]\n", hv_fee_type));
        }


	/* EXEC SQL EXECUTE
		BEGIN
			:hv_return_value := sp_adjustment_type_update(
						:hv_party_type:ind_party_type,
						:hv_code:ind_code,
						:hv_dc_ind:ind_dc_ind,
						:hv_desc:ind_desc,
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "begin :hv_return_value := sp_adjustment_type_update ( :hv_p\
arty_type:ind_party_type , :hv_code:ind_code , :hv_dc_ind:ind_dc_ind , :hv_de\
sc:ind_desc , :hv_dual_control:ind_dual_control , :hv_fee_type:ind_fee_type ,\
 :hv_disabled:ind_disabled , :hv_update_user:ind_update_user ) ; END ;";
 sqlstm.iters = (unsigned int  )1;
 ...
 sqlstm.sqadto[4] = (unsigned short )0;
 sqlstm.sqtdso[4] = (unsigned short )0;
 sqlstm.sqhstv[5] = (unsigned char  *)&hv_dual_control;
 sqlstm.sqhstl[5] = (unsigned long )sizeof(int);
 sqlstm.sqhsts[5] = (         int  )0;
 sqlstm.sqindv[5] = (         short *)&ind_dual_control;
 sqlstm.sqinds[5] = (         int  )0;
 sqlstm.sqharm[5] = (unsigned long )0;
 sqlstm.sqadto[5] = (unsigned short )0;
 sqlstm.sqtdso[5] = (unsigned short )0;
 sqlstm.sqhstv[6] = (unsigned char  *)&hv_fee_type;
 sqlstm.sqhstl[6] = (unsigned long )sizeof(int);
 sqlstm.sqhsts[6] = (         int  )0;
 sqlstm.sqindv[6] = (         short *)&ind_fee_type;
 sqlstm.sqinds[6] = (         int  )0;
 sqlstm.sqharm[6] = (unsigned long )0;
 sqlstm.sqadto[6] = (unsigned short )0;
 sqlstm.sqtdso[6] = (unsigned short )0;
...
DEBUGLOG(("Update Adj Type Normal Exit\n"));
	...
	if (hv_return_value == SP_ERR) {
ERRLOG("AdjustmentType_Update: SP_ERR TxnAbort\n");
DEBUGLOG(("Update: SP_ERR TxnAbort\n"));
		return PD_ERR;
	}

	if (hv_return_value == SP_OTHER_ERR) {
ERRLOG("AdjustmentType_Update: SP_OTHER_ERR TxnAbort\n");
...
ERRLOG("AdjustmentType_Update: SP_INTERNAL_ERR TxnAbort\n");
...
DEBUGLOG(("Delete:code = [%.*s]\n",hv_code.len,hv_code.arr));
	...
	/* EXEC SQL EXECUTE
		BEGIN
			:hv_return_value := sp_adjustment_type_delete(	
						:hv_party_type,
						:hv_code
						);
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "begin :hv_return_value := sp_adjustment_type_delete ( :hv_p\
arty_type , :hv_code ) ; END ;";
 sqlstm.iters = (unsigned int  )1;
