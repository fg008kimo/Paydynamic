CREATE OR REPLACE FUNCTION SP_HIST_MID_BAL_COLLECT(
    IN_END_DATE IN DATE )
RETURN NUMBER IS
	EXP_ERR  EXCEPTION;
	EXP_ERR2 EXCEPTION;
	...
	V_NEW_BATCH_ID HIST_MID_BAL_BATCH.HB_BATCH_ID%type;
	V_PREV_BATCH_ID HIST_MID_BAL_BATCH.HB_BATCH_ID%type;
	V_START_DATE         DATE;
	V_ADD_RANGE_END_DATE DATE;

	V_CURRENT_BAL TXN_DETAIL.TD_CURRENT_BAL%type;
	V_HOLD_BAL TXN_DETAIL.TD_TOTAL_RESERVED_AMOUNT%type;
	V_LIEN TXN_DETAIL.TD_TOTAL_HOLD%type;
	V_MERCHANT_BAL TXN_DETAIL.TD_CURRENT_BAL%type;

	V_CURRENT_BAL_PREV TXN_DETAIL.TD_CURRENT_BAL%type;
	V_HOLD_BAL_PREV TXN_DETAIL.TD_TOTAL_RESERVED_AMOUNT%type;
	V_LIEN_PREV TXN_DETAIL.TD_TOTAL_HOLD%type;
	V_MERCHANT_BAL_PREV TXN_DETAIL.TD_CURRENT_BAL%type;

	V_HD_CURRENT_BAL HIST_MID_BAL_DETAIL.HD_CURRENT_BAL%type;
	V_HD_HOLD_BAL HIST_MID_BAL_DETAIL.HD_HOLD_BAL%type;
	V_HD_LIEN HIST_MID_BAL_DETAIL.HD_LIEN%type;
	V_HD_MERCHANT_BAL HIST_MID_BAL_DETAIL.HD_MERCHANT_BAL%type;

	V_NET_BAL HIST_MID_BAL_DETAIL.HD_BAL%type;
	V_HD_BAL HIST_MID_BAL_DETAIL.HD_BAL%type;
	V_HD_BAL_HKD HIST_MID_BAL_DETAIL.HD_BAL_HKD%type;
	V_HD_BAL_USD HIST_MID_BAL_DETAIL.HD_BAL_USD%type;
	V_HD_TXN_ID HIST_MID_BAL_DETAIL.HD_TXN_ID%type;
...
BEGIN
-- DBMS_OUTPUT.PUT_LINE('>>> START [HIST_MID_BAL_COLLECT] <<<');
...
-- Find PSP Last Txn from start times to end times
				...
				SELECT
			      	...
			      	FROM (
			      		SELECT
			      		      ...
			      		      ROW_NUMBER() OVER ( PARTITION BY CLIENT_ID , TH_MERCHANT_ID , TD_TXN_COUNTRY , TH_SERVICE_CODE , TH_NET_CCY ORDER BY TH_APPROVAL_TIMESTAMP DESC ) RN
			      		FROM TXN_DETAIL ,
			      		     TXN_HEADER ,
			      		     MERCH_DETAIL
			      		...
			      		AND TH_SERVICE_CODE IS NOT NULL
			      		AND TD_CURRENT_BAL IS NOT NULL
			      		AND TH_NET_CCY IS NOT NULL
			      		AND TD_TXN_COUNTRY IS NOT NULL
	...
	LOOP
-- DBMS_OUTPUT.PUT_LINE('--> CLIENT_ID: [' || X.CLIENT_ID || '] MID_ID: [' || X.MID_ID || '] SERVICE_CODE: [' || X.SERVICE_CODE || ']  TXN_CCY: [' || X.TXN_CCY || '] TXN_ID : [' || X.TXN_ID||'] COUNTRY : [' || X.COUNTRY||'] ');
...
--  Txn Bal
		SELECT
			NVL(
				NVL(TD_CURRENT_BAL, 0.00)
				+ NVL(TD_CURRENT_BAL_SETTLEMENT, 0.00)
				- NVL(TD_TOTAL_RESERVED_AMOUNT, 0.00)
				- NVL(TD_TOTAL_HOLD, 0.00)
				- NVL(TD_TOTAL_HOLD_SETTLEMENT, 0.00)
				, 0.00 
			) CURRENT_BAL,
			NVL(TD_TOTAL_RESERVED_AMOUNT, 0.00) HOLD_BAL ,
			NVL(TD_TOTAL_HOLD, 0.00) + NVL(TD_TOTAL_HOLD_SETTLEMENT, 0.00) LIEN ,
			NVL(TD_CURRENT_BAL, 0.00) + NVL(TD_CURRENT_BAL_SETTLEMENT, 0.00) MERCHANT_BAL
		 INTO
			V_CURRENT_BAL ,
			V_HOLD_BAL ,
			V_LIEN ,
...
-- Check Cut off Time
		IF V_CUTOFF_DATE >= V_ADD_RANGE_END_DATE THEN
			...
			V_MERCHANT_BAL_PREV := 0.00;
		END IF;
-- DBMS_OUTPUT.PUT_LINE('--> V_NEW_BATCH_ID: [' || V_NEW_BATCH_ID
--							|| '] MID_ID: [' || X.MID_ID
--							||'] SERVICE_CODE: [' || X.SERVICE_CODE
--							||'] TXN_CCY: [' || X.TXN_CCY 
--							||'] COUNTRY: [' || X.COUNTRY ||']'); 
	   
-- DBMS_OUTPUT.PUT_LINE('--> V_HD_CURRENT_BAL: [' || NVL(V_HD_CURRENT_BAL + (V_CURRENT_BAL - V_CURRENT_BAL_PREV) ,0.00) 
--							|| '] V_HD_HOLD_BAL: [' || NVL(V_HD_HOLD_BAL + (V_HOLD_BAL - V_HOLD_BAL_PREV) ,0.00)
--							||'] V_HD_LIEN: [' || NVL(V_HD_LIEN + (V_LIEN - V_LIEN_PREV) ,0.00) 
--							||'] V_HD_MERCHANT_BAL: [' || NVL(V_HD_MERCHANT_BAL + (V_MERCHANT_BAL - V_MERCHANT_BAL_PREV) ,0.00) ||']');
  
		INSERT INTO HIST_MID_BAL_DETAIL (
			HD_BATCH_ID,
			HD_CLIENT_ID,
			HD_MID_ID,
			HD_SERVICE_CODE,
...
-- DBMS_OUTPUT.PUT_LINE('--> END INSERT ');
  ...
  COMMIT;
-- DBMS_OUTPUT.PUT_LINE('>>> END [HIST_MID_BAL_COLLECT] <<<');
