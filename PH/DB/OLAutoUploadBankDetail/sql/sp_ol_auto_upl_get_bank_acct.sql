...
BEGIN
	...
	and	oauas_acct_type = in_acct_type
	and	oauas_chk_status = 1
	/*
       	and   	((ob_acct_type = 'DSI' and (internal_bank_code in (select olfk_int_bank_code
								   from ol_stmt_format_keywords
								   where olfk_format_type = 'TXN_CODE_DSI')))
		 or (ob_acct_type = 'POA' and (internal_bank_code in (select olfk_int_bank_code
                                                                   from ol_stmt_format_keywords
                                                                   where olfk_format_type = 'TXN_CODE_POA')))
		 or (ob_acct_type = 'ITM' and (internal_bank_code in (select olfk_int_bank_code
                                                                   from ol_stmt_format_keywords
                                                                   where olfk_format_type = 'TXN_CODE_ITM')))
		)
	*/
      	/*and   ob_status_type in ('A', 'B', 'F', 'S', 'SS', 'NF', 'ND')*/
      	/*and   ob_status_type in ('A')*/
	and     ob_acct_type = oauas_acct_type
	and	ob_status_type = oauas_status_type
