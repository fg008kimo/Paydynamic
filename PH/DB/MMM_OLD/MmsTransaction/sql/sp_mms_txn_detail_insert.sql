create or replace function sp_mms_txn_detail_insert(
       ...
       in_stl_bank_id                           mms_txn_detail.mtd_stl_bank_id%type,
       in_party_node_id                         mms_txn_detail.mtd_party_node_id%type,
       in_service_code                          mms_txn_detail.mtd_service_code%type,
       in_src_party_type                        mms_txn_detail.mtd_src_party_type%type,
       in_dst_party_type                        mms_txn_detail.mtd_dst_party_type%type,
       in_carry_forward_ind                     mms_txn_detail.mtd_carry_forward_ind%type,
       in_dc_type                               mms_txn_detail.mtd_dc_type%type,
       ...
       in_prev_dtl_txn_id                       mms_txn_detail.mtd_prev_dtl_txn_id%type,
       in_batch_id                              mms_txn_detail.mtd_batch_id%type,
       in_create_user				mms_txn_detail.mtd_create_user%type,
       in_update_user				mms_txn_detail.mtd_update_user%type,
       out_dtl_txn_id		OUT		mms_txn_detail.mtd_dtl_txn_id%type
)
  RETURN NUMBER IS
...
Begin
	...
	select nvl(max(mtd_dtl_txn_id), 0)
	into   iCnt
	from   mms_txn_detail
