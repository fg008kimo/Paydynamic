CREATE OR REPLACE FUNCTION SP_HIST_BAID_BAL_COLLECT(
	IN_END_DATE IN DATE )
RETURN NUMBER IS
	EXP_ERR  EXCEPTION;
	EXP_ERR2 EXCEPTION;
	...
	V_NEW_BATCH_ID 		HIST_BAID_BAL_BATCH.HB_BATCH_ID%type;
	V_PREV_BATCH_ID		HIST_BAID_BAL_BATCH.HB_BATCH_ID%type;
	V_START_DATE         DATE;
	V_ADD_RANGE_END_DATE DATE;

	V_AVA_BALANCE 		OL_TXN_PSP_DETAIL.OTP_BAL%type;
	V_IN_TRANSIT 		OL_TXN_PSP_DETAIL.OTP_IN_TRANSIT%type;
	V_LIEN 				OL_TXN_PSP_DETAIL.OTP_TOTAL_HOLD%type;

	V_AVA_BALANCE_PREV 	OL_TXN_PSP_DETAIL.OTP_BAL%type;
	V_IN_TRANSIT_PREV 	OL_TXN_PSP_DETAIL.OTP_IN_TRANSIT%type;
	V_LIEN_PREV 		OL_TXN_PSP_DETAIL.OTP_TOTAL_HOLD%type;

	V_HD_AVAILABLE_BAL 	HIST_BAID_BAL_DETAIL.HD_AVAILABLE_BAL%type;
	V_HD_IN_TRANSIT		HIST_BAID_BAL_DETAIL.HD_IN_TRANSIT%type;
	V_HD_LIEN 			HIST_BAID_BAL_DETAIL.HD_LIEN%type;

	V_NET_BAL 			HIST_BAID_BAL_DETAIL.HD_BAL%type;
	V_HD_BAL 			HIST_BAID_BAL_DETAIL.HD_BAL%type;
	V_HD_BAL_HKD 		HIST_BAID_BAL_DETAIL.HD_BAL_HKD%type;
	V_HD_BAL_USD		HIST_BAID_BAL_DETAIL.HD_BAL_USD%type;
	V_HD_TXN_ID			HIST_BAID_BAL_DETAIL.HD_TXN_ID%type;
...
BEGIN
-- DBMS_OUTPUT.PUT_LINE('>>> START [HIST_BAID_BAL_COLLECT] <<<');
...
-- Find PSP Last Txn from start times to end times
	...
	FROM (
		SELECT
			...
			HD_CAT_DESC ,
			ACCT_TYPE ,
			ROW_NUMBER() OVER ( PARTITION BY PID_ID , BAID, TXN_CCY ORDER BY APPROVAL_TIMESTAMP DESC ) RN
		FROM (
			SELECT
				TXN_ID ,
				PID_ID ,
			...
			FROM (
				SELECT
					...
					OTP_PSP_ID	AS PID_ID ,
					OTP_BAID 	AS BAID,
					OTP_TXN_CCY	AS TXN_CCY ,
					OTH_APPROVAL_TIMESTAMP AS APPROVAL_TIMESTAMP ,
					BC_IN_BALANCE AS IN_BALANCE ,
					BC_TYPE AS HD_CAT ,
					BC_DESC AS HD_CAT_DESC ,
					OPD_BANK_ACCT_TYPE AS ACCT_TYPE ,
					ROW_NUMBER() OVER ( PARTITION BY OTP_PSP_ID, OTP_BAID, OTP_TXN_CCY ORDER BY OTH_APPROVAL_TIMESTAMP DESC ) RN
				FROM
					OL_TXN_PSP_DETAIL ,
					OL_TXN_HEADER,
					OL_BANK_ACCT_ID,
				...
				AND OTH_TXN_CODE <> 'PDO'
				--AND BC_IN_BALANCE	= 1
				AND OTH_VERSION_NO >= 2
				AND OTP_TXN_CCY IS NOT NULL
				AND OTP_PSP_ID IS NOT NULL
				AND OTP_BAID IS NOT NULL
				AND OTP_BAL IS NOT NULL
			...
			SELECT
					...
					HD_CCY AS TXN_CCY ,
					OTH_APPROVAL_TIMESTAMP AS APPROVAL_TIMESTAMP,
					HD_IN_BALANCE AS IN_BALANCE ,
					HD_CAT AS HD_CAT ,
					HD_CAT_DESC	AS HD_CAT_DESC ,
					HD_ACCT_TYPE AS ACCT_TYPE
			FROM
					HIST_BAID_BAL_DETAIL ,
					OL_TXN_HEADER
...
--  Txn Bal
		...
		SELECT
			NVL(OTP_BAL, 0.00) - NVL(OTP_TOTAL_HOLD, 0.00) AVA_BALANCE
			,NVL(OTP_IN_TRANSIT,0.00) TOTAL_FLOAT
			,NVL(OTP_TOTAL_HOLD, 0.00) LIEN
		INTO
			V_AVA_BALANCE ,
			V_IN_TRANSIT ,
			V_LIEN
		FROM  OL_TXN_PSP_DETAIL
		WHERE OTP_TXN_ID  = X.TXN_ID;

-- DBMS_OUTPUT.PUT_LINE('--> V_AVA_BALANCE: [' || V_AVA_BALANCE || '] V_IN_TRANSIT: [' || V_IN_TRANSIT ||'] V_LIEN: [' || V_LIEN ||'] ');
...
-- Prev Batch Bal
			SELECT
				...
				NVL(HD_IN_TRANSIT, 0.00) AS HD_IN_TRANSIT ,
			...
			IF V_HD_TXN_ID IS NOT NULL THEN
				SELECT
				 ...
				 INTO
					...
					V_LIEN_PREV
				FROM  OL_TXN_PSP_DETAIL
				WHERE OTP_TXN_ID = V_HD_TXN_ID;
			ELSE
				V_AVA_BALANCE_PREV := 0.00;
				V_IN_TRANSIT_PREV := 0.00;
				V_LIEN_PREV := 0.00;
			END IF ;
...
-- DBMS_OUTPUT.PUT_LINE('--> V_AVA_BALANCE_PREV: [' || V_AVA_BALANCE_PREV || '] V_IN_TRANSIT_PREV: [' || V_IN_TRANSIT_PREV ||'] V_LIEN_PREV: [' || V_LIEN_PREV ||'] ');
...
-- HKD Rate , USD Rate
		...
		IF X.TXN_ID != V_HD_TXN_ID THEN
			V_NET_BAL := (V_AVA_BALANCE - V_AVA_BALANCE_PREV) + (V_IN_TRANSIT - V_IN_TRANSIT_PREV) + (V_LIEN - V_LIEN_PREV) ;
		ELSE
			V_NET_BAL := 0.00;
		END IF ;
...
-- Check Cut off Time
		...
		INSERT INTO HIST_BAID_BAL_DETAIL (
			HD_BATCH_ID,
			HD_PID_ID,
			HD_BAID,
			HD_CCY,
			...
			NVL(V_HD_IN_TRANSIT + (V_IN_TRANSIT - V_IN_TRANSIT_PREV), 0.00),
...
-- DBMS_OUTPUT.PUT_LINE('--> END INSERT ');
	...
	RETURN 0;
-- DBMS_OUTPUT.PUT_LINE('>>> END [HIST_BAID_BAL_COLLECT] <<<');
