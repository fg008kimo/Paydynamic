CREATE OR REPLACE FUNCTION sp_ol_statement_detail_insert(
	...
	in_process_group	ol_statement_detail.olsd_process_group%type,
  ...
  RETURN NUMBER IS
	statement_ref	ol_statement_detail.olsd_statement_ref%type;
BEGIN
	select	'S' || to_char(STMT_SEQ.NEXTVAL, 'FM099999999999999')
	into	statement_ref
	from	dual;
	...
	VALUES (	
		...
		case when in_statement_seq is null
			then (select NVL(MAX(olsd_statement_seq),0)+1 from ol_statement_detail where olsd_filename = in_filename and olsd_bank_acct_num = in_bank_acct_num)
