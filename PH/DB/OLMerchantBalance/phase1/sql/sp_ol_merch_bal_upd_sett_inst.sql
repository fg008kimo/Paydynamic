CREATE OR REPLACE FUNCTION sp_ol_merch_bal_upd_sett_inst(
	...
	in_bal			ol_merchant_balance.om_current_bal_settlement%type,
	in_create_user		ol_merchant_balance.om_create_user%type
)
return number is
begin
	update ol_merchant_balance
	  set om_settlement_in_transit = om_settlement_in_transit + in_bal,
...
exception
  when others then
  return 9;
end sp_ol_merch_bal_upd_sett_inst;
/
