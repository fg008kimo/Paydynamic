CREATE OR REPLACE FUNCTION sp_ol_merch_balance_AF_2_SFL(
	...
	in_float		ol_merchant_balance.om_current_bal%type,
	in_create_user		ol_merchant_balance.om_create_user%type
)
return number is
begin
	update ol_merchant_balance
	  set om_current_bal = om_current_bal - in_float,
              om_total_float_after_payout = om_total_float_after_payout - in_float,	 
	      om_current_bal_settlement = om_current_bal_settlement + in_float,
              om_total_float_settlement = om_total_float_settlement + in_float,
...
exception
  when others then
  return 9;
end sp_ol_merch_balance_AF_2_SFL;
/
