CREATE OR REPLACE FUNCTION sp_merchant_balance_sett_fl_in(
	...
	in_float		merchant_balance.m_total_float_settlement%type,
	in_create_user		merchant_balance.m_create_user%type
)
return number is
begin
	update merchant_balance
	  set m_total_float_settlement = m_total_float_settlement + in_float,
	      m_current_bal_settlement = m_current_bal_settlement + in_float,
	      m_current_bal = m_current_bal - in_float,
...
exception
  when others then
  return 9;
end sp_merchant_balance_sett_fl_in;
/
