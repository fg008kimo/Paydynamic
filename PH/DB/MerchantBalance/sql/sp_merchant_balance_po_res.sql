CREATE OR REPLACE FUNCTION sp_merchant_balance_po_res(
	...
	in_po_res		merchant_balance.m_reserved_payout%type,
	in_create_user		merchant_balance.m_create_user%type
)
return number is
begin
	update merchant_balance
	  set m_reserved_payout = in_po_res,
...
exception
  when others then
  return 9;
end sp_merchant_balance_po_res;
/
