...
{
    21,
    "OLPspTxnStatusRule.pc"
};


static unsigned int sqlctx = 165274147;
...
extern void sqliem( unsigned char *, signed int * );
 ...
 static char *sq0001 = 
"SELECT OSR_STATUS , OSR_AR_IND , OSR_SUB_STATUS , OSR_RECON_STATUS , OSR_CL\
ASSIFIED_STATUS FROM OL_PSP_TXN_STATUS_RULE WHERE OSR_RULE_ID = :b0 AND OSR_D\
ISABLED = 0            ";
...
{12,4130,871,0,0,
5,0,0,1,175,0,9,73,0,2049,1,1,0,1,0,1,3,0,0,
24,0,0,1,0,0,13,76,0,0,5,0,0,1,0,2,1,0,0,2,1,0,0,2,9,0,0,2,1,0,0,2,1,0,0,
59,0,0,1,0,0,15,117,0,0,0,0,0,1,0,
74,0,0,1,0,0,15,136,0,0,0,0,0,1,0,
};
...
-------------------------------                    ------------            --------------
Init Version                                       2014/11/24              LokMan Chow
*/
...
void OLPspTxnStatusRule(char cdebug)
{
        cDebug = cdebug;
}
...
DEBUGLOG(("GetStatusRule rule_id = [%d]\n", hv_rule_id));
	...
	/* EXEC SQL DECLARE c_getstatusrule CURSOR FOR
		SELECT
			...
			OSR_SUB_STATUS,
			OSR_RECON_STATUS,
			OSR_CLASSIFIED_STATUS
		FROM	OL_PSP_TXN_STATUS_RULE
		WHERE	OSR_RULE_ID = :hv_rule_id
		AND	OSR_DISABLED = 0; */ 
...
DEBUGLOG(("GetStatusRule status = [%c]\n", v_status));
			...
			if (ind_ar_ind >= 0) {
				PutField_Char(hTxn, "ar_ind", v_ar_ind);
DEBUGLOG(("GetStatusRule ar_ind = [%c]\n", v_ar_ind));
			}

			if (ind_sub_status >= 0) {
				v_sub_status.arr[v_sub_status.len] = '\0';
				PutField_CString(hTxn, "sub_status", (const char*)v_sub_status.arr);
DEBUGLOG(("GetStatusRule sub_status = [%s]\n", v_sub_status.arr));
			}

			if (ind_recon_status >= 0) {
				PutField_Char(hTxn, "recon_status", v_recon_status);
DEBUGLOG(("GetStatusRule recon_status = [%c]\n", v_recon_status));
			}

			if (ind_classified_status >= 0) {
				PutField_Char(hTxn, "classified_status", v_classified_status);
