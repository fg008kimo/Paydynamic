...
{
    26,
    "OLPayoutGeneratedFileHD.pc"
};


static unsigned int sqlctx = 929454683;
...
extern void sqliem( unsigned char *, signed int * );
 ...
 static char *sq0002 = 
"select ofh_filename , ofh_file_date , ofh_file_pid , ofh_status , ofh_txn_c\
ount , ofh_total_txn_amt , ofh_ccy_id , ofh_file_bank_code from ol_payout_gen\
erated_file_hd where ofh_file_id = :b0            ";

 static char *sq0004 = 
"select ofh_file_id , ofh_seq_num , ofh_filename , ofh_file_date , ofh_file_\
pid , ofh_file_bank_code , ofh_txn_count , ofh_total_txn_amt , ofh_ccy_id fro\
m ol_payout_generated_file_hd where ofh_status = :b0 for update ";
...
{12,4130,871,0,0,
5,0,0,1,322,0,6,143,0,0,11,11,0,1,0,2,3,0,0,1,68,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,
3,0,0,1,9,0,0,1,9,0,0,1,3,0,0,1,3,0,0,1,9,0,0,
64,0,0,2,202,0,9,237,0,2049,1,1,0,1,0,1,68,0,0,
83,0,0,2,0,0,13,239,0,0,8,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,3,0,0,2,3,0,0,2,4,
0,0,2,9,0,0,2,9,0,0,
130,0,0,2,0,0,15,314,0,0,0,0,0,1,0,
145,0,0,2,0,0,15,324,0,0,0,0,0,1,0,
160,0,0,3,72,0,4,344,0,0,1,0,0,1,0,2,68,0,0,
179,0,0,4,216,0,9,418,0,2049,1,1,0,1,0,1,3,0,0,
198,0,0,4,0,0,13,420,0,0,9,0,0,1,0,2,9,0,0,2,3,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,
0,0,2,3,0,0,2,4,0,0,2,9,0,0,
249,0,0,4,0,0,15,503,0,0,0,0,0,1,0,
264,0,0,4,0,0,15,513,0,0,0,0,0,1,0,
279,0,0,5,150,0,4,551,0,0,4,3,0,1,0,2,3,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
310,0,0,6,0,0,17,663,0,0,1,1,0,1,0,1,9,0,0,
329,0,0,6,0,0,21,664,0,0,0,0,0,1,0,
344,0,0,7,76,0,6,695,0,0,2,2,0,1,0,2,3,0,0,1,9,0,0,
367,0,0,8,127,0,4,753,0,0,3,2,0,1,0,2,68,0,0,1,68,0,0,1,3,0,0,
};
...

{
 ...
 sqlstm.stmt = "begin :hv_return_value := sp_ol_payout_gen_hd_insert ( :hv_\
file_id:ind_file_id , :hv_file_date:ind_file_date , :hv_pid:ind_pid , :hv_ban\
k_code:ind_bank_code , :hv_seq_num:ind_seq_num , :hv_filename:ind_filename , \
:hv_ccy_id:ind_ccy_id , :hv_txn_cnt:ind_txn_cnt , :hv_status:ind_status , :hv\
_add_user:ind_add_user ) ; END ;";
 ...
 sqlstm.sqhstv[5] = (unsigned char  *)&hv_seq_num;
 sqlstm.sqhstl[5] = (unsigned long )sizeof(int);
 sqlstm.sqhsts[5] = (         int  )0;
 sqlstm.sqindv[5] = (         short *)&ind_seq_num;
...

{
        ...
        sqlstm.sqladtp = &sqladt;
        sqlstm.sqltdsp = &sqltds;
        sqlstm.stmt = "select max ( ofh_file_id ) INTO :b0:b1 FROM ol_payou\
t_generated_file_hd ";
        sqlstm.iters = (unsigned int  )1;
...

{
  ...
  sqlstm.sqadto[6] = (unsigned short )0;
  sqlstm.sqtdso[6] = (unsigned short )0;
  sqlstm.sqhstv[7] = (unsigned char  *)&v_total_txn_amt;
  sqlstm.sqhstl[7] = (unsigned long )sizeof(double);
  sqlstm.sqhsts[7] = (         int  )0;
  sqlstm.sqindv[7] = (         short *)&ind_total_txn_amt;
  sqlstm.sqinds[7] = (         int  )0;
  sqlstm.sqharm[7] = (unsigned long )0;
  sqlstm.sqadto[7] = (unsigned short )0;
  sqlstm.sqtdso[7] = (unsigned short )0;
...

{
        ...
        sqlstm.sqladtp = &sqladt;
        sqlstm.sqltdsp = &sqltds;
        sqlstm.stmt = "select max ( ofh_seq_num ) INTO :b0:b1 FROM ol_payou\
t_generated_file_hd WHERE ofh_file_date = :b2 AND ofh_file_pid = :b3 AND ofh_\
file_bank_code = :b4 ";
        sqlstm.iters = (unsigned int  )1;
...

{
        ...
        sqlstm.sqladtp = &sqladt;
        sqlstm.sqltdsp = &sqltds;
        sqlstm.stmt = "begin :hv_return_value := sp_ol_payout_gen_hd_delete\
 ( :hv_file_id ) ; END ;";
        sqlstm.iters = (unsigned int  )1;
...
int MatchGenStatus_ForUpdate(const char* csFileId, const int iStatus)
{
        /* EXEC SQL WHENEVER SQLERROR GOTO MatchGenStatus_ForUpdate_error; */ 
...
DEBUGLOG(("MatchGenStatus_ForUpdate file_id = [%ld]\n",hv_file_id));

        hv_status = iStatus;
DEBUGLOG(("MatchGenStatus_ForUpdate status = [%d]\n", hv_status));

        /* EXEC SQL SELECT ofh_file_id
                INTO :v_file_id:ind_file_id
                FROM ol_payout_generated_file_hd
                WHERE ofh_file_id = :hv_file_id
                AND ofh_status = :hv_status
                FOR UPDATE NOWAIT; */ 

{
        ...
        sqlstm.sqladtp = &sqladt;
        sqlstm.sqltdsp = &sqltds;
        sqlstm.stmt = "select ofh_file_id INTO :b0:b1 FROM ol_payout_genera\
ted_file_hd WHERE ofh_file_id = :b2 AND ofh_status = :b3 FOR UPDATE NOWAIT ";
        sqlstm.iters = (unsigned int  )1;
        ...
        sqlstm.sqptdso = sqlstm.sqtdso;
        sqlcxt((void **)0, &sqlctx, &sqlstm, &sqlfpn);
        if (sqlca.sqlcode < 0) goto MatchGenStatus_ForUpdate_error;
}



        if (ind_file_id >= 0) {
DEBUGLOG(("MatchGenStatus_ForUpdate Found\n"));
                return PD_FOUND;
        } else {
DEBUGLOG(("MatchGenStatus_ForUpdate Not Found\n"));
                return PD_NOT_FOUND;
        }

MatchGenStatus_ForUpdate_error:
DEBUGLOG(("MatchGenStatus_ForUpdate_error code %d\n", sqlca.sqlcode));
DEBUGLOG(("\n%.*s\n", sqlca.sqlerrm.sqlerrml, sqlca.sqlerrm.sqlerrmc));
        /* EXEC SQL WHENEVER SQLERROR CONTINUE; */ 
