            ...
            if l_batch_cnt > 0 then
                ...
                if l_tmp_chk = 0 then
                    if l_txn_id is NULL then
                        ...
                        -- end of not found void txnid (l_txn_id is null)
                    else
                        -- void found
                        iv_level := iv_level - 1;
                        ...
                        and   otr_p2_txn_id = l_chk_txn_id;
                        */
                        ...
                        begin
                            ...
                            from (select obtr_txn_id
                                  from (select obtr_txn_id, obtr_relation_timestamp
                                        from ol_batch_txn_relation
                                        where obtr_batch_type in ('G','C','I')
                                        and obtr_batch_id in (select obtr_batch_id
                                                                from (select obtr_batch_id
                                                                      from ol_batch_txn_relation
                                                                      where obtr_batch_type in ('G','C','I')
                                                                      and obtr_txn_id = l_txn_id
                                                                      and obtr_txn_level = 'T'
                                                                      order by obtr_relation_timestamp desc
                                                                ) where rownum < 2
                                        )
                        ...
                        if ((l_chk_txn_id is not null) /* and (l_cnt > 0)*/) then
