CREATE OR REPLACE FUNCTION PHUSER.sp_txn_counters_insert(
...
/* daily count */
        update txn_counters
           set tc_counter = tc_counter + 1,
               tc_total_counter = tc_total_counter + 1,
               tc_req_counter = tc_req_counter + 1,
               tc_total_req_counter = tc_total_req_counter + 1,
               tc_update_user = in_create_user,
               tc_update_timestamp = sysdate
         where tc_txn_code = in_txn_code
           and tc_country_id = in_country_id
	   ...
	   and tc_pay_method = in_pay_method
        ...
        if SQL%ROWCOUNT = 0 THEN
			...
			in_pay_method,
                        ...
                        in_party_id,
                        in_ccy,
                        decode(in_counter,0,0,1),
                        decode(in_counter,0,0,1),
                        decode(in_req_counter,0,0,1),
                        decode(in_req_counter,0,0,1),
                        in_create_user,
                        sysdate
                );
