CREATE OR REPLACE FUNCTION sp_rulelb_pm_custgrp_cardtype (
    ...
    FROM (SELECT rc_customer_segment,
		 ...
		 rb_type,
            ...
            FROM (SELECT rc_customer_segment,
                    ...
                    FROM (  SELECT rc_customer_segment, rm_psp_id
                              FROM (SELECT rc_party_id,
                                     ...
                                     WHERE     rc_criteria_pool_id =
                                                  rp_criteria_pool_id
					   AND RULE_SCHEDULE_PKG.InRunningPeriod (rc_scheduler_id) = 1
                                           AND rc_disabled = 0
                                           AND rp_disabled = 0
				   ...
				   AND rc_customer_segment = CGR_GROUP_CODE
...
EXCEPTION
   ...
   THEN
      RETURN 9;
END sp_rulelb_pm_custgrp_cardtype;
/
