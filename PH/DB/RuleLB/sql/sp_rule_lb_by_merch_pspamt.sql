CREATE OR REPLACE FUNCTION sp_rule_lb_by_merch_pspamt(
...
BEGIN
   OPEN out_cursor FOR
	...
	FROM (SELECT psp_name,
         	...
         	LEFT JOIN
         	(SELECT bm_int_bank_code, bm_psp_channel_id, fe_display_order
          	  FROM bank_mapping, bank_desc
           	 WHERE bm_disabled = 0 AND internal_bank_code = bm_int_bank_code) b
            	ON (   (    a.overrided_bank_code_channel IS NOT NULL
                    AND b.bm_psp_channel_id = a.overrided_bank_code_channel)
                OR (    a.overrided_bank_code_channel IS NULL
                    AND b.bm_psp_channel_id = a.psp_channel_code))
...
EXCEPTION
   ...
   THEN
      RETURN 9;
END sp_rule_lb_by_merch_pspamt;
/
