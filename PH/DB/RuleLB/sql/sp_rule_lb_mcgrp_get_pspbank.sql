CREATE OR REPLACE FUNCTION sp_rule_lb_mcgrp_get_pspbank(
      ...
      SELECT bm_int_bank_code,
              ...
              FROM (  SELECT bm_int_bank_code,
                        ...
                        FROM (SELECT psp_name,
						       ...
						       AND (RC_MAX_TRANSACTION_AMOUNT=0 OR RC_MAX_TRANSACTION_AMOUNT >50)
						       AND (rc_customer_segment = in_group 
								...
								AND (in_merchant_id in ( select CGM_MERCHANT_ID 
											  ...
											  and CGM_PHASE = 1)
                                                                	OR
                                                             	       (in_merchant_id in ( select CGM_MERCHANT_ID 
                                                                                 	   from  CUSTOMER_GROUP_MERCHANT
											   where CGM_DISABLED = 0
											   and CGM_PHASE = 2) 
                                                                     	 AND
                                                                        in_group = (select CG_CODE
                                                                              	    from CUSTOMER_GROUP
								...
								AND ((in_merchant_id in ( select CGM_MERCHANT_ID
								      ...
								      AND
								       (rc_customer_segment is NULL
                                                     			OR
                                                     			rc_customer_segment = (select CG_CODE
                                                                              from CUSTOMER_GROUP
...
EXCEPTION
   ...
   THEN
      RETURN 9;
END sp_rule_lb_mcgrp_get_pspbank;
/
