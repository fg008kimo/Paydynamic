CREATE OR REPLACE FUNCTION sp_rulelb_match_sarip (
...
BEGIN
   SELECT COUNT (*)
     INTO iCnt
     FROM rule_psp_lb_criteria, system_parameter,
   ...
   WHERE     RULE_SCHEDULE_PKG.InRunningPeriod (rc_scheduler_id) = 1
	 ...
	 AND ((rc_allow_special_region = 1 and rc_min_transaction_amount <= in_amount
		and (rc_max_transaction_amount = 0 or rc_max_transaction_amount >= in_amount))
	    OR
	     (rc_allow_special_region = 0 and rc_max_transaction_amount > 0
		and rc_max_transaction_amount >= in_amount and rc_max_transaction_amount <= sp_val));
