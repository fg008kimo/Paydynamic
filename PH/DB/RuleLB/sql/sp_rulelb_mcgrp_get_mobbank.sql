CREATE OR REPLACE FUNCTION sp_rulelb_mcgrp_get_mobbank(
   ...
   in_union_group       customer_group_detail.cgd_code%TYPE,
   ...
   OPEN out_cursor FOR
	...
	from (  SELECT mbm_bank_code,bm_int_bank_code,
		...
		from(
                                        ...
                                        FROM (  SELECT rm_psp_id,
                                                  ...
                                                  FROM (SELECT rc_channel_code,
                                                         ...
                                                         WHERE     RULE_SCHEDULE_PKG.InRunningPeriod ( rc_scheduler_id) = 1
                                                               ...
                                                               AND (( rc_customer_segment in (select mbm_group from mob_bank_map
                                                                                                  where mbm_bank_code in(
                                                                                                        SELECT mbm_bank_code
                                                                                                        FROM mob_bank_map
                                                                                                        WHERE mbm_group in( in_group,in_union_group)
                                                                                                        AND mbm_disabled = 0)
                                              ...
                                              GROUP BY rm_psp_id,
                                                       rc_service_code,
                                                       rc_customer_segment,
                                                       input_group,credit_option,debit_option),
                                             (SELECT psp_name,
                                                     client_id,
                                                     psp_channel_code,
                                                     overrided_bank_code_channel,
				     ...
				     LEFT JOIN mob_segment_ctl d on d.msc_group = c.mbs_group
				     LEFT JOIN mob_segment_threshold e ON e.mst_group = c.mbs_group
				     LEFT JOIN
				     (
					SELECT rb_bank_code,
                                               rb_scheduler_id
					...
					AND RULE_SCHEDULE_PKG.InRunningPeriod (rb_scheduler_id) = 1
				     ...
				     on f.rb_bank_code = b.bm_int_bank_code
                               WHERE    (
					 ((e.mst_is_exceed_cnt = 0 and e.mst_is_exceed_amt = 0 and d.msc_enable_new_cust = 1 and in_new_cust = 1) or in_new_cust = 0)
...
EXCEPTION
   ...
   THEN
      RETURN 9;
END sp_rulelb_mcgrp_get_mobbank;
/
