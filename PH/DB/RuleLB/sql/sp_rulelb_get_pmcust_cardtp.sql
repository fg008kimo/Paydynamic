CREATE OR REPLACE FUNCTION sp_rulelb_get_pmcust_cardtp(
				     ...
				     pd_daily_succ_txn_threshold
						       ...
						       case when rc_card_type_opt = 'C' then 1 else 0 end credit_option,
                                                  ...
                                                  FROM rule_psp_lb_criteria,
                                                       rule_psp_lb_pools,system_parameter
						       ...
						       AND (RC_MAX_TRANSACTION_AMOUNT=0 OR RC_MAX_TRANSACTION_AMOUNT > sp_val)
						       ...
						       AND (rc_customer_segment  is NULL
							    OR (rc_customer_segment in (select CGR_GROUP_CODE
											from CUSTOMER_GROUP_RULES
											where CGR_FROM_GROUP_CODE = in_group
											and CGR_DISABLED = 0
											)
							       )
							   )
					     ...
					     pd_daily_succ_txn_threshold,
							...
							FROM (SELECT r_psp_id,
						      ...
						      LEFT JOIN
                                                             (SELECT pir_region_code,
                                                                     pir_psp_id
                                                              FROM   psp_restrict_ip_region
                                                              WHERE  pir_disabled = 0) 
                                                       ON     (pir_psp_id = r_psp_id)
                                                       WHERE (pd_restrict_deposit_ip = 0
                                                              OR (pd_restrict_deposit_ip = 1 AND pir_region_code = in_ip_region_code)))
						       LEFT JOIN
								(SELECT cpc_psp_id,
								cpc_counter
...
EXCEPTION
   ...
   THEN
      RETURN 9;
END sp_rulelb_get_pmcust_cardtp;
/
