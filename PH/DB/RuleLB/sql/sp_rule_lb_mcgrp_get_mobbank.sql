CREATE OR REPLACE FUNCTION sp_rule_lb_mcgrp_get_mobbank(
...
BEGIN
  ...
  SELECT mbm_bank_code,
    ...
    FROM (  SELECT currency_id,
                   SUM (case when limit_exceed=0 then remaining_limit else 0 end) remain_limit,
                   rc_customer_segment,
                   mbm_bank_code,
                   rb_scheduler_id,
                   rb_note_id
              FROM (  SELECT client_id,
                             ...
                             input_group,
			     case 
                             when remaining_limit >= in_amount
                             then 0  else 1
                             end as limit_exceed
                        FROM (SELECT client_id,
                                     rpp_psp_id,
                                                               ...
                                                               AND (   rc_customer_segment =
                                                                    ...
                                                                    OR (    rc_customer_segment
                                                                        ...
                                                                        AND (SELECT cgm_merchant_id
                                                                              ...
                                                                              WHERE     cgm_merchant_id =
                                                                                           in_merchant_id
                                                                                    AND cgm_disabled =
                                                                                           0)
                                                                               IS NULL)
                                                                    OR (rc_customer_segment IN (
										SELECT mbm_group FROM(
										    SELECT mbm_group,
											   nvl(mst_is_exceed_cnt,0) exceed_cnt, 
											   nvl(mst_is_exceed_amt,0) exceed_amt
                                                                                      FROM (SELECT mbm_group
                                                                                            FROM mob_bank_map,
                                                                                                 mob_bank_selection
                                                                                      WHERE     mbm_bank_code IN (SELECT mbm_bank_code
                                                                                                      FROM mob_bank_map
                                                                                                      WHERE     mbm_group = in_group
                                                                                      ...
                                                                                      AND mbs_disabled = 0)
                                                                                     LEFT JOIN  mob_segment_threshold
                                                                                     ON     mbm_group = mst_group)
										WHERE exceed_cnt = 0 and exceed_amt = 0
										))
								)),
                                       ...
                                       WHERE     bm_disabled = 0
                                        ...
                                        ON     (   (    a.overrided_bank_code_channel
                                                ...
                                                OR (    a.overrided_bank_code_channel
                                                    ...
                                                    AND b.bm_psp_channel_id =
                                                           a.psp_channel_code))
                                           AND b.bs_service_code =
                                                  a.rc_service_code
                                     LEFT JOIN
                                     (SELECT mbm_bank_code,
                                             mbm_group,
                                             mbs_def_assign_grp
                                        FROM mob_bank_map, mob_bank_selection
                                       WHERE     mbm_disabled = 0
                                             AND mbs_disabled = 0
                                             AND mbm_group = mbs_group) c
                                        ON a.rc_customer_segment = c.mbm_group
                               WHERE    (    b.bm_int_bank_code = c.mbm_bank_code
                                         AND c.mbs_def_assign_grp = 0)
                                     OR c.mbs_def_assign_grp = 1)
                    GROUP BY client_id,
                             rpp_psp_id,
                             currency_id,
                             remaining_limit,
   	...
   	--WHERE remaining_limit >= in_amount
          GROUP BY rc_customer_segment,
                   ...
                   rb_scheduler_id,
                   rb_note_id)
   --WHERE remain_limit >= in_amount
ORDER BY remain_limit DESC;
...
EXCEPTION
   ...
   THEN
      RETURN 9;
END sp_rule_lb_mcgrp_get_mobbank;
/
