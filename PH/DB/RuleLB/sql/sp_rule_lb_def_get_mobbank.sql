CREATE OR REPLACE FUNCTION sp_rule_lb_def_get_mobbank(
...
BEGIN
   OPEN out_cursor FOR
    ...
    FROM (  SELECT --bm_int_bank_code,
                   ...
                   --rpp_psp_id,
                   currency_id,
                   SUM (remaining_limit) remain_limit,
                   rc_customer_segment,
                   mbs_bank_code,
                   rb_scheduler_id,
                   rb_note_id
              FROM (  SELECT  --bm_int_bank_code,
                        ...
                        FROM (SELECT psp_name,
                                          ...
                                          FROM (SELECT rc_channel_code,
                                                 ...
                                                 WHERE     RULE_TXN_LIMIT_PKG.FINDSCHEDULE (
                                                       ...
                                                       AND (   rc_customer_segment
                                                                 is null 
                                                            OR ( rc_customer_segment in (SELECT mbs_group
                             ...
                             LEFT JOIN (SELECT mbs_bank_code, mbs_group
                                          FROM mob_bank_select
                                         WHERE mbs_disabled = 0) c
                                ON a.rc_customer_segment = c.mbs_group
                    GROUP BY client_id,
                             rpp_psp_id,
                             currency_id,
                             remaining_limit,
...
EXCEPTION
   ...
   THEN
      RETURN 9;
END sp_rule_lb_def_get_mobbank;
/
