CREATE OR REPLACE FUNCTION sp_ol_get_intra_to_baid_dtl(
	in_baid                         OL_BANK_ACCT_ID.OBAI_BAID%type,
        in_to_baid                      OL_BANK_ACCT_ID.OBAI_BAID%type,
        in_currency                     DEF_CURRENCY.DC_CCY_ID%type,
...
BEGIN
	...
	WHERE   BAID = in_to_baid
        AND     Currency_ID = in_currency
        AND     PSP_Bank_Nature_Code|| '_' ||BAID_Category_Code IN (SELECT BTM_TO_ACCT_TYPE|| '_' ||BTCM_BAID_CATEGORY
                                                                    FROM OL_TC_BAL_TRF_MAP
                                                                    LEFT JOIN OL_TC_BAL_TRF_CAT_MAP ON BTCM_MAP_ID = BTM_TO_BAID_CAT_ID
                                                                    LEFT JOIN TXN_CODE ON BTM_TXN_CODE = TC_CODE
                                                                    WHERE BTM_BAL_TRF_TYPE = 'INTRA'
                                                                    AND BTM_FR_ACCT_TYPE = (SELECT OPD_BANK_ACCT_TYPE
                                                                                            FROM OL_BANK_ACCT_ID,
                                                                                                 OL_PSP_DETAIL
                                                                                            WHERE OBAI_BAID = in_baid
                                                                                            AND OPD_PSP_ID = OBAI_PSP_ID
                                                                                           )
                                                                    AND TC_OFL_ALLOW_FE_INIT = 1);
...
END sp_ol_get_intra_to_baid_dtl;
/
