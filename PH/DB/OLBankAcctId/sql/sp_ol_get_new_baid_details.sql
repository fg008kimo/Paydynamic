CREATE OR REPLACE FUNCTION sp_ol_get_new_baid_details(
	in_baid                 	OL_BANK_ACCT_ID.OBAI_BAID%type,
	in_bank_nature                 	OL_PSP_DETAIL.OPD_BANK_ACCT_TYPE%type,
...
BEGIN
	...
	(
		SELECT  PSP_MASTER.PM_CLIENT_NAME AS Provider_Name,
			...
			OL_MERCHANT_BANK_ACCT.OMB_MERCHANT_ID AS Merchant_ID,
                        ...
                        Total_Balance,
                        Current_Balance,
			ROW_NUMBER() OVER (ORDER BY BAID_Create_Timstamp, BAID, OL_MERCHANT_BANK_ACCT.OMB_MERCHANT_ID) AS RN,
		...
		(
				...
				OL_BANK_ACCT_ID.OBAI_INT_BANK_CODE != '086000'
                        AND   	OL_DEF_BAID_CATEGORY.BC_TYPE = OL_BANK_ACCT_ID.OBAI_CATEGORY
	...
	WHERE	BAID_Create_Timstamp >= (select OBAI_CREATE_TIMESTAMP from OL_BANK_ACCT_ID where OBAI_BAID = in_baid)
	AND	BAID != in_baid
	--AND     ((Bank_Nature_Code = in_bank_nature) OR (Bank_Nature_Code IS NULL)) 
	AND     Bank_Nature_Code = PSP_Bank_Nature_Code
	AND	PSP_Bank_Nature_Code = in_bank_nature;
		...
		(
			SELECT  BAID,
				...
				Merchant_ID,
                                BAID_Code_In_Num,
                                BAID_Create_Timstamp,
				ROW_NUMBER() OVER (ORDER BY BAID_Create_Timstamp, BAID, Merchant_ID) AS RN,
                        	COUNT(*) OVER () AS Total_Count
                        FROM			
			(
				SELECT  PSP_MASTER.PM_CLIENT_NAME AS Provider_Name,
			...
			AND 	Bank_Nature_Code = PSP_Bank_Nature_Code
        		AND     PSP_Bank_Nature_Code = in_bank_nature
			ORDER BY	BAID_Create_Timstamp, BAID, Merchant_ID
		)
...
END sp_ol_get_new_baid_details;
/
