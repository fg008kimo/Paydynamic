CREATE OR REPLACE FUNCTION SP_HIST_OL_COST_SUM_COLLECT(
    IN_END_DATE IN DATE )
RETURN NUMBER IS
	EXP_ERR  EXCEPTION;
	EXP_ERR2 EXCEPTION;
	...
	V_NEW_BATCH_ID HIST_OL_COST_SUM_BATCH.HB_BATCH_ID%type;
	V_PREV_BATCH_ID HIST_OL_COST_SUM_BATCH.HB_BATCH_ID%type;
	V_START_DATE         DATE;
...
BEGIN
-- DBMS_OUTPUT.PUT_LINE('>>> START [HIST_OL_COST_SUM_COLLECT] <<<');
...
-- Find PSP Cost Sum from start times to end times
	...
	SELECT TXN.PARTY_TYPE AS PARTY_TYPE
	      ...
	      ,0 AS IS_PREV		
	FROM (
		SELECT OL_TXN_HEADER.OTH_TXN_ID AS TXN_ID
		       ,OL_TXN_HEADER.OTH_TXN_CODE AS TXN_CODE
		       ,BANK_DESC.COUNTRY AS TXN_COUNTRY
		       ,OL_TXN_ELEMENTS.OTE_AMOUNT AS TXN_AMOUNT
		       ,OL_TXN_ELEMENTS.OTE_AMT_TYPE AS TXN_AMT_TYPE
		       ,CRR_PSP_PRODUCT_CODE_MAP.PM_PRODUCT_CODE AS PRODUCT_CODE
		       ,'PSP' AS PARTY_TYPE
		       ,OL_TXN_PSP_DETAIL.OTP_PSP_ID AS ENTITY_ID
		       ,OL_PSP_DETAIL.OPD_PSP_NAME AS ENTITY_NAME
		...
		ON COST_SUM_TXN_CODE_MAP.PARTY_TYPE = 'P'
			AND COST_SUM_TXN_CODE_MAP.TXN_CODE = OL_TXN_HEADER.OTH_TXN_CODE
		INNER JOIN OL_TXN_ELEMENTS
		ON OL_TXN_ELEMENTS.OTE_TXN_ID = OL_TXN_HEADER.OTH_TXN_ID
			AND OL_TXN_ELEMENTS.OTE_TXN_ELEMENT_TYPE = COST_SUM_TXN_CODE_MAP.TXN_ELEMENT_TYPE
			AND OL_TXN_ELEMENTS.OTE_PARTY_TYPE = COST_SUM_TXN_CODE_MAP.PARTY_TYPE
			AND OL_TXN_HEADER.OTH_AR_IND = 'A'
			AND OL_TXN_HEADER.OTH_STATUS IN (
				'C'
				,'R'
			)
		INNER JOIN OL_TXN_PSP_DETAIL
			ON OL_TXN_PSP_DETAIL.OTP_TXN_ID = OL_TXN_HEADER.OTH_TXN_ID
		INNER JOIN 	OL_BANK_ACCT_ID
			ON OL_BANK_ACCT_ID.OBAI_BAID = OL_TXN_PSP_DETAIL.OTP_BAID
		INNER JOIN 	BANK_DESC
			ON BANK_DESC.INTERNAL_BANK_CODE = OL_BANK_ACCT_ID.OBAI_INT_BANK_CODE
		INNER JOIN OL_PSP_DETAIL
			ON OL_PSP_DETAIL.OPD_PSP_ID = OL_TXN_PSP_DETAIL.OTP_PSP_ID
		INNER JOIN PSP_MASTER
			ON PSP_MASTER.PM_CLIENT_ID = OL_PSP_DETAIL.OPD_CLIENT_ID
		INNER JOIN	OL_COST_SUM_BAID_TXN_CODE_MAP
			ON OL_COST_SUM_BAID_TXN_CODE_MAP.TXN_CODE = OL_TXN_HEADER.OTH_TXN_CODE
		INNER JOIN	OL_COST_SUM_BAID_NATURE_MAP
			ON OL_COST_SUM_BAID_NATURE_MAP.NATURE_MAP_ID = OL_COST_SUM_BAID_TXN_CODE_MAP.NATURE_MAP_ID
				AND OL_COST_SUM_BAID_NATURE_MAP.NATURE = OL_PSP_DETAIL.OPD_BANK_ACCT_TYPE
		INNER JOIN	OL_COST_SUM_BAID_CAT_MAP
			ON OL_COST_SUM_BAID_CAT_MAP.BAID_CAT_MAP_ID = OL_COST_SUM_BAID_TXN_CODE_MAP.BAID_CAT_MAP_ID
				AND OL_COST_SUM_BAID_CAT_MAP.BAID_CAT = OL_BANK_ACCT_ID.OBAI_CATEGORY
		INNER JOIN	OL_COST_SUM_BAID_RECON_MAP
			ON OL_COST_SUM_BAID_RECON_MAP.RECON_STATUS_MAP_ID = OL_COST_SUM_BAID_TXN_CODE_MAP.RECON_STATUS_MAP_ID
				AND OL_COST_SUM_BAID_RECON_MAP.RECON_STATUS = OL_TXN_HEADER.OTH_RECON_STATUS
		...
		SELECT OL_TXN_HEADER.OTH_TXN_ID AS TXN_ID
			...
			,OL_TXN_ELEMENTS.OTE_CCY AS TXN_CCY
			,CASE 
				WHEN OL_TXN_ELEMENTS.OTE_AMT_TYPE = 'DR'
					THEN 1
				ELSE 0
				END AS IS_COST_AMT
			,CASE 
				WHEN OL_TXN_ELEMENTS.OTE_AMT_TYPE = 'CR'
					THEN 1
				ELSE 0
		...
		INNER JOIN OL_TXN_ELEMENTS
		ON OL_TXN_ELEMENTS.OTE_TXN_ID = OL_TXN_HEADER.OTH_TXN_ID
			AND OL_TXN_ELEMENTS.OTE_TXN_ELEMENT_TYPE IN (
				'TAMT'
				,'INTR'
			)
			AND OL_TXN_ELEMENTS.OTE_PARTY_TYPE = 'P'
			AND OL_TXN_HEADER.OTH_AR_IND = 'A'
			AND OL_TXN_HEADER.OTH_STATUS IN (
				'C'
				,'R'
		...
		ON OL_ADJUSTMENT_TYPE.OAT_CODE = OL_TXN_HEADER.OTH_TXN_CODE
			AND OL_ADJUSTMENT_TYPE.OAT_NATURE = 'C'
		INNER JOIN OL_TXN_PSP_DETAIL
		ON OL_TXN_PSP_DETAIL.OTP_TXN_ID = OL_TXN_HEADER.OTH_TXN_ID
		INNER JOIN 	OL_BANK_ACCT_ID
		ON OL_BANK_ACCT_ID.OBAI_BAID = OL_TXN_PSP_DETAIL.OTP_BAID
		INNER JOIN	OL_DEF_BAID_CATEGORY 
		ON OL_DEF_BAID_CATEGORY.BC_TYPE = OL_BANK_ACCT_ID.OBAI_CATEGORY
			AND OL_DEF_BAID_CATEGORY.BC_IN_BALANCE = 1
		INNER JOIN 	BANK_DESC
		ON BANK_DESC.INTERNAL_BANK_CODE = OL_BANK_ACCT_ID.OBAI_INT_BANK_CODE
		INNER JOIN OL_PSP_DETAIL
		ON OL_PSP_DETAIL.OPD_PSP_ID = OL_TXN_PSP_DETAIL.OTP_PSP_ID
		...
		SELECT OL_TXN_HEADER.OTH_TXN_ID AS TXN_ID
		       ,OL_TXN_HEADER.OTH_TXN_CODE AS TXN_CODE
		       ,OL_TXN_MI_DETAIL.OTM_TXN_COUNTRY AS TXN_COUNTRY
		       ,OL_TXN_ELEMENTS.OTE_AMOUNT AS TXN_AMOUNT
		       ,OL_TXN_ELEMENTS.OTE_AMT_TYPE AS TXN_AMT_TYPE
		       ,OL_TXN_MI_DETAIL.OTM_TXN_PRODUCT AS PRODUCT_CODE
		       ,OL_TXN_MI_DETAIL.OTM_PARTY_TYPE AS PARTY_TYPE
		       ,OL_TXN_MI_DETAIL.OTM_ENTITY_ID AS ENTITY_ID
		...
		INNER JOIN OL_TXN_MI_DETAIL
		ON OTH_TXN_ID = OTM_TXN_ID
			AND OL_TXN_MI_DETAIL.OTM_PARTY_TYPE = 'RSP'
		INNER JOIN MI_ENTITY_RSP
		ON MI_ENTITY_RSP.ER_ENTITY_ID = OL_TXN_MI_DETAIL.OTM_ENTITY_ID
		WHERE 1 = 1
		AND OL_TXN_HEADER.OTH_APPROVAL_TIMESTAMP >= CAST(V_START_DATE AS TIMESTAMP)
		AND OL_TXN_HEADER.OTH_APPROVAL_TIMESTAMP < CAST(V_ADD_RANGE_END_DATE AS TIMESTAMP)
		...
		ON OL_TXN_ELEMENTS.OTE_TXN_ID = OL_TXN_HEADER.OTH_TXN_ID
			...
			AND OL_TXN_ELEMENTS.OTE_PARTY_TYPE = 'S'
			AND OL_TXN_HEADER.OTH_AR_IND = 'A'
			AND OL_TXN_HEADER.OTH_STATUS IN (
				'C'
				,'R'
		...
		INNER JOIN MI_ENTITY_RSP
		ON MI_ENTITY_RSP.ER_ENTITY_ID = OL_TXN_MI_DETAIL.OTM_ENTITY_ID
		INNER JOIN MI_OL_ADJUSTMENT_TYPE
		ON MOAT_CODE = OTH_TXN_CODE
			AND MOAT_ENTITY_TYPE = 'RSP'
			AND MOAT_NATURE = 'C'
		WHERE 1 = 1
		AND OL_TXN_HEADER.OTH_APPROVAL_TIMESTAMP >= CAST(V_START_DATE AS TIMESTAMP)
		AND OL_TXN_HEADER.OTH_APPROVAL_TIMESTAMP < CAST(V_ADD_RANGE_END_DATE AS TIMESTAMP)
...
-- Check Cut off Time
		...
		INSERT INTO HIST_OL_COST_SUM_DETAIL
	        (
			HD_BATCH_ID,
			HD_PARTY_TYPE,
			HD_ENTITY_ID,
...
-- DBMS_OUTPUT.PUT_LINE('--> END INSERT ');
	...
	COMMIT;
-- DBMS_OUTPUT.PUT_LINE('>>> END [HIST_OL_COST_SUM_COLLECT] <<<');
...
--RAISE_APPLICATION_ERROR( -20201, 'Error code ' || SQLCODE|| ': ' || SUBSTR(SQLERRM, 1 , 64) );
	RETURN 9;
END SP_HIST_OL_COST_SUM_COLLECT;
/
