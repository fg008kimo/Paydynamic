...
unsigned char PD_IV[] = {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
...
void EncryptKeyDIV(const unsigned char *KEY, const unsigned char *DE_KEY,int Len,unsigned char* EN_KEY,unsigned char* csDIV)
{
	des3_context ctx3;
	des3_set_2keys( &ctx3, (unsigned char*)&KEY[0],
                           (unsigned char*)&KEY[8]);
	mydes3_encrypt( &ctx3,DE_KEY,Len,EN_KEY,csDIV);
}
...
void myECBdes3_encrypt_new(const unsigned char *KEY, int iKeyLen,const unsigned char *DE_KEY,int Len,unsigned char* EN_KEY)
{
        int i;
        des3_context ctx3;
        if ( iKeyLen/8 ==2 ) {
	...
	while( length > 0 )
        {
            ...
            des3_encrypt( ctx, output, output );
            memcpy( iv, output, 8 );
