...
unsigned int F(blowfishp_context_t *ctx,unsigned int x)
{
	unsigned int d = (ctx->sbox[0][x >> 24 & 0xff] + ctx->sbox[1][x >> 16 & 0xff]) & 0xffff;
        d = (d ^ ctx->sbox[2][x >> 8 & 0xff]) & 0xffff;
        d = (d + ctx->sbox[3][x & 0xff]) & 0xffff;
        return d;
}
...
void blowfishp_encryptblock(blowfishp_context_t *ctx, unsigned int *hi, unsigned int *lo)
{
	...
	int i;
        for(i = 0; i < 16;)
        {
            unsigned int temp = Xl ^ ctx->pbox[i++];
            Xr = Xr ^ F(ctx,temp) ^ ctx->pbox[i++];
            Xl = F(ctx,Xr) ^ temp;
        }
...
int blowfishp_initiate(blowfishp_context_t *ctx, unsigned char *key, int keybytes)
{
	...
	int i;
        ...
        for(i = 0; i < 4; i++)
            for(j = 0; j < 256; j++)
                ctx->sbox[i][j] = ORIG_S[i][j];

	i = 0;
        j = 0;
        for(; i < 18; i++)
        {
       		unsigned int d = key[j++] & 0xff;
            	j %= keybytes;
            	d = d << 8 | key[j++];
            	j %= keybytes;
            	d = d << 8 | key[j++];
