...
static tpl_fcell_t* create_fcell(const char *key, int key_len)
{
    ...
    field->key[key_len] = 0;
    field->len = key_len + DELIM_LEN*2;
    field->val = (char *)m_malloc(M_MAX(field->len, INITIAL_FIELD_LEN) + 1);
    sprintf(field->val,"%s%s%s",DELIMITER,field->key,DELIMITER);
    field->next = NULL;
    return field;
}
                                      ...
                                      int may_create)
{
    ...
    {
        *pfield = create_fcell(key, key_len);
        return *pfield;
    }
