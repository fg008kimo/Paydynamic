...
Check Duplicate					   2012/02/22		   LokMan Chow
*/
...
DEBUGLOG(("process_txn:BreakDown MmsMsg Error\n"));
	...
	if (iRet == PD_OK) {
		...
		//PutField_CString(hResponse, "client_ref", csTxnSeq);
...
//init txn or not
		...
		DBObjPtr = CreateObj(DBPtr,"DBMmsTxnSupported","IsInitTxn");
		iInitTxn = (unsigned long)(*DBObjPtr)(csChannelCode,csTxnCode);
DEBUGLOG(("MMSChannel::init_txn = [%d]\n",iInitTxn));
		if(iInitTxn)
			PutField_Char(hContext,"isd_ind",PD_MMS_ISD_IND_INIT);

		//do logging or not
		int iDoLog = PD_NO_LOG;
		DBObjPtr = CreateObj(DBPtr,"DBMmsTxnSupported","IsDoLogging");
		iDoLog = (unsigned long)(*DBObjPtr)(csChannelCode,csTxnCode);
DEBUGLOG(("MMSChannel::do_logging = [%d]\n",iDoLog));
...
//get host ref and host dtl ref
////check duplicate
        if(iRet == PD_OK){

                char *csHostRef = strdup("");
                char *csHostDtlRef = strdup("");
                char csMatchHostRef[PD_MERCHANT_REF_LEN+1];

                if(GetField_CString(hRequest,"host_ref",&csHostRef)){
DEBUGLOG(("process_txn: host_ref = [%s]\n",csHostRef));
                }
                else{
                        csHostRef = strdup("M000");
                }
                if(GetField_CString(hRequest,"host_dtl_ref",&csHostDtlRef)){
DEBUGLOG(("process_txn: host_dtl_ref = [%s]\n",csHostDtlRef));
                }
                else{
                        csHostDtlRef = strdup("000");
                }

                sprintf(csMatchHostRef,"%sm%s",csHostRef,csHostDtlRef);
                PutField_CString(hContext,"th_host_ref",csMatchHostRef);

                DBObjPtr = CreateObj(DBPtr,"DBTransaction","MatchHostRef");

                if ((unsigned long)(DBObjPtr)(csChannelCode,csMatchHostRef) == PD_FOUND) {
			// Not in VMS
                        if (strcmp(csTxnCode, PD_MONEY_MOVE_AUTO_REV)) {
                                iRet = INT_DUP_HOST_REF;
                                PutField_Int(hContext,"internal_error",iRet);
ERRLOG("Channel:[MMS] - [%s][%s]Duplicate Host Ref\n",csHostRef,csHostDtlRef);
DEBUGLOG(("Channel:[MMS] - [%s][%s]Duplicate Merchant Ref\n",csHostRef,csHostDtlRef));
                        }

                }

                FREE_ME(csHostRef);
                FREE_ME(csHostDtlRef);
        }
...
DEBUGLOG(("*****Format Out Reject Message\n"));
			...
			//PutField_Char(hContext, "dtl_status", PD_COMPLETE);
			//PutField_Char(hContext, "dtl_ar_ind", PD_ACCEPT);

			if(!strcmp(csTxnCode,PD_REVERS_MONEY_MOVEMENT)){
                		PutField_Char(hContext,"status",PD_COMPLETE);
...
DEBUGLOG(("process_input_txn:txn_code not found\n"));
	...
	if(iRet == PD_OK){
		...
		if(GetField_Char(hContext,"isd_ind",&cTmp)){
			//if(iTmp==PD_TRUE){
			//	cTmp = PD_MMS_ISD_IND_INIT;
			//}
DEBUGLOG(("process_input_txn txn init_ind [%d]\n",iTmp));
		}

		/*no further action for init txn*/
		if(cTmp != PD_MMS_ISD_IND_INIT){
...
/* mmc_node_id */
                if (GetField_CString(hRequest,"mms_node_id",&csTmp)) {
...
/* init channel */
		if (GetField_CString(hRequest, "init_channel", &csTmp)) {
DEBUGLOG(("AddTxnLog:: init_channel = [%s]\n",csTmp));
                        PutField_CString(hTxn,"init_channel",csTmp);
		}
...
/* dc_ind */
/*
DEBUGLOG(("AddTxnLog:: dc_ind = [%s]\n",PD_MMS_DR));
                        PutField_CString(hTxn,"dc_ind",PD_MMS_DR);
*/

/* carry_forward_ind */
		if (GetField_Int(hRequest, "carry_forward_ind", &iTmp)) {
DEBUGLOG(("AddTxnLog:: carry_forward_ind = [%d]\n",iTmp));
			PutField_Int(hTxn,"carry_forward_ind",iTmp);
		}

                PutField_Char(hTxn,"status",PD_PROCESSING);
                //PutField_Char(hTxn,"init_ind",PD_MMS_ISD_IND_INIT);
...
DEBUGLOG(("UpdateTxnLog:: response_code = [%s]\n",csTmp));
		...
		if (iRet == PD_OK) {
			...
			/*
                	if (GetField_Char(hContext,"dtl_status",&cTmp)) {
DEBUGLOG(("UpdateTxnLog:: dtl_status = [%c]\n",cTmp));
	                        PutField_Char(hTxn,"status",cTmp);
			}

                	if (GetField_Char(hContext,"dtl_ar_ind",&cTmp)) {
DEBUGLOG(("UpdateTxnLog:: dtl_ar_ind = [%c]\n",cTmp));
	                        PutField_Char(hTxn,"ar_ind",cTmp);
			}
			*/
