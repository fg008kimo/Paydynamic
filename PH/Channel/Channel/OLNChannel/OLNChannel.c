...
Handle sub_status requirement change               2014/04/24              Virginia Yun
Handle Non-365 API                                 2014/08/25              Virginia Yun
*/
				...
				if (!strcmp(csTxnCode, PD_INITIAL_OLN_TXN_CODE))    {// initial
					...
					if (GetField_CString(hRequest,"bank_acct_num",&csPtr)) {
						...
						if (GetField_CString(hContext, "txn_seq", &csPtr)) {
							PutField_CString(hResponse, "txn_seq", csPtr);
						}
...
DEBUGLOG(("AddTxnLog:: txn_code = [%s]\n",csPtr));
			if (!strcmp(csPtr, PD_INITIAL_OLN_TXN_M2_CODE)) {
...
/* upload_channel */
		PutField_Int(hTxn,"upload_channel",PD_OLN_UPLOAD);
...
DEBUGLOG(("AddTxnLog:: cust_deposit_datetime = [%s]\n",csPtr));
                        PutField_CString(hDetail,"cust_deposit_datetime",csPtr);
			PutField_Int(hDetail,"cust_deposit_dt_prov",PD_TRUE);
                } 
		/*
		else {
			...
			csDateTime[PD_YYYYMMDDHHMM_LEN] = '\0';
                        PutField_CString(hDetail,"cust_deposit_datetime",csDateTime);
			PutField_Int(hDetail,"cust_deposit_dt_prov",PD_FALSE);
		}
		*/
...
/* customer_tag */
                if (GetField_CString(hRequest,"customer_tag",&csPtr)) {
DEBUGLOG(("AddTxnLog:: customer_tag = [%s]\n",csPtr));
                        PutField_CString(hDetail,"customer_tag",csPtr);
                }

/* deposit_flow */
                if (GetField_Char(hRequest,"deposit_flow",&cPtr)) {
DEBUGLOG(("AddTxnLog:: deposit_flow = [%c]\n",cPtr));
                        PutField_Char(hDetail,"deposit_flow",cPtr);
		}
...
DEBUGLOG(("UpdateTxnLog:: cust_deposit_datetime = [%s]\n", csTmp));
                        PutField_CString(hTxn,"cust_deposit_datetime",csTmp);
		}
/* cust_deposit_dt_prov */
		if (GetField_Int(hContext, "cust_deposit_dt_prov", &iTmp)) {
DEBUGLOG(("UpdateTxnLog:: cust_deposit_dt_prov = [%d]\n", iTmp));
                        PutField_Int(hTxn,"cust_deposit_dt_prov", iTmp);
		}
