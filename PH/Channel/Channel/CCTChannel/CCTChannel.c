...
#define	PD_ACK_RESEND	"resend_cct_rck.sh"
char	cDebug;

void CCTChannel(char	cdebug)
{
	cDebug = cdebug;
}
...
int     process_CCT_bounce_txn(hash_t *hContext,
			   	const hex_t* inMsg,
				hex_t *outMsg);
...
DEBUGLOG(("Authorize() try to VerifyMac()\n"));
			...
			if (iRet != PD_OK) {
DEBUGLOG(("CCTChannel:Authorize() Invalid MAC, txn id [%s] dropped\n",csTxnSeq));
ERRLOG("CCTChannel:Authorize() Invalid MAC, txn id [%s] dropped\n",csTxnSeq);
			}
                }
                else{
...
ERRLOG(("CCTChannel::Authorize() mac not found\n"));
                }


		if (iRet == PD_OK ) {
			MsgObjPtr = CreateObj(MsgPtr,"CctMsg","initReplyFromRequest");
		...
		if (iRet == PD_OK) {
			DBObjPtr = CreateObj(DBPtr,"DBCctTxn","GetApprovated");
                        if ((unsigned long)(DBObjPtr)(csMerchantId,csMerchantRef) > 0) {
                                iRet = INT_DUP_MERCHANT_REF;
                                PutField_Int(hContext,"internal_error",INT_DUP_MERCHANT_REF);
ERRLOG("Channel:[CCT] - MerchantId[%s][%s]Duplicate Merchant Ref\n",csMerchantId,csMerchantRef);
DEBUGLOG(("Channel:[CCT] - MerchantId[%s][%s]Duplicate Merchant Ref\n",csMerchantId,csMerchantRef));
...
DEBUGLOG(("put failure_url into redirect_url\n"));
					...
					MsgObjPtr = CreateObj(MsgPtr,"CctMsg","FormatFEMsg");
				}else {
					MsgObjPtr = CreateObj(MsgPtr,"CctMsg","FormatMsg");
...
DEBUGLOG(("FormatMsg error\n"));
				...
				if (iRet == PD_OK && iInternalErr == 0) {
					char	*csHandler;
                			csHandler = (char*) malloc (20);
                			strcpy(csHandler,"TxnCctOnUsCCT");
DEBUGLOG(("******process txn Create Txn Object [%s]\n",csHandler));
                			TxnObjPtr = CreateObj(TxnPtr,csHandler,"Request");
                			iRet = (unsigned long)(*TxnObjPtr)(hContext,&h_msg->msg,&h_msg->len);
DEBUGLOG(("iRet = [%d] from [%s]:Request()\n",iRet,csHandler));
                			FREE_ME(csHandler);
				}
...
ERRLOG("CCTCHANNEL internal error? [%d]\n",iRet);
		}
	}
...
/* echo_msg_1 */
                if (GetField_CString(hRequest,"echo_msg_1",&csTmp)) {
DEBUGLOG(("AddTxnLog:: echo_msg_1 = [%s]\n",csTmp));
                        PutField_CString(hDetail,"echo_msg_1",csTmp);
                }

/* echo_msg_2 */
                if (GetField_CString(hRequest,"echo_msg_2",&csTmp)) {
DEBUGLOG(("AddTxnLog:: echo_msg_2 = [%s]\n",csTmp));
                        PutField_CString(hDetail,"echo_msg_2",csTmp);
		}
/* echo_msg_3 */
                if (GetField_CString(hRequest,"echo_msg_3",&csTmp)) {
DEBUGLOG(("AddTxnLog:: echo_msg_3 = [%s]\n",csTmp));
...
DEBUGLOG(("AddTxnLog:: version_no = [%d]\n",iTmp));
                ...
                PutField_Char(hDetail,"status",PD_PROCESSING);
               	...
               	DBObjPtr = CreateObj(DBPtr,"DBCctTxn","Add");
...
DEBUGLOG(("process_resp_txn()\n"));
        ...
        MsgObjPtr = CreateObj(MsgPtr,"CctMsg","BreakDownRespMsg");
        if ((*MsgObjPtr)(hRequest,inMsg->msg,inMsg->len) != PD_OK)  {
     		iRet = INT_BREAKDOWN_ERR;
                PutField_Int(hContext,"internal_error",iRet);
...
DEBUGLOG(("process_resp_txn: txn_seq = [%s]\n",csTmpBuf));
		...
		if (!GetField_Char(hRequest,"FERESP",&cPtr)) {
      			...
      			TxnObjPtr = CreateObj(TxnPtr,"TxnCctOnUsCCT","Decrypt");
...
ERRLOG("CCTChannel::UpdateRespTxnLog:: return_psp_channel missing!!\n");
		iRet = PD_ERR;
	}
