...
support SengPay					   2012/04/10		   Cody Chan
Add Sub Status					   2012/04/13		   Cody Chan
*/
#include <stdio.h>
#include <stdlib.h>
#include <strings.h>
...
DEBUGLOG(("** PSP_CHANNEL = [%s]\n",csTmpPspChannel));
				    	...
				    	!strcmp(csTmpPspChannel,PD_CHANNEL_EEP)) {
                                       		char* csPspUrl;
						...
						else if (!strcmp(csTmpPspChannel,PD_CHANNEL_EEP))
...
DEBUGLOG(("redirect_url = [%s]\n",csTmpBuf));
				       		...
				       		||!strcmp(csTmpPspChannel,PD_CHANNEL_HPAY_CNP)
				       		||!strcmp(csTmpPspChannel,PD_CHANNEL_GOPAY)
						) {
                                        	char* csPspUrl;
                                        	char* csRequestFunction;
                                        	char  csTmpBuf[PD_TMP_BUF_LEN + 1];
...
DEBUGLOG(("redirect_url = [%s]\n",csTmpBuf));
						...
						else if (!strcmp(csTmpPspChannel,PD_CHANNEL_HPAY_CNP)) 
						...
						else if (!strcmp(csTmpPspChannel,PD_CHANNEL_GOPAY)) 
							MsgObjPtr = CreateObj(MsgPtr,"GpyMsg","BuildData");
						else {
ERRLOG("XPYChannel: Channel PSP[%s] Not Found\n",csTmpPspChannel);
						}
...
DEBUGLOG(("BOSecurity->GenerateHaiPaySign = [%d]\n",iRet));
							FREE_ME(hCon);
						}

                            		}
					else if (!strcmp(csTmpPspChannel,PD_CHANNEL_YEEPAY)) {
                                                char* csPspUrl;
                                                char* csRequestFunction;
                                                char  csTmpBuf[PD_TMP_BUF_LEN + 1];
                                                char* csMethod;
...
DEBUGLOG(("url_method = [%s]\n",csMethod));
                                                ...
                                                if (GetField_CString(hContext,"action",&csMethod)) {
DEBUGLOG(("action = [%s]\n",csMethod));
                                                        PutField_CString(hResponse,"action",csMethod);
                                                }
...
DEBUGLOG(("psp_key = [%s]\n",csTmp));
                                                        ...
                                                        iRet = (unsigned long)(*BOObjPtr)(hCon,hResponse);
DEBUGLOG(("BOSecurity->GenerateYeeSign = [%d]\n",iRet));
                                                        FREE_ME(hCon);
                                                }

                                        }
					else if (!strcmp(csTmpPspChannel,PD_CHANNEL_ALLINPAY)) {
                                                char* csPspUrl;
                                                char* csRequestFunction;
                                                char  csTmpBuf[PD_TMP_BUF_LEN + 1];
                                                char* csMethod;
...
DEBUGLOG(("set psp_channel_code = [%s]\n",csTmpPspChannel));
                                                        PutField_CString(hCon,"psp_channel_code",csTmpPspChannel);
                                                        BOObjPtr = CreateObj(BOPtr,"BOSecurity","GenerateMD5Sign");
                                                        iRet = (unsigned long)(*BOObjPtr)(hCon,hResponse);
DEBUGLOG(("BOSecurity->GenerateAllinPaySign = [%d]\n",iRet));
                                                        FREE_ME(hCon);
                                                }

                                        }
                                        else if (!strcmp(csTmpPspChannel,PD_CHANNEL_SHENGPAY)) {
                                                char* csPspUrl;
                                                char* csRequestFunction;
                                                char  csTmpBuf[PD_TMP_BUF_LEN + 1];
                                                char* csMethod;
...
DEBUGLOG(("DON't NEED to SEND PSP with its formated message\n"));
					}
                   		}
			}
...
DEBUGLOG(("UpdateTxnLog:: psp_id = [%s]\n",csTmp));
                ...
                if (GetField_Double(hContext,"new_dst_txn_amt",&dTmp)) {
...
/* merchant fee */
        if (GetField_Double(hContext,"src_txn_fee",&dTmp) && iRet == PD_OK) {
                ...
                if(dTmp>0){
DEBUGLOG(("AddTxnFeeChgLog:: src txn fee = [%lf]\n",dTmp));
...
DEBUGLOG(("AddTxnFeeChgLog:: dst_txn_fee = [%lf]\n",dTmp));
