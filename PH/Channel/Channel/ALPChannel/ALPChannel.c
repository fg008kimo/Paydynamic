...
DEBUGLOG(("Call WEBChannel process_psp_resp_txn\n"));
                        PutField_CString(hContext,"return_psp_channel",PD_CHANNEL_ALLINPAY);
...
DEBUGLOG(("UpdateContext:NOT RECORD\n"));
                iRet = PD_ERR;
ERRLOG("Internal Error:ALP Channel:SystemControl Record Not Found\n");
        }
...
DEBUGLOG(("process_bounce_txn:: ()\n"));
	...
	MsgObjPtr = CreateObj(MsgPtr,"YpyMsg","DeBlockBounceBackData");
        if ((*MsgObjPtr)(hRequest,inMsg->msg,inMsg->len) != PD_OK)  {
                iRet = INT_BREAKDOWN_ERR;
                PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("process_bounce_txn:BreakDown YpyMsg Error\n"));
        }
        else {
