...
#define	PD_ACK_RESEND	"resend_p2x_rck.sh"
char	cDebug;

void P2XChannel(char	cdebug)
{
	cDebug = cdebug;
}
...
int     process_P2X_bounce_txn(hash_t *hContext,
			   	const hex_t* inMsg,
				hex_t *outMsg);
...
DEBUGLOG(("Authorize() try to VerifyMac()\n"));
			...
			if (iRet != PD_OK) {
DEBUGLOG(("P2XChannel:Authorize() Invalid MAC, txn id [%s] dropped\n",csTxnSeq));
ERRLOG("P2XChannel:Authorize() Invalid MAC, txn id [%s] dropped\n",csTxnSeq);
			}
                }
                else{
...
ERRLOG(("P2XChannel::Authorize() mac not found\n"));
                }


		if (iRet == PD_OK ) {
			MsgObjPtr = CreateObj(MsgPtr,"P2xMsg","initReplyFromRequest");
...
DEBUGLOG(("put failure_url into redirect_url\n"));
					...
					strcat(csTmpBuf,"?merchantid=");
					if (GetField_CString(hResponse,"merchant_id",&csPtr)) {
						strcat(csTmpBuf,csPtr);
					}
					strcat(csTmpBuf,"&message=");
					if (GetField_CString(hResponse,"emsg",&csPtr)) {
						strcat(csTmpBuf,csPtr);
					}
...
ERRLOG("P2XCHANNEL internal error? [%d]\n",iRet);
		}
	}
...
DEBUGLOG(("process_input\n"));
        ...
        if (iRet == PD_OK) {
                TxnObjPtr = CreateObj(TxnPtr,"TxnP2xOnUsCOM","Authorize");
                iRet = (unsigned long)(*TxnObjPtr)(hContext,Request,Response);
        }     

        if (iRet == PD_OK ) {
                csHandler = (char*) malloc (20);
                strcpy(csHandler,"TxnP2xOnUsP2X");
...
DEBUGLOG(("AddTxnLog:: version_no = [%d]\n",iTmp));
                ...
                PutField_Char(hDetail,"status",PD_PROCESSING);
               	...
               	DBObjPtr = CreateObj(DBPtr,"DBP2xTxn","Add");
...
DEBUGLOG(("UpdateTxnLog:: psp_txn_ccy = [%s]\n",csTmp));
                ...
                if (iRet == PD_OK) {
                        DBObjPtr = CreateObj(DBPtr,"DBP2xTxn","Update");
...
DEBUGLOG(("process_resp_txn()\n"));
        ...
        MsgObjPtr = CreateObj(MsgPtr,"P2xMsg","BreakDownRespMsg");
        if ((*MsgObjPtr)(hRequest,inMsg->msg,inMsg->len) != PD_OK)  {
     		iRet = INT_BREAKDOWN_ERR;
                PutField_Int(hContext,"internal_error",iRet);
...
DEBUGLOG(("process_resp_txn: txn_seq = [%s]\n",csTmpBuf));
		...
		if (!GetField_Char(hRequest,"FERESP",&cPtr)) {
      			...
      			TxnObjPtr = CreateObj(TxnPtr,"TxnP2xOnUsP2X","Decrypt");
...
ERRLOG("P2XChannel::UpdateRespTxnLog:: return_psp_channel missing!!\n");
		iRet = PD_ERR;
	}
