...
Add check merchant ref again                       2017/06/06              Virginia Yun
*/
...
DEBUGLOG(("AddTxnLog:: remark = [%s]\n",csPtr));
		...
		if (iRet == PD_OK) {
			...
			if ((unsigned long)(DBObjPtr)(csTxnId, csMerchantId, csMerchantRef) != NOT_FOUND) {
				iRet = INT_DUP_MERCHANT_REF;
				PutField_Int(hContext,"internal_error",INT_DUP_MERCHANT_REF);
ERRLOG("AddTxnLog:[OPL] - MerchantId[%s][%s]Duplicate Merchant Ref\n",csMerchantId,csMerchantRef);
DEBUGLOG(("AddTxnLog:[OPL] - MerchantId[%s][%s]Duplicate Merchant Ref\n",csMerchantId,csMerchantRef));
			}
		}
...
ERRLOG("OPLChannel::AddTxnLog:AddDetail Error!!\n");
			}
		}
		if (iRet == PD_OK || iRet == INT_DUP_MERCHANT_REF) {
			csHandler = (char*) malloc (20);
...
ERRLOG("OPLChannel::AddTxnLog: %s Error!!\n,csHandler");
			}
		}
