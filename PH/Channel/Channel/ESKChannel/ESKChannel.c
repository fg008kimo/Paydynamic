...
-------------------------------                    ------------            --------------
Init Version                                       2011/02/24              Cody Chan
*/
...
DEBUGLOG(("in msg = [%d]\n",inMsg->len));
                        PutField_CString(hContext,"return_psp_channel",PD_CHANNEL_ESKY);
...
DEBUGLOG(("UpdateContext:NOT RECORD\n"));
                iRet = PD_ERR;
ERRLOG("Internal Error:ESK Channel:SystemControl Record Not Found\n");
        }
...
DEBUGLOG(("process_bounce_txn:: ()\n"));
	...
	MsgObjPtr = CreateObj(MsgPtr,"ESkyMsg","DeBlockBounceBackData");
        if ((*MsgObjPtr)(hRequest,inMsg->msg,inMsg->len) != PD_OK)  {
                iRet = INT_BREAKDOWN_ERR;
                PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("process_bounce_txn:BreakDown ESKMsg Error\n"));
        }
        else {
