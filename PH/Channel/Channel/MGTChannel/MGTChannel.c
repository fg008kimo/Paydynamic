...
Add Log Txn Detail				   2011/07/22		   Cody Chan
Add MMS Handling                                   2011/10/13              Virginia Yun 
Add Auto-Settlement Email                          2013/08/29              Virginia Yun
Add RTF Email					   2014/02/07              Virginia Yun
Update Send Settlement Email			   2014/11/11		   Elvis Wong
Add Support for MMS				   2015/06/10		   Cody Chan
Update Send Settlement Email			   2015/07/31		   Elvis Wong
Add INT_EML_EMAIL_ADDR_NOT_REG Checking            2015/08/20              Elvis Wong
Add txncode to allow AddRemarkLog		   2015/09/04		   Dirk Wong
*/
...
DEBUGLOG(("Result_Code = %s\n",csBuf));
		...
		else if(!strcmp(csTxnCode,PD_AVA_PAYOUT_REQ_TF_FROM)){
			PutField_Char(hContext,"status",PD_PROCESSING);
		}
		else{
...
DEBUGLOG(("*****Format Out Reject Message\n"));
			...
			if(!strcmp(csTxnCode,PD_AVA_PAYOUT_APPROVE_TF_FROM) ||
			   ...
			   !strcmp(csTxnCode,PD_SETTLEMENT_RETURN)){
				if(iInternalErr == INT_INVALID_TXN ||
			 	   iInternalErr == INT_EML_EMAIL_ADDR_NOT_REG ||		 
		...
		if (iRet == PD_OK && iSkipUpdate==PD_FALSE) {
			...
			if (iRet == PD_OK) {
				if (!strcmp(csTxnCode, PD_SETTLEMENT_REQUEST) ||
					...
					PutField_CString(hContext, "rmk_txn_code", PD_SETTLEMENT_REQUEST);
...
DEBUGLOG(("Create Approve Auto_Sett Notification Email!\n"));

                                                                        	/* Need to commit before sent email! */
                                                                        	TxnCommit();
                                                                        	sprintf(csCmd, "send_auto_sett_email_to_merch.sh %s %s %s %c %s %d", csSettTxnSeq, csMerchantId, PD_EML_FUNCT_SETT_AUTO, cSettType, csSettService, iSendNotifyEmail);
                                                                        	system(csCmd);
                                                                	} else {
...
DEBUGLOG(("Create Approve Admin Request Settlement Email!\n"));

												/* Need to commit before sent email! */
                                                                                        	TxnCommit();
                                                                                        	sprintf(csCmd, "send_auto_sett_email_to_merch.sh %s %s %s %c %s %d", csSettTxnSeq, csMerchantId, PD_EML_FUNCT_SETT_ADMIN, cSettType, csSettService, iSendNotifyEmail);
                                                                                        	system(csCmd);
                                                                                	} else if (!strcmp(csAdminSubStatus, PD_MERCHANT_REQUESTED)) {
DEBUGLOG(("Create Approve Merchant Request Settlement Email!\n"));
										
												/* Need to commit before sent email! */
                                                                                        	TxnCommit();
                                                                                        	sprintf(csCmd, "send_auto_sett_email_to_merch.sh %s %s %s %c %s %d", csSettTxnSeq, csMerchantId, PD_EML_FUNCT_SETT_MERCH, cSettType, csSettService, iSendNotifyEmail);
                                                                                        	system(csCmd);
											}
                                                                                }
                                                                        }
				...
				/* Handle RTF , Weekly Payout Reserved Amt Admin Alert Email */
				if (iRet == PD_OK) {
					char	cStatus;
					char	*csEmailTxnSeq;
					char    *csCmd = (char *) malloc(PD_MAX_FILE_LEN + 1);
					...
					hash_t  *hEmailTxn;
					hEmailTxn = (hash_t*)  malloc (sizeof(hash_t));
					hash_init(hEmailTxn,0);

		                        if (!strcmp(csTxnCode, PD_AVA_PAYOUT_REQ_TF_FROM) && (iInternalErr == 0)) {
						if (GetField_Char(hContext, "status", &cStatus)) {
							if (cStatus == PD_PROCESSING) {
								PutField_CString(hEmailTxn, "type", "T");
								PutField_CString(hEmailTxn, "ema_funct_type", PD_EMA_FUNCT_REQ_RTF);

								if (GetField_CString(hContext, "txn_seq", &csEmailTxnSeq)) {
									PutField_CString(hEmailTxn, "txn_seq", csEmailTxnSeq);

									/* Need to commit before sent email! */
                                                                        TxnCommit();
									TxnObjPtr = CreateObj(TxnPtr,"TxnMgtByUsEMA","Authorize");

									if ((*TxnObjPtr)((const hash_t*)hEmailTxn,hEmailTxn,hEmailTxn) == PD_OK) {
DEBUGLOG(("Send Admin Alert Email! [%s]\n", csTxnCode));
									} else {
DEBUGLOG(("Send Admin Alert Email Fail! [%s]\n", csTxnCode));
ERRLOG("MGTCHANNEL Send Admin Alert Email Fail!\n");
									}
								}
							}
						}
					...
					if (!strcmp(csTxnCode, PD_UPDATE_WEEKLY_RESERVED_AMT) && (iInternalErr == 0)) {
						char	*csEmlAmtType = NULL;
						char	*csEmlStatus = NULL;

						// amt_type: "R"; status : "P"
						if (GetField_CString(hRequest, "amt_type", &csEmlAmtType)) {
DEBUGLOG(("Check Weekly Reserve Amt Request amt_type = [%s]\n", csEmlAmtType));
						}

						
						if (GetField_CString(hRequest, "status", &csEmlStatus)) {
DEBUGLOG(("Check Weekly Reserve Amt Request status = [%s]\n", csEmlStatus));
						}

						if (!strcmp(csEmlAmtType, "R") && !strcmp(csEmlStatus, "P")) {
							PutField_CString(hEmailTxn, "type", "A");
							PutField_CString(hEmailTxn, "ema_funct_type", PD_EMA_FUNCT_REQ_PO_RA);

							if (GetField_CString(hRequest, "merchant_id", &csEmlPtr)) {
DEBUGLOG(("Check Weekly Reserve Amt Request merchant_id = [%s]\n", csEmlPtr));
								PutField_CString(hEmailTxn, "merchant_id", csEmlPtr);
							}
							if (GetField_CString(hRequest, "txn_country", &csEmlPtr)) {
DEBUGLOG(("Check Weekly Reserve Amt Request country = [%s]\n", csEmlPtr));
								PutField_CString(hEmailTxn, "txn_country", csEmlPtr);
							}
							if (GetField_CString(hRequest, "service_code", &csEmlPtr)) {
DEBUGLOG(("Check Weekly Reserve Amt Request service = [%s]\n", csEmlPtr));
								PutField_CString(hEmailTxn, "service_code", csEmlPtr);
							}
							if (GetField_CString(hRequest, "txn_ccy", &csEmlPtr)) {
DEBUGLOG(("Check Weekly Reserve Amt Request txn_ccy = [%s]\n", csEmlPtr));
								PutField_CString(hEmailTxn, "txn_ccy", csEmlPtr);
							}
...
DEBUGLOG(("Call TxnMgtbyUs2Host\n"));

                                                       		TxnObjPtr = CreateObj(TxnPtr,"TxnMgtByUs2Host","Authorize");
...
DEBUGLOG(("MMS Mode is Disabled\n"));
                                	...
                                	if (iRet != PD_OK) {
DEBUGLOG(("TxnMgtByUs2Host error\n"));
                                	}
				}
...
ERRLOG("MGTCHANNEL internal error? [%d]\n",iRet);
        }
...
DEBUGLOG(("UpdateContext::node id = [%s]\n",csValue));
						...
						if (!strcmp(csCode, PD_MINI_MMM_ENABLE)) {
DEBUGLOG(("UpdateContext::Mini-MMM Mode= [%s]\n",csValue));
							PutField_CString(hContext,"mini_mmm_mode",csValue);
                                                }
/*
						if (!strcmp(csCode,"VPFEE")) {
DEBUGLOG(("UpdateContext::Void Payout With Fee Return= [%s]\n",csValue));
							PutField_CString(hContext,"VPFEE",csValue);
						}
*/
					}
                                }
...
ERRLOG("MGTChannel::Unable to find based ccy\n");
                        ...
                        if (!GetField_CString(hContext, "mms_mode", &csMmsMode)) {
ERRLOG("MGTChannel::Unable to find mms_mode and set diabled\n");
                                ...
                                if (!strcmp(csMmsMode, PD_ENABLE_MMSMODE)) {
                                        if (!GetField_CString(hContext, "node_id", &csNodeId)){ 
                                                iRet = INT_ERR;
ERRLOG("MGTChannel::Unable to find node_id while MMS Mode is enabled \n");
                                        } 
                                } 
                        }
                }
...
DEBUGLOG(("process_input_txn:txn_code not found\n"));
        ...
        if (iRet == PD_OK) {
                TxnObjPtr = CreateObj(TxnPtr,"TxnMgtByUsCOM","Authorize");
                iRet = (unsigned long)(*TxnObjPtr)(hContext,Request,Response);
        }

	if (iRet == PD_OK ) {
                csHandler = (char*) malloc (20);
                sprintf(csHandler,"TxnMgtByUs%s",csTxnCode);
...
DEBUGLOG(("UpdateTxnDetailLog::remark = [%s]\n",csTmp));
