...
Fix Now3Pay					   2018/05/16		   LokMan Chow
...
DEBUGLOG(("Format out psp_channel_code = [%s]\n",csPspChannel));
							if (!strcmp(csPspChannel,PD_CHANNEL_WECHATPAY)
								...
								|| !strcmp(csPspChannel,PD_CHANNEL_SWIFTPAY_QR)
...
DEBUGLOG(("client_ip = [%s]\n", csPtr));
								...
								if (!strcmp(csPspChannel,PD_CHANNEL_PAYESCAPE_VNET)){
									if (GetField_CString(hContext, "psp_key", &csPtr)) {
										PutField_CString(hResponse, "psp_key", csPtr);
									}
									if (GetField_CString(hContext, "org_ip_addr", &csPtr)) {
										PutField_CString(hResponse, "user_ip", csPtr);
									}
								...
								if (!strcmp(csPspChannel,PD_CHANNEL_OUP_EC)){
									if (GetField_CString(hContext, "psp_key_id", &csPtr)) {
										PutField_CString(hResponse, "cert_id", csPtr);
									}
									if (GetField_CString(hContext, "psp_key_uid", &csPtr)) {
										PutField_CString(hResponse, "version", csPtr);
									}
								}

								if (!strcmp(csPspChannel,PD_CHANNEL_GOLDENPAY_EC)){
									if (GetField_CString(hContext, "psp_key_uid", &csPtr)) {
										PutField_CString(hResponse, "psp_key_uid", csPtr);
									}
									if (GetField_CString(hContext, "customer_tag", &csPtr)) {
										PutField_CString(hResponse, "customer_tag", csPtr);
									}
								...
								if (!strcmp(csPspChannel,PD_CHANNEL_OUP_OPENEPAY_EC)){
									if (GetField_CString(hContext, "psp_key_uid", &csPtr)) {
										PutField_CString(hResponse, "psp_key_uid", csPtr);
									}
								}
...
/* PayLink-HXPay VNET */
		...
		MsgObjPtr = CreateObj(MsgPtr,"HxvMsg","BreakDownMsg");
	}
/* PayLink-HXPay QR*/
	else if (!strcmp(csReturnPspChannel,PD_CHANNEL_HXPAY_QR)) {
		MsgObjPtr = CreateObj(MsgPtr,"HxqMsg","BreakDownMsg");
	}
/* PAYESCAPE-Epaylinks VNET */
	else if (!strcmp(csReturnPspChannel,PD_CHANNEL_PAYESCAPE_VNET)) {
		MsgObjPtr = CreateObj(MsgPtr,"PsvMsg","BreakDownMsg");
	}
/* SAMOPAY-SMPay EC */
...
DEBUGLOG(("process_psp_resp_txn::GetTxnPsp - txn amt = [%lf]\n",dOrgAmt));
							...
							if(GetField_CString(hRequest,"txn_amt",&csPtr)){
								if (!strcmp(csReturnPspChannel,PD_CHANNEL_TWV) || 
								    ...
								    !strcmp(csReturnPspChannel,PD_CHANNEL_ZPAY_EC) ||
								    ...
								    !strcmp(csReturnPspChannel,PD_CHANNEL_UNIPAY_QR) ||
...
DEBUGLOG(("process_psp_resp_txn::VerifySign \n"));
						BOObjPtr = CreateObj(BOPtr,"BOSecurity","VerifyPspSign");
                                		iRet = (unsigned long)(*BOObjPtr)(hContext,hRequest,hRequest);
						if (iRet != PD_OK) {
...
ERRLOG("WEBChannel::UpdateRespTxnLog:: return_psp_channel missing!!\n");
	...
	if(!strcmp(csPspChannelCode,PD_CHANNEL_ECPSS)||
	   ...
	   !strcmp(csPspChannelCode,PD_CHANNEL_SAMOPAY_EC) ||
	   !strcmp(csPspChannelCode,PD_CHANNEL_RFPAY_QR) ||
	   !strcmp(csPspChannelCode,PD_CHANNEL_RFPAY_VNET) ||
	   !strcmp(csPspChannelCode,PD_CHANNEL_OUP_EC) ||
	   !strcmp(csPspChannelCode,PD_CHANNEL_ZPAY_EC) ||
	   !strcmp(csPspChannelCode,PD_CHANNEL_NOW3PAY) ||
	   !strcmp(csPspChannelCode,PD_CHANNEL_TTPAY) ||
	   !strcmp(csPspChannelCode,PD_CHANNEL_TTPAY_QR) ||
	   !strcmp(csPspChannelCode,PD_CHANNEL_ZPAY_QR) ||
	   !strcmp(csPspChannelCode,PD_CHANNEL_XJ_DBAO) ||
	   !strcmp(csPspChannelCode,PD_CHANNEL_TRUSTPAY_EC) ||
	   !strcmp(csPspChannelCode,PD_CHANNEL_XJ_DBAO_QR) ||
	   !strcmp(csPspChannelCode,PD_CHANNEL_XJ_CH) ||
	   !strcmp(csPspChannelCode,PD_CHANNEL_XJ_CH_QR) ||
	   !strcmp(csPspChannelCode,PD_CHANNEL_NOW3PAY_QR) ||
	   !strcmp(csPspChannelCode,PD_CHANNEL_TRUSTPAY_QR) ||
	   !strcmp(csPspChannelCode,PD_CHANNEL_ZXPAY_EC) ||
	   !strcmp(csPspChannelCode,PD_CHANNEL_ZXPAY_QR) ||
	   !strcmp(csPspChannelCode,PD_CHANNEL_HANDPAY_VNET)
	){
