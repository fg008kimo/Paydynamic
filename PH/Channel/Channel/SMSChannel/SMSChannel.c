...
//Response Process Type
	...
	if (iRet == PD_OK) {
		// send unknown debit txn alert
		char *csTmp;
		GetField_CString(hContext, "req_status", &csTmp);
DEBUGLOG(("process_txn:req_status = [%s]\n", csTmp));

		if (!strcmp(csTmp, "ALERT_ONLY")) {
			GetField_CString(hContext, "stat_txn_id", &csTmp);
DEBUGLOG(("process_txn:stat_txn_id = [%s]\n", csTmp));
			char csCmd[PD_MAX_FILE_LEN + 1];
			TxnCommit();
			sprintf(csCmd, "gen_sms_unknown_debit_txn_alert_new.sh \"%s\"", csTmp);
DEBUGLOG(("process_txn:csCmd = [%s]\n", csCmd));
			system(csCmd);
		}
	}
