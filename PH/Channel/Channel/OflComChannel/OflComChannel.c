...
-------------------------------                    ------------            --------------
 ...
 Add bank_code
     ...
     reg_mob_no
     bank_upd_ts
     baid_upd_ts in UpdateNonTxnMsgLog 		   2019/08/22		   Elvis Wong
*/
...
DEBUGLOG(("doResponse() exit iRet = [%d]\n", iRet));
	return iRet;
}
...
DEBUGLOG(("enBlock:: iRet = [%d] from [%s]:FormatMsg\n",iRet, csHandler));
        }

        FREE_ME(csHandler);
DEBUGLOG(("enBlock:: iRet = [%d]\n",iRet));
        return iRet;
}
...
DEBUGLOG(("deBlock:: csHandler[%s]\n",csHandler));
        ...
        if (iRet == PD_OK ) {
DEBUGLOG(("deBlock:: To call [%s]::initReplyFromRequest\n",csHandler));
...
DEBUGLOG(("AddTxnReqRef:: exit iRet = [%d]\n",iRet));
                return iRet;
        }
...
DEBUGLOG(("UpdateTxnReqRef() exit iRet = [%d]\n",iRet));
	return iRet;
}
...
DEBUGLOG((" is non txn = [%d]\n", iNonTxn));

        if (iNonTxn !=  PD_TRUE) {
DEBUGLOG(("AddNonTxnMsgLog:: exit iRet = [%d]\n",iRet));
                return iRet;
        }
...
/* direction */
			PutField_Char(hTxn, "direction", PD_TXN_REQUEST);
...
/* req_dt */
                        if (GetField_CString(hRequest, "req_dt", &csPtr)) {
                                sprintf(csKey, "tag_%d", iCnt);
                                PutField_CString(hTxn, csKey, "req_dt");
                                sprintf(csKey, "value_%d", iCnt);
                                PutField_CString(hTxn, csKey, csPtr);
                                iCnt++;
                        }
...
/* bank_code */
                        if (GetField_CString(hRequest, "bank_code", &csPtr)) {
                                sprintf(csKey, "tag_%d", iCnt);
                                PutField_CString(hTxn, csKey, "bank_code");
                                sprintf(csKey, "value_%d", iCnt);
                                PutField_CString(hTxn, csKey, csPtr);
                                iCnt++;
                        }
...
/* acct_num */
                        if (GetField_CString(hRequest, "acct_num", &csPtr)) {
                                sprintf(csKey, "tag_%d", iCnt);
...
ERRLOG("OflComChannel::AddNonTxnMsgLog:: tag not found!!\n");
				...
				if (iRet == PD_OK) {
					...
					if (j == 0) {
DEBUGLOG((" Call DBOLNonTxnMsgDetail:: Add\n"));
					}
...
/* direction */
               	PutField_Char(hTxn, "direction", PD_TXN_RESPONSE);
DEBUGLOG((" direction = [%c]\n", PD_TXN_RESPONSE));
...
/* pid_nature */
                if (GetField_CString(hResponse, "pid_nature", &csPtr)) {
                        sprintf(csKey, "tag_%d", iCnt);
                        PutField_CString(hTxn, csKey, "pid_nature");
                        sprintf(csKey, "value_%d", iCnt);
                        PutField_CString(hTxn, csKey, csPtr);
                        iCnt++;
                }
...
/* bank_code */
                if (GetField_CString(hResponse, "bank_code", &csPtr)) {
                        sprintf(csKey, "tag_%d", iCnt);
                        PutField_CString(hTxn, csKey, "bank_code");
                        sprintf(csKey, "value_%d", iCnt);
                        PutField_CString(hTxn, csKey, csPtr);
...
/* bank_abbrev */
                if (GetField_CString(hResponse, "bank_abbrev", &csPtr)) {
                        sprintf(csKey, "tag_%d", iCnt);
                        PutField_CString(hTxn, csKey, "bank_abbrev");
                        sprintf(csKey, "value_%d", iCnt);
                        PutField_CString(hTxn, csKey, csPtr);
                        iCnt++;
                }
...
/* province */
                if (GetField_CString(hResponse, "province", &csPtr)) {
                        sprintf(csKey, "tag_%d", iCnt);
                        PutField_CString(hTxn, csKey, "province");
                        sprintf(csKey, "value_%d", iCnt);
                        PutField_CString(hTxn, csKey, csPtr);
                        iCnt++;
                }
...
/* city */
                if (GetField_CString(hResponse, "city", &csPtr)) {
                        sprintf(csKey, "tag_%d", iCnt);
                        PutField_CString(hTxn, csKey, "city");
                        sprintf(csKey, "value_%d", iCnt);
                        PutField_CString(hTxn, csKey, csPtr);
                        iCnt++;
                }
...
/* reg_mob_no */
                if (GetField_CString(hResponse, "reg_mob_no", &csPtr)) {
                        sprintf(csKey, "tag_%d", iCnt);
                        PutField_CString(hTxn, csKey, "reg_mob_no");
                        sprintf(csKey, "value_%d", iCnt);
                        PutField_CString(hTxn, csKey, csPtr);
                        iCnt++;
                }
...
/* r_list */
				...
				sprintf(csListKey, "r_list.%d.pid_nature", i);
                                if (GetField_CString(hResponse, csListKey, &csPtr)) {
                                        sprintf(csKey, "tag_%d", iCnt);
                                        PutField_CString(hTxn, csKey, csListKey);
                                        sprintf(csKey, "value_%d", iCnt);
				...
				sprintf(csListKey, "r_list.%d.baid_upd_ts", i);
                                if (GetField_CString(hResponse, csListKey, &csPtr)) {
                                        sprintf(csKey, "tag_%d", iCnt);
                                        PutField_CString(hTxn, csKey, csListKey);
                                        sprintf(csKey, "value_%d", iCnt);
				...
				sprintf(csListKey, "r_list.%d.bank_code", i);
                                if (GetField_CString(hResponse, csListKey, &csPtr)) {
                                        sprintf(csKey, "tag_%d", iCnt);
                                        PutField_CString(hTxn, csKey, csListKey);
                                        sprintf(csKey, "value_%d", iCnt);
				...
				sprintf(csListKey, "r_list.%d.bank_abbrev", i);
                                if (GetField_CString(hResponse, csListKey, &csPtr)) {
                                        sprintf(csKey, "tag_%d", iCnt);
                                        PutField_CString(hTxn, csKey, csListKey);
                                        sprintf(csKey, "value_%d", iCnt);
				...
				sprintf(csListKey, "r_list.%d.province", i);
                                if (GetField_CString(hResponse, csListKey, &csPtr)) {
                                        sprintf(csKey, "tag_%d", iCnt);
                                        PutField_CString(hTxn, csKey, csListKey);
                                        sprintf(csKey, "value_%d", iCnt);
				...
				sprintf(csListKey, "r_list.%d.city", i);
                                if (GetField_CString(hResponse, csListKey, &csPtr)) {
                                        sprintf(csKey, "tag_%d", iCnt);
                                        PutField_CString(hTxn, csKey, csListKey);
                                        sprintf(csKey, "value_%d", iCnt);
				...
				sprintf(csListKey, "r_list.%d.reg_mob_no", i);
                                if (GetField_CString(hResponse, csListKey, &csPtr)) {
                                        sprintf(csKey, "tag_%d", iCnt);
                                        PutField_CString(hTxn, csKey, csListKey);
                                        sprintf(csKey, "value_%d", iCnt);
				...
				sprintf(csListKey, "r_list.%d.bank_upd_ts", i);
                                if (GetField_CString(hResponse, csListKey, &csPtr)) {
                                        sprintf(csKey, "tag_%d", iCnt);
                                        PutField_CString(hTxn, csKey, csListKey);
                                        sprintf(csKey, "value_%d", iCnt);
...
DEBUGLOG(("UpdateNonTxnMsgLog() exit iRet = [%d]\n",iRet));
        return iRet;
}
...
DEBUGLOG((" status = [%c]\n", PD_PROCESSING));
		...
		if (iRet == PD_OK) {
			DBObjPtr = CreateObj(DBPtr,"DBOLTransaction","Add");
                	iRet = (unsigned long) ((*DBObjPtr)(hTxn));
			if (iRet != PD_OK) {
DEBUGLOG((" Call DBOLTransaction::Add Failure!!\n"));
ERRLOG("OflComChannel::AddTxnLog::Call DBOLTransaction::Add Failure!!\n");
		...
		if (iRet == PD_OK) {
			...
			iRet = (unsigned long) ((*DBObjPtr)(hTxn));
			if (iRet != PD_OK) {
DEBUGLOG((" Call DBOLTransaction::AddDetail Failure!!\n"));
...
DEBUGLOG((" unix_approval_timestamp = [%ld]\n",lPtr));
...
DEBUGLOG((" member_id = [%s]\n",csPtr));
				PutField_CString(hTxn,"member_id",csPtr);
                        ...
                        iRet = (unsigned long)(*DBObjPtr)(hTxn);
			if (iRet != PD_OK) {
DEBUGLOG((" Call DBOLTransaction::UpdateDetail Failure!!\n"));
...
DEBUGLOG(("UpdateTxnLog() exit iRet = [%d]\n",iRet));
        return  iRet;
}
