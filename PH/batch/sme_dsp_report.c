...
static unsigned int sqlctx = 10490299;
...
-------------------------------                    ------------            --------------
Init Version                                       2018/03/08              David Wong
*/
...
void FormatTxnSeq(char *csInTxnSeq, char *csOutTxnSeq);
...
DEBUGLOG(("TXN_SEQ=[%s]\n",csList[IDX_TXN_SEQ]));
		FormatTxnSeq(csList[IDX_TXN_SEQ],csTxnSeq);
DEBUGLOG(("TID=[%s]\n",csList[IDX_TID]));
...
/* Field #0 txn_seq */
		fprintf(fout,"%s%s",csTxnSeq,PD_AMT_DELIMITOR);
...
void FormatTxnSeq(char *csInTxnSeq, char *csOutTxnSeq)
{
	int iTxnSeqLen = (int)strlen(csInTxnSeq);
	csOutTxnSeq[0]='\0';

	while (iTxnSeqLen < PD_TXN_SEQ_LEN) {
		strcat(csOutTxnSeq, "0");
		iTxnSeqLen++;
	}

	strcat(csOutTxnSeq, csInTxnSeq);
}
