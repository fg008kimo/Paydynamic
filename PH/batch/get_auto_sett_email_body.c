...
{
    27,
    "get_auto_sett_email_body.pc"
};


static unsigned int sqlctx = 1322437763;
...
{12,4130,871,0,0,
5,0,0,1,901,0,4,210,0,0,12,1,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,
...
Update                                             2014/11/25              Elvis Wong
Add send_notify_email				   2015/07/31		   Elvis Wong	
Modify to support NBXA Mobile			   2017/07/18		   Dirk Wong
*/
...
int process_get_sett_data(hash_t* hRec, const char *csTxnSeq);
...
int batch_proc(int argc, char* argv[])
{
	...
	if(iRet == PD_OK){
        ...
        if(iRet == PD_OK){
                iRet = process_get_sett_data(hRec, cs_txn_seq);
        }
...
ERRLOG("get_auto_sett_email_body::batch_proc::BOAlertEmail Failed, iRet=%d\n", iRet);
                } else {
                        iRet = SUCCESS;
...
DEBUGLOG(("process_get_sett_data txn_seq = [%.*s]\n",hv_txn_seq.len,hv_txn_seq.arr));

        sprintf(csTmp,"%.*s",hv_txn_seq.len,hv_txn_seq.arr);
        iDynCnt = set_tpl_dyn_cstring(hRec,iDynCnt,"TXN_ID","GLO","STR",csTmp);	

	/* EXEC SQL
                SELECT  NVL(BRAND_NAME, (SELECT SCB_BRAND_NAME FROM DEF_SERVICE_BRAND_NAME WHERE SCB_CODE = SC_CODE)) BRAND_NAME,
                        SD_MERCHANT_ID,
                        SHORT_NAME,
                        ...
                        SC_DESC,
                        SD_REQUEST_CCY,
                        TO_CHAR(NVL(SD_REQUEST_AMT, 0), 'FM999,999,990.00'),
                        SD_DELIVER_CCY,
                        TO_CHAR(NVL(SD_DELIVER_AMT, 0), 'FM999,999,990.00')
                ...
                FROM    COUNTRY,
                        ...
                        TXN_HEADER
                WHERE   TH_TXN_ID = :hv_txn_seq
                AND     TH_TXN_CODE = 'STR'
                AND     TH_STATUS = 'C'
                AND     TH_AR_IND = 'A'
                AND     TH_TXN_ID = SD_TXN_ID
                AND     TH_MERCHANT_ID = SD_MERCHANT_ID
...

{
 ...
 sqlstm.stmt = "select NVL ( BRAND_NAME , ( SELECT SCB_BRAND_NAME FROM DEF_\
SERVICE_BRAND_NAME WHERE SCB_CODE = SC_CODE ) ) BRAND_NAME , SD_MERCHANT_ID ,\
 SHORT_NAME , TO_CHAR ( TH_CREATE_TIMESTAMP , 'YYYY-MM-DD HH24:MI:SS' ) , TO_\
CHAR ( TH_APPROVAL_TIMESTAMP , 'YYYY-MM-DD HH24:MI:SS' ) , COUNTRY_NAME , SC_\
DESC , SD_REQUEST_CCY , TO_CHAR ( NVL ( SD_REQUEST_AMT , 0 ) , 'FM999,999,990\
.00' ) , SD_DELIVER_CCY , TO_CHAR ( NVL ( SD_DELIVER_AMT , 0 ) , 'FM999,999,9\
90.00' ) INTO :b0:b1 , :b2:b3 , :b4:b5 , :b6:b7 , :b8:b9 , :b10:b11 , :b12:b1\
3 , :b14:b15 , :b16:b17 , :b18:b19 , :b20:b21 FROM COUNTRY , DEF_SERVICE_CODE\
 , MERCH_DETAIL , MERCHANT_SETTLEMENT_DETAIL , TXN_HEADER WHERE TH_TXN_ID = :\
b22 AND TH_TXN_CODE = 'STR' AND TH_STATUS = 'C' AND TH_AR_IND = 'A' AND TH_TX\
N_ID = SD_TXN_ID AND TH_MERCHANT_ID = SD_MERCHANT_ID AND SD_MERCHANT_ID = MER\
CHANT_ID AND SD_COUNTRY_ID = COUNTRY_CODE AND SD_SERVICE_CODE = SC_CODE ";
...
DEBUGLOG(("process_get_sett_data brand_name = [%s]\n",v_brand_name.arr));
...
DEBUGLOG(("process_get_sett_data merchant_id = [%s]\n",v_merchant_id.arr));
...
DEBUGLOG(("process_get_sett_data short_name = [%s]\n",v_short_name.arr));
...
DEBUGLOG(("process_get_sett_data request_datetime = [%s]\n",v_request_datetime.arr));
...
DEBUGLOG(("process_get_sett_data approve_datetime = [%s]\n",v_approve_datetime.arr));
...
DEBUGLOG(("process_get_sett_data country_name = [%s]\n",v_country_name.arr));
...
DEBUGLOG(("process_get_sett_data service_desc = [%s]\n",v_service_desc.arr));
...
DEBUGLOG(("process_get_sett_data request_ccy = [%s]\n",v_request_ccy.arr));
...
DEBUGLOG(("process_get_sett_data request_amt = [%f]\n",v_request_amt));
...
DEBUGLOG(("process_get_sett_data deliver_ccy = [%s]\n",v_deliver_ccy.arr));
...
DEBUGLOG(("process_get_sett_data deliver_amt = [%f]\n",v_deliver_amt));
...
DEBUGLOG(("process_get_sett_data total_dyn = [%d]\n",iDynCnt));

DEBUGLOG(("process_get_sett_data() Normal Exit iRet = [%d]\n",iRet));
        return iRet;

processemail_error:
DEBUGLOG(("processemail_error code %d\n", sqlca.sqlcode));
DEBUGLOG(("\n%.*s\n", sqlca.sqlerrm.sqlerrml, sqlca.sqlerrm.sqlerrmc));
ERRLOG("get_auto_sett_email_body::process_get_sett_data sql error %d\n", sqlca.sqlcode);
...
DEBUGLOG(("process_get_exclude_data:: service_code = [%s]\n",csSettService));
	...
	if (!strcmp(csFunct, PD_EML_FUNCT_SETT_ADMIN))
        {
/* notify_email */
                PutField_Int(hRec,"notify_email",iSendNotifyEmail);
DEBUGLOG(("process_get_exclude_data:: notify_email = [%d]\n",iSendNotifyEmail));
        }
        else if (!strcmp(csFunct, PD_EML_FUNCT_SETT_MERCH))
        {
DEBUGLOG(("process_get_exclude_data:: Call DBMerchantSettlementDetail:: GetSettlementDetail\n"));
...
ERRLOG("get_auto_sett_email_body::process_get_exclude_data::GetSettlementRecords::GetSettlementDetail::not found record!!\n");
                }
        }
