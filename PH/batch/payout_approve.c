...
static unsigned int sqlctx = 10169731;
...
{12,4130,871,0,0,
5,0,0,0,0,0,56,372,0,0,1,1,0,1,0,3,102,0,0,
24,0,0,1,154,0,6,379,0,0,4,4,0,1,0,2,3,0,0,1,9,0,0,1,3,0,0,2,102,0,0,
55,0,0,2,258,0,6,394,0,0,8,8,0,1,0,2,3,0,0,1,9,0,0,1,3,0,0,1,3,0,0,1,9,0,0,1,4,
0,0,1,4,0,0,2,102,0,0,
102,0,0,3,141,0,6,409,0,0,4,4,0,1,0,2,3,0,0,1,9,0,0,1,3,0,0,2,102,0,0,
133,0,0,0,0,0,13,431,0,0,3,1,0,1,0,1,102,0,0,2,9,0,0,2,3,0,0,
160,0,0,0,0,0,15,485,0,0,1,1,0,1,0,1,102,0,0,
179,0,0,0,0,0,78,486,0,0,1,1,0,1,0,3,102,0,0,
198,0,0,0,0,0,15,496,0,0,1,1,0,1,0,1,102,0,0,
217,0,0,0,0,0,78,497,0,0,1,1,0,1,0,3,102,0,0,
236,0,0,4,902,0,4,583,0,0,30,2,0,1,0,2,9,0,0,2,3,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
...
371,0,0,5,352,0,4,882,0,0,7,3,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,1,9,0,0,1,
3,0,0,1,3,0,0,
414,0,0,6,371,0,4,965,0,0,6,2,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,1,9,0,0,1,
3,0,0,
};
...
Enhancement					   2014/11/03		   LokMan Chow
*/
...
ERRLOG("payout_approve: Authorize::Unable to find CTPHDATE\n");
                }
...
DEBUGLOG(("Authorize::total_amount = [%lf]\n",dTotalAmt));
	...
	GetField_Int(hContext,"total_count",&iCnt);
...
ERRLOG("Authorize::DBMerchantUploadFileHeader:Update Failed\n");
		...
		if(iRet == PD_OK && c_type == PD_BY_SELECTED_TXN){
			int iBatchCnt = 0;
			DBObjPtr = CreateObj(DBPtr,"DBPayoutApproveTmp","GetOrgBatchId");
...
ERRLOG("payout_approve: Authorize::the file is not for approve!!\n");
                                        }
                                        else{
                                                hash_init(hTxn,0);
                                                ...
                                                if((unsigned long) ((*DBObjPtr)(hTxn))!=PD_OK){
                                                        iRet = INT_ERR;
DEBUGLOG(("Authorize::DBMerchantUploadFileHeader:Update Failed\n"));
ERRLOG("payout_approve: Authorize::DBMerchantUploadFileHeader:Update Failed\n");
                                                }
                                        }
                                }
                        }
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "begin :hv_return_value := sp_merch_upload_dt_get_palist ( \
:hv_batch_id:ind_in_batch_id , :hv_pre_approve_id:ind_pre_approve_id , :c_cur\
sor_dtcon ) ; END ;";
  sqlstm.iters = (unsigned int  )1;
...

{
                 ...
                 sqlstm.sqladtp = &sqladt;
                 sqlstm.sqltdsp = &sqltds;
                 sqlstm.stmt = "begin :hv_return_value := sp_merch_upload_d\
t_get_seqlist ( :hv_batch_id:ind_in_batch_id , :hv_last_seq:ind_last_seq , :h\
v_status:ind_in_status , :hv_in_bank_name:ind_in_bank_name , :hv_min_amt:ind_\
min_amt , :hv_max_amt:ind_max_amt , :c_cursor_dtcon ) ; END ;";
                 sqlstm.iters = (unsigned int  )1;
			...
			/* EXEC SQL EXECUTE
                           BEGIN
                                :hv_return_value := sp_payout_app_tmp_get_seqlist(
                                                                :hv_batch_id:ind_in_batch_id,
                                                                :hv_status:ind_in_status,
                                                                :c_cursor_dtcon);
                           END;
...

{
   ...
   sqlstm.sqladtp = &sqladt;
   sqlstm.sqltdsp = &sqltds;
   sqlstm.stmt = "begin :hv_return_value := sp_payout_app_tmp_get_seqlist (\
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "select ud_batch_id , ud_seq_num , ud_txn_id , ud_merchant_r\
ef , ud_country , ud_identity_id , ud_account_num , ud_account_name , ud_bank\
_name , ud_bank_code , ud_branch , ud_phone_num , ud_province , ud_city , ud_\
payout_amount , ud_request_amount , ud_payout_currency , ud_request_currency \
, ud_member_fee_ccy , ud_member_fee , ud_merchant_fee_ccy , ud_merchant_fee ,\
 ud_markup_ccy , ud_markup_amt , ud_exchange_rate , ud_response_code , ud_rem\
ark , ud_batch_mode INTO :b0:b1 , :b2:b3 , :b4:b5 , :b6:b7 , :b8:b9 , :b10:b1\
1 , :b12:b13 , :b14:b15 , :b16:b17 , :b18:b19 , :b20:b21 , :b22:b23 , :b24:b2\
5 , :b26:b27 , :b28:b29 , :b30:b31 , :b32:b33 , :b34:b35 , :b36:b37 , :b38:b3\
9 , :b40:b41 , :b42:b43 , :b44:b45 , :b46:b47 , :b48:b49 , :b50:b51 , :b52:b5\
3 , :b54:b55 FROM merchant_upload_file_detail WHERE ud_batch_id = :b56 AND ud\
_disabled = 0 AND ud_seq_num = :b57 AND ud_status = 69 for update nowait ";
 sqlstm.iters = (unsigned int  )1;
...
DEBUGLOG(("GetHeader batch_id = [%.*s]\n",hv_batch_id.len,hv_batch_id.arr));
	...
	/* EXEC SQL
			...
			:v_country:ind_country,
			:v_ccy:ind_ccy
                from    merchant_upload_file_header,merchant_upload_file_detail
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "select uh_merchant_id , uh_service_code , ud_country , ud_r\
equest_currency into :b0:b1 , :b2:b3 , :b4:b5 , :b6:b7 from merchant_upload_f\
ile_header , merchant_upload_file_detail where uh_batch_id = :b8 AND uh_disab\
led = :b9 AND uh_batch_id = ud_batch_id AND uh_disabled = :b9 group by uh_mer\
chant_id , uh_service_code , ud_country , ud_request_currency ";
 sqlstm.iters = (unsigned int  )1;
...
DEBUGLOG(("GetHeaderbyTmp batch_id = [%.*s]\n",hv_batch_id.len,hv_batch_id.arr));
	...
	/* EXEC SQL
			...
			payout_approve_tmp
                where   pt_id =:hv_batch_id
		AND	uh_batch_id = ud_batch_id
		and	ud_txn_id = pt_txn_id
                AND     uh_disabled =:hv_disabled
		group by uh_merchant_id,uh_service_code,ud_country,ud_request_currency; */ 

{
 ...
 sqlstm.stmt = "select uh_merchant_id , uh_service_code , ud_country , ud_r\
equest_currency into :b0:b1 , :b2:b3 , :b4:b5 , :b6:b7 from merchant_upload_f\
ile_header , merchant_upload_file_detail , payout_approve_tmp where pt_id = :\
b8 AND uh_batch_id = ud_batch_id and ud_txn_id = pt_txn_id AND uh_disabled = \
:b9 group by uh_merchant_id , uh_service_code , ud_country , ud_request_curre\
ncy ";
 sqlstm.iters = (unsigned int  )1;
