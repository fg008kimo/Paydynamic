...
static unsigned int sqlctx = 9556411;
...
New file format					   2014/03/20		   LokMan Chow
*/
...
/* Field #0 txn_seq */
		fprintf(fout,"%.*s%s",(int)strlen(csList[IDX_TXN_SEQ])-1,&csList[IDX_TXN_SEQ][1],PD_MY_DELIMITOR);

/* Field #1 tid */
		fprintf(fout,"%.*s%s",(int)strlen(csList[IDX_TID]),csList[IDX_TID],PD_MY_DELIMITOR);

/* Field #2 bank_bill_no */
                fprintf(fout,"0%s",PD_MY_DELIMITOR);
...
/* Field #6 fundin date */
		fprintf(fout, "%s%s%s", csDate,csTime,PD_MY_DELIMITOR);

/* Field #7 customer number */
		fprintf(fout,"1");
