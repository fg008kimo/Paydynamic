...
static unsigned int sqlctx = 4883467;
...
void match(char* cs_ph_buf, char* cs_psp_buf)
{
	...
	sprintf(csDate,"%.*s",PD_DATE_LEN,csPhList[IDX_TXN_DATE]);
        ...
        for (i = 0; i < MAX_FIELD_NO; i++) {
		if((i!=IDX_STATUS) && (i!=IDX_SERVICE_FEE) && (i!=IDX_BILL_NO) && (i!=IDX_TXN_DATE) && (i!=IDX_TID)){
        		if (strcmp(csPhList[i], csPspList[i]) != 0) {
DEBUGLOG(("match false [%d] [%s] [%s]\n",i,csPhList[i],csPspList[i]));
				i_match = PD_FALSE;
			}
		}
        }
