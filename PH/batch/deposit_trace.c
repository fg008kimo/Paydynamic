...
static unsigned int sqlctx = 4669843;
...
Change Request                                     2018/11/08              David Wong
Add overriding by PID setting                      2019/03/22              David Wong
Put aes_key into hRequest                          2019/04/29              David Wong
Fix MQ_HEADER_LEN                                  2019/11/01              David Wong
*/
...
//get deposit trace config
		...
		if ((*DBObjPtr)(cs_psp_channel, cs_psp_id, hTxn) == PD_OK) {
...
DEBUGLOG(("- aes_key = [%s]\n",csPtr));
					...
					PutField_CString(hRequest,"aes_key",csPtr);
				}
				hRec = RecordSet_GetNext(rKeySet);
			}
		}
...
DEBUGLOG((" txn_amt = [%lf]\n", dPtr));
				...
				if (GetField_CString(hRec, "tid", &csPtr)) {
					PutField_CString(hRequest, "tid", csPtr);
DEBUGLOG((" tid = [%s]\n", csPtr));
				}
...
//FormatMsg and send to queue
	if(iRet == SUCCESS){
		if (!strcmp(cs_psp_channel, PD_CHANNEL_ESKY)) {
			strcpy(csTmpChannelCode, "ESkyMsg");
		} else if (!strcmp(cs_psp_channel, PD_CHANNEL_ALIPAY_VNET)) {
			int iTmp = pre_request(hRequest);
			if (iTmp != SUCCESS) {
DEBUGLOG(("pre_request FAILED\n"));
			}
...
DEBUGLOG((" recv = [%s][%d]\n", &msg->mtext[MQ_HEADER_LEN], iRecvLen - (MQ_HEADER_LEN)));
						...
						h_msg->msg[iRecvLen - (MQ_HEADER_LEN)] = '\0';
						...
						/*
							...
							3. call BOSecurity.GeneratePspSign (if txn_seq & txn_amt exists)
								- call XxxMsg.BuildAuthData
									- call XxxMsg.BuildCallbackAuthData
								- call corresponding generate sign function
							4. call XxxMsg.FormatCallbackMsg (if txn_seq & txn_amt exists)
							5. send to WEBChannel (if txn_seq & txn_amt exists)
						*/
...
DEBUGLOG(("enquiry_status = [%c]\n", cEnquiryStatus));
							PutField_Char(hTxn, "enquiry_status", cEnquiryStatus);

							if (cEnquiryStatus == PD_PSP_RE_FAILED) {
								if (GetField_CString(hMsg, "fail_reason", &csPtr)) {
DEBUGLOG(("status_msg = [%s]\n", csPtr));
									PutField_CString(hTxn, "fail_reason", csPtr);
								}
							}

							DBObjPtr = CreateObj(DBPtr, "DBDepositTrace", "UpdateTrace");
							if ((*DBObjPtr)(cs_txn_id, c_party, i_seq, hTxn) != PD_OK) {
DEBUGLOG(("DBDepositTrace:UpdateTraceStatus FAILED\n"));
							}
...
DEBUGLOG(("DBTransaction:Update FAILED\n"));
							}
...
DEBUGLOG(("DBTransaction:UpdateDetail FAILED\n"));
							...
							if (cEnquiryStatus == PD_PSP_RE_FAILED) {
								if (!GetField_CString(hMsg, "txn_seq", &csPtr) || !GetField_CString(hMsg, "txn_amt", &csPtr2))
									iTmpRet = PD_ERR;
							}
						}

						PutField_Int(hMsg, "for_enquiry_callback_use", PD_TRUE);
...
int pre_request(hash_t *hIn)
{
	...
	h_msg = (hex_t *) malloc (sizeof(hex_t));
	if ((*MsgObjPtr)(hIn, h_msg->msg, &h_msg->len) == PD_OK) {
		lKey = GetMQKey((const unsigned char *)"PSPPREQQ");
		lRspKey = GetMQKey((const unsigned char *)"PSPPRSPQ");
		...
		iSendLen = MQ_HEADER_LEN + h_msg->len;
DEBUGLOG(("send len = [%d]\n", iSendLen));
DEBUGLOG(("send = [%s][%d]\n", &msg->mtext[MQ_HEADER_LEN], iSendLen - (MQ_HEADER_LEN)));
DEBUGLOG(("rspkey = [%ld][%ld]\n", lRspKey, msg->mtype));
...
DEBUGLOG(("recv = [%s][%d]\n", &msg->mtext[MQ_HEADER_LEN], iRecvLen - (MQ_HEADER_LEN)));
				...
				if ((*MsgObjPtr)(hIn, h_msg->msg, &h_msg->len) == PD_OK) {
					if (GetField_CString(hIn, "token", &csTmp)) {
DEBUGLOG(("token = [%s]\n", csTmp));
					}
					else {
...
DEBUGLOG(("BreakDownGetTokenRspMsg failed\n"));
					iRet = FAILURE;
				}
			}
		}
...
DEBUGLOG(("FormatGetTokenMsg failed\n"));
		iRet = FAILURE;
	}
