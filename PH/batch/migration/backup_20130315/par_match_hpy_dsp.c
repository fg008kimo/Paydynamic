...
{
    20,
    "par_match_hpy_dsp.pc"
};


static unsigned int sqlctx = 80830987;
...
void add_to_stat(char csList[][MAX_FIELD_LEN + 1], const int iMatch);
	...
	while (fgets(cs_psp_buf, PD_MAX_BUF, fp_psp) != NULL) {
		if (cs_psp_buf[strlen(cs_psp_buf) - 1] == 0x0A)
			...
			if (cs_ph_buf[strlen(cs_ph_buf) - 1] == 0x0A)
				...
				memcpy(cs_h_key, csPhList[IDX_TID],PD_TXN_SEQ_LEN);
				memcpy(cs_p_key, csPspList[IDX_TID],PD_TXN_SEQ_LEN);
				cs_h_key[strlen(csPhList[IDX_TID])]='\0';
				cs_p_key[strlen(csPhList[IDX_TID])]='\0';
				
DEBUGLOG(("checking: ph_key[%s] psp_key[%s]\n",cs_h_key, cs_p_key));
				i_ret = memcmp(cs_h_key, cs_p_key, PD_TXN_SEQ_LEN);
                       		if (i_ret == 0) {
DEBUGLOG(("checking: same key\n"));
          				match(cs_ph_buf, cs_psp_buf); 
                      			i_read_next = PD_TRUE;
                                	break;
               			}
                        	else if (i_ret < 0) {
DEBUGLOG(("checking: no psp\n"));
                       			extract(cs_ph_buf,csPhList); 
					if(record(csPhList,"PSP")!=PD_TRUE){
DEBUGLOG(("checking: not rematch\n"));
...
DEBUGLOG(("checking: not rematch\n"));
					...
					add_to_stat(csPspList, PD_FALSE);
                        ...
                        extract(cs_psp_buf, csPspList); 
			if(record(csPspList,"PH")!=PD_TRUE){
DEBUGLOG(("checking: after checking mismatch list: no ph\n"));
                        	output(NULL,csPspList,(char*)"No Ph Tx");
			}
			add_to_stat(csPspList, PD_FALSE);
                        i_read_next = PD_TRUE;
	...
	if(i_read_next == PD_FALSE){
		if(record(csPhList,"PSP")!=PD_TRUE){
DEBUGLOG(("checking: after checking mismatch list: no psp\n"));
			output(csPhList,NULL,(char*)"NO Psp Tx");
		}
        }    
...
void match(char* cs_ph_buf, char* cs_psp_buf)
{
	...
	sprintf(csDate,"%.*s",PD_DATE_LEN,csPhList[IDX_TXN_DATE]);
        ...
        for (i = 0; i < MAX_FIELD_NO; i++) {
		if(i!=IDX_TXN_SEQ && i!=IDX_SERVICE_FEE && i!=IDX_BILL_NO && i!=IDX_TXN_DATE){
