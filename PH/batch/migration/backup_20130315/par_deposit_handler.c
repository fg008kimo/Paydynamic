...
{
    22,
    "par_deposit_handler.pc"
};


static unsigned int sqlctx = 322496251;
...
DEBUGLOG(("handle_deposit_balance: precal_fee [%lf]\n", dPreCalFee));
	...
	if (iRet == SUCCESS) {
		if (prbo_GetOpenBalanceForUpdate(hMerchRec, csMID, csCcy, csCountry, csService) == PD_OK) {
...
DEBUGLOG(("handle_deposit_balance: Merchant amount [%lf]\n", dAmount));
		...
		if (prbo_UpdateFloat(csMID, csCountry, csCcy, csService, dAmount, dAmount, PD_UPDATE_USER) == PD_OK) {
...
DEBUGLOG(("handle_deposit_balance: auto_recon [%c]\n", cAutoRecon));
			...
			// auto_recon -> update bal
	                ...
	                if (prbo_CreditBalance(csPspID, csPspCountry, csToCcy, PD_PSP_BAL, dToAmount, PD_UPDATE_USER) == PD_OK) {
...
DEBUGLOG(("handle_deposit_balance: DBPspBalance.UpdateFloat(Balance) FAIL!\n"));
			...
			// manual recon -> update float
	                ...
	                if (prbo_CreditBalance(csPspID, csPspCountry, csToCcy, PD_PSP_FLOAT, dToAmount, PD_UPDATE_USER) == PD_OK) {
DEBUGLOG(("handle_deposit_balance: DBPspBalance.UpdateFloat(Balance) SUCC!\n"));
			} else {
DEBUGLOG(("handle_deposit_balance: DBPspBalance.UpdateFloat(Balance) FAIL!\n"));
				iRet = FAILURE;
	...
	if (iRet == SUCCESS) {
		if (prbo_GetCurrentValues(csMID, csCcy, csCountry, csService, hMerchRec) == PD_OK) {
...
DEBUGLOG(("handle_deposit_balance: DBMerchantBalance.GetCurrentValues FAIL!\n"));
	...
	if (iRet == SUCCESS) {
                if (prbo_GetBalance(csPspID, csPspCountry, csToCcy, hPspRec) == PD_OK) {
...
//DEBUGLOG(("CreateNewDepositTxn: db_commit [%d]\n", iTmp));
	...
	if (iInitMode == PD_TRUE) {
		csTxnSeq = strdup((char* )prbo_GetNextTxnSeq());
...
DEBUGLOG(("process_deposit_approve: CalPspCosts\n")); 
	iRet = prbo_CalPspCosts(hBalance, hBalance);
	if (iRet == SUCCESS) {
...
//DEBUGLOG(("process_deposit_approve:: Call TxnPspDetail Update\n"));
	...
	if (iRet == SUCCESS) {
		...
		iRet = prbo_AddPspTxnElement(hPspElement);
