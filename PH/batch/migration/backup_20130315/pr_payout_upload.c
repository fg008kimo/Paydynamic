...
extern void sqliem( unsigned char *, signed int * );
 ...
 static char *sq0002 = 
"select mn_service_code ,mn_merchant_id  from par_def_merch_nmb_map where mn\
_merchant_acct_nmb=:b0           ";

 static char *sq0006 = 
"select ps_status_id  from par_payout_status where ps_status=:b0           ";
...
{12,4130,871,0,0,
5,0,0,1,81,0,4,359,0,0,1,0,0,1,0,2,9,0,0,
24,0,0,2,108,0,9,412,0,0,1,1,0,1,0,1,9,0,0,
43,0,0,2,0,0,13,414,0,0,2,0,0,1,0,2,9,0,0,2,9,0,0,
66,0,0,2,0,0,15,442,0,0,0,0,0,1,0,
81,0,0,2,0,0,15,452,0,0,0,0,0,1,0,
96,0,0,3,345,0,6,562,0,0,10,10,0,1,0,2,3,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,
9,0,0,1,9,0,0,1,9,0,0,1,3,0,0,1,4,0,0,
151,0,0,4,0,0,17,1030,0,0,1,1,0,1,0,1,9,0,0,
170,0,0,4,0,0,21,1031,0,0,0,0,0,1,0,
185,0,0,4,0,0,17,1133,0,0,1,1,0,1,0,1,9,0,0,
204,0,0,4,0,0,21,1134,0,0,0,0,0,1,0,
219,0,0,5,953,0,6,1437,0,0,29,29,0,1,0,2,3,0,0,1,9,0,0,1,3,0,0,1,9,0,0,1,9,0,0,
...
350,0,0,6,74,0,9,1553,0,0,1,1,0,1,0,1,9,0,0,
369,0,0,6,0,0,13,1555,0,0,1,0,0,1,0,2,3,0,0,
388,0,0,6,0,0,15,1573,0,0,0,0,0,1,0,
403,0,0,6,0,0,15,1583,0,0,0,0,0,1,0,
};
...
int	GetStatusID(const char *csStatus, int *iStatusID);
...
DEBUGLOG(("process_header:: file_id [%s] \n", csTmp));
		}

		if (PayoutUpload_Header_ChkExist(hTxn) == PD_FOUND) {
			GetField_CString(hTxn, "hd_batch_id", &csTmp);

			csBatchId = strdup(csTmp);

DEBUGLOG(("process_header:: Org BatchID [%s] \n", csBatchId));
...
/*
		...
		if(GetField_CString(hTxn,csTag,&csTmp)){
			sprintf(csTag,"%s_status",csBatchId);
...
*/
		sprintf(csTag, "date_time_%d", i);
		if (GetField_CString(hTxn, csTag, &csTmp)) {

			strncpy(csYear, csTmp, PD_YYYY_LEN);
			strncpy(csMon,  csTmp + PD_YYYY_LEN + 1, PD_MM_LEN);
			strncpy(csDay,  csTmp + PD_YYYY_LEN + 1 + PD_MM_LEN + 1, PD_DD_LEN);
	
			csYear[PD_YYYY_LEN] = '\0';
			csMon[PD_MM_LEN] = '\0';
			csDay[PD_DD_LEN] = '\0';

			sprintf(csDate, "%s%s%s", csYear, csMon, csDay);
		...
		if (PayoutUpload_Header_ChkExist(hTxn) == PD_NOT_FOUND) {
			...
			if (AddParPayoutUploadHD(hTxn) != PD_OK) {
				iRet = FAILURE;
				break;
...
DEBUGLOG(("process_header::Use Org Batch ID [%s]\n", csTmp));

			if (UpdateUploadHeader(hTxn) != PD_OK) {
				iRet = FAILURE;
				break;
			}
*/
...
hv_txn_count:ind_txn_count , :hv_txn_amt:ind_txn_amt ) ; END ;";
        sqlstm.iters = (unsigned int  )1;
...
DEBUGLOG(("parse_detail: Read Detail Error\n"));
		...
		if(iRet==PD_OK){
			for(iCount=0;iCount<i;iCount++){
				...
				else if(iCount==IDX_DETAIL_MEMBER_FEE_CCY_CD)
					sprintf(csTag,"member_fee_ccy_%d",iRec);
				else if(iCount==IDX_DETAIL_MERCHANT_FEE_CCY_CD)
					sprintf(csTag,"merchant_fee_ccy_%d",iRec);
				else if(iCount==IDX_DETAIL_MARKUP_CCY_CD)
				...
				if(strlen(csList[iCount])){
					if(iCount==IDX_DETAIL_REQ_CCY_CD
					 ...
					 ||iCount==IDX_DETAIL_MEMBER_FEE_CCY_CD
					 ||iCount==IDX_DETAIL_MERCHANT_FEE_CCY_CD
					 ||iCount==IDX_DETAIL_MARKUP_CCY_CD){
...
DEBUGLOG(("process_detail Begin [%d]\n",iCnt));
	...
	if (!GetField_Int(hContext,"total_detail_record",&iCnt)) {
DEBUGLOG(("Assume no DETAIL!!!\n"));
		return iRet;
	}
...
DEBUGLOG(("process_detail::GenBatchId batch_id = [%s]\n", csBatchId));
		...
		csTxnSeq  = strdup(prbo_GetNextBatchTxnSeq());
DEBUGLOG(("GenerateBatchSeq: [%d]=[%s]\n",i,csTxnSeq));
		...
		if(GetField_CString(hContext,csTag,&csTmp)){
			iTmp = atoi(csTmp);
DEBUGLOG(("GenerateBatchSeq:seq_num [%d]\n", iTmp));
		...
		if(GetField_CString(hContext,csTag,&csTmp)){
			...
			if (GetStatusID(csTmp, &iStatus) != PD_OK) {
				iRet = FAILURE;
				break;
			}
			PutField_Int(hTxn,"status",iStatus);
...
DEBUGLOG(("TESTING.... seq_num [%d]\n", iTmp));
}
		if (!GetField_CString(hTxn, "batch_id", &csTmp)) {
DEBUGLOG(("TESTING.... header chkexist\n"));
			if (PayoutUpload_Header_ChkExist(hTxn) == PD_FOUND) {
	
				GetField_CString(hTxn, "hd_batch_id", &csTmp);
DEBUGLOG(("TESTING.... find batch_id [%s]\n", csTmp));
...
DEBUGLOG(("TESTING....xx batch_id [%s]\n", csTmp));
		}

if (GetField_Int(hTxn, "seq_num", &iTmp)) {
DEBUGLOG(("TESTING....yyyy seq_num [%d]\n", iTmp));
}

		if (PayoutUpload_Detail_ChkExist(hTxn) == PD_NOT_FOUND) {
			...
			/*
			if (AddParPayoutUploadDT(hTxn) != PD_OK) {
				iRet = FAILURE;
				break;
			}
			*/
			...
			if (GetField_CString(hTxn, "dt_batch_id", &csTmp)) {
DEBUGLOG(("Dtl org batch_id [%s]\n", csTmp));	
				PutField_CString(hTxn, "batch_id", csTmp);
			}

			///not update the ud_vnc_ref_num field
			RemoveField_CString(hTxn,"vnc_ref_num");
		}
...
DEBUGLOG(("UpdateUploadHeader:uh_status = [%d]\n",iStatus));
...
DEBUGLOG(("UpdateUploadDetail:vnc_ref_num = [%s]\n",csBuf));
	...
	// only update while aux_txn_id field is null 
	if(GetField_CString(hRls, "aux_txn_id", &csBuf)) {
DEBUGLOG(("UpdateUploadDetail:aux_txn_id = [%s]\n",csBuf));
                strcat((char*)hv_dynstmt.arr, ",ud_aux_txn_id= NVL(ud_aux_txn_id, '");
                strcat((char*)hv_dynstmt.arr, csBuf);
                strcat((char*)hv_dynstmt.arr, "')");
                hv_dynstmt.len = strlen((const char*)hv_dynstmt.arr);
	}
        ...
        if(GetField_Int(hRls,"status",&iStatus)){
DEBUGLOG(("UpdateUploadDetail:ud_status = [%d]\n",iStatus));
                sprintf(csBuf, "%d", iStatus);
...
struct { unsigned short len; unsigned char arr[10]; } hv_bank_code;
                ...
                /* varchar         hv_branch[80]; */ 
struct { unsigned short len; unsigned char arr[80]; } hv_branch;
...
/*branch*/
        if(GetField_CString(hRls,"branch",&csTmp)){
                ...
                ind_branch = 0;
DEBUGLOG(("Add:branch = [%.*s] [%d]\n",hv_branch.len,hv_branch.arr, hv_branch.len));
        }
...
/*province*/
        if(GetField_CString(hRls,"province",&csTmp)){
                ...
                ind_province = 0;
DEBUGLOG(("Add:province = [%.*s] [%d]\n",hv_province.len,hv_province.arr, hv_province.len));
        }
...
/*city*/
        if(GetField_CString(hRls,"city",&csTmp)){
                ...
                ind_city = 0;
DEBUGLOG(("Add:city = [%.*s] [%d]\n",hv_city.len,hv_city.arr, hv_city.len));
        }
/*payout currency*/
...
hv_status:ind_status , :hv_remark:ind_remark ) ; END ;";
        ...
        sqlstm.sqhstl[12] = (unsigned long )82;
...
struct { unsigned short len; unsigned char arr[50]; } hv_status;
...

{
        ...
        sqlstm.sqptdso = sqlstm.sqtdso;
        sqlcxt((void **)0, &sqlctx, &sqlstm, &sqlfpn);
        if (sqlca.sqlcode < 0) goto getstatusid_error;
}
...

{
                ...
                sqlstm.sqfoff = (         int )0;
                sqlstm.sqfmod = (unsigned int )2;
                sqlstm.sqhstv[0] = (unsigned char  *)&v_status_id;
                sqlstm.sqhstl[0] = (unsigned long )sizeof(int);
                sqlstm.sqhsts[0] = (         int  )0;
                sqlstm.sqindv[0] = (         short *)&ind_status_id;
                sqlstm.sqinds[0] = (         int  )0;
                sqlstm.sqharm[0] = (unsigned long )0;
                sqlstm.sqadto[0] = (unsigned short )0;
                sqlstm.sqtdso[0] = (unsigned short )0;
