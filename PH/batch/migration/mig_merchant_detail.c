...
{
    22,
    "mig_merchant_detail.pc"
};


static unsigned int sqlctx = 319740459;
...
{12,4130,871,0,0,
5,0,0,1,215,0,9,115,0,0,0,0,0,1,0,
20,0,0,1,0,0,13,117,0,0,7,0,0,1,0,2,9,0,0,2,9,0,0,2,3,0,0,2,3,0,0,2,3,0,0,2,3,
0,0,2,3,0,0,
63,0,0,1,0,0,15,195,0,0,0,0,0,1,0,
78,0,0,1,0,0,15,213,0,0,0,0,0,1,0,
93,0,0,2,0,0,32,214,0,0,0,0,0,1,0,
108,0,0,3,155,0,9,474,0,0,1,1,0,1,0,1,9,0,0,
127,0,0,3,0,0,13,476,0,0,4,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,
158,0,0,3,0,0,15,516,0,0,0,0,0,1,0,
173,0,0,3,0,0,15,528,0,0,0,0,0,1,0,
188,0,0,4,0,0,32,529,0,0,0,0,0,1,0,
203,0,0,5,132,0,4,559,0,0,3,2,0,1,0,2,3,0,0,1,9,0,0,1,9,0,0,
230,0,0,6,129,0,4,612,0,0,3,2,0,1,0,2,3,0,0,1,9,0,0,1,9,0,0,
257,0,0,7,129,0,4,665,0,0,3,2,0,1,0,2,3,0,0,1,9,0,0,1,9,0,0,
284,0,0,8,82,0,9,741,0,0,1,1,0,1,0,1,9,0,0,
303,0,0,8,0,0,13,743,0,0,1,0,0,1,0,2,9,0,0,
322,0,0,9,0,0,17,821,0,0,1,1,0,1,0,1,9,0,0,
341,0,0,9,0,0,21,822,0,0,0,0,0,1,0,
};
...
Add MPG 					   2013/03/21              Virginia Yun
*/
...
int chk_is_ec2(char *csUserName, char *csAcctNmb);
int chk_is_nbx(char *csUserName, char *csAcctNmb);
int chk_is_mpg(char *csUserName, char *csAcctNmb);
...
DEBUGLOG(("username [%s]\n", csUserName));
	...
	if (iRet == PD_OK) {
		// service
		iTmpRet = chk_is_ec2(csUserName, csMerchAcctNmb);
		if (iTmpRet == PD_FOUND) {
DEBUGLOG(("service [%s]\n", PD_PAR_SERVICE_EC2));
			PutField_CString(hTxn, "service", PD_PAR_SERVICE_EC2);
		} else if (iTmpRet == PD_NOT_FOUND) {

			iTmpRet = chk_is_nbx(csUserName, csMerchAcctNmb);
...
DEBUGLOG(("service [%s]\n", PD_PAR_SERVICE_NBX));
				...
				iTmpRet = chk_is_mpg(csUserName, csMerchAcctNmb);

				if (iTmpRet == PD_FOUND) {
DEBUGLOG(("service [%s]\n", PD_PAR_SERVICE_MPG));
					PutField_CString(hTxn, "service", PD_PAR_SERVICE_MPG);
				} else if (iTmpRet == PD_NOT_FOUND) {
...
struct { unsigned short len; unsigned char arr[50]; } hv_value;
		...
		/* varchar         hv_value1[PD_SP_VALUE_LEN]; */ 
struct { unsigned short len; unsigned char arr[50]; } hv_value1;
...
DEBUGLOG(("chk_is_ec2 value = [%.*s]\n",hv_value.len,hv_value.arr));
	...
	hv_value1.len = strlen(csAcctNmb);
        memcpy(hv_value1.arr, csAcctNmb, hv_value1.len);
DEBUGLOG(("chk_is_ec2 value1 = [%.*s]\n",hv_value1.len,hv_value1.arr));
...

{
 ...
 (((pdm_code='EC2_TW_EMAIL' and pdm_value=:b2) and pdm_value1=:b3) and rownum\
=1)";
        sqlstm.iters = (unsigned int  )1;
        ...
        sqlstm.sqadto[1] = (unsigned short )0;
        sqlstm.sqtdso[1] = (unsigned short )0;
        sqlstm.sqhstv[2] = (unsigned char  *)&hv_value1;
...
int     chk_is_mpg(char *csUserName, char *csAcctNmb)
{
        ...
        /* EXEC SQL WHENEVER SQLERROR GOTO chkismpg_error; */ 
...
struct { unsigned short len; unsigned char arr[50]; } hv_value1;
        ...
        hv_value.len = strlen(csUserName);
        memcpy(hv_value.arr, csUserName, hv_value.len);
DEBUGLOG(("chk_is_mpg value = [%.*s]\n",hv_value.len,hv_value.arr));

        hv_value1.len = strlen(csAcctNmb);
        memcpy(hv_value1.arr, csAcctNmb, hv_value1.len);
DEBUGLOG(("chk_is_mpg value1 = [%.*s]\n",hv_value1.len,hv_value1.arr));

        /* EXEC SQL
                SELECT count(1)
                   INTO :v_no_of_record:ind_no_of_record
                   FROM par_def_misc
                  WHERE pdm_code = 'MPG_EMAIL'
                    and pdm_value = :hv_value
		    and pdm_value1= :hv_value1
                    and rownum = 1; */ 

{
        ...
        sqlstm.sqltdsp = &sqltds;
        sqlstm.stmt = "select count(1) into :b0:b1  from par_def_misc where\
 (((pdm_code='MPG_EMAIL' and pdm_value=:b2) and pdm_value1=:b3) and rownum=1)";
        sqlstm.iters = (unsigned int  )1;
        ...
        sqlstm.sqptdso = sqlstm.sqtdso;
        sqlcxt((void **)0, &sqlctx, &sqlstm, &sqlfpn);
        if (sqlca.sqlcode < 0) goto chkismpg_error;
}
        ...
        if (ind_no_of_record >= 0) {
                if (v_no_of_record > 0) {
DEBUGLOG(("chk_is_mpg FOUND\n"));
                        iRet = PD_FOUND;
                }
        }

        if (iRet!= PD_FOUND) {
DEBUGLOG(("chk_is_mpg NOT FOUND\n"));
        }

        return iRet;

chkismpg_error:
DEBUGLOG(("chkismpg_error code %d\n", sqlca.sqlcode));
DEBUGLOG(("\n%.*s\n", sqlca.sqlerrm.sqlerrml, sqlca.sqlerrm.sqlerrmc));
        /* EXEC SQL WHENEVER SQLERROR CONTINUE; */ 
...

{
        ...
        sqlstm.sqltdsp = &sqltds;
        sqlstm.stmt = "select count(1) into :b0:b1  from par_def_misc where\
 (((pdm_code='NBX_EMAIL' and pdm_value=:b2) and pdm_value1=:b3) and rownum=1)";
        sqlstm.iters = (unsigned int  )1;
