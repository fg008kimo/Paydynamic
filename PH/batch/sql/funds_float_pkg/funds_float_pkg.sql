...
dbms_output.put_line(' 1st Aprv Date: [' || v_1st_aprv_date || ']');
	...
	LOOP
		...
		if iTmpCnt > 0 then
			...
			/*-- 2nd: Find the non-released AF with PID "000" (the support payout funds already moved to AF) --*/
			select count (*)
			into iTmpCnt
			from MERCHANT_BUCKET
			where MB_POSTING_DATE = v_aprv_date
			...
			if iTmpCnt > 0 then
				...
				/*-- if no "000" records, find the non-released AF with PID <> "000" (the support payout funds still in APO or non-support payout funds only) --*/
				select count (*)
				into iTmpCnt
				from MERCHANT_BUCKET
				where MB_POSTING_DATE = v_aprv_date
				...
				if iTmpCnt > 0 then
					...
					if (iAPOtoAF = 0) and (in_curr_date = v_po_rel_date) then
						iTmpCnt := 0;
						...
						if iTmpCnt > 0 then
							/*-- The released FT still in APO --*/
							/*-- Get the real-time APO and add to the predict SF release amount (if any) --*/
							iGetAvaPo := iGetAvaPo + 1;

						else
							/*-- No FT released to APO, it doesn't mean merchant not support payout --*/
							if v_txn_type != 'D' then
								/* There may be some (+) adjustments on the APO due to non-deposit action */
								iGetAvaPo := iGetAvaPo + 1;
							end if;
						end if;
					end if;
					...
					if iTmpCnt > 0 then
						...
						if (iAPOtoAF = 0) and (in_curr_date = v_po_rel_date) then
							...
							if iTmpCnt > 0 then
								...
								/*-- FT released already, seems nothing moved to AF --*/
								v_rls_amt := 0.0;
							end if;
						else
							/*-- FT released already, seems nothing moved to AF --*/
						...
						if iTmpCnt > 0 then
							...
							/*-- 5th: N/A case --*/
							/*-- No FT found, nothing will be released from FT --*/
							v_rls_amt := 0.0;


							/*-- Calculate the + / - funds from non-deposit actions --*/
							if (v_txn_type != 'D') and (to_number(v_po_rel_date) >= to_number(in_curr_date)) then
								iGetAvaPo := iGetAvaPo + 1;
							end if;
				...
				/*-- For AF0(D) with FT->APO, FT1 and N/A cases --*/
				if iGetAvaPo > 0
				then
					/*-- Get all the released FT in the current date (may be in different posting date) --*/
					if v_max_aprv_date is NULL then

						select funds_float_pkg.find_date(v_po_rel_date, in_country, in_service_code, 'APRV_LAST', iPORlsPeriod)
						into   v_max_aprv_date
						from dual; 

						select nvl(SUM(MB_RELEASED_AMT),0)
						into	v_total_amt
						from MERCHANT_BUCKET
						where MB_MERCHANT_ID = in_merchant_id
						and MB_COUNTRY_ID = in_country
						...
						and MB_SERVICE_CODE = in_service_code
						and MB_BUCKET_TYPE = 'FT'
						and MB_RELEASED_DATE = v_po_rel_date 
						and MB_RELEASED_TO_AF = 0;
					end if;	
