CREATE OR REPLACE PACKAGE REPORT_PKG_T1 is
...
CREATE OR REPLACE PACKAGE BODY REPORT_PKG_T1
IS
   FUNCTION F_RPT001 (
   ...
   IS
      ...
      IS
        SELECT te_txn_id,
               ledger_detail.th_merchant_ref,
               ledger_detail.th_txn_code,
               tc_desc,
               td_open_bal,
               td_current_bal,
               te_txn_element_type,
               ...
               te_ccy,
               te_amt_type,
               ledger_detail.th_approval_timestamp,
               void_th.th_txn_id
       FROM (
           SELECT te_txn_id,
                  th_merchant_ref,
                  th_txn_code,
                  tc_desc,
                  NVL (td_open_bal, 0) as td_open_bal,
                  td_current_bal,
                  te_txn_element_type,
                  dt_desc,
                  te_exec_seq,
         ...
         ORDER BY th_approval_timestamp, te_exec_seq) ledger_detail
      LEFT JOIN txn_header void_th
      ON ledger_detail.te_txn_id = void_th.th_org_txn_id
      AND void_th.th_txn_code != 'TSI'
      AND void_th.th_merchant_id is not null;
