CREATE OR REPLACE FUNCTION baid_txn_null (
  ...
  if in_baid is null  then
   ...
   sql_stmt := 'select
		...
		null ote_exec_seq,
		OTP_TXN_CCY ote_ccy,
		null oth_approval_timestamp,
		null oth_approval_date,
		null otp_report_date,
		null void_txn_id,
		null ote_party_type
                from (
						...
						FROM
						  (SELECT obai_baid,
							...
							OTP_TXN_CCY,'
                             || open_bal_str || ',
							ROW_NUMBER() OVER ( PARTITION BY OBAI_BAID_NAME , OTP_PSP_ID , OTP_TXN_CCY ORDER BY oth_approval_timestamp DESC ) RN
						...
						WHERE RN = 1
						GROUP BY OTP_PSP_ID  ,OTP_TXN_CCY
