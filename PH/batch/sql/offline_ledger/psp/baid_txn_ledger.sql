CREATE OR REPLACE FUNCTION BAID_TXN_LEDGER(
 ...
 cur_kount             number;
 begin
 in_cur :=baid_txn(in_approval_date_from ,in_approval_date_to,in_psp_id,in_baid,in_ccy,in_ele);
 loop
    fetch in_cur into
             OBAI_BAID,
             OBAI_BAID_NAME,
...
-- dbms_output.put_line('xxxxxxx' || in_cur%rowcount);
 ...
 if cur_kount=0 then
  in_cur1 :=baid_txn_null(in_approval_date_from ,in_approval_date_to,in_psp_id,in_baid,in_ccy,in_ele);
 loop
    fetch in_cur1 into
             OBAI_BAID,
             OBAI_BAID_NAME,
...
-- dbms_output.put_line('yyyyyyyy' || in_cur1%rowcount);
 close in_cur1;
end if;
