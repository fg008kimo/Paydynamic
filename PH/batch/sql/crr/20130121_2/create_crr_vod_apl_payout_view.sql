     ...
     SELECT TH.TH_APPROVAL_DATE CR_TXN_DATE,
       ...
       FROM (SELECT TH_TXN_ID,
            ...
            (SELECT TE_TXN_ID,
              ...
              WHERE TE_PARTY_TYPE = 'M' AND TE_TXN_ELEMENT_TYPE = 'TAMT') TE
               ON TE.TE_TXN_ID = TH.TH_TXN_ID
            INNER JOIN merch_detail Mt ON Mt.merchant_id = Th.th_merchant_id
            INNER JOIN clients Ct ON Ct.client_id = Mt.client_id
            INNER JOIN CRR_BUSINESS_TYPE_MAPPING CBM ON CBM.BM_BUSINESS_TYPE = Ct.BUSINESS_TYPE
            INNER JOIN
