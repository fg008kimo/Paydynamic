DROP VIEW CRR_GEN_PAYOUT_VIEW;

CREATE OR REPLACE FORCE VIEW CRR_GEN_PAYOUT_VIEW
(
   CR_TXN_DATE,
   CR_COUNTRY,
   CR_PRODUCT,
...
AS
     SELECT TH.TH_APPROVAL_DATE CR_TXN_DATE,
       ...
       FROM (SELECT TH_TXN_ID, TH_TXN_CODE, TH_APPROVAL_DATE
               FROM TXN_HEADER TH
              WHERE     TH.TH_STATUS IN ('C', 'R')
                    AND TH.TH_AR_IND = 'A'
                    AND TH.TH_TXN_CODE IN ('POG')) TH,
            TXN_DETAIL TD,
            (SELECT TE_TXN_ID,
                    TE_PARTY_TYPE,
                    TE_CCY,
              ...
              WHERE TE.TE_TXN_ELEMENT_TYPE = 'TAMT' AND TE.TE_PARTY_TYPE = 'P') TE,
            TXN_PSP_DETAIL TP,
            (SELECT CR_TXN_CODE,
                    CR_COUNTRY_ID,
                    CR_PRODUCT_CODE,
              ...
              WHERE     CRP.CR_PARTY_TYPE = 'P'
                    AND CRP.CR_JNL_ENTRY_TYPE_ID = 'PSP_AMT'
                    AND CRP.CR_DISABLED = 0
                    AND CRP.CR_TXN_CODE = 'POG') CRP,
            --PSP_PRODUCT PP
            (SELECT *
               FROM PSP_DETAIL TP
     ...
     SELECT TH.TH_APPROVAL_DATE CR_TXN_DATE,
       ...
       FROM (SELECT TH_TXN_ID, TH_TXN_CODE, TH_APPROVAL_DATE
            ...
            (SELECT TE_TXN_ID,
                    ...
                    TE_AMT_TYPE
               FROM TXN_ELEMENTS TE
              WHERE     TE.TE_TXN_ELEMENT_TYPE IN ('TFEE')
     ...
     SELECT TH.TH_APPROVAL_DATE CR_TXN_DATE,
       ...
       FROM (SELECT TH_TXN_ID, TH_TXN_CODE, TH_APPROVAL_DATE
            ...
            (SELECT TE_TXN_ID,
                    ...
                    TE_AMOUNT
               FROM TXN_ELEMENTS TE
              WHERE     TE.TE_TXN_ELEMENT_TYPE IN ('TAMT', 'TFEE')
                    AND TE.TE_PARTY_TYPE = 'P') TE,
            TXN_PSP_DETAIL TP,
            (SELECT CR_TXN_CODE,
                    CR_COUNTRY_ID,
