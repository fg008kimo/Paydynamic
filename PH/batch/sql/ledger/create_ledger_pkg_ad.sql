DROP PACKAGE LEDGER_PKG_AD;

CREATE OR REPLACE PACKAGE LEDGER_PKG_AD
IS
   FUNCTION F_CB_LEDGER (
      in_approval_date_from    txn_header.th_approval_date%TYPE,
      in_approval_date_to      txn_header.th_approval_date%TYPE,
...
DROP PACKAGE BODY LEDGER_PKG_AD;

CREATE OR REPLACE PACKAGE BODY LEDGER_PKG_AD
IS
   FUNCTION F_CB_LEDGER (
      in_approval_date_from    txn_header.th_approval_date%TYPE,
      in_approval_date_to      txn_header.th_approval_date%TYPE,
   ...
   BEGIN
      SELECT COUNT (*)
       ...
       WHERE     th_approval_date < in_approval_date_from
             AND th_merchant_id = in_merchant_id
             AND th_ar_ind = 'A'
      ...
      THEN
         dBal := 0;
      ELSE
         SELECT NVL (td_current_bal, 0),
                NVL (td_current_bal_settlement, 0),
                NVL (td_total_reserved_amount, 0),
                NVL (td_total_hold, 0),
                NVL (td_total_hold_settlement, 0)
           INTO last_open_bal,
                  ...
                  WHERE     th_approval_timestamp =
                               (SELECT MAX (th_approval_timestamp)
                                 ...
                                 WHERE     th_approval_date <
                                       ...
                                       AND th_txn_id = td_txn_id
                                       AND td_txn_country = in_country)
                        AND th_approval_date < in_approval_date_from
                        AND th_merchant_id = in_merchant_id
                        AND th_ar_ind = 'A'
                        AND th_service_code = in_service_code
                        AND th_net_ccy = in_ccy) txn_header
   ...
   BEGIN
      OPEN cursor_cb_ledger FOR
         SELECT *
           FROM TABLE (ledger_pkg_ad.F_cb_ledger (
   ...
   BEGIN
      ...
      THEN
         dBal := 0;
      ELSE
         SELECT NVL (td_total_hold, 0), NVL (td_total_hold_settlement, 0)
   ...
   BEGIN
      OPEN cursor_sb_ledger FOR
         SELECT *
           FROM TABLE (ledger_pkg_ad.F_sb_ledger (
                          TO_DATE (in_host_posting_date, 'YYYYMMDD'),
                          TO_DATE (in_host_posting_date, 'yyyymmdd') + 1,
                          in_merchant_id,
                          in_country,
   ...
   BEGIN
      ...
      THEN
         dBal := 0;
      ELSE
         SELECT NVL (td_total_reserved_amount, 0)
   ...
   BEGIN
      OPEN cursor_rb_ledger FOR
         SELECT *
           FROM TABLE (ledger_pkg_ad.F_rb_ledger (
                          TO_DATE (in_host_posting_date, 'YYYYMMDD'),
                          TO_DATE (in_host_posting_date, 'yyyymmdd') + 1,
                          in_merchant_id,
                          in_country,
   ...
   BEGIN
      OPEN cursor_apo_ledger FOR
         SELECT *
           FROM TABLE (ledger_pkg_ad.F_apo_ledger (
                          TO_DATE (in_host_posting_date, 'YYYYMMDD'),
                          TO_DATE (in_host_posting_date, 'yyyymmdd') + 1,
                          in_merchant_id,
                          in_country,
   ...
   BEGIN
      ...
      ELSE
         SELECT NVL (tp_bal, 0),
                NVL (tp_total_float, 0),
                NVL (tp_total_hold, 0)
           INTO last_open_bal, last_total_float, last_total_hold
           FROM txn_psp_detail,
                (SELECT th_txn_id
                   FROM txn_header, txn_psp_detail
                  WHERE     th_approval_timestamp =
                        ...
                        AND th_ar_ind = 'A'
                        AND th_net_ccy = in_ccy
                        AND th_txn_id = tp_txn_id) txn_header
          WHERE tp_txn_id = th_txn_id;

         dBal := last_open_bal + last_total_float - last_total_hold;
      END IF;
