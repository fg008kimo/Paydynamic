...
IS
   ...
   IS
      ...
      IS
           SELECT /*+ index(txn_detail txn_detail_idx03) */
                  te_txn_id,
                  th_merchant_ref,
                  th_txn_code,
                  tc_desc,
             ...
             FROM txn_detail,
                  (SELECT /*+ index(txn_elements TXN_ELEMENTS_IDX04) */
                          ...
                          te_party_type
                     FROM txn_elements,
                          (SELECT /*+ index(txn_header txn_header_idx21) */ 
                                  th_txn_id,
                                  th_txn_code,
                                  th_approval_timestamp,
                                  th_approval_date,
                          ...
                          (SELECT th_txn_id, th_org_txn_id
                            ...
                            WHERE     th_merchant_id IS NOT NULL
                                  AND th_ar_ind = 'A'
				  AND exists (SELECT 1 /*+ index(t2 txn_header_pk) */
                                              FROM txn_header t2
                                              ...
                                              AND t1.TH_ORG_TXN_ID=t2.TH_TXN_ID )
      ...
      */
      ...
      ELSE
         SELECT NVL (td_current_bal, 0),
           ...
           FROM txn_detail,
                (SELECT max(th_txn_id) as th_txn_id
                   FROM txn_header
                  WHERE     th_approval_timestamp =
                               (SELECT MAX (th_approval_timestamp)
                                 ...
                                 WHERE     th_approval_timestamp <
                                       ...
                                       AND th_txn_id = td_txn_id
                                       and td_txn_country = in_country)
                        AND th_approval_timestamp < in_approval_date_from
                        AND th_merchant_id = in_merchant_id
                        AND th_ar_ind = 'A'
                        AND th_service_code = in_service_code
