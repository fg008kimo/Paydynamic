DROP VIEW PSP_SETTLE_DELIVER_VIEW;

/* Formatted on 4/2/2013 6:44:04 PM (QP5 v5.240.12305.39476) */
CREATE OR REPLACE FORCE VIEW PSP_SETTLE_DELIVER_VIEW
(
   OUT_ID,
   PSP_ID,
   OUT_CODE,
...
AS
     SELECT vw.out_id,
            ...
            vw.in_timestamp,
            MAX.can_void
       FROM psp_settle_deliver_tmp_view vw
            LEFT JOIN
            (  SELECT out_ccy,
                      in_ccy,
   ...
   ORDER BY vw.in_timestamp DESC, vw.out_timestamp DESC;
