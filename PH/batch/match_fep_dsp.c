...
static unsigned int sqlctx = 4869771;
...
-------------------------------                    ------------            --------------
Init Version                                       2013/02/22              Stan Poon
*/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
void match(char* cs_ph_buf, char* cs_psp_buf)
{
	...
	sprintf(csDate,"%.*s",PD_DATE_LEN,csPhList[IDX_TXN_DATE]);
        ...
        for (i = 0; i < MAX_FIELD_NO; i++) {
		if(i==IDX_TXN_SEQ || i==IDX_TXN_AMOUNT ){
