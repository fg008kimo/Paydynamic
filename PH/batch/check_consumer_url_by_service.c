...
{
    32,
    "check_consumer_url_by_service.pc"
};


static unsigned int sqlctx = 1225031315;
...
s NULL and th_status = dm_status ) ) and th_txn_code = dm_txn_code ) WHERE td\
_fe_domain is not null group by td_fe_domain , th_status , th_ar_ind , dm_sta\
tus_desc , th_sub_status , ds_name , short_name order by td_fe_domain        \
    ";
...
24,0,0,1,117,0,6,145,0,0,3,3,0,1,0,2,3,0,0,1,9,0,0,2,102,0,0,
51,0,0,0,0,0,13,166,0,0,7,1,0,1,0,1,102,0,0,2,3,0,0,2,3,0,0,2,3,0,0,2,3,0,0,2,
3,0,0,2,9,0,0,
94,0,0,0,0,0,15,201,0,0,1,1,0,1,0,1,102,0,0,
113,0,0,0,0,0,78,202,0,0,1,1,0,1,0,3,102,0,0,
132,0,0,0,0,0,15,205,0,0,1,1,0,1,0,1,102,0,0,
151,0,0,0,0,0,78,206,0,0,1,1,0,1,0,3,102,0,0,
170,0,0,2,0,0,32,219,0,0,0,0,0,1,0,
185,0,0,3,1372,0,9,373,0,2049,5,5,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,
0,
220,0,0,3,0,0,13,375,0,0,11,0,0,1,0,2,3,0,0,2,9,0,0,2,9,0,0,2,1,0,0,2,1,0,0,2,
9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,
279,0,0,3,0,0,15,488,0,0,0,0,0,1,0,
294,0,0,3,0,0,15,627,0,0,0,0,0,1,0,
309,0,0,4,0,0,32,628,0,0,0,0,0,1,0,
324,0,0,5,219,0,6,661,0,0,4,4,0,1,0,2,3,0,0,2,3,0,0,1,9,0,0,1,9,0,0,
355,0,0,6,0,0,32,695,0,0,0,0,0,1,0,
370,0,0,7,0,0,17,726,0,0,1,1,0,1,0,1,9,0,0,
389,0,0,7,0,0,21,727,0,0,0,0,0,1,0,
404,0,0,8,0,0,32,737,0,0,0,0,0,1,0,
419,0,0,9,434,0,4,788,0,0,3,2,0,1,0,2,3,0,0,1,9,0,0,1,9,0,0,
446,0,0,10,391,0,5,807,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
469,0,0,11,0,0,32,836,0,0,0,0,0,1,0,
484,0,0,12,380,0,5,857,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
507,0,0,13,0,0,32,879,0,0,0,0,0,1,0,
};
...
int find_checking_parameters(const unsigned char* csServiceCode);
...
*/
	if(iRet == SUCCESS)
		...
		iRet = find_checking_parameters((const unsigned char*)cs_service_code);
...

{
 ...
 if (sqlca.sqlcode < 0) goto find_error;
        ...
        /* EXEC SQL EXECUTE
                BEGIN
                        :hv_return_value := sp_check_hand_shake_init(:hv_service_code:ind_service_code,
								     :c_cursor_hs_check);
...

{
 ...
 :hv_service_code:ind_service_code , :c_cursor_hs_check ) ; END ;";
        sqlstm.iters = (unsigned int  )1;
...

{
        ...
        sqlstm.sqltdsp = &sqltds;
        sqlbuft((void **)0,
          "select count ( * ) , short_name , td_fe_domain , th_status , th_a\
r_ind , dm_status_desc , th_sub_status , ds_name , to_char ( sysdate , 'DD-M\
ON-YYYY HH24:MI:SS' ) , to_char ( sysdate - ( :b0 / 1440 ) , 'DD-MON-YYYY HH\
24:MI:SS' ) , to_char ( to_date ( :b1 , 'YYYYMMDDHH24MISS' ) , 'DD-MON-YYYY \
HH24:MI:SS' ) from ( select td_fe_domain , th_status , th_ar_ind , dm_status\
_desc , th_sub_status , ds_name , short_name from ( select th_txn_id , th_me\
rchant_id , short_name , th_txn_code , th_status , th_ar_ind , th_sub_status\
 , ds_name , td_fe_domain from txn_header , def_sub_status , merch_detail , \
txn_detail where th_txn_code in ( 'DSI' ) and ds_sub_status = th_sub_status \
and th_merchant_id = merchant_id and th_txn_id = td_txn_id and th_create_tim\
estamp between to_date ( :b1 , 'YYYYMMDDHH24MISS' ) and sysdate - ( :b0 / 14\
40 ) and th_merchant_id = :b4 order by th_create_timestamp DESC ) left join \
...

{
                ...
                sqlstm.sqadto[0] = (unsigned short )0;
                sqlstm.sqtdso[0] = (unsigned short )0;
                sqlstm.sqhstv[1] = (unsigned char  *)&v_merchant_name;
                sqlstm.sqhstl[1] = (unsigned long )53;
                sqlstm.sqhsts[1] = (         int  )0;
                sqlstm.sqindv[1] = (         short *)&ind_merchant_name;
                sqlstm.sqinds[1] = (         int  )0;
                sqlstm.sqharm[1] = (unsigned long )0;
                sqlstm.sqadto[1] = (unsigned short )0;
                sqlstm.sqtdso[1] = (unsigned short )0;
                sqlstm.sqhstv[2] = (unsigned char  *)&v_domain;
                sqlstm.sqhstl[2] = (unsigned long )53;
                sqlstm.sqhsts[2] = (         int  )0;
                sqlstm.sqindv[2] = (         short *)&ind_domain;
                sqlstm.sqinds[2] = (         int  )0;
                sqlstm.sqharm[2] = (unsigned long )0;
                sqlstm.sqadto[2] = (unsigned short )0;
                sqlstm.sqtdso[2] = (unsigned short )0;
...
url = :hv_domain and cu_merchant_id = '000' and cu_service_code = :hv_service\
_code ; END ;";
        sqlstm.iters = (unsigned int  )1;
...

{
        ...
        sqlstm.sqladtp = &sqladt;
        sqlstm.sqltdsp = &sqltds;
        sqlstm.stmt = "select to_char ( sysdate - 1 / 24 , 'YYYYMMDDHH24MI'\
 ) - to_char ( ch_cutoff_checking_time , 'YYYYMMDDHH24MI' ) INTO :b0:b1 FROM \
check_hand_shake WHERE ch_party_id = ( select decode ( ind , 1 , id , '000' )\
 as party_id from ( select a . party_id as id , b . ind as ind from ( select \
:b2 as party_id from dual ) a left join ( select ch_party_id , 1 as ind from \
check_hand_shake ) b on a . party_id = b . ch_party_id ) where id = :b2 ) ";
        sqlstm.iters = (unsigned int  )1;
...

{
                        ...
                        sqlstm.sqladtp = &sqladt;
                        sqlstm.sqltdsp = &sqltds;
                        sqlstm.stmt = "update check_hand_shake SET ch_cutof\
f_checking_time = sysdate - 1 / 24 , ch_update_timestamp = sysdate WHERE ch_p\
arty_id = ( select decode ( ind , 1 , id , '000' ) as party_id from ( select \
a . party_id as id , b . ind as ind from ( select :b0 as party_id from dual )\
 a left join ( select ch_party_id , 1 as ind from check_hand_shake ) b on a .\
 party_id = b . ch_party_id ) where id = :b0 ) ";
                        sqlstm.iters = (unsigned int  )1;
