...
static unsigned int sqlctx = 84363;
...
//int main(int argc, char* argv[])
{
	...
	strcpy(csTmp, "ABCabc");
        int len = strlen(csTmp);
printf("len = [%d]\n", len);

        for(i = 0; i < len; i++){
printf("tmp = [%d]  [%d] [%d]\n", (int)csTmp[i], (len-i), (int)(csTmp[i]*(len-i)));

		sum = sum + (int)csTmp[i]*(len-i);
        }
...
void output(char csPhList[][MAX_FIELD_LEN + 1], char csPspList[][MAX_FIELD_LEN + 1],char* remark)
{
	...
	if (csPspList == NULL ) {
		for (i = 0 ; i < MAX_FIELD_NO; i++) {
			if ((i == IDX_TID) || (i==IDX_TXN_SEQ) || (i==IDX_TXN_DATE)) 
...
void match(char* cs_ph_buf, char* cs_psp_buf)
{
	...
	sprintf(csDate,"%.*s",PD_DATE_LEN,csPhList[IDX_TXN_DATE]);
        ...
        for (i = 0; i < MAX_FIELD_NO; i++) {
		if(i!=IDX_TXN_DATE && i!=IDX_TID && i!=IDX_STATUS && i!=IDX_SERVICE_FEE){
		...
		strcpy(csAlertList[iAlertCnt], csPhList[IDX_TXN_SEQ]);
                iAlertCnt++;
DEBUGLOG(("match: failed, iAlertCnt=[%d], txn_seq=[%s]\n",iAlertCnt,csPhList[IDX_TXN_SEQ]));
        }
	else{
