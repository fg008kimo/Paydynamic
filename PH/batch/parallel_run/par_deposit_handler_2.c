...
{
    24,
    "par_deposit_handler_2.pc"
};


static unsigned int sqlctx = 1289985691;
...
int 	CreateNewDepositTxn(hash_t *hMyHash, hash_t *hContext, hash_t *hRequest, hash_t *hResponse)
{
	...
	char    csLocalTxnDateTime[PD_DATE_LEN + PD_TIME_LEN + 1];
	char    *csLocalDate;
	char    *csLocalTime;
...
DEBUGLOG(("par_deposit_handler_2::CreateNewDepositTxn Date [%s] Time [%s]\n", csLocalDate, csLocalTime));

		memset(csLocalTxnDateTime, 0, sizeof(csLocalTxnDateTime));
		sprintf(csLocalTxnDateTime, "%s%s", csLocalDate, csLocalTime);

DEBUGLOG(("par_deposit_handler_2::CreateNewDepositTxn TxnDateTime [%s]\n", csLocalTxnDateTime));

		PutField_CString(hRequest, "transmission_datetime", csLocalTxnDateTime);
...
DEBUGLOG(("par_deposit_handler_2::process_deposit_approve:GetPSPRelatdedInfo amount [%lf]\n", dTmp)); 
		...
		/***** Start Add on 20130225 *******/
		else {
			if (!GetField_CString(hMyHash, "psp_channel_code", &csTmp)) {
				iRet = GetPSPRelatedInfo(hMyHash);

				if (GetField_CString(hMyHash, "psp_channel_code", &csTmp)) {
DEBUGLOG(("par_deposit_handler_2::process_deposit_approve:GetPSPRelatdedInfo psp_channel_code 11111 [%s]\n", csTmp)); 
				}
