...
static unsigned int sqlctx = 1294243;
...
{12,4130,871,0,0,
5,0,0,1,136,0,6,71,0,0,5,5,0,1,0,2,3,0,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
40,0,0,2,0,0,32,108,0,0,0,0,0,1,0,
55,0,0,3,129,0,9,150,0,0,0,0,0,1,0,
70,0,0,3,0,0,13,152,0,0,2,0,0,1,0,2,9,0,0,2,1,0,0,
93,0,0,4,103,0,6,166,0,0,4,4,0,1,0,2,3,0,0,1,9,0,0,1,1,0,0,1,9,0,0,
124,0,0,3,0,0,15,182,0,0,0,0,0,1,0,
139,0,0,3,0,0,15,190,0,0,0,0,0,1,0,
154,0,0,5,0,0,32,191,0,0,0,0,0,1,0,
};
...
int batch_proc(int argc, char* argv[])
{
        ...
        /* EXEC SQL BEGIN DECLARE SECTION; */ 
                ...
                /* varchar         hv_lastphdate_code[PD_CODE_LEN]; */ 
struct { unsigned short len; unsigned char arr[10]; } hv_lastphdate_code;
...
struct { unsigned short len; unsigned char arr[20]; } hv_update_user;
        ...
        hv_lastphdate_code.len = strlen(argv[1]);
        memcpy(hv_lastphdate_code.arr,argv[1],hv_lastphdate_code.len);
DEBUGLOG(("batch_proc: lastphdate_code = [%.*s]\n",hv_lastphdate_code.len,hv_lastphdate_code.arr));

        hv_curr_code.len = strlen(argv[2]);
        memcpy(hv_curr_code.arr,argv[2],hv_curr_code.len);
DEBUGLOG(("batch_proc: curr_code = [%.*s]\n",hv_curr_code.len,hv_curr_code.arr));

        hv_last_code.len = strlen(argv[3]);
        memcpy(hv_last_code.arr,argv[3],hv_last_code.len);
...
DEBUGLOG(("batch_proc: update_user = [%.*s]\n",hv_update_user.len,hv_update_user.arr));
        ...
        /* EXEC SQL EXECUTE
                BEGIN
                :v_return_value := sp_system_control_chg_eoddate(
                                        :hv_lastphdate_code,
                                        :hv_curr_code,
                                        :hv_last_code,
                                        :hv_update_user);
...

{
        ...
        sqlstm.stmt = "begin :v_return_value := sp_system_control_chg_eodda\
te ( :hv_lastphdate_code , :hv_curr_code , :hv_last_code , :hv_update_user ) \
; END ;";
        ...
        sqlstm.sqhstv[1] = (unsigned char  *)&hv_lastphdate_code;
        ...
        sqlstm.sqhstv[2] = (unsigned char  *)&hv_curr_code;
        ...
        sqlstm.sqhstv[3] = (unsigned char  *)&hv_last_code;
        ...
        else if (v_return_value == SP_EOD_DATE_SAME ) {
                printf("Current EOD DATE Same As Last Host Date\n");
                iRet = FAILURE;
        }
