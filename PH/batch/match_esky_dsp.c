...
static unsigned int sqlctx = 9745163;
...
-------------------------------                    ------------            --------------
Init Version                                       2013/06/05              LokMan Chow
Update						   2017/04/10		   Elvis Wong
*/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
void match(char* cs_ph_buf, char* cs_psp_buf)
{
	...
	sprintf(csDate,"%.*s",PD_DATE_LEN,csPhList[IDX_TXN_DATE]);
        ...
        for (i = 0; i < MAX_FIELD_NO; i++) {
		if(i!=IDX_SERVICE_FEE && i!=IDX_TXN_DATE && i!=IDX_TID && i!=IDX_STATUS){
