...
{
    25,
    "offline_payout_approve.pc"
};


static unsigned int sqlctx = 1771229821;
...
{12,4130,871,0,0,
5,0,0,0,0,0,56,367,0,0,1,1,0,1,0,3,102,0,0,
24,0,0,1,153,0,6,374,0,0,4,4,0,1,0,2,3,0,0,1,9,0,0,1,3,0,0,2,102,0,0,
55,0,0,2,257,0,6,389,0,0,8,8,0,1,0,2,3,0,0,1,9,0,0,1,3,0,0,1,3,0,0,1,9,0,0,1,4,
0,0,1,4,0,0,2,102,0,0,
102,0,0,3,140,0,6,403,0,0,4,4,0,1,0,2,3,0,0,1,9,0,0,1,3,0,0,2,102,0,0,
133,0,0,0,0,0,13,425,0,0,3,1,0,1,0,1,102,0,0,2,9,0,0,2,3,0,0,
160,0,0,0,0,0,15,479,0,0,1,1,0,1,0,1,102,0,0,
179,0,0,0,0,0,78,480,0,0,1,1,0,1,0,3,102,0,0,
198,0,0,0,0,0,15,490,0,0,1,1,0,1,0,1,102,0,0,
217,0,0,0,0,0,78,491,0,0,1,1,0,1,0,3,102,0,0,
236,0,0,4,937,0,4,577,0,0,30,2,0,1,0,2,9,0,0,2,3,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,4,0,0,
2,4,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,4,0,0,2,9,0,0,2,4,0,0,2,9,0,0,2,4,0,0,2,4,0,
0,2,9,0,0,2,9,0,0,2,1,0,0,1,9,0,0,1,3,0,0,
371,0,0,5,371,0,4,876,0,0,7,3,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,1,9,0,0,1,
3,0,0,1,3,0,0,
414,0,0,6,394,0,4,958,0,0,6,2,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,1,9,0,0,1,
3,0,0,
};
...
int GetHeaderbyTmp(const char *csBatchId, hash_t* hTxn);
...
DEBUGLOG(("Authorize::batch_id= [%s]\n",cs_batch_id));
	...
	if(c_mode!=PD_MODE_APPROVE && c_type==PD_BY_SELECTED_TXN){
		iRet = GetHeaderbyTmp(cs_batch_id,hContext);
	}
	else{
...
ERRLOG("offline_payout_approve: Authorize::Unable to find CTPHDATE\n");
                }
...
DEBUGLOG(("Authorize::add_user= [%s]\n",cs_user));
	...
	if(c_type == PD_BY_SELECTED_TXN){
		PutField_CString(hContext,"tmp_batch_id",cs_batch_id);
	}
...
DEBUGLOG(("Authorize::call GetSeqList\n"));
		...
		if(iRet == PD_OK && c_type != PD_BY_SELECTED_TXN){
...
ERRLOG("Authorize::DBOLMerchantUploadFileHeader:Update Failed\n");
		...
		if(iRet == PD_OK && c_type == PD_BY_SELECTED_TXN){
			int iBatchCnt = 0;
			DBObjPtr = CreateObj(DBPtr,"DBOLPayoutApproveTmp","GetOrgBatchId");
			iRet = (unsigned long) ((*DBObjPtr)(hContext));

			if(iRet == PD_OK){
				GetField_Int(hContext,"org_batch_id_cnt",&iBatchCnt);

				for(i=0; i<iBatchCnt; i++){
					sprintf(csTag,"org_batch_id_%d",i);
					GetField_CString(hContext,csTag,&csBatchId);

					iChk = 0;
					DBObjPtr = CreateObj(DBPtr,"DBOLMerchantUploadFileHeader","MatchBatchStatus_ForUpdate");
					iChk = (unsigned long) ((*DBObjPtr)(csBatchId,PD_PAYOUTFILE_APPROVE_PROCESSING));
					if(iChk==PD_NOT_FOUND){
DEBUGLOG(("Authorize::the file is not for approve!!\n"));
ERRLOG("offline_payout_approve: Authorize::the file is not for approve!!\n");
					}
					else{
						hash_init(hTxn,0);
						...
						if((unsigned long) ((*DBObjPtr)(hTxn))!=PD_OK){
							iRet = INT_ERR;
DEBUGLOG(("Authorize::DBOLMerchantUploadFileHeader:Update Failed\n"));
ERRLOG("offline_payout_approve: Authorize::DBOLMerchantUploadFileHeader:Update Failed\n");
						}
					}
				}
			}
...
struct { unsigned short len; unsigned char arr[150]; } hv_in_bank_name;
...

{
        ...
        if (sqlca.sqlcode < 0) goto get_seqlist_err;
}
...

{
  ...
  sqlstm.sqladtp = &sqladt;
  sqlstm.sqltdsp = &sqltds;
  sqlstm.stmt = "begin :hv_return_value := sp_ol_merch_upload_dt_palist ( :\
hv_batch_id:ind_in_batch_id , :hv_pre_approve_id:ind_pre_approve_id , :c_curs\
or_dtcon ) ; END ;";
  sqlstm.iters = (unsigned int  )1;
  ...
  sqlstm.sqhstv[2] = (unsigned char  *)&hv_pre_approve_id;
  sqlstm.sqhstl[2] = (unsigned long )sizeof(int);
  sqlstm.sqhsts[2] = (         int  )0;
  sqlstm.sqindv[2] = (         short *)&ind_pre_approve_id;
...

{
                 ...
                 sqlstm.sqladtp = &sqladt;
                 sqlstm.sqltdsp = &sqltds;
                 sqlstm.stmt = "begin :hv_return_value := sp_ol_merch_uploa\
d_dt_seqlist ( :hv_batch_id:ind_in_batch_id , :hv_last_seq:ind_last_seq , :hv\
_status:ind_in_status , :hv_in_bank_name:ind_in_bank_name , :hv_min_amt:ind_m\
in_amt , :hv_max_amt:ind_max_amt , :c_cursor_dtcon ) ; END ;";
                 sqlstm.iters = (unsigned int  )1;
                 ...
                 sqlstm.sqadto[1] = (unsigned short )0;
                 sqlstm.sqtdso[1] = (unsigned short )0;
                 sqlstm.sqhstv[2] = (unsigned char  *)&hv_last_seq;
                 sqlstm.sqhstl[2] = (unsigned long )sizeof(int);
                 sqlstm.sqhsts[2] = (         int  )0;
                 sqlstm.sqindv[2] = (         short *)&ind_last_seq;
                 sqlstm.sqinds[2] = (         int  )0;
                 sqlstm.sqharm[2] = (unsigned long )0;
                 sqlstm.sqadto[2] = (unsigned short )0;
                 sqlstm.sqtdso[2] = (unsigned short )0;
                 ...
                 sqlstm.sqhstl[3] = (unsigned long )sizeof(int);
                 sqlstm.sqhsts[3] = (         int  )0;
                 sqlstm.sqindv[3] = (         short *)&ind_in_status;
                 sqlstm.sqinds[3] = (         int  )0;
                 sqlstm.sqharm[3] = (unsigned long )0;
                 sqlstm.sqadto[3] = (unsigned short )0;
                 sqlstm.sqtdso[3] = (unsigned short )0;
                 sqlstm.sqhstv[4] = (unsigned char  *)&hv_in_bank_name;
                 sqlstm.sqhstl[4] = (unsigned long )152;
                 sqlstm.sqhsts[4] = (         int  )0;
                 sqlstm.sqindv[4] = (         short *)&ind_in_bank_name;
                 ...
                 sqlstm.sqhstv[7] = (unsigned char  *)&c_cursor_dtcon;
			...
			/* EXEC SQL EXECUTE
                           BEGIN
                                :hv_return_value := sp_ol_po_app_tmp_get_seqlist(
                                                                :hv_batch_id:ind_in_batch_id,
                                                                :hv_status:ind_in_status,
                                                                :c_cursor_dtcon);
                           END;
...

{
   ...
   sqlstm.sqladtp = &sqladt;
   sqlstm.sqltdsp = &sqltds;
   sqlstm.stmt = "begin :hv_return_value := sp_ol_po_app_tmp_get_seqlist ( \
:hv_batch_id:ind_in_batch_id , :hv_status:ind_in_status , :c_cursor_dtcon ) ;\
 END ;";
   sqlstm.iters = (unsigned int  )1;
...
//seq_num//
                        if(ind_batch_id>=0 && ind_seq_num>=0){
				v_batch_id.arr[v_batch_id.len] = '\0';
//DEBUGLOG(("GetSeqList batch_id[%s], seq_num=[%d]\n",v_batch_id.arr,v_seq_num));

				if(GetPayoutRecord((const char*)v_batch_id.arr,v_seq_num,hTxn) == PD_FOUND){
...
struct { unsigned short len; unsigned char arr[16]; } hv_in_batch_id;
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "select oud_batch_id , oud_seq_num , oud_txn_id , oud_mercha\
nt_ref , oud_country , oud_identity_id , oud_account_num , oud_account_name ,\
 oud_bank_name , oud_bank_code , oud_branch , oud_phone_num , oud_province , \
oud_city , oud_payout_amount , oud_request_amount , oud_payout_currency , oud\
_request_currency , oud_member_fee_ccy , oud_member_fee , oud_merchant_fee_cc\
y , oud_merchant_fee , oud_markup_ccy , oud_markup_amt , oud_exchange_rate , \
oud_response_code , oud_remark , oud_batch_mode INTO :b0:b1 , :b2:b3 , :b4:b5\
...
b48:b49 , :b50:b51 , :b52:b53 , :b54:b55 FROM ol_merchant_upload_file_detail \
WHERE oud_batch_id = :b56 AND oud_disabled = 0 AND oud_seq_num = :b57 AND oud\
_status = 69 for update nowait ";
 sqlstm.iters = (unsigned int  )1;
 ...
 sqlstm.sqhstv[28] = (unsigned char  *)&hv_in_batch_id;
 sqlstm.sqhstl[28] = (unsigned long )18;
 ...
 sqlstm.sqhstv[29] = (unsigned char  *)&hv_in_seq_num;
...
DEBUGLOG(("GetHeader batch_id = [%.*s]\n",hv_batch_id.len,hv_batch_id.arr));
	...
	/* EXEC SQL
			...
			:v_country:ind_country,
                        :v_ccy:ind_ccy
                from    ol_merchant_upload_file_header, ol_merchant_upload_file_detail
...

{
 ...
 sqlstm.sqladtp = &sqladt;
 sqlstm.sqltdsp = &sqltds;
 sqlstm.stmt = "select ouh_merchant_id , ouh_service_code , oud_country , o\
ud_request_currency into :b0:b1 , :b2:b3 , :b4:b5 , :b6:b7 from ol_merchant_u\
pload_file_header , ol_merchant_upload_file_detail where ouh_batch_id = :b8 A\
ND ouh_disabled = :b9 AND ouh_batch_id = oud_batch_id AND ouh_disabled = :b9 \
group by ouh_merchant_id , ouh_service_code , oud_country , oud_request_curre\
ncy ";
 sqlstm.iters = (unsigned int  )1;
...
/*merchant_id*/
/*service_code*/
                if(ind_merchant_id>=0 && ind_service_code>=0 && ind_country>=0 && ind_ccy>=0){
...
struct { unsigned short len; unsigned char arr[4]; } v_ccy;
        ...
        hv_batch_id.len = strlen((const char*)csBatchId);
        memcpy(hv_batch_id.arr,(const char*)csBatchId,hv_batch_id.len);
DEBUGLOG(("GetHeaderbyTmp batch_id = [%.*s]\n",hv_batch_id.len,hv_batch_id.arr));
        ...
        /* EXEC SQL
                ...
                where   opt_id =:hv_batch_id
                AND     ouh_batch_id = oud_batch_id
                and     oud_txn_id = opt_txn_id
                AND     ouh_disabled =:hv_disabled
                group by ouh_merchant_id,ouh_service_code,oud_country,oud_request_currency; */ 

{
        ...
        sqlstm.sqladtp = &sqladt;
        sqlstm.sqltdsp = &sqltds;
        sqlstm.stmt = "select ouh_merchant_id , ouh_service_code , oud_coun\
try , oud_request_currency into :b0:b1 , :b2:b3 , :b4:b5 , :b6:b7 from ol_mer\
chant_upload_file_header , ol_merchant_upload_file_detail , ol_payout_approve\
_tmp where opt_id = :b8 AND ouh_batch_id = oud_batch_id and oud_txn_id = opt_\
txn_id AND ouh_disabled = :b9 group by ouh_merchant_id , ouh_service_code , o\
ud_country , oud_request_currency ";
        sqlstm.iters = (unsigned int  )1;
...
/*service_code*/
		if(ind_merchant_id>=0 && ind_service_code>=0 && ind_country>=0 && ind_ccy>=0){
                        v_merchant_id.arr[v_merchant_id.len]='\0';
                        PutField_CString(hTxn,"merchant_id",(const char*)v_merchant_id.arr);
DEBUGLOG(("GetHeaderbyTmp merchant_id=[%s]\n",v_merchant_id.arr));

			v_service_code.arr[v_service_code.len]='\0';
                        PutField_CString(hTxn,"service_code",(const char*)v_service_code.arr);
DEBUGLOG(("GetHeaderbyTmp service_code=[%s]\n",v_service_code.arr));
...
DEBUGLOG(("GetHeaderbyTmp Normal Exit [%d]\n",iRet));
        return  iRet;
