...
static unsigned int sqlctx = 4868747;
...
-------------------------------                    ------------            --------------
Init Version                                       2013/10/02              [STP]
PRD290 add paid amount field			   2020/11/24		   [ANC]
*/
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
char    csAlertList[MAX_FIELD_NO][MAX_FIELD_LEN+1];
int     iAlertCnt = 0;
...
void alert(char csList[][MAX_FIELD_LEN + 1]);
...
void output(char csPhList[][MAX_FIELD_LEN + 1], char csPspList[][MAX_FIELD_LEN + 1],char* remark)
{
	...
	if (csPspList == NULL ) {
		for (i = 0 ; i < MAX_FIELD_NO; i++) {
			...
			else if ((i == IDX_TXN_AMOUNT) || (i == IDX_PAID_AMOUNT)) {
		...
		for (i = 0 ; i < MAX_FIELD_NO; i++) {
				...
				if (strcmp(csPspList[i],csPhList[i])) {
					if ((i == IDX_TXN_AMOUNT) || (i == IDX_PAID_AMOUNT)) {
						dTmp = atof(csPhList[i]);
						printf("%s*%.2f%s",PD_TD,dTmp,PD_TD_END);
					}
					else 
...
void match(char* cs_ph_buf, char* cs_psp_buf)
{
	...
	sprintf(csDate,"%.*s",PD_DATE_LEN,csPhList[IDX_TXN_DATE]);
        ...
        for (i = 0; i < MAX_FIELD_NO; i++) {
		if(i!=IDX_TID && i!=IDX_BILL_NO && i!=IDX_SERVICE_FEE && i!=IDX_TXN_DATE && i!=IDX_STATUS){
			if (strcmp(csPhList[i], csPspList[i]) != 0) {
				i_match = PD_FALSE;
			}
//DEBUGLOG(("match: compare: [%s] [%s]\n",csPhList[i],csPspList[i])); 
		}
        }
        if (i_match == PD_FALSE) {
...
//DEBUGLOG(("GetTxnPspDetail to_ccy = [%s]\n",csToCcy));
                                ...
                                if (GetField_Double(hRec,"paid_amount",&dToAmt)) {
