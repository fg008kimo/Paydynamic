...
{
    21,
    "tpl_send_sett_mail.pc"
};


static unsigned int sqlctx = 172649323;
...
{12,4130,871,0,0,
5,0,0,1,884,0,4,231,0,0,12,1,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,
...
/*
	PutField_CString(hRec,"test","bb");
	PutField_Double(hRec,"test",0.000);
	PutField_Int(hRec,"test",0);
*/

	double dTmp1 = -100.0010019;
//	double dTmp1 = -100.00100044444;
//	double dTmp1 = -100.00100099999;
//	double dTmp1 = -100.001001;
	double dTmp2;

	PutField_CString(hRec,"test","aa");
	PutField_Double(hRec,"test",dTmp1);
	PutField_Int(hRec,"test",-1);

	GetField_Double(hRec,"test",&dTmp2);
printf("[%.7lf] [%.7lf]\n",dTmp1,dTmp2);

	BOObjPtr = CreateObj(BOPtr,"BOAlertEmail","FunctExclude");
	if ((unsigned long)(*BOObjPtr)(hRec) != PD_OK) {
		iRet = FAILURE;
DEBUGLOG(("BOAlertEmail::FunctExclude() FAILURE!!!\n"));
	}
