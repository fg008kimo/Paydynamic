...
static unsigned int sqlctx = 19408875;
...
,a.status ,a.response_code ,d.tp_tid  from payout_record a ,payout_header b ,\
txn_header c ,txn_psp_detail d where ((((((b.merchant_id=:b0 and b.txn_date=:\
b1) and a.merchant_id=b.merchant_id) and a.batch_id=b.batch_id) and c.th_host\
_posting_date=:b1) and c.th_merchant_ref=a.merchant_ref) and d.tp_txn_id=c.th\
_txn_id) order by d.tp_tid            ";

 static char *sq0009 = 
"select a.batch_id ,a.seq_num ,a.merchant_ref ,a.country ,a.identity_id ,a.a\
ccount_name ,a.account_num ,a.bank_name ,a.bank_code ,a.branch ,a.phone_num ,\
a.province ,a.city ,a.amount ,a.payout_currency ,a.dest_currency ,a.checksum \
,a.status ,a.response_code  from payout_record a ,payout_header b where ((((b\
.merchant_id=:b0 and b.txn_date=:b1) and a.merchant_id=b.merchant_id) and a.b\
atch_id=b.batch_id) and a.status in ('I','R')) order by a.seq_num            ";
...
{12,4130,871,0,0,
5,0,0,1,131,0,9,191,0,0,2,2,0,1,0,1,1,0,0,1,3,0,0,
28,0,0,1,0,0,13,193,0,0,1,0,0,1,0,2,9,0,0,
47,0,0,1,0,0,15,212,0,0,0,0,0,1,0,
62,0,0,1,0,0,15,219,0,0,0,0,0,1,0,
77,0,0,2,0,0,32,220,0,0,0,0,0,1,0,
92,0,0,3,95,0,9,258,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
115,0,0,3,0,0,13,261,0,0,2,0,0,1,0,2,1,0,0,2,9,0,0,
138,0,0,3,0,0,15,294,0,0,0,0,0,1,0,
153,0,0,4,0,0,32,295,0,0,0,0,0,1,0,
168,0,0,5,258,0,9,356,0,0,4,4,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,1,9,0,0,
199,0,0,5,0,0,13,358,0,0,4,0,0,1,0,2,9,0,0,2,1,0,0,2,1,0,0,2,9,0,0,
230,0,0,5,0,0,15,416,0,0,0,0,0,1,0,
245,0,0,5,0,0,15,425,0,0,0,0,0,1,0,
260,0,0,6,0,0,32,426,0,0,0,0,0,1,0,
275,0,0,7,575,0,9,532,0,0,3,3,0,1,0,1,9,0,0,1,9,0,0,1,9,0,0,
302,0,0,7,0,0,13,534,0,0,20,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,
2,9,0,0,2,9,0,0,2,1,0,0,2,9,0,0,2,9,0,0,
397,0,0,7,0,0,15,691,0,0,0,0,0,1,0,
412,0,0,7,0,0,15,698,0,0,0,0,0,1,0,
427,0,0,8,0,0,32,699,0,0,0,0,0,1,0,
442,0,0,9,460,0,9,797,0,0,2,2,0,1,0,1,9,0,0,1,9,0,0,
465,0,0,9,0,0,13,799,0,0,19,0,0,1,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,
9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,2,9,0,0,
2,9,0,0,2,9,0,0,2,1,0,0,2,9,0,0,
556,0,0,9,0,0,15,953,0,0,0,0,0,1,0,
571,0,0,9,0,0,15,960,0,0,0,0,0,1,0,
586,0,0,10,0,0,32,961,0,0,0,0,0,1,0,
};
...
add html format					   2010/11/12		   LokMan Chow
add argv:mode (SENT/UPDATED)			   2010/11/23		   LokMan Chow
add: when mode=FAILED				   2010/12/06		   LokMan Chow
...
int process_txn(const char *csMerchId,FILE *fp,const char* csMode);
int process_not_sent(const char* csMerchId,FILE *fp,const char* csMode);
		...
		for(i=0;i<iNumOfRecord;i++){
			...
			if(iRet!=FAILURE){
				...
				if (fp != NULL) {
					...
					iRet = process_txn(csMerchIdList[i],fp,cs_mode);

					if(iRet!=FAILURE)
						iRet = process_not_sent(csMerchIdList[i],fp,cs_mode);
...

{
                ...
                sqlstm.sqhstv[18] = (unsigned char  *)&v_resp_code;
                sqlstm.sqhstl[18] = (unsigned long )9;
                sqlstm.sqhsts[18] = (         int  )0;
                sqlstm.sqindv[18] = (         short *)&ind_resp_code;
                sqlstm.sqinds[18] = (         int  )0;
                sqlstm.sqharm[18] = (unsigned long )0;
                sqlstm.sqadto[18] = (unsigned short )0;
                sqlstm.sqtdso[18] = (unsigned short )0;
                sqlstm.sqhstv[19] = (unsigned char  *)&v_tid;
                sqlstm.sqhstl[19] = (unsigned long )53;
                sqlstm.sqhsts[19] = (         int  )0;
                sqlstm.sqindv[19] = (         short *)&ind_tid;
		...
		if(!strcmp(csMode,"FAILED")){
			if(v_status!=PD_REJECT)
				continue;
		}

		if(v_status==PD_REJECT){
                        if(atoi((const char*)v_resp_code.arr)>1000)
                                continue;
                }
...
/* Field #16 status*/
		if(v_status==PD_SENT)
			fprintf(fp,"%sSent to PSP%s",PD_TD,PD_TD_END);
		else if(v_status==PD_ACCEPT)
			fprintf(fp,"%sAccepted%s",PD_TD,PD_TD_END);
...
/* Field #16 status*/
		if(v_status==PD_INIT_STATE)
			fprintf(fp,"%sNot Sent%s",PD_TD,PD_TD_END);
		else if(v_status==PD_REJECT)
                	fprintf(fp,"%sRejected%s",PD_TD,PD_TD_END);
		else
			fprintf(fp,"%sUnknown Status%s",PD_TD,PD_TD_END);
