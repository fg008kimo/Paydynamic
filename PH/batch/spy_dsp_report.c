...
static unsigned int sqlctx = 10916283;
...
-------------------------------                    ------------            --------------
Init Version                                       2012/04/23              Virginia Yun
*/
...
DEBUGLOG(("%s\n",cs_input_buf));
                ...
                if (p == NULL)
                        return FAILURE;
                strcpy(csTmp,TrimAllChar((const unsigned char*)p,strlen(p),PD_MY_TOEKN));
...
/*DEBUGLOG(("BANK_BILL_NO=[%s]\n",csList[IDX_BILL_NO]));*/
		...
		int iPos=0;
        	p = mystrtok(csList[IDX_TXN_DATE],PD_MY_DAY_TOKEN);
		if(p!=NULL){
			strcpy(csDate[iPos],p);
			iPos++;
		}
                while((p = mystrtok(NULL,PD_MY_DAY_TOKEN))!= NULL) {
			strcpy(csDate[iPos],p);
			iPos++;
		}

		dTmp = atof(csList[IDX_TXN_AMOUNT]);
...
/* Field #3 txn_amount */
		...
		fprintf(fout,"%.*s%s",(int)strlen(csTmpAmt),csTmpAmt,PD_MY_DELIMITOR);

/* Field #4 fee */
		fprintf(fout,"%.*s%s",(int)strlen(csList[IDX_SERVICE_FEE]-1),&csList[IDX_SERVICE_FEE][1],PD_MY_DELIMITOR);

/* Field #5 txn_date*/
		fprintf(fout,"%04d%02d%02d",atoi(csDate[0]),atoi(csDate[1]),atoi(csDate[2]));
