...
static unsigned int sqlctx = 19118403;
...
{12,4130,871,0,0,
5,0,0,1,61,0,4,585,0,0,1,0,0,1,0,2,3,0,0,
24,0,0,2,63,0,9,598,0,2049,0,0,0,1,0,
39,0,0,2,0,0,13,602,0,0,1,0,0,1,0,2,9,0,0,
58,0,0,2,0,0,15,615,0,0,0,0,0,1,0,
73,0,0,3,0,0,32,723,0,0,0,0,0,1,0,
};
...
char    csReqURL[ERS_REQ_URL_LEN + 1];

char *replace(const char *s, char ch, const char *repl);

int varify_value(rData *oResp);
...
DEBUGLOG(("Output Invalid: %s\n",sRate+4));
			...
			{
DEBUGLOG(("Result: min_bid %s, bid %s, max_bid %s, min_ask %s, ask %s, max_ask %s\n",oResp->min_bid, oResp->bid, oResp->max_bid, oResp->min_ask, oResp->ask, oResp->max_ask));
				////some values will appear as "na", replace them by mid value////
				if( varify_value(oResp) == SUCCESS){
					bResult=SUCCESS;
				}
...
DEBUGLOG(("id[%s %s]: return values are Invalid. Process Ended [%s]\n", oResp->sFromCCY, oResp->sToCCY, sRate+4));
			}
		//}
...
int check_is_double(char *value)
{
	int     iRet = PD_FALSE;
	...
	p = (char*)strchr((const char *)value, '.');
        if (p == NULL){
                if(is_numeric((char*)value)!=PD_FALSE){
                        iRet= PD_TRUE;
                }
        }
        else{
                if(sscanf((const char *)value,"%lf",&dTmp)==1){
                        iRet = PD_TRUE;
                }
        }
...
int varify_value(rData *oResp)
{
	...
	if(check_is_double((char*)&(oResp->bid)) && check_is_double((char*)&(oResp->ask))){
		iRet = SUCCESS;
	}

	if(iRet==SUCCESS){
		if( !strncmp("na",oResp->min_bid,2) || !strncmp("NA",oResp->min_bid,2) ){
			strcpy(oResp->min_bid,oResp->bid);
		}
		else{
			 if(!check_is_double((char*)&(oResp->min_bid))){
				iRet = FAILURE;
			 }
		}

		if( !strncmp("na",oResp->max_bid,2) || !strncmp("NA",oResp->max_bid,2) ){
			strcpy(oResp->max_bid,oResp->bid);
		}
		else{ 
                         if(!check_is_double((char*)&(oResp->max_bid))){
                                iRet = FAILURE;
                         }
                }


		if( !strncmp("na",oResp->min_ask,2) || !strncmp("NA",oResp->min_ask,2) ){
			strcpy(oResp->min_ask,oResp->ask);
		}
		else{ 
                         if(!check_is_double((char*)&(oResp->min_ask))){
                                iRet = FAILURE;
                         }
                }


		if( !strncmp("na",oResp->max_ask,2) || !strncmp("NA",oResp->max_ask,2) ){
			strcpy(oResp->max_ask,oResp->ask);
		}
		else{
                         if(!check_is_double((char*)&(oResp->max_ask))){
                                iRet = FAILURE;
                         }
                }
	}
...
*/
        ...
        sprintf(sTmpReq, "%s%s&Quotecurrency: %s&Basecurrency: %s&Date: %s GMT&Quotetype: %s",csReqURL,REQUEST_HEADER, sFromCCY, sToCCY,sDate,QUOTE_TYPE);
...
DEBUGLOG(("Overall: id[%s %s] date[%s] values[%f %f %f %f %f %f] MeanMinMax[%f] MinMax[%f] rate[%f]\n",oResult->sFromCCY, oResult->sToCCY, oResult->sDate, dMinBid, dMedBid, dMaxBid, dMinAsk, dMedAsk, dMaxAsk, dMeanMinMax, dMinMax, dRate));
	...
	////insert the record into table exchange_rate_ers////
	DBObjPtr = CreateObj(DBPtr,"DBExchangeRateERS","Add");
...
struct { unsigned short len; unsigned char arr[4]; } v_currencyId;
...

{
 ...
 sqlstm.stmt = "select COUNT ( * ) INTO :b0 FROM currency WHERE disabled = \
0 ";
...

{
  ...
  sqlstm.sqhstv[0] = (unsigned char  *)&v_currencyId;
  sqlstm.sqhstl[0] = (unsigned long )6;
  sqlstm.sqhsts[0] = (         int  )0;
  sqlstm.sqindv[0] = (         short *)&ind_v_currencyId;
  sqlstm.sqinds[0] = (         int  )0;
  sqlstm.sqharm[0] = (unsigned long )0;
  sqlstm.sqadto[0] = (unsigned short )0;
  sqlstm.sqtdso[0] = (unsigned short )0;
...

{
 ...
 if (sqlca.sqlcode < 0) goto sql_error;
	...
	// Get Request URL from System Parameter
	memset(csReqURL, 0, sizeof(csReqURL));
	strcpy(csReqURL, REQUEST_URL); // Default

/*	
	DBObjPtr = CreateObj(DBPtr,"DBSystemParameter","FindCode");
        if((unsigned long int)(*DBObjPtr)(ERS_REQ_URL,csReqURL) == FOUND)
        {
DEBUGLOG(("Request URL:[%s]\n",csReqURL));
        }
        else
        {
DEBUGLOG(("Get Request URL Error\n"));
                return FAILURE;
        }
*/
