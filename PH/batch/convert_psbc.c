...
static unsigned int sqlctx = 2396595;
...
ERRLOG("convert_psbc: batch_proc: format_id not found\n");
				iErrorCode = INT_FORMAT_TEMPLATE_ERROR;
				iRet = FAILURE;
				break;
			}
...
DEBUGLOG(("--------------------\n"));
DEBUGLOG(("reverse (Curr): txn_amount = [%.2lf] balance = [%.2lf] tfr_type = [%s]\n", dCurrTxnAmt, dCurrBal, csCurrKeyword));
DEBUGLOG(("reverse (Prev): txn_amount = [%.2lf] balance = [%.2lf] tfr_type = [%s]\n", dPrevTxnAmt, dPrevBal, csPrevKeyword));
...
ERRLOG("convert_psbc: batch_proc: call CheckDetail() fail\n");
				...
				if (GetField_CString(hCurrRec, "balance", &csTmp)) {
					dCurrBal = string2doublewithsign((const unsigned char *)csTmp);
				}
...
DEBUGLOG(("--------------------\n"));
DEBUGLOG(("normal (Prev): txn_amount = [%.2lf] balance = [%.2lf] tfr_type = [%s]\n", dPrevTxnAmt, dPrevBal, csPrevKeyword));
DEBUGLOG(("normal (Curr): txn_amount = [%.2lf] balance = [%.2lf] tfr_type = [%s]\n", dCurrTxnAmt, dCurrBal, csCurrKeyword));
