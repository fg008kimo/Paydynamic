...
static unsigned int sqlctx = 9736635;
...
-------------------------------                    ------------            --------------
Init Version                                       2014/07/29              LokMan Chow
*/
...
{
	char *amtptr;
	char *csOutAmtTmp = (char*) malloc (64);
	double dAmtTmp;

DEBUGLOG(("FormatAmount csInAmt =[%s]\n",csInAmt));

        amtptr = mystrtok(csInAmt, PD_AMT_DELIMITOR);
        if(amtptr!=NULL){
                strcpy(csOutAmtTmp,amtptr);
                while ((amtptr = mystrtok(NULL,PD_AMT_DELIMITOR)) != NULL) {
                        strcat(csOutAmtTmp,amtptr);
                }
        }
        else{
                strcpy(csOutAmtTmp,csInAmt);
        }

DEBUGLOG(("FormatAmount csOutAmtTmp =[%s]\n",csOutAmtTmp));

	dAmtTmp = atof(csOutAmtTmp);
        sprintf(csOutAmt, "%.2f", dAmtTmp);

DEBUGLOG(("FormatAmount csOutAmt =[%s]\n",csOutAmt));
