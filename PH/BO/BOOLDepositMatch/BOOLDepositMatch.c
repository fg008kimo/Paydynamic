...
DEBUGLOG(("HandleDepositMatchPair:: start\n"));
...
// DEBUGLOG(("call DBOLDepositMatchPair::GetPairByMerchTxnId()\n"));
...
DEBUGLOG(("GetPairByMerchTxnId - cannot get stmt_txn_id\n"));
					iRet = PD_ERR;
					break;
				}
...
DEBUGLOG(("call DBOLDepositMatchPair::DeletePair() merch[%s] psp[%s] failed\n", csTxnSeq, csPairStmtTxnId));
				...
				iPairCnt = 0;
// DEBUGLOG(("call DBOLDepositMatchPair::GetPairByStmtTxnId()\n"));
...
DEBUGLOG(("GetPairByStmtTxnId - cannot get merch_txn_id\n"));
							iRet = PD_ERR;
							break;
						}
...
DEBUGLOG(("call DBOLDepositMatchPair::GetPairByStmtTxnId() failed\n"));
					iRet = PD_ERR;
				}
...
DEBUGLOG(("call DBOLDepositMatchPair::GetPairByMerchTxnId() failed\n"));
			iRet = PD_ERR;
		}
...
DEBUGLOG(("stmt_txn_id not found\n"));
			iRet = PD_ERR;
		} else {
...
DEBUGLOG(("status not match!!! current = [%c], expected = [%c]\n", cTmp, PD_COMPLETE));
				iRet = PD_ERR;
			}
...
DEBUGLOG(("ar_ind not found\n"));
					iRet = PD_ERR;
				} else if (cTmp != PD_ACCEPT) {
DEBUGLOG(("ar_ind not match!!! current = [%c], expected = [%c]\n", cTmp, PD_ACCEPT));
					iRet = PD_ERR;
				}
			}
...
DEBUGLOG(("sub_status not match!!! current = [%s], expected = [%s]\n", csTmp, PD_UNALLOCATED));
					iRet = PD_ERR;
				}
			}
...
DEBUGLOG(("multi_match_ind not found\n"));
					iRet = PD_ERR;
				} else if (iTmp != PD_FALSE) {
DEBUGLOG(("multi_match_ind not match!!! current = [%d], expected = [%d]\n", iTmp, PD_FALSE));
					iRet = PD_ERR;
				}
			}
*/
...
DEBUGLOG(("txn_hold_ind not found\n"));
				iRet = PD_ERR;
			} else if (iTmp != PD_FALSE) {
DEBUGLOG(("txn_hold_ind not match!!! current = [%d], expected = [%d]\n", iTmp, PD_FALSE));
				iRet = PD_ERR;
			}
...
DEBUGLOG(("sys_match_ind not found\n"));
					iRet = PD_ERR;
				} else if (iTmp != PD_TRUE) {
DEBUGLOG(("sys_match_ind not match!!! current = [%d], expected = [%d]\n", iTmp, PD_TRUE));
					iRet = PD_ERR;
				}
			}
...
DEBUGLOG(("statement_date not found\n"));
					iRet = PD_ERR;
				}
			}
...
DEBUGLOG(("statement_time not found\n"));
					iRet = PD_ERR;
				}
			}
...
DEBUGLOG(("txn_amount not found\n"));
	...
	if (iRet == PD_OK) {
// DEBUGLOG(("call DBOLMerchantBankAcct::GetMerchBankAcctDtl()\n"));
		...
		if (iTmpRet != PD_OK) {
DEBUGLOG(("call DBOLMerchantBankAcct::GetMerchBankAcctDtl() failed\n"));
			iRet = PD_ERR;
		} else {
			// get merchant_id
...
DEBUGLOG(("country not found\n"));
	...
	if (iRet == PD_OK) {
// DEBUGLOG(("call DBOLMerchConfig::GetMerchConfig()\n"));
...
/*
	...
	if (iRet == PD_OK) {
// DEBUGLOG(("call DBOLRuleDepositMatching::GetRuleDepositMatchingByDate()\n"));
		...
		if (iTmpRet != PD_OK) {
DEBUGLOG(("call DBOLRuleDepositMatching::GetRuleDepositMatchingByDate() failed\n"));
			iRet = PD_ERR;
		}
	}
*/
	...
	if (iRet == PD_OK) {
// DEBUGLOG(("call DBOLMatchTimeInterval::GetMatchTimeInterval()\n"));
		...
		if (iTmpRet != PD_OK) {
DEBUGLOG(("call DBOLMatchTimeInterval::GetMatchTimeInterval() failed\n"));
		}
	}

	// get match criteria by merchant_id
	if (iRet == PD_OK) {
// DEBUGLOG(("call DBOLAutoMatchCriteria::GetAutoMatchCriteria()\n"));
		...
		if (iTmpRet != PD_OK) {
// DEBUGLOG(("call DBOLAutoMatchCriteria::GetAutoMatchCriteria() failed\n"));
// DEBUGLOG(("call DBOLAutoMatchCriteria::GetAutoMatchCriteria() [DEFAULT]\n"));
			DBObjPtr = CreateObj(DBPtr, "DBOLAutoMatchCriteria", "GetAutoMatchCriteria");
			iTmpRet = (unsigned long)(*DBObjPtr)(PD_SYS_PARTY_ID, rMatchCriteria);
			if (iTmpRet != PD_OK) {
DEBUGLOG(("call DBOLAutoMatchCriteria::GetAutoMatchCriteria() [DEFAULT] failed\n"));
				iRet = PD_ERR;
			}
		}
	}
...
DEBUGLOG(("level[%c]\n", cLevel));
DEBUGLOG(("onus_bf_mins[%d] onus_af_mins[%d]\n", iOnUsBfMins, iOnUsAfMins));
DEBUGLOG(("offus_bf_mins[%d] offus_af_mins[%d]\n", iOffUsBfMins, iOffUsAfMins));
...
DEBUGLOG(("match time interval not found\n"));
				iRet = PD_ERR;
				goto stop_match;
			}
...
DEBUGLOG(("field_cnt not found\n"));
				iRet = PD_ERR;
				goto stop_match;
			} else {
DEBUGLOG(("field_cnt = [%d]\n", iFieldCnt));
...
DEBUGLOG(("txn_id = [%s], cust_deposit_datetime = [%s]\n", csTmpTxnId, csTmpCustDepositDatetime));
...
DEBUGLOG(("CompleteMatching:: start\n"));
...
DEBUGLOG(("call DBOLTransaction::GetTxnIdForUpdate()\n"));
			...
			iTmpRet = (unsigned long)(*DBObjPtr)(csStmtTxnId);
			if (iTmpRet != PD_OK) {
DEBUGLOG(("call DBOLTransaction::GetTxnIdForUpdate() failed\n"));
DEBUGLOG(("cannot lock psp side txn id\n"));
				iRet = PD_ERR;
			}
		}
	}
...
DEBUGLOG(("call DBOLTransaction::GetTxnIdForUpdate()\n"));
		...
		if (iTmpRet != PD_OK) {
DEBUGLOG(("call DBOLTransaction::GetTxnIdForUpdate() failed\n"));
DEBUGLOG(("cannot lock merchant side txn id\n"));
			iRet = PD_ERR;
		}
	}
...
// double check basic matching criteria
	if (iRet == PD_OK) {
DEBUGLOG(("call DBOLTransaction::CheckBasicMatchingCriteria()\n"));
		...
		if (iTmpRet != PD_OK) {
DEBUGLOG(("call DBOLTransaction::CheckBasicMatchingCriteria() failed\n"));
...
DEBUGLOG(("txn header found record = [%s]\n", csTxnSeq));
			hRec = RecordSet_GetFirst(rRecordSet);
...
DEBUGLOG(("GetTxnHeader - service code = [%s]\n", csServiceCode));
...
DEBUGLOG(("txn psp detail found record = [%s]\n", csStmtTxnId));
...
// get merchant side txn detail
	if (iRet == PD_OK) {
		...
		if ((unsigned long)(*DBObjPtr)(csTxnSeq, rRecordSet) == PD_OK) {
DEBUGLOG(("txn detail found record = [%s]\n", csTxnSeq));
			hRec = RecordSet_GetFirst(rRecordSet);
...
DEBUGLOG(("GetTxnDetail - failure_callback_url = [%s]\n", csFailCbUrl));
				PutField_CString(hTxn, "failure_callback_url", csFailCbUrl);
			}
		} else {
DEBUGLOG(("call DBOLTransaction::GetTxnDetail() failed\n"));
			iRet = PD_ERR;
		}
		RecordSet_Destroy(rRecordSet);
	}
...
DEBUGLOG(("merchant config found record\n"));
			// get max_auto_cb_amt
			if (!GetField_Double(hTmp, "max_auto_cb_amt", &dMaxAutoCbAmt)) {
DEBUGLOG(("max_auto_cb_amt not found\n"));
				iRet = PD_ERR;
			}
		} else {
DEBUGLOG(("call DBOLMerchConfig::GetMerchConfig() failed\n"));
...
// need: from_txn_seq, org_txn_ccy, org_txn_amt, party_type(default 'M'), add_user(default 'SYSTEM'), amount_type(default 'CR')
	if (iRet == PD_OK) {
		...
		if ((unsigned long)(*BOObjPtr)(hTxn) != PD_OK) {
			iRet = PD_ERR;
DEBUGLOG(("BOOLTxnElements:AddTxnAmtElement failed!!!\n"));
...
DEBUGLOG(("GetReserveAmount - reserve_amt = [%f]\n", dHoldbackAmt));
				...
				if ((unsigned long)(*BOObjPtr)(hTxn) != PD_OK) {
					iRet = PD_ERR;
DEBUGLOG(("BOOLTxnElements:AddReserveAmtElement failed!!!\n"));
...
DEBUGLOG(("override txn_code [%s] channe_code [%s]\n", PD_INITIAL_OLN_TXN_CODE, PD_CHANNEL_OLN));
		}
	}
*/
...
// put: dst_txn_ccy, dst_txn_amt, ex_rate, ex_party
	if (iRet == PD_OK) {
		...
		if ((unsigned long)(*BOObjPtr)(hTxn, hTmp) != PD_OK) {
			iRet = PD_ERR;
DEBUGLOG(("BOExchange:GetExchangeInfo failed!!!\n"));
...
DEBUGLOG(("BOExchange - ex_party = [%c]\n", cExParty));
		}
		if (GetField_Double(hTxn, "ex_rate", &dExRate)) {
DEBUGLOG(("BOExchange - ex_rate = [%lf]\n", dExRate));
		}
	}
...
// need: from_txn_seq, src_txn_fee_ccy, dst_txn_fee_ccy, src_txn_fee, party_type(default 'M'), amount_type(default 'DR'), dst_txn_fee
	if (iRet == PD_OK) {
		...
		if ((unsigned long)(*BOObjPtr)(hTxn) != PD_OK) {
			iRet = PD_ERR;
DEBUGLOG(("BOOLTxnElements:AddTxnFeeElements failed!!!\n"));
...
// lock opening bal
	if (iRet == PD_OK) {
		...
		if ((unsigned long)(*DBObjPtr)(csMerchantId, csTxnCcy, csTxnCountry, csServiceCode, hBal) != PD_OK) {
			iRet = PD_ERR;
DEBUGLOG(("DBOLMerchantBalance:GetCurrentValuesForUpdate failed!!!\n"));
...
DEBUGLOG(("DBOLMerchantBalance - current_bal = [%f]\n", dOpenBal));
			GetField_Double(hBal, "current_bal_settlement", &dOpenBalSett);
DEBUGLOG(("DBOLMerchantBalance - current_bal_settlement = [%f]\n", dOpenBalSett));
			GetField_Double(hBal, "total_reserved_amount", &dTotalReservedAmt);
DEBUGLOG(("DBOLMerchantBalance - total_reserved_amount = [%f]\n", dTotalReservedAmt));
		}
	}
...
// get approval date and approval timestamp
	if (iRet == PD_OK) {
		...
		if (iTmpRet != FOUND) {
			iRet = PD_ERR;
DEBUGLOG(("DBSystemControl:GetApprovalDT failed!!!\n"));
...
DEBUGLOG(("DBSystemControl - approval_date = [%s]\n", csApprovalDate));
			GetField_CString(hTxn, "approval_timestamp", &csApprovalDT);
DEBUGLOG(("DBSystemControl - approval_timestamp = [%s]\n", csApprovalDT));
		}
	}
...
// update curr bal (TxnAbort if fail)
	if (iRet == PD_OK) {
		...
		if ((unsigned long)(*DBObjPtr)(csMerchantId, csTxnCountry, csTxnCcy, csServiceCode,
			...
			iRet = PD_ERR;
DEBUGLOG(("DBOLMerchantBalance:UpdateCurrBal failed!!!\n"));
...
// update curr bal settlement (TxnAbort if fail)
	if (iRet == PD_OK) {
		...
		if ((unsigned long)(*DBObjPtr)(csMerchantId, csTxnCountry, csTxnCcy, csServiceCode,
			...
			iRet = PD_ERR;
DEBUGLOG(("DBOLMerchantBalance:UpdateSettBal failed!!!\n"));
...
// update total reserved amt (TxnAbort if fail)
	if (iRet == PD_OK) {
		...
		if ((unsigned long)(*DBObjPtr)(csMerchantId, csTxnCountry, csTxnCcy, csServiceCode,
			...
			iRet = PD_ERR;
DEBUGLOG(("DBOLMerchantBalance:UpdateReserved failed!!!\n"));
...
// update merchant side txn header
	if (iRet == PD_OK) {
		...
		if ((unsigned long)(*DBObjPtr)(hTmp) != PD_OK) {
			iRet = PD_ERR;
DEBUGLOG(("DBOLTransaction:Update failed!!!\n"));
...
// update baid txn
	if (iRet == PD_OK) {
		...
		if ((unsigned long)(*DBObjPtr)(hTmp) != PD_OK) {
			iRet = PD_ERR;
DEBUGLOG(("DBOLBAIDTxn:UpdateByDstTxnSeq failed!!!\n"));
...
DEBUGLOG(("send callback\n"));
				...
				if (iTmpRet == PD_OK) {
DEBUGLOG(("send callback success\n"));
					TxnCommit();
				} else {
DEBUGLOG(("send callback failure\n"));
					TxnAbort();
				}
			}
		}
