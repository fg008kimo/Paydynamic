...
#include "internal.h"
#include "common.h"
#include "BOParSettlement.h"

char    cDebug;

void BOParSettlement(char    cdebug)
{
        cDebug = cdebug;
}
...
DEBUGLOG(("BOParSettlement:ProcessSettlementTxn()\n"));
...
DEBUGLOG(("ProcessSettlementTxn() add_user = [%s]\n",csUser));
 	...
 	if (GetField_CString(hRequest,"sett_type",&csTmp)) {
		cTmp = csTmp[0];
		PutField_Char(hContext,"type",cTmp);
DEBUGLOG(("ProcessSettlementTxn() type= [%c]\n",cTmp));
	}
	else{
		PutField_Char(hContext,"type",'M');
DEBUGLOG(("ProcessSettlementTxn() type= [M] (Default)\n"));
	}
...
ERRLOG("BOParSettlement::ProcessSettlementTxn() GetSettlementRecords:invalid admin status!!!\n");
			}
		}
...
ERRLOG("BOParSettlement::ProcessSettlementTxn() GetSettlementRecords:invalid merchant status!!!\n");
			}
		}
...
ERRLOG("BOParSettlement::ProcessSettlementTxn:GetExchangeInfo Error\n");
			}
		}
...
DEBUGLOG(("ProcessSettlementTxn::settlement net amt=[%f]\n",dSrcAmt));
				...
				if(GetField_Double(hContext,"dst_txn_amt",&dTmp)){
					...
					PutField_Double(hContext,"dst_txn_amt",dRealDstAmt);
					PutField_Double(hTxn,"deliver_amt",dRealDstAmt);
					PutField_Double(hContext,"settlement_txn_amt",dRealDstAmt);
					PutField_Double(hContext,"bank_bal",dRealDstAmt);
DEBUGLOG(("ProcessSettlementTxn::settlement dest net amt=[%f]\n",dRealDstAmt));
				...
				iRet = INT_ERR;
				PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("ProcessSettlementTxn::BOParSettlementFee Error\n"));
ERRLOG("BOParSettlement::ProcessSettlementTxn:BOParSettlementFee Error\n");
			}
...
DEBUGLOG(("Authorize::BOBalance:DebitSebBalance Error\n"));
		...
		if(iRet==PD_OK){
			if(GetField_CString(hRequest,"admin_sub_status",&csTmp)){
				if(!strcmp(csTmp,PD_SYSTEM_REQUESTED)){
					DBObjPtr = CreateObj(DBPtr,"DBMerchantBalance","UpdateLastSettlementDate");
...
DEBUGLOG(("BOSettlement::call DBFundsOut->Add\n"));
                        ...
                        PutField_Double(hFunds,"fundout_amt",dRealDstAmt);
                        PutField_CString(hFunds,"fundout_ccy",csToCcy);
...
ERRLOG("BOParSettlement::ProcessSettlementTxn::insufficient aval settlement balance!!\n");
				}
			}
			else{
DEBUGLOG(("ProcessSettlementTxn::GetAvalBalanceForUpdate Error\n"));
ERRLOG("BOParSettlement::ProcessSettlementTxn::GetAvalBalanceForUpdate Error!!\n");
				iRet = INT_ERR;
				PutField_Int(hContext,"internal_error",iRet);
			}
		}
...
DEBUGLOG(("ProcessSettlementTxn::Call DBMerchantBalance:GetSettAvalBalance\n"));
			...
			if ((*DBObjPtr)(hContext,
                                ...
                                csServiceCode,
				&dSettlementBal) != PD_OK) {
DEBUGLOG(("ProcessSettlementTxn::DBMerchantBalance:GetSettAvalBalance Failed!!\n"));
                        	iRet = INT_ERR;
			} 
			else {
DEBUGLOG(("ProcessSettlementTxn::GetAvalBalance for settlement=[%f]\n",dSettlementBal));
...
ERRLOG("BOParSettlement::ProcessSettlementTxn() GetSettlementRecords:merchant cannot cancel!!!\n");
					}
				}
...
DEBUGLOG(("Call BOBalance:RestoreSebBalance()\n"));
			...
			if((unsigned long)(*BOObjPtr)(hContext,hRequest)!=PD_OK)
				iRet = INT_ERR;
DEBUGLOG(("TxnMgtByUsVST: Authroize:: BOBalance:RestoreSebBalance() result = [%d]\n",iRet));
		}
...
DEBUGLOG(("ProcessSettlementTxn::Call BOBalance:CreditMerchantAvalAmount()\n"));
			...
			if((unsigned long)(*BOObjPtr)(hContext,hRequest)!=PD_OK)
				iRet = INT_ERR;
DEBUGLOG(("BOParSettlement: ProcessSettlementTxn:: BOBalance:CreditMerchantAvalAmount() result = [%d]\n",iRet));
		}
		*/
		if (iRet == PD_OK) {
DEBUGLOG(("ProcessSettlementTxn::Call BOBalance:UpdateSettlementAmount()\n"));
			...
			if((unsigned long)(*BOObjPtr)(hContext)!=PD_OK)
				iRet = INT_ERR;
DEBUGLOG(("BOParSettlement: ProcessSettlementTxn:: BOBalance:UpdateSettlementAmount() result = [%d]\n",iRet));
		}
...
DEBUGLOG(("BOParSettlement::ProcessSettlementTxn() unknown txn_code!!!\n"));
ERRLOG("BOParSettlement::ProcessSettlementTxn() unknown txn_code!!!\n");
		iRet = INT_ERR;
	}
...
DEBUGLOG(("BOParSettlement::ProcessSettlementTxn() iRet[%d]\n",iRet));
	FREE_ME(hTxn);
	return	iRet;
}
...
DEBUGLOG(("BOParSettlement:GetSettlementRecords()\n"));
...
ERRLOG("BOParSettlement::GetSettlementRecords() GetHeader::not found record!!\n");
		iRet=INT_NOT_RECORD;
	}
	RecordSet_Destroy(rRecordSet);
...
ERRLOG("BOParSettlement::GetSettlementRecords::GetTxnDetail::not found record!!\n");
                        iRet=INT_NOT_RECORD;
                }
...
ERRLOG("BOParSettlement::GetSettlementRecords::GetSettlementDetail::not found record!!\n");
			iRet = INT_NOT_RECORD;
		}
	}
	...
	RecordSet_Destroy(rRecordSet);
	FREE_ME(rRecordSet);
DEBUGLOG(("BOParSettlement:GetSettlementRecords() iRet=[%d]\n",iRet));
	return	iRet;
}
