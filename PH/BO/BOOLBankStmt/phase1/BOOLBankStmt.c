...
*/
	if (iRet == PD_OK) {
		if (!strcmp(csScriptName, EXCEL_CONVERT_SCRIPT)) {
			...
			snprintf(csSysCmd, sizeof(csSysCmd), "%s \"%s\" \"%s\" \"%s\" \"%s\"", csScriptName, csInFileEncoding, OUT_FILE_ENCODING, csNewFullName, csConvertedFullName);
		}
...
int ProcessStmtFile(hash_t *hContext, hash_t *hRequest, recordset_t *rRecordFormat)
{
	...
	char *csTxnAmt=NULL, *csTxnSeq=NULL;
	int iFullMatch=0, iHoldCreditSide=0, iRealTimePost=0, iDefault=0;
	int iMinAmt=0, iMaxAmt=0, iUniqueAmt=0, iDisplayOrder=0;
	...
	hash_t *hFormat = NULL, *hFile = NULL, *hPrevFile = NULL, *hRec = NULL, *hRecDb = NULL;
	...
	/* Process */
		if (iRet == PD_OK) {
			...
			/* DB deuplication */
				...
				if (iDtlRet == FOUND) {
DEBUGLOG(("ProcessStmtFile() line %d call DBOLStatement::CheckBankStmtRecord() file[%s] FOUND\n",iCurrLine,csFileCompareKey));
					PutField_CString(hFile, "as_ind", "skip");
					iSkipCount++;
				} else if (iDtlRet == NOT_FOUND) {
// DEBUGLOG(("ProcessStmtFile() line %d call DBOLStatement::CheckBankStmtRecord() file[%s] NOT FOUND\n",iCurrLine,csFileCompareKey));
				} else {
					iRet = INT_ERR;
...
DEBUGLOG(("ProcessStmtFile() line %d [%.2lf] [%.2lf] [%.2lf] Match\n",iCurrLine,d2ndLastTxnAmt,dLastTxnAmt,dTxnAmt));
						...
						csTxnCode = NULL;
						if (hPrevFile == NULL) {
DEBUGLOG(("ProcessStmtFile() DBOLBAIDTxn::GetBaidTxn() call\n"));
...
DEBUGLOG(("ProcessStmtFile() call DBSystemParameter::FindCode()\n"));
...
{
	int	iRet = PD_OK;
	int	iDetailRow=0, iSampleRow=0, iCurrLine=0, iStartYear, iReverse;
	int	iTmp;
	char	*csTmp=NULL;
...
/* iSampleRow */
	if (!GetField_Int(hRls,"sample_row",&iSampleRow)){
		iRet = PD_ERR;
DEBUGLOG(("CheckDetail: sample_row NOT FOUND!!!\n"));
	}
...
/* statement_date */
	if(GetField_CString(hRls,"statement_date",&csStmtDate) || GetField_CString(hRls,"statement_datetime",&csStmtDate)) {
		...
		if (strcmp(csOutStmtDate,csStmtDate)) {
			...
			iRet = PD_ERR;
if (iDetailRow == 1) DEBUGLOG(("CheckDetail: Date Format Invalid [%s] in[%s][%s]!!!\n",csOutStmtDate,csStmtDate,csDateTemplate));
		} else {
...
if (iSampleRow == 1) DEBUGLOG(("CheckDetail: Date [%s] in[%s][%s]\n",csOutStmtDate,csStmtDate,csDateTemplate));
		}
...
/* statement_time */
	if(GetField_CString(hRls,"statement_time",&csStmtTime) || GetField_CString(hRls,"statement_datetime",&csStmtTime)) {
		...
		if (strcmp(csOutStmtTime,csStmtTime)) {
			...
			strncat(cs_tmp_buf2,cs_tmp_buf1,sizeof(cs_tmp_buf2)-strlen(cs_tmp_buf2)-1);
			iRet = PD_ERR;
if (iDetailRow == 1) DEBUGLOG(("CheckDetail: Time Format Invalid [%s] in[%s][%s]!!!\n",csOutStmtTime,csStmtTime,csTimeTemplate));
		} else {
...
if (iSampleRow == 1) DEBUGLOG(("CheckDetail: Time  [%s]  in[%s][%s]\n",csOutStmtTime,csStmtTime,csTimeTemplate));
		}
...
/* Default Time */
 ...
 if (iSampleRow == 1) DEBUGLOG(("CheckDetail: Time  [%s]  in[%s]<sys_date[%s]\n","235959",csStmtDate,csSysDate));
		...
		if (strcmp(csCountry,PD_TAIWAN) &&
			...
			if (!GetField_Int(hRls, "new_stmt_time", &iTmp)) {
				if ((strcmp(csStmtDate,csTolDate) == 0 && strcmp(csStmtTime,csTolTime) > 0) ||
					...
					strncat(cs_tmp_buf2,cs_tmp_buf1,sizeof(cs_tmp_buf2)-strlen(cs_tmp_buf2)-1);
					iRet = PD_ERR;
if (iDetailRow == 1) DEBUGLOG(("CheckDetail: Curr Date Time [%s][%s] > Tolerance Date Time [%s][%s]!!!\n",csStmtDate,csStmtTime,csTolDate,csTolTime));
				} else {
 if (iSampleRow == 1) DEBUGLOG(("CheckDetail: Curr Date Time [%s][%s] <= Tolerance Date Time [%s][%s]\n",csStmtDate,csStmtTime,csTolDate,csTolTime));
				}
			} else {
				if (strcmp(csStmtDate,csSysDate) > 0) {
					...
					strncat(cs_tmp_buf2,cs_tmp_buf1,sizeof(cs_tmp_buf2)-strlen(cs_tmp_buf2)-1);
					iRet = PD_ERR;
if (iDetailRow == 1) DEBUGLOG(("CheckDetail:: Curr Date [%s] > Tolerance Date [%s]!!!\n",csStmtDate,csSysDate));
				} else {
 if (iSampleRow == 1) DEBUGLOG(("CheckDetail: Curr Date [%s] <= Tolerance Date [%s]\n",csStmtDate,csSysDate));
				}
			}
		} else {
 if (iSampleRow == 1) DEBUGLOG(("CheckDetail: ** No Tolerance Date Time Check for %s\n",csCountry));
		}
	}
...
/* Time Zone */	
 ...
 if (iSampleRow == 1) DEBUGLOG(("CheckDetail: Change Time Zone [%s] in[%s][%s]\n",csOutStmtDateTime,csStmtDateTime,PD_DEFAULT_DATETIME_FORMAT));
 ...
 if (iSampleRow == 1) DEBUGLOG(("CheckDetail: ** No Change Time Zone for %s\n",csCountry));
			}
		}
	}
...
/* txn_amount */
	if (GetField_CString(hRls,"cr_txn_amount",&csTmp)) {
		...
		if (atof(csTmp) > 1E-9) {
			...
			if (!strcmp(csCountry,PD_TAIWAN)) {
if (iDetailRow == 1) DEBUGLOG(("CheckDetail: [%s %s] move to other side\n",PD_CR,csTmp));
