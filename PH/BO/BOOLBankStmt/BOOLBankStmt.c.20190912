...
Replace system() by execvp()                       2016/06/28              David Wong
Reduce Debug Log				   2016/12/09		   Elvis Wong
Add 
   file_id,
   bank_acct_type 
in ConvertStmtFile			   	   2018/05/10		   Elvis Wong
PSBC stmt format change				   2018/06/21		   Ethan Yip
Update
   Check Duplicated Records                        2019/02/13              Elvis Wong
Add excel worksheet name checking                  2019/05/09              David Wong
*/
...
DEBUGLOG(("ConvertStmtFile file_id = [%s]\n", csFileId));
        }

/* bank_acct_type */
	if (!GetField_CString(hContext,"bank_acct_type",&csBankAcctType)) {
DEBUGLOG(("ConvertStmtFile bank_acct_type = [%s]\n", csBankAcctType));
	}
...
/*
			command[0] = (char*)EXCEL_CONVERT_SCRIPT;
			command[1] = csNewFullName;
			command[2] = ">";
			command[3] = csConvertedFullName;
			command[4] = NULL;
*/
			command[0] = "/bin/ksh";
			command[1] = "-c";
...
/*
			command[0] = (char*)TEXT_CONVERT_SCRIPT;
			command[1] = "-f";
			command[2] = csInFileEncoding;
			command[3] = "-t";
			command[4] = (char*)OUT_FILE_ENCODING;
			command[5] = csNewFullName;
			command[6] = ">";
			command[7] = csConvertedFullName;
			command[8] = NULL;
*/
			command[0] = "/bin/ksh";
			command[1] = "-c";
			...
			// Bank: ABC
			if (!strcmp(csIntBankCode, PD_ABC_BANK_CODE)) {
				snprintf(csSysCmd, sizeof(csSysCmd), "%s \"%s\" \"%s\" \"%s\" \"%s\" \"%s\" \"%s\"", csScriptName, csInFileEncoding, OUT_FILE_ENCODING, csNewFullName, csConvertedFullName, csFileId, csBankAcctType);

                                command[0] = csScriptName;
                                command[1] = csInFileEncoding;
                                command[2] = (char*)OUT_FILE_ENCODING;
                                command[3] = csNewFullName;
                                command[4] = csConvertedFullName;
                                command[5] = csFileId;
                                command[6] = csBankAcctType;
                                command[7] = NULL;
			}
			// Other Banks 
			else {
...
*/
DEBUGLOG(("call execute()\n"));
		execute(command);
	}

DEBUGLOG(("ConvertStmtFile iRet = [%d]\n", iRet));
...
DEBUGLOG(("ProcessStmtFile() line %03d Self DeDup file[%s] prev_file[%s] FAILURE FOUND\n",iCurrLine,csFileCompareKey,csPrevFileCompareKey));
							...
							PutField_CString(hRec, "dedup_type", "self");
							break; //
						}
...
DEBUGLOG(("ProcessStmtFile() line %03d DBOLStatementTmp::CheckTmpRecord() file[%s] FAILURE FOUND\n",iCurrLine,csFileCompareKey));
							...
							iSkipCount++;
							PutField_CString(hRec, "as_ind", "skip"); //
							PutField_CString(hRec, "dedup_type", "tmp"); //
...
DEBUGLOG(("ProcessStmtFile() line %03d DBOLStatement::CheckBankStmtRecord() file[%s] FOUND\n",iCurrLine,csFileCompareKey));
								iSkipCount++;
								PutField_CString(hRec, "as_ind", "skip");
								PutField_CString(hRec, "dedup_type", "db");
							} else if (iTmpRet == NOT_FOUND) {
 ...
 */
        if (iRet == PD_OK) {
			...
			iError = 0;
                        ...
                        hFile = RecordSet_GetFirst(myFile);
                        while (hFile) {
                                if (GetField_CString(hFile, "dedup_type", &csTmp)) {
                                        if (!strcmp(csTmp, "self")) {
						iError++;
DEBUGLOG(("ProcessStmtDetail() Check Duplicated Records, rec_cnt = [%d], dedup_type = [%s][edit], Skipped Record!!!\n", iRecCount, csTmp));
					} else if ((!strcmp(csTmp, "tmp")) || (!strcmp(csTmp, "db"))) {
                                                if (iNewRecExisted == PD_TRUE) {
							iError++;
DEBUGLOG(("ProcessStmtDetail() Check Duplicated Records, rec_cnt = [%d], dedup_type = [%s][edit], Skipped Record!!!\n", iRecCount, csTmp));
                                                } else {
DEBUGLOG(("ProcessStmtDetail() Check Duplicated Records, rec_cnt = [%d], dedup_type = [%s], Skipped Record!!!\n", iRecCount, csTmp));
						}
                                        } else {
                                                iRet = INT_ERR;
						PutField_Int(hContext, "internal_error", iRet);
DEBUGLOG(("ProcessStmtDetail() Check Duplicated Records, Get dedup Error!!!\n"));
ERRLOG("BOOLBankStmt::ProcessStmtDetail() Check Duplicated Records, Get dedup Error!!!\n");
                                                break;
                                        }
                                } else {
DEBUGLOG(("ProcessStmtDetail() Check Duplicated Records, rec_cnt = [%d], NOT Skipped Record!!!\n", iRecCount));
			...
			if (iError > 0) {
				/* result_cnt and result_code */
                            	PutField_Int(hContext, "result_cnt", iError);
                              	PutField_Int(hContext, "result_code", INT_BANK_STMT_RECORD_DUPLICATE);
DEBUGLOG(("ProcessStmtDetail() Check Duplicated Records, result_cnt = [%d] result_msg = [%d]\n", iError, INT_BANK_STMT_RECORD_DUPLICATE));
			}
                }
        }
...
DEBUGLOG(("ProcessStmtDetail() line %03d BOOLBankStmtMatch::MatchStmtWithSms() [%s] = [%s] found\n",iCurrLine,csTag,csTxnCode));							

								break; //
							} else if (iDtlRet == PD_NOT_FOUND) {										
// DEBUGLOG(("ProcessStmtDetail() line %03d BOOLBankStmtMatch::MatchStmtWithSms() not found\n",iCurrLine));			
DEBUGLOG(("ProcessStmtDetail() line %03d BOOLBankStmtMatch::MatchStmtWithSms() [%s] = [%s] not found\n",iCurrLine,csTag,csTxnCode));							
...
int SplitLineByhFormat(const char *csLine, const char *csFormatId, const hash_t *hFormat, hash_t *hRls)
{
	...
	char csNewField[PD_TMP_MSG_BUF_LEN], csNewLine[PD_TMP_MSG_BUF_LEN], csNewField2[PD_TMP_MSG_BUF_LEN];
...
DEBUGLOG((" desc %s (%s)[%s]\n", csTag, csDesc, csNewField));
	...
	/* desc_2 - CONTENT */
		...
		if (GetField_CString(hFormat, csTag, &csDesc)) {
			if (GetField_CString(hRls, csDesc, &csTmp)) {
					if (*csTmp != '\0') {
                                        	sprintf(csConcatField,"%s %s",csTmp,csNewField2);
                                        	strcpy(csNewField2,csConcatField);
                                	}
			}
