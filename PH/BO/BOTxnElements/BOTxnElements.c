...
Add Psp Txn Elements				   2012/10/30		   Cody Chan
Add void support 'RSP','PSP-Intransit' and 'Bank'  2015/11/30		   LokMan Chow
Add void support PSP Settlement			   2015/12/11		   Dirk Wong
Add void support Merchant Settlement		   2016/05/11		   Dirk Wong
...
DEBUGLOG(("VoidOrgTxnElements:: sub_txn_code = [%s]\n",csSubTxnCode));
	}
...
DEBUGLOG(("VoidOrgTxnElements:: call DBTxnElements:GetAllFeeChgDetail\n"));
	...
	if ((unsigned long)(*DBObjPtr)(csOrgTxnSeq,rRecordSet) == PD_OK) {
		...
		while(hRec && (iRet == PD_OK)){
			...
			if((cType == PD_TYPE_PSP) &&
                           (!strcmp(csSubTxnCode,PD_DEPOSIT_REFUND_M))) {
DEBUGLOG(("VoidOrgTxnElements:: skip PSP elements for RDM\n"));
			...
			else if(cType == PD_MI_PARTY_RSP ||
				cType == PD_MI_PARTY_PSP_INTR ||
				cType == PD_MI_PARTY_OPBANK){
