...
Add checking for bank balance transfer in	   2015/12/18		   LokMan Chow
remove the ACR Bank negative balance checking	   2016/01/14		   LokMan Chow
*/
...
#include "internal.h"
#include "common.h"
#include "BOMiAdjustment.h"

char    cDebug;
OBJPTR(DB);
OBJPTR(BO);

void BOMiAdjustment(char    cdebug)
{
        cDebug = cdebug;
}
...
DEBUGLOG(("GetCurrentCode::Call MiAdjustmentType: GetMaxCode\n"));

	DBObjPtr = CreateObj(DBPtr,"DBMiAdjustmentType","GetMaxCode");
...
ERRLOG("BOMiAdjustment::GetCurrentCode::GetMaxCode not found!!\n");
		iRet=INT_ERR;
        }
...
DEBUGLOG(("ValidateProcess:: csEntityType [%s] csCode [%s] iCodeInNum[%d] csAmtType[%s] cBalType[%c] cNature[%c] iIsProrata[d]\n", 
		...
		if (cAction != PD_ACTION_ADD) { 
			...
			DBObjPtr = CreateObj(DBPtr,"DBMiAdjustmentType","GetMiAdjustmentTypeRec");
			if ((*DBObjPtr)(csEntityType, csCode, rRecordSet) == PD_OK) {

				hRec = RecordSet_GetFirst(rRecordSet);
				while (hRec) {
					if (GetField_Int(hRec, "disabled", &iCurrentDisabled)) {
DEBUGLOG(("ValidateProcess::GetMiAdjustmentTypeRec::disabled = [%d]\n",iCurrentDisabled));
					}

					if (GetField_CString(hRec, "amt_type", &csCurrentAmtType)) {
DEBUGLOG(("ValidateProcess::GetMiAdjustmentTypeRec::amt_type = [%s]\n",csCurrentAmtType));
					}

					if (GetField_Char(hRec, "bal_type", &cCurrentBalType)) {
DEBUGLOG(("ValidateProcess::GetMiAdjustmentTypeRec::bal_type = [%c]\n",cCurrentBalType));
					}

					if (GetField_Char(hRec, "nature", &cCurrentNature)) {
DEBUGLOG(("ValidateProcess::GetMiAdjustmentTypeRec::nature = [%c]\n",cCurrentNature));
					}

					if (GetField_Int(hRec, "is_prorata", &iCurrentIsProrata)) {
DEBUGLOG(("ValidateProcess::GetMiAdjustmentTypeRec::is_prorata = [%d]\n",iCurrentIsProrata));
					}
				...
				iRet = PD_ERR;
DEBUGLOG(("ValidateProces::GetMiAdjustmentTypeRec:: failed\n"));
ERRLOG("BOMiAdjustment::ValidateProcess::GetMiAdjustmentTypeRec:: failed!!\n");
			}
...
DEBUGLOG(("ValidateProcess::GetMiAdjustmentTypeRec::Assigne disabled = [%d]\n",iCurrentDisabled));
					}
...
DEBUGLOG(("ValidateProcess::GetMiAdjustmentTypeRec::amt_type = [%s] not match with new amt_type [%s], not allow update\n",csCurrentAmtType, csNewAmtType));
							iRet = PD_ERR;
						}

						if (cNewBalType != cCurrentBalType) {
DEBUGLOG(("ValidateProcess::GetMiAdjustmentTypeRec::bal_type = [%c] not match with new bal_type [%c], not allow update\n",cCurrentBalType, cNewBalType));
							iRet = PD_ERR;
						}

						if (cNewNature != cCurrentNature) {
DEBUGLOG(("ValidateProcess::GetMiAdjustmentTypeRec::Nature = [%c] not match with new nature [%c], not allow update\n",cCurrentNature, cNewNature));
							iRet = PD_ERR;
						}

						if (iNewIsProrata != iCurrentIsProrata) {
DEBUGLOG(("ValidateProcess::GetMiAdjustmentTypeRec::is_prorata = [%d] not match with new is_prorata [%d], not allow update\n",iCurrentIsProrata, iNewIsProrata));
							iRet = PD_ERR;
						}
                        		}
                		}
...
ERRLOG("BOMiAdjustment::ValidateProcess::Call Transaction: ChkTxnCode Exist :: ERR!!\n");
					iRet = PD_ERR;
				}	
			}
...
DEBUGLOG(("ProcessPartyBalance::Call DBMiAdjustment:GetMiAdjustmentTypeRec\n"));
...
DEBUGLOG(("ProcessPartyBalance::GetMiAdjustmentTypeRec::amt_type = [%s]\n",csAmtType));
					PutField_CString(hInRec, "amt_type", csAmtType);
				}

				if (GetField_Char(hRec, "bal_type", &cBalType)) {
DEBUGLOG(("ProcessPartyBalance::GetMiAdjustmentTypeRec::bal_type = [%c]\n", cBalType));
					PutField_Char(hInRec, "bal_type", cBalType);
				}

				if (GetField_Char(hRec, "nature", &cNature)) {
DEBUGLOG(("ProcessPartyBalance::GetMiAdjustmentTypeRec::nature = [%c]\n", cNature));
					PutField_Char(hInRec, "nature", cNature);
				}

				if (GetField_Int(hRec, "is_prorata", &iIsProrata)) {
DEBUGLOG(("ProcessPartyBalance::GetMiAdjustmentTypeRec::is_prorata= [%d]\n", iIsProrata));
...
DEBUGLOG(("ProcessPartyBalance::GetMiAdjustmentTypeRec::disabled = [%d]\n",iDisabled));

					if (iDisabled == PD_DISABLED) {
DEBUGLOG(("ProcessPartyBalance::GetMiAdjustmentTypeRec::not allow to use the code\n"));
ERRLOG("BOOLMiAdjustment::ProcessPartyBalance::GetMiAdjustmentTypeRec:: code in disabled!!\n");
			...
			iRet = PD_ERR;
DEBUGLOG(("ProcessPartyBalance::GetMiAdjustmentTypeRec:: failed\n"));
ERRLOG("BOMiAdjustment::ProcessPartyBalance::GetMiAdjustmentTypeRec:: failed!!\n");
			PutField_Int(hInRec, "internal_error", iRet);
		}
	}
	else {
...
/*
	//Not Allow DR OPBank to negative balance
	if (iRet == PD_OK) {
		if (!strcmp(csEntityType,PD_MI_ENTITY_OPBANK)) {
			DBObjPtr = CreateObj(DBPtr,"DBMiEntityBalance","GetEntityBalance");
			iRet = (unsigned long)((DBObjPtr)(hInRec,hInRec));
			if (iRet == PD_OK) {
				if (!strcmp(csAmtType,PD_DR)) {
					if (GetField_Double(hInRec,"acct_bal",&dTmp)) {
						if (dTmp < dAmt) {
DEBUGLOG(("ProcessPartyBalance:: acct_bal [%f] is not enough for DR txn_amt [%f]\n",dTmp,dAmt));
ERRLOG("BOMiAdjustment::ProcessPartyBalance:: acct_bal [%f] is not enough for DR txn_amt [%f]\n",dTmp,dAmt);
					...
					if (GetField_Double(hInRec,"acct_bal",&dTmp)) {
						double dCost = 0.0;
						GetField_Double(hInRec,"cost_amt",&dCost);
						if(dTmp+dAmt<dCost){
							iRet = INT_INSUFFICIENT_FUND;
							PutField_Int(hInRec, "internal_error", iRet);
DEBUGLOG(("ProcessPartyBalance:: acct_bal [%f] is not enough for CR txn_amt [%f] then DR cost [%f]\n",dTmp,dAmt,dCost));
ERRLOG("BOMiAdjustment::ProcessPartyBalance:: acct_bal [%f] is not enough for CR txn_amt [%f] then DR cost [%f]\n",dTmp,dAmt,dCost);
						}
					}
				}
			}
...
*/
	...
	if (iRet == PD_OK) {
		...
		PutField_Char(hInRec, "batch_mode", PD_ONLINE);
