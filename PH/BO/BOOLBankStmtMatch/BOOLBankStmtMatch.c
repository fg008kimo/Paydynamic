...
Remove PD_TXN_CODE_RETURNED_DEPOSIT to_hold part   2015/10/30              David Wong
*/
...
DEBUGLOG(("MatchStmtWithSms() start\n"));

/* in_file_name */
	if (GetField_CString(hContext, "in_file_name", &csInFileName)) {
// DEBUGLOG(("in_file_name = [%s]\n", csInFileName));
	} else {
DEBUGLOG(("in_file_name NOT FOUND!!!\n"));
		iRet = PD_ERR;
	}
...
// DEBUGLOG(("sms_match_threshold = [%d]\n", iSmsMatchThreshold));
	} else {
DEBUGLOG(("sms_match_threshold NOT FOUND!!!\n"));
		iRet = PD_ERR;
	}
...
DEBUGLOG(("dOpenBal = [%.2lf]\n", dOpenBal));
	} else {
DEBUGLOG(("dOpenBal NOT FOUND!!!\n"));
		iRet = PD_ERR;
	}
*/
...
DEBUGLOG(("Is Txn Code First Match = [%d]\n", iTxnCodeFirstMatch));
		GetField_Int(hFile, "line", &iCurrLine);
DEBUGLOG(("Match Start at line %d\n",iCurrLine));
...
ERRLOG("BOOLBankStmtMatch::MatchStmtWithSms() call BOOLStmtEngine::ProcessMatching() stat_txn_id NOT FOUND!!!\n");
				...
				if (iRet == PD_OK) {
					...
					if (!GetField_CString(hFile, "txn_id", &csBAIDTxnId)) {
						iRet = INT_ERR;
DEBUGLOG(("call BOOLStmtEngine::ProcessMatching() txn_id NOT FOUND!!!\n"));
ERRLOG("BOOLBankStmtMatch::MatchStmtWithSms() call BOOLStmtEngine::ProcessMatching() txn_id NOT FOUND!!!\n");
				...
				iRet = INT_ERR;
DEBUGLOG(("call BOOLStmtEngine::ProcessMatching() FAILURE!!!\n"));
...
DEBUGLOG(("call DBOLStatement::MatchStmt() within [%d] mins\n",iSmsMatchThreshold));
				DBObjPtr = CreateObj(DBPtr, "DBOLStatement", "MatchStmt");
				iDtlRet = (unsigned long)(*DBObjPtr)(hFile);
				if (iDtlRet == PD_FOUND) {
					if (!GetField_CString(hFile,"stat_txn_id",&csStatTxnId)) {
...
ERRLOG("BOOLBankStmtMatch::MatchStmtWithSms() call DBOLStatement::MatchStmt() txn_id NOT FOUND!!!\n");
					...
					iRet = INT_ERR;
DEBUGLOG(("call DBOLStatement::MatchStmt() FAILURE!!!\n"));
...
*/
		...
		/* MatchRespTxn */
			if (iRet == PD_OK && iDtlRet == PD_FOUND) {
				/* Voided SMS */
DEBUGLOG(("call DBOLBAIDTxn::MatchRespTxn()\n"));
...
DEBUGLOG(("call DBOLBAIDTxn::MatchRespTxn() Not Found\n"));
...
DEBUGLOG(("call DBOLTransaction::GetTxnHeader()\n"));
						...
						if ((unsigned long)(*DBObjPtr)(csDstTxnId,rTxn) != PD_OK) {
							iRet = INT_ERR;
DEBUGLOG(("call DBOLTransaction::GetTxnHeader() FAILURE!!!\n"));
...
DEBUGLOG(("ERRNO = [%d] (%s)...!!!\n",errsv,strerror(errsv)));
										...
										if (WIFEXITED(iDtlRet)) {
											...
											if (status == SUCCESS) {
DEBUGLOG(("Exited, status %d, No email sent\n",status));
											} else if (status == FAILURE) {
DEBUGLOG(("Exited, status %d, Email sent\n",status));
											} else {
DEBUGLOG(("Exited, status %d, Email FAILURE!!!\n",status));
											}
...
DEBUGLOG(("call DBOLBAIDTxn::GetBaidTxn() OrgTxnCode [%s][%s] Stmt TxnCode [%s] HoldInd [%d]\n",csOrgTxnCode,csOrgSubStatus,csTxnCode,iTxnHoldInd));
								} else {
									iTxnHoldInd = 0;
DEBUGLOG(("call DBOLBAIDTxn::GetBaidTxn() OrgTxnCode [%s] Stmt TxnCode [%s]\n",csOrgTxnCode,csTxnCode));
								}
							}
						}
				...
				if (iRet == PD_OK) {
					//if (csDstTxnId != NULL) {
					if (iFoundDstTxnId) {
DEBUGLOG(("call DBOLTransaction::GetTxnIdForUpdate()\n"));
						DBObjPtr = CreateObj(DBPtr, "DBOLTransaction", "GetTxnIdForUpdate");
						if ((unsigned long)(*DBObjPtr)(csDstTxnId) != PD_OK) {
							iRet = INT_ERR;
DEBUGLOG(("call DBOLTransaction::GetTxnIdForUpdate() FAILURE!!!\n"));
...
DEBUGLOG(("call DBOLStatement::UpdateSmsDetail()\n"));
					DBObjPtr = CreateObj(DBPtr, "DBOLStatement", "UpdateSmsDetail");
					if ((unsigned long)(*DBObjPtr)(hFile) != PD_OK) {
						iRet = INT_ERR;
DEBUGLOG(("call DBOLStatement::UpdateSmsDetail() FAILURE!!!\n"));
...
DEBUGLOG(("call DBOLTransaction::Update()\n"));
						...
						if ((unsigned long)(*DBObjPtr)(hRec) != PD_OK) {
							iRet = INT_ERR;
DEBUGLOG(("call DBOLTransaction::Update() FAILURE!!!\n"));
ERRLOG("BOOLBankStmtMatch::MatchStmtWithSms() call DBOLTransaction::Update() FAILURE!!!\n");
				...
				if (iRet == PD_OK) {
					...
					//	}
						...
						PutField_CString(hRec,"update_user",csUser);
DEBUGLOG(("call DBOLTxnPspDetail::Update()\n"));
						DBObjPtr = CreateObj(DBPtr, "DBOLTxnPspDetail", "Update");
						if ((unsigned long)(*DBObjPtr)(hRec) != PD_OK) {
							iRet = INT_ERR;
DEBUGLOG(("call DBOLTxnPspDetail::Update() FAILURE!!!\n"));
...
DEBUGLOG(("call DBOLTxnDepositHoldLog::Add()\n"));
							DBObjPtr = CreateObj(DBPtr, "DBOLTxnDepositHoldLog", "Add");
							if ((unsigned long)(*DBObjPtr)(hRec) != PD_OK) {
								iRet = INT_ERR;
DEBUGLOG(("call DBOLTxnDepositHoldLog::Add() FAILURE!!!\n"));
...
DEBUGLOG(("call DBOLBankAcctId::GetBankAcctIdCountry()\n"));
					...
					if (iTmpRet != FOUND) {
DEBUGLOG(("call DBOLBankAcctId::GetBankAcctIdCountry() failed\n"));
						iRet = INT_ERR;
					} else {
DEBUGLOG(("country = [%s]\n", csTxnCountry));
					}
...
DEBUGLOG(("call DBOLStatement::AddDetail()\n"));
						DBObjPtr = CreateObj(DBPtr, "DBOLStatement", "AddDetail");
						if ((unsigned long)(*DBObjPtr)(hFile) != PD_OK) {
							iRet = INT_ERR;
DEBUGLOG(("call DBOLStatement::AddDetail() FAILURE!!!\n"));
...
DEBUGLOG(("open_bal=[%.2lf], txn_amt=[DR%.2lf], curr_bal=[%.2lf]\n",dOpenBal,dTxnAmt,dCurrBal));
						} else if (!strcmp(csAmtType, PD_CR)) {
							dCurrBal = dOpenBal + dTxnAmt;
DEBUGLOG(("open_bal=[%.2lf], txn_amt=[CR%.2lf], curr_bal=[%.2lf]\n",dOpenBal,dTxnAmt,dCurrBal));
						}
						...
						/* hold recon */
						if (GetField_Int(hFile, "hold_recon", &iTmp)) {
							PutField_Int(hRec, "hold_recon", iTmp);
						} else {
							PutField_Int(hRec, "hold_recon", 0);
						}
...
DEBUGLOG(("call DBOLBAIDTxn::Add()\n"));
						DBObjPtr = CreateObj(DBPtr, "DBOLBAIDTxn", "Add");
						if ((unsigned long)(*DBObjPtr)(hRec) != PD_OK) {
							iRet = INT_ERR;
DEBUGLOG(("call DBOLBAIDTxn::Add() FAILURE!!!\n"));
...
DEBUGLOG(("call DBOLStatement::UpdateTxnId()\n"));
...
DEBUGLOG(("update est_net_amount\n"));
DEBUGLOG(("dEstNetAmount = [%lf]\n", dEstNetAmount));
						if (dEstNetAmount != 0.0) {
							hash_init(hRec, 0);
							PutField_CString(hRec, "txn_seq", csBAIDTxnId);
...
* start stmt engine
******************************/
DEBUGLOG(("call stmt engine\n"));
				if (iRet == PD_OK) {
					GetField_CString(hFile, "format_txn_code", &csTxnCode);
DEBUGLOG(("format_txn_code = [%s]\n", csTxnCode));
					if (!strcmp(csTxnCode, PD_TXN_CODE_UNKNOWN_CREDIT) || !strcmp(csTxnCode, PD_TXN_CODE_UNKNOWN_DEBIT)) {
DEBUGLOG(("unknown credit/debit, skip calling engines\n"));
					} else {
...
DEBUGLOG(("failed_baid_txn_code = [%s]\n", csFailedBaidTxnCode));
								}
							}
						} else {
							iIsClassified = PD_TRUE;
...
DEBUGLOG(("baid_txn_id_1 = [%s]\n", csTmp));
							...
							if (!strcmp(csNextEngineTxnCode, PD_CHARGEBACK_REVERSAL)) {
DEBUGLOG(("DCR relate with VOD case\n"));
							} else {
...
DEBUGLOG(("call BOOLTxnEngine::DoAction()\n"));
							BOObjPtr = CreateObj(BOPtr, "BOOLTxnEngine", "DoAction");
							iTmpRet = (unsigned long)(*BOObjPtr)(hRec);
							if (iTmpRet != PD_OK) {
								iRet = INT_ERR;
DEBUGLOG(("call BOOLTxnEngine::DoAction() FAILURE!!!\n"));
ERRLOG("BOOLBankStmtMatch::MatchStmtWithSms() call BOOLTxnEngine::DoAction() FAILURE!!!\n");
							} else {
								// special handling for deposit chargeback reversal
								if (!strcmp(csNextEngineTxnCode, PD_CHARGEBACK_REVERSAL)) {
									char *csVodTxnId = NULL;
									char csVodBaidTxnId[PD_TXN_SEQ_LEN + 1];
									char *csVodStatTxnId = NULL;
									char *csVodStmtRef = NULL;
									char *csDcrBaidTxnId = NULL;
									char csDcrTxnId[PD_TXN_SEQ_LEN + 1];
									char *csDcrStatTxnId = NULL;
									char *csDcrStmtRef = NULL;
									char *csObdTxnId = NULL;
									char *csObdBaidTxnId[PD_TXN_SEQ_LEN + 1];
									char *csObdStatTxnId = NULL;
									char *csObdStmtRef = NULL;
									int iSendAlert = 0;
									int iDynCnt = 0;
									
									if (GetField_CString(hStmtEngine, "psp_txn_id_1", &csVodTxnId)) {
DEBUGLOG(("VOD txn id = [%s]\n", csVodTxnId));
										// get DCR txn_id
										GetField_CString(hStmtEngine, "baid_txn_id_1", &csDcrBaidTxnId);
DEBUGLOG(("DCR baid txn id = [%s]\n", csDcrBaidTxnId));
										DBObjPtr = CreateObj(DBPtr, "DBOLStmtTxnRelation", "GetTxnIdByStmtTxnId");
										iTmpRet = (unsigned long)(*DBObjPtr)(csDcrBaidTxnId, csDcrTxnId);
DEBUGLOG(("DCR txn id = [%s]\n", csDcrTxnId));

										// add DCR VOD relation
										hash_t *hTmp;
										...
										PutField_CString(hTmp, "p1_txn_id", csDcrTxnId);
										PutField_CString(hTmp, "p2_txn_id", csVodTxnId);
										PutField_Char(hTmp, "txn_level", PD_TXN_LEVEL);
										PutField_Char(hTmp, "relation_type", PD_REL_LINKAGE);
										PutField_CString(hTmp, "create_user", PD_UPDATE_USER);
DEBUGLOG(("call DBOLTxnRelationSpc::Add()\n"));
										DBObjPtr = CreateObj(DBPtr, "DBOLTxnRelationSpc", "Add");
										iTmpRet = (unsigned long)(*DBObjPtr)(hTmp);

										// send alert
										if (GetField_Int(hStmtEngine, "send_alert", &iSendAlert)) {
DEBUGLOG(("send alert\n"));
											// get DCR stat_txn_id
											hash_destroy(hTmp);
											hash_init(hTmp, 0);
											...
											DBObjPtr = CreateObj(DBPtr, "DBOLBAIDTxn", "GetBaidTxn");
											iTmpRet = (unsigned long)(*DBObjPtr)(csDcrBaidTxnId, hTmp);
											GetField_CString(hTmp, "stat_txn_id", &csDcrStatTxnId);
											...
											iTmpRet = (unsigned long)(*DBObjPtr)(csDcrStatTxnId, hTmp);
											GetField_CString(hTmp, "stmt_ref", &csDcrStmtRef);

											// get VOD baid_txn_id
											DBObjPtr = CreateObj(DBPtr, "DBOLStmtTxnRelation", "GetStmtTxnIdByTxnId");
											iTmpRet = (unsigned long)(*DBObjPtr)(csVodTxnId, csVodBaidTxnId);
											...
											DBObjPtr = CreateObj(DBPtr, "DBOLBAIDTxn", "GetBaidTxn");
											iTmpRet = (unsigned long)(*DBObjPtr)(csVodBaidTxnId, hTmp);
											GetField_CString(hTmp, "stat_txn_id", &csVodStatTxnId);
											...
											DBObjPtr = CreateObj(DBPtr, "DBOLStatement", "GetStmtDtl");
											iTmpRet = (unsigned long)(*DBObjPtr)(csVodStatTxnId, hTmp);
											GetField_CString(hTmp, "stmt_ref", &csVodStmtRef);
											...
											PutField_CString(hTmp, "p2_txn_id", csVodTxnId);
											PutField_Char(hTmp, "txn_level", 'T');
											PutField_Char(hTmp, "relation_type", 'L');

											DBObjPtr = CreateObj(DBPtr, "DBOLTxnRelation", "GetP1TxnId");
											iTmpRet = (unsigned long)(*DBObjPtr)(hTmp, rTmp);
											hTmp2 = RecordSet_GetFirst(rTmp);
											GetField_CString(hTmp2, "p1_txn_id", &csObdTxnId);

											// get OBD baid_txn_id
											DBObjPtr = CreateObj(DBPtr, "DBOLStmtTxnRelation", "GetStmtTxnIdByTxnId");
											iTmpRet = (unsigned long)(*DBObjPtr)(csObdTxnId, csObdBaidTxnId);
											...
											DBObjPtr = CreateObj(DBPtr, "DBOLBAIDTxn", "GetBaidTxn");
											iTmpRet = (unsigned long)(*DBObjPtr)(csObdBaidTxnId, hTmp);
											GetField_CString(hTmp, "stat_txn_id", &csObdStatTxnId);
											...
											DBObjPtr = CreateObj(DBPtr, "DBOLStatement", "GetStmtDtl");
											iTmpRet = (unsigned long)(*DBObjPtr)(csObdStatTxnId, hTmp);
											GetField_CString(hTmp, "stmt_ref", &csObdStmtRef);
											...
											PutField_CString(hTmp, "funct", "DSI_CHGBK_REV");
											PutField_Char(hTmp, "party_type", 'G');
											PutField_CString(hTmp, "party_id", "000");
											iDynCnt = set_tpl_dyn_cstring(hTmp, iDynCnt, "VOD_STMT_REF", "GLO", "STR", csVodStmtRef);
											iDynCnt = set_tpl_dyn_cstring(hTmp, iDynCnt, "DCR_STMT_REF", "GLO", "STR", csDcrStmtRef);
											iDynCnt = set_tpl_dyn_cstring(hTmp, iDynCnt, "OBD_STMT_REF", "GLO", "STR", csObdStmtRef);
											PutField_Int(hTmp, "total_dyn", iDynCnt); 

DEBUGLOG(("call BOAlertEmail::ProcessTpl()\n"));
...
DEBUGLOG(("call DBOLTxnPspDetail::GetDepositTxnToHold()\n"));
				DBObjPtr = CreateObj(DBPtr, "DBOLTxnPspDetail", "GetDepositTxnToHold");
				if ((unsigned long)(*DBObjPtr)(hRec) != PD_OK) {
					iRet = INT_ERR;
DEBUGLOG(("call DBOLTxnPspDetail::GetDepositTxnToHold() FAILURE!!!\n"));
ERRLOG("BOOLBankStmtMatch::MatchStmtWithSms() call DBOLTxnPspDetail::GetDepositTxnToHold() FAILURE!!!\n");
					...
					if (GetField_CString(hRec,"txn_id",&csTmp)) {
						iDtlRet = PD_FOUND;
DEBUGLOG(("call DBOLTxnPspDetail::GetDepositTxnToHold() txn_id = [%s]\n",csTmp));
						PutField_Int(hFile,"hold",1);
					} else {
						iDtlRet = PD_NOT_FOUND;
DEBUGLOG(("call DBOLTxnPspDetail::GetDepositTxnToHold() txn_id NOT FOUND\n",csTmp));
					}
				}
