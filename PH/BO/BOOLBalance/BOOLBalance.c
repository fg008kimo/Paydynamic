...
Modify UpdatePayoutAmount			   2014/09/23		   LokMan Chow
Modify ProcessMerchantAdjAmount			   2016/01/06		   LokMan Chow
Update ProcessFundinPayout                         2016/07/04              Elvis Wong
and ProcessTransferAvaPayout
Add ProcessAvaPayout				   2016/09/14	 	   Elvis Wong
Modify ProcessMerchantAdjAmount			   2016/09/26		   Dirk Wong
Add IsUpdatePayoutTempBAIDBal			   2019/05/30		   Elvis Wong
*/
...
DEBUGLOG(("BOOLBalance:UpdateTxnAmount() org_psp_id is missing!!!\n"));
		...
		/* org baid */
		if (GetField_CString(hContext,"org_baid",&csOrgBaid)) {
DEBUGLOG(("BOOLBalance:UpdateTxnAmount() org_baid = [%s]\n",csOrgBaid));
		}
		else {
DEBUGLOG(("BOOLBalance:UpdateTxnAmount() org_baid is missing!!!\n"));
		}
...
DEBUGLOG(("BOOLBalance:UpdateTxnAmount() DBOLBAIDBal::GetBalance() called\n"));
			DBObjPtr = CreateObj(DBPtr, "DBOLBAIDBal", "GetBalance");
			if ((unsigned long)(*DBObjPtr)(csOrgBaid, csOrgTxnCountry, csOrgDstCcy, hContext) != PD_ERR){
				if (GetField_Double(hContext, "balance", &dTmp)) {
DEBUGLOG(("BOOLBalance:UpdateTxnAmount() balance = [%f]\n", dTmp));
				}
				if (GetField_Double(hContext, "total_hold", &dTmp)) {
DEBUGLOG(("BOOLBalance:UpdateTxnAmount() total_hold = [%f]\n", dTmp));
				}
				if (GetField_Double(hContext, "prepaid", &dTmp)) {
DEBUGLOG(("BOOLBalance:UpdateTxnAmount() prepaid = [%f]\n", dTmp));
				}
				if (GetField_Double(hContext, "in_transit", &dTmp)) {
DEBUGLOG(("BOOLBalance:UpdateTxnAmount() in_transit = [%f]\n", dTmp));
				}
			} else {
				iRet = INT_ERR;
DEBUGLOG(("BOOLBalance:UpdateTxnAmount() DBOLBAIDBal::GetBalance() Failed\n"));
ERRLOG("BOOLBalance:UpdateTxnAmount() DBOLBAIDBal::GetBalance() Failed\n");
			}
		}
	}
...
/* PSP */
	...
	/* lock opening balance */
		if (iRet == PD_OK) {
DEBUGLOG(("BOOLBalance:CreditDepositAmount() DBOLBAIDBal::GetBalanceForUpdate() called\n"));
			...
			if ((*DBObjPtr)(csBaid,
					...
					hContext) != PD_OK) {
				iRet = INT_ERR;
DEBUGLOG(("BOOLBalance:CreditDepositAmount() DBOLBAIDBal::GetBalanceForUpdate() Failed\n"));
ERRLOG("BOOLBalance:CreditDepositAmount() DBOLBAIDBal::GetBalanceForUpdate() Failed\n");
			}
		}
	}
...
/* PSP */
		...
		if (iRet == PD_OK) {
DEBUGLOG(("BOOLBalance:CreditDepositAmount() DBOLBAIDBal::UpdateBalance() called\n"));
			...
			if ((*DBObjPtr)(csBaid,
					...
					PD_UPDATE_USER) != PD_OK) {
				iRet = INT_ERR;
DEBUGLOG(("BOOLBalance:CreditDepositAmount() DBOLBAIDBal::UpdateBalance() Failed\n"));
ERRLOG("BOOLBalance:CreditDepositAmount() DBOLBAIDBal::UpdateBalance() Failed\n");
			}
		}
	}
...
/* PSP */
		...
		/*
		if (iRet == PD_OK) {
DEBUGLOG(("BOOLBalance:DebitDepositAmount() DBOLBAIDBal::GetAvalBaidBal() called\n"));
			DBObjPtr = CreateObj(DBPtr, "DBOLBAIDBal", "GetAvalBaidBal");
			if ((unsigned long)(*DBObjPtr)(csBaid, csPspCCY, csCountry, &dAvaBalance) != PD_OK) {
DEBUGLOG(("BOOLBalance:DebitDepositAmount() DBOLBAIDBal::GetAvalBaidBal() Failed\n"));
ERRLOG("BOOLBalance::DebitDepositAmount() DBOLBAIDBAL::GetAvalBaidBal() Failed\n");
				iRet=INT_ERR;
			}
			else {
DEBUGLOG(("BOOLBalance:DebitDepositAmount() BAID Balance [%lf]\n", dAvaBalance));
				if (dAvaBalance < (dPspAmount - dPspFee)) {
DEBUGLOG(("BOOLBalance:DebitDepositAmount() BAID Balance [%lf] < debit balance [%lf]\n", dAvaBalance, (dPspAmount - dPspFee)));
ERRLOG("BOOLBalance::DebitDepositAmount() BAID Balance [%lf] < debit balance [%lf]\n", dAvaBalance, (dPspAmount - dPspFee));
					iRet = INT_INSUFFICIENT_FUND;
					PutField_Int(hContext,"internal_error",iRet);
				}
			}
		...
		*/
		...
		/* lock opening balance */
		if (iRet == PD_OK) {
DEBUGLOG(("BOOLBalance:DebitDepositAmount DBOLBAIDBal::GetBalanceForUpdate() called\n"));
			DBObjPtr = CreateObj(DBPtr,"DBOLBAIDBal","GetBalanceForUpdate");
			if ((*DBObjPtr)(csBaid,
					csCountry,
					csPspCCY,
					hContext) != PD_OK) {
				iRet = INT_ERR;
DEBUGLOG(("BOOLBalance:DebitDepositAmount DBOLBAIDBal::GetBalanceForUpdate() Failed\n"));
ERRLOG("BOOLBalance:DebitDepositAmount DBOLBAIDBal::GetBalanceForUpdate() Failed\n");
			} else {
				GetField_Double(hContext, "balance", &dTmp);
				PutField_Double(hContext, "open_bal", dTmp);
				GetField_Double(hContext, "in_transit", &dTmp);
...
DEBUGLOG(("BOOLBalance:DebitDepositAmount() Call DBOLMerchantBalance:UpdateSettBal[%c]\n", PD_IND_DEBIT));
...
DEBUGLOG(("BOOLBalance:DebitDepositAmount() UpdateReserved\n"));
				DBObjPtr = CreateObj(DBPtr, "DBOLMerchantBalance", "UpdateReserved");
					if ((*DBObjPtr)(csMerchantID,
						...
						PD_UPDATE_USER) != PD_OK) {
					iRet = INT_ERR;
DEBUGLOG(("BOOLBalance:DebitDepositAmount() UpdateFloat Failed\n"));
				}
			}
		}
/* PSP */
		...
		if (iRet == PD_OK) {
DEBUGLOG(("BOOLBalance:DebitDepositAmount() DBOLBAIDBal::UpdateBalance() called\n"));
			DBObjPtr = CreateObj(DBPtr, "DBOLBAIDBal", "UpdateBalance");
			if ((*DBObjPtr)(csBaid,
					csCountry,
					csPspCCY,
					...
					PD_UPDATE_USER) != PD_OK) {
				iRet = INT_ERR;
DEBUGLOG(("BOOLBalance:DebitDepositAmount() DBOLBAIDBal::UpdateBalance() Failed\n"));
ERRLOG("BOOLBalance:DebitDepositAmount() DBOLBAIDBal::UpdateBalance() Failed\n");
			}
		}
	}
...
DEBUGLOG(("BOOLBalance:ProcessHold() baid = [%s]\n",csBaid));
	}
...
DEBUGLOG(("BOOLBalance: ProcessHold: DBOLBAIDBal->GetBalanceForUpdate\n"));
		DBObjPtr = CreateObj(DBPtr, "DBOLBAIDBal", "GetBalanceForUpdate");
		if ((unsigned long)(*DBObjPtr)(csBaid, csTxnCountry, csTxnCcy, hVal) == PD_OK) {
...
DEBUGLOG(("BOOLBalance: balance = [%f]\n", dBal));
				PutField_Double(hContext, "psp_open_bal", dBal);
			}
			if (GetField_Double(hVal, "total_hold", &dHold)) {
DEBUGLOG(("BOOLBalance: total_hold = [%f]\n", dHold));
			}
			if (GetField_Double(hVal, "in_transit", &dInTransit)) {
DEBUGLOG(("BOOLBalance: in_transit = [%f]\n", dInTransit));
				PutField_Double(hContext, "psp_open_in_transit", dInTransit);
			}
			dAval = dBal - dHold;
		}
	}
...
DEBUGLOG(("BOOLBalance: reserved_payout= [%f]\n",dTmp));
			...
			hash_init(hVal, 0);
DEBUGLOG(("BOOLBalance: ProcessHold: DBOLBAIDBal->UpdateHoldBalance\n"));
			DBObjPtr = CreateObj(DBPtr, "DBOLBAIDBal", "UpdateHoldBalance");
			iRet = (unsigned long)(*DBObjPtr)(csBaid,
							csTxnCountry,
							csTxnCcy,
							cHold,
							dAmt,
			...
			if (iRet == PD_OK) {
DEBUGLOG(("BOOLBalance: ProcessHold: DBOLBAIDBal->GetBalance\n"));
				DBObjPtr = CreateObj(DBPtr, "DBOLBAIDBal", "GetBalance");
				if ((unsigned long)(*DBObjPtr)(csBaid, csTxnCountry, csTxnCcy, hVal) == PD_OK) {
...
DEBUGLOG(("BOOLBalance: total_hold = [%f]\n", dTmp));
					...
					if (GetField_Double(hVal, "prepaid", &dTmp)) {
						PutField_Double(hContext, "prepaid", dTmp);
DEBUGLOG(("BOOLBalance: prepaid = [%f]\n", dTmp));
					}
					if (GetField_Double(hVal, "in_transit", &dTmp)) {
...
DEBUGLOG(("BOOLBalance:UpdatePayoutAmount() org_psp_id is missing!!!\n"));
	...
	if (GetField_CString(hContext,"org_baid",&csOrgBaid)) {
DEBUGLOG(("BOOLBalance:UpdatePayoutAmount() org_baid = [%s]\n",csOrgBaid));
	}
	else {
		if(cParty==PD_TYPE_MERCHANT){
			csOrgBaid = strdup("000");
		}
		else{
DEBUGLOG(("BOOLBalance:UpdatePayoutAmount() org_baid is missing!!!\n"));
		}
	}
...
DEBUGLOG(("BOOLBalance: reserved_payout= [%f]\n",dTmp));
	...
	if((iRet==PD_OK) && (cParty!=PD_TYPE_MERCHANT)){
		hash_init(hVal,0);
		DBObjPtr = CreateObj(DBPtr,"DBOLBAIDBal","GetBalance");
		if((unsigned long)(*DBObjPtr)(csOrgBaid,csOrgTxnCountry,csOrgDstCcy,hVal)!=PD_ERR){
			if(GetField_Double(hVal,"balance",&dTmp)){
...
DEBUGLOG(("BOOLBalance:PayoutSuccess() GetOpenBalanceForUpdate Failed\n"));
	...
	if(cParty!=PD_TYPE_MERCHANT){
		...
		if ((*DBObjPtr)(csBaid,
				...
				hContext) != PD_OK) {
			iRet = INT_ERR;
DEBUGLOG(("BOOLBalance:PayoutSuccess() DBOLBAIDBal::GetBalanceForUpdate() Failed\n"));
			return iRet;
		}
		else{
			if(GetField_Double(hContext,"balance",&dTmp)){
				PutField_Double(hContext,"psp_open_bal",dTmp);
			}
			if(GetField_Double(hContext, "in_transit", &dTmp)) {
				PutField_Double(hContext, "open_in_transit", dTmp);
			}
...
DEBUGLOG(("BOOLBalance:PayoutSuccess DBOLBAIDBal::UpdateBalance() called\n"));
		DBObjPtr = CreateObj(DBPtr, "DBOLBAIDBal", "UpdateBalance");
		if ((*DBObjPtr)(csBaid,
				csCountry,
				csPspCCY,
				...
				PD_UPDATE_USER) != PD_OK) {
			iRet = INT_ERR;
DEBUGLOG(("BOOLBalance:PayoutSuccess() DBOLBAIDBal::UpdateBalance() Failed\n"));
ERRLOG("BOOLBalance:PayoutSuccess() DBOLBAIDBal::UpdateBalance() Failed\n");
		}
	}
...
DEBUGLOG(("BOOLBalance:PayoutVoided() GetOpenBalanceForUpdate Failed\n"));
	...
	if (cParty!=PD_TYPE_MERCHANT){
		...
		if ((*DBObjPtr)(csBaid,
				...
				hContext) != PD_OK) {
			iRet = INT_ERR;
DEBUGLOG(("BOOLBalance:PayoutVoided() DBOLBAIDBal::GetBalanceForUpdate() Failed\n"));
			return iRet;
		}
		else{
			if(GetField_Double(hContext,"balance",&dTmp)){
...
DEBUGLOG(("BOOLBalance:DBOLMerchantBalance::UpdateFloat() Failed\n"));
	...
	if (cParty==PD_TYPE_PSP || cParty==PD_TYPE_GLOBAL) {
DEBUGLOG(("BOOLBalance:PayoutVoided DBOLBAIDBal::UpdateBalance() called\n"));
		DBObjPtr = CreateObj(DBPtr, "DBOLBAIDBal", "UpdateBalance");
		if ((*DBObjPtr)(csBaid,
				csCountry,
				csPspCCY,
				...
				PD_UPDATE_USER) != PD_OK) {
			iRet = INT_ERR;
DEBUGLOG(("BOOLBalance:PayoutVoided() DBOLBAIDBal::UpdateBalance() Failed\n"));
ERRLOG("BOOLBalance:PayoutVoided() DBOLBAIDBal::UpdateBalance() Failed\n");
		}
	}
...
//party_type
	if(GetField_Char(hContext,"party_type",&cType)){
DEBUGLOG(("BOOLBalance:UpdateSettlementAmount() party_type = [%s]\n",cType));
	}
...
///org baid
		if (GetField_CString(hContext,"baid",&csOrgBAID)) {
DEBUGLOG(("BOOLBalance:UpdateSettlementAmount() baid = [%s]\n",csOrgBAID));
		}
		else {
DEBUGLOG(("BOOLBalance:UpdateSettlementAmount() baid is missing!!!\n"));
		}
	}
...
DEBUGLOG(("BOOLBalance:UpdateSettlementAmount() Call DBOLMerchantBalance:UpdateSettInTransit[%c]\n",PD_IND_DEBIT));
		...
		else if (!strcmp(csTxnCode, PD_VOID_TXN_CODE) && !strcmp(csSubTxnCode, PD_OL_SETTLEMENT_VOID))  {
/* lock opening balance */
			DBObjPtr = CreateObj(DBPtr,"DBOLMerchantBalance","GetOpenBalanceForUpdate");
			if ((*DBObjPtr)(hContext,
				csOrgMerchantId,
...
DEBUGLOG(("BOOLBalance: reserved_payout= [%f]\n",dTmp));
	...
	///////psp settlement
		...
		DBObjPtr = CreateObj(DBPtr,"DBOLBAIDBal","GetBalanceForUpdate");
		if ((*DBObjPtr)(csOrgBAID,
				csOrgTxnCountry,
				csOrgTxnCcy,
				hContext) != PD_OK) {
			iRet = INT_ERR;
DEBUGLOG(("BOOLBalance:UpdateSettlementAmount() DBOLBAIDBal::GetBalanceForUpdate() Failed\n"));
			return iRet;
		}
		else{
			if(GetField_Double(hContext,"balance",&dTmp)){
...
DEBUGLOG(("BOOLBalance:UpdateSettlementAmount() DBOLBAIDBal::UpdateBalance() called\n"));
		DBObjPtr = CreateObj(DBPtr, "DBOLBAIDBal", "UpdateBalance");
		if ((*DBObjPtr)(csOrgBAID,
				csOrgTxnCountry,
				csOrgTxnCcy,
				...
				PD_UPDATE_USER) != PD_OK) {
			iRet = INT_ERR;
DEBUGLOG(("BOOLBalance:UpdateSettlementAmount() DBOLBAIDBal::UpdateBalance() Failed\n"));
ERRLOG("BOOLBalance:UpdateSettlementAmount() DBOLBAIDBal::UpdateBalance() Failed\n");
		}
	}
...
/* return_mfee */
        if (GetField_Int(hContext,"return_mfee",&iReturnMerchFee)) {
DEBUGLOG(("BOOLBalance:ProcessFundinPayout() return_mfee = [%d]\n",iReturnMerchFee));
        }

	// Calculate Net Amount for Merchant Void (Debit)
        if (cPartyType == PD_TYPE_MERCHANT) {
                if (iVoidFlag == PD_TRUE) {
                        if (cDCInd == PD_IND_DEBIT) {

				// Check Void Transaction without Fee
...
DEBUGLOG(("BOOLBalance:ProcessFundinPayout() net_amt = [%lf]\n",dNetAmt));
                        }
                }
        } 
...
DEBUGLOG(("BOOLBalance: ProcessFundinPayout fundin_payout = [%f]\n",dOrgFundinAmt));
			...
			if (GetField_Double(hVal, "reserved_payout", &dOrgResvPOAmt)) {
...
ERRLOG("BOOLBalance: ProcessFundinPayout: Original Fundin [%lf] < Debit Fundin [%lf]\n",dOrgFundinAmt, dFundinAmt);
				...
				if (cDCInd == PD_IND_DEBIT) {
					...
					// Check Remain Available for Payout Balance
DEBUGLOG(("BOOLBalance: ProcessFundinPayout: Void: DBOLMerchantBalance->GetRemainPOBalance\n"));
                                        DBObjPtr = CreateObj(DBPtr,"DBOLMerchantBalance","GetRemainPOBalance");
					...
					if (iRet != PD_OK) {
DEBUGLOG(("BOOLBalance: ProcessFundinPayout: Void: GetRemainPOBalance FAILED!\n"));
ERRLOG("BOOLBalance: ProcessFundinPayout: Void: GetRemainPOBalance FAILED\n");
                                        } else {

/* merchant_net_float */
                                                if (GetField_Double(hContext, "merchant_net_float", &dTmp)) {
                                                        if (dTmp < 0.0) {
DEBUGLOG(("BOOLBalance: ProcessFundinPayout: Void: merchant_net_float problem [%lf]\n", dTmp));
                                                                iRet = INT_ERR;
								PutField_Int(hContext, "internal_error", iRet);
                                                        }
                                                }
...
/* merchant_aval_po */
                                                if (GetField_Double(hContext, "merchant_aval_po", &dOrgAvaPOBal)) {
DEBUGLOG(("BOOLBalance: ProcessFundinPayout: Void: merchant_aval_po = [%lf]\n",dOrgAvaPOBal));
                                                }

/* merchant_fundin_po */
                                                if (GetField_Double(hContext, "merchant_fundin_po", &dOrgFundin)) {
DEBUGLOG(("BOOLBalance: ProcessFundinPayout: Void: merchant_fundin_po = [%lf]\n",dOrgFundin));
                                                }

/* merchant_reserved_po */
                                                if (GetField_Double(hContext, "merchant_reserved_po", &dOrgResvPO)) {
DEBUGLOG(("BOOLBalance: ProcessFundinPayout: Void: merchant_reserved_po = [%lf]\n",dOrgResvPO));
                                                }

/* merchant_prev_ft */
						dOrgPrevFt = dOrgAvaPOBal - dOrgFundin - dOrgResvPO;				
DEBUGLOG(("BOOLBalance: ProcessFundinPayout: Void: merchant_prev_ft = [%lf]\n",dOrgPrevFt));

                                                if (dNetAmt > dOrgAvaPOBal) {
                                                        iRet = INT_INSUFFICIENT_FUND;
                                                        //PutField_Int(hContext,"fn_internal_error",iRet);
                                                        PutField_Int(hContext, "internal_error", iRet);

DEBUGLOG(("BOOLBalance: ProcessFundinPayout: Void: insufficient fund [%s] [%s] [%s] [%lf] [%lf]\n", csMerchantId, csTxnCcy,csServiceCode, dOrgAvaPOBal, dNetAmt));
ERRLOG("BOOLBalance: ProcessFundinPayout: Void: insufficient fund [%s] [%s] [%s] [%lf] [%lf]\n",csMerchantId,csTxnCcy,csServiceCode,dOrgAvaPOBal, dNetAmt);
                                                }
                                        }

					// Check Merchant Open Balance
                                        if (dBalInPsp < dNetAmt) {
DEBUGLOG(("BOOLBalance: ProcessFundinPayout: Void: Original BalInPsp[%lf] < Debit NetAmt [%lf]\n",dBalInPsp, dNetAmt));
ERRLOG("BOOLBalance: ProcessFundinPayout: Void: Original BalInPsp [%lf] < Debit NetAmt [%lf]\n",dBalInPsp, dNetAmt);
                                                iRet = INT_INVALID_PAY_AMOUNT;
                                                //PutField_Int(hContext, "fn_internal_error", iRet);
                                                PutField_Int(hContext, "internal_error", iRet);
                                        }
...
DEBUGLOG(("BOOLBalance: ProcessFundinPayout: DBOLMerchantBalance->UpdateFundInPayout\n"));
				...
				if (iChgBalMode == PD_TRUE) {
						...
						//
						// Calculate the deduct_fundin, deduct_previous_float, deduct_reserved_po by using net_amount
						//
						
						// Deduct Fundin (FI)
						if (dNetAmt >= dOrgFundin) {
                                                        dDeductFundin = dOrgFundin;
                                                        dRemainNetAmt = dNetAmt - dOrgFundin;
                                                } else {
                                                        dDeductFundin = dNetAmt;
                                                        dRemainNetAmt = 0.0;
                                                }

						// Deduct Previous Float (DF)
						if (dRemainNetAmt > 0.0) {
							if (dOrgPrevFt >= 0) {
                                                        	if (dRemainNetAmt >= dOrgPrevFt) {
                                                                	dDeductPrevFt = dOrgPrevFt;
                                                                	dRemainNetAmt = dRemainNetAmt - dOrgPrevFt;
                                                        	} else {
                                                                	dDeductPrevFt = dRemainNetAmt;
                                                                	dRemainNetAmt = 0.0;
                                                        	}
							} else {
								dDeductPrevFt = 0.0;
						...
						// Deduct Reserved Payout (PR)
						if (dRemainNetAmt > 0.0) {
                                                        dDeductResvPO = dRemainNetAmt;
                                                } else {
                                                        dDeductResvPO = 0.0;
                                                }

						//
						// Calculate the org_deduct_fundin, org_deduct_previous_float, org_deduct_reserved_po by using txn_amount
						// 

						// Deduct Fundin (FI)
                                                if (dTxnAmt >= dOrgFundin) {
                                                        dOrgDeductFundin = dOrgFundin;
							dRemainTxnAmt = dTxnAmt - dOrgFundin;
                                                } else {
                                                        dOrgDeductFundin = dTxnAmt;
                                                        dRemainTxnAmt = 0.0;
                                                }
							...
							if (dOrgPrevFt >= 0) {
                                                        	if (dRemainTxnAmt >= dOrgPrevFt) {
                                                                	dOrgDeductPrevFt = dOrgPrevFt;
                                                                	dRemainTxnAmt = dRemainTxnAmt - dOrgPrevFt;
                                                        	} else {
...
DEBUGLOG(("BOOLBalance: ProcessFundinPayout deduct [%lf] [%lf] [%lf] total [%lf]\n", dDeductFundin, dDeductPrevFt, dDeductResvPO, dNetAmt));
DEBUGLOG(("BOOLBalance: ProcessFundinPayout org deduct [%lf] [%lf] [%lf] fee [%lf]\n", dOrgDeductFundin, dOrgDeductPrevFt, dOrgDeductResvPO, dFee));

						// Update for Txn Elements
						PutField_Double(hContext, "org_actual_fundin", dOrgDeductFundin);
                                                PutField_Double(hContext, "org_actual_resv_po", dOrgDeductResvPO);
                                                PutField_Double(hContext, "org_actual_prev_ft", dOrgDeductPrevFt);

                                                PutField_Double(hContext, "actual_fundin", dDeductFundin);
                                                PutField_Double(hContext, "actual_resv_po", dDeductResvPO);
                                                PutField_Double(hContext, "actual_prev_ft", dDeductPrevFt);

                                                PutField_Double(hContext, "actual_net_amt", dNetAmt);

                                                dTmp = dDeductFundin + dDeductResvPO + dDeductPrevFt;
                                                PutField_Double(hContext, "net_amt", dTmp);

                                                PutField_Double(hContext, "actual_txn_fee", dFee);
                                                PutField_CString(hContext, "actual_txn_fee_ccy", csTxnCcy);

                                                PutField_CString(hContext, "markup_ccy", csTxnCcy);
...
DEBUGLOG(("BOBalance: ProcessFundinPayout: Call ProcessTransferAvaPO\n"));
                                                iRet = ProcessTransferAvaPayout(hContext);
                                                if (iRet != PD_OK) {
DEBUGLOG(("BOBalance: ProcessFundinPayout: Call ProcessTransferAvaPO FAILED\n"));
ERRLOG("BOBalance: ProcessFundinPayout: Call ProcessTransferAvaPO FAILED\n");
                                                }
                                        }
				}
			}
...
DEBUGLOG(("BOOLBalance:ProcessMerchantAdjAmount() bal_type = [%c]\n",cBalType));
	}
	else {
DEBUGLOG(("BOOLBalance:ProcessMerchantAdjAmount() no input bal_type\n"));
	}

	if (iRet == PD_OK) {
		hash_init(hVal, 0);

		if(iAllowModify == PD_FALSE && iAllowBalNegative == PD_FALSE && cBalType == PD_MERCH_AVA_SETT_POOL){
...
DEBUGLOG(("BOOLBalance:ProcessMerchantAdjAmount() [%lf] [%lf] INSUFFICIENT_FUND\n",dBal,dNetAmt));
	...
	if (iRet == PD_OK) {
		if(cBalType == PD_MERCH_AVA_SETT_POOL){
...
DEBUGLOG(("BOOLBalance:ProcessMerchantAdjAmount() UpdateAdjBal Failed !\n"));
				iRet = INT_ERR;
			}
		}
		else{
DEBUGLOG(("BOOLBalance:ProcessMerchantAdjAmount() Call UpdateAdjPOBal\n"));
			DBObjPtr = CreateObj(DBPtr,"DBOLMerchantBalance","UpdateAdjPOBal");
			if((unsigned long)(*DBObjPtr)(csMerchantId,csTxnCountry,csTxnCcy, csServiceCode, dNetAmt, cDCType, csUser)!= PD_OK){
DEBUGLOG(("BOOLBalance:ProcessMerchantAdjAmount() UpdateAdjPOBal Failed !\n"));
				iRet = INT_ERR;
			}
		}
	}
...
/* void_flag */
        if (GetField_Int(hContext,"void_flag",&iVoidFlag)) {
DEBUGLOG(("BOOLBalance:ProcessTransferAvaPayout() void_flag  = [%d]\n",iVoidFlag));
        }
...
DEBUGLOG(("BOOLBalance:ProcessTransferAvaPayout() Add AddMarkupAmtElement\n"));
                ...
                if (iRet == PD_OK) {
                        /* Add Element */
			PutField_Int(hElement, "void_flag", iVoidFlag);
                        PutField_Char(hElement, "party_type", PD_TYPE_MERCHANT);
                        if (cDCType == PD_IND_DEBIT) {
                                PutField_CString(hElement, "amount_type", PD_DR);
                        } else {
...
int GetTotalReserveForUpdate(hash_t* hContext,
		...
		double *dBalance)
{
	int iRet = PD_OK;
	double dReserveAmt = 0.0;
DEBUGLOG(("BOOLBalance:GetTotalReserveForUpdate()\n"));
	*dBalance = 0.0;

DEBUGLOG(("BOOLBalance:GetTotalReserveForUpdate() mid = [%s]\n", csMerchantId));
DEBUGLOG(("BOOLBalance:GetTotalReserveForUpdate() txn_ccy = [%s]\n", csTxnCcy));
DEBUGLOG(("BOOLBalance:GetTotalReserveForUpdate() txn_country = [%s]\n",csTxnCountry));
DEBUGLOG(("BOOLBalance:GetTotalReserveForUpdate() service = [%s]\n", csService));
	DBObjPtr = CreateObj(DBPtr,"DBOLMerchantBalance","GetOpenBalanceForUpdate");
...
DEBUGLOG(("BOOLBalance:GetTotalReserveForUpdate() reserve amount = [%f]\n", dReserveAmt));
			*dBalance = dReserveAmt;
		}
	} else {
DEBUGLOG(("BOOLBalance:GetTotalReserveForUpdate() balance for [%s] [%s] [%s] not found\n", csMerchantId, csTxnCcy, csService));
ERRLOG("BOOLBalance:GetTotalReserveForUpdate() balance for [%s] [%s] [%s] not found\n", csMerchantId, csTxnCcy, csService);
	}
...
DEBUGLOG(("BOOLBalance:ReleaseMerchantResAmount()\n"));
	DBObjPtr = CreateObj(DBPtr, "DBOLMerchantBalance", "ReleaseReserve");
	if ((*DBObjPtr)(csMerchantID,
			csCountry,
			csMerchantCCY,
			csServiceCode,
			...
			PD_UPDATE_USER) != PD_OK) {
		iRet = INT_ERR;
DEBUGLOG(("BOOLBalance:ReleaseMerchantResAmount() failed\n"));
	}

	return iRet;
}

int GetAvalBaidBalanceForUpdate(const char* csTxnCountry,
		const char* csTxnCcy,
		const char* csBaid,
		double *dBalance)
{
	int iRet = PD_OK;
DEBUGLOG(("BOOLBalance:GetAvalBaidBalanceForUpdate()\n"));

DEBUGLOG(("BOOLBalance:GetAvalBaidBalanceForUpdate() baid = [%s]\n", csBaid));
DEBUGLOG(("BOOLBalance:GetAvalBaidBalanceForUpdate() txn_ccy = [%s]\n", csTxnCcy));
DEBUGLOG(("BOOLBalance:GetAvalBaidBalanceForUpdate() txn_country = [%s]\n", csTxnCountry));
	DBObjPtr = CreateObj(DBPtr, "DBOLBAIDBal", "GetAvalBaidBal");
	if ((unsigned long)(*DBObjPtr)(csBaid, csTxnCcy, csTxnCountry, dBalance) == PD_OK) {
DEBUGLOG(("Balance = [%lf]\n", *dBalance));
	} else {
DEBUGLOG(("BOOLBalance:GetAvalBaidBalanceForUpdate() balance for [%s] [%s] not found\n", csBaid, csTxnCcy));
ERRLOG("BOOLBalance:GetAvalBaidBalanceForUpdate() balance for [%s] [%s] not found\n", csBaid, csTxnCcy);
		*dBalance = 0.0;
	}
...
int UpdateAmount(hash_t* hContext)
{
	...
	char cAmtType;
	double dTxnAmt = 0.0;
	//char csUpdateUser[PD_USER_LEN+1];
	char *csUpdateUser = strdup("");
	char *csPool;
	char csFuncName[PD_TMP_BUF_LEN];
	double	dTmp = 0.0;
	...
	if (iRet == PD_OK) {
		if (GetField_CString(hContext, "baid", &csBaid)) {
DEBUGLOG(("UpdateAmount:: baid = [%s]\n", csBaid));
		} else {
DEBUGLOG(("UpdateAmount:: baid not found!!\n"));
ERRLOG("BOOLBalance: UpdateAmount:: baid not found!!\n");
			iRet = INT_ERR;
		}
	}
...
DEBUGLOG(("UpdateAmount:: country = [%s]\n", csCountry));
		} else {
DEBUGLOG(("UpdateAmount:: country not found!!\n"));
ERRLOG("BOOLBalance: UpdateAmount:: country not found!!\n");
			iRet = INT_ERR;
		}
	}
...
DEBUGLOG(("UpdateAmount:: currency = [%s]\n", csCcy));
		} else {
DEBUGLOG(("UpdateAmount:: currency not found!!\n"));
ERRLOG("BOOLBalance: UpdateAmount:: currency not found!!\n");
			iRet = INT_ERR;
		}
	}

	if (iRet == PD_OK) {
		if (GetField_Char(hContext, "amt_type", &cAmtType)) {
DEBUGLOG(("UpdateAmount:: amt_type = [%c]\n", cAmtType));
		} else {
DEBUGLOG(("UpdateAmount:: amt_type not found!!\n"));
ERRLOG("BOOLBalance: UpdateAmount:: amt_type not found!!\n");
			iRet = INT_ERR;
		}
	}
...
ERRLOG("BOOLBalance: UpdateAmount:: txn_amt not found!!\n");
			iRet = INT_ERR;
		}
	}
...
DEBUGLOG(("UpdateAmount:: update_user (default) = [%s]\n", csUpdateUser));
	...
	if (iRet == PD_OK) {
DEBUGLOG(("UpdateAmount:: DBOLBAIDBal:GetBalanceForUpdate\n"));
		hash_init(hVal,0);
                DBObjPtr = CreateObj(DBPtr,"DBOLBAIDBal","GetBalanceForUpdate");
                if ((*DBObjPtr)(csBaid,
                                csCountry,
                                ...
                                hVal) != PD_OK) {
                        iRet = INT_ERR;
DEBUGLOG(("UpdateAmount:: DBOLBAIDBal:GetBalanceForUpdate Failed\n"));
		}
		else{
			if(GetField_Double(hVal,"balance",&dTmp)){
				PutField_Double(hContext,"open_bal",dTmp);
	...
	if (iRet == PD_OK) {
		if (GetField_CString(hContext, "pool", &csPool)) {
DEBUGLOG(("UpdateAmount:: pool = [%s]\n", csPool));
		} else {
DEBUGLOG(("UpdateAmount:: pool not found!!\n"));
ERRLOG("BOOLBalance: UpdateAmount:: pool not found!!\n");
                        iRet = INT_ERR;
                }
        }
	...
	if (iRet == PD_OK) {
		if (!strcmp(csPool, PD_ACCT_BAL_POOL)) {
			sprintf(csFuncName, "UpdateBalance");
		} else if (!strcmp(csPool, PD_LIEN_POOL)) {
			sprintf(csFuncName, "UpdateHoldBalance");
		} else if (!strcmp(csPool, PD_PREPAID_POOL)) {
			sprintf(csFuncName, "UpdatePrepaid");
		} else if (!strcmp(csPool, PD_INTRANSIT_POOL)) {
			sprintf(csFuncName, "UpdateInTransit");
		} else {
			iRet = INT_ERR;
DEBUGLOG(("UpdateAmount:: invalid pool!!\n"));
ERRLOG("BOOLBalance: UpdateAmount:: invalid pool!!\n");
		}

		if(iRet==PD_OK){
DEBUGLOG(("UpdateAmount:: Call DBOLBAIDBal:: %s\n", csFuncName));
			DBObjPtr = CreateObj(DBPtr, "DBOLBAIDBal", csFuncName);
			if ((*DBObjPtr)(csBaid,
					csCountry,
					csCcy,
					cAmtType,
					...
					csUpdateUser) != PD_OK) {
				iRet = INT_ERR;
DEBUGLOG(("UpdateAmount:: Call DBOLBAIDBal:: %s failed\n", csFuncName));
ERRLOG("BOOLBalance: UpdateAmount:: Call DBOLBAIDBal:: %s failed\n", csFuncName);
			}
		}
	}
	...
	DBObjPtr = CreateObj(DBPtr,"DBSystemControl","GetApprovalDT");
        (*DBObjPtr)(hContext);
DEBUGLOG(("UpdateAmount:: Call DBSystemControl:: GetApprovalDT\n"));

	if(iRet == PD_OK){
		hash_init(hVal,0);
		DBObjPtr = CreateObj(DBPtr,"DBOLBAIDBal","GetBalance");
		if((unsigned long)(*DBObjPtr)(csBaid,csCountry,csCcy,hVal)!=PD_ERR){
			if(GetField_Double(hVal,"balance",&dTmp)){
				PutField_Double(hContext,"bal",dTmp);
...
/* party_type */
        if(GetField_Char(hContext,"party_type",&cPartyType)){
DEBUGLOG(("ProcessAvaPayout() party_type = [%c]\n",cPartyType));
        } else {
DEBUGLOG(("ProcessAvaPayout() party_type is missing!!!\n"));
	}

/* merchant_id */
        if (GetField_CString(hContext,"merchant_id",&csMerchantId)) {
DEBUGLOG(("ProcessAvaPayout() merchant_id = [%s]\n",csMerchantId));
        }

/* txn_country */
        if (GetField_CString(hContext,"txn_country",&csTxnCountry)) {
DEBUGLOG(("ProcessAvaPayout() txn_country = [%s]\n",csTxnCountry));
        }  else {
DEBUGLOG(("ProcessAvaPayout() txn_country is missing!!!\n"));
        }

/* txn_ccy */
        if (GetField_CString(hContext,"txn_ccy",&csTxnCcy)) {
DEBUGLOG(("ProcessAvaPayout() txn_ccy = [%s]\n",csTxnCcy));
        } else {
DEBUGLOG(("ProcessAvaPayout() txn_ccy is missing!!!\n"));
        }

/* txn_amt */
        if (GetField_Double(hContext,"txn_amt",&dTxnAmt)) {
DEBUGLOG(("ProcessAvaPayout() txn_amt = [%lf]\n",dTxnAmt));
        } else {
DEBUGLOG(("ProcessAvaPayout() txn_amt is missing!!!\n"));
	}

/* service_code */
        if (GetField_CString(hContext,"service_code",&csServiceCode)) {
DEBUGLOG(("ProcessAvaPayout() service_code = [%s]\n",csServiceCode));
        } else {
DEBUGLOG(("ProcessAvaPayout() service_code is missing!!!\n"));
	}

/* dc_ind */
        if (GetField_Char(hContext, "dc_ind", &cDCInd)) {
DEBUGLOG(("ProcessAvaPayout() dc_ind = [%c]\n",cDCInd));
        } else {
DEBUGLOG(("ProcessAvaPayout() dc_ind is missing!!!\n"));
        }
...
DEBUGLOG(("ProcessAvaPayout:: DBOLMerchantBalance::GetOpenBalanceForUpdate\n"));
                	DBObjPtr = CreateObj(DBPtr,"DBOLMerchantBalance","GetOpenBalanceForUpdate");
                	if((unsigned long)(*DBObjPtr)(hVal, csMerchantId,csTxnCcy,csTxnCountry,csServiceCode) != INT_ERR) {

/* merchant_open_bal */
                        	if (GetField_Double(hVal, "merchant_open_bal", &dBalInPsp)) {
DEBUGLOG(("ProcessAvaPayout:: open_bal = [%f]\n",dBalInPsp));
                                	PutField_Double(hContext, "merchant_open_bal", dBalInPsp);
                        	}

/* merchant_open_bal_settlement */
				if (GetField_Double(hVal, "merchant_open_bal_settlement", &dTmp)) {
DEBUGLOG(("ProcessAvaPayout:: open_bal_settlement = [%f]\n",dTmp));
                                	PutField_Double(hContext,"merchant_open_bal_settlement",dTmp);
                        	}
				...
				if (cDCInd == PD_IND_DEBIT) {
					...
					// Check Remain Available for Payout Balance				
DEBUGLOG(("ProcessAvaPayout:: Call DBOLMerchantBalance::GetRemainPOBalance\n"));
                        		DBObjPtr = CreateObj(DBPtr,"DBOLMerchantBalance","GetRemainPOBalance");
                        		iRet = (unsigned long)(*DBObjPtr)(hContext,
                                                          		csMerchantId,
                                                          		csTxnCcy,
                        		...
                        		if (iRet != PD_OK) {
						iRet = INT_ERR;
DEBUGLOG(("ProcessAvaPayout:: Call DBOLMerchantBalance::GetRemainPOBalance FAILED!\n"));
ERRLOG("BOOLBalance:: ProcessAvaPayout:: Call DBOLMerchantBalance::GetRemainPOBalance FAILED\n");
                        		} else {

/* merchant_aval_po */
                                		if (GetField_Double(hContext, "merchant_aval_po", &dAvaPOBal)) {
DEBUGLOG(("ProcessAvaPayout:: Call DBOLMerchantBalance::GetRemainPOBalance:: merchant_aval_po = [%lf]\n",dAvaPOBal));
                                		}
					...
					// Check Merchant Open Balance
                        	        if (dBalInPsp < dTxnAmt) {
DEBUGLOG(("ProcessAvaPayout:: Original BalInPsp[%lf] < Debit NetAmt [%lf]\n",dBalInPsp, dTxnAmt));
ERRLOG("ProcessAvaPayout:: Original BalInPsp [%lf] < Debit NetAmt [%lf]\n",dBalInPsp, dTxnAmt);
                                	      	iRet = INT_INVALID_PAY_AMOUNT;
					}
				}
			} else {
				iRet = INT_ERR;
DEBUGLOG(("ProcessAvaPayout:: DBOLMerchantBalance::GetOpenBalanceForUpdate Failed\n"));
ERRLOG("ProcessAvaPayout:: DBOLMerchantBalance::GetOpenBalanceForUpdate Failed\n");
                	}
...
DEBUGLOG(("ProcessAvaPayout:: DBSystemControl::GetApprovalDT\n"));
			DBObjPtr = CreateObj(DBPtr,"DBSystemControl","GetApprovalDT");
        		if ((unsigned long)(*DBObjPtr)(hContext) == FOUND) {
                			
/* approval_date */
				if (GetField_CString(hContext,"approval_date",&csTmp)) {
DEBUGLOG(("ProcessAvaPayout:: DBSystemControl::GetApprovalDT:: approval_date = [%s]\n",csTmp));
				}
               		}
		}
...
DEBUGLOG(("ProcessAvaPayout:: DBOLMerchantBalance::UpdateCurrBal()\n"));
                        DBObjPtr = CreateObj(DBPtr,"DBOLMerchantBalance","UpdateCurrBal");
                        if ((*DBObjPtr)(csMerchantId,
                                        csTxnCountry,
                                        csTxnCcy,
                                        ...
                                        PD_UPDATE_USER) != PD_OK) {
                                iRet = INT_ERR;
DEBUGLOG(("ProcessAvaPayout:: DBOLMerchantBalance::UpdateCurrBal() Failed\n"));
ERRLOG("BOOLBalance:: ProcessAvaPayout:: DBOLMerchantBalance::UpdateCurrBal() Failed\n");
                        }
                }
		...
		if (iRet == PD_OK){
                     	...
                     	DBObjPtr = CreateObj(DBPtr,"DBOLMerchantBalance","GetCurrentValues");
                       	if((unsigned long)(*DBObjPtr)(csMerchantId,csTxnCcy,csTxnCountry,csServiceCode,hVal) != INT_ERR){
                          	if(GetField_Double(hVal,"current_bal",&dTmp)){
                                 	PutField_Double(hContext,"current_bal",dTmp);
DEBUGLOG(("ProcessAvaPayout:: DBOLMerchantBalance:: GetCurrentValues() current_bal= [%f]\n",dTmp));
                               	}
                             	if(GetField_Double(hVal,"total_float",&dTmp)){
                                     	PutField_Double(hContext,"total_float",dTmp);
DEBUGLOG(("ProcessAvaPayout:: DBOLMerchantBalance:: GetCurrentValues() total_float = [%f]\n",dTmp));
                              	}
                             	if(GetField_Double(hVal,"current_bal_settlement",&dTmp)){
                                      	PutField_Double(hContext,"current_bal_settlement",dTmp);
DEBUGLOG(("ProcessAvaPayout:: DBOLMerchantBalance:: GetCurrentValues() current_bal_settlement= [%f]\n",dTmp));
                               	}
                               	if(GetField_Double(hVal,"total_float_settlement",&dTmp)){
                                    	PutField_Double(hContext,"total_float_settlement",dTmp);
DEBUGLOG(("ProcessAvaPayout:: DBOLMerchantBalance:: GetCurrentValues() total_float_settlement= [%f]\n",dTmp));
                             	}
                              	if(GetField_Double(hVal,"total_reserved_amount",&dTmp)){
                                     	PutField_Double(hContext,"total_reserved_amount",dTmp);
DEBUGLOG(("ProcessAvaPayout:: DBOLMerchantBalance:: GetCurrentValues() total_reserved_amount= [%f]\n",dTmp));
                               	}
                              	if(GetField_Double(hVal,"total_hold",&dTmp)){
                                       	PutField_Double(hContext,"total_hold",dTmp);
DEBUGLOG(("ProcessAvaPayout:: DBOLMerchantBalance:: GetCurrentValues() total_hold= [%f]\n",dTmp));
                               	}
                              	if(GetField_Double(hVal,"total_hold_settlement",&dTmp)){
                                      	PutField_Double(hContext,"total_hold_settlement",dTmp);
DEBUGLOG(("ProcessAvaPayout:: DBOLMerchantBalance:: GetCurrentValues() total_hold_settlement= [%f]\n",dTmp));
                             	}
                            	if(GetField_Double(hVal,"total_float_after_payout",&dTmp)){
                                     	PutField_Double(hContext,"total_float_after_payout",dTmp);
DEBUGLOG(("ProcessAvaPayout:: DBOLMerchantBalance:: GetCurrentValues() total_float_after_payout= [%f]\n",dTmp));
                               	}
                              	if(GetField_Double(hVal,"fundin_payout",&dTmp)){
                                    	PutField_Double(hContext,"fundin_payout",dTmp);
DEBUGLOG(("ProcessAvaPayout:: DBOLMerchantBalance:: GetCurrentValues() fundin_payout= [%f]\n",dTmp));
                              	}
                             	if(GetField_Double(hVal,"reserved_payout",&dTmp)){
                                   	PutField_Double(hContext,"reserved_payout",dTmp);
DEBUGLOG(("ProcessAvaPayout:: DBOLMerchantBalance:: GetCurrentValues() reserved_payout= [%f]\n",dTmp));
                             	}
                     	} else {
				iRet = INT_ERR;
DEBUGLOG(("ProcessAvaPayout:: DBOLMerchantBalance:: GetCurrentValues() Failed\n"));
ERRLOG("BOOLBalance:: ProcessAvaPayout:: DBOLMerchantBalance:: GetCurrentValues() Failed\n");
                     	}
...
DEBUGLOG(("ProcessAvaPayout:: BOOLTxnElements:: AddTxnAmtElement\n"));
                    	BOObjPtr = CreateObj(BOPtr,"BOOLTxnElements","AddTxnAmtElement");
                    	iRet = (unsigned long)(*BOObjPtr)(hContext);
		}
	}
...
DEBUGLOG(("ProcessAvaPayout:: iRet = [%d]\n", iRet));
        return iRet;
}
...
int IsUpdatePayoutTempBAIDBal(const char* csBaid)
{
	...
	// Check Payout Temp BAID Acct
	DBObjPtr = CreateObj(DBPtr,"DBOLBankAcctId","GetPayoutTempBAIDAcct");
        iDtlRet = (unsigned long)(*DBObjPtr)(csBaid);
        if (iDtlRet == FOUND) {

		// Check System Parameter - OFL_PAYOUT_TEMP_BAID_BAL_UPDATE
		DBObjPtr = CreateObj(DBPtr,"DBSystemParameter","FindCode");
        	iDtlRet = (unsigned long)(DBObjPtr)(PD_OFL_PO_TEMP_BAID_BAL_UPDATE,csValue);
		if (iDtlRet == FOUND) {
	...
	// Result
	if (iRet == PD_TRUE) {
DEBUGLOG(("BOOLBalance:IsUpdatePayoutTempBAIDBal() TRUE!!\n"));
	} else {
DEBUGLOG(("BOOLBalance:IsUpdatePayoutTempBAIDBal() FALSE!!\n"));
	}
