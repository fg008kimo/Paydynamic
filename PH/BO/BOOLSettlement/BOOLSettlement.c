...
Add ProcessEmailSettingBySettlement                2015/08/18              Elvis Wong
...
Modify for void settlement			   2016/05/12		   Dirk Wong
...
ERRLOG("BOOLSettlement::Authorize::Call BOEmailSetting:: Failure!!!\n");
                        }
		}
...
DEBUGLOG(("Authorize::Send_notify_email = [%d]\n", iSendNotifyEmail));
                                ...
                                PutField_Char(hEmailSettingRec,"party_type",cSettType);
                                PutField_CString(hEmailSettingRec,"party_id",csMerchantId);
                                PutField_CString(hEmailSettingRec,"funct",PD_EML_FUNCT_OL_SETT_ADMIN);
...
ERRLOG("BOOLSettlement::ProcessSettlementTxn() GetSettlementRecords:invalid merchant status!!!\n");
		...
		PutField_CString(hContext,"sub_txn_code",PD_OL_SETTLEMENT_VOID);
...
DEBUGLOG(("ProcessSettlementTxn::Call Update Org Transaction\n"));
		...
		if(iRet==PD_OK){
                        PutField_CString(hTxn,"txn_code",PD_OL_SETTLEMENT_REQUEST);
                        PutField_CString(hTxn,"add_user",csUser);

DEBUGLOG(("ProcessSettlementTxn::Call DBOLTxnRemarks:Add\n"));
