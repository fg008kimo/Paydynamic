...
-------------------------------                    ------------            --------------
Init Version                                       2011/04/14              [GOD]
Add ProcessHold					   2011/12/13		   [MSN]
Add GetAvalPspBalanceForUpdate			   2011/12/15	           [SWK]	
Add Txn Elements				   2012/02/02		   [GOD]
Void before/after recon, debit float/bal	   2012/07/26		   [MSN] 
Add DebitMerchantResAmount                         2012/08/22              [GOD]
Add PayoutVoid same_date_cancel			   2012/09/03		   [MSN]
Void deposit, update available settlement	   2012/11/30		   [MSN]
DebitSebBalance can be negative			   2013/02/08		   [MSN]
Add UpdateAFPOFloat				   2013/02/18		   [MSN]
Check current_bal for Deposit Refund		   2013/02/19		   [STP]
Check allow_bal_neg				   2013/02/22		   [WCS]
Change Calling for shortend the rowlock for        2013/02/25              [GOD]
CreditDeposit					
Check current_bal for Merch Adj(exclude chargeback)2013/03/01		   [STP]
Add ProcessFixedAmtHoldBack                        2013/08/25	           [SWK]
FixAmtHoldBack not allow ava settlement negative   2014/01/30		   [MSN]
update ProcessHold				   2014/02/06		   [MSN]
update weekly payout capacity			   2014/11/28		   [MSN]
Update ProcessFundinPayout			   2016/06/27		   [WMC]
and ProcessTransferAvaPayout
Revise UpdateTxnAmount and CreditDepositAmount
 for PRD255					   2020/03/30		   [MSN]
*/
...
DEBUGLOG(("UpdateTxnAmount() Start\n"));
...
DEBUGLOG((" org_txn_seq is missing!!!\n"));
	}

/*org txn country */
	if (GetField_CString(hContext,"org_txn_country",&csOrgTxnCountry)) {	
DEBUGLOG((" org_txn_country = [%s]\n",csOrgTxnCountry));
	}
	else {
DEBUGLOG((" org_txn_country is missing!!!\n"));
	}

/*org txn ccy */
	if (GetField_CString(hContext,"org_txn_ccy",&csOrgTxnCcy)) {	
DEBUGLOG((" org_txn_ccy = [%s]\n",csOrgTxnCcy));
	}
	else {
DEBUGLOG((" org_txn_ccy is missing!!!\n"));
	}
/*org service Code */
	if (GetField_CString(hContext,"org_service_code",&csOrgServiceCode)) {	
DEBUGLOG((" org_service_code = [%s]\n",csOrgServiceCode));
	}
	else {
DEBUGLOG((" org_service_code is missing!!!\n"));
	}

/*org merchant id */
	if (GetField_CString(hContext,"org_merchant_id",&csOrgMerchantId)) {	
DEBUGLOG((" org_merchant_id = [%s]\n",csOrgMerchantId));
	}
	else {
DEBUGLOG((" org_merchant_id is missing!!!\n"));
	}

/*org psp ccy */
	if (GetField_CString(hContext,"org_dst_txn_ccy",&csOrgDstCcy)) {	
DEBUGLOG((" org_dst_txn_ccy = [%s]\n",csOrgDstCcy));
	}
	else {
DEBUGLOG((" org_dst_txn_ccy is missing!!!\n"));
	}
...
DEBUGLOG((" org_psp_id is missing!!!\n"));
	}
...
DEBUGLOG((" org_net_amt = [%lf]\n",dOrgNetAmt));
		}
		else {
DEBUGLOG((" org_net_amt is missing!!!\n"));
		}
	}

/*org psp txn amount */
	if (GetField_Double(hContext,"org_dst_net_amt",&dOrgDstTxnAmt)) {	
DEBUGLOG((" org_dst_net_amt = [%lf]\n",dOrgDstTxnAmt));
	}
	else {
DEBUGLOG((" org_dst_net_amt is missing!!!\n"));
	}
...
DEBUGLOG(("Call DebitDepositAmount\n"));
...
DEBUGLOG(("Call DBMerchantBalance::GetCurrentValues()\n"));
...
DEBUGLOG((" total_float_settlement= [%f]\n",dTmp));
				}
...
DEBUGLOG((" total_float_after_payout= [%f]\n",dTmp));
				}
			}

DEBUGLOG(("Call DBPspBalance::GetBalance()\n"));
...
DEBUGLOG(("None Deposit txn!!\n"));
		iRet = INT_ERR;
	}

DEBUGLOG(("UpdateTxnAmount() iRet = [%d]\n", iRet));
	return iRet;
}
			...
			double dPspFee) 
{
	...
	hash_t* hMerchBal;
	hMerchBal = (hash_t*) malloc (sizeof(hash_t));
	hash_init(hMerchBal,0);

DEBUGLOG(("CreditDepositAmount() Start\n"));
DEBUGLOG((" merchant = [%lf], psp = [%lf], res = [%lf]\n",dNetAmount,dPspAmount,dReserveAmount));

	
/* Add txn element for Reserved Amount */
	if (iRet == PD_OK) {
DEBUGLOG(("Call BOTxnElements::AddReserveAmtElement()\n"));
...
/* Add txn element for float Amount */
	if (iRet == PD_OK) {
DEBUGLOG(("Call BOTxnElements::AddMerchantFloatAmtElement()\n"));
...
/* Add PSP Elements */
	if (iRet == PD_OK) {
		...
		if (GetField_CString(hContext,"org_txn_seq",&csOrgTxnSeq)) {	
			...
			PutField_CString(hTxnEle, "txn_seq", csOrgTxnSeq);
			PutField_CString(hTxnEle, "txn_ccy", csPspCCY);
			...
			PutField_Double(hTxnEle, "txn_amt", dPspAmount);

DEBUGLOG(("Call BOTxnElements::AddPspTxnElement() [%s]\n", PD_ELEMENT_TXN_AMT));
			BOObjPtr = CreateObj(BOPtr,"BOTxnElements","AddPspTxnElement");
			iRet = (unsigned long)(*BOObjPtr)(hTxnEle);
DEBUGLOG(("AddPspTxnElement result = [%d]\n",iRet));

			if (iRet == PD_OK) {
				if(dPspFee > 0.0){
					PutField_CString(hTxnEle, "txn_element_type", PD_ELEMENT_TXN_FEE);
					PutField_CString(hTxnEle, "amount_type", PD_DR);
					PutField_Double(hTxnEle, "txn_amt", dPspFee);
...
DEBUGLOG(("AddPspTxnElement result = [%d]\n",iRet));
	...
	if (iRet == PD_OK) {
DEBUGLOG(("Call DBPspBalance::CreditBalanceRet()\n"));
		DBObjPtr = CreateObj(DBPtr,"DBPspBalance","CreditBalanceRet");
        	if ((*DBObjPtr)(csPspID,
				...
				hPspBal) != PD_OK) {
			iRet = INT_ERR;
DEBUGLOG(("CreditBalanceRet Failed\n"));
		}
		else{
			if(GetField_Double(hPspBal, "psp_bal", &dPtr)){
				PutField_Double(hContext, "psp_balance", dPtr);
DEBUGLOG((" psp_balance = [%f]\n",dPtr));
			}
			if(GetField_Double(hPspBal, "total_float", &dPtr)){
				PutField_Double(hContext, "psp_total_float", dPtr);
DEBUGLOG((" psp_total_float = [%f]\n",dPtr));
			}
			if(GetField_Double(hPspBal, "total_hold", &dPtr)){
				PutField_Double(hContext, "psp_total_hold", dPtr);
DEBUGLOG((" psp_total_hold = [%f]\n",dPtr));
			}
		}
	}
...
DEBUGLOG(("Call DBSystemControl::GetApprovalDT()\n"));
        DBObjPtr = CreateObj(DBPtr,"DBSystemControl","GetApprovalDT");
	iTmpRet = (unsigned long)(DBObjPtr)(hContext);
        if(iTmpRet != FOUND){
		iRet = INT_ERR;
DEBUGLOG(("GetApprovalDT Failed\n"));
	}
	else{
		if (GetField_CString(hContext,"approval_date",&csLocalPostingDate)) {
DEBUGLOG((" approval_date = [%s]\n",csLocalPostingDate));
		}
	}
	
	if(iRet == PD_OK){
DEBUGLOG(("Call DBMerchantBucket::CreditMerchantAmount()\n"));
		...
		if ((*DBObjPtr)(csLocalPostingDate,
			...
			iRet = INT_ERR;
DEBUGLOG(("CreditMerchantAmount [%s] Failed\n", PD_BUCKET_TYPE_FLOAT));
		}
	}

	if (iRet == PD_OK) {
DEBUGLOG(("Call DBMerchantBalance::UpdateFloatRet()\n"));
		DBObjPtr = CreateObj(DBPtr,"DBMerchantBalance","UpdateFloatRet");
        	if ((*DBObjPtr)(csMerchantID,
				...
				hMerchBal) != PD_OK) {
			iRet = INT_ERR;
DEBUGLOG(("UpdateFloatRet Failed\n"));
		}
		else{

			if(GetField_Double(hMerchBal,"current_bal",&dPtr)){
				PutField_Double(hContext,"current_bal",dPtr);
DEBUGLOG((" current_bal= [%f]\n",dPtr));
			}
			if(GetField_Double(hMerchBal,"total_float",&dPtr)){
				PutField_Double(hContext,"total_float",dPtr);
DEBUGLOG((" total_float= [%f]\n",dPtr));
			}
			if(GetField_Double(hMerchBal,"total_reserved_amount",&dPtr)){
				PutField_Double(hContext,"total_reserved_amount",dPtr);
DEBUGLOG((" total_reserved_amount= [%f]\n",dPtr));
			}
			if(GetField_Double(hMerchBal,"total_hold",&dPtr)){
				PutField_Double(hContext,"total_hold",dPtr);
DEBUGLOG((" total_hold= [%f]\n",dPtr));
			}
			if(GetField_Double(hMerchBal,"current_bal_settlement",&dPtr)){
				PutField_Double(hContext,"current_bal_settlement",dPtr);
DEBUGLOG((" current_bal_settlement= [%f]\n",dPtr));
			}
			if(GetField_Double(hMerchBal,"total_float_settlement",&dPtr)){
				PutField_Double(hContext,"total_float_settlement",dPtr);
DEBUGLOG((" total_float_settlement= [%f]\n",dPtr));
			}
			if(GetField_Double(hMerchBal,"total_hold_settlement",&dPtr)){
				PutField_Double(hContext,"total_hold_settlement",dPtr);
DEBUGLOG((" total_hold= [%f]\n",dPtr));
			}
			if(GetField_Double(hMerchBal,"fundin_payout",&dPtr)){
				PutField_Double(hContext,"fundin_payout",dPtr);
DEBUGLOG((" fundin_payout= [%f]\n",dPtr));
			}
			if(GetField_Double(hMerchBal,"reserved_payout",&dPtr)){
				PutField_Double(hContext,"reserved_payout",dPtr);
DEBUGLOG((" reserved_payout= [%f]\n",dPtr));
			}
			if(GetField_Double(hMerchBal,"total_float_after_payout",&dPtr)){
				PutField_Double(hContext,"total_float_after_payout",dPtr);
DEBUGLOG((" total_float_after_payout= [%f]\n",dPtr));
			}
		}
	}
	...
	FREE_ME(hPspBal);
	FREE_ME(hMerchBal);
DEBUGLOG(("CreditDepositAmount() iRet = [%d]\n",iRet));
	return iRet;
}
...
DEBUGLOG(("BOBalance:DebitDepositAmount() merchant = [%lf], psp = [%lf], res = [%lf]\n",dNetAmount,dPspAmount,dReserveAmount));
	...
	GetField_CString(hContext,"sub_txn_code",&csSubTxnCode);
DEBUGLOG(("BOBalance:DebitDepositAmount() sub_txn_code = [%s]\n",csSubTxnCode));
...
DEBUGLOG(("BOBalance:DebitDepositAmount() UpdateFloat Failed\n"));
	...
	if ((iRet == PD_OK) && (strcmp(csSubTxnCode,PD_DEPOSIT_REFUND_M)!=0)) {
		hash_t* hTxn;
		hTxn= (hash_t*) malloc (sizeof(hash_t));
		hash_init(hTxn,0);
...
DEBUGLOG(("BOBalance:ProcessFundinPayout() return_mfee = [%d]\n",iReturnMerchFee));
        }
...
DEBUGLOG(("BOBalance:ProcessFundinPayout() net_amt = [%lf]\n",dNetAmt));
			}
		}
	}	
...
DEBUGLOG(("BOBalance: ProcessFundinPayout: Void: DBMerchantBalance->GetRemainPOBalance\n"));
                			DBObjPtr = CreateObj(DBPtr,"DBMerchantBalance","GetRemainPOBalance");
					...
					if (iRet != PD_OK) {
DEBUGLOG(("BOBalance: ProcessFundinPayout: Void: GetRemainPOBalance FAILED!\n"));
ERRLOG("BOBalance: ProcessFundinPayout: Void: GetRemainPOBalance FAILED\n");
					} else {

/* merchant_net_float */
						if (GetField_Double(hContext, "merchant_net_float", &dTmp)) {
                        				if (dTmp < 0.0) {
DEBUGLOG(("BOBalance: ProcessFundinPayout: Void: merchant_net_float problem [%lf]\n", dTmp));
                                				iRet = INT_ERR;
								PutField_Int(hContext, "internal_error", iRet);
                        				}
                				}
...
DEBUGLOG(("BOBalance: ProcessFundinPayout: Void: merchant_aval_po = [%lf]\n",dOrgAvaPOBal));
                                                }

/* merchant_fundin_po */
						if (GetField_Double(hContext, "merchant_fundin_po", &dOrgFundin)) {
DEBUGLOG(("BOBalance: ProcessFundinPayout: Void: merchant_fundin_po = [%lf]\n",dOrgFundin));						
						}

/* merchant_reserved_po */
                                                if (GetField_Double(hContext, "merchant_reserved_po", &dOrgResvPO)) {
DEBUGLOG(("BOBalance: ProcessFundinPayout: Void: merchant_reserved_po = [%lf]\n",dOrgResvPO));  
                                                }

/* merchant_prev_ft */
                                                dOrgPrevFt = dOrgAvaPOBal - dOrgFundin - dOrgResvPO;
DEBUGLOG(("BOBalance: ProcessFundinPayout: Void: merchant_prev_ft = [%lf]\n",dOrgPrevFt));
...
DEBUGLOG(("BOBalance: ProcessFundinPayout: Void: insufficient fund [%s] [%s] [%s] [%lf] [%lf]\n", csMerchantId, csTxnCcy,csServiceCode, dOrgAvaPOBal, dNetAmt));
ERRLOG("BOBalance: ProcessFundinPayout: Void: insufficient fund [%s] [%s] [%s] [%lf] [%lf]\n",csMerchantId,csTxnCcy,csServiceCode,dOrgAvaPOBal, dNetAmt);
                        			}
                			}
        			
					// Check Merchant Open Balance
					if (dBalInPsp < dNetAmt) {
DEBUGLOG(("BOBalance: ProcessFundinPayout: Void: Original BalInPsp[%lf] < Debit NetAmt [%lf]\n",dBalInPsp, dNetAmt));
ERRLOG("BOBalance: ProcessFundinPayout: Void: Original BalInPsp [%lf] < Debit NetAmt [%lf]\n",dBalInPsp, dNetAmt);
                                                iRet = INT_INVALID_PAY_AMOUNT;
                                                //PutField_Int(hContext, "fn_internal_error", iRet);
                                                PutField_Int(hContext, "internal_error", iRet);
				       	}
...
DEBUGLOG(("BOBalance: ProcessFundinPayout deduct [%lf] [%lf] [%lf] total [%lf]\n", dDeductFundin, dDeductPrevFt, dDeductResvPO, dNetAmt));
DEBUGLOG(("BOBalance: ProcessFundinPayout org deduct [%lf] [%lf] [%lf] fee [%lf]\n", dOrgDeductFundin, dOrgDeductPrevFt, dOrgDeductResvPO, dFee));
...
/* void_flag */
        if (GetField_Int(hContext,"void_flag",&iVoidFlag)) {
DEBUGLOG(("BOBalance:ProcessTransferAvaPayout() void_flag  = [%d]\n",iVoidFlag));
        }
...
DEBUGLOG(("BOBalance:DebitSebBalance() bank_bal is missing!!!\n"));
	...
	if (GetField_CString(hContext,"update_user",&csUser)) {
DEBUGLOG(("BOBalance:DebitSebBalance() update_user = [%s]\n",csUser));
	}
...
DEBUGLOG(("BOBalance:DebitSebBalance:TxnSetContributors:Add Failed\n"));
	...
	if(iEmailWarn){
		sprintf(csCmd,"debit_seb_alert.sh %s %s %s %.2f %.2f",csTxnId,csBankCcy,csUser,dTotalAmt,dTotalAmt-dNetAmt);
		system(csCmd);
	}
...
int CheckInputSebRate(hash_t *hContext, const hash_t *hRequest)
{
        int     iRet = INT_ERR;
        //int     iCnt = 0;
        int     iFundinId = 0;
	...
	if(GetField_Int(hContext,"fundsin_id",&iFundinId)){
DEBUGLOG(("BOBalance:CheckInputSebRate() fundin_id = [%d]\n",iFundinId));
	}

	if(GetField_Int(hRequest,"check_oanda_rate_diff",&iCheckOandaRate)){
DEBUGLOG(("BOBalance:CheckInputSebRate() check_oanda_rate_diff = [%d]\n",iCheckOandaRate));
	}

	if(GetField_Int(hRequest,"skip_tol_check",&iSkipTolCheck)){
DEBUGLOG(("BOBalance:CheckInputSebRate() skip_tol_check = [%d]\n",iSkipTolCheck));
	}
	
	DBObjPtr = CreateObj(DBPtr,"DBFundsIn","GetApprovedFundsInById");
	if ((unsigned long)(*DBObjPtr)(iFundinId,hTxn) == PD_FOUND) {
		if(GetField_CString(hTxn,"fundin_ccy",&csFundinCcy)){
DEBUGLOG(("GetApprovedFundsInById() fundin_ccy = [%s]\n",csFundinCcy));
		}

		if(GetField_CString(hTxn,"bank_ccy",&csBankCcy)){
DEBUGLOG(("GetApprovedFundsInById() bank_ccy = [%s]\n",csBankCcy));
		}

		if(GetField_Double(hTxn,"fundin_amt",&dFundinAmt)){
DEBUGLOG(("GetApprovedFundsInById() fundin_amt = [%lf]\n",dFundinAmt));
		}

		if(GetField_Double(hTxn,"bank_amt",&dBankAmt)){
...
DEBUGLOG(("Check Input rate with Oanda Rate and Org Rate\n"));
...
DEBUGLOG(("Default Input Rate & Oanda Rate difference threshold = [%lf]\n",dDefOandaThreshold));
		}
		if ((unsigned long)(DBObjPtr)(PD_AVFXRATE_ALLOW_DIFF,csValueTmp) == FOUND) {
			sscanf(csValueTmp, "%lf",&dDefThreshold);
DEBUGLOG(("Default Input Rate & Org Rate difference threshold = [%lf]\n",dDefThreshold));
		}
		FREE_ME(csValueTmp);
...
DEBUGLOG(("OandaThreshold [%lf], dThreshold [%lf]\n",dOandaThreshold,dThreshold));
		...
		//if((unsigned long)(*DBObjPtr)(csFundinCcy,csBankCcy,hOanda)==PD_OK){
		if((unsigned long)(*DBObjPtr)(csBankCcy,csFundinCcy,hOanda)==PD_OK){
			if(GetField_Double(hOanda,"rate",&dOandaRate)){
				dDiff = 0.0;
				//dRate = newround((dBankAmt/dFundinAmt),PD_ROUND_UP_DEC);
				dRate = newround((dFundinAmt/dBankAmt),PD_ROUND_UP_DEC);
				if(dRate>dOandaRate){
...
ERRLOG("BOBalance:CheckInputSebRate()Input rate & Oanda Rate with difference [%lf\%] > [%lf\%]!!!!\n",dDiff*100,dOandaThreshold);
						iRet = INT_LARGE_DIFF_OANDA_NEW_RATE;
					}
					else{
DEBUGLOG(("Input rate & Oanda Rate with difference [%lf\%] > [%lf\%]. Ignore Checking\n",dDiff*100,dOandaThreshold));
						PutField_Double(hContext,"input_diff",dDiff*100);
						PutField_Double(hContext,"oanda_tol",dOandaThreshold);
						PutField_Int(hContext,"result",INT_LARGE_DIFF_OANDA_NEW_RATE);
						iRet = PD_SKIP_OK;
					}
				}
			}
		...
		if(iRet==PD_OK){
			...
			hash_init(hOrg,0);
			DBObjPtr = CreateObj(DBPtr,"DBSebBalance","FindBalanceByCcy");
			if ((unsigned long)(*DBObjPtr)(csFundinCcy,csBankCcy,
						       hOrg) == PD_FOUND) {
...
ERRLOG("BOBalance:CheckInputSebRate()Input rate & Org Rate with difference [%lf\%] > [%lf\%]!!!!\n",dDiff*100,dThreshold);
							iRet = INT_LARGE_DIFF_NEW_RATE;
						}
						else{
DEBUGLOG(("Input rate & Org Rate with difference [%lf\%] > [%lf\%]. Ignore Checking\n",dDiff*100,dThreshold));
							PutField_Double(hContext,"input_diff",dDiff*100);
							PutField_Double(hContext,"org_tol",dThreshold);
							PutField_Int(hContext,"result",INT_LARGE_DIFF_NEW_RATE);
						}
					}
				}
				else{
	...
	FREE_ME(hTxn);
DEBUGLOG(("BOBalance:CheckInputSebRate() iRet = [%d]\n",iRet));
	return iRet;
}
