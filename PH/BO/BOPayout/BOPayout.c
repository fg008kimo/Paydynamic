...
enhancement for POA                                2014/10/30              LokMan Chow
free memory bug fix				   2015/04/29		   LokMan Chow
Amend CreatePayoutFile and WritePayoutFile	   2016/10/06		   Elvis Wong
Add new format "NTP"				   2017/10/04		   Dirk Wong 
*/
...
#define PD_TD_NUM_STYLE "<td class=\"numstyle\">"
#define PD_TR_END       "</tr>"
#define PD_TD_END       "</td>"
...
#define MOBILE_PREFIX_A  "13" 
#define MOBILE_PREFIX_B  "15" 
#define CN_MOBILE_NO_LEN 11 
...
DEBUGLOG(("BOPayout:: online_payout = [%d]\n",iOnlinePayout));
		}
	}
			  ...
			  hash_t* hResponse)
{
	...
	hash_t  *hRec, *hTxn, *hDetail, *hUploadTxn, *hElement, *hBal;
	hTxn= (hash_t*) malloc (sizeof(hash_t));
	hash_init(hTxn,0);
	hDetail= (hash_t*) malloc (sizeof(hash_t));
	hash_init(hDetail,0);
...
DEBUGLOG(("GetExistingPsp txn_count [%d]\n",iTmp));
					...
					if(GetField_Int(hRec,"txn_split_count",&iTmp)){
						PutField_Int(hTxn,"split_cnt",iTmp);
DEBUGLOG(("GetExistingPsp txn_split_count [%d]\n",iTmp));
					}

					if(GetField_Double(hRec,"total_amt",&dAmt)){
						PutField_Double(hTxn,"total_amt",dAmt);
...
ERRLOG("BOPayout:handle_PayoutGenerate:GetAvalPspBalanceForPO aval psp balance[%.2f] less than total generate amount[%lf]!!!!\n",dAvalBal,dAmt);
									break;
								}
							}
						}
							...
							iRet = INT_ERR;
DEBUGLOG(("GetAvalPspBalanceForPO find aval psp balance failed!!!!\n"));
ERRLOG("BOPayout:handle_PayoutGenerate:GetAvalPspBalanceForPO find aval psp balance failed!!!!\n");
							break;
						}
					}
					else{
						iRet = INT_ERR;
DEBUGLOG(("GetAvalPspBalance find balance failed!!!!\n"));
ERRLOG("BOPayout:handle_PayoutGenerate:GetAvalPspBalance find balance failed!!!!\n");
						break;
					}
...
DEBUGLOG(("CreatePayoutFileDetail for[%s] Failed!!!!\n",csPspId));
					...
					DBObjPtr = CreateObj(DBPtr,"DBPayoutGeneratedFileHD","Add");
					if((unsigned long)(*DBObjPtr)(hTxn)!=PD_OK){
DEBUGLOG(("handle_PayoutGenerate::PayoutGeneratedFileHD::Add Error!!!\n"));
						iRet = INT_ERR;
						PutField_Int(hContext,"internal_error",iRet);
						break;
					}
...
DEBUGLOG(("handle_PayoutGenerate::No pre-gen record found!!!\n"));
	...
	if(iRet == PD_OK){
		int i, iTxnCnt;
		recordset_init(rRecordSet,0);
DEBUGLOG(("handle_PayoutGenerate::Call DBMerchantUploadFileDetail:GetPregeneratedRecords\n"));
		...
		if((unsigned long)(*DBObjPtr)(iGenId,cBatchMode,rRecordSet)==PD_OK){
			...
			while (hRec && (iRet==PD_OK)) {
				...
				if (GetField_CString(hRec, "psp_id", &csPspId)) {
					PutField_CString(hDetail,"psp_id",csPspId);
					PutField_CString(hTxn,"psp_id",csPspId);
...
DEBUGLOG(("GetPregeneratedRecords::psp_id = [%s]\n",csPspId));
					...
					sprintf(csTag, "%s_psp_channel", csPspId);
					GetField_CString(hContext,csTag,&csPspChannel);
...
DEBUGLOG(("handle_PayoutGenerate:: dTmpAmt: [%lf]\n",dTmpAmt));
					for(iTmp=0;iTmp<(iSplitCnt-1);iTmp++){
						dLeftAmt = dLeftAmt - dTmpAmt;
					}
DEBUGLOG(("handle_PayoutGenerate:: dLeftAmt: [%lf]\n",dLeftAmt));
...
DEBUGLOG(("handle_PayoutGenerate:: GeneratePayoutSeq: [%d]=[%s]\n",i,csPayoutTxnSeq));
					...
					if ((unsigned long)(*DBObjPtr)(hDetail) != PD_OK) {
						iRet = INT_ERR;
DEBUGLOG(("handle_PayoutGenerate:: DBPayoutGeneratedFileDT:Add Failed\n"));
ERRLOG("handle_PayoutGenerate:: DBPayoutGeneratedFileDT:Add Failed\n");
						break;
					}
					...
					PutField_Int(hDetail,csTag,iTxnCnt+1);
...
DEBUGLOG(("handle_PayoutGenerate::DBTransaction:Update Failed\n"));
			...
			if(iRet == PD_OK && i>0 && !iOnlinePayout){
				...
				PutField_CString(hTxn,"psp_id",csPspId);
				PutField_CString(hTxn,"psp_channel",csPspChannel);
...
ERRLOG("BOPayout::handle_PayoutGenerate:PayoutGeneratedFileHD::Update Error!!!\n");
	...
	if(iRet == PD_OK && iOnlinePayout){
		recordset_init(rRecordSet,0);
DEBUGLOG(("handle_PayoutGenerate::Call DBMerchantUploadFileDetail:GetPregenSummary\n"));
		DBObjPtr = CreateObj(DBPtr,"DBMerchantUploadFileDetail","GetPregenSummary");
		if((unsigned long)(*DBObjPtr)(iGenId,rRecordSet)==PD_OK){
			hRec = RecordSet_GetFirst(rRecordSet);
			while (hRec && (iRet==PD_OK)) {
				hash_init(hTxn,0);
...
DEBUGLOG(("GetPayoutRecords::Call DBPayoutApproveTmp:GetDetailBySelectedTxn\n"));
		DBObjPtr = CreateObj(DBPtr,"DBPayoutApproveTmp","GetDetailBySelectedTxn");
	}
	else{
...
DEBUGLOG(("BOPayout:ProcessOnlinePayout()\n"));
	...
	GetField_Int(hContext, "total_cnt", &iCnt);//total batch count
DEBUGLOG(("BOPayout:ProcessOnlinePayout() total batch count = [%d]\n",iCnt));
	for (i = 0; i < iCnt; i++) {
		csNewList[0] ='\0';
		sprintf(csTag, "batch_id_%d",i);
		if(GetField_CString(hContext,csTag,&csBatchId)){
			...
			if ((*DBObjPtr)((unsigned long)lBatchId,rRecordSet) == PD_OK) {
				...
				while (hRec) {
					if(GetField_CString(hRec,"service_code",&csServiceCode)){
DEBUGLOG(("BOPayout:ProcessOnlinePayout() Find PSP by [%s][%s]\n",csMerchantId,csServiceCode));
...
DEBUGLOG(("BOPayout:ProcessOnlinePayout() PSP Found[%d][%s]\n",iPspCnt,csPspTmp));
									iPspCnt++;
								}
...
DEBUGLOG(("ProcessOnlinePayout::Call DBMerchantUploadFileDetail:GetRecordByRandomWithPara[%s][%s][%s]\n",csMerchantId,csPspId,csTmp));
			DBObjPtr = CreateObj(DBPtr,"DBMerchantUploadFileDetail","GetRecordByRandomWithPara");
			iResult = (unsigned long)(*DBObjPtr)(hTxn,rDetailSet);
			if(iResult == PD_OK){
				hDtl = RecordSet_GetFirst(rDetailSet);
...
DEBUGLOG(("GetRecordByRandomWithPara::payout_amount = [%lf]\n",dTxnAmt));
					...
					if(iAccept){
						...
						DBObjPtr = CreateObj(DBPtr,"DBPayoutGeneratedTmp","Add");
						if((unsigned long)(*DBObjPtr)(hGenTmp)!=PD_OK){
DEBUGLOG(("ProcessOnlinePayout::DBPayoutGeneratedTmp::Add Error!!!\n"));
ERRLOG("BOPayout:ProcessOnlinePayout::DBPayoutGeneratedTmp::Add Error!!!\n");
							iChk= PD_ERR;
						}
					}
					hDtl = RecordSet_GetNext(rDetailSet);
	...
	FREE_ME(hTxn);
	FREE_ME(hPsp);
DEBUGLOG(("BOPayout:ProcessOnlinePayout() iRet=[%d]\n",iRet));
	return iRet;
}
...
DEBUGLOG(("CreatePayoutFileDetail::psp_merchant_id [%s]!!\n",csTmp));
		}
	}
	FREE_ME(hPspDetail);
...
int CreatePayoutFile(hash_t* hContext)
{
	...
	char    *csPspId;
	char    *csPspChannel;
	char    csPspFormatCode[PD_PSP_PO_FILE_FMT_CODE_LEN +1];
	char    csTag[PD_TAG_LEN +1];
...
DEBUGLOG(("BOPayout:CreatePayoutFile() writ to file[%s]\n",csPayOutFile));
	...
	// Get Psp Payout File Format Code
	if (iRet==PD_OK) {
DEBUGLOG(("BOPayout:CreatePayoutFile() Call PspPayoutFileFormatMapping:GetPspPayoutFileFormat\n"));
       		DBObjPtr = CreateObj(DBPtr,"DBPspPayoutFileFormatMapping","GetPspPayoutFileFormat");
       		iDtlRet = (unsigned long) (*DBObjPtr)(csPspId,csPspFormatCode); 
		if (iDtlRet == FOUND){
DEBUGLOG(("BOPayout:CreatePayoutFile() Call PspPayoutFileFormatMapping:GetPspPayoutFileFormat: format_code[%s]\n", csPspFormatCode));

			if ((strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_ECPSS))
                    	    && (strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_YEEPAY))
                    	    && (strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_EPLUTUS))
                    	    && (strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_UNIPAYGO))
                    	    && (strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_UNIPAYGO_NH))
                    	    && (strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_TENPAY))
                  	    && (strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_TWV))
                  	    && (strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_DEFAULT))
                  	    && (strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_NTP))
                	)
			{
                        	sprintf(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_DEFAULT);
DEBUGLOG(("BOPayout:CreatePayoutFile() Call PspPayoutFileFormatMapping:GetPspPayoutFileFormat: format_code invalid, changes to default [%s]\n",csPspFormatCode));
                	}
		} else if (iDtlRet == NOT_FOUND){
			sprintf(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_DEFAULT);
DEBUGLOG(("BOPayout:CreatePayoutFile() Call PspPayoutFileFormatMapping:GetPspPayoutFileFormat: format_code not found, changes to default [%s]\n",csPspFormatCode));		
		} else {
DEBUGLOG(("BOPayout:CreatePayoutFile() Call PspPayoutFileFormatMapping:GetPspPayoutFileFormat Failure!!!\n"));
ERRLOG("BOPayout:CreatePayoutFile() Call PspPayoutFileFormatMapping:GetPspPayoutFileFormat Failure!!!\n");
			iRet=INT_ERR;
		}
	}
...
ERRLOG("BOPayout:CreatePayoutFile::Open Payoutfile problem!!\n");
	...
	if(iRet==PD_OK && !strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_ECPSS)) {
		...
		fprintf(fp, "%s%s批次号%s%s订单号%s%s帐户名称%s%s银行帐号%s%s开户银行%s%s省%s%s市%s%s金额%s%s打款原因%s%s支行讯息%s%s手机号%s%s\n", PD_TR, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TR_END);

	}
	else if(iRet==PD_OK && !strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_EPLUTUS)) { 
		GetField_Int(hContext,"split_cnt",&iCnt);
                GetField_Double(hContext,"total_amt",&dAmt);

		fprintf(fp, "<html><body><table>\n");
		fprintf(fp, "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\"/>\n");
		fprintf(fp, "<style type=\"text/css\"> .format{ mso-number-format:'\\@';} </style>\n");
		fprintf(fp, "%s%s总笔数%s%s%d%s%s总金额%s%s%.2f%s%s\n",PD_TR,PD_TD,PD_TD_END,PD_TD,iCnt,PD_TD_END,PD_TD,PD_TD_END,PD_TD,dAmt,PD_TD_END,PD_TR_END);
		fprintf(fp, "%s%s商户编号%s%s下发姓名	%s%s身份证号%s%s手机号%s%s银行%s%s分行支行%s%s银行帐号%s%s下发金额 (CNY)%s%s省份%s%s城市%s%s备注%s%s\n",PD_TR,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TR_END);
	}
	else if(iRet==PD_OK && !strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_UNIPAYGO)) {
		fprintf(fp, "<html><body><table>\n");
		fprintf(fp, "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\"/>\n");
		fprintf(fp, "<style type=\"text/css\"> .format{ mso-number-format:'\\@';} .numstyle{mso-number-format:'0\\.00';} </style>\n");
		fprintf(fp,"%s%s发卡行%s%s银行代码%s%s卡号%s%s开户名%s%s金额（元）%s%s收款人手机号码%s%s备注%s%s省份%s%s城市%s%s分行支行%s%s\n",PD_TR,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TR_END);
	}
	else if(iRet==PD_OK && !strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_UNIPAYGO_NH)) {
		fprintf(fp, "<html><body><table>\n");
		fprintf(fp, "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\"/>\n");
		fprintf(fp, "<style type=\"text/css\"> .format{ mso-number-format:'\\@';} </style>\n");	
		fprintf(fp, "%s%sRow No%s%sBank%s%sProvince%s%sCity%s%sBranch%s%sCust Name%s%sCard No%s%sAmount%s%sTel No%s%sRemark%s%sRefCode%s%sChannel_Id%s%s\n", PD_TR, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TR_END);
	}
	else if(iRet==PD_OK && !strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_TENPAY)) {
		GetField_Int(hContext,"split_cnt",&iCnt);
                GetField_Double(hContext,"total_amt",&dAmt);
                GetField_CString(hContext,"psp_merchant_id",&csPMID);

                fprintf(fp, "<html><body><table>\n");
                fprintf(fp, "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\"/>\n");
                fprintf(fp, "<style type=\"text/css\"> .format{ mso-number-format:'\\@';} </style>\n");
		fprintf(fp, "%s%s商户号%s%s批次序列号%s%s总笔数%s%s总金额(元)%s%s付款说明%s%s\n",PD_TR,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TR_END);
		fprintf(fp, "%s%s%s%s%s%s%s%s%d%s%s%.2f%s%s劳务费%s%s\n",PD_TR,PD_TD_STYLE,csPMID,PD_TD_END,PD_TD_STYLE,getdatetime(),PD_TD_END,PD_TD,iCnt,PD_TD_END,PD_TD,dAmt,PD_TD_END,PD_TD,PD_TD_END,PD_TR_END);
		fprintf(fp, "%s%s单笔序号%s%s收款方银行账号%s%s银行类型%s%s真实姓名%s%s付款金额(元)%s%s账户类型%s%s开户地区%s%s开户城市%s%s支行名称%s%s付款说明%s%s收款人手机号%s%s卡类型%s%s商户单号%s%s商户备注%s%s备用%s%s\n",PD_TR,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TR_END);
	}
	else if(iRet==PD_OK && !strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_TWV)) {
		if(GetField_Int(hContext,"split_cnt",&iCnt)){
			fprintf(fp,"%d\n",iCnt);
		}
	}
	else if(iRet==PD_OK && !strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_DEFAULT)) {
		fprintf(fp, "<html><body><table>\n");
                fprintf(fp, "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\"/>\n");
                fprintf(fp, "<style type=\"text/css\"> .format{ mso-number-format:'\\@';} </style>\n");
	...
	else if(iRet==PD_OK && !strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_NTP)) {
		GetField_Int(hContext,"split_cnt",&iCnt);
                GetField_Double(hContext,"total_amt",&dAmt);
                GetField_CString(hContext,"psp_merchant_id",&csPMID);
                ...
                fprintf(fp, "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\"/>\n");
                fprintf(fp, "<style type=\"text/css\"> .format{ mso-number-format:'\\@';} </style>\n");
		fprintf(fp, "%s%s請輸入商戶名稱：%s%s%s%s%s%s%s%s%s%s%s日期%s%s%s%s%s總金額%s%s%.2f%s%s\n",
				PD_TR,
				  PD_TD,PD_TD_END,
				  PD_TD,PD_TD_END,
				  PD_TD_STYLE,csPMID,PD_TD_END,
				  PD_TD,PD_TD_END,
				  PD_TD,PD_TD_END,
				  PD_TD,PD_TD_END,
				  PD_TD_STYLE,csPHDate,PD_TD_END,
				  PD_TD,PD_TD_END,
				  PD_TD,dAmt,PD_TD_END,
				PD_TR_END);
		fprintf(fp, "%s%s序號%s%s帳戶姓名%s%s開戶銀行%s%s省分%s%s城市（地區）%s%s支行%s%s帳號%s%s金額%s%s備註%s%s\n",
				PD_TR,
				  PD_TD,PD_TD_END,
				  PD_TD,PD_TD_END,
				  PD_TD,PD_TD_END,
...
DEBUGLOG(("WritePayoutFile::psp_channel [%s]\n", csPspChannel));
	...
	// Get Psp Payout File Format Code
	if (iRet==PD_OK) {
DEBUGLOG(("WritePayoutFile() Call PspPayoutFileFormatMapping:GetPspPayoutFileFormat\n"));
                DBObjPtr = CreateObj(DBPtr,"DBPspPayoutFileFormatMapping","GetPspPayoutFileFormat");
                iDtlRet = (unsigned long) (*DBObjPtr)(csPspId,csPspFormatCode);
                if (iDtlRet == FOUND){
DEBUGLOG(("WritePayoutFile() Call PspPayoutFileFormatMapping:GetPspPayoutFileFormat: format_code[%s]\n", csPspFormatCode));
                        ...
                        {
                                sprintf(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_DEFAULT);
DEBUGLOG(("WritePayoutFile() Call PspPayoutFileFormatMapping:GetPspPayoutFileFormat: format_code invalid, changes to default [%s]\n",csPspFormatCode));
                        }
                } else if (iDtlRet == NOT_FOUND){
                        sprintf(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_DEFAULT);
DEBUGLOG(("WritePayoutFile() Call PspPayoutFileFormatMapping:GetPspPayoutFileFormat: format_code not found, changes to default [%s]\n",csPspFormatCode));
                } else {
DEBUGLOG(("WritePayoutFile() Call PspPayoutFileFormatMapping:GetPspPayoutFileFormat Failure!!!\n"));
ERRLOG("BOPayout:WritePayoutFile() Call PspPayoutFileFormatMapping:GetPspPayoutFileFormat Failure!!!\n");
                        iRet=INT_ERR;
                }
        }	
	...
	if(iRet==PD_OK) {
		if(GetField_Int(hRec,"end_file",&iTmp)){
			if(iTmp==PD_TRUE){
DEBUGLOG(("WritePayoutFile:end_file\n"));
				if(!strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_TWV)){
				}
				else if(!strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_UNIPAYGO)){
					fprintf(fp,"%s%sEND%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s\n",PD_TR,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TD,PD_TD_END,PD_TR_END);
	...
	if(iRet==PD_OK){
DEBUGLOG(("WritePayoutFile:write_file\n"));
		if(!strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_TWV)){
...
DEBUGLOG(("WritePayoutFile:payout_amount [%d]\n", (int)dTmp));
		...
		else if(!strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_YEEPAY)){
...
DEBUGLOG(("WritePayoutFile:payout_amount [%f]\n", dTmp));
			...
			fprintf(fp, "%s劳务费%s",PD_TD,PD_TD_END);	
...
DEBUGLOG(("WritePayoutFile:phone_num [%s]\n", csTmp));
		...
		else if(!strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_EPLUTUS)){
...
DEBUGLOG(("WritePayoutFile:city [%s]\n", csTmp));
		...
		else if(!strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_ECPSS)){
			...
			fprintf(fp, "%s", PD_TR);
                        ...
                        char    csTag[PD_TAG_LEN+1];
                        if (GetField_CString(hRec, "psp_id", &csTmp)) {
                                sprintf(csTag,"%s_txn_cnt",csTmp);
                                GetField_Int(hRec,csTag,&iTxnCnt);
                        }
DEBUGLOG(("WritePayoutFile:seq [%d]\n",iTxnCnt));
                        fprintf(fp, "%s%d%s",PD_TD,iTxnCnt,PD_TD_END);
...
DEBUGLOG(("WritePayoutFile:txn_id [%s]\n", csTmp));
			...
			//bank_name,bank_code (follow psp)
			if (!GetField_CString(hRec, "bank_name", &csTmp)) {
                                csTmp = strdup("");
                                fprintf(fp, "%s%s%s",PD_TD,csTmp,PD_TD_END);
                                fprintf(fp, "%s%s%s",PD_TD,csTmp,PD_TD_END);
                                ...
                                csBankName[0]='\0';
                                DBObjPtr = CreateObj(DBPtr,"DBBankNameMapping","BankNameNCodeConvert");
                                if ((unsigned long)(*DBObjPtr)(csPspChannel,csTmp,csBankName,csBankCode)==FOUND) {
DEBUGLOG(("WritePayoutFile:bank_name[%s] bank_code[%s] (converted)\n", csBankName,csBankCode));
                                }
                                else{
                                        strcpy(csBankName,csTmp);
                                        csBankName[strlen(csTmp)]='\0';
...
DEBUGLOG(("WritePayoutFile:bank_name [%s] (same as input)\n", csBankName));
                                ...
                                fprintf(fp, "%s%s%s",PD_TD,csBankName,PD_TD_END);
                                fprintf(fp, "%s%s%s",PD_TD,csBankCode,PD_TD_END);
                        }
...
DEBUGLOG(("WritePayoutFile:payout_amount [%f]\n", dTmp));
                        fprintf(fp, "%s%.2f%s",PD_TD_NUM_STYLE,dTmp,PD_TD_END);

                        //mobile no.
                        char    csMobileNo[CN_MOBILE_NO_LEN+1];
			struct timeval  tv;
                        gettimeofday(&tv, NULL);
                        unsigned int time_msec = (tv.tv_sec)*1000 + (tv.tv_usec)/1000;
                        srand(time_msec);

                        int iRand = (int)(rand()%9999999)+100000000;
                        if(iRand%2)
                                sprintf(csMobileNo,"%s%d",MOBILE_PREFIX_A,iRand);
                        else
                                sprintf(csMobileNo,"%s%d",MOBILE_PREFIX_B,iRand);
DEBUGLOG(("WritePayoutFile:phone_num [%s]\n", csMobileNo));
                        fprintf(fp, "%s%s%s",PD_TD,csMobileNo,PD_TD_END);
                        //fprintf(fp, "%s%s",PD_TD,PD_TD_END);
...
DEBUGLOG(("WritePayoutFile:put txn_id to remark [%s]\n", csTmp));
			fprintf(fp, "%s%s%s",PD_TD_STYLE,csTmp,PD_TD_END);
...
DEBUGLOG(("WritePayoutFile:branch [%s]\n", csTmp));
		...
		else if(!strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_UNIPAYGO_NH)){
			fprintf(fp, "%s", PD_TR);
...
DEBUGLOG(("WritePayoutFile:seq_num [%d]\n", iTxnCnt));
                        fprintf(fp, "%s%d%s",PD_TD,iTxnCnt,PD_TD_END);
...
DEBUGLOG(("WritePayoutFile:txn_id [%s]\n", csTmp));
		...
		else if(!strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_TENPAY)){
			fprintf(fp, "%s", PD_TR);
...
DEBUGLOG(("WritePayoutFile:payout_amount [%f]\n", dTmp));
                        ...
                        fprintf(fp, "%s个人%s",PD_TD,PD_TD_END);
...
DEBUGLOG(("WritePayoutFile:phone_num [%s]\n", csMobileNo));
                        ...
                        fprintf(fp, "%s借记卡%s",PD_TD,PD_TD_END);
...
DEBUGLOG(("WritePayoutFile:txn_id [%s]\n", csTmp));
		...
		else if(!strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_DEFAULT)){
	//		fprintf(fp, "%s%sPayout Trans No.%s%sReceiver Name%s%sID Card No.%s%sMobile No.%s%sBank%s%sBranch%s%sBank Account%s%sAmount%s%sProvince%s%sCity%s%sRemarks%s%s\n", PD_TR, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TD, PD_TD_END, PD_TR_END);
...
DEBUGLOG(("WritePayoutFile:upload_txn_id [%s]\n", csTmp));
		...
		else if(!strcmp(csPspFormatCode,PD_PSP_PO_FILE_FMT_CODE_NTP)){
			fprintf(fp, "%s", PD_TR);
			 ...
			 hash_t* hResponse)
{
	...
	char	csTag[PD_TAG_LEN +1];
	char	cInputInd;
	char    csTxnList[PD_TMP_MSG_BUF_LEN+1];
...
DEBUGLOG(("HandleCancelGenerate()\n"));
	...
	if(GetField_Char(hContext,"input_ind",&cInputInd)){
DEBUGLOG(("HandleCancelGenerate() input_ind = [%c]\n",cInputInd));
	}
...
DEBUGLOG(("HandleCancelGenerate() file_id = [%s]\n",csTmp));
		...
		for(i=0; i<iCnt; i++){
			sprintf(csTag,"org_txn_id_%d",i);
			if(GetField_CString(hContext,csTag,&csTmp)){
				if(i>0) strcat(csTxnList,",");
				strcat(csTxnList,csTmp);
				csTxnList[strlen(csTxnList)]= '\0';
				PutField_CString(hTxn,"txn_list",csTxnList);
			}
		}
	}
...
DEBUGLOG(("GetDetail:: call MatchRespTxnStatus\n"));
				DBObjPtr = CreateObj(DBPtr,"DBTransaction","MatchRespTxnStatus");
				if ((unsigned long)(DBObjPtr)(csOrgTxnId,PD_COMPLETE,PD_ACCEPT) != PD_FOUND) {
DEBUGLOG(("GetDetail:: MatchRespTxnStatus[%s] cannot be cancelled, txn status not valid\n",csOrgTxnId));
					continue;
				}

			}
			if (GetField_CString(hRec, "upload_txn_seq", &csUploadTxnId)) {
				//sprintf(csTag, "upload_txn_id_%d", i);
				PutField_CString(hUploadTxn,"txn_seq",csUploadTxnId);
...
DEBUGLOG(("GetDetail::txn_country = [%s]\n",csTmp));
			...
			if (GetField_CString(hRec, "payout_ccy", &csTmp)) {
				...
				PutField_CString(hRequest,csTag,csTmp);
				//sprintf(csTag,"dst_txn_ccy_%d",i);
				sprintf(csTag,"org_psp_txn_ccy");
				PutField_CString(hContext,csTag,csTmp);
			}
			if(GetField_Double(hRec,"payout_amount",&dTmp)){
				//sprintf(csTag, "gen_payout_amt_%d", i);
				//PutField_Double(hRequest,csTag,dTmp);
				sprintf(csTag, "org_dst_net_amt");
				PutField_Double(hContext,csTag,dTmp);
DEBUGLOG(("GetDetail::payout_amount = [%lf]\n",dTmp));
			...
			if (GetField_CString(hRec, "client_id", &csTmp)) {
				PutField_CString(hRequest,"client_id",csTmp);
DEBUGLOG(("GetDetail::client id = [%s]\n",csTmp));
			}
			if (GetField_Double(hRec, "psp_cost", &dTmp)) {
				//sprintf(csTag,"precal_fee_%d",i);
				//PutField_Double(hRequest,csTag,dTmp);
...
ERRLOG("BOPayout::GetDetail:DBPayoutGeneratedFileDT Update Failed!!!\n");
			...
			if(iRet==PD_OK && cInputInd==PD_BY_GEN_TXN){
				DBObjPtr = CreateObj(DBPtr,"DBMerchantUploadFileDetail","MatchRespTxn_ForUpdate");
				if((unsigned long)(*DBObjPtr)(csUploadTxnId,PAYOUT_MASTER_TRANSACTION_GENERATED) == PD_FOUND){
					DBObjPtr = CreateObj(DBPtr,"DBMerchantUploadFileDetail","ResumeGeneratedTxn");
					iRet = (unsigned long)(*DBObjPtr)(csUploadTxnId,csUser);
				}
			}
			...
			if(iRet==PD_OK){
				...
				PutField_Char(hContext,"party_type",PD_TYPE_PSP);
DEBUGLOG(("HandleCancelGenerate::Call BOBalance:UpdatePayoutAmount\n"));
				BOObjPtr = CreateObj(BOPtr,"BOBalance","UpdatePayoutAmount");
				if((unsigned long)(*BOObjPtr)(hContext)!=PD_OK){
DEBUGLOG(("HandleCancelGenerate::BOBalance:UpdatePayoutAmount Failed\n"));
					iRet = INT_ERR;
				}
				else{
					iRet = UpdateTxnLog(hContext);
					if(iRet!=PD_OK){
DEBUGLOG(("HandleCancelGenerate::BOBalance:UpdateTxnLog Failed\n"));
						break;
					}
					else{
						TxnCommit();
		...
		if(i>0 && cInputInd==PD_BY_FILE && iRet==PD_OK){
...
//check Bank Account Name
	if(iRet==PD_OK){
		if(strncmp(csList[IDX_DETAIL_COUNTRY],"CN",2)==0){
			int iCnt = 0;
			int j = 0;
...
DEBUGLOG(("ProcessPayoutRecords::Call BOBalance:GetAvalPayoutBalance\n"));
				BOObjPtr = CreateObj(BOPtr,"BOBalance","GetAvalPayoutBalance");
...
ERRLOG("BOPayout::ProcessPayoutRecords::GetAvalPayoutBalance Error!!\n");
					iRet = INT_ERR;
				}
			}
