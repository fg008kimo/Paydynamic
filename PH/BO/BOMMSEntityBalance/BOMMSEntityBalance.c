/*
Partnerdelight (c)2015. All rights reserved. No part of this software may be reproduced in any form without written permission
of an authorized representative of Partnerdelight.

Change Description                                 Change Date             Change By
-------------------------------                    ------------            --------------
Init Version                                       2015/06/15              Cody Chan
Modify DebitIntransit and			   2015/07/06		   Cody Chan
Remove from_ccy
Add UpdateNatureBalance
Add over txn element for acct_bal		   2015/07/20		   Cody Chan
Add override txn amt to				   2015/07/21		   Cody Chan
updateEntityNatureBalance
Remove Country					   2015/07/22		   Cody Chan
Break Nature Txn Element			   2015/08/04		   Cody Chan
Add SelectNatureBalanceForUpdate		   2015/08/18		   Dirk Wong
*/

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "common.h"
#include "utilitys.h"
#include "ObjPtr.h"
#include "myhash.h"
#include "myrecordset.h"
#include "internal.h"
#include "BOMMSEntityBalance.h"

char    cDebug;

OBJPTR(DB);
OBJPTR(BO);

int UpdateEntityNatureBalance(hash_t* hContext, const hash_t* hReq);

void BOMMSEntityBalance(char    cdebug)
{
        cDebug = cdebug;
}


int SelectEntityBalanceForUpdate(hash_t *hContext, const hash_t* hReq) 
{
	int	iRet = PD_OK;
	double	dTmp;
	char	*csPtr;

	hash_t	*hData,*hRsp;

DEBUGLOG(("SelectEntityBalanceForUpdate()\n"));
	hData = (hash_t*) malloc (sizeof(hash_t));
	hash_init(hData,0);
	hRsp = (hash_t*) malloc (sizeof(hash_t));
	hash_init(hRsp,0);

/* entity id */
	if (GetField_CString(hContext,"entity_id",&csPtr)) {
DEBUGLOG(("SelectEntityBalanceForUpdate() Entity ID = [%s]\n",csPtr));
		PutField_CString(hData,"entity_id",csPtr);
	}
	else {
DEBUGLOG(("SelectEntityBalanceForUpdate() Entity ID not found\n"));
ERRLOG("BOMMSEntityBalance::SelectEntityBalanceForUpdate() Entity ID not found\n");
		iRet = PD_ERR;
	}


/* ccy */
	if (iRet == PD_OK ) {
		if (GetField_CString(hReq,"ccy",&csPtr)) {
DEBUGLOG(("SelectEntityBalanceForUpdate() ccy = [%s]\n",csPtr));
			PutField_CString(hData,"ccy",csPtr);
		}
		else {
DEBUGLOG(("SelectEntityBalanceForUpdate() ccy to found\n"));
ERRLOG("BOMMSEntityBalance::SelectEntityBalanceForUpdate() ccy not found\n");
			iRet = PD_ERR;
		}
	}

/* create entity balance for opening balance */
	if (iRet == PD_OK) {
		DBObjPtr = CreateObj(DBPtr,"DBMmsEntityBalance","GetEntityBalanceForUpdate");
		iRet = (unsigned long)((DBObjPtr)(hData,hRsp));

		if (iRet == PD_OK) {
/*opening_acct_bal */
			if (GetField_Double(hRsp,"acct_bal",&dTmp)) {
DEBUGLOG(("SelectEntityBalanceForUpdate() open_acct_bal = [%lf]\n",dTmp));
				PutField_Double(hContext,"open_acct_bal",dTmp);
			}

/*opening_prepaid */
			if (GetField_Double(hRsp,"prepaid",&dTmp)) {
DEBUGLOG(("SelectEntityBalanceForUpdate() open_prepaid = [%lf]\n",dTmp));
				PutField_Double(hContext,"open_prepaid",dTmp);
			}

/*opening_intransit */
			if (GetField_Double(hRsp,"intransit",&dTmp)) {
DEBUGLOG(("SelectEntityBalanceForUpdate() open_intransit = [%lf]\n",dTmp));
				PutField_Double(hContext,"open_intransit",dTmp);
			}
/*opening_lien */
			if (GetField_Double(hRsp,"lien",&dTmp)) {
DEBUGLOG(("SelectEntityBalanceForUpdate() open_lien = [%lf]\n",dTmp));
				PutField_Double(hContext,"open_lien",dTmp);
			}


		}
	}

	FREE_ME(hData);
	FREE_ME(hRsp);
DEBUGLOG(("SelectEntityBalanceForUpdate:: () Noraml Exit iRet = [%d]\n",iRet));
	return iRet;
}


int SelectNatureBalanceForUpdate(hash_t *hContext) 
{
	int	iRet = PD_OK;
	double	dTmp;
	char	*csPtr;

	hash_t	*hData,*hRsp;

DEBUGLOG(("SelectNatureBalanceForUpdate()\n"));
	hData = (hash_t*) malloc (sizeof(hash_t));
	hash_init(hData,0);
	hRsp = (hash_t*) malloc (sizeof(hash_t));
	hash_init(hRsp,0);

/* entity id */
	if (GetField_CString(hContext,"entity_id",&csPtr)) {
DEBUGLOG(("SelectNatureBalanceForUpdate() Entity ID = [%s]\n",csPtr));
		PutField_CString(hData,"entity_id",csPtr);
	}
	else {
DEBUGLOG(("SelectNatureBalanceForUpdate() Entity ID not found\n"));
ERRLOG("BOMMSEntityBalance::SelectNatureBalanceForUpdate() Entity ID not found\n");
		iRet = PD_ERR;
	}

/* nature id */
	if (GetField_CString(hContext,"nature_id",&csPtr)) {
DEBUGLOG(("SelectNatureBalanceForUpdate() Nature ID = [%s]\n",csPtr));
		PutField_CString(hData,"nature_id",csPtr);
	}
	else {
DEBUGLOG(("SelectNatureBalanceForUpdate() Nature ID not found\n"));
ERRLOG("BOMMSEntityBalance::SelectNatureBalanceForUpdate() Nature ID not found\n");
		iRet = PD_ERR;
	}


/* ccy */
	if (iRet == PD_OK ) {
		if (GetField_CString(hContext,"ccy",&csPtr)) {
DEBUGLOG(("SelectNatureBalanceForUpdate() ccy = [%s]\n",csPtr));
			PutField_CString(hData,"ccy",csPtr);
		}
		else {
DEBUGLOG(("SelectNatureBalanceForUpdate() ccy to found\n"));
ERRLOG("BOMMSEntityBalance::SelectNatureBalanceForUpdate() ccy not found\n");
			iRet = PD_ERR;
		}
	}

/* create nature balance for opening balance */
	if (iRet == PD_OK) {
		DBObjPtr = CreateObj(DBPtr,"DBMmsEntityBalance","GetNatureBalanceForUpdate");
		iRet = (unsigned long)((DBObjPtr)(hData,hRsp));

		if (iRet == PD_OK) {
/*opening_acct_bal */
			if (GetField_Double(hRsp,"acct_bal",&dTmp)) {
DEBUGLOG(("SelectNatureBalanceForUpdate() open_acct_bal = [%lf]\n",dTmp));
				PutField_Double(hContext,"open_acct_bal",dTmp);
			}

/*opening_prepaid */
			if (GetField_Double(hRsp,"prepaid",&dTmp)) {
DEBUGLOG(("SelectNatureBalanceForUpdate() open_prepaid = [%lf]\n",dTmp));
				PutField_Double(hContext,"open_prepaid",dTmp);
			}

/*opening_intransit */
			if (GetField_Double(hRsp,"intransit",&dTmp)) {
DEBUGLOG(("SelectNatureBalanceForUpdate() open_intransit = [%lf]\n",dTmp));
				PutField_Double(hContext,"open_intransit",dTmp);
			}
/*opening_lien */
			if (GetField_Double(hRsp,"lien",&dTmp)) {
DEBUGLOG(("SelectNatureBalanceForUpdate() open_lien = [%lf]\n",dTmp));
				PutField_Double(hContext,"open_lien",dTmp);
			}


		}
	}

	FREE_ME(hData);
	FREE_ME(hRsp);
DEBUGLOG(("SelectNatureBalanceForUpdate:: () Noraml Exit iRet = [%d]\n",iRet));
	return iRet;
}


/* Prepaid */
int CreditPrepaid(hash_t* hContext, const hash_t* hReq)
{
        int     iRet = PD_OK;
DEBUGLOG(("CrediPrepaid:: ()\n"));
        PutField_CString(hContext,"amt_type",PD_CR);
        PutField_CString(hContext,"bal_amt","prepaid");

        iRet = UpdateEntityNatureBalance(hContext,hReq);

        RemoveField_CString(hContext,"amt_type");
        RemoveField_CString(hContext,"bal_amt");

DEBUGLOG(("CreditPrepaid:: return iRet = [%d]\n",iRet));
        return  iRet;
}

int DebitPrepaid(hash_t* hContext, const hash_t* hReq)
{
        int     iRet = PD_OK;
DEBUGLOG(("DebitPrepaid:: ()\n"));
        PutField_CString(hContext,"amt_type",PD_DR);
        PutField_CString(hContext,"bal_amt","prepaid");

        iRet = UpdateEntityNatureBalance(hContext,hReq);
        RemoveField_CString(hContext,"amt_type");
        RemoveField_CString(hContext,"bal_amt");

DEBUGLOG(("DebitPrepaid:: return iRet = [%d]\n",iRet));
        return  iRet;
}

/* intransit */
int CreditIntransit(hash_t* hContext, const hash_t* hReq)
{
	int	iRet = PD_OK;
DEBUGLOG(("CreditIntransit:: ()\n"));
	PutField_CString(hContext,"amt_type",PD_CR);
	PutField_CString(hContext,"bal_amt","intransit");

	iRet = UpdateEntityNatureBalance(hContext,hReq);

	RemoveField_CString(hContext,"amt_type");
	RemoveField_CString(hContext,"bal_amt");

DEBUGLOG(("CreditIntransit:: return iRet = [%d]\n",iRet));
	return	iRet;
}

int DebitIntransit(hash_t* hContext, const hash_t* hReq)
{
	int	iRet = PD_OK;
DEBUGLOG(("DebitIntransit:: ()\n"));
	PutField_CString(hContext,"amt_type",PD_DR);
	PutField_CString(hContext,"bal_amt","intransit");

	iRet = UpdateEntityNatureBalance(hContext,hReq);
	RemoveField_CString(hContext,"amt_type");
	RemoveField_CString(hContext,"bal_amt");

DEBUGLOG(("DebitIntransit:: return iRet = [%d]\n",iRet));
	return	iRet;
}

/* intransit end */

int CreditAcctBal(hash_t* hContext, const hash_t* hReq)
{
        int     iRet = PD_OK;
DEBUGLOG(("CreditAcctBal:: ()\n"));
        PutField_CString(hContext,"amt_type",PD_CR);
        PutField_CString(hContext,"bal_amt","acct_bal");

        iRet = UpdateEntityNatureBalance(hContext,hReq);

        RemoveField_CString(hContext,"amt_type");
        RemoveField_CString(hContext,"bal_amt");

DEBUGLOG(("CreditAcctBal:: return iRet = [%d]\n",iRet));
        return  iRet;
}

int DebitAcctBal(hash_t* hContext, const hash_t* hReq)
{
        int     iRet = PD_OK;
DEBUGLOG(("DebitAcctBal:: ()\n"));
        PutField_CString(hContext,"amt_type",PD_DR);
        PutField_CString(hContext,"bal_amt","acct_bal");

        iRet = UpdateEntityNatureBalance(hContext,hReq);
        RemoveField_CString(hContext,"amt_type");
        RemoveField_CString(hContext,"bal_amt");

DEBUGLOG(("DebitAcctBal:: return iRet = [%d]\n",iRet));
        return  iRet;
}

/*** lien ****/

int CreditLien(hash_t* hContext, const hash_t* hReq)
{
        int     iRet = PD_OK;
DEBUGLOG(("CreditLien:: ()\n"));
	double	dOpenAcctBal = 0.0;
	double	dOpenPrepaid = 0.0;
	double	dOpenLien = 0.0;
	double	dOpenIntransit = 0.0;

/* debit acct_bal */
        PutField_CString(hContext,"amt_type",PD_DR);
        PutField_CString(hContext,"bal_amt","acct_bal");

        iRet = UpdateEntityNatureBalance(hContext,hReq);

        RemoveField_CString(hContext,"amt_type");
        RemoveField_CString(hContext,"bal_amt");
/* debit acct_bal End */

/* credit lien */
	if (iRet == PD_OK) {
/* temp store opening bal */
		GetField_Double(hContext,"open_acct_bal",&dOpenAcctBal);
DEBUGLOG(("CreditLien open_acct_bal = [%lf]\n",dOpenAcctBal));

		GetField_Double(hContext,"open_perpaid",&dOpenPrepaid);
DEBUGLOG(("CreditLien open_perpaid = [%lf]\n",dOpenPrepaid));

		GetField_Double(hContext,"open_lien",&dOpenLien);
DEBUGLOG(("CreditLien open_lien = [%lf]\n",dOpenLien));

		GetField_Double(hContext,"open_intransit",&dOpenIntransit);
DEBUGLOG(("CreditLien open_intransit = [%lf]\n",dOpenIntransit));

        	PutField_CString(hContext,"amt_type",PD_CR);
        	PutField_CString(hContext,"bal_amt","lien");

        	iRet = UpdateEntityNatureBalance(hContext,hReq);

        	RemoveField_CString(hContext,"amt_type");
        	RemoveField_CString(hContext,"bal_amt");

		if (iRet == PD_OK) {
/* restore opening bal */
			PutField_Double(hContext,"open_acct_bal",dOpenAcctBal);
			PutField_Double(hContext,"open_prepaid",dOpenPrepaid);
			PutField_Double(hContext,"open_lien",dOpenLien);
			PutField_Double(hContext,"open_intransit",dOpenIntransit);
		}
	}
/* credit lien End */

DEBUGLOG(("CreditLien:: return iRet = [%d]\n",iRet));
        return  iRet;
}


int DebitLien(hash_t* hContext, const hash_t* hReq)
{
        int     iRet = PD_OK;
DEBUGLOG(("DebitLien:: ()\n"));
        double  dOpenAcctBal = 0.0;
        double  dOpenPrepaid = 0.0;
        double  dOpenLien = 0.0;
        double  dOpenIntransit = 0.0;

/* debit lien */
        PutField_CString(hContext,"amt_type",PD_DR);
        PutField_CString(hContext,"bal_amt","lien");

        iRet = UpdateEntityNatureBalance(hContext,hReq);

        RemoveField_CString(hContext,"amt_type");
        RemoveField_CString(hContext,"bal_amt");
/* debit lien End */

/* credit acct_bal */
        if (iRet == PD_OK) {
/* temp store opening bal */
                GetField_Double(hContext,"open_acct_bal",&dOpenAcctBal);
DEBUGLOG(("DebitLien open_acct_bal = [%lf]\n",dOpenAcctBal));

                GetField_Double(hContext,"open_perpaid",&dOpenPrepaid);
DEBUGLOG(("DebitLien open_perpaid = [%lf]\n",dOpenPrepaid));

                GetField_Double(hContext,"open_lien",&dOpenLien);
DEBUGLOG(("DebitLien open_lien = [%lf]\n",dOpenLien));

                GetField_Double(hContext,"open_intransit",&dOpenIntransit);
DEBUGLOG(("DebitLien open_intransit = [%lf]\n",dOpenIntransit));

                PutField_CString(hContext,"amt_type",PD_CR);
                PutField_CString(hContext,"bal_amt","acct_bal");

                iRet = UpdateEntityNatureBalance(hContext,hReq);

                RemoveField_CString(hContext,"amt_type");
                RemoveField_CString(hContext,"bal_amt");

                if (iRet == PD_OK) {
/* restore opening bal */
                        PutField_Double(hContext,"open_acct_bal",dOpenAcctBal);
                        PutField_Double(hContext,"open_prepaid",dOpenPrepaid);
                        PutField_Double(hContext,"open_lien",dOpenLien);
                        PutField_Double(hContext,"open_intransit",dOpenIntransit);
                }
        }
/* credit acct_bal End */

DEBUGLOG(("DebitLien:: return iRet = [%d]\n",iRet));
        return  iRet;
}

/*** Lien ***/


int UpdateEntityNatureBalance(hash_t* hContext, const hash_t* hReq)
{
	int	iRet = PD_OK;
	int	iAllowBalNegative = PD_TRUE;
	int 	i,iBalCnt = 0;
	
	double	dOpenBal;
	double	dTxnAmt,dNatureTxnAmt;
	double	dTmp;

	char	*csPtr;
	char	*csBalAmt;
	char	*csAmtType;
	char	*csTxnCcy;
	char	*csEntityId;
	
	char	csTag[PD_TAG_LEN + 1];

	hash_t	*hData,*hRsp,*hElement;

	hData = (hash_t*) malloc (sizeof(hash_t));
	hash_init(hData,0);

	hRsp = (hash_t*) malloc (sizeof(hash_t));
	hash_init(hRsp,0);

	hElement = (hash_t*) malloc (sizeof(hash_t));
	hash_init(hElement,0);

DEBUGLOG(("UpdateEntityNatureBalance:: Start()\n"));

/* txn_seq */
	if (GetField_CString(hContext,"txn_seq",&csPtr)) {
DEBUGLOG(("UpdateEntityNatureBalance() txn_seq = [%s]\n",csPtr));
                PutField_CString(hData,"txn_seq",csPtr);
        }

/* entity_id */
        if (GetField_CString(hContext,"entity_id",&csEntityId)) {
DEBUGLOG(("UpdateEntityNatureBalance() Entity ID = [%s]\n",csEntityId));
      	}
       	else {
DEBUGLOG(("UpdateEntityNatureBalance() Entity ID nto found\n"));
ERRLOG("BOMMSEntityBalance::UpdateEntityNatureBalance() Entity ID not found\n");
       		iRet = PD_ERR;
       	}

/* ccy */
       	if (iRet == PD_OK) {
               	if (GetField_CString(hReq,"ccy",&csTxnCcy)) {
DEBUGLOG(("UpdateEntityNatureBalance() ccy = [%s]\n",csTxnCcy));
                       	PutField_CString(hData,"ccy",csTxnCcy);
               	}
               	else {
DEBUGLOG(("UpdateEntityNatureBalance() ccy not found\n"));
ERRLOG("BOMMSEntityBalance::UpdateEntityNatureBalance() ccy not found\n");
                       	iRet = PD_ERR;
               	}
       	}

/* amt_type */
	if (iRet == PD_OK) {
                if (GetField_CString(hContext,"amt_type",&csAmtType)) {
DEBUGLOG(("UpdateEntityNatureBalance() amt_type = [%s]\n",csAmtType));
                }
        }

/* bal_amt */
	if (iRet == PD_OK) {	
		if (GetField_CString(hContext,"bal_amt",&csBalAmt)) {
DEBUGLOG(("UpdateEntityNatureBalance() bal_amt = [%s]\n",csBalAmt));
		}
	}

/* txn_amt */
       	if (iRet == PD_OK) {
		if (GetField_Double(hContext,"override_txn_amt",&dTxnAmt)) {
DEBUGLOG(("UpdateEntityNatureBalance() txn_amt = [%lf]\n",dTxnAmt));
               	}
           	else if (GetField_Double(hContext,"txn_amt",&dTxnAmt)) {
DEBUGLOG(("UpdateEntityNatureBalance() txn_amt = [%lf]\n",dTxnAmt));
           	}
           	else {
DEBUGLOG(("UpdateEntityNatureBalance() unable to get txn_amt\n"));
ERRLOG("BOMMSEntityBalance::UpdateEntityNatureBalance() unable to get txn_amt\n");
                      	iRet = PD_ERR;
            	}
	}

/* allow_bal_neg */
	if (iRet == PD_OK) {
		if (GetField_Int(hReq, "allow_bal_neg", &iAllowBalNegative)) {
DEBUGLOG(("UpdateEntityNatureBalance() allow_bal_neg = [%d]\n", iAllowBalNegative));
            	}
	}

/* bal_cnt */
	if (iRet == PD_OK) {
		if (GetField_Int(hContext,"bal_cnt",&iBalCnt)) {
DEBUGLOG(("UpdateEntityNatureBalance() bal_cnt = [%d]\n",iBalCnt));
		}
	}

	// Get Entity Balance (Open Balance)
	if (iRet == PD_OK) {
		PutField_CString(hData,"entity_id",csEntityId);
                PutField_CString(hData,"ccy",csTxnCcy);
	 	DBObjPtr = CreateObj(DBPtr,"DBMmsEntityBalance","GetEntityBalanceForUpdate");
                iRet = (unsigned long)((DBObjPtr)(hData,hRsp));

                if (iRet == PD_OK) {
/* curr_acct_bal */
                        if (GetField_Double(hRsp,"acct_bal",&dTmp)) {
DEBUGLOG(("UpdateEntityNatureBalance() open_acct_bal = [%lf]\n",dTmp));
                                PutField_Double(hContext,"open_acct_bal",dTmp);
                        }

/* curr_prepaid */
                        if (GetField_Double(hRsp,"prepaid",&dTmp)) {
DEBUGLOG(("UpdateEntityNatureBalance() open_prepaid = [%lf]\n",dTmp));
                                PutField_Double(hContext,"open_prepaid",dTmp);
                        }

/* curr_intransit */
                        if (GetField_Double(hRsp,"intransit",&dTmp)) {
DEBUGLOG(("UpdateEntityNatureBalance() open_intransit = [%lf]\n",dTmp));
                                PutField_Double(hContext,"open_intransit",dTmp);
                        }

/* curr_lien */
                        if (GetField_Double(hRsp,"lien",&dTmp)) {
DEBUGLOG(("UpdateEntityNatureBalance() open_lien = [%lf]\n",dTmp));
                                PutField_Double(hContext,"open_lien",dTmp);
                        }
                }
		else {
			iRet = INT_MMS_NATURE_BAL_NOT_FOUND;
			PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("UpdateEntityNatureBalance() Nature Bal not found\n"));
ERRLOG("BOMMSEntityBalance:UpdateEntityNatureBalance() Nature Bal not found\n");
		}
	}

	// Check if allow balance negative
	if  (iRet == PD_OK) {
		if (iAllowBalNegative == PD_FALSE) {

/* bal_amt */
			
			if (GetField_Double(hRsp,csBalAmt,&dOpenBal)) {
DEBUGLOG(("UpdateEntityNatureBalance() [%s] = [%lf]\n",csBalAmt,dOpenBal));
                        }	

			if ((!strcmp(csAmtType,PD_DR))
			    && (!strcmp(csBalAmt,"acct_bal")) 
			    && (dOpenBal < dTxnAmt) 
			)
			{
DEBUGLOG(("UpdateEntityNatureBalance() txn_amt[%lf] > open_bal[%lf]\n",dTxnAmt,dOpenBal));
ERRLOG("BOMMSEntityBalance:UpdateEntityNatureBalance() txn_amt[%lf] > open_bal[%lf]\n",dTxnAmt,dOpenBal);
                              	iRet = INT_INVALID_PAY_AMOUNT;
                           	PutField_Int(hContext,"internal_error",iRet);
                    	}
		}
	}

	// Update Entity Nature Balance
	if  (iRet == PD_OK) {
		for (i = 1 ; i <= iBalCnt; i++) {
			hash_init(hData,0);

/* entity_id */
			PutField_CString(hData,"entity_id",csEntityId);
			PutField_CString(hElement,"entity_id",csEntityId);
               
/* ccy */		
		 	PutField_CString(hData,"ccy",csTxnCcy);

/* nature_txn_ccy */
			sprintf(csTag,"bal.%d.nature_txn_ccy",i);
                	PutField_CString(hContext,csTag,csTxnCcy);

/* nature_net_ccy */
			sprintf(csTag,"bal.%d.nature_net_ccy",i);
                	PutField_CString(hContext,csTag,csTxnCcy);

/* nature_id */
			sprintf(csTag,"bal.%d.nature_id",i);
                	if (GetField_CString(hContext,csTag,&csPtr)) {
DEBUGLOG(("UpdateEntityNatureBalance() [%s] = [%s]\n",csTag,csPtr));
                       		PutField_CString(hData,"nature_id",csPtr);
/* for txn element */
				sprintf(csTag,"bal.%d.nature_id",i);
                       		PutField_CString(hElement,csTag,csPtr);
                	}
                	else {
DEBUGLOG(("UpdateEntityNatureBalance() %s not found\n",csTag));
ERRLOG("BOMMSEntityBalance::UpdateEntityNatureBalance() %s not found\n",csTag);
                       		iRet = PD_ERR;
                	}

        		if (iRet == PD_OK ) {
				iRet = PD_ERR;

/* override_amt */
				sprintf(csTag,"bal.%d.override_amt",i);
				if (GetField_Double(hContext,csTag,&dNatureTxnAmt)) {
DEBUGLOG(("UpdateEntityNatureBalance:: () %s = [%lf]\n",csTag,dNatureTxnAmt));
					iRet = PD_OK;
				}
				else {
/* amt */
					sprintf(csTag,"bal.%d.amt",i);
					if (GetField_Double(hContext,csTag,&dNatureTxnAmt)) {
DEBUGLOG(("UpdateEntityNatureBalance:: () %s = [%lf]\n",csTag,dNatureTxnAmt));
						iRet = PD_OK;
					}
				}

				if (iRet == PD_OK) {

/* net_amt */
					sprintf(csTag,"bal.%d.net_amt",i);
                			PutField_Double(hContext,csTag,dNatureTxnAmt);
DEBUGLOG(("UpdateEntityNatureBalance:: () %s = [%lf]\n",csTag,dNatureTxnAmt));

/* amt_type, bal_amt */
DEBUGLOG(("UpdateEntityNatureBalance:: () amt_type = [%s] bal_amt = [%s]\n",csAmtType,csBalAmt));

					sprintf(csTag,"bal.%d.element_amt",i);
					if (!strcmp(csAmtType,PD_CR)) {
						PutField_Double(hData,csBalAmt,dNatureTxnAmt);
						PutField_Double(hElement,csTag,dNatureTxnAmt);
					}
					else if (!strcmp(csAmtType,PD_DR)) {
						PutField_Double(hData,csBalAmt,dNatureTxnAmt *(-1));
						PutField_Double(hElement,csTag,dNatureTxnAmt *(-1));
					}
					else {
DEBUGLOG(("UpdateEntityNatureBalance() invalid amt type [%s]\n",csPtr));
ERRLOG("BOMMSEntityBalance::UpdateEntityNatureBalance() invalid amt type [%s]\n",csPtr);
						iRet = PD_ERR;
					}
				}
                		else {
DEBUGLOG(("UpdateEntityNatureBalance() [%s] not found\n",csTag));
ERRLOG("BOMMSEntityBalance::UpdateEntityNatureBalance() %s not found\n",csTag);
                		}
			}

			// Create Nature Balance if doesn't exit
			if (iRet == PD_OK) {
				DBObjPtr = CreateObj(DBPtr,"DBMmsEntityBalance","Update");
				iRet = (unsigned long)((DBObjPtr)(hData));
DEBUGLOG(("UpdateEntityNatureBalance iRet = [%d] from MmsEntityBalance:Update\n",iRet));
			}
		}
	}

	// Update Txn Element for Intransit
	if (iRet == PD_OK) {
		if (!strcmp(csBalAmt,"intransit") ||
		    !strcmp(csBalAmt,"lien")	||
		    !strcmp(csBalAmt,"acct_bal") ||
		    !strcmp(csBalAmt,"prepaid")	
		   ) {
			PutField_Int(hElement,"bal_cnt",iBalCnt);
/* txn_seq */
			if(GetField_CString(hContext,"txn_seq",&csPtr)){
                       		PutField_CString(hElement,"txn_seq",csPtr);
                	}

			// Element Type
			
/* intransit */
			if (!strcmp(csBalAmt,"intransit")) {
				PutField_CString(hElement,"txn_element_type",PD_ELEMENT_IN_TRANSIT);
			}
/* lien */
			else if (!strcmp(csBalAmt,"lien")) {
				PutField_CString(hElement,"txn_element_type",PD_ELEMENT_HOLD_AMT);
			}
/* acct_bal */
			else if (!strcmp(csBalAmt,"acct_bal")) {
				if (GetField_CString(hContext,"override_element_type",&csPtr)) {
DEBUGLOG(("UpdateEntityNatureBalance:Override transaction element to [%s]\n",csPtr));
                                        PutField_CString(hElement,"txn_element_type",csPtr);
				}
				else {
DEBUGLOG(("UpdateEntityNatureBalance:acct_bal set to default \n"));
					PutField_CString(hElement,"txn_element_type",PD_ELEMENT_TXN_AMT);
				}
			}
/* prepaid */
                        else if (!strcmp(csBalAmt,"prepaid")) {
                                if (GetField_CString(hContext,"override_element_type",&csPtr)) {
DEBUGLOG(("UpdateEntityNatureBalance:Override transaction element to [%s]\n",csPtr));
                                        PutField_CString(hElement,"txn_element_type",csPtr);
                                }
                                else {
DEBUGLOG(("UpdateEntityNatureBalance:acct_bal set to default \n"));
                                        PutField_CString(hElement,"txn_element_type",PD_ELEMENT_PERPAID);
                                }
                        }

/* ccy */
			PutField_CString(hElement,"ccy",csTxnCcy);

/* amt_type, txn_amt */
			if (!strcmp(csAmtType,PD_CR)) {
				PutField_Double(hElement,"amount",dTxnAmt);
			}
			else {
				PutField_Double(hElement,"amount",dTxnAmt * (-1));
			}
	
			// Add Txn Element
			if  (iRet == PD_OK) {
				BOObjPtr = CreateObj(BOPtr,"BOMMSTxnElement","AddTxnElement");
                		iRet = (unsigned long)((BOObjPtr)(hElement));
			}
		}
	}

	// Get Entity Balance (Closing Balance)
	if (iRet == PD_OK) {
                DBObjPtr = CreateObj(DBPtr,"DBMmsEntityBalance","GetEntityBalanceForUpdate");
                iRet = (unsigned long)((DBObjPtr)(hData,hRsp));

                if (iRet == PD_OK) {
/* curr_acct_bal */
                        if (GetField_Double(hRsp,"acct_bal",&dTmp)) {
DEBUGLOG(("UpdateEntityNatureBalance() curr_acct_bal = [%lf]\n",dTmp));
                                PutField_Double(hContext,"curr_acct_bal",dTmp);
                        }
/* curr_prepaid */
                        if (GetField_Double(hRsp,"prepaid",&dTmp)) {
DEBUGLOG(("UpdateEntityNatureBalance() curr_prepaid = [%lf]\n",dTmp));
                                PutField_Double(hContext,"curr_prepaid",dTmp);
                        }
/* curr_intransit */
                        if (GetField_Double(hRsp,"intransit",&dTmp)) {
DEBUGLOG(("UpdateEntityNatureBalance() curr_intransit = [%lf]\n",dTmp));
                                PutField_Double(hContext,"curr_intransit",dTmp);
                        }
/* curr_lien */
                        if (GetField_Double(hRsp,"lien",&dTmp)) {
DEBUGLOG(("UpdateEntityNatureBalance() curr_lien = [%lf]\n",dTmp));
                                PutField_Double(hContext,"curr_lien",dTmp);
                        }
                }
        }

	FREE_ME(hData);
	FREE_ME(hRsp);
	FREE_ME(hElement);
DEBUGLOG(("UpdateEntityNatureBalance:: () Noraml Exit iRet = [%d]\n",iRet));
	return iRet;
}

int UpdateNatureBalance(hash_t* hContext, const hash_t* hReq)
{
	int	iRet = PD_OK;
	char	*csPtr;
	char	*csBalAmt;
	char	*csAmtType;
	char	*csTxnCcy;
	double	dTmp;
	double	dTxnAmt;

	hash_t	*hData;
	hash_t	*hRsp;

DEBUGLOG(("UpdateyNatureBalance:: ()\n"));
	hData = (hash_t*) malloc (sizeof(hash_t));
	hash_init(hData,0);
	hRsp = (hash_t*) malloc (sizeof(hash_t));
	hash_init(hRsp,0);

      	if (GetField_CString(hContext,"txn_seq",&csPtr)) {
DEBUGLOG(("UpdateNatureBalance() txn_seq = [%s]\n",csPtr));
              	PutField_CString(hData,"txn_seq",csPtr);
       	}

      	if (GetField_CString(hReq,"entity_id",&csPtr)) {
DEBUGLOG(("UpdateNatureBalance() entity_id = [%s]\n",csPtr));
              	PutField_CString(hData,"entity_id",csPtr);
       	}
       	else {
DEBUGLOG(("UpdateNatureBalance() entity_id to found\n"));
ERRLOG("BOMMSEntityBalance::UpdateNatureBalance() entity_id not found\n");
               	iRet = PD_ERR;
       	}


/* ccy */
       	if (iRet == PD_OK ) {
               	if (GetField_CString(hReq,"ccy",&csTxnCcy)) {
DEBUGLOG(("UpdateNatureBalance() ccy = [%s]\n",csTxnCcy));
                       	PutField_CString(hData,"ccy",csTxnCcy);
               	}
               	else {
DEBUGLOG(("UpdateNatureBalance() ccy to found\n"));
ERRLOG("BOMMSEntityBalance::UpdateNatureBalance() ccy not found\n");
                       	iRet = PD_ERR;
               	}
       	}

/* nature_id */
       	if (iRet == PD_OK ) {
               	if (GetField_CString(hReq,"nature_id",&csPtr)) {
DEBUGLOG(("UpdateNatureBalance() ccy = [%s]\n",csPtr));
                       	PutField_CString(hData,"nature_id",csPtr);
			
/* for txn element */
			PutField_Int(hData,"bal_cnt",1);
                       	PutField_CString(hData,"bal.1.nature_id",csPtr);
               	}
               	else {
DEBUGLOG(("UpdateNatureBalance() nature_id to found\n"));
ERRLOG("BOMMSEntityBalance::UpdateNatureBalance() nature_id not found\n");
                       	iRet = PD_ERR;
               	}
       	}
/* amt_type */
       	if (iRet == PD_OK ) {
               	if (GetField_CString(hReq,"amt_type",&csAmtType)) {
DEBUGLOG(("UpdateNatureBalance() amt_type = [%s]\n",csAmtType));
               	}
               	else {
DEBUGLOG(("UpdateNatureBalance() amt_type to found\n"));
ERRLOG("BOMMSEntityBalance::UpdateNatureBalance() amt_type not found\n");
                       	iRet = PD_ERR;
               	}
       	}
/* bal_amt */
       	if (iRet == PD_OK ) {
               	if (GetField_CString(hReq,"bal_amt",&csBalAmt)) {
DEBUGLOG(("UpdateNatureBalance() bal_amt = [%s]\n",csBalAmt));
               	}
               	else {
DEBUGLOG(("UpdateNatureBalance() bal_amt to found\n"));
ERRLOG("BOMMSEntityBalance::UpdateNatureBalance() bal_amt not found\n");
                       	iRet = PD_ERR;
               	}
       	}

        if (GetField_Double(hReq,"txn_amt",&dTxnAmt)) {
DEBUGLOG(("UpdateNatureBalance() txn_amt = [%lf]\n",dTxnAmt));
        }

	if (!strcmp(csAmtType,PD_CR)) {
        	PutField_Double(hData,csBalAmt,dTxnAmt);
        	PutField_Double(hData,"bal.1.element_amt",dTxnAmt);
	}
        else if (!strcmp(csAmtType,PD_DR)) {
                PutField_Double(hData,csBalAmt,dTxnAmt *(-1));
        	PutField_Double(hData,"bal.1.element_amt",dTxnAmt *(-1));
	}


	if (iRet == PD_OK) {
 		DBObjPtr = CreateObj(DBPtr,"DBMmsEntityBalance","GetEntityBalanceForUpdate");
        	iRet = (unsigned long)((DBObjPtr)(hData,hRsp));

		if (iRet == PD_OK) {
/*curr_acct_bal */
                        if (GetField_Double(hRsp,"acct_bal",&dTmp)) {
DEBUGLOG(("UpdateNatureBalance() open_acct_bal = [%lf]\n",dTmp));
                                PutField_Double(hContext,"open_acct_bal",dTmp);
                        }

/*curr_prepaid */
                        if (GetField_Double(hRsp,"prepaid",&dTmp)) {
DEBUGLOG(("UpdateNatureBalance() open_prepaid = [%lf]\n",dTmp));
                                PutField_Double(hContext,"open_prepaid",dTmp);
                        }

/*curr_intransit */
                        if (GetField_Double(hRsp,"intransit",&dTmp)) {
DEBUGLOG(("UpdateNatureBalance() open_intransit = [%lf]\n",dTmp));
                                PutField_Double(hContext,"open_intransit",dTmp);
                        }
/*curr_lien */
                        if (GetField_Double(hRsp,"lien",&dTmp)) {
DEBUGLOG(("UpdateNatureBalance() open_lien = [%lf]\n",dTmp));
                                PutField_Double(hContext,"open_lien",dTmp);
                        }
                }
                else {
                        iRet = INT_MMS_NATURE_BAL_NOT_FOUND;
                        PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("UpdateNatureBalance() Nature Bal not found\n"));
ERRLOG("BOMMSEntityBalance:UpdateNatureBalance() Nature Bal not found\n");
                }
	}

	if (iRet == PD_OK) {
		DBObjPtr = CreateObj(DBPtr,"DBMmsEntityBalance","Update");
           	iRet = (unsigned long)((DBObjPtr)(hData));
DEBUGLOG(("BOMMSEntityBalance:UpdateNatureBalance iRet = [%d] from MmsEntityBalance:Update\n",iRet));
	}

/* update txn element for intransit */
        if (iRet == PD_OK) {
                if (!strcmp(csBalAmt,"intransit") ||
                    !strcmp(csBalAmt,"lien")    ||
                    !strcmp(csBalAmt,"acct_bal")
                   ) {
                        if(GetField_CString(hContext,"txn_seq",&csPtr)){
                                PutField_CString(hData,"txn_seq",csPtr);
                        }

/* element type */
        /* intransit */
                        if (!strcmp(csBalAmt,"intransit")) {
                                PutField_CString(hData,"txn_element_type",PD_ELEMENT_IN_TRANSIT);
                        }
        /* lien */
                        else if (!strcmp(csBalAmt,"lien")) {
                                PutField_CString(hData,"txn_element_type",PD_ELEMENT_HOLD_AMT);
                        }
        /* acct_bal */
                        else if (!strcmp(csBalAmt,"acct_bal")) {
                                PutField_CString(hData,"txn_element_type",PD_ELEMENT_TXN_AMT);
                        }
/* ccy */
                        PutField_CString(hData,"ccy",csTxnCcy);

                        if (!strcmp(csAmtType,PD_CR)) {
                                PutField_Double(hData,"amount",dTxnAmt);
                        }
                        else
                                PutField_Double(hData,"amount",dTxnAmt * (-1));

                        BOObjPtr = CreateObj(BOPtr,"BOMMSTxnElement","AddTxnElement");
                        iRet = (unsigned long)((BOObjPtr)(hData));
                }
        }

/* closing balance */
        if (iRet == PD_OK) {
                DBObjPtr = CreateObj(DBPtr,"DBMmsEntityBalance","GetEntityBalanceForUpdate");
                iRet = (unsigned long)((DBObjPtr)(hData,hRsp));

                if (iRet == PD_OK) {
/*curr_acct_bal */
                        if (GetField_Double(hRsp,"acct_bal",&dTmp)) {
DEBUGLOG(("UpdateNatureBalance() curr_acct_bal = [%lf]\n",dTmp));
                                PutField_Double(hContext,"curr_acct_bal",dTmp);
                        }

/*curr_prepaid */
                        if (GetField_Double(hRsp,"prepaid",&dTmp)) {
DEBUGLOG(("UpdateNatureBalance() curr_prepaid = [%lf]\n",dTmp));
                                PutField_Double(hContext,"curr_prepaid",dTmp);
                        }

/*curr_intransit */
                        if (GetField_Double(hRsp,"intransit",&dTmp)) {
DEBUGLOG(("UpdateNatureBalance() curr_intransit = [%lf]\n",dTmp));
                                PutField_Double(hContext,"curr_intransit",dTmp);
                        }
/*curr_lien */
                        if (GetField_Double(hRsp,"lien",&dTmp)) {
DEBUGLOG(("UpdateNatureBalance() curr_lien = [%lf]\n",dTmp));
                                PutField_Double(hContext,"curr_lien",dTmp);
                        }
                }
        }

	FREE_ME(hData);
	FREE_ME(hRsp);
DEBUGLOG(("UpdateEntityNatureBalance:: () Noraml Exit iRet = [%d]\n",iRet));
	return iRet;
}


