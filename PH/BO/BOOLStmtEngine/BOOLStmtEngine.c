...
-------------------------------                    ------------            --------------
Init Version                                       2014/11/05              [WCS]
Add record locking                                 2015/08/06              [WCS]
Add special_handling "send_alert"                  2015/10/30              [WCS]
Avoid duplicated engine call                       2021/04/30              [WCS]
*/
...
int ProcessUnknownMatching(hash_t *hContext) {
	...
	char csLastTxnCode[PD_TXN_CODE_LEN] = PD_DEFAULT_TXN_CODE;
...
DEBUGLOG(("ProcessUnknownMatching Start\n"));

	// get baid_txn_id
	if (iRet == PD_OK) {
		if (!GetField_CString(hContext, "baid_txn_id", &csBaidTxnId)) {
DEBUGLOG(("baid_txn_id not found\n"));
			iRet = PD_ERR;
		} else {
...
DEBUGLOG(("call DBOLBAIDTxn::GetBaidTxn()\n"));
		...
		if (iTmpRet != PD_OK) {
DEBUGLOG(("call DBOLBAIDTxn::GetBaidTxn() failed\n"));
			iRet = PD_ERR;
		} else {
			// get pid
			if (!GetField_CString(hTxnDtl, "pid", &csPspId)) {
DEBUGLOG(("pid not found\n"));
				iRet = PD_ERR;
			} else {
DEBUGLOG(("pid = [%s]\n", csPspId));
			}
...
DEBUGLOG(("unknown credit/debit, skip calling engines\n"));
					continue;
				} else if (!strcmp(csTmp, csLastTxnCode)) {
DEBUGLOG(("duplicated txn type, skip calling engines\n"));
					continue;
				}
				strcpy(csLastTxnCode, csTmp);
				PutField_CString(hTxnDtl, "baid_txn_code", csTmp);
//				PutField_CString(hContext, "baid_txn_code", csTmp);
			}

DEBUGLOG(("call ProcessMatching()\n"));
			iRet = ProcessMatching(hTxnDtl);
			if (iRet == PD_FOUND) {
DEBUGLOG(("call ProcessMatching() success, found\n"));
				// first_baid_txn_code
...
DEBUGLOG(("call ProcessMatching() failed\n"));
				break;
			} else {
DEBUGLOG(("call ProcessMatching() success, not found\n"));
			}
		}
...
ERRLOG("BOOLStmtEngine:: ProcessUnknownMatching:: call BOOLTxnEngine::DoAction() FAILURE!!!\n");
					...
					if (!strcmp(csNextEngineTxnCode, PD_CHARGEBACK_REVERSAL)) {
						...
						if (GetField_CString(hTxnDtl, "psp_txn_id_1", &csVodTxnId)) {
DEBUGLOG(("VOD txn id = [%s]\n", csVodTxnId));
							// get DCR txn id
							GetField_CString(hTxnDtl, "baid_txn_id_1", &csDcrBaidTxnId);
...
DEBUGLOG(("ProcessMatching Start\n"));

/*
	// get baid_txn_id_1
	if (iRet == PD_OK) {
		if (!GetField_CString(hContext, "baid_txn_id_1", &csBaidTxnId)) {
DEBUGLOG(("baid_txn_id_1 not found\n"));
			iRet = PD_ERR;
		} else {
DEBUGLOG(("baid_txn_id_1 = [%s]\n", csBaidTxnId));
		}
...
DEBUGLOG(("failed_baid_txn_code not found\n"));	
...
DEBUGLOG(("first_baid_txn_code not found\n"));
				iRet = PD_ERR;
			} else {
...
DEBUGLOG(("second_baid_txn_code not found\n"));
			} else {
...
DEBUGLOG(("next_engine_action not found\n"));
				iRet = PD_ERR;
			} else {
...
DEBUGLOG(("next_engine_txn_code not found\n"));
				iRet = PD_ERR;
			} else {
...
DEBUGLOG(("next_engine_recon_type not found\n"));
				iRet = PD_ERR;
			} else {
...
DEBUGLOG(("exit_on_error not found\n"));
					iRet = PD_ERR;
					break;
				} else {
...
DEBUGLOG(("statement matched!\n"));
				...
				if (GetField_CString(hContext, "psp_txn_id_1", &csTmp)) {
/*
					// special handling for DCR finding VOD
					GetField_CString(hContext, "first_baid_txn_code", &csFirstBaidTxnCode);
					if (!strcmp(csFirstBaidTxnCode, PD_DEPOSIT_CHARGEBACK_REVERSAL_BY_BANK) && !GetField_CString(hContext, "second_baid_txn_code", &csSecondBaidTxnCode)) {
DEBUGLOG(("DCR finding VOD special case\n"));

						// get VOD BAID Txn ID
						DBObjPtr = CreateObj(DBPtr, "DBOLStmtTxnRelation", "GetStmtTxnIdByTxnId");
						iTmpRet = (unsigned long)(*DBObjPtr)(csTmp, csBaidTxnId);
DEBUGLOG(("VOD BAID Txn ID = [%s]\n", csBaidTxnId));
						...
						PutField_Char(hTmp, "party", PD_TYPE_PSP);
						GetField_CString(hContext, "baid_txn_id_1", &csTmp);
						PutField_CString(hTmp, "p1_txn_id", csTmp);
						PutField_CString(hTmp, "p2_txn_id", csBaidTxnId);
						PutField_Char(hTmp, "txn_level", PD_STMT_LEVEL);
						PutField_Char(hTmp, "relation_type", PD_REL_LINKAGE);
						PutField_CString(hTmp, "create_user", PD_UPDATE_USER);
...
DEBUGLOG(("call DBOLTxnRelationSpc::Add()\n"));
						...
						iTmpRet = (unsigned long)(*DBObjPtr)(hTmp);
						if (iTmpRet != PD_OK) {
DEBUGLOG(("call DBOLTxnRelationSpc::Add() failed\n"));
							iRet = PD_NOT_FOUND;
						}
...
*/
DEBUGLOG(("call DBOLTransaction::GetTxnIdForUpdateNoWait()\n"));
						DBObjPtr = CreateObj(DBPtr, "DBOLTransaction", "GetTxnIdForUpdateNoWait");
						iTmpRet = (unsigned long)(*DBObjPtr)(csTmp);
						if (iTmpRet != PD_OK) {
DEBUGLOG(("call DBOLTransaction::GetTxnIdForUpdateNoWait() failed\n"));
							iRet = PD_NOT_FOUND;
						}
/*
					}
...
DEBUGLOG(("statement not matched!\n"));
				iRet = PD_NOT_FOUND;
			}
		}
	}
...
DEBUGLOG(("action_seq not found\n"));
				iRet = PD_ERR;
				break;
			} else {
DEBUGLOG(("action_seq = [%d]\n", iActionSeq));
			}
...
DEBUGLOG(("exit_on_error = [%d]\n", iExitOnError));
			...
			if (iCnt != 1) {
				if (iSubRuleId != iLastSubRuleId) {
					...
					if (iMatched) {
DEBUGLOG(("last sub_rule matched! skip remaining sub_rule(s)\n"));
						break;
					}
				} else if (iExitSubRule) {
					// skip this sub-rule
DEBUGLOG(("this sub-rule failed! skip\n"));
					hMatchRule = RecordSet_GetNext(rMatchRule);
					continue;
				}
			}
...
DEBUGLOG(("second_party = [%s]\n", csTmp));
	...
	if (iRet == PD_OK) {
DEBUGLOG(("call DBOLStmtMatchFilter::GetStmtMatchFilter()\n"));
		...
		if (iTmpRet != PD_OK) {
DEBUGLOG(("call DBOLStmtMatchFilter::GetStmtMatchFilter() failed\n"));
			iRet = PD_ERR;
		}
	}
...
// DEBUGLOG(("special_handling = [%s]\n", csSpecialHandling));
				...
				} else if (!strcmp(csSpecialHandling, "send_alert")) {
					iTmpRet = TmpSkip(hMatchFilter);
					iSendAlert = 1;
					PutField_Int(hContext, "send_alert", iSendAlert);
				}
			}

			// get from_input (mandatory)
			if (!GetField_Int(hMatchFilter, "from_input", &iFromInput)) {
DEBUGLOG(("from_input not found\n"));
				iRet = PD_ERR;
				break;
			} else {
...
DEBUGLOG(("left_operand not found\n"));
					iRet = PD_ERR;
					break;
				}
			} else {
...
DEBUGLOG(("right_operand not found\n"));
					iRet = PD_ERR;
					break;
				}
			} else {
...
DEBUGLOG(("missing input or output filter\n"));
				iRet = PD_ERR;
		}
	}
...
// DEBUGLOG(("from_clause = [%s]\n", csTmp));
	...
	// execute engine sql
	if (iRet == PD_OK) {
DEBUGLOG(("call DBOLStmtMatchEngine::ExecuteSql()\n"));
	...
	if (iRet == PD_FOUND) {
DEBUGLOG(("check ExecuteSql result\n"));
...
DEBUGLOG(("CheckKeywords Start\n"));

	// baid_txn_id_2
	if (iRet == PD_OK) {
		if (!GetField_CString(hContext, "baid_txn_id_2", &csBaidTxnId)) {
DEBUGLOG(("baid_txn_id_2 not found\n"));
			iRet = PD_ERR;
		} else {
...
DEBUGLOG(("check_baid_txn_code not found\n"));
			iRet = PD_ERR;
		} else {
DEBUGLOG(("check_baid_txn_code = [%s]\n", csCheckBaidTxnCode));
		}
	}
...
//				PutField_CString(hStmtDtl, "format_txn_code", csCheckBaidTxnCode);
	...
	// get keywords
	if (iRet == PD_OK) {
DEBUGLOG(("call DBOLStmtFormat::GetTxnCodeKeywordsSingle()\n"));
		DBObjPtr = CreateObj(DBPtr, "DBOLStmtFormat", "GetTxnCodeKeywordsSingle");
		iTmpRet = (unsigned long)(*DBObjPtr)(hStmtDtl, rKeywords);
		if (iTmpRet != PD_FOUND) {
DEBUGLOG(("call DBOLStmtFormat::GetTxnCodeKeywordsSingle() failed\n"));
			iRet = PD_ERR;
		}
	}
...
DEBUGLOG(("action_type = [%s]\n", csActionType));
	...
	if (iRet == PD_OK) {
		if (!strcmp(csActionType, "fr_baid1_to_psp1") ||
			!strcmp(csActionType, "fr_baid1_to_psp1_obp") ||
...
DEBUGLOG(("psp_txn_id_1 not found\n"));
				iRet = PD_ERR;
			} else {
...
DEBUGLOG(("psp_txn_id_2 not found\n"));
				iRet = PD_ERR;
			} else {
DEBUGLOG(("psp_txn_id_2 = [%s]\n", csTxnId));
			}
		} else {
DEBUGLOG(("unsupported action_type\n"));
			iRet = PD_ERR;
		}
	}
...
DEBUGLOG(("call DBOLTransaction::GetTxnHeader()\n"));
			...
			if (iTmpRet != PD_OK) {
DEBUGLOG(("call DBOLTransaction::GetTxnHeader() failed\n"));
				iRet = PD_ERR;
			} else {
...
DEBUGLOG(("txn_code = [%s]\n", csTxnCode));
				...
				if (iRet == PD_OK) {
					// get psp txn detail
DEBUGLOG(("call DBOLTxnPspDetail::GetTxnPspDetail()\n"));
					DBObjPtr = CreateObj(DBPtr, "DBOLTxnPspDetail", "GetTxnPspDetail");
					iTmpRet = (unsigned long)(*DBObjPtr)(csTxnId, hTxnDtl);
					if (iTmpRet != PD_OK) {
DEBUGLOG(("call DBOLTxnPspDetail::GetTxnPspDetail() failed\n"));
						iRet = PD_ERR;
					} else {
						// get txn_amt
...
DEBUGLOG(("bank_charge not found\n"));
					iRet = PD_ERR;
				} else {
...
DEBUGLOG(("SplitAmount() exit iRet = [%d]\n", iRet));
	return iRet;
}
