...
-------------------------------                    ------------            --------------
Init Version                                       2013/07/26              [WCS]
Call OLAcctStatusAction.GetAllowActionFlag()       2014/04/22              [WCS]
Use stmt engine to match bank stmt                 2014/12/15              [WCS]
Change GetBankAccts to GetDepositBankAccts	   2014/02/26              [WMC]
Add csWholeBankAcctNum		
Add ValidateAmount and CheckWholeAmount	   	   2016/05/06		   [WMC]
Add req_bank_second and check req_bank_channel     2017/03/10              [WCS]
Accept also 2-digit in year field		   2018/04/23		   [WMC]
Convert 6-digit to 4-digit in bank acct field 	   2018/04/23		   [WMC]
Add bank acct number checking configuration	   2020/09/07		   [WMC]
Add "MUTLI_ACCT" checking to apply on              2020/10/16              [MIC]
SMS bank account with same 4 last digit 
Add "MOBILE_NUM_NOT_MATCH" checking		   2021/05/14		   [WCS]
*/
...
#define TWO_DIG_YEAR_PREFIX  	"20"
#define SMS_BANK_ACCT_NUM_LEN	4
...
DEBUGLOG(("ValidateField:: csValue = [%s]\n", csValue));
	...
	if (!strcmp(csDesc, "req_bank_acc") || !strcmp(csDesc, "req_payer_acc")) {
		int iLen = SMS_BANK_ACCT_NUM_LEN;
        	int iIsOnlyDigit = PD_TRUE;
        	char *csFieldTag = NULL;
        	hash_t *hParam;

		// get param
		// - req_bank_acc
		// - req_payer_acc
        	hParam = RecordSet_GetFirst(rRecordParam);
        	while (hParam) {

                	// get field tag
                	if (GetField_CString(hParam, "field_tag", &csFieldTag)) {

                		// get param
                        	if (!strcmp(csFieldTag, csDesc)) {

                                	// get only_digit
                                	if (!GetField_Int(hParam, "only_digit", &iIsOnlyDigit)) {
DEBUGLOG(("ValidateField:: call DBOLSmsFormat::GetParam() only_digit not found\n"));
                               	        	return PD_FALSE;
                            		}
                                
					// get len
                                	if (!GetField_Int(hParam, "len", &iLen)) {
DEBUGLOG(("ValidateField:: call DBOLSmsFormat::GetParam() len not found\n"));
                                	        return PD_FALSE;
                                	}
...
DEBUGLOG(("ValidateField:: call DBOLSmsFormat::GetParam() field_tag not found\n"));
                        	return PD_FALSE;
                	}
                	hParam = RecordSet_GetNext(rRecordParam);
       		}

		// check is only_digit
        	if (iIsOnlyDigit == PD_TRUE) {
...
DEBUGLOG(("ValidateField:: contains non-digit character\n"));
		...
		// if input bank_acc len > get param len and <= maximum bank account len, cut the len to get param len
		if ((strlen(csValue) > iLen) && (strlen(csValue) <= PD_BANK_ACCT_NUM_LEN)) {
DEBUGLOG(("ValidateField:: bank_acc is %s\n", csValue));
			char csBuf[PD_TMP_BUF_LEN];
			memset(csBuf, 0, sizeof(csBuf));	
			for (i = 0; i < strlen(csValue); i++) {
				if (i >= (strlen(csValue)-iLen)) {
					csBuf[j] = csValue[i];
					j++;
				}
                        }
			strcpy(csValue, csBuf);
DEBUGLOG(("ValidateField:: bank_acc[last %d digit] is %s\n", iLen, csValue));
		}

		// check input bank_acc len
		if (strlen(csValue) != iLen) {
DEBUGLOG(("ValidateField:: length != %d\n", iLen));
			return PD_FALSE;
		}
	} else if (!strcmp(csDesc, "req_bank_year")) {
		// input year must be in 2 or 4 digits
		if (strlen(csValue) == 2) {
DEBUGLOG(("ValidateField:: original year is %s\n", csValue));
		   // if req_bank_year is in 2 digits, e.g. 18, prepend "20"
		   char csBuf[PD_TMP_BUF_LEN];
		   sprintf(csBuf, "%s%s", TWO_DIG_YEAR_PREFIX, csValue);
		   strcpy(csValue, csBuf);
DEBUGLOG(("ValidateField:: converted year =  %s\n", csValue));
		}
		if (strlen(csValue) != 4) {
...
//	char *csStatusType;
	...
	char csTrimAmt[PD_MAX_BUFFER];
        ...
        int iBankAcctNumLen = SMS_BANK_ACCT_NUM_LEN;
	char *csFieldTag = NULL;
	...
	recordset_t *rRecordParam;
        rRecordParam = (recordset_t*) malloc (sizeof(recordset_t));
        recordset_init(rRecordParam, 0);
...
DEBUGLOG(("ProcessSmsText:: call DBOLSmsFormat::GetFormat() failed\n"));
	...
	if (iRet == PD_OK) {
		...
		while (hFormat) {
			...
			RecordSet_Destroy(rRecordParam);
                        recordset_init(rRecordParam, 0);
...
DEBUGLOG(("ProcessSmsText:: content count not found\n"));
			...
			// get all field param
DEBUGLOG(("ProcessSmsText:: call DBOLSmsFormat::GetParam()\n"));
                        DBObjPtr = CreateObj(DBPtr, "DBOLSmsFormat", "GetParam");
                        iTmpRet = (unsigned long)(*DBObjPtr)(csFormatId, rRecordParam);
                        if (iTmpRet != PD_OK) {
DEBUGLOG(("ProcessSmsText:: call DBOLSmsFormat::GetParam() failed\n"));
                                iRet = PD_ERR;
                                break;
                        }
...
DEBUGLOG(("ProcessSmsText:: %s = [%s]\n", csDelimiterDesc, csGenDelim));
								iValid = ValidateField(csDelimiterDesc, csGenDelim, rRecordParam);

								if (iValid) {
//DEBUGLOG(("debug: value=[%s] len=[%d]\n", csGenDelim, strlen(csGenDelim)));
									PutField_CString(hText, csDelimiterDesc, csGenDelim);

									// remove the delimiter from sms text
//DEBUGLOG(("debug: value=[%s] len=[%d]\n", csTmpText, strlen(csTmpText)));
									memmove(csTmpText, csTmpText+strlen(csGenDelim), strlen(csTmpText+strlen(csGenDelim))+1);
//DEBUGLOG(("debug: value=[%s] len=[%d]\n", csTmpText, strlen(csTmpText)));
DEBUGLOG(("ProcessSmsText:: after %s = [%s]\n", csTagDelimiter, csTmpText));
									iRet = PD_OK;
								}
...
DEBUGLOG(("ProcessSmsText:: %s = [%s]\n", csDesc, csValue));
									...
									if (iAlert) {
										...
										iValid = ValidateField(csDesc, csValue, rRecordParam);
									}
...
DEBUGLOG(("ProcessSmsText:: bank acct num not found\n"));
		...
		// - req_bank_acc
                if (iRet == PD_OK) {
                        ...
                        while (hParam) {
                                if (GetField_CString(hParam, "field_tag", &csFieldTag)) {
                                        if (!strcmp(csFieldTag, "req_bank_acc")) {
                                                if (!GetField_Int(hParam, "len", &iBankAcctNumLen)) {
DEBUGLOG(("ProcessSmsText:: call DBOLSmsFormat::GetParam() len not found\n"));
                                                        iRet = PD_ERR;
                                                }
                                                break;
                                        }
                                } else {
DEBUGLOG(("ProcessSmsText:: call DBOLSmsFormat::GetParam() field_tag not found\n"));
                                        iRet = PD_ERR;
                                }
                                hParam = RecordSet_GetNext(rRecordParam);
                        }
		...
		// check number of sms post bank acct
                if (iRet == PD_OK) {
			if (iBankAcctNumLen <= SMS_BANK_ACCT_NUM_LEN) {
                        	hash_init(hRec, 0);
DEBUGLOG(("ProcessSmsText:: call DBOLBankAccts::ChkNumOfSmsDepositBankAccts()\n"));
                        	DBObjPtr = CreateObj(DBPtr, "DBOLBankAccts", "ChkNumOfSmsDepositBankAccts");
                        	iTmpRet = (unsigned long)(*DBObjPtr)(csIntBankCode, csBankAcctNum, iBankAcctNumLen);
                        	if (iTmpRet == NOT_FOUND) {
DEBUGLOG(("ProcessSmsText:: no bank acct [sms post]\n"));
                        	} else if (iTmpRet == 1) {
DEBUGLOG(("ProcessSmsText:: unique bank acct [sms post]\n"));
                        	} else if (iTmpRet > 1) {
DEBUGLOG(("ProcessSmsText:: multi bank acct [sms post]\n"));
                        	        iRet = PD_ERR;
                        	        strcpy(csStatus, "MULTI_ACCT");
                        	} else {
DEBUGLOG(("ProcessSmsText:: call DBOLSmsFormat::ChkNumOfSmsDepositBankAccts() failed\n"));
                        	        iRet = PD_ERR;
                        	}
                        	hash_destroy(hRec);
			}
...
DEBUGLOG(("ProcessSmsText:: call DBOLBankAccts::GetDepositBankAccts()\n"));
                	...
                	iTmpRet = (unsigned long)(*DBObjPtr)(csIntBankCode, csBankAcctNum, iBankAcctNumLen, hRec);
                	if (iTmpRet != PD_OK) {
DEBUGLOG(("ProcessSmsText:: call DBOLSmsFormat::GetDepositBankAccts() failed\n"));
                        	iRet = PD_ERR;
                        	strcpy(csStatus, "BANK_ACCT_NOT_FOUND");
                	} else {
				if (GetField_CString(hRec, "reg_mob_num", &csRegMobNum) && GetField_CString(hTxn, "smsc", &csSmsc)) {
DEBUGLOG(("ProcessSmsText:: reg_mob_num = [%s]\n", csRegMobNum));
DEBUGLOG(("ProcessSmsText:: smsc = [%s]\n", csSmsc));
					if (strcmp(csRegMobNum, csSmsc)) {
DEBUGLOG(("ProcessSmsText:: reg_mob_num does not match smsc\n"));
						iRet = PD_ERR;
						strcpy(csStatus, "MOBILE_NUM_NOT_MATCH");
					}
				}
...
DEBUGLOG(("ProcessSmsText:: call DBOLBankAccts::GetDepositBankAccts()\n"));
		DBObjPtr = CreateObj(DBPtr, "DBOLBankAccts", "GetDepositBankAccts");
		iTmpRet = (unsigned long)(*DBObjPtr)(csIntBankCode, csWholeBankAcctNum, iBankAcctNumLen, hRec);
		if (iTmpRet != PD_OK) {
DEBUGLOG(("ProcessSmsText:: call DBOLSmsFormat::GetDepositBankAccts() failed\n"));
			iRet = PD_ERR;
			strcpy(csStatus, "BANK_ACCT_NOT_FOUND");
