...
DEBUGLOG(("CreateCustomerGroup DBCustomerGroupMerchant: GetMerchantPhase phase = [%d]\n",iPhase));
						...
						PutField_CString(hRec,"merchant_id",csMerchantId);
						PutField_CString(hRec,"org_customer_group",csCustomerGroup);
						PutField_CString(hRec,"customer_tag",csCustomerTag);
						PutField_Int(hRec,"update_customer_detail",PD_TRUE);	

						// Convert Customer Group  
						iRet = ConvertCustomerGroup(hRec);
				
						if (iRet == PD_OK) {
							if (GetField_CString(hRec,"new_customer_group",&csNewCustomerGroup)) {			
								sprintf(csCustomerGroup, csNewCustomerGroup);
								PutField_CString(hContext,"customer_group",csCustomerGroup);
DEBUGLOG(("CreateCustomerGroup ConvertCustomerGroup: customer_group (new) = [%s]\n",csCustomerGroup));
							}
						}
					}
				}	
...
int ConvertCustomerGroup(hash_t* hTxn)
{
	...
	int 	iUpdateCustomerDetail = PD_FALSE;
	char* 	csMerchantId = NULL;
	char* 	csOrgCustomerGroup = NULL;
	char* 	csCustomerTag = NULL;

	char	csNewCustomerGroup[PD_CUSTOMER_GROUP_CODE_LEN + 1];
...
/* merchant_id */
	if (GetField_CString(hTxn,"merchant_id",&csMerchantId)) {
DEBUGLOG(("ConvertCustomerGroup: merchant_id = [%s]\n",csMerchantId));
        }

/* org_customer_group */
        if (GetField_CString(hTxn,"org_customer_group",&csOrgCustomerGroup)) {
DEBUGLOG(("ConvertCustomerGroup: org_customer_group = [%s]\n",csOrgCustomerGroup));
        }

/* customer_tag */
	if (GetField_CString(hTxn,"customer_tag",&csCustomerTag)) {
DEBUGLOG(("ConvertCustomerGroup: customer_tag = [%s]\n",csCustomerTag));
        }

/* update_customer_detail */
        if (GetField_Int(hTxn,"update_customer_detail",&iUpdateCustomerDetail)) {
DEBUGLOG(("ConvertCustomerGroup: update_customer_detail = [%d]\n",iUpdateCustomerDetail));
        }

	// Check Is Current Group
	DBObjPtr = CreateObj(DBPtr,"DBCustomerGroupRules","IsCurrentGroup");
        if ((unsigned long)(DBObjPtr)(csMerchantId,csOrgCustomerGroup) == NOT_FOUND) {

		// Get New Customer Group
		DBObjPtr = CreateObj(DBPtr,"DBCustomerGroupRules","GetFromGroup");
        	if ((unsigned long)(DBObjPtr)(csMerchantId,csNewCustomerGroup) == FOUND) {
	
			if (iUpdateCustomerDetail == PD_TRUE) {
		      		// update customer group detail

            			PutField_CString(hRec,"merchant_id",csMerchantId);
            			PutField_CString(hRec,"customer_group",csNewCustomerGroup);
              			PutField_CString(hRec,"customer_tag",csCustomerTag);
               			PutField_CString(hRec,"update_user",PD_UPDATE_USER);

          			DBObjPtr = CreateObj(DBPtr,"DBCustomerGroupDetail","UpdateGroup");
           			iRet = (unsigned long)(DBObjPtr)(hRec);
DEBUGLOG(("ConvertCustomerGroup: iRet = [%d] new group [%s] for [%s][%s]\n",iRet,csNewCustomerGroup,csMerchantId,csCustomerTag));

           			if (iRet == PD_OK) {
					// add change group log	
	
                 			PutField_CString(hRec,"from_customer_group",csOrgCustomerGroup);
                  			PutField_CString(hRec,"to_customer_group",csNewCustomerGroup);
                      		
					DBObjPtr = CreateObj(DBPtr,"DBCustomerGroupLog","Add");
                   			iRet = (unsigned long)(DBObjPtr)(hRec);
DEBUGLOG(("ConvertCustomerGroup: iRet = [%d] old group [%s] to new group [%s] for [%s]\n",iRet,csOrgCustomerGroup,csNewCustomerGroup,csMerchantId));
           			}
			}
	
			PutField_CString(hTxn,"new_customer_group",csNewCustomerGroup);
DEBUGLOG(("ConvertCustomerGroup: new_customer_group = [%s]\n",csNewCustomerGroup));
		}
       	}
...
DEBUGLOG(("ConvertCustomerGroup: iRet = [%d]\n",iRet));
        return iRet;
}
