...
DEBUGLOG(("handle_PayoutApprove::call BOMerchant->GetMerchantTxnInfo\n"));
	...
	if(iRet==PD_OK){
		double	dSrcFee = 0.0;
		double	dNetAmt = 0.0;
		if(GetField_Double(hContext,"total_src_txn_fee",&dSrcFee)){
		...
		if(GetField_Double(hContext,"net_amt",&dNetAmt)){
			PutField_Double(hContext,"org_txn_amt",dNetAmt-dSrcFee);
		}
...
DEBUGLOG(("GetPspAssignedTxn::payout_amount = [%lf]\n",dPayoutAmt));
		...
		if(iOnlineResp){
			...
			dPayoutAmt = 0.0;
			recordset_init(rRecordSet,0);
DEBUGLOG(("handle_PayoutPreSend::Call DBMerchantUploadFileDetail:GetExistingPsp\n"));
        		DBObjPtr = CreateObj(DBPtr,"DBMerchantUploadFileDetail","GetExistingPsp");
			if((unsigned long)(*DBObjPtr)(hRequest,rRecordSet)==PD_OK){
				//if(!GetField_Int(hResponse,"total_cnt",&i))
				i=0;
...
DEBUGLOG(("BOPayout::handle_PayoutCancelBeforeApproved() batch_id = [%s]\n",csTmp));
	...
	PutField_CString(hRequest,"txn_code",PD_PAYOUT_CANCEL_BEFORE_APPROVED);
	...
	if ((*DBObjPtr)((unsigned long)lBatchId,rRecordSet) == PD_OK) {
		...
		while (hRec) {
			if (GetField_CString(hRec,"txn_seq",&csTmp)) {
				...
				PutField_Char(hHeader,"ar_ind",PD_REJECTED);

				if(GetField_Char(hRequest,"action_party",&cTmp)){
DEBUGLOG(("handle_PayoutCancelBeforeApproved::Cancel Party[%c]\n",cTmp));
...
DEBUGLOG(("BOPayout::handle_PayoutCancelApproved() batch_id = [%s]\n",csTmp));
	...
	if(iRet == PD_OK || iRet == INT_NOT_RECORD || iRet == INT_NO_PENDING_RECORD){
		if(GetField_Int(hRequest,"status",&iStatus)){
			if(iStatus!=PD_PAYOUTFILE_APPROVED){
				if(!((iStatus==PD_PAYOUTFILE_UPLOADED)
				   ||(iStatus==PD_PAYOUTFILE_PROCESSING))){
					iRet = INT_INVALID_TXN;
					PutField_Int(hContext,"internal_error",iRet);
...
DEBUGLOG(("BOPayout::Call handle_PayoutCancelBeforeApproved()\n"));
			...
			iRet = INT_INVALID_TXN;
			PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("handle_PayoutCancelApproved() GetPayoutRecords:status not found!!!\n"));
ERRLOG("BOPayout::handle_PayoutCancelApproved() GetPayoutRecords:status not found!!!\n");
		}
	}
	...
	if(iRet==PD_OK){
		...
		BOObjPtr = CreateObj(BOPtr,"BOBalance","UpdatePayoutAmount");
		if((unsigned long)(*BOObjPtr)(hContext)!=PD_OK){
DEBUGLOG(("handle_PayoutCancelApproved::UpdatePayoutAmount Failed!!!\n"));
ERRLOG("BOPayout::handle_PayoutCancelApproved:UpdatePayoutAmount Failed!!!\n");
			iRet = INT_ERR;
		}
	}
...
DEBUGLOG(("GetMerchantDetail::client_id= [%s]\n",csTmp));
	...
	if(iRet==PD_OK){
		recordset_init(rRecordSet,0);
DEBUGLOG(("BOPayout::Call DBMerchantUploadFileDetail:GetUnvoidedDetail\n"));
                DBObjPtr = CreateObj(DBPtr,"DBMerchantUploadFileDetail","GetUnvoidedDetail");
                if ((*DBObjPtr)((unsigned long)lBatchId,rRecordSet) == PD_OK) {
                        i = 0;
...
int GetPayoutDetailByCustomMode(hash_t *hContext,
				hash_t* hRequest,
				hash_t* hResponse)
{
	int	iRet = PD_OK;
...
DEBUGLOG(("GetPayoutDetailByCustomMode::total_cnt = [%d]\n", iCnt));
		//PutField_Int(hResponse, "total_cnt", iCnt);
	}
	else {
DEBUGLOG(("GetPayoutDetailByCustomMode::total_cnt not found\n"));
ERRLOG("BOPayout::GetPayoutDetailByCustomMode::total_cnt not found\n");
		iRet = INT_FORMAT_ERR;
		PutField_Int(hContext,"internal_error",iRet);
	}
...
DEBUGLOG(("GetPayoutDetailByCustomMode:: [%s] = [%s]\n",csTag,csTxnId));
...
int GetPayoutDetailByBatch(hash_t *hContext,
			   hash_t* hRequest,
			   hash_t* hResponse)
{
	int	iRet = PD_OK;
...
/* batch_id */
	if (GetField_CString(hRequest, "batch_id", &csBatchId)) {
		lBatchId = ctol((const unsigned char *)csTmp,strlen(csTmp));
		//PutField_CString(hTxn,"batch_id",csTmp);
DEBUGLOG(("GetPayoutDetailByBatch::batch_id = [%s]\n", csBatchId));
	}
	else {
DEBUGLOG(("GetPayoutDetailByBatch::batch_id not found\n"));
ERRLOG("BOPayout::GetPayoutDetailByBatch::batch_id not found\n");
		iRet = INT_FORMAT_ERR;
		PutField_Int(hContext,"internal_error",iRet);
	}
