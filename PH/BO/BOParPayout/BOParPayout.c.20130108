...
DEBUGLOG(("BOParPayout:ProcessPayoutTxn()\n"));
...
DEBUGLOG(("BOParPayout::ProcessPayoutTxn() unknown txn_code!!!\n"));
ERRLOG("BOParPayout::ProcessPayoutTxn() unknown txn_code!!!\n");
		iRet = INT_ERR;
	}
...
DEBUGLOG(("BOParPayout::ProcessPayoutTxn() iRet[%d]\n",iRet));
	FREE_ME(hTxn);
	FREE_ME(hHeader);
	return	iRet;
}
...
DEBUGLOG(("BOParPayout::handle_PayoutPreGenerate()\n"));
...
DEBUGLOG(("BOParPayout::handle_PayoutPreSend()\n"));
	if(GetField_CString(hRequest,"merchant_id",&csTmp)){
DEBUGLOG(("BOParPayout::handle_PayoutPreSend() merchant_id [%s]\n",csTmp));
	}
...
DEBUGLOG(("BOParPayout::handle_PayoutSend()\n"));
...
DEBUGLOG(("handle_PayoutSend::Add BPayoutGeneratedFileDT Failed!!!!\n"));
			...
			if(iRet==PD_OK){
				...
				if ((*DBObjPtr)(csBatchId,iTmp,hTxn) != PD_OK) {
					iRet = INT_ERR;
DEBUGLOG(("BOParPayout::DBMerchantUploadFileDetail:UpdateDetailByBatchId Failed\n"));
					PutField_Int(hContext,"internal_error",iRet);
					break;
				}
			}
			...
			if(iRet==PD_OK){
				...
				if ((unsigned long)(*DBObjPtr)(hOrgHd) != PD_OK) {
					iRet = INT_ERR;
DEBUGLOG(("BOParPayout::DBTransaction:Update Failed\n"));
				}
			}
			hRec = RecordSet_GetNext(rRecordSet);
		}
...
DEBUGLOG(("BOParPayout::handle_PayoutSend Add PayoutQueueHd Success\n"));
							}
							else{
								iRet=INT_ERR;
DEBUGLOG(("BOParPayout::handle_PayoutSend Add PayoutQueueHd Failed!!!!\n"));
								PutField_Int(hContext,"internal_error",iRet);
								break;
							}
...
ERRLOG("BOParPayout::handle_PayoutSend:WritePayoutFile Error!!!\n");
								}
							}//end for
							FREE_ME(hFile);
						}
		...
		iRet = INT_NO_PENDING_RECORD;
DEBUGLOG(("BOParPayout::handle_PayoutSend: No Online Payout Records \n"));
	}
...
int handle_to_PayoutGenerate(hash_t *hContext,
			  ...
			  hash_t* hHeader)
{
DEBUGLOG(("BOParPayout::handle_to_PayoutGenerate()\n"));
...
DEBUGLOG(("BOParPayout::handle_PayoutGenerate()\n"));
...
ERRLOG("BOParPayout::GetPreGenHeader::txn_country not found!!\n");
				iRet=INT_ERR;
				break;
                	}
...
DEBUGLOG(("BOParPayout::handle_PayoutGenerate: GetPspDetail - client id = [%s]\n",csTmp));
					}
				}
				FREE_ME(hPsp);
			}
...
ERRLOG("BOParPayout::handle_PayoutGenerate:WritePayoutFile Error!!!\n");
					}
...
ERRLOG("BOParPayout::handle_PayoutGenerate:DBPayoutGeneratedFileDT Update Failed!!!\n");
					}
				}
			}
...
ERRLOG("BOParPayout::handle_PayoutGenerate:HandlePayoutBalance Failed!!!\n");
					iRet = INT_ERR;
				}
			}
...
ERRLOG("BOParPayout::handle_PayoutIndvCancel:UpdatePayoutAmount Failed!!!\n");
				iRet = INT_ERR;
			}
		}
...
ERRLOG("BOParPayout::handle_PayoutIndvCancel:DBPayoutGeneratedFileDT UpdateByGenId Failed!!!\n");
			}
...
DEBUGLOG(("BOParPayout::UpdateHeader()\n"));
...
DEBUGLOG(("UpdateHeader::DBMerchantUploadFileHeader:UpdateHeader Failed\n"));
	...
	RecordSet_Destroy(rRecordSet);
        FREE_ME(rRecordSet);
DEBUGLOG(("BOParPayout::UpdateHeader() iRet=[%d]\n",iRet));
	return  iRet;
}
...
DEBUGLOG(("BOParPayout:GetPayoutRecordsByMode()\n"));
...
DEBUGLOG(("BOParPayout:GetPayoutRecords()\n"));

	if(GetField_Int(hTxn,"current_index",&i)){
DEBUGLOG(("BOParPayout:GetPayoutRecords: current_index [%d]\n",i));
	}
	if(GetField_Double(hTxn,"total_amt",&dLimitedAmt)){
DEBUGLOG(("BOParPayout:GetPayoutRecords: total_amt [%lf]\n",dLimitedAmt));
	}
	if(GetField_Char(hContext,"input_ind",&cInputInd)){
DEBUGLOG(("BOParPayout:GetPayoutRecords::input_ind = [%c]\n",cInputInd));
	}
...
ERRLOG("BOParPayout::GetPayoutRecords::GetDetailByCondition::no record found!!\n");
						iRet=INT_NOT_RECORD;
						PutField_Int(hContext,"internal_error",iRet);
					}
					break;
...
DEBUGLOG(("BOParPayout::UpdateDetail for batch[%s]\n",csBatchId));
	}
	if (GetField_CString(hRequest,"add_user",&csTmp)){
		PutField_CString(hTxn,"update_user",csTmp);
	}
...
DEBUGLOG(("BOParPayout::UpdateDetail approve_id[%d]\n",iTmp));
...
DEBUGLOG(("BOParPayout::UpdateDetail seq_num[%d]\n",iSeqNum));
		}
		else{
DEBUGLOG(("BOParPayout::UpdateDetail seq_num is missing!!!\n"));
			continue;
		}
...
//*******************
DEBUGLOG(("Prepare to call MerchantUploadFileDetail.UpdateDetailByBatchId\n"));
//*******************
...
DEBUGLOG(("BOParPayout:UpdateDetail iRet=[%d]\n",iRet));
	FREE_ME(hTxn);
	return  iRet;
}
...
DEBUGLOG(("BOParPayout:InsertGenFileDT\n"));
...
DEBUGLOG(("InsertGenFileDT::DBPayoutGeneratedFileDT:Add Failed\n"));
	...
	FREE_ME(hTxn);
DEBUGLOG(("BOParPayout:InsertGenFileDT iRet=[%d]\n",iRet));
	return  iRet;
}
...
ERRLOG("BOParPayout::CheckAvalBalance::GetAvalPayoutBalance Error!!\n");
		iRet = INT_ERR;
	}
...
ERRLOG("BOParPayout::CheckAvalBalance::insufficient aval payout balance!!\n");
			}
		}
	}
...
DEBUGLOG(("BOParPayout:GetPayoutFee()\n"));
...
ERRLOG("BOParPayout:GetPayoutFee::::GetExchangeInfo Error\n");
			}
		}
	}
	//else{
...
ERRLOG("BOParPayout::GetPayoutFee:GetTxnFee Error\n");
			}
		}
		else{
			iIntErr = INT_ERR;
...
DEBUGLOG(("BOParPayout:GetPayoutFee() iRet=[%d]\n",iRet));
	return 	iRet;
}
...
DEBUGLOG(("BOParPayout:GetPayoutPsp()\n"));

 	if (GetField_CString(hContext,"bank_name",&csBankName)) {
DEBUGLOG(("BOParPayout::GetPayoutPsp() bank_name = [%s]\n",csBankName));
        }
        else {
DEBUGLOG(("BOParPayout::GetPayoutPsp() bank_name not found!!!\n"));
		iRet = INT_BANK_CODE_NOT_FOUND;
		PutField_Int(hContext,"internal_error",iRet);
        }

 	if (GetField_Double(hContext,"txn_amt",&dOrgTxnAmt)) {
DEBUGLOG(("BOParPayout::GetPayoutPsp() txn_amt = [%lf]\n",dOrgTxnAmt));
        }
        else {
DEBUGLOG(("BOParPayout::GetPayoutPsp() txn_amt not found!!!\n"));
		iRet = INT_TXN_AMT_MISSING;
		PutField_Int(hContext,"internal_error",iRet);
        }

	if (GetField_CString(hContext,"operator_type",&csOperType)) {
DEBUGLOG(("BOParPayout:GetPayoutPsp() operator_type = [%s]\n",csOperType));
	}
	if (GetField_CString(hContext,"psp_id",&csPspId)) {
DEBUGLOG(("BOParPayout:GetPayoutPsp() psp_id = [%s]\n",csPspId));
	}
	if (GetField_Double(hContext,"total_amt",&dPspAmt)) {
DEBUGLOG(("BOParPayout:GetPayoutPsp() total_amt = [%f]\n",dPspAmt));
	}

	sprintf(csTag,"%s_total_payout_amt_%s",csPspId,csBankName);
 	if (GetField_Double(hContext,csTag,&dTotalPayoutAmt)) {
DEBUGLOG(("BOParPayout::GetPayoutPsp() org total_payout_amt = [%lf]\n",dTotalPayoutAmt));
        }
...
DEBUGLOG(("BOParPayout:GetPayoutPsp() amount_min = [%f]\n",dMinAmt));
				}
				if (GetField_Double(hRec,"amt_max",&dMaxAmt)) {
DEBUGLOG(("BOParPayout:GetPayoutPsp() amount_max = [%f]\n",dMaxAmt));
				}

				if(dOrgTxnAmt>=dMinAmt && dOrgTxnAmt<=dMaxAmt){
					if (GetField_Int(hRec,"tier_id",&iTier)) {
DEBUGLOG(("BOParPayout:GetPayoutPsp() tier_id = [%d]\n",iTier));
					}
					if (GetField_Double(hRec,"amt_pct",&dPct)) {
DEBUGLOG(("BOParPayout:GetPayoutPsp() amount_pct = [%lf]\n",dPct));
DEBUGLOG(("BOParPayout:GetPayoutPsp() amount for tier[%d] = [%lf]\n",iTier,(dPspAmt*dPct/100)));
					}
				}
...
DEBUGLOG(("BOParPayout:GetPayoutPsp() Payout Amt[%lf] > Limited PSP Amt[%f]!!!\n",(dTotalPayoutAmt + dOrgTxnAmt),dPspAmt));
...
DEBUGLOG(("BOParPayout::GetPayoutPsp() %s (org) = [%lf], (new) = [%lf]\n",csTag,dTotalPayoutAmt,(dTotalPayoutAmt+dOrgTxnAmt)));
			}
		}
...
DEBUGLOG(("BOParPayout:GetPayoutPsp() exit = [%d]\n",iRet));
	return iRet;
}
...
DEBUGLOG(("BOParPayout:GetPayoutRule()\n"));
DEBUGLOG(("BOParPayout::Call DBRulePayoutPsp:GetRulePayoutPspWithPriority\n"));
...
DEBUGLOG(("BOParPayout: GetPspPayoutSplitAmt - split amount= [%lf]\n",dTmp));
					}
					else{
DEBUGLOG(("BOParPayout: GetPspPayoutSplitAmt - no split amount found\n"));
					}
				}
...
ERRLOG("BOParPayout::GetPayoutRule:GetRulePayoutPspWithPriority::not found record!!\n");
	...
	FREE_ME(rRecordSet);
	FREE_ME(hTxn);
DEBUGLOG(("BOParPayout:GetPayoutPsp() iRet=[%d]\n",iRet));
	return iRet;
}
...
DEBUGLOG(("BOParPayout:AssignPsp()\n"));
...
DEBUGLOG(("BOParPayout:AssignPsp() iRet=[%d]\n",iRet));
	return iRet;
}
...
DEBUGLOG(("BOParPayout:FormatResponse()\n"));
...
DEBUGLOG(("BOParPayout:FormatResponse() iRet=[%d]\n",iRet));
	return iRet;
}
...
DEBUGLOG(("BOParPayout:GetPayoutFilePath filename missing!!!!!\n"));
		iRet = INT_ERR;
	}
	if(!GetField_Int(hTxn,"seq_num",&iSeqNum)){
DEBUGLOG(("BOParPayout:GetPayoutFilePath seq_num missing!!!!!\n"));
		iRet = INT_ERR;
	}
	if(!GetField_CString(hTxn,"file_date",&csDate)){
DEBUGLOG(("BOParPayout:GetPayoutFilePath file_date missing!!!!!\n"));
		iRet = INT_ERR;
	}
	if(!GetField_CString(hTxn,"psp_id",&csPspId)){
DEBUGLOG(("BOParPayout:GetPayoutFilePath psp_id missing!!!!!\n"));
		iRet = INT_ERR;
	}
	...
	sprintf(csPayOutFilePath, "%s/%.*s/%.*s/%.*s/%d", getenv("OUTPUT_PAYOUT"),
							...
							PD_DATE_LEN,csDate,
							iSeqNum);
DEBUGLOG(("BOParPayout:GetPayoutFilePath FilePath[%s]\n",csPayOutFilePath));
...
DEBUGLOG(("BOParPayout:GetPayoutFilePath Whole FilePath[%s]\n",csPayOutFile));
	sprintf(csTag,"%s_path",csPspId);
...
DEBUGLOG(("BOParPayout:CreatePayoutFileDetail()\n"));
...
DEBUGLOG(("BOParPayout:CreatePayoutFileDetail() iRet=[%d]\n",iRet));
	return iRet;
}
...
DEBUGLOG(("BOParPayout:CreatePayoutFile()\n"));

	sprintf(csTag,"%s_path",csPspId);
	if(GetField_CString(hContext,csTag,&csPayOutFile)){
DEBUGLOG(("BOParPayout:CreatePayoutFile() writ to file[%s]\n",csPayOutFile));
	}
...
ERRLOG("BOParPayout:CreatePayoutFile::Open Payoutfile problem!!\n");
			iRet=INT_ERR;
		}
	}
	...
	else if(iRet==PD_OK && !strcmp(csPspId,PD_CHANNEL_TWV)) {
	...
	fclose(fp);
DEBUGLOG(("BOParPayout:CreatePayoutFile() iRet=[%d]\n",iRet));
	return iRet;
}
...
DEBUGLOG(("BOParPayout:WriteAssignedRecord()\n"));
	if (GetField_CString(hRequest,"add_user",&csTmp)){
...
DEBUGLOG(("BOParPayout:WriteAssignedRecord() batch_mode [%c]\n",cBatchMode));
	}
...
ERRLOG("BOParPayout:WriteAssignedRecord::GetPspAssignedTxn::Failed!!\n");
		iRet=INT_NOT_RECORD;
	}
...
DEBUGLOG(("BOParPayout:WritePayoutFile()\n"));
	FILE    *fp;
	fp = fopen(csPayOutFile, "a");
	if (fp == NULL) {
DEBUGLOG(("WritePayoutFile::Open Payoutfile [%s] problem\n", csPayOutFile));
ERRLOG("BOParPayout:WritePayoutFile::Open Payoutfile problem!!\n");
		iRet=INT_ERR;
	}
...
DEBUGLOG(("WritePayoutFile::psp_id [%s]\n", csPspId));
	...
	if(iRet==PD_OK && !GetField_CString(hRec,"psp_id",&csPspId)) {
...
ERRLOG("BOParPayout::GetDetail:DBPayoutGeneratedFileDT Update Failed!!!\n");
				}
			}
...
ERRLOG("BOParPayout::HandleCancelGenerate:HandlePayoutBalance Failed!!!\n");
			iRet = INT_ERR;
		}
	}
...
//***********************
DEBUGLOG(("HandlePayoutBalance::Call UpdateTxnLog\n"));
//***********************
			iRet = UpdateTxnLog_Par(hContext);
//***********************
DEBUGLOG(("HandlePayoutBalance::Call UpdateTxnLog iRet [%d]\n", iRet));
//***********************
			if(iRet!=PD_OK){
...
DEBUGLOG(("BOParPayout::UpdateDetailByTxn for batch[%s]\n",csBatchId));
	}

	if (GetField_Int(hContext,"seq_num",&iSeqNum)){
DEBUGLOG(("BOParPayout::UpdateDetailByTxn for seq_num[%d]\n",iSeqNum));
	}
	...
	if (GetField_CString(hContext,"txn_seq",&csTmp)){
		PutField_CString(hTxn,"txn_seq",csTmp);
DEBUGLOG(("BOParPayout::UpdateDetailByTxn txn_seq[%s]\n",csTmp));
	}

	if (GetField_CString(hContext,"aux_txn_seq",&csTmp)){
		PutField_CString(hTxn,"aux_txn_seq",csTmp);
DEBUGLOG(("BOParPayout::UpdateDetailByTxn aux_txn_seq[%s]\n",csTmp));
	}

	if(GetField_Int(hContext,"approve_id",&iTmp)){
		PutField_Int(hTxn,"approve_id",iTmp);
DEBUGLOG(("BOParPayout::UpdateDetailByTxn approve_id[%d]\n",iTmp));
	}

	if(GetField_Double(hContext,"dst_net_amt",&dTmp)){
	//if(GetField_Double(hContext,"payout_amt",&dTmp)){
		PutField_Double(hTxn,"payout_amt",dTmp);
DEBUGLOG(("BOParPayout::UpdateDetailByTxn payout_amt[%lf]\n",dTmp));
	}

	if(GetField_CString(hContext,"src_txn_fee_ccy",&csTmp)){
		PutField_CString(hTxn,"merchant_fee_ccy",csTmp);
DEBUGLOG(("BOParPayout::UpdateDetailByTxn merchant_fee_ccy[%s]\n",csTmp));
	}

	if(GetField_Double(hContext,"src_txn_fee",&dTmp)){
		PutField_Double(hTxn,"merchant_fee",dTmp);
DEBUGLOG(("BOParPayout::UpdateDetailByTxn merchant_fee[%lf]\n",dTmp));
	}

	if(GetField_CString(hContext,"dst_txn_fee_ccy",&csTmp)){
		PutField_CString(hTxn,"member_fee_ccy",csTmp);
DEBUGLOG(("BOParPayout::UpdateDetailByTxn member_fee_ccy[%s]\n",csTmp));
	}

	if(GetField_Double(hContext,"dst_txn_fee",&dTmp)){
		PutField_Double(hTxn,"member_fee",dTmp);
DEBUGLOG(("BOParPayout::UpdateDetailByTxn member_fee[%lf]\n",dTmp));
	}
	...
	if (GetField_Int(hContext,"status",&iTmp)){
		PutField_Int(hTxn,"status",iTmp);
DEBUGLOG(("BOParPayout::UpdateDetailByTxn status[%d]\n",iTmp));
	}
...
DEBUGLOG(("BOParPayout:UpdateDetailByTxn iRet=[%d]\n",iRet));
	FREE_ME(hTxn);
	return  iRet;
}
//
...
DEBUGLOG(("BOParPayout::HandleCancelUpload position[%d]\n",iPos));
	}
	
	sprintf(csTag,"batch_id_%d",iPos);
	if (GetField_CString(hRequest,csTag,&csTmp)){
		PutField_CString(hTxn,"batch_id",csTmp);
DEBUGLOG(("BOParPayout::HandleCancelUpload batch[%s]\n",csTmp));
	}

	sprintf(csTag,"seq_num_%d",iPos);
	if (GetField_Int(hRequest,csTag,&iTmp)){
		PutField_Int(hTxn,"seq_num",iTmp);
DEBUGLOG(("BOParPayout::HandleCancelUpload seq_num[%d]\n",iTmp));
	}

	PutField_Int(hTxn,"status",PAYOUT_MASTER_TRANSACTION_CANCELLED);

DEBUGLOG(("BOParPayout:HandleCancelUpload Call UpdateDetailByTxn\n"));
	iRet = UpdateDetailByTxn(hTxn,hRequest);

	FREE_ME(hTxn);
DEBUGLOG(("BOParPayout:HandleCancelUpload iRet[%d]\n",iRet));
	return	iRet;
}
...
int HandleCancelConfirm(hash_t *hContext, hash_t *hRequest)
{
	...
	if (GetField_Int(hContext,"position",&iPos)){
DEBUGLOG(("BOParPayout::HandleCancelConfirm position[%d]\n",iPos));
	}
...
DEBUGLOG(("BOParPayout::HandleCancelConfirm batch[%s]\n",csTmp));
	}
...
DEBUGLOG(("BOParPayout::HandleCancelConfirm seq_num[%d]\n",iTmp));
	}
/*
	if(iRet==PD_OK){
...
*/
	if(iRet==PD_OK){
		PutField_Int(hTxn,"status",PAYOUT_MASTER_TRANSACTION_CANCELLED);
DEBUGLOG(("BOParPayout:HandleCancelConfirm Call UpdateDetailByTxn\n"));
		iRet = UpdateDetailByTxn(hTxn,hRequest);
	}
/*	
	if(iRet==PD_OK){
...
*/
	FREE_ME(hTxn);
DEBUGLOG(("BOParPayout:HandleCancelConfirm iRet[%d]\n",iRet));
	return	iRet;
}
...
int HandleCancelApprove(hash_t *hContext, hash_t *hRequest)
{
	...
	if (GetField_Int(hContext,"position",&iPos)){
DEBUGLOG(("BOParPayout::HandleCancelApprove position[%d]\n",iPos));
	}
...
DEBUGLOG(("BOParPayout:GetPreApproveRecord()\n"));

	if(GetField_Int(hContext,"pre_approve_id",&iPreApproveId)){
DEBUGLOG(("BOParPayout:GetPreApproveRecord::pre_approve_id = [%d]\n",iPreApproveId));
	}
...
ERRLOG("BOParPayout::GetPreApproveRecord::GetDetailByPreApproveId::no record found!!\n");
		iRet=INT_NOT_RECORD;
		PutField_Int(hContext,"internal_error",iRet);
	}
...
//*********************
DEBUGLOG(("UpdateTxnLog_Par START!!!\n"));
//*********************
...
DEBUGLOG(("UpdateTxnLog::DBTxnPspDetail:Update Failed\n"));
	...
	FREE_ME(hTxn);
//*********************
DEBUGLOG(("UpdateTxnLog_Par END iRet [%d]!!!\n", iRet));
//*********************
	return iRet;
}
