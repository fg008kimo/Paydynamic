...
Handle New Fee Type on Online Deposit              2017/04/13              Virginia Yun
Modify Fee Type on Online Deposit for NG Mobile    2017/10/12              Elvis Wong
*/
...
int GetTFTNetAmount(hash_t *hContext,const hash_t* hRequest);
int GetRTTNetAmount(hash_t *hContext,const hash_t* hRequest);
int GetDspAmount(hash_t *hContext,const hash_t* hRequest);
int GetPayoutAmount(hash_t *hContext,const hash_t* hRequest);
int GetSettlementAmount(hash_t *hContext,const hash_t* hRequest);
...
DEBUGLOG(("BOFee::GetTxnFee() sub_txn_code = [%s]\n",csSubTxnCode));
			iSubTxnCode = PD_TRUE;
		}
	}
	else {
...
DEBUGLOG(("BOFee::GetTxnFee() %s = [%s]\n",csTag,csPtr));
		...
		if(!strcmp(csTxnCode,PD_SETTLEMENT_REQUEST)||
		   ...
		   //!strcmp(csTxnCode,PD_PAYOUT_VOID_MERCHANT) ||
		   ...
		   !strcmp(csTxnCode,PD_MERCHANT_BAL_TFF_VOID) ||
		   !strcmp(csTxnCode,PD_MERCHANT_BAL_TFT_VOID) ||
		   ...
		   !strcmp(csTxnCode,PD_OFL_AVA_PAYOUT_TRF_IN) ||
		   !strcmp(csTxnCode,PD_AVA_PAYOUT_APPROVE_TF_FROM) ||
		   !strcmp(csTxnCode,PD_AVA_PAYOUT_APPROVE_TF_TO) ||
   		   iAdjTxn == PD_TRUE)	 {
			sprintf(csOrgPayMethod,"%s",PD_ALL_METHOD);
DEBUGLOG(("BOFee::GetTxnFee() %s = [%s]\n",csTag,PD_ALL_METHOD));
	
			if (iSubTxnCode) {	
				if(!strcmp(csSubTxnCode,PD_PAYOUT_VOID)){
					sprintf(csOrgPayMethod,"%s",PD_ALL_METHOD);
DEBUGLOG(("BOFee::GetTxnFee() %s = [%s]\n",csTag,PD_ALL_METHOD));
				}
                        }
		   	...
		   	!strcmp(csTxnCode,PD_PAYOUT_APPROVE) ||
		   	!strcmp(csTxnCode,PD_OL_PAYOUT_APPROVE) ||
			!strcmp(csTxnCode,PD_REQ_OPL_TXN_CODE) ||
		   	!strcmp(csTxnCode,PD_PAYOUT_VOID_MERCHANT) ||
		   	!strcmp(csTxnCode,PD_OL_PAYOUT_VOID_MERCHANT) || 
		   	!strcmp(csTxnCode,PD_RETURN_POL_TXN_CODE) ){
			sprintf(csOrgPayMethod,"%s",PD_ALL_METHOD);
DEBUGLOG(("BOFee::GetTxnFee() %s = [%s]\n",csTag,PD_ALL_METHOD));
		}
		else{
...
DEBUGLOG(("BOFee::GetTxnFee() %s = [%s]\n",csTag,csOrgServiceCode));
			PutField_CString(hGetFeeType,"service_code",csOrgServiceCode);
        	}
        	else {
DEBUGLOG(("BOFee::GetTxnFee() %s is missing!!!\n",csTag));
        	}
...
DEBUGLOG(("BOFee::GetTxnFee() custom_tag bank_code = [%s]\n",csBankCode));
		PutField_CString(hRec,"bank_code",csBankCode);
		PutField_CString(hGetFeeType,"bank_code",csBankCode);
	}

/* Check Fee Type for Online Deposit */	
...
/*
		if (csBankCode != NULL) {
DEBUGLOG(("BOFee::GetTxnFee() DBBankFeeMapping.GetFeeTypeByBankCode\n"));
			DBObjPtr = CreateObj(DBPtr,"DBBankFeeMapping","GetFeeTypeByBankCode");
			if ((DBObjPtr)(csBankCode,csOutFeeType)) {
DEBUGLOG(("BOFee::GetTxnFee() Fee_Type by Bank = [%s]\n", csOutFeeType));
			}
		}
*/

		if ((csOrgServiceCode != NULL) || (csBankCode != NULL)) {
DEBUGLOG(("BOFee::GetTxnFee() DBServiceFeeMapping.GetFeeType\n"));
                        DBObjPtr = CreateObj(DBPtr,"DBServiceFeeMapping","GetFeeType");
                        if ((DBObjPtr)(hGetFeeType,csOutFeeType)) {
DEBUGLOG(("BOFee::GetTxnFee() Service [%s] and Bank [%s] -> Fee Type [%s]\n", csOrgServiceCode, csBankCode, csOutFeeType));
				PutField_Double(hContext,"src_txn_fee",0.0);
				PutField_Double(hContext,"dst_txn_fee",0.0);
                        }
		}
	}

	if(!strcmp(csTxnCode,PD_VOID_TXN_CODE)){
		iRet = FindFee(hRec,
			dTxnAmt,
			dDstTxnAmt,
			/*PD_TXN_CHG_TYPE_FEE,*/
...
DEBUGLOG(("[%s] [%s] [%s] [%s] [%s] [%s] [%s] [%s] [%s]\n", 
        csFeeType, csCountry, csChannelCode, csServiceCode, csTxnCode, 
	csCcy, csMerchantId, csMerchantClientId, csPayMethod));
...
DEBUGLOG(("BOFee:GetNETAmount() net amt for TFF = [%lf]\n",dNetAmt));
			PutField_Double(hContext,"net_amt",dNetAmt);
		}
/* TFT */
		else if (!strcmp(csTxnCode, PD_MERCHANT_BAL_TFT) ||
                         !strcmp(csTxnCode, PD_OFL_MERCHANT_BAL_TFT)) {
			iRet = GetTFTNetAmount(hContext,hRequest);
		}
/* RTT */
		else if (!strcmp(csTxnCode, PD_AVA_PAYOUT_APPROVE_TF_TO) ||
			 !strcmp(csTxnCode, PD_AVA_PAYOUT_REQ_TF_TO) ||
			 !strcmp(csTxnCode, PD_OFL_AVA_PAYOUT_TRF_IN) ||
			 !strcmp(csTxnCode, PD_OFL_AVA_PAYOUT_REQ_TF_TO)) {
			iRet = GetRTTNetAmount(hContext,hRequest);
		}
...
/*VOA ORI*/
		else if(!strcmp(csTxnCode, PD_PAYOUT_VOID_MERCHANT) ||
			!strcmp(csTxnCode, PD_OL_PAYOUT_VOID_MERCHANT) ||
			!strcmp(csTxnCode, PD_RETURN_POL_TXN_CODE) ){
			dNetAmt = dTxnAmt - dSrcTxnFee;// - dMarkupAmt;
DEBUGLOG(("BOFee:GetNETAmount() net amt for VOA/OVA/ORI = [%lf]\n",dNetAmt));
			PutField_Double(hContext,"net_amt",dNetAmt);
		}
/*STR*/		
		else if(!strcmp(csTxnCode, PD_SETTLEMENT_REQUEST) ||
			!strcmp(csTxnCode, PD_OL_SETTLEMENT_REQUEST)){
			iRet = GetSettlementAmount(hContext,hRequest);
			//dNetAmt = dTxnAmt; //+ dSrcTxnFee;
...
DEBUGLOG(("BOFee:GetNETAmount() net amt for VDS = [%lf]\n",dNetAmt));
			...
			else if(!strcmp(csTmp, PD_DEPOSIT_CHARGEBACK)){
                                dNetAmt = dTxnAmt + dSrcTxnFee;
DEBUGLOG(("BOFee:GetNETAmount() net amt for VCH = [%lf]\n",dNetAmt));
                        }
...
DEBUGLOG(("BOFee:GetTFTNETAmount()\n"));
/* set fee ccy to dst */
	if (GetField_CString(hContext,"dst_txn_ccy",&csTxnCcy)) {
		PutField_CString(hContext,"src_txn_fee_ccy",csTxnCcy);
DEBUGLOG(("BOFee:GetTFTNETAmount()  set fee ccy as dst \n"));
	}
	
	if (GetField_Double(hContext,"dst_txn_amt",&dDstTxnAmt)){
DEBUGLOG(("BOFee:GetTFTNETAmount() dst_txn_amt = [%lf]\n",dDstTxnAmt));
        }
       	if (GetField_Double(hContext,"markup_amt",&dMarkupAmt)){
DEBUGLOG(("BOFee:GetTFTNETAmount() markup_amt = [%lf]\n",dMarkupAmt));
        }

        GetField_Double(hContext,"src_txn_fee",&dTotalFee);
DEBUGLOG(("BOFee:GetTFTNETAmount() src txn fee = [%lf]\n",dTotalFee));

        dNetAmt = dDstTxnAmt - dTotalFee - dMarkupAmt;
DEBUGLOG(("BOFee:GetTFTNETAmount() net_amount = [%lf]\n",dNetAmt));
        PutField_Double(hContext,"net_amt",dNetAmt);
	return iRet;
}
...
DEBUGLOG(("BOFee:GetRTTNETAmount()\n"));

        if (GetField_CString(hContext,"dst_txn_ccy",&csTxnCcy)) {
                PutField_CString(hContext,"src_txn_fee_ccy",csTxnCcy);
DEBUGLOG(("BOFee:GetRTTNETAmount()  set fee ccy as dst [%s]\n", csTxnCcy));
        }

        if (GetField_Double(hContext,"dst_txn_amt",&dDstTxnAmt)){
DEBUGLOG(("BOFee:GetRTTNETAmount() dst_txn_amt = [%lf]\n",dDstTxnAmt));
        }

	/*
        if (GetField_Double(hContext,"markup_amt",&dMarkupAmt)){
DEBUGLOG(("BOFee:GetRTTNETAmount() markup_amt = [%lf]\n",dMarkupAmt));
        }
	*/

        GetField_Double(hContext,"dst_txn_fee",&dTotalFee);
DEBUGLOG(("BOFee:GetRTTNETAmount() dst txn fee = [%lf]\n",dTotalFee));

        dNetAmt = dDstTxnAmt - dTotalFee - dMarkupAmt;
DEBUGLOG(("BOFee:GetRTTNETAmount() net_amount = [%lf]\n",dNetAmt));
        PutField_Double(hContext,"net_amt",dNetAmt);
...
DEBUGLOG(("BOFee:GetSettlementAmount()\n"));

/* net amt */
        if (GetField_Double(hContext,"org_txn_amt",&dTxnAmt)){
DEBUGLOG(("BOFee:GetSettlementAmount() org txn_amt = [%lf]\n",dTxnAmt));
	}
        else if (GetField_Double(hContext,"txn_amt",&dTxnAmt)){
DEBUGLOG(("BOFee:GetSettlementAmount() txn_amt = [%lf]\n",dTxnAmt));
        }
/*
       	if (GetField_Double(hContext,"markup_amt",&dMarkupAmt)){
DEBUGLOG(("BOFee:GetSettlementAmount() markup_amt = [%lf]\n",dMarkupAmt));
        }
*/
        GetField_Double(hContext,"src_txn_fee",&dTotalSrcFee);
DEBUGLOG(("BOFee:GetSettlementAmount() src txn fee = [%lf]\n",dTotalSrcFee));

        dNetAmt = dTxnAmt -  dTotalSrcFee;
DEBUGLOG(("BOFee:GetSettlementAmount() net amount = [%lf]+[%lf]\n",dNetAmt,dTotalSrcFee));
        PutField_Double(hContext,"net_amt",dNetAmt);

/* psp txn amt */
/* converted amt */
        if (GetField_Double(hContext,"dst_txn_amt",&dConvertedAmt)){
DEBUGLOG(("BOFee:GetSettlementAmount() dst_txn_amt = [%lf]\n",dConvertedAmt));
        }


        GetField_Double(hContext,"dst_txn_fee",&dTotalDstFee);
DEBUGLOG(("BOFee:GetSettlementAmount() dst txn fe = [%lf]\n",dTotalDstFee));
        dDstTxnAmt = dConvertedAmt - dTotalDstFee;
DEBUGLOG(("BOFee:GetSettlementAmount() dst net amt = [%lf]\n",dDstTxnAmt));
        PutField_Double(hContext,"dst_net_amt",dDstTxnAmt);

DEBUGLOG(("BOFee:GetSettlementAmount() exit = [%d]\n",iRet));
        return iRet;
}
