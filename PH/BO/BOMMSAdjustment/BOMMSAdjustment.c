...
ERRLOG("BOMMSAdjustment:: AddAdjustmentType:: desc missing!!\n");
...
ERRLOG("BOMMSAdjustment:: AddAdjustmentType:: etype_cnt missing!!\n");
...
DEBUGLOG(("ProcessEntityNatureBalance: txn_amt = [%lf]\n",dTxnAmt));
        } else {
...
DEBUGLOG(("ProcessEntityNatureBalance: fee_amt = [%lf]\n",dFeeAmt));
        } else {
		dFeeAmt = 0.0;
DEBUGLOG(("ProcessEntityNatureBalance: fee_amt (by default) = [%lf]\n",dFeeAmt));		
	}
...
/* nature */
				if (GetField_Char(hAdjTypeRec, "nature", &cTmp)) {
...
/* system_defined */
				if (GetField_Int(hAdjTypeRec, "system_defined", &iTmp)) {
DEBUGLOG(("ProcessEntityNatureBalance::GetAdjustmentTypeRec::system_defined = [%d]\n", iTmp));
                        	}

/* allow_modify */
                        	if (GetField_Int(hAdjTypeRec, "allow_modify", &iTmp)) {
...
ERRLOG("BOMMSAdjustment::ProcessEntityNatureBalance::Call DBMmsAdjustmentTypeEntity::Find::Error!!\n");
	...
	if (iRet == PD_OK) {
		if (!strcmp(csAmtType, PD_DR)) {
			iRet = CheckCreditLimit(hContext, hData);
			if (iRet != PD_OK) {
				PutField_Int(hContext,"internal_error",iRet);
			}
		}
...
/* amt_type */
		PutField_CString(hData,"amt_type",csAmtType);		
...
DEBUGLOG(("ProcessEntityNatureBalance:: net_amt = [%lf]\n",dNetAmt));		
			...
			memset(csTag, 0, sizeof(csTag));
                        switch (cBalType) {
                                ...
                                case PD_MMS_BAL_TYPE_PREPAID:
                                        ...
                                        break;
                                default:
DEBUGLOG(("ProcessEntityNatureBalance:: bal_type invalid!!\n"));
ERRLOG("BOMMSAdjustment:: ProcessEntityNatureBalance:: bal_type invalid!!\n");
...
DEBUGLOG(("ProcessEntityNatureBalance:: amt_type invalid!!\n"));
ERRLOG("BOMMSAdjustment:: ProcessEntityNatureBalance:: amt_type invalid!!\n");
...
DEBUGLOG(("ProcessEntityNatureBalance::call BOMMSEntityBalance:%s Failure!!, iRet = [%d]\n",csTag,iRet));
ERRLOG("BOMMSAdjustment::ProcessEntityNatureBalance::call BOMMSEntityBalance:%s Failure!!, iRet = [%d]\n",csTag,iRet);
           		}
		}
        }	
...
/* user */
	...
	// Add Txn Element and Txn Element Detail for Fee
	if (iRet == PD_OK) {
...
/* bal_cnt */
        		if (GetField_Int(hData,"bal_cnt",&iBalCnt)) {
DEBUGLOG(("ProcessEntityNatureBalance: bal_cnt = [%d]\n",iBalCnt));
				PutField_Int(hTxnElementRec,"bal_cnt",iBalCnt);
        		} else {
DEBUGLOG(("ProcessEntityNatureBalance:: bal_cnt missing!!\n"));
ERRLOG("BOMMSAdjustment:: ProcessEntityNatureBalance:: bal_cnt missing!!\n");
...
/* nat_amt */
                        		...
                        		if (GetField_Double(hData,csTag,&dTmp)) {
DEBUGLOG(("ProcessEntityNatureBalance: nat_amt_%d = [%lf]\n",i,dTmp));
                        			sprintf(csTag,"bal.%d.element_amt",i);
                                		PutField_Double(hTxnElementRec, csTag, dTmp);
                        		} else {
DEBUGLOG(("ProcessEntityNatureBalance:: nat_amt missing!!\n"));
ERRLOG("BOMMSAdjustment:: ProcessEntityNatureBalance:: nat_amt missing!!\n");
...
/* nat_id */
                        		...
                        		if (GetField_CString(hData,csTag,&csTmp)) {
DEBUGLOG(("ProcessEntityNatureBalance: nat_id_%d = [%s]\n",i,csTmp));
                                		PutField_CString(hTxnElementRec, csTag, csTmp);
                        		} else {
DEBUGLOG(("ProcessEntityNatureBalance:: nat_id missing!!\n"));
ERRLOG("BOMMSAdjustment:: ProcessEntityNatureBalance:: nat_id missing!!\n");
...
DEBUGLOG(("ProcessEntityNatureBalance:: amount = [%lf]\n",dFeeAmt));	
...
//DEBUGLOG(("ProcessEntityNatureBalance:: add_user = [%s]\n",csUser));
			...
			if (iRet == PD_OK) {
				...
				// Add Txn Element and Txn Element Detail
...
int ProcessFxBalance(hash_t* hContext, hash_t* hData)
{
        ...
        hash_t  *hFxRec;
        hFxRec = (hash_t *)malloc(sizeof(hash_t));
        hash_init(hFxRec, 0);

DEBUGLOG(("ProcessFxBalance() Start!\n"));

/* txn_seq */
        if (GetField_CString(hData,"txn_seq",&csTmp)) {
DEBUGLOG(("ProcessFxBalance: txn_seq = [%s]\n",csTmp));
		PutField_CString(hFxRec,"fx_id",csTmp);
        } else {
DEBUGLOG(("ProcessFxBalance:: txn_id missing!!\n"));
ERRLOG("BOMMSAdjustment:: ProcessFxBalance:: txn_id missing!!\n");
                iRet = INT_TXN_ID_NOT_FOUND;
        }

/* code */
        if (GetField_CString(hData,"code",&csTmp)) {
DEBUGLOG(("ProcessFxBalance: code = [%s]\n",csTmp));
		PutField_CString(hFxRec,"code",csTmp);
        } else {
DEBUGLOG(("ProcessFxBalance:: code missing!!\n"));
ERRLOG("BOMMSAdjustment:: ProcessFxBalance:: code missing!!\n");
                iRet = INT_MMS_TXN_CODE_NOT_FOUND;
        }

/* entity_id */
        if (GetField_CString(hData,"entity_id",&csTmp)) {
DEBUGLOG(("ProcessFxBalance: entity_id = [%s]\n",csTmp));
		PutField_CString(hFxRec,"entity_id",csTmp);
        } else {
DEBUGLOG(("ProcessFxBalance:: entity_id missing!!\n"));
ERRLOG("BOMMSAdjustment:: ProcessFxBalance:: entity_id missing!!\n");
                iRet = INT_MMS_ENTITY_ID_NOT_FOUND;
        }

/* amt_type */
        if (GetField_CString(hData,"amt_type",&csAmtType)) {
DEBUGLOG(("ProcessFxBalance: amt_type = [%s]\n",csAmtType));
		if (!strcmp(csAmtType, PD_CR)) { 
			PutField_Char(hFxRec,"direction",PD_FUNDS_IN);
		} else if (!strcmp(csAmtType, PD_DR)) {
			PutField_Char(hFxRec,"direction",PD_FUNDS_OUT);
		}
        } else {
DEBUGLOG(("ProcessFxBalance:: amt_type missing!!\n"));
ERRLOG("BOMMSAdjustment:: ProcessFxBalance:: amt_type missing!!\n");
                iRet = INT_AMT_TYPE_NOT_FOUND;
      	}

/* user */
        if (GetField_CString(hData,"user",&csTmp)) {
DEBUGLOG(("ProcessFxBalance: user = [%s]\n",csTmp));
		PutField_CString(hFxRec,"user",csTmp);
		PutField_CString(hFxRec,"add_user",csTmp);
		PutField_CString(hFxRec,"update_user",csTmp);
        } else {
DEBUGLOG(("ProcessFxBalance:: user missing!!\n"));
ERRLOG("BOMMSAdjustment:: ProcessFxBalance:: user missing!!\n");
                iRet = INT_USER_NOT_FOUND;
        }
...
/* aff_fx_amt */
                if (GetField_Double(hData,"aff_fx_amt",&dTmp)) {
DEBUGLOG(("ProcessFxBalance: aff_fx_amt = [%lf]\n",dTmp));
                } else {
/*
DEBUGLOG(("ProcessFxBalance:: aff_fx_amt missing!!\n"));
ERRLOG("BOMMSAdjustment:: ProcessFxBalance:: aff_fx_amt missing!!\n");
                        iRet = INT_MMS_AFF_FX_AMT_NOT_FOUND;
*/
                }

/* non_aff_fx_amt */
                if (GetField_Double(hData,"non_aff_fx_amt",&dTmp)) {
DEBUGLOG(("ProcessFxBalance: non_aff_fx_amt = [%lf]\n",dTmp));
                } else {
/*
DEBUGLOG(("ProcessFxBalance:: non_aff_fx_amt missing!!\n"));
ERRLOG("BOMMSAdjustment:: ProcessFxBalance:: non_aff_fx_amt missing!!\n");
...
DEBUGLOG(("ProcessFxBalance: fx_cnt = [%d]\n",iFxCnt));
			PutField_Int(hFxRec,"fx_cnt",iFxCnt);
                } else {
DEBUGLOG(("ProcessFxBalance:: fx_cnt missing!!\n"));
ERRLOG("BOMMSAdjustment:: ProcessFxBalance:: fx_cnt missing!!\n");
...
/* fx_acr_ind */
                                ...
                                if (GetField_Int(hData,csTag,&iTmp)) {
DEBUGLOG(("ProcessFxBalance: fx_acr_ind_%d = [%d]\n",i,iTmp));
                                        PutField_Int(hFxRec, csTag, iTmp);
                                } else {
DEBUGLOG(("ProcessFxBalance:: fx_acr_ind missing!!\n"));
ERRLOG("BOMMSAdjustment:: ProcessFxBalance:: fx_acr_ind missing!!\n");
...
/* fx_occy */
                                ...
                                if (GetField_CString(hData,csTag,&csTmp)) {
DEBUGLOG(("ProcessFxBalance: fx_occy_%d = [%s]\n",i,csTmp));
                                        PutField_CString(hFxRec, csTag, csTmp);
                                } else {
DEBUGLOG(("ProcessFxBalance:: fx_occy missing!!\n"));
ERRLOG("BOMMSAdjustment:: ProcessFxBalance:: fx_occy missing!!\n");
...
/* fx_ccy */
                                ...
                                if (GetField_CString(hData,csTag,&csTmp)) {
DEBUGLOG(("ProcessFxBalance: fx_ccy_%d = [%s]\n",i,csTmp));
                                        PutField_CString(hFxRec, csTag, csTmp);
                                } else {
DEBUGLOG(("ProcessFxBalance:: fx_ccy missing!!\n"));
ERRLOG("BOMMSAdjustment:: ProcessFxBalance:: fx_ccy missing!!\n");
...
/* fx_amt */
                                ...
                                if (GetField_Double(hData,csTag,&dTmp)) {
DEBUGLOG(("ProcessFxBalance: fx_amt_%d = [%lf]\n",i,dTmp));
                                        PutField_Double(hFxRec, csTag, dTmp);
                                } else {
DEBUGLOG(("ProcessFxBalance:: fx_amt missing!!\n"));
ERRLOG("BOMMSAdjustment:: ProcessFxBalance:: fx_amt missing!!\n");
	...
	// Update Fx Balance
		...
		if (iFxCnt > 0) {	
			...
			// Record Affect Fx Detail
DEBUGLOG(("ProcessFxBalance:: Call BOMMSAverageCostRate RecordAffectFXDT \n"));
                	BOObjPtr = CreateObj(BOPtr, "BOMMSAverageCostRate","RecordAffectFXDT");
                	iRet = (unsigned long)(*BOObjPtr)(hFxRec);
                	if (iRet == PD_OK) {
DEBUGLOG(("ProcessFxBalance:: Call BOMMSAverageCostRate:: RecordAffectFXDT:: Success!!!\n"));

				// Calculate Affect Fx Detail
DEBUGLOG(("ProcessFxBalance:: Call BOMMSAverageCostRate CalculateAffectFXDT \n"));
                        	BOObjPtr = CreateObj(BOPtr, "BOMMSAverageCostRate","CalculateAffectFXDT");
                        	iRet = (unsigned long)(*BOObjPtr)(hFxRec);
                        	if (iRet == PD_OK) {
DEBUGLOG(("ProcessFxBalance:: Call BOMMSAverageCostRate:: CalculateAffectFXDT:: Success!!!\n"));
                        	} else {
DEBUGLOG(("ProcessFxBalance:: Call BOMMSAverageCostRate:: CalculateAffectFXDT:: Failure!!!, iRet = [%d]\n", iRet));
ERRLOG("BOMMSAdjustment::ProcessFxBalance::Call BOMMSAverageCostRate::CalculateAffectFXDT:: Failure!!!, iRet = [%d]\n", iRet);
                        	}
                	} else {
DEBUGLOG(("ProcessFxBalance:: Call BOMMSAverageCostRate:: RecordAffectFXDT:: Failure!!!, iRet = [%d]\n", iRet));
ERRLOG("BOMMSAdjustment::ProcessFxBalance::Call BOMMSAverageCostRate::RecordAffectFXDT:: Failure!!!, iRet = [%d]\n", iRet);
                	}
		}
	}
...
DEBUGLOG(("ProcessFxBalance() normal exit iRet = [%d]\n",iRet));
        return iRet;
}
...
int CheckCreditLimit(hash_t* hContext, hash_t* hData)
{
	...
	hash_t  *hCRLimitRec;
        hCRLimitRec = (hash_t *)malloc(sizeof(hash_t));
        hash_init(hCRLimitRec, 0);

	cClType = ' ';

DEBUGLOG(("CheckCreditLimit() Start!\n"));

/* cl_type */
        if (GetField_Char(hData,"cl_type",&cClType)) {
DEBUGLOG(("CheckCreditLimit: cl_type = [%c]\n",cClType));
                PutField_Int(hCRLimitRec, "cl_type", cClType);
        }

/* cl_rate */
        if (GetField_Double(hData,"cl_rate",&dClRate)) {
DEBUGLOG(("CheckCreditLimit: cl_rate = [%lf]\n",dClRate));
                PutField_Int(hCRLimitRec, "cl_rate", dClRate);
        }

/* cl_frate */
        if (GetField_Double(hData,"cl_frate",&dClFlatRate)) {
DEBUGLOG(("CheckCreditLimit: cl_frate = [%lf]\n",dClFlatRate));
                PutField_Int(hCRLimitRec, "cl_frate", dClFlatRate);
        }

/* cl_amt */
        if (GetField_Double(hData,"cl_amt",&dClAmt)) {
DEBUGLOG(("CheckCreditLimit: cl_amt = [%lf]\n",dClAmt));
                PutField_Int(hCRLimitRec, "cl_amt", dClAmt);
        }

/* txn_ccy */
        if (GetField_CString(hData,"txn_ccy",&csTxnCcy)) {
DEBUGLOG(("CheckCreditLimit: txn_ccy = [%s]\n",csTxnCcy));
                PutField_CString(hCRLimitRec,"ccy",csTxnCcy);
        } else {
DEBUGLOG(("CheckCreditLimit:: txn_ccy is missing\n"));
ERRLOG("BOMMSAdjustment::CheckCreditLimit:: txn_ccy is missing\n");
                iRet = INT_MMS_CCY_NOT_FOUND;
        }

/* txn_amt */
        if (GetField_Double(hData,"txn_amt",&dTxnAmt)) {
DEBUGLOG(("CheckCreditLimit: txn_amt = [%lf]\n",dTxnAmt));
                PutField_Double(hCRLimitRec,"txn_amt",dTxnAmt);
        } else {
DEBUGLOG(("CheckCreditLimit:: txn_amt is missing\n"));
ERRLOG("BOMMSAdjustment::CheckCreditLimit:: txn_amt is missing\n");
                iRet = INT_MMS_AMT_NOT_FOUND;
        }

/* user */
        if (GetField_CString(hData,"user",&csTmp)) {
DEBUGLOG(("CheckCreditLimit: user = [%s]\n",csTmp));
                PutField_CString(hCRLimitRec,"user",csTmp);
                PutField_CString(hCRLimitRec,"add_user",csTmp);
                PutField_CString(hCRLimitRec,"update_user",csTmp);
        } else {
DEBUGLOG(("CheckCreditLimit:: user missing!!\n"));
ERRLOG("BOMMSAdjustment:: CheckCreditLimit:: user missing!!\n");
                iRet = INT_USER_NOT_FOUND;
        }

	if (iRet == PD_OK) {
	
		// Check Credit Limit Type
		if ((cClType == PD_MMS_CREDIT_LIMIT_TYPE_POSITIVE) 
		    || (cClType == PD_MMS_CREDIT_LIMIT_TYPE_NEGATIVE)
		)
		{
			// Get Acct Balance Current Opening Balance
DEBUGLOG(("CheckCreditLimit:: Call BOMMSEntityBalance SelectEntityBalanceForUpdate\n"));
                	BOObjPtr = CreateObj(DBPtr,"BOMMSEntityBalance","SelectEntityBalanceForUpdate");
                	iRet = (unsigned long)(BOObjPtr)(hContext,hCRLimitRec);
                	if (iRet == PD_OK) {
DEBUGLOG(("CheckCreditLimit:: Call BOMMSEntityBalance:: SelectEntityBalanceForUpdate:: Success!!!\n"));

/* open_acct_bal */
				if (GetField_Double(hContext,"open_acct_bal",&dOpenAcctBal)) {
DEBUGLOG(("CheckCreditLimit:: Call BOMMSEntityBalance:: open_acct_bal = [%lf]\n",dOpenAcctBal));				
				}
                	} else {
DEBUGLOG(("CheckCreditLimit:: Call BOMMSEntityBalance:: SelectEntityBalanceForUpdate:: Failure!!!\n"));
ERRLOG("BOMMSAdjustment::CheckCreditLimit::Call BOMMSEntityBalance::SelectEntityBalanceForUpdate:: Failure!!!\n");
                	}
	
			// Calculate Credit Limit
        		if (iRet == PD_OK) {
				// 
				// Credit Limit Type: Positive
				// Condition: Set Open Acct Bal = 0.0, if Open Acct Bal < 0.0
				// Formula: Txn Amt <= Open Acct Bal + (Fixed Amt + (Open Acct Bal * Credit Limit Rate))
				//
				if (cClType == PD_MMS_CREDIT_LIMIT_TYPE_POSITIVE) {
...
DEBUGLOG(("CheckCreditLimit: open_acct_bal = [%lf]\n",dOpenAcctBal));
                
/*
					if (dClAmt != 0.0 ) {
                        			dCreditLimit = dOpenAcctBal + (dOpenAcctBal * dClRate + dClFlatRate);
DEBUGLOG(("CheckCreditLimit: credit_limit [%lf] = [%lf] + ([%lf] * [%lf] + [%lf])\n", dCreditLimit, dOpenAcctBal, dOpenAcctBal, dClRate, dClFlatRate));
                			} else {
                        			dCreditLimit = dOpenAcctBal;
DEBUGLOG(("CheckCreditLimit: credit_limit [%lf] = [%lf]\n",dCreditLimit, dOpenAcctBal));
                			}
*/
					dCreditLimit = dOpenAcctBal + (dOpenAcctBal * dClRate + dClFlatRate);
DEBUGLOG(("CheckCreditLimit: credit_limit [%lf] = [%lf] + ([%lf] * [%lf] + [%lf])\n", dCreditLimit, dOpenAcctBal, dOpenAcctBal, dClRate, dClFlatRate));
				...
				// 
				// Credit Limit Type: Negative
				// Condition: Return Error, if Open Acct Bal < 0.0
				// Formula: Txn Amt <= Open Acct Bal - (Fixed Amt + (Open Acct Bal * Credit Limit Rate))
				//
				else if (cClType == PD_MMS_CREDIT_LIMIT_TYPE_NEGATIVE) {

					if (dOpenAcctBal < 0.0) {
DEBUGLOG(("CheckCreditLimit:: Credit Limit (Negative), Open Acct Balance [%lf] < 0.0\n",dOpenAcctBal));
ERRLOG("BOMMSAdjustment::CheckCreditLimit:: Credit Limit (Negative), Open Acct Balance [%lf] < 0.0\n",dOpenAcctBal);
...
/*
						if (dClAmt != 0.0 ) {
                                         	       dCreditLimit = dOpenAcctBal + (dOpenAcctBal * dClRate + dClFlatRate) * (-1);
DEBUGLOG(("CheckCreditLimit: credit_limit [%lf] = [%lf] - ([%lf] * [%lf] + [%lf])\n", dCreditLimit, dOpenAcctBal, dOpenAcctBal, dClRate, dClFlatRate));
                                        	} else {
                                        	       dCreditLimit = dOpenAcctBal;
DEBUGLOG(("CheckCreditLimit: credit_limit [%lf] = [%lf]\n",dCreditLimit, dOpenAcctBal));
...
DEBUGLOG(("CheckCreditLimit: credit_limit [%lf] = [%lf] - ([%lf] * [%lf] + [%lf])\n", dCreditLimit, dOpenAcctBal, dOpenAcctBal, dClRate, dClFlatRate));
			...
			// Compare Txn Amount with Credit Limit
			if (iRet == PD_OK) {
				
				if (dTxnAmt <= dCreditLimit) {
DEBUGLOG(("CheckCreditLimit:: Adjustment Txn Amount [%lf] <= Credit Limit [%lf]\n",dTxnAmt,dCreditLimit));
                            	} else {
DEBUGLOG(("CheckCreditLimit:: Insufficient Adjustment Txn Amount [%lf] > Credit Limit [%lf]\n",dTxnAmt,dCreditLimit));
ERRLOG("BOMMSAdjustment::CheckCreditLimit:: Insufficient Adjustment Txn Amount [%lf] > Credit Limit [%lf]\n",dTxnAmt,dCreditLimit);
                                     	iRet = INT_MMS_INSUFFICIENT_TXN_AMT;
                             	}			
			}					
		} else {
DEBUGLOG(("CheckCreditLimit:: Invalid cl_type = [%C]\n",cClType));
ERRLOG("BOMMSAdjustment::CheckCreditLimit:: Invalid cl_type = [%c]\n",cClType);
                   	iRet = INT_ERR;		
		}
	}

	hash_destroy(hCRLimitRec);
        FREE_ME(hCRLimitRec);

DEBUGLOG(("CheckCreditLimit() normal exit iRet = [%d]\n",iRet));
        return iRet;
}
