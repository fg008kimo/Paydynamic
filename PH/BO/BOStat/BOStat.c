...
-------------------------------                    ------------            --------------
Init Version                                       2011/04/27              [GOD]
Add Count Request Txn Amt			   2012/01/17		   [GOD]
Add Customer Group Counter			   2013/09/11		   [GOD]
PRD304 Add Customer & PID Counters		   2021/03/17		   [ANC]
*/
                        ...
                        const hash_t *hRequest)
{
	...
	char	*csOrgChannelCode;
	char	*csOrgServiceCode;
	char	*csOrgCustomerGroup;
	char	*csOrgTxnCcy;
	char	*csOrgPostingDate;
	char	*csOrgMerchantId;
	char	*csOrgCustomerTag;
	//char	csMonth[PD_TIME_LEN];
	double	dOrgTxnAmt;


	if (GetField_CString(hContext,"host_posting_date",&csOrgPostingDate)) {
DEBUGLOG(("BOStat:UpdateCustomerGroup() Org Posting Date = [%s]\n",csOrgPostingDate));
		//memcpy(csMonth,csOrgPostingDate,PD_TIME_LEN);
		//csMonth[PD_TIME_LEN] = '\0';
        }
/*org customer tag*/
        if (GetField_CString(hContext,"org_customer_tag",&csOrgCustomerTag)) {
DEBUGLOG(("BOStat:UpdateCustomerGroup() org_customer_tag = [%s]\n",csOrgCustomerTag));
        }
        else {
DEBUGLOG(("BOStat:UpdateCustomerGroup() org_customer_group is missing!!!\n"));
        }

/*org customer group */
        if (GetField_CString(hContext,"org_customer_group",&csOrgCustomerGroup)) {
DEBUGLOG(("BOStat:UpdateCustomerGroup() org_customer_group = [%s]\n",csOrgCustomerGroup));
        }
        else {
DEBUGLOG(("BOStat:UpdateCustomerGroup() org_customer_group is missing!!!\n"));
        }
...
/*org country id */
        if (GetField_CString(hContext,"org_txn_country",&csOrgCountryId)) {
DEBUGLOG(("BOStat:UpdateCustomerGroup() org_txn_country = [%s]\n",csOrgCountryId));
        }
        else {
DEBUGLOG(("BOStat:UpdateCustomerGroup() org_txn_country is missing!!!\n"));
        }


/*org channel code */
        if (GetField_CString(hContext,"org_channel_code",&csOrgChannelCode)) {
DEBUGLOG(("BOStat:UpdateCustomerGroup() org_channel_code = [%s]\n",csOrgChannelCode));
        }
        else {
DEBUGLOG(("BOStat:UpdateCustomerGroup() org_channel_code is missing!!!\n"));
        }

/*org merchant id*/
        if (GetField_CString(hContext,"org_merchant_id",&csOrgMerchantId)) {
DEBUGLOG(("BOStat:UpdateCustomerGroup() org_merchant_id = [%s]\n",csOrgMerchantId));
        }
        else {
DEBUGLOG(("BOStat:UpdateCustomerGroup() org_service_code is missing!!!\n"));
        }

/*org sercice code */
        if (GetField_CString(hContext,"org_service_code",&csOrgServiceCode)) {
DEBUGLOG(("BOStat:UpdateCustomerGroup() org_service_code = [%s]\n",csOrgServiceCode));
        }
        else {
DEBUGLOG(("BOStat:UpdateCustomerGroup() org_service_code is missing!!!\n"));
        }
...
/*org txn ccy */
        if (GetField_CString(hContext,"org_txn_ccy",&csOrgTxnCcy)) {
DEBUGLOG(("BOStat:UpdateCustomerGroup() org_txn_ccy = [%s]\n",csOrgTxnCcy));
        }
        else {
DEBUGLOG(("BOStat:UpdateCustomerGroup() org_txn_ccy is missing!!!\n"));
        }


/*org psp txn amount */
        if (GetField_Double(hContext,"org_txn_amt",&dOrgTxnAmt)) {
DEBUGLOG(("BOStat:UpdateCustomerGroup() org_txn_amt = [%lf]\n",dOrgTxnAmt));
        }
	else {
DEBUGLOG(("BOStat:UpdateCustomerGroup() org_txn_amt is missing!!!\n"));
	}

	DBObjPtr = CreateObj(DBPtr,"DBCustomerGroupCounters","CounterCreate");
        if ((*DBObjPtr)(csOrgCustomerGroup,
                        csOrgCountryId,
			csOrgChannelCode,
			csOrgServiceCode,
                        csOrgTxnCcy,
			csOrgPostingDate,
                        ...
                        PD_UPDATE_USER) != PD_OK) {
                iRet = INT_ERR;
DEBUGLOG(("BOStat:UpdateDspReqMerchantStat() DBCustomerGroupCounters::CounterCreate Failed\n"));
        }

	if(iRet == PD_OK){
		DBObjPtr = CreateObj(DBPtr,"DBCustomerTxnHist","AddHist");
		if ((*DBObjPtr)(csOrgPostingDate,
				csOrgMerchantId,
				csOrgCustomerTag,
				csOrgCustomerGroup,
				dOrgTxnAmt,
				PD_UPDATE_USER) != PD_OK) {
			iRet = INT_ERR;
DEBUGLOG(("BOStat:UpdateDspReqMerchantStat() DBCustomerGroupCounters::AddHist Failed\n"));
		}
	}
...
/*org customer tag*/
	if (GetField_CString(hContext,"org_customer_tag",&csOrgCustomerTag)) {
DEBUGLOG(("BOStat:UpdateCustomerPspCounter() Customer Tag = [%s]\n",csOrgCustomerTag));
	}
	else {
DEBUGLOG(("BOStat:UpdateCustomerPspCounter() org_customer_tag is missing!!!\n"));
	}

/*org psp id*/
	if (GetField_CString(hContext,"org_psp_id",&csOrgPspId)) {
DEBUGLOG(("BOStat:UpdateCustomerPspCounter() PSP ID = [%s]\n",csOrgPspId));
	}
	else {
DEBUGLOG(("BOStat:UpdateCustomerPspCounter() org_psp_id is missing!!!\n"));
	}

/*host posting date*/
	if (GetField_CString(hContext,"host_posting_date",&csOrgPostingDate)) {
DEBUGLOG(("BOStat:UpdateCustomerPspCounter() Host Posting Date = [%s]\n",csOrgPostingDate));
	}

/*update category*/
	if (GetField_CString(hContext,"upd_category",&csCategory)) {
DEBUGLOG(("BOStat:UpdateCustomerPspCounter() Category = [%s]\n",csCategory));
	}
	else {
DEBUGLOG(("BOStat:UpdateCustomerPspCounter() upd_category is missing!!!\n"));
	}

/*update counter*/
	if (GetField_Double(hContext,"upd_counter",&dCounter)) {
DEBUGLOG(("BOStat:UpdateCustomerPspCounter() Counter = [%lf]\n",dCounter));
	}
	else {
DEBUGLOG(("BOStat:UpdateCustomerPspCounter() upd_counter is missing!!!\n"));
	}

	DBObjPtr = CreateObj(DBPtr,"DBCustomerPspCounters","CounterCreate");
	if ((*DBObjPtr)(csOrgCustomerTag,
			csOrgPspId,
			csOrgPostingDate,
			csCategory,
			dCounter,
			PD_UPDATE_USER) != PD_OK) {
				iRet = INT_ERR;
DEBUGLOG(("BOStat:UpdateCustomerPspCounter() DBCustomerPspCounters::CounterCreate Failed\n"));
	}

DEBUGLOG(("BOStat:UpdateCustomerPspCounter() iRet = [%d]\n",iRet));
	return iRet;
}
