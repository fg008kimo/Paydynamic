...
Deposit Cost Adjustment				   2014/05/28		   Stan Poon
Add for Offline CRR				   2015/03/20		   Dirk Wong
Add GetAllExRate, CalExAmt			   2015/08/06		   Dirk Wong
Add for MiniMMM CRR				   2015/12/22		   Dirk Wong
Handle online void merchant settlement		   2016/05/19		   Dirk Wong
Handle online void merch balance transfer	   2016/08/04		   Dirk Wong
...
int GetAllExRate(hash_t* hAllExRate, char* csEffectDate);
...
DEBUGLOG(("BOCrrJnl::Add(): csAccMonth = [%s]\n",csAccMonth));
	...
	double dTmp = 0.0;
	hash_t *hAllExRate  = (hash_t*) malloc (sizeof(hash_t));
	hash_init(hAllExRate,0);

	iRet = GetAllExRate(hAllExRate,csTxnDate);
...
DEBUGLOG(("BOCrrJnl::Add(): ex rate = [%lf] ex amt = [%lf]\n",dFxRate,dConvertAmt));
								...
								sprintf(csTag,"%s%s",csTxnCcy,csBaseCcy);
								if (GetField_Double(hAllExRate,csTag,&dTmp)) {
									dFxRate = dTmp;
									dConvertAmt = CalExAmt(dTxnAmt,dTmp,csBaseCcy);
...
int Build(hash_t* hJnlHdr)
{
	...
	int	iJnlTypeId, iSingle, iGroupId;
	char	*csPrefix=NULL, *csDesc=NULL;
...
DEBUGLOG(("BOCrrJnl::Build(): post_user = [%s]\n",csPostUser));
	...
	recordset_t *rJnl;
	rJnl = (recordset_t*) malloc (sizeof(recordset_t));
...
DEBUGLOG(("Cleanup Journal Begin \n"));
	...
	char cPostType;
	if (!strcmp(csProduct,"OFF"))
		cPostType = PD_JLT_POST_OFFLINE;
	else
		cPostType = PD_JLT_POST_ONLINE;

	if (!strcmp(csJnlType,PD_JLT_PX_ALL)) {
		DBObjPtr = CreateObj(DBPtr,"DBCrrJnlHeader","GetAllPostJnl");
		iRet = (unsigned long) ((*DBObjPtr)(rRec, csTxnDate, csCountry, csProduct, cPostType));
	} else {
		DBObjPtr = CreateObj(DBPtr,"DBCrrJnlHeader","GetPostJnl");
		iRet = (unsigned long) ((*DBObjPtr)(rRec, csTxnDate, csCountry, csProduct, csJnlType, cPostType));
	}
	if (iRet == PD_OK) {
...
DEBUGLOG(("!!GetApprovedDeposit NOT IN Auto Post List[%s]\n", PD_JLT_PX_ONDF));
			}
		}
	}
	...
	/* if (iRet == PD_OK) {
		if ((!strcmp(csJnlType,PD_JLT_PX_ALL)) || (!strcmp(csJnlType,PD_JLT_PX_ONDV))) {
			DBObjPtr = CreateObj(DBPtr,"DBCrrJnlType","IsAutoPostJnl");	
			if ((unsigned long)((*DBObjPtr)(PD_JLT_PX_ONDV))) {
				// GetVoidDeposit
...
DEBUGLOG(("!!GetVoidDeposit NOT IN Auto Post List[%s]\n", PD_JLT_PX_ONDV));
			}
		}
	}*/
	
	/* GetReconDeposit ONDA */
	if (iRet == PD_OK) {
		if ((!strcmp(csJnlType,PD_JLT_PX_ALL)) || (!strcmp(csJnlType,PD_JLT_PX_ONDA))) {
                        DBObjPtr = CreateObj(DBPtr,"DBCrrJnlType","IsAutoPostJnl");
                        if ((unsigned long)((*DBObjPtr)(PD_JLT_PX_ONDA))) {
				// GetReconDeposit
...
DEBUGLOG(("!!GetReconDeposit NOT IN Auto Post List[%s]\n", PD_JLT_PX_ONDA));
			}
		}
	}
	
	/* GetVoidReconDeposit ONDAV */
	if (iRet == PD_OK) {
		if ((!strcmp(csJnlType,PD_JLT_PX_ALL)) || (!strcmp(csJnlType,PD_JLT_PX_ONDAV))) {
                        DBObjPtr = CreateObj(DBPtr,"DBCrrJnlType","IsAutoPostJnl");
                        if ((unsigned long)((*DBObjPtr)(PD_JLT_PX_ONDAV))) {
				// GetVoidReconDeposit
...
DEBUGLOG(("!!GetVoidReconDeposit NOT IN Auto Post List[%s]\n", PD_JLT_PX_ONDAV));
			}
		}
	}

	/* GetDepositCostAdj AJDC */
	if (iRet == PD_OK) {
		if ((!strcmp(csJnlType,PD_JLT_PX_ALL)) || (!strcmp(csJnlType,PD_JLT_PX_AJDC))) {
			DBObjPtr = CreateObj(DBPtr,"DBCrrJnlType","IsAutoPostJnl");	
			if ((unsigned long)((*DBObjPtr)(PD_JLT_PX_AJDC))) {
				// GetDepositCostAdj
DEBUGLOG(("GetDepositCostAdj Begin\n"));
				recordset_init(rRec,0);
				DBObjPtr = CreateObj(DBPtr,"DBCrrJnlHeader","GetDepositCostAdj");
				if ((unsigned long) ((*DBObjPtr)(rRec, csTxnDate, csCountry, csProduct)) == PD_OK) {
					iRet = postSingleJournal("Adj-Deposit Cost", rRec, csPostUser); 
				} else {
					iRet = FAILURE;
				}
				RecordSet_Destroy(rRec);
DEBUGLOG(("GetDepositCostAdj End. Return:[%d]\n",iRet));
			} else {
DEBUGLOG(("!!GetDepositCostAdj NOT IN Auto Post List[%s]\n", PD_JLT_PX_ONDF));
			}
		}
	}

	/* GetApprovedPayout ONP */
	if (iRet == PD_OK) {
		if ((!strcmp(csJnlType,PD_JLT_PX_ALL)) || (!strcmp(csJnlType,PD_JLT_PX_ONP))) {
                        DBObjPtr = CreateObj(DBPtr,"DBCrrJnlType","IsAutoPostJnl");
                        if ((unsigned long)((*DBObjPtr)(PD_JLT_PX_ONP))) {
				// GetApprovedPayout
...
DEBUGLOG(("!!GetApprovedPayout NOT IN Auto Post List[%s]\n", PD_JLT_PX_ONP));
			}
		}
	}
	
	/* GetGeneratedPayout ONPA */
	if (iRet == PD_OK) {
		if ((!strcmp(csJnlType,PD_JLT_PX_ALL)) || (!strcmp(csJnlType,PD_JLT_PX_ONPA))) {
                        DBObjPtr = CreateObj(DBPtr,"DBCrrJnlType","IsAutoPostJnl");
                        if ((unsigned long)((*DBObjPtr)(PD_JLT_PX_ONPA))) {
				// GetGeneratedPayout
...
DEBUGLOG(("!!GetGeneratedPayout NOT IN Auto Post List[%s]\n", PD_JLT_PX_ONPA));
			}
		}
	}
	
	/* GetVoidApprovedPayout ONPV */
	if (iRet == PD_OK) {
		if ((!strcmp(csJnlType,PD_JLT_PX_ALL)) || (!strcmp(csJnlType,PD_JLT_PX_ONPV))) {
                        DBObjPtr = CreateObj(DBPtr,"DBCrrJnlType","IsAutoPostJnl");
                        if ((unsigned long)((*DBObjPtr)(PD_JLT_PX_ONPV))) {
...
DEBUGLOG(("!!GetVoidApprovedPayout NOT IN Auto Post List[%s]\n", PD_JLT_PX_ONPV));
			}
		}
	}
	
	/* GetVoidGenPayout ONPAV */
	if (iRet == PD_OK) {
		if ((!strcmp(csJnlType,PD_JLT_PX_ALL)) || (!strcmp(csJnlType,PD_JLT_PX_ONPAV))) {
                        DBObjPtr = CreateObj(DBPtr,"DBCrrJnlType","IsAutoPostJnl");
                        if ((unsigned long)((*DBObjPtr)(PD_JLT_PX_ONPAV))) {
				// GetVoidGenPayout
...
DEBUGLOG(("!!GetVoidGenPayout NOT IN Auto Post List[%s]\n", PD_JLT_PX_ONPAV));
			}
		}
	}
	...
	if (iRet == PD_OK) {
		if ((!strcmp(csJnlType,PD_JLT_PX_ALL)) || (!strcmp(csJnlType,PD_JLT_PX_ONMS))) {
                        DBObjPtr = CreateObj(DBPtr,"DBCrrJnlType","IsAutoPostJnl");
                        if ((unsigned long)((*DBObjPtr)(PD_JLT_PX_ONMS))) {
				// GetApprovedMst
...
DEBUGLOG(("!!GetApprovedMst NOT IN Auto Post List[%s]\n", PD_JLT_PX_ONMS));
			}
		}
	}

	/* GetDeliveredMst ONMSD */
	/*
	if (iRet == PD_OK) {
		if ((!strcmp(csJnlType,PD_JLT_PX_ALL)) || (!strcmp(csJnlType,PD_JLT_PX_ONMSD))) {
                        DBObjPtr = CreateObj(DBPtr,"DBCrrJnlType","IsAutoPostJnl");
                        if ((unsigned long)((*DBObjPtr)(PD_JLT_PX_ONMSD))) {
				// GetDeliveredMst
...
DEBUGLOG(("!!GetDeliveredMst NOT IN Auto Post List[%s]\n", PD_JLT_PX_ONMSD));
			}
		} 
	}
	*/
	...
	/* GetVoidMst VOMS */
	if (iRet == PD_OK) {
		if ((!strcmp(csJnlType,PD_JLT_PX_ALL)) || (!strcmp(csJnlType,PD_JLT_PX_VOMS))) {
                        DBObjPtr = CreateObj(DBPtr,"DBCrrJnlType","IsAutoPostJnl");
                        if ((unsigned long)((*DBObjPtr)(PD_JLT_PX_VOMS))) {
				// GetVoidMst
...
DEBUGLOG(("!!GetVoidMst NOT IN Auto Post List[%s]\n", PD_JLT_PX_VOMS));
			}
		}
	}


	/* GetApprovedMbt ONMTF */
	if (iRet == PD_OK) {
		if ((!strcmp(csJnlType,PD_JLT_PX_ALL)) || (!strcmp(csJnlType,PD_JLT_PX_ONMTF))) {
                        DBObjPtr = CreateObj(DBPtr,"DBCrrJnlType","IsAutoPostJnl");
                        if ((unsigned long)((*DBObjPtr)(PD_JLT_PX_ONMTF))) {
				// GetApprovedMbt
...
DEBUGLOG(("!!GetApprovedMbt NOT IN Auto Post List[%s]\n", PD_JLT_PX_ONMTF));
			}
		}
	}
	...
	/* GetVoidMbt ONMTF */
	if (iRet == PD_OK) {
		if ((!strcmp(csJnlType,PD_JLT_PX_ALL)) || (!strcmp(csJnlType,PD_JLT_PX_VMBT))) {
                        DBObjPtr = CreateObj(DBPtr,"DBCrrJnlType","IsAutoPostJnl");
                        if ((unsigned long)((*DBObjPtr)(PD_JLT_PX_VMBT))) {
				// GetVoidMbt
DEBUGLOG(("GetVoidMbt Begin\n"));
				recordset_init(rRec,0);
				DBObjPtr = CreateObj(DBPtr,"DBCrrJnlHeader","GetVoidMbt");
				if ((unsigned long) ((*DBObjPtr)(rRec, csTxnDate, csCountry, csProduct)) == PD_OK) {
					iRet = postMultipleJournal("Void Merchant Balance Transfer", rRec, csPostUser); 
				} else {
					iRet = FAILURE;
				}	
				RecordSet_Destroy(rRec);
DEBUGLOG(("GetVoidMbt End. Return:[%d]\n",iRet));
			} else {
DEBUGLOG(("!!GetVoidMbt NOT IN Auto Post List[%s]\n", PD_JLT_PX_VMBT));
			}
		}
	}
	...
	if (iRet == PD_OK) {
		if ((!strcmp(csJnlType,PD_JLT_PX_ALL)) || (!strcmp(csJnlType,PD_JLT_PX_OMBTF))) {
                        DBObjPtr = CreateObj(DBPtr,"DBCrrJnlType","IsAutoPostJnl");
                        if ((unsigned long)((*DBObjPtr)(PD_JLT_PX_OMBTF))) {
				// GetMbt2Off
...
DEBUGLOG(("!!GetMbt2Off NOT IN Auto Post List[%s]\n", PD_JLT_PX_OMBTF));
			}
		}
	}

	/* GetVoidMbt2Off VMBTF */
	if (iRet == PD_OK) {
		if ((!strcmp(csJnlType,PD_JLT_PX_ALL)) || (!strcmp(csJnlType,PD_JLT_PX_VMBTF))) {
                        DBObjPtr = CreateObj(DBPtr,"DBCrrJnlType","IsAutoPostJnl");
                        if ((unsigned long)((*DBObjPtr)(PD_JLT_PX_VMBTF))) {
				// GetMbt2Off
DEBUGLOG(("GetVoidMbt2Off Begin\n"));
				recordset_init(rRec,0);
				DBObjPtr = CreateObj(DBPtr,"DBCrrJnlHeader","GetVoidMbt2Off");
				if ((unsigned long) ((*DBObjPtr)(rRec, csTxnDate, csCountry, csProduct)) == PD_OK) {
					iRet = postMultipleJournal("Void Approved Merchant Balance Transfer To Offline", rRec, csPostUser); 
				} else {
					iRet = FAILURE;
				}	
				RecordSet_Destroy(rRec);
DEBUGLOG(("GetVoidMbt2Off End. Return:[%d]\n",iRet));
			} else {
DEBUGLOG(("!!GetVoidMbt2Off NOT IN Auto Post List[%s]\n", PD_JLT_PX_VMBTF));
			}
		}
	}

	/* GetApprovedPbt ONPTR */
	if (iRet == PD_OK) {
		if ((!strcmp(csJnlType,PD_JLT_PX_ALL)) || (!strcmp(csJnlType,PD_JLT_PX_ONPTR))) {
                        DBObjPtr = CreateObj(DBPtr,"DBCrrJnlType","IsAutoPostJnl");
                        if ((unsigned long)((*DBObjPtr)(PD_JLT_PX_ONPTR))) {
				// GetApprovedPbt
...
DEBUGLOG(("!!GetApprovedPbt NOT IN Auto Post List[%s]\n", PD_JLT_PX_ONPTR));
			}
		}
	}
	...
	if (iRet == PD_OK) {
		if ((!strcmp(csJnlType,PD_JLT_PX_ALL)) || (!strcmp(csJnlType,PD_JLT_PX_OPTFP))) {
                        DBObjPtr = CreateObj(DBPtr,"DBCrrJnlType","IsAutoPostJnl");
                        if ((unsigned long)((*DBObjPtr)(PD_JLT_PX_OPTFP))) {
				// GetPbt2Off
...
DEBUGLOG(("!!GetPbt2Off NOT IN Auto Post List[%s]\n", PD_JLT_PX_OPTFP));
			}
		}
	}

	/* GetApprovedMFundIn OMFI */
	/* if (iRet == PD_OK) {
		if ((!strcmp(csJnlType,PD_JLT_PX_ALL)) || (!strcmp(csJnlType,PD_JLT_PX_OMFI))) {
                        DBObjPtr = CreateObj(DBPtr,"DBCrrJnlType","IsAutoPostJnl");
                        if ((unsigned long)((*DBObjPtr)(PD_JLT_PX_OMFI))) {
				// GetApprovedMFundIn
...
DEBUGLOG(("!!GetApprovedMFundIn NOT IN Auto Post List[%s]\n", PD_JLT_PX_OMFI));
			}
		}
	}*/
	...
	if (iRet == PD_OK) {
		if ((!strcmp(csJnlType,PD_JLT_PX_ALL)) || (!strcmp(csJnlType,PD_JLT_PX_FMBTO))) {
                        DBObjPtr = CreateObj(DBPtr,"DBCrrJnlType","IsAutoPostJnl");
                        if ((unsigned long)((*DBObjPtr)(PD_JLT_PX_FMBTO))) {
				// GetOff2Mbt
...
DEBUGLOG(("!!GetOff2Mbt NOT IN Auto Post List[%s]\n", PD_JLT_PX_FMBTO));
			}
		}
	}
	...
	if (iRet == PD_OK) {
		if ((!strcmp(csJnlType,PD_JLT_PX_ALL)) || (!strcmp(csJnlType,PD_JLT_PX_FPTOP))) {
                        DBObjPtr = CreateObj(DBPtr,"DBCrrJnlType","IsAutoPostJnl");
                        if ((unsigned long)((*DBObjPtr)(PD_JLT_PX_FPTOP))) {
				// GetOff2Pbt
...
DEBUGLOG(("!!GetOff2Pbt NOT IN Auto Post List[%s]\n", PD_JLT_PX_FPTOP));
			}
		}
	}
	...
	/*if (iRet == PD_OK) {
		if ((!strcmp(csJnlType,PD_JLT_PX_ALL)) || (!strcmp(csJnlType,PD_JLT_PX_INTPM))) {
                        DBObjPtr = CreateObj(DBPtr,"DBCrrJnlType","IsAutoPostJnl");
                        if ((unsigned long)((*DBObjPtr)(PD_JLT_PX_INTPM))) {
				// GetPsp2Mb
...
DEBUGLOG(("!!GetPsp2Mb NOT IN Auto Post List[%s]\n", PD_JLT_PX_INTPM));
			}
		}
	}*/
	...
	/* New Jnl */
	if (iRet == PD_OK) {
		DBObjPtr = CreateObj(DBPtr,"DBCrrJnlType","GetNewAutoPostJnl");
		recordset_init(rJnl,0);
		if ((unsigned long)((*DBObjPtr)(rJnl)) == PD_FOUND) {
DEBUGLOG(("GetNewAutoPostJnl FOUND\n"));
			hJnl = RecordSet_GetFirst(rJnl);
			while (hJnl) {
				GetField_Int(hJnl,"jnl_type_id",&iJnlTypeId);
				GetField_CString(hJnl,"desc",&csDesc);
				GetField_CString(hJnl,"prefix",&csPrefix);
				GetField_Int(hJnl,"group_id",&iGroupId);
				GetField_Int(hJnl,"single",&iSingle);
DEBUGLOG(("GetNewAutoPostJnl jnl_type_id [%d] prefix[%s] group_id[%d]\n",iJnlTypeId,csPrefix,iGroupId));

				if (!strcmp(csJnlType,PD_JLT_PX_ALL) || !strcmp(csJnlType,csPrefix)) {
DEBUGLOG(("GetNewAutoPostJnl [%s] Begin\n",csDesc));
					recordset_init(rRec,0);
					DBObjPtr = CreateObj(DBPtr,"DBCrrJnlHeader","GetJnlDataPkg");
					if ((unsigned long) ((*DBObjPtr)(rRec, csTxnDate, iJnlTypeId, iGroupId, csCountry, csProduct)) == PD_OK) {
						if (iSingle == 1) {
							iRet = postSingleJournal(csDesc, rRec, csPostUser); 
						} else {
							iRet = postMultipleJournal(csDesc, rRec, csPostUser); 
						}
					} else {
						iRet = FAILURE;
					}	
					RecordSet_Destroy(rRec);
DEBUGLOG(("GetNewAutoPostJnl [%s] End. Return:[%d]\n",csDesc,iRet));
				}

				hJnl = RecordSet_GetNext(rJnl);
			}
		} else {
DEBUGLOG(("GetNewAutoPostJnl NOT FOUND\n"));
		}
		RecordSet_Destroy(rJnl);
	}
	
	/* Offline Jnl */
	if (iRet == PD_OK) {
		DBObjPtr = CreateObj(DBPtr,"DBCrrJnlType","GetOfflineAutoPostJnl");
		recordset_init(rJnl,0);
		if ((unsigned long)((*DBObjPtr)(rJnl)) == PD_FOUND) {
DEBUGLOG(("GetOfflineAutoPostJnl FOUND\n"));
			hJnl = RecordSet_GetFirst(rJnl);
			while (hJnl) {
				GetField_Int(hJnl,"jnl_type_id",&iJnlTypeId);
				GetField_CString(hJnl,"desc",&csDesc);
				...
				GetField_Int(hJnl,"group_id",&iGroupId);
				GetField_Int(hJnl,"single",&iSingle);
DEBUGLOG(("GetOfflineAutoPostJnl jnl_type_id [%d] prefix[%s] group_id[%d]\n",iJnlTypeId,csPrefix,iGroupId));

				if (!strcmp(csJnlType,PD_JLT_PX_ALL) || !strcmp(csJnlType,csPrefix)) {
DEBUGLOG(("GetOfflineAutoPostJnl [%s] Begin\n",csDesc));
					recordset_init(rRec,0);
					DBObjPtr = CreateObj(DBPtr,"DBCrrJnlHeader","GetOfflineJnlDataPkg");
					if ((unsigned long) ((*DBObjPtr)(rRec, csTxnDate, iJnlTypeId, iGroupId, csCountry, csProduct)) == PD_OK) {
						if (iSingle == 1) {
							iRet = postSingleJournal(csDesc, rRec, csPostUser); 
						} else {
					...
					RecordSet_Destroy(rRec);
DEBUGLOG(("GetOfflineAutoPostJnl [%s] End. Return:[%d]\n",csDesc,iRet));
				}
...
DEBUGLOG(("GetOfflineAutoPostJnl NOT FOUND\n"));
	...
	/* MiniMMM Jnl */
	if (iRet == PD_OK) {
		DBObjPtr = CreateObj(DBPtr,"DBCrrJnlType","GetMiAutoPostJnl");
		recordset_init(rJnl,0);
		if ((unsigned long)((*DBObjPtr)(rJnl)) == PD_FOUND) {
DEBUGLOG(("GetMiAutoPostJnl FOUND\n"));
			hJnl = RecordSet_GetFirst(rJnl);
			while (hJnl) {
				GetField_Int(hJnl,"jnl_type_id",&iJnlTypeId);
				GetField_CString(hJnl,"desc",&csDesc);
				...
				GetField_Int(hJnl,"group_id",&iGroupId);
				GetField_Int(hJnl,"single",&iSingle);
DEBUGLOG(("GetMiAutoPostJnl jnl_type_id [%d] prefix[%s] group_id[%d]\n",iJnlTypeId,csPrefix,iGroupId));

				if (!strcmp(csJnlType,PD_JLT_PX_ALL) || !strcmp(csJnlType,csPrefix)) {
DEBUGLOG(("GetMiAutoPostJnl [%s] Begin\n",csDesc));
					recordset_init(rRec,0);
					DBObjPtr = CreateObj(DBPtr,"DBCrrJnlHeader","GetMiJnlDataPkg");
					if ((unsigned long) ((*DBObjPtr)(rRec, csTxnDate, iJnlTypeId, iGroupId, csCountry, csProduct)) == PD_OK) {
						if (iSingle == 1) {
							iRet = postSingleJournal(csDesc, rRec, csPostUser); 
						} else {
					...
					RecordSet_Destroy(rRec);
DEBUGLOG(("GetMiAutoPostJnl [%s] End. Return:[%d]\n",csDesc,iRet));
				}
...
DEBUGLOG(("postSingleJournal: v_txn_cnt not found\n"));
		...
		// Remark (txn_id)
		if (GetField_CString(hJnl,"v_th_txn_id",&csTxnID)) {
DEBUGLOG(("postSingleJournal: remark(txn_id) = [%s]\n", csTxnID));
			PutField_CString(hJnlDtl,"jd_remarks",csTxnID);
		} else {
DEBUGLOG(("postSingleJournal: v_th_txn_id not found\n"));
		}

		// Report Date
		if (GetField_CString(hJnl,"v_party_type",&csTmp)) {
			cTmp = csTmp[0];
DEBUGLOG(("postSingleJournal: party_type = [%c]\n",cTmp));

			if ( cTmp == PD_MI_PARTY_OPBANK ) {
				hash_t *hTxnMiDetail;
				...
				iRet = (unsigned long)((*DBObjPtr)(csTxnID,rsTxnMiDetail));

				if (iRet == PD_OK) {
					hTxnMiDetail = RecordSet_GetFirst(rsTxnMiDetail);

					if (GetField_CString(hTxnMiDetail,"report_date",&csTmp)) {
						PutField_CString(hJnlDtl,"report_date",csTmp);
					}
				}
...
*/
		...
		char csReqCcy[PD_CCY_ID_LEN];
		if (!strcmp(csTxnCcy,csFxGainLossCcy)) {
			dConvertAmt = dTmpAmt;
		}

		if (!strcmp(csTxnCcy,PD_CCY_ISO_RMB)) {
			strcpy(csReqCcy, PD_CCY_ISO_CNY);
		} else {
			strcpy(csReqCcy, csTxnCcy);
		}

		sprintf(csTag,"%s%s",csTxnCcy,csFxGainLossCcy);
		if (GetField_Double(hAllExRate,csTag,&dTmp)) {
			dConvertAmt = CalExAmt(dTmpAmt,dTmp,csFxGainLossCcy);
		}
...
//End Dirk 20150810
	...
	dFxGainLoss = newround(dFxGainLoss,6);
DEBUGLOG((">>> iFound = [%d] ||| dFxGainLoss = [%.06f]\n",iFound,dFxGainLoss));
	if (iFound) {
...
int GetAllExRate(hash_t* hAllExRate, char* csEffectDate)
{
	...
	recordset_t *rsAllExRate = (recordset_t*) malloc (sizeof(recordset_t));
	recordset_init(rsAllExRate,0);
	...
	DBObjPtr = CreateObj(DBPtr,"DBExchangeRate","GetAllExchangeRateByDate");
	if ((unsigned long)((*DBObjPtr)(csEffectDate,rsAllExRate)) == PD_OK) {
		hRsRate = RecordSet_GetFirst(rsAllExRate);
		while (hRsRate) {
			if (GetField_CString(hRsRate,"from_ccy",&csFromCcy)) {
				if (GetField_CString(hRsRate,"to_ccy",&csToCcy)) {
					if (GetField_Double(hRsRate,"med_ask",&dTmp)) {
						sprintf(csTag,"%s%s",csFromCcy,csToCcy);
						PutField_Double(hAllExRate,csTag,dTmp);
					}
				}
			}
			hRsRate = RecordSet_GetNext(rsAllExRate);
		}
	} else {
DEBUGLOG(("postSingleJournal: Call DBExchangeRate::GetAllExchangeRateByDate FAILED!\n"));
		iRet = INT_ERR;
	}
