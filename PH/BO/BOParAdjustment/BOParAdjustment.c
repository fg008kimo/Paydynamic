...
#include "internal.h"
#include "common.h"
#include "BOParAdjustment.h"

char    cDebug;
OBJPTR(DB);
OBJPTR(BO);

void BOParAdjustment(char    cdebug)
{
        cDebug = cdebug;
}
...
ERRLOG("BOParAdjustment::GetCurrentCode::GetMaxCode not found!!\n");
		iRet=INT_ERR;
        }
...
ERRLOG("BOParAdjustment::ValidateProcess::GetAdjustmentTypeRec:: failed!!\n");
                                }
...
ERRLOG("BOParAdjustment::ValidateProcess::Call Transaction: ChkTxnCode Exist :: ERR!!\n");
					iRet = PD_ERR;
				}	
			}
		}
...
ERRLOG("BOParAdjustment::ProcessPartyBalance::GetAdjustmentTypeRec:: code in disabled!!\n");
...
ERRLOG("BOParAdjustment::ProcessPartyBalance::GetAdjustmentTypeRec:: failed!!\n");
...
ERRLOG("BOParAdjustment::ProcessPartyBalance::approval_date not found!!\n");
...
ERRLOG("BOParAdjustment::ProcessPartyBalance::psp_id missing!!\n");
			iRet = INT_ERR;
		...
		if (iRet == PD_OK) {
DEBUGLOG(("BOParAdjustment::ProcessPartyBalance::Call BOBalance:GetAvalPspBalanceForUpdate\n"));
...
DEBUGLOG(("BOParAdjustment::BOBalance:GetAvalPspBalanceForUpdate Failed\n"));
ERRLOG("BOParAdjustment::BOBalance:GetAvalPspBalanceForUpdate Failed\n");
                                iRet=INT_ERR;
                        }
                        else {
DEBUGLOG(("BOParAdjustment::ProcessPartyBalance::PSP Balance [%lf]\n", dAvaBalance));
                        }
		...
		if (iVoidFlag == PD_FALSE) {
			if (cDCInd == PD_ADJ_TYPE_DEBIT) {
				if (iRet == PD_OK) {
					...
					{
DEBUGLOG(("BOParAdjustment::ProcessPartyBalance::CANNOT process as not enought PSP [%s] Balance [%lf] Amt [%lf] NetAmt [%lf]\n", csPspId, dAvaBalance, dAmt, dNetAmt));
				...
				if (iRet==PD_OK) {
DEBUGLOG(("BOParAdjustment::Call DBPspBalance:DebitBalance\n"));
					...
					{
DEBUGLOG(("BOParAdjustment::DBPspBalance:DebitBalance Failed\n"));
						iRet = INT_ERR;
					}
				}
			} else if (cDCInd == PD_ADJ_TYPE_CREDIT)  {
                        	if (iRet==PD_OK) {
DEBUGLOG(("BOParAdjustment::Call DBPspBalance:CreditBalance\n"));
                                	DBObjPtr = CreateObj(DBPtr,"DBPspBalance","CreditBalance");
	                                //if ((*DBObjPtr)(csPspId,csCountry, csCcy, PD_PSP_BAL, dAmt, csUpdateUser) != PD_OK) 
	                                if ((*DBObjPtr)(csPspId,csCountry, csCcy, PD_PSP_BAL, dNetAmt, csUpdateUser) != PD_OK) 
					{
DEBUGLOG(("BOParAdjustment::DBPspBalance:CreditBalance Failed\n"));
						iRet = INT_ERR;
                                	}
                        	}
			}
			...
			/******* Void *******/
                        ...
                        if (cDCInd == PD_ADJ_TYPE_DEBIT) { // convert to credit as void
                                if (iRet==PD_OK) {
DEBUGLOG(("BOParAdjustment::(Void) Call DBPspBalance:CreditBalance\n"));
                                        DBObjPtr = CreateObj(DBPtr,"DBPspBalance","CreditBalance");
                                        //if ((*DBObjPtr)(csPspId,csCountry, csCcy, PD_PSP_BAL, dAmt, csUpdateUser) != PD_OK) 
                                        if ((*DBObjPtr)(csPspId,csCountry, csCcy, PD_PSP_BAL, dNetAmt, csUpdateUser) != PD_OK) 
					{
DEBUGLOG(("BOParAdjustment::(Void) DBPspBalance:CreditBalance Failed\n"));
                                                iRet = INT_ERR;
                                        }
                                }
...
DEBUGLOG(("BOParAdjustment::ProcessPartyBalance::CANNOT process as not enought PSP [%s] Balance [%lf] Amt [%lf] NetAmt [%lf]\n", csPspId, dAvaBalance, dAmt, dNetAmt));
						...
						PutField_Int(hInRec, "internal_error", iRet);
                                ...
                                if (iRet==PD_OK) {
DEBUGLOG(("BOParAdjustment::(Void) Call DBPspBalance:DebitBalance\n"));
                                        DBObjPtr = CreateObj(DBPtr,"DBPspBalance","DebitBalance");
                                        //if ((*DBObjPtr)(csPspId,csCountry, csCcy, PD_PSP_BAL, dAmt, csUpdateUser) != PD_OK) 
                                        if ((*DBObjPtr)(csPspId,csCountry, csCcy, PD_PSP_BAL, dNetAmt, csUpdateUser) != PD_OK) 
					{
DEBUGLOG(("BOParAdjustment::(Void) DBPspBalance:DebitBalance Failed\n"));
                                                iRet = INT_ERR;
                                        }
                                }
                        } 
		...
		/* Get PSP Balance */
                hash_init(hTxn,0);
DEBUGLOG(("BOParAdjustment::Call DBPspBalance:GetBalance\n"));
...
ERRLOG("BOParAdjustment::PSP GetBalance::Balance for PspId[%s] not found\n",csPspId);
...
ERRLOG("BOParAdjustment::ProcessPartyBalance::merchant_id missing!!\n");
			iRet = INT_ERR;
...
ERRLOG("BOParAdjustment::ProcessPartyBalance::service_code missing!!\n");
			iRet = INT_ERR;
		}
		...
		if (iRet == PD_OK) {
DEBUGLOG(("BOParAdjustment::ProcessPartyBalance::Call BOParBalance:ProcessMerchantAdjAmount\n"));
                        BOObjPtr = CreateObj(BOPtr,"BOParBalance","ProcessMerchantAdjAmount");
                        iRet = (unsigned long)(*BOObjPtr)(hInRec); 
			if(iRet!=PD_OK){
DEBUGLOG(("BOParAdjustment::BOBalance:ProcessMerchantAdjAmount Failed\n"));
ERRLOG("BOParAdjustment::BOBalance:ProcessMerchantAdjAmount Failed\n");
                                PutField_Int(hInRec, "internal_error", iRet);
                        }
		}
...
ERRLOG("BOParAdjustment::ProcessPartyBalance::Incorrect PartyType [%c]\n", cPartyType);
