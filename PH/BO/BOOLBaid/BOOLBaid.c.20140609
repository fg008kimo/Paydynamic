...
DEBUGLOG(("Authorize() call DBOLBAIDTxn::GetBaidTxn() failed\n"));
	...
	// get stmt details
	if (iRet == PD_OK) {
		if (GetField_CString(hBaidTxn, "stat_txn_id", &csStatTxnId)) {
DEBUGLOG(("PostBaidTxn() call DBOLStatement::GetStmtDtl()\n"));
			DBObjPtr = CreateObj(DBPtr, "DBOLStatement", "GetStmtDtl");
			iTmpRet = (unsigned long)(*DBObjPtr)(csStatTxnId, hStmtDtl);
			if (iTmpRet != PD_OK) {
DEBUGLOG(("Authorize() call DBOLStatement::GetStmtDtl() failed\n"));
				iRet = PD_ERR;
			}
		} else {
DEBUGLOG(("Authorize() cannot get stat_txn_id from baid txn\n"));
			iRet = PD_ERR;
		}
	}
