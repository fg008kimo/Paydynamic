...
int GetNewBAIDDetails(const char* csBAID, int iMaxBAIDCnt, recordset_t* rNewBAIDDetails)
{
        int     iRet = PD_OK;
	int	iDtlRet = PD_FOUND;
...
DEBUGLOG(("bank_code not found!!\n"));
ERRLOG("BOOLBaid::GetNewBAIDDetails() Call DBOLBankAcctId::GetBAIDBalDetails::bank_code not found!!\n");
                        iRet = INT_INVALID_BAID;
                }
	} else if (iDtlRet == PD_NOT_FOUND) {
DEBUGLOG(("Call DBOLBankAcctId::GetBAIDBalDetails::Record not found!!\n"));
...
DEBUGLOG(("Call DBOLBankAcctId::GetBAIDBalDetails Failure!!\n"));
ERRLOG("BOOLBaid::GetNewBAIDDetails() Call DBOLBankAcctId::GetBAIDBalDetails Failure!!\n");
                iRet = INT_ERR;
        }
...
DEBUGLOG(("Call DBOLBankAcctId::GetNewBAIDDetails::baid = [%s],baid_total = [%d]\n", csBAID, iMaxBAIDCnt));
        	DBObjPtr = CreateObj(DBPtr,"DBOLBankAcctId","GetNewBAIDDetails");
      		iDtlRet = (unsigned long)(*DBObjPtr)(csBAID, iMaxBAIDCnt, rRecordset);
      		if (iDtlRet == PD_FOUND) {
...
DEBUGLOG((" [%d]pid_bank_nature = [%s]\n", iBAIDCnt, csTmp));
                        	                PutField_CString(hNewBAIDDetails,"pid_bank_nature",csTmp);
                        	        }
                       		}

/* bank_nature */
                        	if (iRet == PD_OK) {
                        	        if (GetField_CString(hRec,"bank_nature",&csTmp)) {
DEBUGLOG((" [%d]bank_nature = [%s]\n", iBAIDCnt, csTmp));
                        	                PutField_CString(hNewBAIDDetails,"bank_nature",csTmp);
                        	        }
                        	}
