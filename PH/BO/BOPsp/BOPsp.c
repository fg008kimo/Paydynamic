...
-------------------------------                    ------------            --------------
Init Version                                       2010/09/16              [GOD]
Update 0.0.1 (add PK: client)			   2011/01/07		   [MSN]
Update 0.0.2 (get psp key)			   2011/01/07		   [MSN]
Add STP PSP				           2011/02/28		   [GOD]
Add BOTxnLb					   2012/01/04		   [GOD]
Update PspCosts					   2012/04/12		   [GOD]
ClientId store in psp_master table		   2012/12/11		   [MSN]
Add Selected PSP				   2013/03/13		   [GOD]
Add CheckPspStatus				   2013/03/15		   [WCS]
Add DBPspCosts cost_type & cost_charging_method	   2015/05/18		   [WWK]
Add get AES and RSA key				   2015/08/05		   [MSN]
Support Card type option			   2016/10/20		   [MSN]
Add field paid_amt in GetPspReconTxnDetail()	   2020/11/18		   [WMC]
Update GetPspReconTxnDetail()			   2021/03/10		   [WMC]
*/
...
DEBUGLOG(("BOPsp: GetTxnPsp- service_code = [%s]\n",csServiceCode));
        }
...
DEBUGLOG(("BOPsp STP call GetStpPsp\n"));
		if (GetField_CString(hContext,"selected_pid",&csSelectedPID)) {
			csPspId = strdup (csSelectedPID);
DEBUGLOG(("BOPsp selected PID = [%s]\n",csPspId));
			...
			DBObjPtr = CreateObj(DBPtr,"DBPspDetail","GetPspDetail");
       			if ((*DBObjPtr)(csPspId,
					myHash) != PD_OK) {
ERRLOG("BOPsp:internal error select psp detail for selected pid\n");
DEBUGLOG(("BOPsp:internal error select psp detail for selected pid\n"));
				iRet = PD_ERR;
			}
			else {
				hRec = myHash;
			...
			DBObjPtr = CreateObj(DBPtr,"DBTxnLbStp","GetStpPsp");
				...
				if(!strcmp(csTxnCode,PD_WITHDRAW_BATCH_TXN_CODE)){
				/*For WTB case, default psp is TWV*/
					PutField_CString(hContext,"psp_id","TWV");
					csPspId=strdup("TWV");
					iFreePspId=PD_TRUE;
				}
			}
			else {
DEBUGLOG(("BOPsp try to set\n"));
...
DEBUGLOG(("BOPsp: GetTxnPsp - psp_ccy = [%s]\n",csPtr));
				...
				PutField_CString(hContext,"dst_txn_ccy",csPtr);
                       	...
                       	else if (GetField_CString(hRec,"ccy",&csPtr)) {
DEBUGLOG(("BOPsp: GetTxnPsp - psp_ccy = [%s]\n",csPtr));
				PutField_CString(hContext,"psp_ccy",csPtr);
				PutField_CString(hContext,"dst_txn_ccy",csPtr);
			}
...
DEBUGLOG(("BOPsp: check IsUseLB Error!!!\n"));
ERRLOG("BOPsp::Call IsUseLB() check IsUseLB Error!!!\n");
                }
                else if(iChk == PD_FALSE) {
DEBUGLOG(("BOPsp: Not using Load Balancer\n"));
...
DEBUGLOG(("BOPsp: call BOTxnLb\n"));
			...
			BOObjPtr = CreateObj(BOPtr,"BOTxnLb","GetTxnPsp_Card");
			iRet = (unsigned long)(*BOObjPtr)(hContext, hRequest);
DEBUGLOG(("BOPsp: return from BOTxnLb = [%d]\n",iRet));
			if (iRet == PD_OK) {
				if (GetField_CString(hContext,"psp_id",&csPspId)) {
DEBUGLOG(("BOPsp: psp id return from BOTxnLb = [%s]\n",csPspId));
					hash_t* hPspDetail;
					hPspDetail = (hash_t*) malloc (sizeof(hash_t));
                			hash_init(hPspDetail,0);
					DBObjPtr = CreateObj(DBPtr,"DBPspDetail","GetPspDetail");
       					if ((*DBObjPtr)(csPspId,hPspDetail) == PD_OK) {
                                		if (GetField_CString(hPspDetail,"client_id",&csClientId)) {
DEBUGLOG(("BOPsp: GetTxnPsp - LB MODE psp_client_id = [%s]\n",csClientId));
                                        		PutField_CString(hContext,"psp_client_id",csClientId);
                                		}

                                		if (GetField_Char(hPspDetail,"online",&cOnline)) {
DEBUGLOG(("BOPsp: GetTxnPsp - LB MODE online = [%c]\n",cOnline));
                                        		PutField_Char(hContext,"online",cOnline);
                                		}

                                		if (GetField_CString(hPspDetail,"psp_channel_code",&csPtr)) {
DEBUGLOG(("BOPsp: GetTxnPsp - LB MODE channel_code  = [%s]\n",csPtr));
                                        		PutField_CString(hContext,"psp_channel_code",csPtr);
                                		}
                                		else {
DEBUGLOG(("BOPsp: GetTxnPsp - LB MODE channel_code not found\n"));
                                		}

                                		if (GetField_CString(hPspDetail,"ccy",&csPtr)) {
DEBUGLOG(("BOPsp: GetTxnPsp - LB MODE psp_ccy = [%s]\n",csPtr));
                                        		PutField_CString(hContext,"psp_ccy",csPtr);
                                        		PutField_CString(hContext,"dst_txn_ccy",csPtr);
                                		}
                                		else {
DEBUGLOG(("BOPsp: GetTxnPsp - LB MODE psp_ccy not found\n"));
                                		}

                                		if (GetField_CString(hPspDetail,"psp_merchant_id",&csPtr)) {
DEBUGLOG(("BOPsp: GetTxnPsp - LB MODE psp_merchant id = [%s]\n",csPtr));
                                        		PutField_CString(hContext,"psp_merchant_id",csPtr);
                                		}

                                		if (GetField_CString(hPspDetail,"pid_group",&csPtr)) {
DEBUGLOG(("BOPsp: GetTxnPsp - LB MODE pid_group = [%s]\n",csPtr));
                                        		PutField_CString(hContext,"pid_group",csPtr);
                                		}
					}
					else {
						iRet = PD_ERR;
ERRLOG("BOPsp: Error getting record for [%s]!!!\n",csPspId);
DEBUGLOG(("BOPsp: Error getting record for [%s]!!!\n",csPspId));
					}
...
/*Get Psp Key*/ //AES
	if(iRet == PD_OK){
		recordset_init(rKeySet,0);
		DBObjPtr = CreateObj(DBPtr,"DBPspKeys","GetPspKey");
		if (!(*DBObjPtr)(csPspId,PD_AES_KEY_NAME,rKeySet)) {
			hKey = RecordSet_GetFirst(rKeySet);
			while(hKey){
				if (GetField_CString(hKey,"key_value",&csTmp)) {
DEBUGLOG(("BOPsp: GetTxnPsp - aes_key = [%s]\n",csTmp));
...
/*Get Psp Key*/ //RSA
	if(iRet == PD_OK){
		recordset_init(rKeySet,0);
		DBObjPtr = CreateObj(DBPtr,"DBPspKeys","GetPspKey");
		if (!(*DBObjPtr)(csPspId,PD_RSA_KEY_NAME,rKeySet)) {
			hKey = RecordSet_GetFirst(rKeySet);
			while(hKey){
				if (GetField_CString(hKey,"privatepem",&csTmp)) {
DEBUGLOG(("BOPsp: GetTxnPsp - rsa_key file = [%s]\n",csTmp));
...
/*Get Psp Key*/ //PUK
	if(iRet == PD_OK){
		recordset_init(rKeySet,0);
		DBObjPtr = CreateObj(DBPtr,"DBPspKeys","GetPspKey");
		if (!(*DBObjPtr)(csPspId,PD_PUK_KEY_NAME,rKeySet)) {
			hKey = RecordSet_GetFirst(rKeySet);
			while(hKey){
				if (GetField_CString(hKey,"privatepem",&csTmp)) {
DEBUGLOG(("BOPsp: GetTxnPsp - puk_key file= [%s]\n",csTmp));
					PutField_CString(hContext,"puk_key",csTmp);
				}
				break;
			}
		}
...
DEBUGLOG(("BOPsp:CalPspCosts()\n"));

 	if (GetField_Double(hContext,"org_dst_net_amt",&dOrgTxnAmt)) {
DEBUGLOG(("BOPsp::CalPspCosts() org_dst_net_amt = [%f]\n",dOrgTxnAmt));
        }
        else {
DEBUGLOG(("BOPsp::CalPspCosts() org_dst_net_amt not found!!!\n"));
        }

/*org psp id */
        if (GetField_CString(hContext,"org_psp_id",&csOrgPspId)) {
DEBUGLOG(("BOPsp:CalPspCosts() org_psp_id = [%s]\n",csOrgPspId));
        }
        else {
DEBUGLOG(("BOPsp:CalPspCosts() org_psp_id is missing!!!\n"));
        }

/*txn type*/
        if (GetField_Char(hContext,"txn_type",&cType)) {
DEBUGLOG(("BOPsp:CalPspCosts() txn_type = [%c]\n",cType));
        }
        else {
DEBUGLOG(("BOPsp:CalPspCosts() txn_type is missing!!!\n"));
        }

	DBObjPtr = CreateObj(DBPtr,"DBPspCosts","GetPspCosts");
	if ((unsigned long)(*DBObjPtr)(csOrgPspId, cType, rRecordSet)==PD_FOUND) {
		hRec = RecordSet_GetFirst(rRecordSet);
		while(hRec){
			if (GetField_Char(hRec,"type",&cPreCalType)) {
DEBUGLOG(("BOPsp:CalPspCosts() Precal_type = [%c]\n",cPreCalType));
			}
			if (GetField_Double(hRec,"value",&dPreCalValue)) {
DEBUGLOG(("BOPsp:CalPspCosts() Precal_value = [%f]\n",dPreCalValue));
			}
			if (GetField_Double(hRec,"add_value",&dPreCalAddValue)) {
DEBUGLOG(("BOPsp:CalPspCosts() Precal_additional_value = [%f]\n",dPreCalAddValue));
			}
			if (GetField_Double(hRec,"min_value",&dPreCalMinValue)) {
DEBUGLOG(("BOPsp:CalPspCosts() Precal_min_value = [%f]\n",dPreCalMinValue));
			}
			if (GetField_Double(hRec,"max_value",&dPreCalMaxValue)) {
DEBUGLOG(("BOPsp:CalPspCosts() Precal_max_value = [%f]\n",dPreCalMaxValue));
			}

			if (GetField_Char(hRec,"value_type",&cValueType)) {
DEBUGLOG(("BOPsp:CalPspCosts() value_type = [%c]\n",cValueType));
			}

			if (GetField_Int(hRec,"scale",&iScale)) {
DEBUGLOG(("BOPsp:CalPspCosts() scale = [%d]\n",iScale));
			}

DEBUGLOG(("BOPsp:CalPspCosts() Precal costs Now!!!\n"));
			double dTmp = 0;
...
DEBUGLOG(("BOPsp:CalPspCosts() costs before rounding = [%f]\n",dTmp));
...
DEBUGLOG(("BOPsp:CalPspCosts() undefine value_type = [%c]\n",cValueType));
ERRLOG("BOPsp:CalPspCosts() undefine value_type = [%c]\n",cValueType);
			}
			...
			if (GetField_Char(hRec,"cost_type",&cCostType)) {
				PutField_Char(hContext,"cost_type",cCostType);
DEBUGLOG(("BOPsp:CalPspCosts() cost_type = [%c]\n",cCostType));
			}
			
			if (GetField_Char(hRec,"cost_charging_method",&cCostChargingMethod)) {
				PutField_Char(hContext,"cost_charging_method",cCostChargingMethod);
DEBUGLOG(("BOPsp:CalPspCosts() cost_charging_method = [%c]\n",cCostChargingMethod));
			}
			
			hRec = RecordSet_GetNext(rRecordSet);
		}

DEBUGLOG(("BOPsp:CalPspCosts() Precal fee = [%f]\n",dFee));
...
DEBUGLOG(("BOPsp:CalPspCosts() exit = [%d]\n",iRet));
	return iRet;
...
DEBUGLOG(("GetTxnPspDetailForReconByReconInd: txn_amt = [%lf]\n",dTmp));
				...
				if(GetField_Double(hRec,"paid_amt",&dTmp)){
DEBUGLOG(("GetTxnPspDetailForReconByReconInd: paid_amt = [%lf]\n",dTmp));
                                        dPspPaidAmt += dTmp;

                                        sprintf(csTag,"paid_amt_%d",iCnt);
                                        PutField_Double(hContext,csTag,dTmp);
                                }else{
DEBUGLOG(("GetTxnPspDetailForReconByReconInd: no paid_amt, set paid_amt = txn_amt = [%lf]\n",dTmp));
					dPspPaidAmt += dTmp;
...
DEBUGLOG(("GetTxnPspDetailForReconByReconInd: service_fee = [%lf]\n",dTmp));
			...
			PutField_Int(hContext,"total_txn_cnt",iCnt);
			PutField_Double(hContext,"total_txn_amt",dPspAmt);
			PutField_Double(hContext,"total_paid_amt",dPspPaidAmt);
			PutField_Double(hContext,"total_txn_fee",dPspFee);
		}
		else{
...
ERRLOG("BOPsp::GetPspReconTxnDetail::Input Count[%d] and PH Count[%d] not match\n",iTotalCnt,iCnt);
		...
		if(iRet == PD_OK){
			if(GetField_Double(hContext,"txn_amt",&dInputAmt)){
				if((int)(dInputAmt*100)!=(int)(dPspAmt*100)){
...
DEBUGLOG(("GetTxnPspDetailForRecon: txn_amt = [%lf]\n",dTmp));
					...
					if(GetField_Double(hRec,"paid_amt",&dTmp)){
DEBUGLOG(("GetTxnPspDetailForRecon: paid_amt = [%lf]\n",dTmp));
                                                dPspPaidAmt += dTmp;
                                        }else{
DEBUGLOG(("GetTxnPspDetailForRecon: no paid_amt, set paid_amt = txn_amt = [%lf]\n",dTmp));
						dPspPaidAmt += dTmp;
					}
                                	if(GetField_Double(hRec,"service_fee",&dTmp)){
...
DEBUGLOG(("AddReconInd: Failed [%s]\n",csTmp));
                        	...
                        	PutField_Double(hResponse,"paid_amt",dPspPaidAmt);
                        	PutField_Double(hResponse,"service_fee",dPspFee);
                        	PutField_Double(hResponse,"psp_balance",dPspPaidAmt-dPspFee);
                	}
                	else{
                        	iRet = INT_ERR;
DEBUGLOG(("BOPsp::GetPspReconTxnDetail::Call DBTxnPspDetail:GetTxnPspDetailForRecon Failed\n"));
...
/* Reverse Psp Balance Transfer */
		...
		if(GetField_CString(hContext,"org_txn_seq",&csReconId)){
DEBUGLOG(("BOPsp::GetPspReconTxnDetail:: org_txn_id = [%s]\n",csReconId));
		...
		if(iRet==PD_OK){
			...
			double dAmt=0.0;
			double dFee=0.0;
			DBObjPtr = CreateObj(DBPtr,"DBPspReconHistory","GetReconDetail");
...
DEBUGLOG(("GetReconDetail: txn_amt = [%lf]\n",dTmp));
					...
					if(GetField_Double(hRec,"paid_amt",&dTmp)){
                                                ...
                                                PutField_Double(hContext,csTag,dTmp);
                                                dPaidAmt +=dTmp;
DEBUGLOG(("GetReconDetail: paid_amt = [%lf]\n",dTmp));
                                        }else{
						sprintf(csTag,"paid_amt_%d",iCnt);
                                                PutField_Double(hContext,csTag,dTmp);
                                                dPaidAmt +=dTmp;
DEBUGLOG(("GetReconDetail: no paid_amt, set paid_amt = txn_amt = [%lf]\n",dTmp));
					}
                                        if(GetField_Double(hRec,"service_fee",&dTmp)){
                                                sprintf(csTag,"service_fee_%d",iCnt);
                                                PutField_Double(hContext,csTag,dTmp);
...
DEBUGLOG(("GetReconDetail: service_fee = [%lf]\n",dTmp));
                                        ...
                                        hRec = RecordSet_GetNext(rRecordSet);
                                        PutField_Int(hContext,"total_txn_cnt",iCnt);
                                        PutField_Double(hContext,"total_txn_amt",dAmt);
                                        PutField_Double(hContext,"total_paid_amt",dPaidAmt);
                                        PutField_Double(hContext,"total_txn_fee",dFee);
...
DEBUGLOG(("BOPsp:GetAvalPspBalanceForPO()\n"));

 	if (GetField_Double(hContext,"org_psp_bal",&dOrgPspAmt)) {
		if(dOrgPspAmt<0.0)
			dOrgPspAmt = 0.0;
		dTmp = dOrgPspAmt;
DEBUGLOG(("BOPsp::GetAvalPspBalanceForPO() org_psp_bal = [%f]\n",dOrgPspAmt));
        }
        else {
DEBUGLOG(("BOPsp::GetAvalPspBalanceForPO() org_psp_bal not found!!!\n"));
        }
	if(GetField_Int(hContext,"txn_cnt",&iTxnCount)){
DEBUGLOG(("BOPsp::GetAvalPspBalanceForPO() txn_count = [%d]\n",iTxnCount));
	}
        else {
DEBUGLOG(("BOPsp::GetAvalPspBalanceForPO() txn_count not found!!!\n"));
        }

/*org psp id */
        if (GetField_CString(hContext,"psp_id",&csPspId)) {
DEBUGLOG(("BOPsp:GetAvalPspBalanceForPO() psp_id = [%s]\n",csPspId));
        }
        else {
DEBUGLOG(("BOPsp:GetAvalPspBalanceForPO() psp_id is missing!!!\n"));
        }

	DBObjPtr = CreateObj(DBPtr,"DBPspCosts","GetPspCosts");
	if ((unsigned long)(*DBObjPtr)(csPspId, PD_TYPE_PAYOUT, rRecordSet)==PD_FOUND) {
		hRec = RecordSet_GetFirst(rRecordSet);
		while(hRec){
			if (GetField_Char(hRec,"type",&cPreCalType)) {
DEBUGLOG(("BOPsp:GetAvalPspBalanceForPO() Precal_type = [%c]\n",cPreCalType));
			}
			if (GetField_Double(hRec,"value",&dPreCalValue)) {
DEBUGLOG(("BOPsp:GetAvalPspBalanceForPO() Precal_value = [%f]\n",dPreCalValue));
			}
			if (GetField_Double(hRec,"add_value",&dPreCalAddValue)) {
DEBUGLOG(("BOPsp:GetAvalPspBalanceForPO() Precal_additional_value = [%f]\n",dPreCalAddValue));
			}
			if (GetField_Double(hRec,"min_value",&dPreCalMinValue)) {
DEBUGLOG(("BOPsp:GetAvalPspBalanceForPO() Precal_min_value = [%f]\n",dPreCalMinValue));
			}
			if (GetField_Double(hRec,"max_value",&dPreCalMaxValue)) {
DEBUGLOG(("BOPsp:GetAvalPspBalanceForPO() Precal_max_value = [%f]\n",dPreCalMaxValue));
			}

			if (GetField_Char(hRec,"value_type",&cValueType)) {
DEBUGLOG(("BOPsp:GetAvalPspBalanceForPO() value_type = [%c]\n",cValueType));
			}

			if (GetField_Int(hRec,"scale",&iScale)) {
DEBUGLOG(("BOPsp:GetAvalPspBalanceForPO() scale = [%d]\n",iScale));
			}

DEBUGLOG(("BOPsp:GetAvalPspBalanceForPO() Precal costs Now!!!\n"));
			if (cPreCalType == PD_PRECENTAGE) {
				dTmp = newtrunc((dOrgPspAmt / (1+(dPreCalValue/100))),PD_DECIMAL_LEN);
DEBUGLOG(("BOPsp:GetAvalPspBalanceForPO() aval_psp_balance for payout = [%f]\n",dTmp));
			}
			else if (cPreCalType == PD_FIXED_AMOUNT) {
				dTmp = newtrunc((dOrgPspAmt - (dPreCalValue*iTxnCount)),PD_DECIMAL_LEN);
DEBUGLOG(("BOPsp:GetAvalPspBalanceForPO() aval_psp_balance for payout = [%f]\n",dTmp));
			}
			else if(cPreCalType == PD_PRECENT_WITH_FIX){
				dTmp = newtrunc((dOrgPspAmt - (dPreCalAddValue*iTxnCount))/(1+(dPreCalValue/100)),PD_DECIMAL_LEN) ;
DEBUGLOG(("BOPsp:GetAvalPspBalanceForPO() aval_psp_balance for payout = [%f]\n",dTmp));
			}
	...
	if(dTmp<0.0)
		dTmp = 0.0;
	PutField_Double(hContext,"aval_psp_bal",dTmp);


	RecordSet_Destroy(rRecordSet);
        FREE_ME(rRecordSet);

DEBUGLOG(("BOPsp:GetAvalPspBalanceForPO() exit = [%d]\n",iRet));
	return iRet;
...
/* Get PSP Detail */
	...
	DBObjPtr = CreateObj(DBPtr, "DBPspDetail", "GetPspDetail");
	if ((*DBObjPtr)(csPspId, hDetail) != PD_OK) {
ERRLOG("BOPsp: CheckPspStatus psp_detail for [%s] not found\n", csPspId);
DEBUGLOG(("BOPsp: CheckPspStatus psp_detail for [%s] not found\n", csPspId));
		iRet = INT_ERR;
	}
	else {
		if (GetField_CString(hDetail, "status", &csTmp)) {
DEBUGLOG(("BOPsp: CheckPspStatus psp_detail - status for [%s] = [%s]\n", csPspId, csTmp));
			if (strcmp(csTmp, PD_ACC_OPEN)) {
ERRLOG("BOPsp: CheckPspStatus psp_detail [%s] not opened\n", csPspId);
DEBUGLOG(("BOPsp: CheckPspStatus psp_detail [%s] not opened\n", csPspId));
				iRet = INT_ACCOUNT_DISABLED;
			}
		}
		else {
ERRLOG("BOPsp: CheckPspStatus psp_detail - status for [%s] not found\n", csPspId);
DEBUGLOG(("BOPsp: CheckPspStatus psp_detail - status for [%s] not found\n", csPspId));
			iRet = INT_ERR;
		}

		if (iRet == PD_OK) {
			if (GetField_CString(hDetail, "client_id", &csClientId)) {
DEBUGLOG(("BOPsp: CheckPspStatus psp_detail - client_id for [%s] = [%s]\n", csPspId, csClientId));
			}
			else {
ERRLOG("BOPsp: CheckPspStatus psp_detail - client_id for [%s] not found\n", csPspId);
DEBUGLOG(("BOPsp: CheckPspStatus psp_detail - client_id for [%s] not found\n", csPspId));
				iRet = INT_ERR;
			}
		}
...
/* Get PSP Master */
		hash_t *hMaster;
		hMaster = (hash_t*) malloc (sizeof(hash_t));
		hash_init(hMaster, 0);

		DBObjPtr = CreateObj(DBPtr, "DBPspMaster", "GetPspMaster");
		if ((*DBObjPtr)(csClientId, hMaster) != PD_OK) {
ERRLOG("BOPsp: CheckPspStatus psp_master for [%s] not found\n", csPspId);
DEBUGLOG(("BOPsp: CheckPspStatus psp_master for [%s] not found\n", csPspId));
			iRet = INT_ERR;
		}
		else {
			if (GetField_CString(hMaster, "status", &csTmp)) {
DEBUGLOG(("BOPsp: CheckPspStatus psp_master - status for [%s] = [%s]\n", csPspId, csTmp));
				if (strcmp(csTmp, PD_ACC_OPEN)) {
ERRLOG("BOPsp: CheckPspStatus psp_master [%s] not opened\n", csPspId);
DEBUGLOG(("BOPsp: CheckPspStatus psp_master [%s] not opened\n", csPspId));
					iRet = INT_ACCOUNT_DISABLED;
				}
			}
			else {
ERRLOG("BOPsp: CheckPspStatus psp_master - status for [%s] not found\n", csPspId);
DEBUGLOG(("BOPsp: CheckPspStatus psp_master - status for [%s] not found\n", csPspId));
				iRet = INT_ERR;
			}
		}
