...
Add CheckTotalEmailNumber and CheckDefault
when total_email_num is 0 			   2015/08/21		   Elvis Wong	
*/
...
DEBUGLOG(("AddEmailSettingHistory() (default) mail_id = [%d]\n", iTmp));
...
/* default */
        if (GetField_Int(hContext, "default", &iDefault)) {
DEBUGLOG(("ProcessTpl() default = [%d]\n", iDefault));
        }
...
DEBUGLOG(("ProcessTpl() call DBEmailSetting::DisableEmailSettingRecord()\n"));
                                        	...
                                        	iDtlRet = (unsigned long)(*DBObjPtr)(hContext);
                                        	if (iDtlRet == PD_FOUND) {
DEBUGLOG(("ProcessTpl() call DBEmailSetting::DisableEmailSettingRecord() FOUND!!!\n"));

							// AddEmailSettingHistory
                                                       	iRet = AddEmailSettingHistory(hContext);
						} else if (iDtlRet == PD_NOT_FOUND) {
DEBUGLOG(("ProcessTpl() call DBEmailSetting::DisableEmailSettingRecord() NOT FOUND!!!\n"));
...
DEBUGLOG(("ProcessTpl() email addr not registered!!\n"));
ERRLOG("BOEmailSetting::ProcessTpl:: email addr not registered!!\n");
					}
				} else {
...
DEBUGLOG(("ProcessTpl() call DBEmailSetting::UpdateEmailSettingDefault()\n"));
                        	...
                        	iDtlRet = (unsigned long)(*DBObjPtr)(hContext);
				if (iDtlRet == PD_FOUND) {
DEBUGLOG(("ProcessTpl() call DBEmailSetting::UpdateEmailSettingDefault() FOUND!!!\n"));

					// AddEmailSettingHistory
					iRet = AddEmailSettingHistory(hContext);

				} else if (iDtlRet == PD_NOT_FOUND) {
DEBUGLOG(("ProcessTpl() call DBEmailSetting::UpdateEmailSettingDefault() NOT FOUND!!!\n"));
