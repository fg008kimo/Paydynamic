...
DEBUGLOG(("CalculateAffectFXDT() acr_ind = [%d]\n",iACR));
				...
				if(cDirection == PD_FUNDS_OUT){  // && !iCal){
					...
					else if(!strcmp(csACRType,PD_TYPE_ACR)){  // && iACR){
...
DEBUGLOG(("Find full way fx rate [%s->%s] Failed!!!!\n",csOrgFromCcy,csOrgToCcy));
						...
						if(iRet == PD_OK){
							...
							dNewRate = newround(dNewBalance/((dFromAmt/dOrgHalfFxRate) + (dOrgFxBalance/dOrgFxRate)),PD_ROUND_UP_DEC);
...
DEBUGLOG(("CreditACR:: ()\n"));
	...
	if(!GetField_CString(hIn,"org_fx_id",&csOrgFxId)){
		iRet = INT_ERR;
DEBUGLOG(("CreditACR() org_fx_id is missing!!!!!\n"));
ERRLOG("BOMMMAverageCostRate: CreditACR() org_fx_id is missing!!!!!\n");
	}
	if(GetField_CString(hIn,"fx_id",&csTmp)){
		PutField_CString(hBal,"fx_id",csTmp);
	}
...
ERRLOG("BOMMMAverageCostRate::VoidAffectFx() fx_id not found\n");
	}
...
DEBUGLOG(("VoidAffectFx() acr_ind = [%d]\n",iACR));
				...
				if(!strcmp(csACRType,PD_TYPE_ENTITY_FX)){
					...
					if(cDirection == PD_FUNDS_OUT){
						iRet = VoidRemitEntityFx(hDetail);
					}
					else{
						iRet = VoidRecvEntityFx(hDetail);
					}
				}
				else if(!strcmp(csACRType,PD_TYPE_ACR)){
					...
					if(cDirection == PD_FUNDS_OUT){
						iRet = VoidRemitACR(hDetail);
					}
					else{
						iRet = VoidRecvACR(hDetail);
					}
				}
				else{
...
ERRLOG("BOMMMAverageCostRate::VoidAffectFx() GetAffectFXDT Failed!!!!!\n");
		}
