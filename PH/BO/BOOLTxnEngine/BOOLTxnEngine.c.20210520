...
-------------------------------                    ------------            --------------
Init Version                                       2014/12/09              [MSN]
Add fields for batch_txn_relation		   2015/05/15		   [MSN]
update check unknown txn function
	check txncode when post txn		   2015/06/23		   [MSN]
Add check manual_hold_recon in DoAction()	   2015/09/08		   [WMC]
Add Update DBOLBAIDTxn in DoPostTxn()		   2015/09/18		   [WMC]
Edit special handling for VOD & DCR                2015/10/30              [WCS]
Fix sys_match_ind                                  2020/05/14              [WCS]
Handle wait recon confirm lists                    2021/02/18              [WCS]
Reduce log					   2021/05/04		   [MSN]
*/
...
int	CheckBeforeStatus(const int iRuleId, const char *csLevel,const char *csTxnId, const char *csTriggerType);
...
int	DoAction(hash_t* hContext)
{
	...
	int  iTxnAction = 0;
	int  iActDtId = 0;
	int  iManualHoldRecon = PD_MANUAL_UNHOLD_RECON_DEFAULT;
	char *csTmp= NULL;
	char *csTxnCode= NULL;
	...
	char *csLinkBaidTxnId = NULL;
	char *csReconType = NULL;
	char *csBankStmtType = NULL;
	char *csTriggerType = NULL;
	double dTotalAmt = 0.0;
	double dTotalBaidAmt = 0.0;
	int iAddedHoldLog = PD_FALSE;
	char *csOrgTxnId = NULL;
	int iTmpRet;

	double dTmpTotalAmt = 0.0;
	double dTmpTotalBaidAmt = 0.0;
...
DEBUGLOG(("activity = [%s]\n", csTmp));
		...
		iRet = INT_ERR;
DEBUGLOG(("activity is missing!!!\n"));
ERRLOG("BOOLTxnEngine: DoAction:: activity is missing!!!\n");
	}
	
	//get bank stmt type
	if(GetField_CString(hContext,"bank_stmt_type",&csBankStmtType)){
		PutField_CString(hTxn,"bank_stmt_type",csBankStmtType);
DEBUGLOG(("bank_stmt_type = [%s]\n", csBankStmtType));
	}
	else{
		iRet = INT_ERR;
DEBUGLOG(("bank_stmt_type is missing!!!\n"));
ERRLOG("BOOLTxnEngine: DoAction:: bank_stmt_type is missing!!!\n");
	...
	if(GetField_CString(hContext,"trigger_type",&csTriggerType)){
		PutField_CString(hTxn,"trigger_type",csTriggerType);
DEBUGLOG(("trigger_type = [%s]\n", csTriggerType));
	}
	else{
		iRet = INT_ERR;
DEBUGLOG(("trigger_type is missing!!!\n"));
...
DEBUGLOG(("input_channel = [%s]\n", csTmp));
		...
		iRet = INT_ERR;
DEBUGLOG(("input_channel is missing!!!\n"));
...
DEBUGLOG(("recon_type = [%s]\n", csReconType));
		...
		iRet = INT_ERR;
DEBUGLOG(("recon_type is missing!!!\n"));
...
DEBUGLOG(("use_pending_fund = [%d]\n",iUsePF));
	}
	else{
DEBUGLOG(("use_pending_fund = [%d] (default)\n",iUsePF));
	}
...
DEBUGLOG(("include_interest = [%d]\n",iIncludeInterest));

		if(iIncludeInterest){
			if(GetField_Double(hContext,"interest",&dTmp)){
				PutField_Double(hTxn,"interest",dTmp);
DEBUGLOG(("interest = [%lf]\n",dTmp));
			}
		}
	}
	else{
DEBUGLOG(("include_interest = [%d] (default)\n",iIncludeInterest));
	}
...
DEBUGLOG(("baid_txn_cnt = [%d]\n", iBaidTxnCnt));
		}

		if (!strcmp(csBankStmtType, "BALANCE_TRANSFER_INTER") && !strcmp(csTriggerType, PD_TRIGGER_MANUAL)) {
			hash_init(hDetail, 0);
			if (GetField_CString(hContext, "txn_id_0", &csTmp)) {
				PutField_Char(hDetail, "party_type", PD_TYPE_PSP);
				PutField_CString(hDetail, "p2_txn_id", csTmp);
				PutField_Char(hDetail, "txn_level", PD_TXN_LEVEL);
				PutField_Char(hDetail, "relation_type", PD_REL_BAL_TRF);
				DBObjPtr = CreateObj(DBPtr, "DBOLTxnRelation", "GetP1TxnId");
				iTmpRet = (unsigned long)(*DBObjPtr)(hDetail, rTmp);
				if (iTmpRet == PD_FOUND) {
					hRec = RecordSet_GetFirst(rTmp);
					if (GetField_CString(hRec, "p1_txn_id", &csOrgTxnId)) {
						iTxnCnt++;
						PutField_Int(hContext, "txn_cnt", iTxnCnt);
						PutField_Int(hTxn, "txn_cnt", iTxnCnt);
						PutField_CString(hContext, "txn_id_1", csOrgTxnId);
					}
				}
...
/*
				if (GetTxnDetail(PD_TXN_HEADER_LEVEL, csTmp, hDetail, PD_FALSE) != PD_OK) {
					...
					if (GetField_CString(hDetail, "org_txn_id", &csOrgTxnId)) {
						iTxnCnt++;
						PutField_Int(hContext, "txn_cnt", iTxnCnt);
						PutField_Int(hTxn, "txn_cnt", iTxnCnt);
						PutField_CString(hContext, "txn_id_1", csOrgTxnId);
		...
		for(i=0; i<iTxnCnt; i++){
			...
			if(GetField_CString(hContext,csTag,&csTmp)){
				if(GetTxnDetail(PD_TXN_HEADER_LEVEL,csTmp,hDetail,PD_FALSE)!=PD_OK){
					...
					//check manual_hold_recon
					if(GetField_Int(hDetail,"manual_hold_recon",&iManualHoldRecon)){
                                                if ((iManualHoldRecon == PD_MANUAL_HOLD_RECON)
                                                    || ((iManualHoldRecon == PD_MANUAL_UNHOLD_RECON) && (!strcmp(csTriggerType,PD_TRIGGER_SYSTEM)))
                                                )
                                                {
                                                        iRet = INT_BAID_MANUAL_HOLD_RECON;
DEBUGLOG(("Check Manual Hold Recon Error!!!\n"));
                                                }
                                        }
					...
					//set the first txn_id to org_txn_id
					if(i==0)
		...
		for(i=0; i<iBaidTxnCnt; i++){
			...
			if(GetField_CString(hContext,csTag,&csTmp)){
				if(GetTxnDetail(PD_BAID_TXN_LEVEL,csTmp,hDetail,PD_FALSE)!=PD_OK){
					...
					//set the first txn_id to org_baid_txn_id
					if(i==0)
						PutField_CString(hTxn,"org_baid_txn_id",csTmp);
...
DEBUGLOG(("Check Manual Hold Recon Error!!!\n"));
					...
					if(!strcmp(csReconType,PD_ENGINE_RECON_RETURN) ||
					   !strcmp(csReconType,PD_ENGINE_RECON_RETURN_DSP)){
...
DEBUGLOG(("Total Txn Amt[%f], Total Baid Txn Amt[%f]\n", dTotalAmt,dTotalBaidAmt));
		if(dTotalAmt>0.0 && dTotalBaidAmt>0.0 && newround(dTotalBaidAmt, PD_DECIMAL_LEN)>=newround(dTotalAmt, PD_DECIMAL_LEN)){
...
DEBUGLOG((">>>>action batch id [%s] assigned<<<<\n", csTmpBatchSeq));
	}
...
DEBUGLOG(("txn_id[%s]\n",csLinkTxnId));
				...
				PutField_Char(hRel,"batch_sub_type",PD_GENERAL_RECON);
				PutField_Int(hRel,"is_input_txn",PD_TRUE);
				PutField_Int(hRel,"is_regen_txn",PD_FALSE);

				DBObjPtr = CreateObj(DBPtr, "DBOLBatchTxnRelation", "Add");
				if((unsigned long)(*DBObjPtr)(hRel)!=PD_OK){
					iRet = INT_ERR;
DEBUGLOG(("Add Action Batch Relation failed!!!\n"));
ERRLOG("BOOLTxnEngine: DoAction() Add Action Batch Relation failed!!!\n");
				}
				hash_destroy(hRel);
			}
			if(GetField_CString(hTxn,"baid_txn_id",&csLinkBaidTxnId)){
				PutField_CString(hTxn,"link_baid_txn_id",csLinkBaidTxnId);//for linkage
DEBUGLOG(("baid_txn_id[%s]\n",csLinkBaidTxnId));
				...
				PutField_Char(hRel,"batch_type",PD_GENERAL);
				PutField_CString(hRel,"txn_id", csLinkBaidTxnId);
				PutField_Char(hRel,"batch_sub_type",PD_GENERAL_RECON);
				PutField_Int(hRel,"is_input_txn",PD_TRUE);
				PutField_Int(hRel,"is_regen_txn",PD_FALSE);
...
DEBUGLOG(("posted_txn_id[%s]\n",csTmp));
			}
			if(GetField_CString(hTxn,"posted_baid_txn_id",&csTmp)){
DEBUGLOG(("posted_baid_txn_id[%s]\n",csTmp));
			}*/
...
DEBUGLOG(("bank_acct_type not found!!!\n"));
ERRLOG("BOOLTxnEngine: DoAction:: bank_acct_type not found!!!\n");
				iRet=INT_ERR;
			}
			else{
DEBUGLOG(("psp_txn bank_acct_type = [%s]\n", csBankAcctType));
			}
...
DEBUGLOG(("baid_txn bank_acct_type: from[%s] to[%s]\n", csFrBankAcctType,csToBankAcctType));
			}
...
DEBUGLOG(("baid_txn bank_acct_type: from[%s] to[%s]\n", csBankAcctType,csBankAcctType));
			...
			if (iRet == PD_OK) {
				...
				if ((unsigned long)(*DBObjPtr)(hTxn,rRecordSet) == PD_OK) {
					...
					while (hRec) {
						if(GetField_Int(hRec,"action_id",&iTmp)){
							PutField_Int(hRel,"action_id",iTmp);

							if(GetField_CString(hContext,"act_batch_id",&csTmp))
								PutField_CString(hRel,"batch_id",csTmp);

							if(GetField_Int(hContext,"use_pending_fund",&iTmp))
								PutField_Int(hRel,"use_pending_fund",iTmp);

							if(GetField_Double(hContext,"interest",&dTmp))
								PutField_Double(hRel,"interest",dTmp);

							if(GetField_Double(hContext,"bank_charge",&dTmp))
								PutField_Double(hRel,"bank_charge",dTmp);

							if(GetField_CString(hContext,"add_user",&csTmp))
								PutField_CString(hRel,"create_user",csTmp);
							else
								PutField_CString(hRel,"create_user",PD_UPDATE_USER);

						}

						if(GetField_Int(hRec,"action_seq",&iTmp)){
DEBUGLOG(("GetTxnEngine: Action Seq ####[%d]####\n",iTmp));
						}
...
DEBUGLOG((" Bank Statement record is missing!!!!\n"));
							}
						}
						if(GetField_Char(hRec,"input_system_txn",&cTmp)){
							if(cTmp==PD_MANDATORY && iTxnCnt==0){
								iRet = INT_INVALID_TXN;
DEBUGLOG((" PSP Transaction record is missing!!!!\n"));
							}
						}

						if(GetField_Int(hCheck,"allow_stmt_partial",&iTmp) &&
						   strcmp(csReconType,PD_ENGINE_RECON_RETURN) &&
						   strcmp(csReconType,PD_ENGINE_RECON_RETURN_DSP)){
							if(!iTmp && dTotalAmt>0.0 && dTotalBaidAmt>0.0 && newround(dTotalBaidAmt, PD_DECIMAL_LEN) > newround(dTotalAmt, PD_DECIMAL_LEN)){
								iRet = INT_INVALID_TXN;
DEBUGLOG((" NOT allow_stmt_partial! Total bank stmt amt should not greater than the total txn amt!!!!\n"));
							}
						}


						if(GetField_Int(hCheck,"allow_txn_partial",&iTmp) &&
						   strcmp(csReconType,PD_ENGINE_RECON_RETURN) &&
						   strcmp(csReconType,PD_ENGINE_RECON_RETURN_DSP)){
							if(!iTmp && dTotalAmt>0.0 && dTotalBaidAmt>0.0 && newround(dTotalBaidAmt, PD_DECIMAL_LEN) < newround(dTotalAmt, PD_DECIMAL_LEN)){
								iRet = INT_INVALID_TXN;
DEBUGLOG((" NOT allow_txn_partial! Total bank stmt amt should not smaller than the total txn amt!!!!\n"));
							}
						}

						if (iRet == PD_OK) {
							if(GetField_Int(hRec,"allow_stmt_partial",&iTmp) &&
							   !strcmp(csReconType, PD_ENGINE_RECON_RETURN_DSP)) {
								if (!iTmp) {
DEBUGLOG((" (Return Deposit) NOT allow_stmt_partial!\n"));
									dTmpTotalAmt = newround(dTotalAmt * 2, PD_DECIMAL_LEN);
									dTmpTotalBaidAmt = newround(dTotalBaidAmt, PD_DECIMAL_LEN);
DEBUGLOG((" (Return Deposit) Total Amt [%lf] BaidAmt [%lf]\n", dTmpTotalAmt, dTmpTotalBaidAmt));

									if (dTmpTotalBaidAmt > dTmpTotalAmt){
										iRet = INT_INVALID_TXN;
DEBUGLOG((" (Return Deposit) Total bank stmt amt should not greater than the  total txn amt!!!!\n"));
									}
								}
							}
						}
						...
						if (iRet == PD_OK) {
							if(GetField_Int(hRec,"allow_txn_partial",&iTmp) &&
							   !strcmp(csReconType, PD_ENGINE_RECON_RETURN_DSP)) {
								if (!iTmp) {
DEBUGLOG((" (Return Deposit) NOT allow_txn_partial!\n"));
									dTmpTotalAmt = newround(dTotalAmt * 2, PD_DECIMAL_LEN);
									dTmpTotalBaidAmt = newround(dTotalBaidAmt, PD_DECIMAL_LEN);
DEBUGLOG((" (Return Deposit) Total Amt [%lf] BaidAmt [%lf]\n", dTmpTotalAmt, dTmpTotalBaidAmt));
									...
									if (dTmpTotalBaidAmt < dTmpTotalAmt){
										iRet = INT_INVALID_TXN;
DEBUGLOG((" (Return Deposit) Total bank stmt amt should not smaller than the total txn amt!!!!\n"));
									}
								}
							}
						}
...
DEBUGLOG(("Special Action[%d] iRet [%d], is_run_rule [%d]\n",iTmp,iRet,iRunRule));
						}
...
DEBUGLOG(("bf_status_rule_id[%d], action_level[%s], txn_id[%s]\n",iTmp,csActionLevel,csTxnId));

								if(csTxnId){
									iRet = CheckBeforeStatus(iTmp,csActionLevel,csTxnId,csTriggerType);
									if(iRet!=PD_OK){
										iRunRule = PD_FALSE;
DEBUGLOG(("status checking failed!!!!!\n"));
...
DEBUGLOG(("txn_id is missing!!!!!\n"));
						...
						if(iRet==PD_OK && iRunRule==PD_TRUE){
							...
							if(GetField_Int(hRec,"action_detail_id",&iActDtId) &&
							   GetField_CString(hRec,"action_desc",&csTmp)){
DEBUGLOG(("Action Detail ID [%d] Desc [%s]\n",iActDtId, csTmp));
							}
							...
							if(iTxnAction==PD_ENGINE_ACTION_POST_TXN){
								...
								if(GetField_CString(hTxn,"fr_bank_acct_type",&csFrBankAcctType) &&
								   GetField_CString(hTxn,"to_bank_acct_type",&csToBankAcctType)){
									...
									if ((unsigned long)(*DBObjPtr)(iActDtId,
											csFrBankAcctType,
											csToBankAcctType,
											csTxnCode,
											hTxn) == PD_OK) {

										if(GetField_CString(hTxn,"new_txn_code",&csTmp)){

											DBObjPtr = CreateObj(DBPtr, "DBOLBAIDTxnCode", "IsUnknownTxn");
											if((unsigned long)(*DBObjPtr)(csTmp)==PD_FALSE){
												if(DoPostTxn(hTxn,hRec)!=PD_OK){
													iRet = INT_ERR;
DEBUGLOG(("DoPostTxn Failed!!!\n"));
ERRLOG("BOOLTxnEngine:: DoAction() DoPostTxn Failed!!!\n");
												}
											}
											else{
												iRet = INT_INVALID_TXN;
DEBUGLOG(("DoPostTxn Try To Post Unknown Txn Code. Stop Process!!!\n"));
ERRLOG("BOOLTxnEngine:: DoAction() DoPostTxn Try To Post Unknown Txn Code. Stop Process!!!\n");
											}
										}
										else{
											iRet = INT_ERR;
DEBUGLOG(("DBOLTxnEngineTxnCodeMap: GetNewTxnCode Failed!!!\n"));
...
ERRLOG("BOOLTxnEngine:: DoAction() GetNewTxnCode Failed!!!\n");
									...
									iRet = INT_ERR;
DEBUGLOG(("fr_bank_acct_type/to_bank_acct_type is missing!!!\n"));
ERRLOG("BOOLTxnEngine:: DoAction() fr_bank_acct_type/to_bank_acct_type is missing!!!\n");
								...
								if(DoUpdateTxn(hTxn,hRec)!=PD_OK){
									iRet = INT_ERR;
DEBUGLOG(("DoUpdateTxn Failed!!!\n"));
ERRLOG("BOOLTxnEngine:: DoAction() DoUpdateTxn Failed!!!\n");
								}
							}
/*
DEBUGLOG(("csBankStmtType = [%s]\n", csBankStmtType));
DEBUGLOG(("csTxnCode = [%s]\n", csTxnCode));
DEBUGLOG(("csActionLevel = [%s]\n", csActionLevel));
DEBUGLOG(("iTxnAction = [%d]\n", iTxnAction));
*/

							if(iRet==PD_OK && !iAddedHoldLog){
								/*program special handle*/
								//for return deposit, update the otp_txn_hold_ind of = 1 for OBD psp_txn
								if(!strcmp(csBankStmtType,PD_RETURN_DEPOSIT) &&
								   !strcmp(csTxnCode,PD_BANK_DEPOSIT_TXN_CODE)){
									...
									char	*csStatTxnId;
									char *csStmtRef = NULL;
									char csBuf[PD_REMARKS_LEN + 1];

									PutField_CString(hUpd,"txn_seq", csLinkTxnId);
									PutField_Int(hUpd,"txn_hold_ind", PD_TRUE);
									if(GetField_CString(hTxn,"add_user",&csTmp))
										PutField_CString(hUpd,"update_user",csTmp);
									
									DBObjPtr = CreateObj(DBPtr, "DBOLTxnPspDetail", "Update");
									iRet = (unsigned long)(*DBObjPtr)(hUpd);
DEBUGLOG(("DoAction() update [%s] otp_txn_hold_ind = 1, iRet = [%d]\n",csLinkTxnId,iRet));


									if (GetField_CString(hTxn, "stat_txn_id", &csStatTxnId)) {
										PutField_CString(hUpd, "stat_txn_id", csStatTxnId);

										// get stmt_ref
										DBObjPtr = CreateObj(DBPtr, "DBOLStatement", "GetStmtDtl");
										iRet = (unsigned long)(*DBObjPtr)(csStatTxnId, hStmtDtl);
										GetField_CString(hStmtDtl, "stmt_ref", &csStmtRef);
DEBUGLOG(("DoAction() stmt_ref = [%s]\n", csStmtRef));
									}

									if (csStmtRef != NULL) {
										// add remarks
										sprintf(csBuf, "Transaction was held as the Deposit Chargeback txn, Ref. %s is found", csStmtRef);
										PutField_CString(hUpd, "remark", csBuf);
									} else {
										PutField_CString(hUpd, "remark", "System Hold");
									}
									if(GetField_CString(hTxn,"add_user",&csTmp))
										PutField_CString(hUpd,"add_user",csTmp);
									DBObjPtr = CreateObj(DBPtr, "DBOLTxnDepositHoldLog", "Add");
									iRet = (unsigned long)(*DBObjPtr)(hUpd);
DEBUGLOG(("DoAction() Add TxnDepositHoldLog Add [%s] [%s], iRet = [%d]\n",csLinkTxnId, csStatTxnId, iRet));
									...
									iAddedHoldLog = PD_TRUE;
								} else if (!strcmp(csBankStmtType, PD_CHARGEBACK_REVERSAL) &&
										(!strcmp(csTxnCode, PD_TXN_CODE_RETURNED_DEPOSIT) || !strcmp(csTxnCode, PD_TXN_CODE_UNKNOWN_DEBIT)) &&
										(iTxnAction == PD_ENGINE_ACTION_POST_TXN) &&
										(!strcmp(csActionLevel, PD_TXN_HEADER_LEVEL))) {
									...
									char *csOrgBaidTxnId = NULL;
									char *csStatTxnId = NULL;
									char *csStmtRef = NULL;
									char csBuf[PD_REMARKS_LEN + 1];

									if (GetField_CString(hTxn, "posted_txn_id", &csNewTxnId)) {
DEBUGLOG(("DoAction() vod_txn_id = [%s]\n", csNewTxnId));
										if (GetField_CString(hTxn, "org_baid_txn_id", &csOrgBaidTxnId)) {
DEBUGLOG(("DoAction() dcr_baid_txn_id = [%s]\n", csOrgBaidTxnId));
											// get stat_txn_id
											DBObjPtr = CreateObj(DBPtr, "DBOLBAIDTxn", "GetBaidTxn");
											iRet = (unsigned long)(*DBObjPtr)(csOrgBaidTxnId, hStmtDtl);
											if (GetField_CString(hStmtDtl, "stat_txn_id", &csStatTxnId)) {
DEBUGLOG(("DoAction() dcr_stat_txn_id = [%s]\n", csStatTxnId));
												hash_destroy(hStmtDtl);
												hash_init(hStmtDtl, 0);
												// get stmt_ref
												DBObjPtr = CreateObj(DBPtr, "DBOLStatement", "GetStmtDtl");
												iRet = (unsigned long)(*DBObjPtr)(csStatTxnId, hStmtDtl);
												if (GetField_CString(hStmtDtl, "stmt_ref", &csStmtRef)) {
DEBUGLOG(("DoAction() dcr_stmt_ref = [%s]\n", csStmtRef));
													// add remarks
													sprintf(csBuf, "Deposit Chargeback Reversal by Bank txn, Ref. %s is found", csStmtRef);
													PutField_CString(hUpd, "remark", csBuf);
													PutField_CString(hUpd, "txn_seq", csNewTxnId);
													if (GetField_CString(hTxn, "add_user", &csTmp))
														PutField_CString(hUpd, "update_user", csTmp);

													DBObjPtr = CreateObj(DBPtr, "DBOLTransaction", "UpdateDetailRemark");
													iRet = (unsigned long)(*DBObjPtr)(hUpd);
DEBUGLOG(("DoAction() update [%s] remark, iRet = [%d]\n", csNewTxnId, iRet));
												}
											}
										}
									}
...
ERRLOG("BOOLTxnEngine:: DoAction() DoLinkage Failed!!!\n");
					...
					if(iRet==PD_OK){
						//Add OLBatchEngineActionLog
						DBObjPtr = CreateObj(DBPtr, "DBOLBatchEngineActionLog", "Add");
						iRet = (unsigned long)(*DBObjPtr)(hRel);
DEBUGLOG(("DBOLBatchEngineActionLog::Add iRet = [%d]\n",iRet));
					}
				}
				else{
					iNextAction = PD_FALSE;
					iRet = INT_ERR;
DEBUGLOG(("TxnEngine Rule Not found!!!\n"));
ERRLOG("BOOLTxnEngine:: DoAction() TxnEngine Rule Not found!!!\n");
				}
				hash_destroy(hCheck);
				hash_destroy(hRel);
			}
...
DEBUGLOG(("Loop End\n"));
	...
	if(iRet==PD_OK){
		iTmpRet = PD_OK;
		hash_t *hDel;
		hDel = (hash_t*) malloc (sizeof(hash_t));
		...
		for(i = 0; i < iBaidTxnCnt; i++) {
			...
			sprintf(csTag, "baid_txn_id_%d", i);
			if (GetField_CString(hContext, csTag, &csTmp)) {
				PutField_CString(hDel, "baid_txn_id", csTmp);
			}

			if (iTmpRet == PD_OK) {
DEBUGLOG(("DBOLWaitReconCfmList::DeleteOr\n"));
				DBObjPtr = CreateObj(DBPtr, "DBOLWaitReconCfmList", "DeleteOr");
				iTmpRet = (unsigned long)(*DBObjPtr)(hDel);
DEBUGLOG(("iTmpRet = [%d]\n", iTmpRet));
			}

			if (iTmpRet == PD_OK) {
DEBUGLOG(("DBOLWaitPayoutReconCfmList::DeleteBAID()\n"));
				DBObjPtr = CreateObj(DBPtr, "DBOLWaitPayoutReconCfmList", "DeleteBAID");
				iTmpRet = (unsigned long)(*DBObjPtr)(hDel);
DEBUGLOG(("iTmpRet = [%d]\n", iTmpRet));
			}
		...
		for(i = 0; i < iTxnCnt; i++) {
			...
			sprintf(csTag, "txn_id_%d", i);
			if (GetField_CString(hContext, csTag, &csTmp)) {
				PutField_CString(hDel, "psp_txn_id", csTmp);
			}

			if (iTmpRet == PD_OK) {
DEBUGLOG(("DBOLWaitPayoutReconCfmList::DeleteTxn()\n"));
				DBObjPtr = CreateObj(DBPtr, "DBOLWaitPayoutReconCfmList", "DeleteTxn");
				iTmpRet = (unsigned long)(*DBObjPtr)(hDel);
DEBUGLOG(("iTmpRet = [%d]\n", iTmpRet));
			}
	...
	if(iRet!=PD_OK){
		if(!strcmp(csTriggerType,PD_TRIGGER_MANUAL))
			TxnAbort();
...
void	ConvertHash(const hash_t* hIn, hash_t* hOut, const int iIdx, const char* csMode)
{
	...
	if(!strcmp(csMode,PD_MULTIPLE_TO_SINGLE)){
		...
		sprintf(csTag,"manual_hold_recon_%d",iIdx);
		if(GetField_Int(hIn,csTag,&iTmp)){
			PutField_Int(hOut,"manual_hold_recon",iTmp);
		}
		...
		sprintf(csTag,"est_net_amount_%d",iIdx);
		if(GetField_Double(hIn,csTag,&dTmp)){
			PutField_Double(hOut,"est_net_amount",dTmp);
		}

		sprintf(csTag,"grp_tracking_txn_seq_%d",iIdx);
		if(GetField_CString(hIn,csTag,&csTmp)){
			PutField_CString(hOut,"grp_tracking_txn_seq",csTmp);
		}

		sprintf(csTag,"tracking_txn_seq_%d",iIdx);
		if(GetField_CString(hIn,csTag,&csTmp)){
			PutField_CString(hOut,"tracking_txn_seq",csTmp);
		}

		sprintf(csTag,"report_date_%d",iIdx);
		if(GetField_CString(hIn,csTag,&csTmp)){
			PutField_CString(hOut,"report_date",csTmp);
		}

		sprintf(csTag,"settlement_date_%d",iIdx);
		if(GetField_CString(hIn,csTag,&csTmp)){
			PutField_CString(hOut,"settlement_date",csTmp);
		}
		...
		sprintf(csTag,"manual_hold_recon_%d",iIdx);
		if(GetField_Int(hIn,"manual_hold_recon",&iTmp)){
			PutField_Int(hOut,csTag,iTmp);
		}
		...
		sprintf(csTag,"est_net_amount_%d",iIdx);
		if(GetField_Double(hIn,"est_net_amount",&dTmp)){
			PutField_Double(hOut,csTag,dTmp);
		}

		sprintf(csTag,"grp_tracking_txn_seq_%d",iIdx);
		if(GetField_CString(hIn,"grp_tracking_txn_seq",&csTmp)){
			PutField_CString(hOut,csTag,csTmp);
		}

		sprintf(csTag,"tracking_txn_seq_%d",iIdx);
		if(GetField_CString(hIn,"tracking_txn_seq",&csTmp)){
			PutField_CString(hOut,csTag,csTmp);
		}

		sprintf(csTag,"report_date_%d",iIdx);
		if(GetField_CString(hIn,"report_date",&csTmp)){
			PutField_CString(hOut,csTag,csTmp);
		}

		sprintf(csTag,"settlement_date_%d",iIdx);
		if(GetField_CString(hIn,"settlement_date",&csTmp)){
			PutField_CString(hOut,csTag,csTmp);
		}
	}
...
DEBUGLOG(("psp_txn_amt[%lf]\n",dAmt));
			...
			if (GetField_CString(hTmp, "grp_tracking_txn_seq", &csTmp)) {
				PutField_CString(hTxn, "grp_tracking_txn_seq", csTmp);
DEBUGLOG(("grp_tracking_txn_seq [%s]\n",csTmp));
			}
			if (GetField_CString(hTmp, "tracking_txn_seq", &csTmp)) {
				PutField_CString(hTxn, "tracking_txn_seq", csTmp);
DEBUGLOG(("tracking_txn_seq [%s]\n",csTmp));
			}
			if (GetField_CString(hTmp, "report_date", &csTmp)) {
				PutField_CString(hTxn, "report_date", csTmp);
DEBUGLOG(("report_date [%s]\n",csTmp));
			}
			if (GetField_CString(hTmp, "settlement_date", &csTmp)) {
				PutField_CString(hTxn, "settlement_date", csTmp);
DEBUGLOG(("settlement_date [%s]\n",csTmp));
			}
			if (GetField_Int(hTmp, "manual_hold_recon", &iTmp)) {
...
DEBUGLOG(("manual_hold_recon [%d]\n",iTmp));
			...
			if(GetField_CString(hTmp,"bank_code",&csTmp)){
				...
				if((unsigned long)(*DBObjPtr)(csTmp, csTxnCountry)){
					if(iIsPostedTxn) PutField_CString(hTxn,"posted_txn_country",csTxnCountry);
					else		 PutField_CString(hTxn,"txn_country",csTxnCountry);
DEBUGLOG(("txn_country [%s]\n",csTxnCountry));
				}
			}
			*/
DEBUGLOG(("DBOLBankAcctId:GetBankAcctIdCountry\n"));
			char csTxnCountry[PD_COUNTRY_LEN + 1];
...
DEBUGLOG(("baid_bank_acct_type [%s]\n",csBankAcctType));
					}
				}
			}
...
DEBUGLOG(("baid_baid [%s]\n",csBaid));

				DBObjPtr = CreateObj(DBPtr, "DBOLBankAcctId", "GetBankAcctIdDtl");
				if((unsigned long)(*DBObjPtr)(csBaid, hTmp)==PD_OK){
					if(GetField_CString(hTmp,"category",&csTmp)){
						PutField_CString(hTxn,"baid_baid_category",csTmp);
DEBUGLOG(("baid_baid_category [%s]\n",csTmp));
...
DEBUGLOG(("baid_txn_amt [%lf]\n",dTmp));
			...
			if(GetField_Double(hBaidTxn,"est_net_amount", &dTmp)){
				PutField_Double(hTxn, "est_net_amount", dTmp);
			}
			if(GetField_Int(hBaidTxn,"manual_hold_recon",&iTmp)){
DEBUGLOG(("manual_hold_recon [%d]\n",iTmp));
				PutField_Int(hTxn, "manual_hold_recon", iTmp);
                        }
...
DEBUGLOG(("baid_amt_type[%s]\n",csAmtType));
		...
		DBObjPtr = CreateObj(DBPtr, "DBOLBAIDTxnCode", "IsUnknownTxn");
		iIsUnknown = (unsigned long)(*DBObjPtr)(csTxnCode);
                if(iIsUnknown!=PD_ERR){
			PutField_Int(hTxn,"unknown_txn",iIsUnknown);
DEBUGLOG(("[%s] is unknown txn[%d]\n",csTxnCode,iIsUnknown));
		}
		else{
			iRet = INT_ERR;
DEBUGLOG(("Check Unknown Txn Failed!!!\n"));
ERRLOG("BOOLTxnEngine:: GetTxnDetail() Check Unknown Txn Failed!!!\n");
		}
...
int	DoPostTxn(hash_t* hContext,hash_t* hRequest)
{
	...
	hash_t          *hTxnHold;
        hTxnHold = (hash_t*) malloc (sizeof(hash_t));
        hash_init(hTxnHold,0);
...
DEBUGLOG(("is_offset [%d]\n",iIsOffset));
	...
	if ((unsigned long)(*DBObjPtr)("CTPHDATE",csTmpDate) != FOUND) {
		iRet = INT_ERR;
DEBUGLOG(("current date not found!!!!!\n"));
...
DEBUGLOG(("baid_txn: rule[%s] org[%s]\n",csBaidCat,csTmpBaidCat));
								if(!strcmp(csBaidCat,csTmpBaidCat)){
...
DEBUGLOG(("psp_id[%s] category[%s] -> baid\n",csTmp,csBaidCat));
								DBObjPtr = CreateObj(DBPtr, "DBOLBankAcctId", "GetBaidByPspCategory");
								iRet = (unsigned long)(*DBObjPtr)(csTmp,csBaidCat,hTxn);

								if(iRet!=PD_OK){
									//use provider, baid_category =>get baid
									DBObjPtr = CreateObj(DBPtr, "DBOLBankAcctId", "GetBaidByProviderCategory");
									iRet = (unsigned long)(*DBObjPtr)(csTmp,csBaidCat,hTxn);
								}
							}
						}
...
DEBUGLOG(("BOOLBalance:UpdateAmount iRet=[%d]\n",iRet));
						}
						if(iRet == PD_OK){
...
DEBUGLOG(("settlement_date [%s]\n", csTmp));
							...
							if (GetField_CString(hTxn, "new_cat_psp_id", &csTmp)) {
								if (GetField_CString(hTxn, "psp_id", &csOrgPspId)) {
DEBUGLOG(("Replace psp_id from [%s] to [%s]\n", csOrgPspId, csTmp));
									PutField_CString(hTxn, "psp_id", csTmp);
								}
							}
...
DEBUGLOG(("DBOLTxnPspDetail:Add/Update iRet=[%d]\n",iRet));

							if (GetField_CString(hTxn, "new_cat_psp_id", &csTmp)) {
								if (GetField_CString(hTxn, "psp_id", &csTmp)) {
DEBUGLOG(("Resume psp_id from[%s] to [%s]\n", csTmp, csOrgPspId));
									PutField_CString(hTxn, "psp_id", csOrgPspId);
								}
							}
						}
						if(iRet == PD_OK){
							...
							iRet = (unsigned long)(*BOObjPtr)(hTxn);
DEBUGLOG(("DBOLTxnElements:AddPspTxnElement iRet=[%d]\n",iRet));
						}
					}
				}
			}
...
DEBUGLOG(("do update status\n"));
				//get status rule
				hash_init(hTxn,0);
				PutField_CString(hTxn,"txn_seq",(const char*)csTmpTxnSeq);

				if(!iIsOffset){
DEBUGLOG(("set NOT Offset Information\n"));
					PutField_Double(hTxn,"txn_amt",dAmt);
...
DEBUGLOG(("set Offset Information\n"));
				...
				if((unsigned long)(*DBObjPtr)(iTmp, hTxn)==PD_OK){
					...
					if((unsigned long)(*DBObjPtr)(hTxn)!=PD_OK){
						iRet = INT_ERR;
DEBUGLOG(("update status failed\n"));
ERRLOG("BOOLTxnEngine:: DoPostTxn() update status failed!!!\n");
					}
				}
				else{
					iRet = INT_TXN_ENGINE_CONFIG_MISSING;
DEBUGLOG(("status rule not found\n"));
ERRLOG("BOOLTxnEngine:: DoPostTxn() status rule not found!!!\n");
			...
			if(iRet==PD_OK){
				...
				PutField_Char(hRel,"batch_sub_type",PD_GENERAL_RECON);
				PutField_Int(hRel,"is_input_txn",PD_FALSE);
				PutField_Int(hRel,"is_regen_txn",PD_FALSE);
...
ERRLOG("BOOLTxnEngine:: DoPostTxn() status rule not found\n");
				...
				iRet = INT_TXN_ENGINE_CONFIG_MISSING;
DEBUGLOG(("no status rule in the action\n"));
ERRLOG("BOOLTxnEngine:: DoPostTxn() no status rule in the action\n");
			...
			if(iRet==PD_OK){
				...
				if(GetField_Int(hContext,"manual_hold_recon",&iTmp))
					PutField_Int(hTxn,"manual_hold_recon",iTmp);

				if(GetField_Int(hContext,"sys_match_ind",&iTmp))
					PutField_Int(hTxn,"pid_sys_match_ind",iTmp);
...
DEBUGLOG(("baid_category_default: [%s]\n",PD_BAID_CATEGORY_DEFAULT));
...
DEBUGLOG(("psp_id: [%s]\n",csTmp));
				...
				if((unsigned long)(*DBObjPtr)(hTxn)!=PD_OK){
					iRet = INT_ERR;
DEBUGLOG(("Add BAID Txn failed\n"));
ERRLOG("BOOLTxnEngine:: DoPostTxn() Add BAID Txn failed\n");
				} else {

					if(GetField_CString(hTxn,"txn_seq",&csTmp)) {
DEBUGLOG(("baid_txn_id: [%s]\n",csTmp));
                                                PutField_CString(hTxnHold,"txn_seq",csTmp);
                                        }

					if(GetField_Int(hTxn,"manual_hold_recon",&iTmp)) {
DEBUGLOG(("manual_hold_recon: [%d]\n",iTmp));
						PutField_Int(hTxnHold,"manual_hold_recon",iTmp);
                                	}	

					DBObjPtr = CreateObj(DBPtr, "DBOLBAIDTxn", "Update");
					if((unsigned long)(*DBObjPtr)(hTxnHold)!=PD_OK){
						iRet = INT_ERR;
DEBUGLOG(("Update BAID Txn failed\n"));
ERRLOG("BOOLTxnEngine:: DoPostTxn() Update BAID Txn failed\n");
					}
				}
...
ERRLOG("BOOLTxnEngine:: DoUpdateTxn() posted_baid_txn_id missing!!!!!!\n");
		...
		if(GetField_CString(hContext,"posted_txn_id",&csTxnId)){
			...
			iRet = INT_ERR;
DEBUGLOG(("posted_txn_id missing!!!!!!\n"));
...
DEBUGLOG(("BOOLBalance:UpdateAmount iRet=[%d]\n",iRet));
						if(iRet == PD_OK){
							...
							if((unsigned long)(*DBObjPtr)(hTxn)!=PD_OK){
								iRet = INT_ERR;
DEBUGLOG(("DBOLTxnPspDetail:Update failed!!!\n"));
ERRLOG("BOOLTxnEngine:: DoUpdateTxn() DBOLTxnPspDetail:Update failed!!!\n");
						...
						if(iRet == PD_OK){
							...
							if((unsigned long)(*BOObjPtr)(hTxn)!=PD_OK){
								iRet = INT_ERR;
DEBUGLOG(("DBOLTxnElements:AddPspTxnElement Failed!!\n"));
...
DEBUGLOG(("exp recon_status:[%c] now:[%c]\n",cRuleReconStatus,cReconStatus));
			...
			if(!strcmp(csTriggerType,PD_TRIGGER_MANUAL)){
				//lock txn when manual mode
				DBObjPtr = CreateObj(DBPtr, "DBOLBAIDTxn", "GetTxnIdForUpdate");
				iRet = (unsigned long)(*DBObjPtr)(csTxnId);
				if (iRet != PD_OK) {
DEBUGLOG(("DBOLBAIDTxn::GetTxnIdForUpdate() failed\n"));
					iRet = INT_ERR;
				}
			}
...
DEBUGLOG(("link_level [%s]\n",csLinkLevel));
		}
		if(GetField_Int(hRule,"is_override",&iIsOverride)){
DEBUGLOG(("is_override [%d]\n",iIsOverride));
		}
		if(GetField_Char(hRule,"relation_type",&cRelType)){
DEBUGLOG(("relation_type [%c]\n",cRelType));
		}
	
		if(GetField_CString(hContext,"org_txn_id",&csOrgTxnId)){
DEBUGLOG(("org txn_id [%s]\n",csOrgTxnId));
		}

		if(GetField_CString(hContext,"org_baid_txn_id",&csOrgBaidTxnId)){
DEBUGLOG(("org baid_txn_id [%s]\n",csOrgBaidTxnId));
		}


		if(GetField_CString(hContext,"link_txn_id",&csLinkTxnId)){
DEBUGLOG(("link txn_id [%s]\n",csLinkTxnId));
		}

		if(GetField_CString(hContext,"link_baid_txn_id",&csLinkBaidTxnId)){
DEBUGLOG(("link baid_txn_id [%s]\n",csLinkBaidTxnId));
		}
...
DEBUGLOG(("new [%s] txn_id [%s]\n",csLevel,csNewTxnId));
				...
				iRet = INT_ERR;
DEBUGLOG(("invalid action level[%s]\n",csLevel));
...
ERRLOG("BOOLTxnEngine:: DoLinkage() invalid action [%d]\n",iAction);
	...
	if(iRet == PD_OK){
		...
		if(!strcmp(csLinkLevel,"SAME") || !strcmp(csLinkLevel,"BOTH")){
			if( (!strcmp(csLevel,PD_BAID_TXN_LEVEL) ||
			     !strcmp(csLevel,PD_POST_BAID_TXN_LEVEL)) && csLinkBaidTxnId){
				//P, csLinkBaidTxnId, csNewTxnId, S, L
				PutField_CString(hRule,"p1_txn_id",csLinkBaidTxnId);	
				PutField_Char(hRule,"txn_level",PD_STMT_LEVEL);	
				iDoAdd = PD_TRUE;
			}
			else if( (!strcmp(csLevel,PD_TXN_HEADER_LEVEL) ||
				  !strcmp(csLevel,PD_POST_TXN_HEADER_LEVEL)) && csLinkTxnId){
				//P, csLinkTxnId, csNewTxnId, T, L
				PutField_CString(hRule,"p1_txn_id",csLinkTxnId);	
...
ERRLOG("BOOLTxnEngine:: DoLinkage() DBOLTxnRelation:Add Failed!!!\n");
		...
		if(!strcmp(csLinkLevel,"CROSS") || !strcmp(csLinkLevel,"BOTH") || !strcmp(csLinkLevel,"SAME_ORG_CROSS")){
			if( (!strcmp(csLevel,PD_BAID_TXN_LEVEL) ||
			     !strcmp(csLevel,PD_POST_BAID_TXN_LEVEL)) && csLinkTxnId){
				//csNewTxnId, csLinkTxnId
				PutField_CString(hRule,"stmt_txn_id",csNewTxnId);	
				PutField_CString(hRule,"txn_id",csLinkTxnId);	
				iDoAdd = PD_TRUE;
			}
			else if( (!strcmp(csLevel,PD_TXN_HEADER_LEVEL) ||
				  !strcmp(csLevel,PD_POST_TXN_HEADER_LEVEL)) &&csLinkBaidTxnId){
				//csLinkBaidTxnId, csNewTxnId
				PutField_CString(hRule,"stmt_txn_id",csLinkBaidTxnId);	
			...
			if(iDoAdd){
				...
				if((unsigned long)(*DBObjPtr)(hRule)!=PD_OK){
					iRet=INT_ERR;
DEBUGLOG(("DBOLStmtTxnRelation:Add Failed!!!\n"));
ERRLOG("BOOLTxnEngine:: DoLinkage() DBOLStmeTxnRelation:Add Failed!!!\n");
				}
			}
		}
		if(!strcmp(csLinkLevel,"SAME_ORG_CROSS")){
			if( (!strcmp(csLevel,PD_BAID_TXN_LEVEL) ||
