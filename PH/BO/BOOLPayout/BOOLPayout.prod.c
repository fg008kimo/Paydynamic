...
DEBUGLOG(("DBRulePayoutPsp:GetExistingPsp psp_id [%s]\n",csTmp));
				...
				if(GetField_CString(hRec,"to_bank_code",&csTmp)){
					sprintf(csTag, "to_bank_code_%d", iCnt);
					PutField_CString(hContext,csTag,csTmp);
DEBUGLOG(("DBRulePayoutPsp:GetExistingPsp to_bank_code[%s]\n",csTmp));
				}
				iCnt++;
				hRec = RecordSet_GetNext(rRecordSet);
			}
...
DEBUGLOG(("BOOLPayout::handle_PayoutGenerate: GetPspDetail - client id = [%s]\n",csTmp));
					}
					if (GetField_Int(hPsp,"payout_format",&iFormat)) {
DEBUGLOG(("BOOLPayout::handle_PayoutGenerate: GetPspDetail - payout_format = [%d]\n",iFormat));
					}
				}
				FREE_ME(hPsp);
			}
...
/* no need to create pending POG transaction
					if(iRet==PD_OK){
...
DEBUGLOG(("handle_PayoutGenerate::DBOLTransaction:Update Failed\n"));
					...
					if(iRet == PD_OK){
						...
						if ((unsigned long)(DBObjPtr)(PD_PAYOUT_BANK_STM,csValueTmp) == FOUND) {
							cTmp = csValueTmp[0];
DEBUGLOG(("handle_PayoutGenerate FindCode need Payout Bank Statement = [%c]\n",cTmp));
							if(cTmp == PD_YES){
								iNeedBankStm = PD_TRUE;
							}
						}
						FREE_ME(csValueTmp);
					}
...
/* no need to create pending POG transaction	
					//: update ol_txn_header, add ol_txn_detail, add ol_txn_psp_detail
					//---------txn_header
					//status
					if(iNeedBankStm){
					...
					//txn_amt//net_amt
					PutField_Double(hContext,"txn_amt",dPayoutAmt);
					PutField_Double(hContext,"net_amt",dPayoutAmt);
					//txn_ccy//net_ccy
					PutField_CString(hContext,"txn_ccy",csPayoutCcy);
					PutField_CString(hContext,"net_ccy",csPayoutCcy);
					DBObjPtr = CreateObj(DBPtr,"DBOLTransaction","Update");
					...
					if(iRet == PD_OK){
						...
						if((unsigned long) ((*DBObjPtr)(hContext))!=PD_OK){
							iRet = INT_ERR;
DEBUGLOG(("handle_PayoutGenerate::DBOLTransaction:AddDetail Failed\n"));
						}
					}
					...
					PutField_Double(hContext,"txn_amount",dPayoutAmt);
					if(iRet == PD_OK){
						DBObjPtr = CreateObj(DBPtr,"DBOLTxnPspDetail","Add");
						if((unsigned long) ((*DBObjPtr)(hContext))!=PD_OK){
							iRet = INT_ERR;
DEBUGLOG(("handle_PayoutGenerate::DBOLTxnPspDetail:Add Failed\n"));
						}
					}
*/
					i++;
...
DEBUGLOG(("While Loop Strat  iCnt[%d], Split Cnt[%d]\n",iCnt,iSplitCnt));
					//struct timespec tv;
					//clock_gettime(CLOCK_REALTIME, &tv);
					struct timeval  tv;
					gettimeofday(&tv, NULL);
DEBUGLOG(("WriteAssignedRecord:: gettimeofday \n"));
					unsigned int time_msec = (tv.tv_sec)*1000 + (tv.tv_usec)/1000;
					srand(time_msec+iCnt);
DEBUGLOG(("WriteAssignedRecord:: srand(%d+%d)\n",time_msec,iCnt));

					int iRand = (int)(rand()%iRandMax);
DEBUGLOG(("WriteAssignedRecord:: iRand [%d]\n",iRand));
					double dRandAmt = newround(((double)iRand/dIdx),2) + dAddAmt;

DEBUGLOG(("WriteAssignedRecord:: dRandAmt : [%lf]\n",dRandAmt));
...
DEBUGLOG(("WriteAssignedRecord:: dLeftAmt<=dTmpAmt: [%lf][%lf], break loop\n",dLeftAmt,dTmpAmt));
						break;
					}
				}
...
*/
				...
				if(GetField_Double(hContext,csTag,&dTmp)){
					PutField_Double(hDetail,"payout_amount",dTmp);
DEBUGLOG(("WriteAssignedRecord:: GeneratePayoutAmount: [%d][%d]=[%f]\n",i,iTmp,dTmp));
				}
