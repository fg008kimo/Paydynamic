...
update HandleGeneratePayoutApi                     2018/07/03              Elvis Wong
*/
...
//check Reason, if null, put default value "交易失敗"//
        if(iRet == PD_OK){
		sprintf(csTag,"remark_%d",iRecord);
		if(!strlen(csList[RT_IDX_DETAIL_REASON])){
			PutField_CString(hTxn,csTag,"交易失敗");
			PutField_CString(hTxn,"reason","交易失敗");
		}
		else{
...
DEBUGLOG(("Call UpdateTxnLog()\n"));
                                                                iRet = UpdateTxnLog(hTxn);
                                                                if (iRet == PD_OK) {
DEBUGLOG(("Call UpdateTxnLog() Success!!\n"));
...
DEBUGLOG(("Get All Pregen Records for Update - End\n"));				
	...
	if (iRet == PD_OK) {
		...
		if (GetField_Int(hContext,"psp_count",&iPspCnt)) {
...
/* txn_cnt */
					sprintf(csTag,"%s_%s_txn_cnt",csPspId,csBankCode);
					GetField_Int(hContext,csTag,&iTmp);
					PutField_Int(hTxn,"txn_cnt",iTmp);
