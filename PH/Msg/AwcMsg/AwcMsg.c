
/*
 * PDProTech (c)2017. All rights reserved. No part of this software may be reproduced in any form without written permission
 * of an authorized representative of PDProTech.
 *
 * Change Description                                 Change Date             Change By
 * -------------------------------                    ------------            --------------
 *  Init Version                                      2017/01/09              David Wong
 *
 */

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <ctype.h>
#include "AwcMsg.h"
#include "common.h"
#include "utilitys.h"
#include "queue_defs.h"
#include <zlib.h>
#include "b64.h"
#include <time.h>
#include "ObjPtr.h"
#include <json-c/json.h>

char	cDebug;

struct key_value_pair
{
	char key[PD_TMP_BUF_LEN];
	char value[PD_TMP_MSG_BUF_LEN];
};

OBJPTR(DB);

void	AwcMsg(char cdebug)
{
	cDebug = cdebug;
}


int FormatMsg(const hash_t* hIn,unsigned char *outMsg,int *outLen)
{
	int	iRet = PD_OK;

	char*	csPtr = NULL;
	char*	csBuf;
	char*	csCodeURL= NULL;
	int	iTmp;
	int	iTmpRet;

	hash_t	*hLog;
	hLog = (hash_t*) malloc (sizeof(hash_t));
	hash_init(hLog,0);

DEBUGLOG(("FormatMsg()\n"));

	csCodeURL = (char*) malloc (PD_TMP_MSG_BUF_LEN+1);
	memset(csCodeURL,0,sizeof(csCodeURL));

	outMsg[0]= '\0';

//return_code
	if (GetField_CString(hIn,"return_code",&csPtr)) {
DEBUGLOG(("FormatMsg:: return_code = [%s]\n",csPtr));
		strcat((char*)outMsg,"return_code");
		strcat((char*)outMsg,MY_AWC_FIELD_TOKEN);
		strcat((char*)outMsg,csPtr);
		strcat((char*)outMsg,MY_AWC_TOKEN);
	} else {
DEBUGLOG(("FormatMsg:: return_code is missing\n"));
	}

//code_url
	if (GetField_CString(hIn,"code_url",&csPtr)) {
DEBUGLOG(("FormatMsg:: code_url = [%s]\n",csPtr));


		strcat((char*)outMsg,"code_url");
		strcat((char*)outMsg,MY_AWC_FIELD_TOKEN);
		base64_encode((unsigned char *)csPtr,strlen((char*)csPtr),csCodeURL,PD_MAX_BUFFER);
		strcat((char*)outMsg,csCodeURL);
		strcat((char*)outMsg,MY_AWC_TOKEN);
	} else {
DEBUGLOG(("FormatMsg:: code_url is missing\n"));
	}

//txn_seq
	if (GetField_CString(hIn,"txn_seq",&csPtr)) {
DEBUGLOG(("FormatMsg:: txn_seq = [%s]\n",csPtr));
		DBObjPtr = CreateObj(DBPtr,"DBTxnQrRequestLog","GetByTxnId");
		iTmpRet = (unsigned long int)(*DBObjPtr)(csPtr,hLog);
		if (iTmpRet == PD_FOUND) {
//time_init
			if (GetField_CString(hLog,"qrcode_init_timestamp",&csPtr)) {
DEBUGLOG(("FormatMsg:: time_init = [%s]\n",csPtr));
				strcat((char*)outMsg,"time_init");
				strcat((char*)outMsg,MY_AWC_FIELD_TOKEN);
				strcat((char*)outMsg,csPtr);
				strcat((char*)outMsg,MY_AWC_TOKEN);
			} else {
DEBUGLOG(("FormatMsg:: time_init is missing\n"));
			}

//time_expire
			if (GetField_Int(hLog,"expiry",&iTmp)) {
				char csTimeExpire[PD_TMP_BUF_LEN + 1];
				sprintf((char*)csTimeExpire,"%d",iTmp);
DEBUGLOG(("FormatMsg:: time_expire = [%s]\n",csTimeExpire));
				strcat((char*)outMsg,"time_expire");
				strcat((char*)outMsg,MY_AWC_FIELD_TOKEN);
				strcat((char*)outMsg,csTimeExpire);
				strcat((char*)outMsg,MY_AWC_TOKEN);
			} else {
DEBUGLOG(("FormatMsg:: time_expire is missing\n"));
			}

//time_enable
			if (GetField_Int(hLog,"enable_button",&iTmp)) {
				char csTimeEnable[PD_TMP_BUF_LEN + 1];
				sprintf((char*)csTimeEnable,"%d",iTmp);
DEBUGLOG(("FormatMsg:: time_enable = [%s]\n",csTimeEnable));
				strcat((char*)outMsg,"time_enable");
				strcat((char*)outMsg,MY_AWC_FIELD_TOKEN);
				strcat((char*)outMsg,csTimeEnable);
				strcat((char*)outMsg,MY_AWC_TOKEN);
			} else {
DEBUGLOG(("FormatMsg:: time_enable is missing\n"));
			}

//time_auto_check
			if (GetField_Int(hLog,"auto_check_txn_status",&iTmp)) {
				char csTimeAutoCheck[PD_TMP_BUF_LEN + 1];
				sprintf((char*)csTimeAutoCheck,"%d",iTmp);
DEBUGLOG(("FormatMsg:: time_auto_check = [%s]\n",csTimeAutoCheck));
				strcat((char*)outMsg,"time_auto_check");
				strcat((char*)outMsg,MY_AWC_FIELD_TOKEN);
				strcat((char*)outMsg,csTimeAutoCheck);
				strcat((char*)outMsg,MY_AWC_TOKEN);
			} else {
DEBUGLOG(("FormatMsg:: time_auto_check is missing\n"));
			}

//time_redirect
			if (GetField_Int(hLog,"redirect",&iTmp)) {
				char csTimeRedirect[PD_TMP_BUF_LEN + 1];
				sprintf((char*)csTimeRedirect,"%d",iTmp);
DEBUGLOG(("FormatMsg:: time_redirect = [%s]\n",csTimeRedirect));
				strcat((char*)outMsg,"time_redirect");
				strcat((char*)outMsg,MY_AWC_FIELD_TOKEN);
				strcat((char*)outMsg,csTimeRedirect);
				strcat((char*)outMsg,MY_AWC_TOKEN);
			} else {
DEBUGLOG(("FormatMsg:: time_redirect is missing\n"));
			}

//time_curr
			if (GetField_CString(hLog,"qrcode_curr_timestamp",&csPtr)) {
DEBUGLOG(("FormatMsg:: time_curr = [%s]\n",csPtr));
				strcat((char*)outMsg,"time_db_local");
				strcat((char*)outMsg,MY_AWC_FIELD_TOKEN);
				strcat((char*)outMsg,csPtr);
			} else {
DEBUGLOG(("FormatMsg:: time_curr is missing\n"));
			}
		}
	} else {
DEBUGLOG(("FormatMsg:: txn_seq is missing\n"));
	}

DEBUGLOG(("outmsg = [%s]\n",outMsg));

	csBuf = (char*) malloc (PD_MAX_BUFFER +1);
	base64_encode(outMsg,strlen((char*)outMsg),csBuf,PD_MAX_BUFFER);
	outMsg[0] = '\0';
	strcat((char*)outMsg,"qr_parameter");
	strcat((char*)outMsg,"=");
	strcat((char*)outMsg,csBuf);
	FREE_ME(csBuf);
	*outLen = strlen((const char*)outMsg);

DEBUGLOG(("FormatMsg() [%s][%d]\n",outMsg,*outLen));
DEBUGLOG(("FormatMsg() Exit\n"));
	FREE_ME(csPtr);

	hash_destroy(hLog);
	FREE_ME(hLog);

	return 	iRet;
}


int BreakDownMsg(hash_t *hOut,const unsigned char *inMsg,int inLen)
{
	int	iRet = PD_OK;
	char	*csPtr;
	hash_t	*hRec;

	hRec = (hash_t*) malloc (sizeof(hash_t));
	hash_init(hRec,0);

DEBUGLOG(("BreakDownMsg()\n"));
DEBUGLOG(("DATA = [%s][%d]\n",inMsg,inLen));

        if (Str2Cls(hRec,(char*)inMsg,MY_AWC_TOKEN,MY_AWC_FIELD_TOKEN) == PD_OK) {
//inMsg
			PutField_CString(hOut,"in_msg",(char*)inMsg);

//cusorderid
			if (GetField_CString(hRec,"cusorderid",&csPtr)) {
				PutField_CString(hOut,"txn_seq",csPtr);
DEBUGLOG(("BreakDownMsg:: txn_seq = [%s]\n",csPtr));
			} else {
DEBUGLOG(("BreakDownMsg:: txn_seq not found\n"));
			}

//trxamt
			if (GetField_CString(hRec,"trxamt",&csPtr)) {
				PutField_CString(hOut,"txn_amt",csPtr);
DEBUGLOG(("BreakDownMsg:: txn_amt = [%s]\n",csPtr));
			} else {
DEBUGLOG(("BreakDownMsg:: txn_amt not found\n"));
			}

//trxid
			if (GetField_CString(hRec,"trxid",&csPtr)) {
				PutField_CString(hOut,"tid",csPtr);
DEBUGLOG(("BreakDownMsg:: tid = [%s]\n",csPtr));
			} else {
DEBUGLOG(("BreakDownMsg:: tid not found\n"));
			}

//paytime
			if (GetField_CString(hRec,"paytime",&csPtr)) {
DEBUGLOG(("BreakDownMsg:: txn_date = [%s]\n",csPtr));
				csPtr[PD_DATETIME_LEN] = '\0';
				char csTxnDate[PD_DATE_LEN + 1];
				sprintf(csTxnDate,"%.*s",PD_DATE_LEN,csPtr);

				PutField_CString(hOut,"fundin_date",csPtr);
				PutField_CString(hOut,"txn_date",csTxnDate);
			}

//trxstatus
			if (GetField_CString(hRec,"trxstatus",&csPtr)) {
				PutField_CString(hOut,"status",csPtr);
DEBUGLOG(("BreakDownMsg:: status = [%s]\n",csPtr));
			} else {
DEBUGLOG(("BreakDownMsg:: status not found\n"));
			}

//sign
			if (GetField_CString(hRec,"sign",&csPtr)) {
				PutField_CString(hOut,"sign",csPtr);
DEBUGLOG(("BreakDownMsg:: sign = [%s]\n",csPtr));
			} else {
DEBUGLOG(("BreakDownMsg:: sign not found\n"));
			}

	} else {
DEBUGLOG(("BreakDownMsg() Error\n"));
		iRet = PD_ERR;
	}

	hash_destroy(hRec);
	FREE_ME(hRec);
DEBUGLOG(("BreakDownMsg Exit\n"));
	return iRet;
}


int initReplyFromRequest(const hash_t* hRequest, hash_t* hResponse)
{
	int	iRet = PD_OK;

	char	*csTmp = NULL;
DEBUGLOG(("initReplyFromRequest()\n"));
	

/* order_num */
        if (GetField_CString(hRequest,"merchant_ref",&csTmp)) {
DEBUGLOG(("initReplyFromRequest: order_num = [%s]\n",csTmp));
                PutField_CString(hResponse,"order_num",csTmp);
        }


DEBUGLOG(("initReplyFromRequest() Exit\n"));
	FREE_ME(csTmp);
	return iRet;
}


int BuildAuthData(hash_t* hIn)
{
	int	iRet = PD_OK;
	char*	csPtr,*csDATA;
	char*	csBuf;
	double	dTmp;
	int	iTmp;
	csDATA = (char*) malloc (PD_TMP_MSG_BUF_LEN + 1);
	csBuf = (char*) malloc (MAX_MSG_SIZE + 1);

DEBUGLOG(("BuildAuthData()\n"));
	memset(csDATA,0,sizeof(csDATA));

//appid
	if (GetField_CString(hIn,"psp_key_id",&csPtr)) {
DEBUGLOG(("BuildAuthData:: appid = [%s]\n",csPtr));
		strcpy(csDATA,"appid");
		strcat(csDATA,MY_AWC_FIELD_TOKEN);
		strcat(csDATA,csPtr);
		strcat(csDATA,MY_AWC_TOKEN);
	} else {
DEBUGLOG(("BuildAuthData:: appid is missing\n"));
	}

//body
	strcat(csDATA,"body");
	strcat(csDATA,MY_AWC_FIELD_TOKEN);
	if (GetField_CString(hIn,"psp_passphrase",&csPtr)) {
DEBUGLOG(("BuildAuthData:: body = [%s]\n", csPtr));
		strcat(csDATA,csPtr);
	} else {
DEBUGLOG(("BuildAuthData:: body = [%s]\n", MY_AWC_BODY));
		strcat(csDATA,MY_AWC_BODY);
	}
	strcat(csDATA,MY_AWC_TOKEN);

//cusid
	if (GetField_CString(hIn,"psp_merchant_id",&csPtr)) {
DEBUGLOG(("BuildAuthData:: cusid = [%s]\n",csPtr));
		strcat(csDATA,"cusid");
		strcat(csDATA,MY_AWC_FIELD_TOKEN);
		strcat(csDATA,csPtr);
		strcat(csDATA,MY_AWC_TOKEN);
	} else {
DEBUGLOG(("BuildAuthData:: cusid is missing\n"));
	}

//key
	strcat(csDATA,PD_REPLACE_PSP_KEY);
	strcat(csDATA,MY_AWC_TOKEN);

//notify_url
	if (GetField_CString(hIn,"return_url_only",&csPtr)) {
DEBUGLOG(("BuildAuthData:: notify_url = [%s]\n",csPtr));
		strcat(csDATA,"notify_url");
		strcat(csDATA,MY_AWC_FIELD_TOKEN);
		strcat(csDATA,csPtr);
		strcat(csDATA,MY_AWC_TOKEN);
	} else {
DEBUGLOG(("BuildAuthData:: notify_url is missing\n"));
	}

//paytype
	if (GetField_CString(hIn,"psp_key_uid",&csPtr)) {
DEBUGLOG(("BuildAuthData:: paytype = [%s]\n",csPtr));
		strcat(csDATA,"paytype");
		strcat(csDATA,MY_AWC_FIELD_TOKEN);
		strcat(csDATA,csPtr);
		strcat(csDATA,MY_AWC_TOKEN);
	} else {
DEBUGLOG(("BuildAuthData:: paytype is missing\n"));
	}

//randomstr
	if (GetField_CString(hIn,"order_num",&csPtr)) {
DEBUGLOG(("BuildAuthData:: randomstr = [%s]\n",csPtr));
		strcat(csDATA,"randomstr");
		strcat(csDATA,MY_AWC_FIELD_TOKEN);
		strcat(csDATA,csPtr);
		strcat(csDATA,MY_AWC_TOKEN);
	} else {
DEBUGLOG(("BuildAuthData:: randomstr is missing\n"));
	}

//reqsn
	if (GetField_CString(hIn,"order_num",&csPtr)) {
DEBUGLOG(("BuildAuthData:: reqsn = [%s]\n",csPtr));
		strcat(csDATA,"reqsn");
		strcat(csDATA,MY_AWC_FIELD_TOKEN);
		strcat(csDATA,csPtr);
		strcat(csDATA,MY_AWC_TOKEN);
	} else {
DEBUGLOG(("BuildAuthData:: reqsn is missing\n"));
	}

//trxamt
	if (GetField_Double(hIn,"psp_txn_amt",&dTmp)) {
		char csTmpAmt[PD_TMP_BUF_LEN + 1];
		sprintf((char*)csTmpAmt,"%ld",double2long(dTmp));
DEBUGLOG(("BuildAuthData:: trxamt = [%s]\n",csTmpAmt));
		strcat(csDATA,"trxamt");
		strcat(csDATA,MY_AWC_FIELD_TOKEN);
		strcat(csDATA,csTmpAmt);
		strcat(csDATA,MY_AWC_TOKEN);
	} else {
DEBUGLOG(("BuildAuthData:: trxamt is missing\n"));
	}

//validtime
	if (GetField_Int(hIn,"time_expire",&iTmp)) {
		char csTimeExpire[PD_TMP_BUF_LEN + 1];
		sprintf((char*)csTimeExpire,"%d",iTmp/60);
DEBUGLOG(("BuildAuthData:: validtime = [%s]\n",csTimeExpire));
		strcat(csDATA,"validtime");
		strcat(csDATA,MY_AWC_FIELD_TOKEN);
		strcat(csDATA,csTimeExpire);
		strcat(csDATA,MY_AWC_TOKEN);
	} else {
DEBUGLOG(("BuildAuthData:: validtime is missing\n"));
	}

//version
	if (GetField_CString(hIn,"version",&csPtr)) {
DEBUGLOG(("BuildAuthData:: version = [%s]\n",csPtr));
		strcat(csDATA,"version");
		strcat(csDATA,MY_AWC_FIELD_TOKEN);
		strcat(csDATA,csPtr);
		//strcat(csDATA,MY_AWC_TOKEN);
	} else {
DEBUGLOG(("BuildAuthData:: version is missing\n"));
	}

	PutField_CString(hIn,"auth_data",csDATA);
DEBUGLOG(("BuildAuthData:: auth_data = [%s]\n",csDATA));
	FREE_ME(csDATA);

DEBUGLOG(("BuildAuthData() Exit iRet = [%d]\n",iRet));
	return iRet;
}


int string_sort_func(const void *a1, const void *b1)
{
	const char *a = (const char *)a1;
	const char *b = (const char *)b1;
	return(strcmp(a, b));
}


int BuildRspAuthData(hash_t* hIn)
{
        int     iRet = PD_OK;
        char*   csPtr,*csDATA;
        csDATA = (char*) malloc (PD_TMP_MSG_BUF_LEN  +1);

DEBUGLOG(("BuildRspAuthData()\n"));
        memset(csDATA,0,sizeof(csDATA));

//return_code
	if (GetField_CString(hIn,"return_code",&csPtr)) {
DEBUGLOG(("BuildRspAuthData:: return_code = [%s]\n",csPtr));
		//appid
		if (GetField_CString(hIn,"appid",&csPtr)) {
DEBUGLOG(("BuildRspAuthData:: appid = [%s]\n",csPtr));
			strcat(csDATA,"appid");
			strcat(csDATA,MY_AWC_FIELD_TOKEN);
			strcat(csDATA,csPtr);
			strcat(csDATA,MY_AWC_TOKEN);
		} else {
DEBUGLOG(("BuildRspAuthData:: appid is missing\n"));
		}

		//chnltrxid
		if (GetField_CString(hIn,"chnltrxid",&csPtr)) {
DEBUGLOG(("BuildRspAuthData:: chnltrxid = [%s]\n",csPtr));
			strcat(csDATA,"chnltrxid");
			strcat(csDATA,MY_AWC_FIELD_TOKEN);
			strcat(csDATA,csPtr);
			strcat(csDATA,MY_AWC_TOKEN);
		} else {
DEBUGLOG(("BuildRspAuthData:: chnltrxid is missing\n"));
		}

		//cusid
		if (GetField_CString(hIn,"cusid",&csPtr)) {
DEBUGLOG(("BuildRspAuthData:: cusid = [%s]\n",csPtr));
			strcat(csDATA,"cusid");
			strcat(csDATA,MY_AWC_FIELD_TOKEN);
			strcat(csDATA,csPtr);
			strcat(csDATA,MY_AWC_TOKEN);
		} else {
DEBUGLOG(("BuildRspAuthData:: cusid is missing\n"));
		}

		//errmsg
		if (GetField_CString(hIn,"errmsg",&csPtr)) {
DEBUGLOG(("BuildRspAuthData:: errmsg = [%s]\n",csPtr));
			strcat(csDATA,"errmsg");
			strcat(csDATA,MY_AWC_FIELD_TOKEN);
			strcat(csDATA,csPtr);
			strcat(csDATA,MY_AWC_TOKEN);
		} else {
DEBUGLOG(("BuildRspAuthData:: errmsg is missing\n"));
		}

		//fintime
		if (GetField_CString(hIn,"fintime",&csPtr)) {
DEBUGLOG(("BuildRspAuthData:: fintime = [%s]\n",csPtr));
			strcat(csDATA,"fintime");
			strcat(csDATA,MY_AWC_FIELD_TOKEN);
			strcat(csDATA,csPtr);
			strcat(csDATA,MY_AWC_TOKEN);
		} else {
DEBUGLOG(("BuildRspAuthData:: fintime is missing\n"));
		}

		//key
		strcat(csDATA,PD_REPLACE_PSP_KEY);
		strcat(csDATA,MY_AWC_TOKEN);

		//code_url
		if (GetField_CString(hIn,"code_url",&csPtr)) {
DEBUGLOG(("BuildRspAuthData:: code_url = [%s]\n",csPtr));
			strcat(csDATA,"payinfo");
			strcat(csDATA,MY_AWC_FIELD_TOKEN);
			strcat(csDATA,csPtr);
			strcat(csDATA,MY_AWC_TOKEN);
		} else {
DEBUGLOG(("BuildRspAuthData:: code_url is missing\n"));
		}

		//randomstr
		if (GetField_CString(hIn,"randomstr",&csPtr)) {
DEBUGLOG(("BuildRspAuthData:: randomstr = [%s]\n",csPtr));
			strcat(csDATA,"randomstr");
			strcat(csDATA,MY_AWC_FIELD_TOKEN);
			strcat(csDATA,csPtr);
			strcat(csDATA,MY_AWC_TOKEN);
		} else {
DEBUGLOG(("BuildRspAuthData:: randomstr is missing\n"));
		}

		//reqsn
		if (GetField_CString(hIn,"reqsn",&csPtr)) {
DEBUGLOG(("BuildRspAuthData:: reqsn = [%s]\n",csPtr));
			strcat(csDATA,"reqsn");
			strcat(csDATA,MY_AWC_FIELD_TOKEN);
			strcat(csDATA,csPtr);
			strcat(csDATA,MY_AWC_TOKEN);
		} else {
DEBUGLOG(("BuildRspAuthData:: reqsn is missing\n"));
		}

		//return_code
		if (GetField_CString(hIn,"return_code",&csPtr)) {
DEBUGLOG(("BuildRspAuthData:: return_code = [%s]\n",csPtr));
			strcat(csDATA,"retcode");
			strcat(csDATA,MY_AWC_FIELD_TOKEN);
			strcat(csDATA,csPtr);
			strcat(csDATA,MY_AWC_TOKEN);
		} else {
DEBUGLOG(("BuildRspAuthData:: return_code is missing\n"));
		}

		//trxid
		if (GetField_CString(hIn,"trxid",&csPtr)) {
DEBUGLOG(("BuildRspAuthData:: trxid = [%s]\n",csPtr));
			strcat(csDATA,"trxid");
			strcat(csDATA,MY_AWC_FIELD_TOKEN);
			strcat(csDATA,csPtr);
			strcat(csDATA,MY_AWC_TOKEN);
		} else {
DEBUGLOG(("BuildRspAuthData:: trxid is missing\n"));
		}

		//trxstatus
		if (GetField_CString(hIn,"trxstatus",&csPtr)) {
DEBUGLOG(("BuildRspAuthData:: trxstatus = [%s]\n",csPtr));
			strcat(csDATA,"trxstatus");
			strcat(csDATA,MY_AWC_FIELD_TOKEN);
			strcat(csDATA,csPtr);
			//strcat(csDATA,MY_AWC_TOKEN);
		} else {
DEBUGLOG(("BuildRspAuthData:: trxstatus is missing\n"));
		}
	} else {
		//in_msg
		if (GetField_CString(hIn,"in_msg",&csPtr)) {
DEBUGLOG(("BuildRspAuthData:: in_msg = [%s]\n",csPtr));
		} else {
DEBUGLOG(("BuildRspAuthData:: in_msg is missing\n"));
		}

		struct key_value_pair k_v_pair[50];
		char *csBuf, *csBuf_2;
		char *p, *q;
		char *csKey, *csValue;
		int iPairCnt = 0;
		int iTmp;

		int iLen = strlen(csPtr);
		csBuf = (char*) malloc (iLen + 1);
		memcpy(csBuf, csPtr, iLen);
		csBuf[iLen] = '\0';

		p = strtok(csBuf, MY_AWC_TOKEN);
		if (p != NULL) {
			if (strlen(p) > 1) {
				if (p[strlen(p) - 1] == 0x0d)
					p[strlen(p) - 1] = '\0';
				else
					p[strlen(p)] = '\0';

				// further break down
				csBuf_2 = (char*) malloc (PD_TMP_MSG_BUF_LEN + 1);
				strcpy(csBuf_2, p);

				q = strstr(csBuf_2, MY_AWC_FIELD_TOKEN);
				if (q) {
					if (strlen(q) > strlen(MY_AWC_FIELD_TOKEN)) {
						csKey = (char*) malloc (PD_TMP_MSG_BUF_LEN + 1);
						csValue = strdup(q + 1);
						memcpy(csKey, p, strlen(p) - strlen(q));
						csKey[strlen(p) - strlen(q)] = '\0';
						if (strcmp(csKey, "sign")) {
							strcpy(k_v_pair[iPairCnt].key, csKey);
							strcpy(k_v_pair[iPairCnt].value, csValue);
							iPairCnt++;
						}
						free(csKey);
						csKey = NULL;
						free(csValue);
						csValue = NULL;
					}
				}
				free(csBuf_2);
				csBuf_2 = NULL;
			}
		}

		while ((p = strtok(NULL, MY_AWC_TOKEN)) != NULL) {
			if (strlen(p) > 1) {
				if (p[strlen(p) - 1] == 0x0d)
					p[strlen(p) - 1] = '\0';
				else
					p[strlen(p)] = '\0';

				// further break down
				csBuf_2 = (char*) malloc (PD_TMP_MSG_BUF_LEN + 1);
				strcpy(csBuf_2, p);

				q = strstr(csBuf_2, MY_AWC_FIELD_TOKEN);
				if (q) {
					if (strlen(q) > strlen(MY_AWC_FIELD_TOKEN)) {
						csKey = (char*) malloc (PD_TMP_MSG_BUF_LEN + 1);
						csValue = strdup(q + 1);
						memcpy(csKey, p, strlen(p) - strlen(q));
						csKey[strlen(p) - strlen(q)] = '\0';
						if (strcmp(csKey, "sign")) {
							strcpy(k_v_pair[iPairCnt].key, csKey);
							strcpy(k_v_pair[iPairCnt].value, csValue);
							iPairCnt++;
						}
						free(csKey);
						csKey = NULL;
						free(csValue);
						csValue = NULL;
					}
				}
				free(csBuf_2);
				csBuf_2 = NULL;
			}
		}

		free(csBuf);
		csBuf = NULL;

		if (GetField_CString(hIn,"psp_ptk_key_value",&csPtr)) {
DEBUGLOG(("BuildRspAuthData:: psp_ptk_key_value = [%s]\n",csPtr));
			strcpy(k_v_pair[iPairCnt].key, "key");
			strcpy(k_v_pair[iPairCnt].value, csPtr);
			iPairCnt++;
		} else {
DEBUGLOG(("BuildRspAuthData:: psp_ptk_key_value is missing\n"));
		}

		qsort(k_v_pair, iPairCnt, sizeof(struct key_value_pair), string_sort_func);
		for (iTmp = 0; iTmp < iPairCnt; iTmp++) {
			strcat(csDATA,k_v_pair[iTmp].key);
			strcat(csDATA,MY_AWC_FIELD_TOKEN);
			strcat(csDATA,k_v_pair[iTmp].value);
			if (iTmp != iPairCnt - 1)
				strcat(csDATA,MY_AWC_TOKEN);
		}
	}

	PutField_CString(hIn,"auth_data",csDATA);
DEBUGLOG(("BuildRspAuthData:: auth_data = [%s]\n",csDATA));
	FREE_ME(csDATA);
DEBUGLOG(("BuildRspAuthData() Exit iRet = [%d]\n",iRet));
	return iRet;
}


int BuildInqAuthData(hash_t* hIn)
{
        int     iRet = PD_OK;
        char*   csPtr;
        char*   csBuf;
        csBuf = (char*) malloc (MAX_MSG_SIZE + 1 );

DEBUGLOG(("BuildInqAuthData()\n"));
        memset(csBuf,0,MAX_MSG_SIZE);
        csBuf[0] = '\0';

/* 1 input_charset*/
        strcat((char*)csBuf,"input_charset");
        strcat((char*)csBuf,MY_AWC_FIELD_TOKEN);
        strcat((char*)csBuf,"UTF-8");
        strcat((char*)csBuf,MY_AWC_TOKEN);

/* 2 out_trade_no*/
        if (GetField_CString(hIn,"org_txn_seq",&csPtr)) {
                strcat((char*)csBuf,"out_trade_no");
                strcat((char*)csBuf,MY_AWC_FIELD_TOKEN);
                strcat((char*)csBuf,csPtr);
                strcat((char*)csBuf,MY_AWC_TOKEN);
DEBUGLOG(("BuildInqAuthData:: out_trade_no = [%s]\n",csPtr));
        }
        else {
DEBUGLOG(("BuildInqAuthData:: org_txn_seq is missing!!!\n"));
        }

/* 3 partner */
        if (GetField_CString(hIn,"psp_merchant_id",&csPtr)) {
                strcat((char*)csBuf,"partner");
                strcat((char*)csBuf,MY_AWC_FIELD_TOKEN);
                strcat((char*)csBuf,csPtr);
                strcat((char*)csBuf,MY_AWC_TOKEN);
DEBUGLOG(("BuildInqAuthData:: partner = [%s]\n",csPtr));
        }
        else {
                iRet = PD_ERR;
DEBUGLOG(("BuildInqAuthData:: ***psp_merchant_id is missing\n"));
        }

/* 4 transaction_id*/
        if (GetField_CString(hIn,"tid",&csPtr)) {
                strcat((char*)csBuf,"transaction_id");
                strcat((char*)csBuf,MY_AWC_FIELD_TOKEN);
                strcat((char*)csBuf,csPtr);
                strcat((char*)csBuf,MY_AWC_TOKEN);
DEBUGLOG(("BuildInqAuthData:: transaction_id = [%s]\n",csPtr));
        }
        else {
DEBUGLOG(("BuildInqAuthData:: tid is missing!!!\n"));
        }

        PutField_CString(hIn,"auth_data",csBuf);
DEBUGLOG(("BuildInqAuthData:: auth_data = [%s]\n",csBuf));
        FREE_ME(csBuf);

DEBUGLOG(("BuildInqAuthData() Exit iRet = [%d]\n",iRet));
        return  iRet;
}


int FormatInqMsg(const hash_t* hIn,unsigned char *outMsg,int *outLen)
{
	int	iRet = PD_OK;

	char*	csPtr = NULL;
	char*	csURL = NULL;
	char* 	csBuf;

	//double	dTmp;
DEBUGLOG(("FormatInqMsg()\n"));

	csBuf = (char*) malloc (MAX_MSG_SIZE + 1 );

	outMsg[0]= '\0';
//psp_url
	if (GetField_CString(hIn,"psp_url",&csURL)) {
		if (GetField_CString(hIn,"request_function",&csPtr)) {
			strcpy((char*)csBuf,"url");
DEBUGLOG(("FormatInqMsg:: psp_url = [%s]\n",csURL));
DEBUGLOG(("FormatInqMsg:: request function  = [%s]\n",csPtr));
			strcat((char*)csBuf,MY_AWC_FIELD_TOKEN);
			strcat((char*)csBuf,csURL);
			strcat((char*)csBuf,"/");
			strcat((char*)csBuf,csPtr);
			strcat((char*)csBuf,"?");
DEBUGLOG(("FormatInqMsg:: psp_url = [%s]\n",csBuf));
		}	
	
		sprintf((char*)outMsg,"%0*d",PD_WEB_HEADER_LEN_LEN,(int)strlen(csBuf));
DEBUGLOG(("FormatInqMsg:: outMsg = [%s]\n",outMsg));
		strcat((char*)outMsg,csBuf);
DEBUGLOG(("FormatInqMsg:: outMsg = [%s]\n",outMsg));

	}
	FREE_ME(csBuf);

//input_charset
	strcat((char*)outMsg,"input_charset");
	strcat((char*)outMsg,MY_AWC_FIELD_TOKEN);
	strcat((char*)outMsg,"UTF-8");
	strcat((char*)outMsg,MY_AWC_TOKEN);
DEBUGLOG(("FormatInqMsg:: input_charset = [UTF-8]\n"));


//partner
	if (GetField_CString(hIn,"psp_merchant_id",&csPtr)) {
DEBUGLOG(("FormatInqMsg:: partner = [%s]\n",csPtr));
		strcat((char*)outMsg,"partner");
		strcat((char*)outMsg,MY_AWC_FIELD_TOKEN);
		strcat((char*)outMsg,csPtr);
		strcat((char*)outMsg,MY_AWC_TOKEN);
	}
	else {
DEBUGLOG(("FormatInqMsg:: partner is missing\n"));
	}

//out_trade_no
	if (GetField_CString(hIn,"org_txn_seq",&csPtr)) {
DEBUGLOG(("FormatInqMsg:: out_trade_no = [%s]\n",csPtr));
		strcat((char*)outMsg,"out_trade_no");
		strcat((char*)outMsg,MY_AWC_FIELD_TOKEN);
		strcat((char*)outMsg,csPtr);
		strcat((char*)outMsg,MY_AWC_TOKEN);
	}
	else {
DEBUGLOG(("FormatInqMsg:: out_trade_no is missing\n"));
	}

//transaction_id
	if (GetField_CString(hIn,"tid",&csPtr)) {
DEBUGLOG(("FormatInqMsg:: transaction_id = [%s]\n",csPtr));
		strcat((char*)outMsg,"transaction_id");
		strcat((char*)outMsg,MY_AWC_FIELD_TOKEN);
		strcat((char*)outMsg,csPtr);
		strcat((char*)outMsg,MY_AWC_TOKEN);
	}
	else {
DEBUGLOG(("FormatInqMsg:: transaction_id is missing\n"));
	}

//sign
	if (GetField_CString(hIn,"sign",&csPtr)) {
DEBUGLOG(("FormatInqMsg:: sign = [%s]\n",csPtr));
		strcat((char*)outMsg,"sign");
		strcat((char*)outMsg,MY_AWC_FIELD_TOKEN);
		strcat((char*)outMsg,csPtr);
//		strcat((char*)outMsg,MY_AWC_TOKEN);
	}	
	else {
DEBUGLOG(("FormatInqMsg:: sign not found!!!\n"));
	}


DEBUGLOG(("outmsg = [%s]\n",outMsg));

	*outLen = strlen((const char*)outMsg);
DEBUGLOG(("FormatInqMsg() [%s][%d]\n",outMsg,*outLen));
DEBUGLOG(("FormatInqMsg() Exit\n"));
	FREE_ME(csPtr);
	FREE_ME(csURL);
	return 	iRet;
}


int FormatInitMsg(const hash_t* hIn,unsigned char *outMsg,int *outLen)
{
	int	iRet = PD_OK;

	char*	csPtr = NULL;
	char*	csURL = NULL;
	char*	csBuf;
	double	dTmp;
	int	iTmp;

DEBUGLOG(("FormatInitMsg()\n"));

	csBuf = (char*) malloc (MAX_MSG_SIZE + 1);

	outMsg[0]= '\0';
//psp_url
	if (GetField_CString(hIn,"psp_url",&csURL)) {
		if (GetField_CString(hIn,"request_function",&csPtr)) {
			strcpy((char*)csBuf,"url");
DEBUGLOG(("FormatInitMsg:: psp_url = [%s]\n",csURL));
DEBUGLOG(("FormatInitMsg:: request function = [%s]\n",csPtr));
			strcat((char*)csBuf,MY_AWC_FIELD_TOKEN);
			strcat((char*)csBuf,csURL);
			strcat((char*)csBuf,"/");
			strcat((char*)csBuf,csPtr);
			//strcat((char*)csBuf,"?");
DEBUGLOG(("FormatInitMsg:: psp_url = [%s]\n",csBuf));
		}

		sprintf((char*)outMsg,"%0*d",PD_WEB_HEADER_LEN_LEN,(int)strlen(csBuf));
DEBUGLOG(("FormatInitMsg:: outMsg = [%s]\n",outMsg));
		strcat((char*)outMsg,csBuf);
		//strcat((char*)outMsg,MY_AWC_URL_END_TOKEN);
	}
	FREE_ME(csBuf);

//cusid
	if (GetField_CString(hIn,"psp_merchant_id",&csPtr)) {
DEBUGLOG(("FormatInitMsg:: cusid = [%s]\n",csPtr));
		strcat((char*)outMsg,"cusid");
		strcat((char*)outMsg,MY_AWC_FIELD_TOKEN);
		strcat((char*)outMsg,csPtr);
		strcat((char*)outMsg,MY_AWC_TOKEN);
	} else {
DEBUGLOG(("FormatInitMsg:: cusid is missing\n"));
	}

//appid
	if (GetField_CString(hIn,"psp_key_id",&csPtr)) {
DEBUGLOG(("FormatInitMsg:: appid = [%s]\n",csPtr));
		strcat((char*)outMsg,"appid");
		strcat((char*)outMsg,MY_AWC_FIELD_TOKEN);
		strcat((char*)outMsg,csPtr);
		strcat((char*)outMsg,MY_AWC_TOKEN);
	} else {
DEBUGLOG(("FormatInitMsg:: appid is missing\n"));
	}

//version
	if (GetField_CString(hIn,"version",&csPtr)) {
DEBUGLOG(("FormatInitMsg:: version = [%s]\n",csPtr));
		strcat((char*)outMsg,"version");
		strcat((char*)outMsg,MY_AWC_FIELD_TOKEN);
		strcat((char*)outMsg,csPtr);
		strcat((char*)outMsg,MY_AWC_TOKEN);
	} else {
DEBUGLOG(("FormatInitMsg:: version is missing\n"));
	}

//trxamt
	if (GetField_Double(hIn,"psp_txn_amt",&dTmp)) {
		char csTmpAmt[PD_TMP_BUF_LEN + 1];
		sprintf((char*)csTmpAmt,"%ld",double2long(dTmp));
DEBUGLOG(("FormatInitMsg:: trxamt = [%s]\n",csTmpAmt));
		strcat((char*)outMsg,"trxamt");
		strcat((char*)outMsg,MY_AWC_FIELD_TOKEN);
		strcat((char*)outMsg,csTmpAmt);
		strcat((char*)outMsg,MY_AWC_TOKEN);
	} else {
DEBUGLOG(("FormatInitMsg:: trxamt is missing\n"));
	}

//reqsn
	if (GetField_CString(hIn,"order_num",&csPtr)) {
DEBUGLOG(("FormatInitMsg:: reqsn = [%s]\n",csPtr));
		strcat((char*)outMsg,"reqsn");
		strcat((char*)outMsg,MY_AWC_FIELD_TOKEN);
		strcat((char*)outMsg,csPtr);
		strcat((char*)outMsg,MY_AWC_TOKEN);
	} else {
DEBUGLOG(("FormatInitMsg:: reqsn is missing\n"));
	}

//paytype
	if (GetField_CString(hIn,"psp_key_uid",&csPtr)) {
DEBUGLOG(("FormatInitMsg:: paytype = [%s]\n",csPtr));
		strcat((char*)outMsg,"paytype");
		strcat((char*)outMsg,MY_AWC_FIELD_TOKEN);
		strcat((char*)outMsg,csPtr);
		strcat((char*)outMsg,MY_AWC_TOKEN);
	} else {
DEBUGLOG(("FormatInitMsg:: paytype is missing\n"));
	}

//randomstr
	if (GetField_CString(hIn,"order_num",&csPtr)) {
DEBUGLOG(("FormatInitMsg:: randomstr = [%s]\n",csPtr));
		strcat((char*)outMsg,"randomstr");
		strcat((char*)outMsg,MY_AWC_FIELD_TOKEN);
		strcat((char*)outMsg,csPtr);
		strcat((char*)outMsg,MY_AWC_TOKEN);
	} else {
DEBUGLOG(("FormatInitMsg:: randomstr is missing\n"));
	}

//body
	strcat((char*)outMsg,"body");
	strcat((char*)outMsg,MY_AWC_FIELD_TOKEN);
	if (GetField_CString(hIn,"psp_passphrase",&csPtr)) {
DEBUGLOG(("FormatInitMsg:: body = [%s]\n",csPtr));
		strcat((char*)outMsg,csPtr);
	} else {
DEBUGLOG(("FormatInitMsg:: body = [%s]\n",MY_AWC_BODY));
		strcat((char*)outMsg,MY_AWC_BODY);
	}
	strcat((char*)outMsg,MY_AWC_TOKEN);

//validtime
	if (GetField_Int(hIn,"time_expire",&iTmp)) {
		char csTimeExpire[PD_TMP_BUF_LEN + 1];
		sprintf((char*)csTimeExpire,"%d",iTmp/60);
DEBUGLOG(("FormatInitMsg:: validtime = [%s]\n",csTimeExpire));
		strcat((char*)outMsg,"validtime");
		strcat((char*)outMsg,MY_AWC_FIELD_TOKEN);
		strcat((char*)outMsg,csTimeExpire);
		strcat((char*)outMsg,MY_AWC_TOKEN);
	} else {
DEBUGLOG(("FormatInitMsg:: validtime is missing\n"));
	}

//notify_url
	if (GetField_CString(hIn,"return_url_only",&csPtr)) {
DEBUGLOG(("FormatInitMsg:: notify_url = [%s]\n",csPtr));
		strcat((char*)outMsg,"notify_url");
		strcat((char*)outMsg,MY_AWC_FIELD_TOKEN);
		strcat((char*)outMsg,csPtr);
		strcat((char*)outMsg,MY_AWC_TOKEN);
	} else {
DEBUGLOG(("FormatInitMsg:: notify_url is missing\n"));
	}

//sign
	if (GetField_CString(hIn,"sign",&csPtr)) {
DEBUGLOG(("FormatInitMsg:: sign = [%s]\n",csPtr));
		strcat((char*)outMsg,"sign");
		strcat((char*)outMsg,MY_AWC_FIELD_TOKEN);
		strcat((char*)outMsg,csPtr);
		//strcat((char*)outMsg,MY_AWC_TOKEN);
	} else {
DEBUGLOG(("FormatInitMsg:: sign is missing\n"));
	}

DEBUGLOG(("outmsg = [%s]\n",outMsg));

	*outLen = strlen((const char*)outMsg);
DEBUGLOG(("FormatInitMsg() [%s][%d]\n",outMsg,*outLen));
DEBUGLOG(("FormatInitMsg() Exit\n"));
	FREE_ME(csPtr);
	FREE_ME(csURL);
	return 	iRet;
}


int BreakDownInitRspMsg(hash_t *hOut,const unsigned char *inMsg,int inLen)
{
	int	iRet = PD_OK;
	char	*csPtr;
	char	*csReturnCode;
	hash_t	*hRec;

	hRec = (hash_t*) malloc (sizeof(hash_t));
	hash_init(hRec,0);

DEBUGLOG(("BreakDownInitRspMsg()\n"));
DEBUGLOG(("DATA = [%s][%d]\n",inMsg,inLen));

	struct json_object *jobj;
	enum json_type type;

	jobj = json_tokener_parse((const char *)inMsg);
	if (jobj != NULL) {
		json_object_object_foreach(jobj, key, val) {
			type = json_object_get_type(val);
			switch (type) {
				case json_type_string:
					PutField_CString(hRec, key, json_object_get_string(val));
				break;
				default:
DEBUGLOG(("BreakDownInitRspMsg:: unsupported type\n"));
				break;
			}
		}

		//determine which return message it is
		if (GetField_CString(hRec,"retcode",&csReturnCode)) {
			//return_code
			PutField_CString(hOut,"return_code",csReturnCode);
DEBUGLOG(("BreakDownInitRspMsg:: return_code = [%s]\n",csReturnCode));

			//return_msg
			if (GetField_CString(hRec,"retmsg",&csPtr)) {
				PutField_CString(hOut,"return_msg",csPtr);
DEBUGLOG(("BreakDownInitRspMsg:: return_msg = [%s]\n",csPtr));
			}

			if (!strcmp(csReturnCode,"SUCCESS")) {
				//cusid
				if (GetField_CString(hRec,"cusid",&csPtr)) {
					PutField_CString(hOut,"cusid",csPtr);
DEBUGLOG(("BreakDownInitRspMsg:: cusid = [%s]\n",csPtr));
				}

				//appid
				if (GetField_CString(hRec,"appid",&csPtr)) {
					PutField_CString(hOut,"appid",csPtr);
DEBUGLOG(("BreakDownInitRspMsg:: appid = [%s]\n",csPtr));
				}

				//trxid
				if (GetField_CString(hRec,"trxid",&csPtr)) {
					PutField_CString(hOut,"trxid",csPtr);
DEBUGLOG(("BreakDownInitRspMsg:: trxid = [%s]\n",csPtr));
				}

				//chnltrxid
				if (GetField_CString(hRec,"chnltrxid",&csPtr)) {
					PutField_CString(hOut,"chnltrxid",csPtr);
DEBUGLOG(("BreakDownInitRspMsg:: chnltrxid = [%s]\n",csPtr));
				}

				//reqsn
				if (GetField_CString(hRec,"reqsn",&csPtr)) {
					PutField_CString(hOut,"reqsn",csPtr);
DEBUGLOG(("BreakDownInitRspMsg:: reqsn = [%s]\n",csPtr));
				}

				//randomstr
				if (GetField_CString(hRec,"randomstr",&csPtr)) {
					PutField_CString(hOut,"randomstr",csPtr);
DEBUGLOG(("BreakDownInitRspMsg:: randomstr = [%s]\n",csPtr));
				}

				//trxstatus
				if (GetField_CString(hRec,"trxstatus",&csPtr)) {
					PutField_CString(hOut,"trxstatus",csPtr);
DEBUGLOG(("BreakDownInitRspMsg:: trxstatus = [%s]\n",csPtr));
				}

				//fintime
				if (GetField_CString(hRec,"fintime",&csPtr)) {
					PutField_CString(hOut,"fintime",csPtr);
DEBUGLOG(("BreakDownInitRspMsg:: fintime = [%s]\n",csPtr));
				}

				//errmsg
				if (GetField_CString(hRec,"errmsg",&csPtr)) {
					PutField_CString(hOut,"errmsg",csPtr);
DEBUGLOG(("BreakDownInitRspMsg:: errmsg = [%s]\n",csPtr));
				}

				//payinfo
				if (GetField_CString(hRec,"payinfo",&csPtr)) {
					PutField_CString(hOut,"code_url",csPtr);
DEBUGLOG(("BreakDownInitRspMsg:: payinfo = [%s]\n",csPtr));
				}

				//sign
				if (GetField_CString(hRec,"sign",&csPtr)) {
					PutField_CString(hOut,"sign",csPtr);
DEBUGLOG(("BreakDownInitRspMsg:: sign = [%s]\n",csPtr));
				}
			}
		}
		else{
DEBUGLOG(("BreakDownInitRspMsg() Invalid Response\n"));
			iRet = PD_ERR;
		}
	} else {
DEBUGLOG(("BreakDownInitRspMsg() Error\n"));
		iRet = PD_ERR;
	}

	json_object_put(jobj);

	hash_destroy(hRec);
	FREE_ME(hRec);
DEBUGLOG(("BreakDownInitRspMsg Exit\n"));
	return iRet;
}

