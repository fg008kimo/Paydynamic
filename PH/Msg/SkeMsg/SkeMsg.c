...
-------------------------------                    ------------            --------------
Init Version                                       2017/12/08              David Wong
*/
...
// payKey
		if (GetField_CString(hIn, "psp_merchant_id", &csTmp)) {
			...
			strcat((char*)outMsg, MY_SKE_FIELD_TOKEN);
			strcat((char*)outMsg, csTmp);
			strcat((char*)outMsg, MY_SKE_TOKEN);
DEBUGLOG(("payKey = [%s]\n", csTmp));
		}
		else {
			iRet = PD_ERR;
...
// productType
		...
		strcat((char*)outMsg, MY_SKE_FIELD_TOKEN);
		strcat((char*)outMsg, MY_SKE_PRODUCT_TYPE);
		strcat((char*)outMsg, MY_SKE_TOKEN);
...
// payBankAccountNo
		strcat((char*)outMsg, "payBankAccountNo");
		strcat((char*)outMsg, MY_SKE_FIELD_TOKEN);
		strcat((char*)outMsg, MY_SKE_PAY_BANK_ACCT_NUM);
		strcat((char*)outMsg, MY_SKE_TOKEN);
DEBUGLOG(("payBankAccountNo = [%s]\n", MY_SKE_PAY_BANK_ACCT_NUM));
...
DEBUGLOG(("DATA = [%s][%d]\n", inMsg, inLen));
	...
	if (Str2Cls(hRec, (char *)inMsg, MY_SKE_TOKEN, MY_SKE_FIELD_TOKEN) == PD_OK) {
// inMsg
...
// notifyUrl
	if (GetField_CString(hIn, "return_url_only", &csPtr)) {
		...
		strcat((char*)csBuf, MY_SKE_FIELD_TOKEN);
		strcat((char*)csBuf, csPtr);
		strcat((char*)csBuf, MY_SKE_TOKEN);
DEBUGLOG(("notifyUrl = [%s]\n", csPtr));
	}
	else {
		iRet = PD_ERR;
...
// payBankAccountNo
	strcat((char*)csBuf, "payBankAccountNo");
	strcat((char*)csBuf, MY_SKE_FIELD_TOKEN);
	strcat((char*)csBuf, MY_SKE_PAY_BANK_ACCT_NUM);
	strcat((char*)csBuf, MY_SKE_TOKEN);
DEBUGLOG(("payBankAccountNo = [%s]\n", MY_SKE_PAY_BANK_ACCT_NUM));
...
// productType
	...
	strcat((char*)csBuf, MY_SKE_FIELD_TOKEN);
	strcat((char*)csBuf, MY_SKE_PRODUCT_TYPE);
	strcat((char*)csBuf, MY_SKE_TOKEN);
DEBUGLOG(("productType = [%s]\n", MY_SKE_PRODUCT_TYPE));
...
DEBUGLOG(("BuildRspAuthData()\n"));
	...
	if (GetField_CString(hIn, "in_msg", &inMsg)) {
		if (Str2KVPair(k_v_pair, &iPairCnt, inMsg, MY_SKE_TOKEN, MY_SKE_FIELD_TOKEN) == PD_OK) {
//DEBUGLOG(("iPairCnt = [%d]\n", iPairCnt));
...
DEBUGLOG(("key = [%s], value = [%s]\n", k_v_pair[iTmp].key, k_v_pair[iTmp].value));
				...
				if (strcmp(k_v_pair[iTmp].key, "sign")) {
					...
					strcat(csBuf, MY_SKE_FIELD_TOKEN);
					strcat(csBuf, k_v_pair[iTmp].value);
					strcat(csBuf, MY_SKE_TOKEN);
				}
			}
		}
	}
