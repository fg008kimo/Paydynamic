...
Amend p8_Url Value				   2013/08/22              Virginia Yun
New Format					   2015/04/29		   LokMan Chow
*/
...
/* p7 Pdesc */
/* p8 url */
		// Use return_url_only with add fe path
...
DEBUGLOG(("BuildReqData:: 4. psp_txn_ccy = [%s]\n",csPtr));
                        strcat((char*)csDATA,csPtr );
//                        strcat((char*)csDATA,MY_YPY_DATA_TOKEN);
        }
...
//5 psp_txn_amt
        if (GetField_Double(hIn,"psp_txn_amt",&dTmp)) {
DEBUGLOG(("BuildReqData:: 5. AMOUNT = [%f]\n",dTmp));
		char    csTmpAmt[PD_TMP_BUF_LEN +1];
		sprintf(csTmpAmt,"%.2f",dTmp);
...
DEBUGLOG(("BuildRspData:: ro_BankOrderId:bank_bill_no = [%s]\n",csPtr));
                strcat((char*)csDATA,csPtr);
	}

/* rp_PayDate */
	if (GetField_CString(hIn,"fundin_date",&csPtr)) {
DEBUGLOG(("BuildRspData:: rp_PayDate:fundin_date = [%s]\n",csPtr));
                strcat((char*)csDATA,csPtr);
	}
