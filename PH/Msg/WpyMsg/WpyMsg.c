...
// merchant_code
		if (GetField_CString(hIn, "psp_merchant_id", &csTmp)) {
			...
			strcat((char*)outMsg, MY_WPY_FIELD_TOKEN);
			strcat((char*)outMsg, csTmp);
			strcat((char*)outMsg, MY_WPY_TOKEN);
...
DEBUGLOG(("DATA = [%s][%d]\n", inMsg, inLen));
	...
	if (Str2Cls(hRec, (char *)inMsg, MY_WPY_TOKEN, MY_WPY_FIELD_TOKEN) == PD_OK) {
// merchant_code
...
// bank_code
	if (GetField_CString(hIn, "bank_code", &csPtr)) {
		...
		strcat((char*)csBuf, MY_WPY_FIELD_TOKEN);
		DBObjPtr = CreateObj(DBPtr, "DBMobBankMap", "IsMobileOption");
...
DEBUGLOG(("BuildAuthData:: bank_code = [%s]\n", PD_BANK_CODE_EC));
		...
		strcat((char*)csBuf, MY_WPY_TOKEN);
	}
	else {
		iRet = PD_ERR;
