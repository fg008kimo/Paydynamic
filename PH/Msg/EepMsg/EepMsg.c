...
DEBUGLOG(("DATA = [%s][%d]\n",inMsg,inLen));
	...
	if (Str2Cls(hRec,(char *)inMsg,MY_EEP_TOKEN, MY_EEP_FIELD_TOKEN) == PD_OK) {
...
DEBUGLOG(("EepMsg BuildAuthData[str_ok] = [%s]\n",csPtr));
                strcat(csBuf,csPtr);
        }
        else {
DEBUGLOG(("EepMsg:BuildAuthData status[str_ok] Not FOUND!!!\n"));
        }       
                        
/* e_oderno */
        if (GetField_CString(hIn,"txn_seq",&csPtr)) {
DEBUGLOG(("EepMsg:BuildAuthData() txn_seq= [%s]\n",csPtr));
                strcat(csBuf,csPtr);
        }
        else {  
DEBUGLOG(("EepMsg:BuildAuthData() txn_seq is missing***\n"));
        }
                
/* psp_merchant_id */   
        if (GetField_CString(hIn,"psp_merchant_id",&csPtr)) {
DEBUGLOG(("EepMsg:BuildAuthData() psp_merchant_id= [%s]\n",csPtr));
                strcat(csBuf,csPtr);
        }
        else {  
DEBUGLOG(("EepMsg:BuildAuthData() psp_merchant_id is missing***\n"));
        }       
                        
/* txn amount */
        if (GetField_Double(hIn,"psp_txn_amt",&dTmp)) {
DEBUGLOG(("EepMsg:BuildAuthData() txn_amt= [%f]\n",dTmp));
                sprintf(csTmp,"%.2f",dTmp);
                strcat(csBuf,csTmp);
DEBUGLOG(("EepMsg:BuildAuthData() txn_amt= [%s]\n",csTmp));
        }
...
DEBUGLOG(("EepMsg:BuildAuthData() txn_amt is missing***\n"));
        }


/* tid */
        if (GetField_CString(hIn,"tid",&csPtr)) {
DEBUGLOG(("EepMsg:BuildAuthData tid[str_no] = [%s]\n",csPtr));
                strcat(csBuf,csPtr);
        }
        else {
DEBUGLOG(("EepMsg tid[str_no] Not Found!!!\n"));
        }
