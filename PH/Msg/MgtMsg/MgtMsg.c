...
Add response batchid				   2015/11/19		   LokMan Chow
Add Format 'txn_status' for API 'ITS'		   2016/12/12		   LokMan Chow
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
DEBUGLOG(("FormatMsg <%s id='%d'> = <%d>\n",csTag,i,iTmp));
				...
				if(GetField_Char(hIn,csTag,&cPtr)){
					strcpy(csTag,"card");
DEBUGLOG(("FormatMsg <%s id='%d'> = <%c>\n",csTag,i,cPtr));
					sprintf(csBuf,"<%s id='%d'>%c</%s>",csTag,i,cPtr,csTag);
					strcat((char*)outMsg,csBuf);
				}
			}
		}
...
/*status*/
        ...
        if (GetField_Char(hIn,csTag,&cPtr)) {
        	strcpy(csTag,"status");
DEBUGLOG(("FormatMsg <%s> = <%c>\n",csTag,cPtr));
                sprintf(csBuf,"<%s>%c</%s>",csTag,cPtr,csTag);
                strcat((char*)outMsg,csBuf);
	}
/* redirect_url */      
...
DEBUGLOG(("put tagname [%s] = [%s]\n",csTmpTag,csTmp));
			...
			PutField_CString(hOut,csTmpTag,PD_STRING_TYPE);
DEBUGLOG(("put vsec [%s] = [%s]\n",csTmpTag,PD_STRING_TYPE));

			sprintf(csTmpTag,"%d_type",iTotalDyn);
			PutField_CString(hOut,csTmpTag,PD_TPL_GLOBAL_TYPE);
DEBUGLOG(("put type [%s] = [%s]\n",csTmpTag,PD_TPL_GLOBAL_TYPE));
