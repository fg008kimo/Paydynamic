...
-------------------------------                    ------------            --------------
Init Version					   2013/09/03              Cody Chan
MD5 Sign message changed			   2014/02/27		   LokMan Chow
...
/* MerNo */
		if (GetField_CString(hIn,"psp_merchant_id",&csTmp)) {
			...
			strcat((char*)outMsg,MY_ECP_FIELD_TOKEN);
			strcat((char*)outMsg,csTmp);
			strcat((char*)outMsg,MY_ECP_TOKEN);
...
DEBUGLOG(("DATA = [%s][%d]\n",inMsg,inLen));
	...
	if (Str2Cls(hRec,(char *)inMsg,MY_ECP_TOKEN, MY_ECP_FIELD_TOKEN) == PD_OK) {
...
/* MD5info */
                if (GetField_CString(hRec,"SignMD5info",&csPtr)) {
                        PutField_CString(hOut,"sign",csPtr);
			PutField_Int(hOut,"new_sign",PD_TRUE);
DEBUGLOG(("BreakDownMsg:: sign = [%s]\n",csPtr));
                }
                else{
			if (GetField_CString(hRec,"MD5info",&csPtr)) {
				PutField_CString(hOut,"sign",csPtr);
				PutField_Int(hOut,"new_sign",PD_FALSE);
DEBUGLOG(("BreakDownMsg:: sign (old parameter) = [%s]\n",csPtr));
			}
			else{
...
int BuildRspAuthData(hash_t* hIn)
{
	...
	char	csTag[PD_TAG_LEN +1];
        csBuf = (char*) malloc (MAX_MSG_SIZE + 1 );
	int	iNewSign = PD_TRUE;

DEBUGLOG(("BuildRspAuthData()\n"));
	memset(csBuf,0,MAX_MSG_SIZE);
	csBuf[0] = '\0';

	if(GetField_Int(hIn,"new_sign",&iNewSign)){
DEBUGLOG(("BuildRspAuthData:: build new sign msg = [%d]\n",iNewSign));
	}
