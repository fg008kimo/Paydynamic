...
-------------------------------                    ------------            --------------
Init Version					   20110520                LokMan Chow
*/
...
/* merchant_id */
		if (GetField_CString(hIn,"psp_merchant_id",&csTmp)) {
			strcat((char*)outMsg,"HO_MERID");
			strcat((char*)outMsg,MY_HHPAY_FIELD_TOKEN);
			strcat((char*)outMsg,csTmp);
			strcat((char*)outMsg,MY_HHPAY_TOKEN);
DEBUGLOG(("FormatMsg:: HO_MERID = [%s]\n",csTmp));
		}
		else {
			iRet = PD_ERR;
...
/* enctype */
			...
			strcat((char*)outMsg,"HO_ENCTYPE");
                        strcat((char*)outMsg,MY_HHPAY_FIELD_TOKEN);
                        strcat((char*)outMsg,csTmp);
                        strcat((char*)outMsg,MY_HHPAY_TOKEN);
DEBUGLOG(("FormatMsg:: HO_ENCTYPE = [%s]\n",csTmp));
		}
		else{
			strcat((char*)outMsg,"HO_ENCTYPE");
			strcat((char*)outMsg,MY_HHPAY_FIELD_TOKEN);
			...
			strcat((char*)outMsg,MY_HHPAY_TOKEN);
DEBUGLOG(("FormatMsg:: default HO_ENCTYPE = [01]\n"));
		}
...
/* bank_id */
		if (GetField_CString(hIn,"bank_code",&csTmp)) {
			strcat((char*)outMsg,"HO_BANKID");
			strcat((char*)outMsg,MY_HHPAY_FIELD_TOKEN);
			strcat((char*)outMsg,csTmp);
DEBUGLOG(("FormatMsg:: HO_BANKID = [%s]\n",csTmp));
		}
		else {
DEBUGLOG(("FormatMsg:: HO_BANKID is missing!!!\n"));
		}
...
DEBUGLOG(("DATA = [%s][%d]\n",inMsg,inLen));
	...
	if (Str2Cls(hRec,(char *)inMsg,MY_HHPAY_TOKEN, MY_HHPAY_FIELD_TOKEN) == PD_OK) {

/*merchant id */
		if (GetField_CString(hRec,"HO_MERID",&csPtr)) {
...
/*encrypt_type */
		if (GetField_CString(hRec,"HO_ENCTYPE",&csPtr)) {
...
/* data */
		if (GetField_CString(hRec,"HO_DATA",&csPtr)) {
                	iRet = DEBlockHHpayData(hOut, (const unsigned char *) csPtr,strlen(csPtr));
        	}
		else{
...
/* SIGN */
		if (GetField_CString(hRec,"HO_SIGN",&csPtr)) {
...
DEBUGLOG(("BuildData:: HO_MERORDERID = [%s]\n",csPtr));
                strcat((char*)csDATA,csPtr);
                strcat((char*)csDATA,MY_HHPAY_DATA_TOKEN);
        }

// transmission_datetime
        if (GetField_CString(hIn,"local_tm_date",&csPtr)) {
DEBUGLOG(("BuildData:: HO_MERDATE = [%s]\n",csPtr));
		strcat((char*)csDATA,csPtr);
                strcat((char*)csDATA,MY_HHPAY_DATA_TOKEN);
        }
...
DEBUGLOG(("BuildData:: HO_AMOUNT = [%s]\n",csTmp));
...
DEBUGLOG(("BuildData:: HO_CURTYPE = [%s]\n",csPtr));
...
ERRLOG("HhpMsg:BuildData unsupport CCY ISO to HHPAY\n");
		}
        }
	else {
...
// tradeway
        ...
        strcat((char*)csDATA,MY_HHPAY_DATA_TOKEN);
DEBUGLOG(("BuildData:: HO_TRADWAY = [%s]\n","1"));

/* recvenctype */
        if (GetField_CString(hIn,"recvenctype",&csPtr)) {
DEBUGLOG(("BuildData:: HO_RECVENCTYPE = [%s]\n",csPtr));
                strcat((char*)csDATA,csPtr);
                strcat((char*)csDATA,MY_HHPAY_DATA_TOKEN);
        }
...
DEBUGLOG(("BuildData:: HO_RETURNURL = [%s]\n",csPtr));
                strcat((char*)csDATA,csPtr);
                strcat((char*)csDATA,MY_HHPAY_DATA_TOKEN);
        }
...
DEBUGLOG(("BuildData:: HO_FAILEDURL = [%s]\n",csPtr));
                strcat((char*)csDATA,csPtr);
                strcat((char*)csDATA,MY_HHPAY_DATA_TOKEN);
        }
	else {
...
/* success_url */
        if (GetField_CString(hIn,"fe_url",&csPtr)) {
DEBUGLOG(("BuildData:: HO_SUCCESSURL = [%s]\n",csPtr));
                strcat((char*)csDATA,csPtr);
                strcat((char*)csDATA,MY_HHPAY_DATA_TOKEN);
        }
...
DEBUGLOG(("FormatMsg:: HO_SHOWNAMOUNT = [%s]\n",csBuf));
...
DEBUGLOG(("BuildData:: remark = [%s]\n",csPtr));
                csBuf[0]='\0';
                base64_encode((unsigned char *)csPtr,strlen(csPtr),csBuf,PD_MAX_BUFFER);
DEBUGLOG(("BuildData:: HO_REMARK = [%s]\n",csBuf));
                strcat((char*)csDATA,csPtr);
        }
...
/* Merchant Ref */
                p = strtok(csDATA,MY_HHPAY_DATA_TOKEN);
                if (p) {
...
/* txn_id */
                p = strtok(NULL,MY_HHPAY_DATA_TOKEN);
                if (p) {
...
/* remark */
                p = strtok(NULL,MY_HHPAY_DATA_TOKEN);
                if (p) {
			iLen = base64_decode(p,(unsigned char *)csBuf,PD_MAX_BUFFER);
			if(iLen>0){
				csBuf[iLen]='\0';
