...
DEBUGLOG(("DATA = [%s][%d]\n", inMsg, inLen));
	...
	if (Str2Cls(hRec, (const char *)csDATA, MY_HDV_TOKEN, MY_HDV_FIELD_TOKEN) == PD_OK) {
		if (!iIsTraceReturn) {
		...
		json_object_object_foreach(jobj, key, val) {
			...
			switch (type) {
				case json_type_string:
					if (strcmp(key, "reserved")) {
						strcat((char*)csOrigMsg, key);
						strcat((char*)csOrigMsg, MY_HDV_FIELD_TOKEN);
						strcat((char*)csOrigMsg, json_object_get_string(val));
						strcat((char*)csOrigMsg, MY_HDV_TOKEN);
					}
				break;
				case json_type_int:
					if (strcmp(key, "reserved")) {
						strcat((char*)csOrigMsg, key);
						strcat((char*)csOrigMsg, MY_HDV_FIELD_TOKEN);
						sprintf((char*)csOrigMsg, "%s%d", (char*)csOrigMsg, json_object_get_int(val));
						strcat((char*)csOrigMsg, MY_HDV_TOKEN);
					}
				break;
				case json_type_double:
					if (strcmp(key, "reserved")) {
						strcat((char*)csOrigMsg, key);
						strcat((char*)csOrigMsg, MY_HDV_FIELD_TOKEN);
						sprintf((char*)csOrigMsg, "%s%f", (char*)csOrigMsg, json_object_get_double(val));
						strcat((char*)csOrigMsg, MY_HDV_TOKEN);
					}
				break;
				default:
DEBUGLOG(("unsupported type\n"));
	...
	PutField_CString(hOut, "orig_msg", (const char *)csOrigMsg);

	if (Str2Cls(hRec, (char *)csOrigMsg, MY_HDV_TOKEN, MY_HDV_FIELD_TOKEN) == PD_OK) {

		sprintf(csTag, "insCode");
		if (GetField_CString(hRec, csTag, &csTmp)) {
DEBUGLOG(("%s = [%s]\n", csTag, csTmp));
			strcat((char*)csMsg, csTmp);
			strcat((char*)csMsg, MY_HDV_SIGN_TOKEN);
		} else {
...
DEBUGLOG(("%s not found\n", csTag));
		...
		if (GetField_CString(hRec, csTag, &csTmp)) {
DEBUGLOG(("%s = [%s]\n", csTag, csTmp));
			PutField_CString(hOut, "fail_reason", csTmp);
			strcat((char*)csMsg, csTmp);
			strcat((char*)csMsg, MY_HDV_SIGN_TOKEN);
		} else {
DEBUGLOG(("%s not found\n", csTag));
