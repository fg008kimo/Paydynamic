...
Add DinPay-RFPay EC				   2017/12/11		   David Wong
...
Add PAYESCAPE-Epaylinks VNET			   2018/02/12		   LokMan Chow
...
Add OUP-YLPay EC				   2018/04/12		   LokMan Chow
...
Add ZPAY EC                                        2018/04/30              LokMan Chow
...
Add OUP-OpenePay EC				   2018/10/11		   David Wong
Add EASYPAY-DF QR				   2018/11/26		   David Wong
Add XJ-DBao QR					   2018/12/13		   David Wong
...
Add UNIPAYGO QR					   2019/06/27		   David Wong
Add TRUSTPAY QR					   2019/07/11		   David Wong
Add provider XPAY EC				   2019/07/17		   David Wong
Add provider XPAY QR				   2019/07/19		   David Wong
Add EASYPAY-HANDPAY VNET			   2019/09/05		   David Wong
*/
...
//call N2P-AIPWechat
					MsgObjPtr = CreateObj(MsgPtr,"NtwMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format NtwMsg error\n"));
ERRLOG("FormatMsg format NtwMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
...
//call XJ-CHWechat
					MsgObjPtr = CreateObj(MsgPtr,"XjwMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format XjwMsg error\n"));
ERRLOG("FormatMsg format XjwMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
...
//call XJ Mobile
					MsgObjPtr = CreateObj(MsgPtr,"XjmMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format XjmMsg error\n"));
ERRLOG("FormatMsg format XjmMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
				else if(!strcmp(csPspChannel,PD_CHANNEL_UNIPAY_WECHAT)){
//call Unipaygo Wechat
					MsgObjPtr = CreateObj(MsgPtr,"UnwMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format UnwMsg error\n"));
ERRLOG("FormatMsg format UnwMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
...
//call XJ-JINHAI QR
					MsgObjPtr = CreateObj(MsgPtr,"XjqMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format XjqMsg error\n"));
ERRLOG("FormatMsg format XjqMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
...
//call PayLink-Dinpay
					MsgObjPtr = CreateObj(MsgPtr,"DpyMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format DpyMsg error\n"));
ERRLOG("FormatMsg format DpyMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
...
//call Cloud123Pay
					MsgObjPtr = CreateObj(MsgPtr,"CopMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format CopMsg error\n"));
ERRLOG("FormatMsg format CopMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
				else if (!strcmp(csPspChannel,PD_CHANNEL_DINPAY_QR)){
//call PayLink-Dinpay QR
					MsgObjPtr = CreateObj(MsgPtr,"DpqMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format DpqMsg error\n"));
ERRLOG("FormatMsg format DpqMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
...
//call XJ-Ehk new API
					MsgObjPtr = CreateObj(MsgPtr,"XjeMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format XjeMsg error\n"));
ERRLOG("FormatMsg format XjeMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
...
//call STAR-KKPay EC
					MsgObjPtr = CreateObj(MsgPtr,"SkeMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format SkeMsg error\n"));
ERRLOG("FormatMsg format SkeMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
				else if (!strcmp(csPspChannel,PD_CHANNEL_RFPAY_EC)){
//call PayLink-RFPay EC
					MsgObjPtr = CreateObj(MsgPtr,"RfeMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format RfeMsg error\n"));
ERRLOG("FormatMsg format RfeMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
...
//call TZ-CoalaPay EC
					MsgObjPtr = CreateObj(MsgPtr,"CpeMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format CpeMsg error\n"));
ERRLOG("FormatMsg format CpeMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
...
//call STAR-KKPay QR
					MsgObjPtr = CreateObj(MsgPtr,"SkqMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format SkqMsg error\n"));
ERRLOG("FormatMsg format SkqMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
				else if (!strcmp(csPspChannel,PD_CHANNEL_STAR_KKPAY_VNET)){
//call STAR-KKPay VNET
					MsgObjPtr = CreateObj(MsgPtr,"SkvMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format SkvMsg error\n"));
ERRLOG("FormatMsg format SkvMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
				else if (!strcmp(csPspChannel,PD_CHANNEL_OPENEPAY)){
//call EASYPAY-OpenePay
					MsgObjPtr = CreateObj(MsgPtr,"OppMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format OppMsg error\n"));
ERRLOG("FormatMsg format OppMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
				else if (!strcmp(csPspChannel,PD_CHANNEL_SHANFU_EC)){
//call SHANFU-SFPay EC
					MsgObjPtr = CreateObj(MsgPtr,"SfeMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format SfeMsg error\n"));
ERRLOG("FormatMsg format SfeMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
				else if (!strcmp(csPspChannel,PD_CHANNEL_HXPAY_VNET)){
//call PayLink-HXPay VNET
					MsgObjPtr = CreateObj(MsgPtr,"HxvMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format HxvMsg error\n"));
ERRLOG("FormatMsg format HxvMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
				else if (!strcmp(csPspChannel,PD_CHANNEL_HXPAY_QR)){
//call PayLink-HXPay QR
					MsgObjPtr = CreateObj(MsgPtr,"HxqMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format HxqMsg error\n"));
ERRLOG("FormatMsg format HxqMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
				else if (!strcmp(csPspChannel,PD_CHANNEL_PAYESCAPE_VNET)){
//call PAYESCAPE-Epaylinks VNET
					MsgObjPtr = CreateObj(MsgPtr,"PsvMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format PsvMsg error\n"));
ERRLOG("FormatMsg format PsvMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
				else if (!strcmp(csPspChannel,PD_CHANNEL_SAMOPAY_EC)){
//call SAMOPAY-SMPay EC
					MsgObjPtr = CreateObj(MsgPtr,"SmeMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format SmeMsg error\n"));
ERRLOG("FormatMsg format SmeMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
				else if (!strcmp(csPspChannel,PD_CHANNEL_RFPAY_QR)){
//call PayLink-RFPay QR
					MsgObjPtr = CreateObj(MsgPtr,"RfqMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format RfqMsg error\n"));
ERRLOG("FormatMsg format RfqMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
				else if (!strcmp(csPspChannel,PD_CHANNEL_SHANFU_QR)){
//call SHANFU-Payease QR
					MsgObjPtr = CreateObj(MsgPtr,"SfqMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format SfqMsg error\n"));
ERRLOG("FormatMsg format SfqMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
				else if (!strcmp(csPspChannel,PD_CHANNEL_RFPAY_VNET)){
//call PayLink-RFPay VNET
					MsgObjPtr = CreateObj(MsgPtr,"RfvMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format RfvMsg error\n"));
ERRLOG("FormatMsg format RfvMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
				else if (!strcmp(csPspChannel,PD_CHANNEL_BFBAO_VNET)){
//call PayLink-BFBao VNET
					MsgObjPtr = CreateObj(MsgPtr,"BfvMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format BfvMsg error\n"));
ERRLOG("FormatMsg format BfvMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
...
//call OUP-YLPay EC
					MsgObjPtr = CreateObj(MsgPtr,"OupMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format OupMsg error\n"));
ERRLOG("FormatMsg format OupMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
				else if (!strcmp(csPspChannel,PD_CHANNEL_BFBAO_QR)){
//call PayLink-BFBao QR
					MsgObjPtr = CreateObj(MsgPtr,"BfqMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format BfqMsg error\n"));
ERRLOG("FormatMsg format BfqMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
				else if (!strcmp(csPspChannel,PD_CHANNEL_GOLDENPAY_VNET)){
//call GOLDENPAY-SandPay VNET
					MsgObjPtr = CreateObj(MsgPtr,"GpvMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format GpvMsg error\n"));
ERRLOG("FormatMsg format GpvMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
...
//call C123PAY QR
					MsgObjPtr = CreateObj(MsgPtr,"CoqMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format CoqMsg error\n"));
ERRLOG("FormatMsg format CoqMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
				else if (!strcmp(csPspChannel,PD_CHANNEL_GOLDENPAY_EC)){
//call GOLDENPAY-SandPay EC
					MsgObjPtr = CreateObj(MsgPtr,"GpeMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format GpeMsg error\n"));
ERRLOG("FormatMsg format GpeMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
...
//call ZPAY EC
					MsgObjPtr = CreateObj(MsgPtr,"ZpeMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format ZpeMsg error\n"));
ERRLOG("FormatMsg format ZpeMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
...
//call ESKYPAY QR
					MsgObjPtr = CreateObj(MsgPtr,"EsqMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format EsqMsg error\n"));
ERRLOG("FormatMsg format EsqMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
...
//call Now3Pay
					MsgObjPtr = CreateObj(MsgPtr,"NepMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format NepMsg error\n"));
ERRLOG("FormatMsg format NepMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
...
//call PayLink-TTPay
					MsgObjPtr = CreateObj(MsgPtr,"TtpMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format TtpMsg error\n"));
ERRLOG("FormatMsg format TtpMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
				else if(!strcmp(csPspChannel,PD_CHANNEL_TTPAY_QR)){
//call PayLink-TTPay QR
					MsgObjPtr = CreateObj(MsgPtr,"TtqMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format TtqMsg error\n"));
ERRLOG("FormatMsg format TtqMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
...
//call ZPAY QR
					MsgObjPtr = CreateObj(MsgPtr,"ZpqMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format ZpqMsg error\n"));
ERRLOG("FormatMsg format ZpqMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
				else if (!strcmp(csPspChannel,PD_CHANNEL_WELLPAY)){
//call PayLink-WellPay
					MsgObjPtr = CreateObj(MsgPtr,"WpyMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format WpyMsg error\n"));
ERRLOG("FormatMsg format WpyMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
...
//call XJ-DBao
					MsgObjPtr = CreateObj(MsgPtr,"XdbMsg","FormatMsg");
					if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format XdbMsg error\n"));
ERRLOG("FormatMsg format XdbMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
				}
				else if (!strcmp(csPspChannel, PD_CHANNEL_SWIFTPAY_QR)) {
//call PayLink-SwiftPay QR
					MsgObjPtr = CreateObj(MsgPtr, "SpqMsg", "FormatMsg");
					if ((*MsgObjPtr)(hIn, outMsg, outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format SpqMsg error\n"));
ERRLOG("FormatMsg format SpqMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n", outMsg));
				}
				else if (!strcmp(csPspChannel, PD_CHANNEL_XPAY_VNET)) {
//call GOLDENPAY-XPay VNET
					MsgObjPtr = CreateObj(MsgPtr, "XpvMsg", "FormatMsg");
					if ((*MsgObjPtr)(hIn, outMsg, outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format XpvMsg error\n"));
ERRLOG("FormatMsg format XpvMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n", outMsg));
				}
				else if (!strcmp(csPspChannel, PD_CHANNEL_TRUSTPAY_EC)) {
//call TRUSTPAY-SandPay EC
					MsgObjPtr = CreateObj(MsgPtr, "TpeMsg", "FormatMsg");
					if ((*MsgObjPtr)(hIn, outMsg, outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format TpeMsg error\n"));
ERRLOG("FormatMsg format TpeMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n", outMsg));
				}
				else if (!strcmp(csPspChannel, PD_CHANNEL_OUP_OPENEPAY_EC)) {
//call OUP-OpenePay EC
					MsgObjPtr = CreateObj(MsgPtr, "OoeMsg", "FormatMsg");
					if ((*MsgObjPtr)(hIn, outMsg, outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format OoeMsg error\n"));
ERRLOG("FormatMsg format OoeMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n", outMsg));
				}
...
//call EASYPAY-DF QR
					MsgObjPtr = CreateObj(MsgPtr, "DfqMsg", "FormatMsg");
					if ((*MsgObjPtr)(hIn, outMsg, outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format DfqMsg error\n"));
ERRLOG("FormatMsg format DfqMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n", outMsg));
				}
...
//call XJ-DBao QR
					MsgObjPtr = CreateObj(MsgPtr, "XdqMsg", "FormatMsg");
					if ((*MsgObjPtr)(hIn, outMsg, outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format XdqMsg error\n"));
ERRLOG("FormatMsg format XdqMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n", outMsg));
				}
...
//call XJ-CH
					MsgObjPtr = CreateObj(MsgPtr, "XjcMsg", "FormatMsg");
					if ((*MsgObjPtr)(hIn, outMsg, outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format XjcMsg error\n"));
ERRLOG("FormatMsg format XjcMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n", outMsg));
				}
...
//call XJ-CH QR
					MsgObjPtr = CreateObj(MsgPtr, "XjrMsg", "FormatMsg");
					if ((*MsgObjPtr)(hIn, outMsg, outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format XjrMsg error\n"));
ERRLOG("FormatMsg format XjrMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n", outMsg));
				}
...
//call Now3Pay QR
					MsgObjPtr = CreateObj(MsgPtr, "NeqMsg", "FormatMsg");
					if ((*MsgObjPtr)(hIn, outMsg, outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format NeqMsg error\n"));
ERRLOG("FormatMsg format NeqMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n", outMsg));
				}
				else if (!strcmp(csPspChannel, PD_CHANNEL_XPAY_QR)) {
//call GOLDENPAY-XPay QR
					MsgObjPtr = CreateObj(MsgPtr, "XpqMsg", "FormatMsg");
					if ((*MsgObjPtr)(hIn, outMsg, outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format XpqMsg error\n"));
ERRLOG("FormatMsg format XpqMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n", outMsg));
				}
				else if (!strcmp(csPspChannel, PD_CHANNEL_UNIPAY_QR)) {
//call UNIPAYGO QR
					MsgObjPtr = CreateObj(MsgPtr, "UnqMsg", "FormatMsg");
					if ((*MsgObjPtr)(hIn, outMsg, outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format UnqMsg error\n"));
ERRLOG("FormatMsg format UnqMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n", outMsg));
				}
				else if (!strcmp(csPspChannel, PD_CHANNEL_TRUSTPAY_QR)) {
//call TRUSTPAY QR
					MsgObjPtr = CreateObj(MsgPtr, "TpqMsg", "FormatMsg");
					if ((*MsgObjPtr)(hIn, outMsg, outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format TpqMsg error\n"));
ERRLOG("FormatMsg format TpqMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n", outMsg));
				}
				else if (!strcmp(csPspChannel, PD_CHANNEL_ZXPAY_EC)) {
//call provider XPAY EC
					MsgObjPtr = CreateObj(MsgPtr, "XyeMsg", "FormatMsg");
					if ((*MsgObjPtr)(hIn, outMsg, outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format XyeMsg error\n"));
ERRLOG("FormatMsg format XyeMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n", outMsg));
				}
				else if (!strcmp(csPspChannel, PD_CHANNEL_ZXPAY_QR)) {
//call provider XPAY QR
					MsgObjPtr = CreateObj(MsgPtr, "XyqMsg", "FormatMsg");
					if ((*MsgObjPtr)(hIn, outMsg, outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format XyqMsg error\n"));
ERRLOG("FormatMsg format XyqMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n", outMsg));
				}
				else if (!strcmp(csPspChannel, PD_CHANNEL_HANDPAY_VNET)) {
//call EASYPAY-HANDPAY VNET
					MsgObjPtr = CreateObj(MsgPtr, "HdvMsg", "FormatMsg");
					if ((*MsgObjPtr)(hIn, outMsg, outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format HdvMsg error\n"));
ERRLOG("FormatMsg format HdvMsg error\n");
					}
					else
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n", outMsg));
				}
...
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
					...
					else if (!strcmp(csPspChannel,PD_CHANNEL_N2P_AIPWECHAT) && cArInd == PD_REJECT) {
						MsgObjPtr = CreateObj(MsgPtr,"NtwMsg","FormatMsg");
						if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format NtwMsg error\n"));
ERRLOG("FormatMsg format NtwMsg error\n");
...
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
					...
					else if (!strcmp(csPspChannel,PD_CHANNEL_XJ_CHWECHAT) && cArInd == PD_REJECT) {
						MsgObjPtr = CreateObj(MsgPtr,"XjwMsg","FormatMsg");
						if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format XjwMsg error\n"));
ERRLOG("FormatMsg format XjwMsg error\n");
...
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
					...
					else if (!strcmp(csPspChannel,PD_CHANNEL_UNIPAY_WECHAT) && cArInd == PD_REJECT) {
						MsgObjPtr = CreateObj(MsgPtr,"UnwMsg","FormatMsg");
						if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format UnwMsg error\n"));
ERRLOG("FormatMsg format UnwMsg error\n");
...
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
					...
					else if (!strcmp(csPspChannel,PD_CHANNEL_XJ_JINHAI_QR) && cArInd == PD_REJECT) {
						MsgObjPtr = CreateObj(MsgPtr,"XjqMsg","FormatMsg");
						if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format XjqMsg error\n"));
ERRLOG("FormatMsg format XjqMsg error\n");
...
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
					...
					else if (!strcmp(csPspChannel,PD_CHANNEL_DINPAY_QR) && cArInd == PD_REJECT) {
						MsgObjPtr = CreateObj(MsgPtr,"DpqMsg","FormatMsg");
						if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format DpqMsg error\n"));
ERRLOG("FormatMsg format DpqMsg error\n");
...
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
					...
					else if (!strcmp(csPspChannel,PD_CHANNEL_STAR_KKPAY_QR) && cArInd == PD_REJECT) {
						MsgObjPtr = CreateObj(MsgPtr,"SkqMsg","FormatMsg");
						if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format SkqMsg error\n"));
ERRLOG("FormatMsg format SkqMsg error\n");
...
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
					...
					else if (!strcmp(csPspChannel,PD_CHANNEL_HXPAY_QR) && cArInd == PD_REJECT) {
						MsgObjPtr = CreateObj(MsgPtr,"HxqMsg","FormatMsg");
						if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format HxqMsg error\n"));
ERRLOG("FormatMsg format HxqMsg error\n");
...
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
					...
					else if (!strcmp(csPspChannel,PD_CHANNEL_RFPAY_QR) && cArInd == PD_REJECT) {
						MsgObjPtr = CreateObj(MsgPtr,"RfqMsg","FormatMsg");
						if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format RfqMsg error\n"));
ERRLOG("FormatMsg format RfqMsg error\n");
...
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
					...
					else if (!strcmp(csPspChannel,PD_CHANNEL_SHANFU_QR) && cArInd == PD_REJECT) {
						MsgObjPtr = CreateObj(MsgPtr,"SfqMsg","FormatMsg");
						if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format SfqMsg error\n"));
ERRLOG("FormatMsg format SfqMsg error\n");
...
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
					...
					else if (!strcmp(csPspChannel,PD_CHANNEL_BFBAO_QR) && cArInd == PD_REJECT) {
						MsgObjPtr = CreateObj(MsgPtr,"BfqMsg","FormatMsg");
						if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format BfqMsg error\n"));
ERRLOG("FormatMsg format BfqMsg error\n");
...
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
					...
					else if (!strcmp(csPspChannel,PD_CHANNEL_C123PAY_QR) && cArInd == PD_REJECT) {
						MsgObjPtr = CreateObj(MsgPtr,"CoqMsg","FormatMsg");
						if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format CoqMsg error\n"));
ERRLOG("FormatMsg format CoqMsg error\n");
...
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
					...
					else if (!strcmp(csPspChannel,PD_CHANNEL_ESKY_QR) && cArInd == PD_REJECT) {
						MsgObjPtr = CreateObj(MsgPtr,"EsqMsg","FormatMsg");
						if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format EsqMsg error\n"));
ERRLOG("FormatMsg format EsqMsg error\n");
...
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
					...
					else if (!strcmp(csPspChannel,PD_CHANNEL_TTPAY_QR) && cArInd == PD_REJECT) {
						MsgObjPtr = CreateObj(MsgPtr,"TtqMsg","FormatMsg");
						if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format TtqMsg error\n"));
ERRLOG("FormatMsg format TtqMsg error\n");
...
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
					...
					else if (!strcmp(csPspChannel,PD_CHANNEL_ZPAY_QR) && cArInd == PD_REJECT) {
						MsgObjPtr = CreateObj(MsgPtr,"ZpqMsg","FormatMsg");
						if ((*MsgObjPtr)(hIn,outMsg,outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format ZpqMsg error\n"));
ERRLOG("FormatMsg format ZpqMsg error\n");
...
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n",outMsg));
					...
					else if (!strcmp(csPspChannel, PD_CHANNEL_SWIFTPAY_QR) && cArInd == PD_REJECT) {
						MsgObjPtr = CreateObj(MsgPtr, "SpqMsg", "FormatMsg");
						if ((*MsgObjPtr)(hIn, outMsg, outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format SpqMsg error\n"));
ERRLOG("FormatMsg format SpqMsg error\n");
...
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n", outMsg));
					...
					else if (!strcmp(csPspChannel, PD_CHANNEL_DF_QR) && cArInd == PD_REJECT) {
						MsgObjPtr = CreateObj(MsgPtr, "DfqMsg", "FormatMsg");
						if ((*MsgObjPtr)(hIn, outMsg, outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format DfqMsg error\n"));
ERRLOG("FormatMsg format DfqMsg error\n");
...
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n", outMsg));
					...
					else if (!strcmp(csPspChannel, PD_CHANNEL_XJ_DBAO_QR) && cArInd == PD_REJECT) {
						MsgObjPtr = CreateObj(MsgPtr, "XdqMsg", "FormatMsg");
						if ((*MsgObjPtr)(hIn, outMsg, outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format XdqMsg error\n"));
ERRLOG("FormatMsg format XdqMsg error\n");
...
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n", outMsg));
					...
					else if (!strcmp(csPspChannel, PD_CHANNEL_XJ_CH_QR) && cArInd == PD_REJECT) {
						MsgObjPtr = CreateObj(MsgPtr, "XjrMsg", "FormatMsg");
						if ((*MsgObjPtr)(hIn, outMsg, outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format XjrMsg error\n"));
ERRLOG("FormatMsg format XjrMsg error\n");
...
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n", outMsg));
					...
					else if (!strcmp(csPspChannel, PD_CHANNEL_NOW3PAY_QR) && cArInd == PD_REJECT) {
						MsgObjPtr = CreateObj(MsgPtr, "NeqMsg", "FormatMsg");
						if ((*MsgObjPtr)(hIn, outMsg, outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format NeqMsg error\n"));
ERRLOG("FormatMsg format NeqMsg error\n");
...
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n", outMsg));
					...
					else if (!strcmp(csPspChannel, PD_CHANNEL_XPAY_QR) && cArInd == PD_REJECT) {
						MsgObjPtr = CreateObj(MsgPtr, "XpqMsg", "FormatMsg");
						if ((*MsgObjPtr)(hIn, outMsg, outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format XpqMsg error\n"));
ERRLOG("FormatMsg format XpqMsg error\n");
...
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n", outMsg));
					...
					else if (!strcmp(csPspChannel, PD_CHANNEL_UNIPAY_QR) && cArInd == PD_REJECT) {
						MsgObjPtr = CreateObj(MsgPtr, "UnqMsg", "FormatMsg");
						if ((*MsgObjPtr)(hIn, outMsg, outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format UnqMsg error\n"));
ERRLOG("FormatMsg format UnqMsg error\n");
...
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n", outMsg));
					...
					else if (!strcmp(csPspChannel, PD_CHANNEL_TRUSTPAY_QR) && cArInd == PD_REJECT) {
						MsgObjPtr = CreateObj(MsgPtr, "TpqMsg", "FormatMsg");
						if ((*MsgObjPtr)(hIn, outMsg, outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format TpqMsg error\n"));
ERRLOG("FormatMsg format TpqMsg error\n");
...
DEBUGLOG(("FormatMsg:: outmsg = [%s]\n", outMsg));
					...
					else if (!strcmp(csPspChannel, PD_CHANNEL_ZXPAY_QR) && cArInd == PD_REJECT) {
						MsgObjPtr = CreateObj(MsgPtr, "XyqMsg", "FormatMsg");
						if ((*MsgObjPtr)(hIn, outMsg, outLen) != PD_OK) {
DEBUGLOG(("FormatMsg format XyqMsg error\n"));
ERRLOG("FormatMsg format XyqMsg error\n");
