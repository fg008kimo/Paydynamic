...
-------------------------------                    ------------            --------------
Init Version					   2019/07/17              David Wong
*/
...
// Country
		json_object_object_add(jobj, "Country", json_object_new_string(MY_XYE_COUNTRY));
...
// Pay_Code
		if (GetField_CString(hIn, "bank_code", &csBankCode)) {
			...
			if (iOpt == PD_FALSE && iOpt != INT_ERR) {
				iIsVnet = PD_TRUE;
				json_object_object_add(jobj, "Pay_Code", json_object_new_string(MY_XYE_PAYCODE_WEB));
DEBUGLOG(("Pay_Code = [%s]\n", MY_XYE_PAYCODE_WEB));
			} else if (iOpt && iOpt != INT_ERR) {

				DBObjPtr = CreateObj(DBPtr, "DBBankMapping", "i2eBankCodeMapping");
				if ((DBObjPtr)(csBankCode, "XYE_H5", "CN", csOutBankCode)) {
					json_object_object_add(jobj, "Pay_Code", json_object_new_string(csOutBankCode));
DEBUGLOG(("Pay_Code = [%s]\n", csOutBankCode));
				} else {
					json_object_object_add(jobj, "Pay_Code", json_object_new_string(MY_XYE_PAYCODE_EC));
DEBUGLOG(("Pay_Code = [%s]\n", MY_XYE_PAYCODE_EC));
				}
			}
		}
...
DEBUGLOG(("Currency = [%s]\n", csPtr));
			...
			json_object_object_add(jobj, "Currency", json_object_new_string(MY_XYE_CCY_CODE));
...
// Verno
		json_object_object_add(jobj, "Verno", json_object_new_string(MY_XYE_VERSION));
...
DEBUGLOG(("after encode\n"));
                ...
                strcat((char*)outMsg, MY_XYE_TOKEN);
...
DEBUGLOG(("BuildAuthData:: Currency = [%s]\n", csPtr));
		...
		strcat((char*)csBuf, MY_XYE_CCY_CODE);
...
DEBUGLOG(("FormatInqMsg() Start\n"));
	...
	if (GetField_CString(hIn, "psp_url", &csURL)) {
		if (GetField_CString(hIn, "request_function", &csPtr)) {
			...
			strcat((char*)csBuf, MY_XYE_FIELD_TOKEN);
			strcat((char*)csBuf, csURL);
			strcat((char*)csBuf, "/");
			strcat((char*)csBuf, csPtr);

			/* added for gw to identify the request */
			strcat((char*)csBuf, MY_XYE_TOKEN);
...
DEBUGLOG(("***psp_merchant_id is missing\n"));
	...
	// Verno
DEBUGLOG(("Verno = [%s]\n", MY_XYE_VERSION));
	strcat((char*)csBuf, MY_XYE_VERSION);
