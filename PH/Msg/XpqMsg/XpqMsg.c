...
DEBUGLOG(("return_code = [%s]\n", csPtr));
		...
		strcat((char*)outMsg, MY_XPQ_FIELD_TOKEN);
		if (!strcmp(csPtr, MY_XPQ_SUCCESS_STATUS))
			strcat((char*)outMsg,"SUCCESS");
		else
			strcat((char*)outMsg, csPtr);
		strcat((char*)outMsg, MY_XPQ_TOKEN);
	} else {
...
// amount
	if (GetField_Double(hIn, "psp_txn_amt", &dTmp)) {
		...
		strcat((char*)csBuf, MY_XPQ_FIELD_TOKEN);
		strcat((char*)csBuf, (char*)csTmpAmt);
		strcat((char*)csBuf, MY_XPQ_TOKEN);
DEBUGLOG(("amount = [%s]\n", csTmpAmt));
	}
	else {
...
// payType
	if (GetField_CString(hIn, "bank_code", &csPtr)) {
		...
		if ((DBObjPtr)(csPtr, "XPQ", "CN", csOutBankCode)) {
...
// settleType
	strcat((char*)csBuf, "settleType");
	strcat((char*)csBuf, MY_XPQ_FIELD_TOKEN);
	strcat((char*)csBuf, MY_SETTLE_TYPE);
	strcat((char*)csBuf, MY_XPQ_TOKEN);
DEBUGLOG(("settleType = [%s]\n", MY_SETTLE_TYPE));

// traceno
	if (GetField_CString(hIn, "order_num", &csPtr)) {
		strcat((char*)csBuf, "traceno");
		strcat((char*)csBuf, MY_XPQ_FIELD_TOKEN);
		strcat((char*)csBuf, csPtr);
		strcat((char*)csBuf, MY_XPQ_TOKEN);
DEBUGLOG(("traceno = [%s]\n", csPtr));
...
DEBUGLOG(("DATA = [%s][%d]\n", inMsg, inLen));
	...
	if (Str2Cls(hRec, (char *)inMsg, MY_XPQ_TOKEN, MY_XPQ_FIELD_TOKEN) == PD_OK) {
/* merchno */
		if (GetField_CString(hRec, "merchno", &csPtr)) {
			PutField_CString(hOut, "psp_merchant_id", csPtr);
DEBUGLOG(("merchno:psp_merchant_id = [%s]\n", csPtr));
		}
		else {
DEBUGLOG(("merchno:psp_merchant_id not found\n"));
		}
...
DEBUGLOG(("traceno:tid = [%s]\n", csPtr));
		}
		else {
DEBUGLOG(("traceno:tid not found\n"));
		}
...
// settleType
	strcat((char*)outMsg, "settleType");
	strcat((char*)outMsg, MY_XPQ_FIELD_TOKEN);
	strcat((char*)outMsg, MY_SETTLE_TYPE);
	strcat((char*)outMsg, MY_XPQ_TOKEN);
DEBUGLOG(("settleType = [%s]\n", MY_SETTLE_TYPE));
...
DEBUGLOG(("DATA = [%s][%d]\n", inMsg, inLen));
	...
	jobj = json_tokener_parse(code_convert(PD_GB2312_CODE, PD_HOST_CODE, (char *)inMsg));
	if (jobj != NULL) {
		json_object_object_foreach(jobj, key, val) {
			...
			switch (type) {
				case json_type_string:
					...
					strcat((char*)csMsg, MY_XPQ_FIELD_TOKEN);
					strcat((char*)csMsg, json_object_get_string(val));
					strcat((char*)csMsg, MY_XPQ_TOKEN);
				break;
				default:
DEBUGLOG(("unsupported type\n"));
	...
	if (Str2Cls(hRec, (char *)csMsg, MY_XPQ_TOKEN, MY_XPQ_FIELD_TOKEN) == PD_OK) {
...
DEBUGLOG(("message:return_msg = [%s]\n", csPtr));
			...
			if (!strcmp(csReturnCode, MY_XPQ_SUCCESS_STATUS)) {
				// merchno
				if (GetField_CString(hRec, "merchno", &csPtr)) {
					PutField_CString(hOut, "merchno", csPtr);
DEBUGLOG(("merchno = [%s]\n", csPtr));
				}
...
DEBUGLOG(("traceno:requestId = [%s]\n", csPtr));
				}

				// refno
				if (GetField_CString(hRec, "refno", &csPtr)) {
					PutField_CString(hOut, "refno", csPtr);
DEBUGLOG(("refno = [%s]\n", csPtr));
				}

				// barCode
				if (GetField_CString(hRec, "barCode", &csPtr)) {
					PutField_CString(hOut, "code_url", csPtr);
DEBUGLOG(("barCode:code_url = [%s]\n", csPtr));
				}
...
DEBUGLOG(("BuildRspAuthData()\n"));
	...
	if (GetField_CString(hIn, "in_msg", &inMsg)) {
		if (Str2KVPair(k_v_pair, &iPairCnt, inMsg, MY_XPQ_TOKEN, MY_XPQ_FIELD_TOKEN) == PD_OK) {
//DEBUGLOG(("iPairCnt = [%d]\n", iPairCnt));
...
DEBUGLOG(("key = [%s], value = [%s]\n", k_v_pair[iTmp].key, k_v_pair[iTmp].value));
				...
				if (strcmp(k_v_pair[iTmp].key, "signature")) {
					...
					strcat(csBuf, MY_XPQ_FIELD_TOKEN);
					strcat(csBuf, k_v_pair[iTmp].value);
					strcat(csBuf, MY_XPQ_TOKEN);
				}
			}
		}
	}
