...
/* adjustment */
        if (GetField_Double(hIn,"adjustment",&dTmp)) {
DEBUGLOG(("FormatMsg:: adjustment = [%f]\n",dTmp));
                sprintf((char*)csBuf,"%ld",double2long(dTmp));
DEBUGLOG(("FormatMsg:: adj = [%s]\n",csBuf));
                strcat((char*)outMsg,"adj");
                strcat((char*)outMsg,MY_MMS_FIELD_TOKEN);
                strcat((char*)outMsg,(char*)csBuf);
                strcat((char*)outMsg,MY_MMS_TOKEN);
        }
...
/* refund */
        if (GetField_Double(hIn,"bank_charge_refund",&dTmp)) {
DEBUGLOG(("FormatMsg:: bank_charge_refund = [%f]\n",dTmp));
                sprintf((char*)csBuf,"%ld",double2long(dTmp));
DEBUGLOG(("FormatMsg:: bk_chrg_refund = [%s]\n",csBuf));
                strcat((char*)outMsg,"refund");
                strcat((char*)outMsg,MY_MMS_FIELD_TOKEN);
                strcat((char*)outMsg,(char*)csBuf);
                strcat((char*)outMsg,MY_MMS_TOKEN);
        }
...
DEBUGLOG(("FormatMsg :: init_channel = [%s]\n",csPtr));
...
/* adjustment*/
                if (GetField_CString(hRec,"adj",&csPtr)) {
                        PutField_CString(hOut,"adj",csPtr);
DEBUGLOG(("BreakDownMsg:: adj= [%s]\n",csPtr));
                }	

/* refund */
                if (GetField_CString(hRec,"refund",&csPtr)) {
                        PutField_CString(hOut,"refund",csPtr);
DEBUGLOG(("BreakDownMsg:: refund= [%s]\n",csPtr));
                }	
...
/* verno */
                if (GetField_CString(hRec,"verno",&csPtr)) {
                        PutField_CString(hOut,"version_no",csPtr);
DEBUGLOG(("BreakDownNodeMsg:: version_no = [%s]\n",csPtr));
                }

/* enctype */
                if (GetField_CString(hRec,"enctype",&csPtr)) {
                        PutField_CString(hOut,"encrypt_type",csPtr);
DEBUGLOG(("BreakDownNodeMsg:: encrypt_type = [%s]\n",csPtr));
                }

/* ret */
                if (GetField_CString(hRec,"ret",&csPtr)) {
			iTmp = atoi(csPtr);
                        PutField_Int(hOut,"response_code",iTmp);
DEBUGLOG(("BreakDownNodeMsg:: response_code = [%d]\n",iTmp));
                }

/* country */
                if (GetField_CString(hRec,"country",&csPtr)) {
                        PutField_CString(hOut,"txn_country",csPtr);
DEBUGLOG(("BreakDownNodeMsg:: txn_country = [%s]\n",csPtr));
                }
