...
-------------------------------                    ------------            --------------
Init Version                                       2018/02/09              David Wong
*/
...
// service
		...
		strcat((char*)outMsg, MY_HXV_FIELD_TOKEN);
		strcat((char*)outMsg, MY_HXV_SERVICE);
		strcat((char*)outMsg, MY_HXV_TOKEN);
DEBUGLOG(("service = [%s]\n", MY_HXV_SERVICE));
...
// version
		...
		strcat((char*)outMsg, MY_HXV_FIELD_TOKEN);
		strcat((char*)outMsg, MY_HXV_VERSION);
		strcat((char*)outMsg, MY_HXV_TOKEN);
DEBUGLOG(("version = [%s]\n", MY_HXV_VERSION));
...
// payChannelType
		strcat((char*)outMsg, "payChannelType");
		strcat((char*)outMsg, MY_HXV_FIELD_TOKEN);
		strcat((char*)outMsg, MY_HXV_PAY_CH_TYPE);
		strcat((char*)outMsg, MY_HXV_TOKEN);
DEBUGLOG(("payChannelType = [%s]\n", MY_HXV_PAY_CH_TYPE));
...
// curCode
		...
		strcat((char*)outMsg, MY_HXV_FIELD_TOKEN);
		strcat((char*)outMsg, MY_HXV_CUR_CODE);
		strcat((char*)outMsg, MY_HXV_TOKEN);
DEBUGLOG(("curCode = [%s]\n", MY_HXV_CUR_CODE));
...
// orderSource
		...
		strcat((char*)outMsg, MY_HXV_FIELD_TOKEN);
		strcat((char*)outMsg, MY_HXV_ORDER_SRC);
		strcat((char*)outMsg, MY_HXV_TOKEN);
...
// signType
		...
		strcat((char*)outMsg, MY_HXV_FIELD_TOKEN);
		strcat((char*)outMsg, MY_HXV_SIGN_TYPE);
		strcat((char*)outMsg, MY_HXV_TOKEN);
DEBUGLOG(("signType = [%s]\n", MY_HXV_SIGN_TYPE));
...
// sign
		if (GetField_CString(hIn, "sign", &csTmp)) {
			...
			strcat((char*)outMsg, MY_HXV_FIELD_TOKEN);
			strcat((char*)outMsg, str_replace(str_replace(str_replace(url_encode(csTmp),"%2f","%2F"),"%2b","%2B"),"%3d","%3D"));
DEBUGLOG(("sign = [%s]\n", str_replace(str_replace(str_replace(url_encode(csTmp),"%2f","%2F"),"%2b","%2B"),"%3d","%3D")));
		}
		else {
			iRet = PD_ERR;
		...
		json_object_object_foreach(jobj, key, val) {
			...
			switch (type) {
				case json_type_string:
					...
					strcat((char*)csMsg, MY_HXV_FIELD_TOKEN);
					strcat((char*)csMsg, json_object_get_string(val));
					strcat((char*)csMsg, MY_HXV_TOKEN);
				break;
				default:
DEBUGLOG(("BreakDownInitRspMsg:: unsupported type\n"));
	...
	if (Str2Cls(hRec, (char *)csMsg, MY_HXV_TOKEN, MY_HXV_FIELD_TOKEN) == PD_OK) {
// inMsg
...
// bgUrl
	if (GetField_CString(hIn, "return_url_only", &csPtr)) {
		...
		strcat((char*)csBuf, MY_HXV_FIELD_TOKEN);
		strcat((char*)csBuf, csPtr);
		strcat((char*)csBuf, MY_HXV_TOKEN);
DEBUGLOG(("bgUrl = [%s]\n", csPtr));
	}
	else {
		iRet = PD_ERR;
...
// curCode
	...
	strcat((char*)csBuf, MY_HXV_FIELD_TOKEN);
	strcat((char*)csBuf, MY_HXV_CUR_CODE);
	strcat((char*)csBuf, MY_HXV_TOKEN);
DEBUGLOG(("curCode = [%s]\n", MY_HXV_CUR_CODE));
...
// orderSource
	...
	strcat((char*)csBuf, MY_HXV_FIELD_TOKEN);
	strcat((char*)csBuf, MY_HXV_ORDER_SRC);
	strcat((char*)csBuf, MY_HXV_TOKEN);
DEBUGLOG(("orderSource = [%s]\n", MY_HXV_ORDER_SRC));
...
// payChannelType
	strcat((char*)csBuf, "payChannelType");
	strcat((char*)csBuf, MY_HXV_FIELD_TOKEN);
	strcat((char*)csBuf, MY_HXV_PAY_CH_TYPE);
	strcat((char*)csBuf, MY_HXV_TOKEN);
DEBUGLOG(("payChannelType = [%s]\n", MY_HXV_PAY_CH_TYPE));
...
// service
	...
	strcat((char*)csBuf, MY_HXV_FIELD_TOKEN);
	strcat((char*)csBuf, MY_HXV_SERVICE);
	strcat((char*)csBuf, MY_HXV_TOKEN);
DEBUGLOG(("service = [%s]\n", MY_HXV_SERVICE));
...
// signType
	...
	strcat((char*)csBuf, MY_HXV_FIELD_TOKEN);
	strcat((char*)csBuf, MY_HXV_SIGN_TYPE);
	strcat((char*)csBuf, MY_HXV_TOKEN);
DEBUGLOG(("signType = [%s]\n", MY_HXV_SIGN_TYPE));
...
// version
	...
	strcat((char*)csBuf, MY_HXV_FIELD_TOKEN);
	strcat((char*)csBuf, MY_HXV_VERSION);
	//strcat((char*)csBuf, MY_HXV_TOKEN);
DEBUGLOG(("version = [%s]\n", MY_HXV_VERSION));
...
DEBUGLOG(("BuildRspAuthData()\n"));
	...
	if (GetField_CString(hIn, "in_msg", &inMsg)) {
		if (Str2KVPair(k_v_pair, &iPairCnt, inMsg, MY_HXV_TOKEN, MY_HXV_FIELD_TOKEN) == PD_OK) {
//DEBUGLOG(("iPairCnt = [%d]\n", iPairCnt));
...
DEBUGLOG(("key = [%s], value = [%s]\n", k_v_pair[iTmp].key, k_v_pair[iTmp].value));
				...
				if (strcmp(k_v_pair[iTmp].key, "sign")) {
					...
					strcat(csBuf, MY_HXV_FIELD_TOKEN);
					strcat(csBuf, k_v_pair[iTmp].value);
					strcat(csBuf, MY_HXV_TOKEN);
				}
			}
		}
	}
