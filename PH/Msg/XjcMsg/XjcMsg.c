...
// p0_Cmd
		...
		strcat((char*)outMsg,MY_XJC_FIELD_TOKEN);
		strcat((char*)outMsg,PD_P0_CMD);
		strcat((char*)outMsg,MY_XJC_TOKEN);
...
DEBUGLOG(("DATA = [%s][%d]\n",inMsg,inLen));
	...
	if (Str2Cls(hRec,(char *)inMsg,MY_XJC_TOKEN, MY_XJC_FIELD_TOKEN) == PD_OK) {
/* p1_MerId */
...
DEBUGLOG(("FormatInqMsg() Start\n"));
	...
	if (GetField_CString(hIn, "psp_url", &csURL)) {
		if (GetField_CString(hIn, "request_function", &csPtr)) {
			...
			strcat((char*)csBuf, MY_XJC_FIELD_TOKEN);
			strcat((char*)csBuf, csURL);
			strcat((char*)csBuf, "/");
			strcat((char*)csBuf, csPtr);

			/* added for gw to identify the request */
			strcat((char*)csBuf, MY_XJC_TOKEN);
