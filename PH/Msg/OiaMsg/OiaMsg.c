/*
PDProTech (c)2018. All rights reserved. No part of this software may be reproduced in any form without written permission
of an authorized representative of PDProTech.

Change Description                                 Change Date             Change By
-------------------------------                    ------------            --------------
Init Version (Clone from OPL)                      2018/11/12              Virginia Yun
Add bank_code
    bank_abbrev
    province
    city
    reg_mob_no
    bank_upd_ts
    baid_upd_ts in FormatMsg
Add req_dt
    bank_code
    acct_num in BreakDownMsg			   2019/08/22		   Elvis Wong
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <ctype.h>
#include "OiaMsg.h"
#include "common.h"
#include "utilitys.h"
#include "b64.h"
#include "queue_defs.h"
#include "internal.h"
#include <zlib.h>
#include "ObjPtr.h"
#include "msg.h"
#include "myrecordset.h"


static char	cDebug;

OBJPTR(Msg);
OBJPTR(DB);

void	OiaMsg(char cdebug)
{
	cDebug = cdebug;
}

char *str_replace(char *orig, char *rep, char *with);


int FormatMsg(const hash_t* hIn, unsigned char *outMsg, int *outLen)
{
	int	iRet = PD_OK;
	char    *csPtr = NULL;
	char    csData[MAX_MSG_SIZE + 1];
	csData[0] = '\0';

DEBUGLOG(("FormatMsg()\n"));

	json_object *jobj = json_object_new_object();
	json_object *new_jobj = json_object_new_object();

//msg_type
	if (GetField_CString(hIn, "process_type", &csPtr)) {
		json_object_object_add(jobj, "msg_type", json_object_new_string(csPtr));
DEBUGLOG((" process_type = [%s]\n", csPtr));
	}
	else {
		iRet = PD_ERR;
DEBUGLOG((" ***process_type is missing\n"));
	}

//msg_code
	if (iRet == PD_OK ) {
		if (GetField_CString(hIn, "msg_code", &csPtr)) {
			json_object_object_add(jobj, "msg_code", json_object_new_string(csPtr));
DEBUGLOG((" process_code = [%s]\n", csPtr));
		}
		else {
			iRet = PD_ERR;
DEBUGLOG((" ***process_code is missing\n"));
		}
	}

//version
	if (iRet == PD_OK) {
		if (GetField_CString(hIn, "version", &csPtr)) {
			json_object_object_add(jobj, "version", json_object_new_string(csPtr));
DEBUGLOG((" version = [%s]\n", csPtr));
		}
		else {
			iRet = PD_ERR;
DEBUGLOG((" ***version is missing\n"));
		}
	}

//enc_type
	if (iRet == PD_OK ) {
		if (GetField_CString(hIn, "enc_type", &csPtr)) {
			json_object_object_add(jobj, "enc_type", json_object_new_string(csPtr));
DEBUGLOG((" enc_type = [%s]\n", csPtr));
		}
		else {
			iRet = PD_ERR;
DEBUGLOG((" ***enc_type is missing\n"));
		}
	}

//mac
	if (iRet == PD_OK ) {
		if (GetField_CString(hIn, "mac", &csPtr)) {
			json_object_object_add(jobj, "sign", json_object_new_string(csPtr));
DEBUGLOG((" mac = [%s]\n", csPtr));
		}
		else {
			iRet = PD_ERR;
DEBUGLOG((" ***mac is missing\n"));
		}
	}

//info

	if (iRet == PD_OK ) {
//info:err_code
		if (GetField_CString(hIn, "response_code", &csPtr)) {
			json_object_object_add(new_jobj, "err_code", json_object_new_string(csPtr));
DEBUGLOG((" info.err_code = [%s]\n", csPtr));
		}
		else {
			iRet = PD_ERR;
DEBUGLOG((" info.err_code is missing\n"));
		}
	}

	if (iRet == PD_OK ) {
//info:prov_id
		if (GetField_CString(hIn, "prov_id", &csPtr)) {
			json_object_object_add(new_jobj, "prov_id", json_object_new_string(csPtr));
DEBUGLOG((" info.prov_id = [%s]\n", csPtr));
		}

//info:prov_name
		if (GetField_CString(hIn, "prov_name", &csPtr)) {
			json_object_object_add(new_jobj, "prov_name", json_object_new_string(csPtr));
DEBUGLOG((" info.prov_name = [%s]\n", csPtr));
		}

//info:pid_nature
                if (GetField_CString(hIn, "pid_nature", &csPtr)) {
                        json_object_object_add(new_jobj, "pid_nature", json_object_new_string(csPtr));
DEBUGLOG((" info.pid_nature = [%s]\n", csPtr));
                }

//info:baid
		if (GetField_CString(hIn, "baid", &csPtr)) {
			json_object_object_add(new_jobj, "baid", json_object_new_string(csPtr));
DEBUGLOG((" info.baid = [%s]\n", csPtr));
		}

//info:baid_status
		if (GetField_CString(hIn, "baid_status", &csPtr)) {
			json_object_object_add(new_jobj, "baid_status", json_object_new_string(csPtr));
DEBUGLOG((" info.baid_status = [%s]\n", csPtr));
		}

//info:bank_nature
		if (GetField_CString(hIn, "bank_nature", &csPtr)) {
			json_object_object_add(new_jobj, "bank_nature", json_object_new_string(csPtr));
DEBUGLOG((" info.bank_nature = [%s]\n", csPtr));
		}

//info:acct_status
                if (GetField_CString(hIn, "acct_status", &csPtr)) {
                        json_object_object_add(new_jobj, "acct_status", json_object_new_string(csPtr));
DEBUGLOG((" info.acct_status = [%s]\n", csPtr));
                }

//info:bank_code
		if (GetField_CString(hIn, "bank_code", &csPtr)) {
                        json_object_object_add(new_jobj, "bank_code", json_object_new_string(csPtr));
DEBUGLOG((" info.bank_code = [%s]\n", csPtr));
                }

//info:bank_name
		if (GetField_CString(hIn, "bank_name", &csPtr)) {
			json_object_object_add(new_jobj, "bank_name", json_object_new_string(csPtr));
DEBUGLOG((" info.bank_name = [%s]\n", csPtr));
		}

//info:bank_abbrev
                if (GetField_CString(hIn, "bank_abbrev", &csPtr)) {
                        json_object_object_add(new_jobj, "bank_abbrev", json_object_new_string(csPtr));
DEBUGLOG((" info.bank_abbrev = [%s]\n", csPtr));
                }

//info:acct_num
                if (GetField_CString(hIn, "acct_num", &csPtr)) {
                        json_object_object_add(new_jobj, "acct_num", json_object_new_string(csPtr));
DEBUGLOG((" info.acct_num = [%s]\n", csPtr));
                }

//info:acct_short_name
                if (GetField_CString(hIn, "acct_short_name", &csPtr)) {
                        json_object_object_add(new_jobj, "acct_short_name", json_object_new_string(csPtr));
DEBUGLOG((" info.acct_short_name = [%s]\n", csPtr));
                }

//info:owner_name
		if (GetField_CString(hIn, "owner_name", &csPtr)) {
			json_object_object_add(new_jobj, "owner_name", json_object_new_string(csPtr));
DEBUGLOG((" info.owner_name = [%s]\n", csPtr));
		}

//info:branch_name
		if (GetField_CString(hIn, "branch_name", &csPtr)) {
			json_object_object_add(new_jobj, "branch_name", json_object_new_string(csPtr));
DEBUGLOG((" info.branch_name = [%s]\n", csPtr));
		}

//info:province
                if (GetField_CString(hIn, "province", &csPtr)) {
                        json_object_object_add(new_jobj, "province", json_object_new_string(csPtr));
DEBUGLOG((" info.province = [%s]\n", csPtr));
                }

//info:city
                if (GetField_CString(hIn, "city", &csPtr)) {
                        json_object_object_add(new_jobj, "city", json_object_new_string(csPtr));
DEBUGLOG((" info.city = [%s]\n", csPtr));
                }

//info:reg_mob_no
                if (GetField_CString(hIn, "reg_mob_no", &csPtr)) {
                        json_object_object_add(new_jobj, "reg_mob_no", json_object_new_string(csPtr));
DEBUGLOG((" info.reg_mob_no = [%s]\n", csPtr));
                }

//info:merchant_id
		if (GetField_CString(hIn, "merchant_id", &csPtr)) {
			json_object_object_add(new_jobj, "merchant_id", json_object_new_string(csPtr));
DEBUGLOG((" info.merchant_id = [%s]\n", csPtr));
		}

//info:req_id
		if (GetField_CString(hIn, "req_id", &csPtr)) {
			json_object_object_add(new_jobj, "req_id", json_object_new_string(csPtr));
DEBUGLOG((" info.req_id = [%s]\n", csPtr));
		}

//info:to_baid
                if (GetField_CString(hIn, "to_baid", &csPtr)) {
                        json_object_object_add(new_jobj, "to_baid", json_object_new_string(csPtr));
DEBUGLOG((" info.to_baid = [%s]\n", csPtr));
                }

//info:txn_id
		if (GetField_CString(hIn, "txn_id", &csPtr)) {
			json_object_object_add(new_jobj, "txn_id", json_object_new_string(csPtr));
DEBUGLOG((" info.txn_id = [%s]\n", csPtr));
		}

//info:to_txn_id
		if (GetField_CString(hIn, "to_txn_id", &csPtr)) {
			json_object_object_add(new_jobj, "to_txn_id", json_object_new_string(csPtr));
DEBUGLOG((" info.to_txn_id = [%s]\n", csPtr));
		}

//info:r_list
		int     iCnt = 0;
		int	iTmp = 0;

		if (GetField_Int(hIn, "r_list", &iCnt)) {
DEBUGLOG((" info.r_list = [%d]\n", iCnt));
		}
		if(iCnt > 0){
			struct json_object *jdt, *json_new_array;
			json_new_array = json_object_new_array();

			int     i, iPtr;
			char    csTag[PD_TAG_LEN + 1];
			char	*csBuf;
			csBuf = (char*) malloc (PD_TMP_BUF_LEN);

			for(i = 0; i < iCnt; i++){
				jdt = json_object_new_object();
//info:r_list.dt_err_code
				sprintf(csTag, "r_list.%d.dt_err_code", i);
				if(GetField_Int(hIn, csTag, &iPtr)){
					sprintf(csBuf, "%d", iPtr);
					json_object_object_add(jdt, "dt_err_code", json_object_new_string(csBuf));
					iTmp ++;
				}
//info:r_list.prov_id
				sprintf(csTag, "r_list.%d.prov_id", i);
				if(GetField_CString(hIn, csTag, &csPtr)){
					json_object_object_add(jdt, "prov_id", json_object_new_string(csPtr));
					iTmp ++;
				}
//info:r_list.prov_name
				sprintf(csTag, "r_list.%d.prov_name", i);
				if(GetField_CString(hIn, csTag, &csPtr)){
					json_object_object_add(jdt, "prov_name", json_object_new_string(csPtr));
					iTmp ++;
				}
//info:r_list.pid_nature
                                sprintf(csTag, "r_list.%d.pid_nature", i);
                                if(GetField_CString(hIn, csTag, &csPtr)){
                                        json_object_object_add(jdt, "pid_nature", json_object_new_string(csPtr));
                                        iTmp ++;
                                }
//info:r_list.baid
                                sprintf(csTag, "r_list.%d.baid", i);
                                if(GetField_CString(hIn, csTag, &csPtr)){
                                        json_object_object_add(jdt, "baid", json_object_new_string(csPtr));
                                        iTmp ++;
                                }
//info:r_list.baid_status
                                sprintf(csTag, "r_list.%d.baid_status", i);
                                if(GetField_CString(hIn, csTag, &csPtr)){
                                        json_object_object_add(jdt, "baid_status", json_object_new_string(csPtr));
                                        iTmp ++;
                                }
//info:r_list.baid_upd_ts
                                sprintf(csTag, "r_list.%d.baid_upd_ts", i);
                                if(GetField_CString(hIn, csTag, &csPtr)){
                                        json_object_object_add(jdt, "baid_upd_ts", json_object_new_string(csPtr));
                                        iTmp ++;
                                }
//info:r_list.country
				sprintf(csTag, "r_list.%d.country", i);
				if(GetField_CString(hIn, csTag, &csPtr)){
					json_object_object_add(jdt, "country", json_object_new_string(csPtr));
					iTmp ++;
				}
//info:r_list.currency
				sprintf(csTag, "r_list.%d.currency", i);
				if(GetField_CString(hIn, csTag, &csPtr)){
					json_object_object_add(jdt, "currency", json_object_new_string(csPtr));
					iTmp ++;
				}
//info:r_list.baid_bal
                                sprintf(csTag, "r_list.%d.baid_bal", i);
                                if(GetField_CString(hIn, csTag, &csPtr)){
                                        json_object_object_add(jdt, "baid_bal", json_object_new_string(csPtr));
                                        iTmp ++;
                                }
//info:r_list.acct_bal
                                sprintf(csTag, "r_list.%d.acct_bal", i);
                                if(GetField_CString(hIn, csTag, &csPtr)){
                                        json_object_object_add(jdt, "acct_bal", json_object_new_string(csPtr));
                                        iTmp ++;
                                }
//info:r_list.bank_nature
				sprintf(csTag, "r_list.%d.bank_nature", i);
				if(GetField_CString(hIn, csTag, &csPtr)){
					json_object_object_add(jdt, "bank_nature", json_object_new_string(csPtr));
					iTmp ++;
				}
//info:r_list.acct_status
                                sprintf(csTag, "r_list.%d.acct_status", i);
                                if(GetField_CString(hIn, csTag, &csPtr)){
                                        json_object_object_add(jdt, "acct_status", json_object_new_string(csPtr));
                                        iTmp ++;
                                }
//info.r_list.bank_code
				sprintf(csTag, "r_list.%d.bank_code", i);
                                if(GetField_CString(hIn, csTag, &csPtr)){
                                        json_object_object_add(jdt, "bank_code", json_object_new_string(csPtr));
                                        iTmp ++;
                                }
//info:r_list.bank_name
				sprintf(csTag, "r_list.%d.bank_name", i);
				if(GetField_CString(hIn, csTag, &csPtr)){
					json_object_object_add(jdt, "bank_name", json_object_new_string(csPtr));
					iTmp ++;
				}
//info:r_list.bank_abbrev
                                sprintf(csTag, "r_list.%d.bank_abbrev", i);
                                if(GetField_CString(hIn, csTag, &csPtr)){
                                        json_object_object_add(jdt, "bank_abbrev", json_object_new_string(csPtr));
                                        iTmp ++;
                                }
//info:r_list.acct_num
                                sprintf(csTag, "r_list.%d.acct_num", i);
                                if(GetField_CString(hIn, csTag, &csPtr)){
                                        json_object_object_add(jdt, "acct_num", json_object_new_string(csPtr));
                                        iTmp ++;
                                }
//info:r_list.acct_short_name
                                sprintf(csTag, "r_list.%d.acct_short_name", i);
                                if(GetField_CString(hIn, csTag, &csPtr)){
                                        json_object_object_add(jdt, "acct_short_name", json_object_new_string(csPtr));
                                        iTmp ++;
                                }
//info:r_list.owner_name
				sprintf(csTag, "r_list.%d.owner_name", i);
				if(GetField_CString(hIn, csTag, &csPtr)){
					json_object_object_add(jdt, "owner_name", json_object_new_string(csPtr));
					iTmp ++;
				}
//info:r_list.branch_name
				sprintf(csTag, "r_list.%d.branch_name", i);
				if(GetField_CString(hIn, csTag, &csPtr)){
					json_object_object_add(jdt, "branch_name", json_object_new_string(csPtr));
					iTmp ++;
				}
//info:r_list.province
                                sprintf(csTag, "r_list.%d.province", i);
                                if(GetField_CString(hIn, csTag, &csPtr)){
                                        json_object_object_add(jdt, "province", json_object_new_string(csPtr));
                                        iTmp ++;
                                }
//info:r_list.city
                                sprintf(csTag, "r_list.%d.city", i);
                                if(GetField_CString(hIn, csTag, &csPtr)){
                                        json_object_object_add(jdt, "city", json_object_new_string(csPtr));
                                        iTmp ++;
                                }
//info:r_list.reg_mob_no
                                sprintf(csTag, "r_list.%d.reg_mob_no", i);
                                if(GetField_CString(hIn, csTag, &csPtr)){
                                        json_object_object_add(jdt, "reg_mob_no", json_object_new_string(csPtr));
                                        iTmp ++;
                                }
//info:r_list.bank_upd_ts
                                sprintf(csTag, "r_list.%d.bank_upd_ts", i);
                                if(GetField_CString(hIn, csTag, &csPtr)){
                                        json_object_object_add(jdt, "bank_upd_ts", json_object_new_string(csPtr));
                                        iTmp ++;
                                }
//info:r_list.merchant_id
				sprintf(csTag, "r_list.%d.merchant_id", i);
				if(GetField_CString(hIn, csTag, &csPtr)){
					json_object_object_add(jdt, "merchant_id", json_object_new_string(csPtr));
					iTmp ++;
				}
//info:r_list.create_dt
				sprintf(csTag, "r_list.%d.create_dt", i);
                                if(GetField_CString(hIn, csTag, &csPtr)){
                                        json_object_object_add(jdt, "create_dt", json_object_new_string(csPtr));
                                        iTmp ++;
                                }				
//info:r_list.rec_end_ind	
				sprintf(csTag, "r_list.%d.rec_end_ind", i);
                                if(GetField_CString(hIn, csTag, &csPtr)){
                                        json_object_object_add(jdt, "rec_end_ind", json_object_new_string(csPtr));
                                        iTmp ++;
                                }

				if(iTmp > 0) json_object_array_add(json_new_array, jdt);
			}

			json_object_object_add(new_jobj, "r_list", json_new_array);

			FREE_ME(csBuf);
		}

	}

//add sub obj to obj
	json_object_object_add(jobj, MY_API_INFO_OBJ , new_jobj);
	strcpy((char*)csData, str_replace((char*)(json_object_to_json_string_ext(jobj, JSON_C_TO_STRING_PLAIN)), "\\/", "/"));
	strcpy((char*)outMsg, csData);

	*outLen = strlen((const char*)outMsg);
DEBUGLOG((" outmsg = [%s][%d]\n", outMsg, *outLen));

	json_object_put(jobj);
DEBUGLOG(("FormatMsg() Exit iRet = [%d]\n",iRet));
        return  iRet;
}


int BreakDownMsg(hash_t *hOut,const unsigned char *inMsg,int inLen)
{
	int	iRet = PD_OK;
	struct	json_object *jobj;
	
	char	*csPtr;
	int     iTmp;

	hash_t  *hRec;

	hRec = (hash_t*)  malloc (sizeof(hash_t));
	hash_init(hRec,0);

DEBUGLOG(("BreakDownMsg()\n"));
DEBUGLOG((" inmsg = [%s][%d]\n",inMsg,inLen));

	jobj = json_tokener_parse((const char *)inMsg);
DEBUGLOG(("BreakDownMsg:: parsed\n"));
	iRet = ParseJ(jobj, hRec);
DEBUGLOG(("BreakDownMsg:: json_parsed\n"));
        json_object_put(jobj);
DEBUGLOG(("BreakDownMsg:: json_put\n"));

	if (iRet == PD_OK) {
// msg_type
		if (GetField_CString(hRec, "msg_type", &csPtr)) {
			PutField_CString(hOut, "process_type", csPtr);
DEBUGLOG(("BreakDownMsg:: process_type = [%s]\n", csPtr));
		}
		else {
DEBUGLOG(("BreakDownMsg:: process_type not found\n"));
		}

// msg_code
		if (GetField_CString(hRec, "msg_code", &csPtr)) {
			PutField_CString(hOut, "process_code", csPtr);
DEBUGLOG(("BreakDownMsg:: process_code = [%s]\n", csPtr));
		}
		else {
DEBUGLOG(("BreakDownMsg:: process_code not found\n"));
		}

// version
		if (GetField_CString(hRec, "version", &csPtr)) {
			PutField_CString(hOut, "version", csPtr);
DEBUGLOG(("BreakDownMsg:: version = [%s]\n", csPtr));
		}
		else {
DEBUGLOG(("BreakDownMsg:: version not found\n"));
		}

// enc_type
		if (GetField_CString(hRec, "enc_type", &csPtr)) {
			PutField_CString(hOut, "enc_type", csPtr);
DEBUGLOG(("BreakDownMsg:: enc_type = [%s]\n", csPtr));
		}
		else {
DEBUGLOG(("BreakDownMsg:: enc_type not found\n"));
		}

// sign
		if (GetField_CString(hRec, "sign", &csPtr)) {
			PutField_CString(hOut, "mac", csPtr);
DEBUGLOG(("BreakDownMsg:: sign = [%s]\n", csPtr));
		}
		else {
DEBUGLOG(("BreakDownMsg:: sign not found\n"));
		}

// info

// info.baid
		if (GetField_CString(hRec, "baid", &csPtr)) {
			PutField_CString(hOut, "baid", csPtr);
DEBUGLOG(("BreakDownMsg:: info.baid = [%s]\n", csPtr));
		}

// info.req_id
		if (GetField_CString(hRec, "req_id", &csPtr)) {
			PutField_CString(hOut, "req_ref", csPtr);
			PutField_CString(hOut, "req_id", csPtr);
DEBUGLOG(("BreakDownMsg:: info.req_id = [%s]\n", csPtr));
		}

// info.to_baid
		if (GetField_CString(hRec, "to_baid", &csPtr)) {
			PutField_CString(hOut, "to_baid", csPtr);
DEBUGLOG(("BreakDownMsg:: info.to_baid = [%s]\n", csPtr));
		}

// info.amount
		if (GetField_CString(hRec, "amount", &csPtr)) {
			PutField_CString(hOut, "txn_amt", csPtr);
			PutField_CString(hOut, "amount", csPtr);
DEBUGLOG(("BreakDownMsg:: info.amount = [%s]\n", csPtr));
		}

// info.currency
		if (GetField_CString(hRec, "currency", &csPtr)) {
			PutField_CString(hOut, "txn_ccy", csPtr);
			PutField_CString(hOut, "currency", csPtr);
DEBUGLOG(("BreakDownMsg:: info.currency = [%s]\n", csPtr));
		}

// info.bank_nature
		if (GetField_CString(hRec, "bank_nature", &csPtr)) {
			PutField_CString(hOut, "bank_nature", csPtr);
DEBUGLOG(("BreakDownMsg:: info.bank_nature = [%s]\n", csPtr));
		}

// info.rec_date
		if (GetField_CString(hRec, "rec_date", &csPtr)) {
                        PutField_CString(hOut, "rec_date", csPtr);
DEBUGLOG(("BreakDownMsg:: info.rec_date = [%s]\n", csPtr));
                }

// info.auto_upload_ind
                if (GetField_CString(hRec, "auto_upload_ind", &csPtr)) {
                        PutField_CString(hOut, "auto_upload_ind", csPtr);
DEBUGLOG(("BreakDownMsg:: info.auto_upload_ind = [%s]\n", csPtr));
                }

// info.bal_over_ind
                if (GetField_CString(hRec, "bal_over_ind", &csPtr)) {
                        PutField_CString(hOut, "bal_over_ind", csPtr);
DEBUGLOG(("BreakDownMsg:: info.bal_over_ind = [%s]\n", csPtr));
                }

// info.req_dt
                if (GetField_CString(hRec, "req_dt", &csPtr)) {
                        PutField_CString(hOut, "req_dt", csPtr);
DEBUGLOG(("BreakDownMsg:: info.req_dt = [%s]\n", csPtr));
                }

// info.bank_code
		if (GetField_CString(hRec, "bank_code", &csPtr)) {
                        PutField_CString(hOut, "bank_code", csPtr);
DEBUGLOG(("BreakDownMsg:: info.bank_code = [%s]\n", csPtr));
                }

// info.acct_num
		if (GetField_CString(hRec, "acct_num", &csPtr)) {
                        PutField_CString(hOut, "acct_num", csPtr);
DEBUGLOG(("BreakDownMsg:: info.acct_num = [%s]\n", csPtr));
                }

// info.p_list
		if (GetField_Int(hRec, "p_list", &iTmp)) {
DEBUGLOG(("BreakDownMsg:: info.p_list = [%d]\n", iTmp));
			PutField_Int(hOut, "p_list", iTmp);

//info.p_list.#.baid
			char    csKey[PD_TMP_BUF_LEN +1];	
			int 	i = 0;

			for (i= 0; i < iTmp; i++) {
				sprintf(csKey, "p_list.%d.baid", i);

				if (GetField_CString(hRec, csKey, &csPtr)) {
					PutField_CString(hOut, csKey, csPtr);
DEBUGLOG(("BreakDownMsg:: info.[%s]= [%s]\n", csKey, csPtr));

				}
			}
		}

	} 

	if (iRet == PD_OK) {
		iRet = BuildAuthData(hOut);
DEBUGLOG(("BreakDownMsg:: return from BuildAuthData = [%d]\n",iRet));
	}

        hash_destroy(hRec);
        FREE_ME(hRec);

DEBUGLOG(("BreakDownMsg() Exit iRet = [%d]\n", iRet));

	return iRet;
}


int initReplyFromRequest(const hash_t* hIn, hash_t* hOut)
{
        int     iRet = PD_OK;
        char    *csPtr = NULL;

DEBUGLOG(("initReplyFromRequest ()\n"));

//msg_code
	if (GetField_CString(hIn, "process_code", &csPtr)) {
		PutField_CString(hOut, "msg_code", csPtr);
DEBUGLOG(("initReplyFromRequest:: msg_code = [%s]\n", csPtr));
	}

//version
	if (GetField_CString(hIn, "version", &csPtr)) {
		PutField_CString(hOut, "version", csPtr);
DEBUGLOG(("initReplyFromRequest:: version = [%s]\n", csPtr));
	}

//enc_type
	if (GetField_CString(hIn, "enc_type", &csPtr)) {
		PutField_CString(hOut, "enc_type", csPtr);
DEBUGLOG(("initReplyFromRequest:: enc_type = [%s]\n", csPtr));
	}

DEBUGLOG(("initReplyFromRequest() Exit [%d]\n", iRet));
        return iRet;
}


int BuildAuthData(hash_t* hIn)
{
        int     iRet = PD_OK;
        int     iDtlRet = PD_FOUND;
	int     i = 0;
	int     iSeq = 0;
	int     iPtr = 0;
        int     iTmp = 0;

	char    *csVersion = NULL;
        char    *csPtr = NULL;
	char    *csTagName = NULL;
	char    *csData;
	char    *csBuf;

	char    csKey[PD_TMP_BUF_LEN + 1];

	hash_t *hRec;
        recordset_t     *rRecordSet;
        rRecordSet = (recordset_t*) malloc (sizeof(recordset_t));
        recordset_init(rRecordSet,0);

DEBUGLOG(("BuildAuthData ()\n"));

        csData = (char*) malloc (PD_TMP_MSG_BUF_LEN+1);
	memset(csData, 0, sizeof(csData));
	csData[0] = '\0';

	csBuf = (char*) malloc (PD_TMP_MSG_BUF_LEN + 1);
        memset(csBuf, 0, sizeof(csBuf));
        csBuf[0] = '\0';

/* version */
	if (iRet == PD_OK) {
        	if (GetField_CString(hIn, "version", &csVersion)) {
DEBUGLOG(("BuildAuthData:: version = [%s]\n", csVersion));
        	} else {
DEBUGLOG(("BuildAuthData:: version not found!!\n"));
ERRLOG("OiaMsg:: BuildAuthData:: version not found!!\n");
                	iRet = PD_ERR;
		}
	}

	// Get Auth Param
	if (iRet == PD_OK) {
DEBUGLOG(("BuildAuthData:: Call DBOLSignAuthParam:: GetParam\n"));
        	DBObjPtr = CreateObj(DBPtr,"DBOLSignAuthParam","GetParam");
        	iDtlRet = (unsigned long)(DBObjPtr)(PD_CHANNEL_OIA, PD_DEFAULT_TXN_CODE, csVersion, PD_TXN_REQUEST, rRecordSet);
		if (iDtlRet == PD_FOUND) {
//DEBUGLOG(("BuildAuthData:: Call DBOLSignAuthParam:: GetParam Found!!\n"));
		} else if (iDtlRet == PD_NOT_FOUND) {
DEBUGLOG(("BuildAuthData:: Call DBOLSignAuthParam:: GetParam Not Found!!\n"));
		} else {
DEBUGLOG(("BuildAuthData:: Call DBOLSignAuthParam:: GetParam Failure!!\n"));
ERRLOG("OiaMsg:: BuildAuthData:: Call DBOLSignAuthParam:: GetParam Failure!!\n");
			iRet = PD_ERR;
		}
	}

	// Build Auth Data
	if (iRet == PD_OK) {
	
		if (GetField_Int(hIn, "p_list", &iTmp)) {
DEBUGLOG(("BuildAuthData:: List [%d]\n", iTmp));
	
			for (i=0; i < iTmp; i++) {

				hRec = RecordSet_GetFirst(rRecordSet);
                		while (hRec && iRet == PD_OK) {

                	        	if((GetField_Int(hRec, "seq", &iSeq))
                        	   	   &&(GetField_CString(hRec, "tag_name", &csTagName))
                        		)
                        		{
                                		// Integer
                                		sprintf(csKey, "p_list.%d.%s", i, csTagName);
                                		if(GetField_Int(hIn, csKey, &iPtr)){
                                	        	sprintf(csBuf, "%d", iPtr);
                                	        	strcat((char*)csData, csBuf);
DEBUGLOG(("BuildAuthData:: [%d] %d - %s (array) = [%s]\n", i, iSeq, csTagName, csBuf));
                                		}

                                		// CString
                                		sprintf(csKey, "p_list.%d.%s", i, csTagName);
                                		if(GetField_CString(hIn, csKey, &csPtr)){
                                	        	strcat((char*)csData, csPtr);
DEBUGLOG(("BuildAuthData:: [%d] %d - %s (array) = [%s]\n", i, iSeq, csTagName, csPtr));
                                		}
                        		}

                        		hRec = RecordSet_GetNext(rRecordSet);
                		}
			}
		} else {
	
			hRec = RecordSet_GetFirst(rRecordSet);
                	while (hRec && iRet == PD_OK) {

                        	if((GetField_Int(hRec, "seq", &iSeq))
                           	   &&(GetField_CString(hRec, "tag_name", &csTagName))
                        	)
                        	{
                                	// Integer
                                	if(GetField_Int(hIn, csTagName, &iPtr)){
                                        	sprintf(csBuf, "%d", iPtr);
                                        	strcat((char*)csData, csBuf);
DEBUGLOG(("BuildAuthData:: %d - %s = [%s]\n", iSeq, csTagName, csBuf));
                                	}

                                	// CString
                                	if(GetField_CString(hIn, csTagName, &csPtr)){
                                        	strcat((char*)csData, csPtr);
DEBUGLOG(("BuildAuthData:: %d - %s = [%s]\n", iSeq, csTagName, csPtr));
                                	}
                        	}

                        	hRec = RecordSet_GetNext(rRecordSet);
                	}	
		}

        	PutField_CString(hIn,"auth_data",csData);
DEBUGLOG(("BuildAuthData:: auth_data= [%s]\n",csData));

	}

	RecordSet_Destroy(rRecordSet);
        FREE_ME(rRecordSet);

        FREE_ME(csData);
	FREE_ME(csBuf);

DEBUGLOG(("BuildAuthData() Exit iRet = [%d]\n", iRet));
        return iRet;
}


int BuildRspAuthData(hash_t *hIn)
{
	int	iRet = PD_OK;
	int	iDtlRet = PD_FOUND;
	int     i = 0;
        int     iSeq = 0;
	int     iPtr = 0;
	int     iTmp = 0;

	char    *csVersion = NULL;
	char    *csPtr = NULL;
	char    *csTagName = NULL;
	char    *csData;
	char	*csBuf;

	char	csKey[PD_TMP_BUF_LEN + 1];

	hash_t *hRec;
	recordset_t     *rRecordSet;
        rRecordSet = (recordset_t*) malloc (sizeof(recordset_t));
        recordset_init(rRecordSet,0);

DEBUGLOG(("BuildRspAuthData()\n"));

	csData = (char*) malloc (PD_TMP_MSG_BUF_LEN + 1);
	memset(csData, 0, sizeof(csData));
	csData[0] = '\0';

	csBuf = (char*) malloc (PD_TMP_MSG_BUF_LEN + 1);
	memset(csBuf, 0, sizeof(csBuf));
	csBuf[0] = '\0';

/* version */
	if (iRet == PD_OK) {
        	if (GetField_CString(hIn, "version", &csVersion)) {
DEBUGLOG(("BuildRspAuthData:: version = [%s]\n", csVersion));
        	} else {
DEBUGLOG(("BuildRspAuthData:: version not found!!\n"));
ERRLOG("OiaMsg:: BuildRspAuthData:: version not found!!\n");
        	        iRet = PD_ERR;
		}
	}

	// Get Auth Param
	if (iRet == PD_OK) {
DEBUGLOG(("BuildRspAuthData:: Call DBOLSignAuthParam:: GetParam\n"));                       
		DBObjPtr = CreateObj(DBPtr,"DBOLSignAuthParam","GetParam");
    		iDtlRet = (unsigned long)(DBObjPtr)(PD_CHANNEL_OIA, PD_DEFAULT_TXN_CODE, csVersion, PD_TXN_RESPONSE, rRecordSet);
		if (iDtlRet == PD_FOUND) {
//DEBUGLOG(("BuildRspAuthData:: Call DBOLSignAuthParam:: GetParam Found!!\n"));
		} else if (iDtlRet == PD_NOT_FOUND) {
DEBUGLOG(("BuildRspAuthData:: Call DBOLSignAuthParam:: GetParam Not Found!!\n"));
		} else {
DEBUGLOG(("BuildRspAuthData:: Call DBOLSignAuthParam:: GetParam Failure!!\n"));
ERRLOG("OiaMsg:: BuildRspAuthData:: Call DBOLSignAuthParam:: GetParam Failure!!\n");
			iRet = PD_ERR;
		}
	}			
	
	// Build Auth Data
	if (iRet == PD_OK) {

		hRec = RecordSet_GetFirst(rRecordSet);
           	while (hRec && iRet == PD_OK) {	

			if((GetField_Int(hRec, "seq", &iSeq))
			   &&(GetField_CString(hRec, "tag_name", &csTagName))
			)
			{
				// Integer
				if(GetField_Int(hIn, csTagName, &iPtr)){
                                        sprintf(csBuf, "%d", iPtr);
                                        strcat((char*)csData, csBuf);
DEBUGLOG(("BuildRspAuthData:: %d - %s = [%s]\n", iSeq, csTagName, csBuf));
                                }

				// CString
				if(GetField_CString(hIn, csTagName, &csPtr)){
			
					strcat((char*)csData, csPtr);		
DEBUGLOG(("BuildRspAuthData:: %d - %s = [%s]\n", iSeq, csTagName, csPtr));	
				}
			}

			hRec = RecordSet_GetNext(rRecordSet);
		}

		if (iRet == PD_OK) {

			if (GetField_Int(hIn, "r_list", &iTmp)) {
DEBUGLOG(("BuildRspAuthData List [%d]\n", iTmp));
			}

			for (i = 0; i < iTmp; i++) {
	
				hRec = RecordSet_GetFirst(rRecordSet);
                		while (hRec && iRet == PD_OK) {

                        		if((GetField_Int(hRec, "seq", &iSeq))
                           		   &&(GetField_CString(hRec, "tag_name", &csTagName))
                        		)
                        		{
						// Interger
						sprintf(csKey, "r_list.%d.%s", i, csTagName);
                                		if(GetField_Int(hIn, csKey, &iPtr)){
                                        		sprintf(csBuf, "%d", iPtr);
                                        		strcat((char*)csData, csBuf);
DEBUGLOG(("BuildRspAuthData:: [%d] %d - %s (array) = [%s]\n", i, iSeq, csTagName, csBuf));
                                		}

						// CString
						sprintf(csKey, "r_list.%d.%s", i, csTagName);
                                		if(GetField_CString(hIn, csKey, &csPtr)){
                                        		strcat((char*)csData, csPtr);
DEBUGLOG(("BuildRspAuthData:: [%d] %d - %s (array) = [%s]\n", i, iSeq, csTagName, csPtr));
                                		}
                        		}

                        		hRec = RecordSet_GetNext(rRecordSet);
                		}
			}
		}

		PutField_CString(hIn, "auth_data", csData);
DEBUGLOG(("BuildRspAuthData:: auth_data = [%s]\n", csData));

	}

	RecordSet_Destroy(rRecordSet);
        FREE_ME(rRecordSet);

	FREE_ME(csData);
	FREE_ME(csBuf);

DEBUGLOG(("BuildRspAuthData() Exit iRet = [%d]\n", iRet));
	return iRet;
}


char *str_replace(char *orig, char *rep, char *with) {
        char *result;   // the return string
        char *ins;      // the next insert point
        char *tmp;      // varies
        int len_rep;    // length of rep (the string to remove)
        int len_with;   // length of with (the string to replace rep with)
        int len_front;  // distance between rep and end of last rep
        int count;      // number of replacements

        if (!orig || !rep)
                return NULL;
        len_rep = strlen(rep);
        if (len_rep == 0)
                return NULL; // empty rep causes infinite loop during count
        if (!with)
                with = "";
        len_with = strlen(with);

        // count the number of replacements needed
        ins = orig;
        for (count = 0; (tmp = strstr(ins, rep)); ++count) {
                ins = tmp + len_rep;
        }

        tmp = result = malloc(strlen(orig) + (len_with - len_rep) * count + 1);

        if (!result)
                return NULL;

        // first time through the loop, all the variable are set correctly
        // from here on,
        // tmp points to the end of the result string
        // ins points to the next occurrence of rep in orig
        // orig points to the remainder of orig after "end of rep"

        while (count--) {
                ins = strstr(orig, rep);
                len_front = ins - orig;
                tmp = strncpy(tmp, orig, len_front) + len_front;
                tmp = strcpy(tmp, with) + len_with;
                orig += len_front + len_rep; // move to next "end of rep"
        }
        strcpy(tmp, orig);
        return result;
}

