...
-------------------------------                    ------------            --------------
Init Version					   2013/03/13              Cody Chan   
New HaiPay (FL)
*/
...
/* merchant_id */
		if (GetField_CString(hIn,"psp_merchant_id",&csTmp)) {
			...
			strcat(csTmpData,MY_HPAY_FIELD_TOKEN);
			strcat(csTmpData,csTmp);
			strcat(csTmpData,MY_HPAY_TOKEN);
...
DEBUGLOG(("FormatMsg:: Tmp DATA = [%s]\n",csTmpData));
		if (GetField_CString(hIn,"psp_id",&csPspID)) {
			...
			iRet = (unsigned long)(*BOObjPtr)(csTmpData,csPspID,csResult);	
			if (iRet == PD_OK) {
...
DEBUGLOG(("FormatMsg:: Msg = [%s]\n",csBuf));
			...
			iRet = PD_ERR;
DEBUGLOG(("FormatMsg:: psp_id not found\n"));
ERRLOG("HaiMsg::FormatMsg psp_id not found\n");
		}
...
DEBUGLOG(("DATA = [%s][%d]\n",inMsg,inLen));
	...
	if (Str2Cls(hRec,(const char *)inMsg,MY_HPAY_TOKEN, MY_HPAY_FIELD_TOKEN) == PD_OK) {
...
/* data */
		if (GetField_CString(hRec,"Data",&csPtr)) {
                	iRet = DEBlockHpayData(hOut,(const unsigned char *)csPtr,strlen(csPtr));
        	}
		else {
			if (GetField_CString(hRec,"DATA",&csPtr)) {
                		iRet = DEBlockHpayData(hOut,(const unsigned char *)csPtr,strlen(csPtr));
...
ERRLOG("HaiMsg:BuildData unsupport CCY ISO to HIPAY\n");
		}
        }
	else {
...
DEBUGLOG(("iRet = [%d] DATA = [%s]\n",iRet,csDATA));
		...
		if (!strncmp(csDATA,MY_HPAY_DATA_TOKEN,1))
