...
DEBUGLOG(("handle_data length = [%d] tmp = [%s]\n", length, tmp));
	isNull = PD_FALSE;
	data = (void *) tmp;
DEBUGLOG(("handle_data iIsStart = [%d]\n", iIsStart));
	if (!iIsStart) {
		char *tmp2 = malloc(length+strlen(last_content)+1);
		tmp2[0] = '\0';
		strcat(tmp2, last_content);
		strcat(tmp2, tmp);
		tmp2[length+strlen(last_content)] = '\0';
		last_content = tmp2;
	} else {
		last_content = tmp;
	}
...
ERRLOG("DpqXml::BreakDownXMLMsg %s at line %d\n", XML_ErrorString(XML_GetErrorCode(parser)), XML_GetCurrentLineNumber(parser));
