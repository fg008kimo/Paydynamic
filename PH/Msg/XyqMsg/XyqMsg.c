...
DEBUGLOG(("return_code = [%s]\n", csPtr));
		...
		strcat((char*)outMsg, MY_XYQ_FIELD_TOKEN);
		if (!strcmp(csPtr, MY_XYQ_SUCCESS_STATUS))
			strcat((char*)outMsg, "SUCCESS");
		else
			strcat((char*)outMsg, csPtr);
		strcat((char*)outMsg, MY_XYQ_TOKEN);
	} else {
...
DEBUGLOG(("Currency = [%s]\n", csPtr));
			...
			strcat((char*)csBuf, MY_XYQ_CCY_CODE);
...
DEBUGLOG(("request_function = [%s]\n", csPtr));
			strcat((char*)csBuf, MY_XYQ_FIELD_TOKEN);
...
DEBUGLOG(("psp_url = [%s]\n", csBuf));
		...
		strcat((char*)csBuf, MY_XYQ_TOKEN);
...
DEBUGLOG(("Country (default) = [%s]\n", MY_XYQ_COUNTRY));
	json_object_object_add(jobj, "Country", json_object_new_string(MY_XYQ_COUNTRY));

// Pay_Code
DEBUGLOG(("Pay_Code (default) = [%s]\n", MY_XYQ_PAYCODE_QR));
	json_object_object_add(jobj, "Pay_Code", json_object_new_string(MY_XYQ_PAYCODE_QR));
...
DEBUGLOG(("Currency (default) = [%s]\n", MY_XYQ_CCY_CODE));
		json_object_object_add(jobj, "Currency", json_object_new_string(MY_XYQ_CCY_CODE));
	}
...
DEBUGLOG(("Verno (default) = [%s]\n", MY_XYQ_VERSION));
	json_object_object_add(jobj, "Verno", json_object_new_string(MY_XYQ_VERSION));
...
DEBUGLOG(("Error_Msg:return_msg = [%s]\n", csPtr));
			...
			if (!strcmp(csReturnCode, MY_XYQ_SUCCESS_STATUS)) {
				// Merchant_ID
				if (GetField_CString(hRec, "Merchant_ID", &csPtr)) {
					PutField_CString(hOut, "psp_merchant_id", csPtr);
...
DEBUGLOG(("***psp_merchant_id is missing\n"));
	...
	// Verno
DEBUGLOG(("Verno = [%s]\n", MY_XYQ_VERSION));
	strcat((char*)csBuf, MY_XYQ_VERSION);
