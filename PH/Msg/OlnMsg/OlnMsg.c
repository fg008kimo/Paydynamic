...
-------------------------------                    ------------            --------------
Init Version                                       2013/08/12              Cody Chan
Add Method 2					   2014/03/05		   Cody Chan
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
DEBUGLOG(("FormatMsg (INI):: process_type = [%s]\n",csPtr));
                                ...
                                strcat((char*)outMsg,MY_OLN_FIELD_TOKEN);
                                strcat((char*)outMsg,csPtr);
                                strcat((char*)outMsg,MY_OLN_TOKEN);
                        }
...
DEBUGLOG(("DATA = [%s][%d]\n",inMsg,inLen));
        ...
        if (Str2Cls(hRec,(char*)inMsg,MY_OLN_TOKEN, MY_OLN_FIELD_TOKEN) == PD_OK) {	
...
/* encrypted order_num */
		if (GetField_CString(hRec,PD_SESSION_ID,&csPtr)) {
                        PutField_CString(hOut,"org_encrypted_txn_seq",csPtr);
DEBUGLOG(("BreakDownMsg:org_encrypted_txn_seq = [%s]\n",csPtr));
                }
/* verno */
...
/* pay_method */
		if (GetField_CString(hRec,"pay_method",&csPtr)) {
			PutField_CString(hOut,"pay_method",csPtr);
...
/* deposit_flow */
		if (GetField_CString(hRec,"deposit_flow",&csPtr)) {
                        PutField_Char(hOut,"deposit_flow",csPtr[0]);
DEBUGLOG(("BreakDownMsg:: deposit_flow = [%c]\n",csPtr[0]));
		}
/* cust_deposit_datetime */
		if (GetField_CString(hRec,"cust_deposit_date",&csPtr)) {
                        PutField_CString(hOut,"cust_deposit_datetime",csPtr);
DEBUGLOG(("BreakDownMsg:: cust_deposit_datetime = [%s]\n",csPtr));
                }
...
/* cust_tag */
		if (GetField_CString(hRec,"cust_tag",&csPtr)) {
                        PutField_CString(hOut,"cust_tag",csPtr);
DEBUGLOG(("BreakDownMsg:: cust_tag = [%s]\n",csPtr));
                }

/* echo_msg_1 */
		if (GetField_CString(hRec,"echo_msg1",&csPtr)) {
                        PutField_CString(hOut,"echo_msg_1",csPtr);
DEBUGLOG(("BreakDownMsg:: echo_msg_1 = [%s]\n",csPtr));
                }

/* echo_msg_2 */
		if (GetField_CString(hRec,"echo_msg2",&csPtr)) {
                        PutField_CString(hOut,"echo_msg_2",csPtr);
DEBUGLOG(("BreakDownMsg:: echo_msg_2 = [%s]\n",csPtr));
                }
/* echo_msg_3 */
		if (GetField_CString(hRec,"echo_msg3",&csPtr)) {
...
DEBUGLOG(("BreakDownMsg:user_agent = [%s]\n",csDst));
                        PutField_CString(hOut,"user_agent",(const char*)csDst);
                }
...
DEBUGLOG(("FormatAckMsg()\n"));
        ...
        *outLen = 0;
        ...
        strcat((char*)csBuf,MY_OLN_FIELD_TOKEN);
...
DEBUGLOG(("FormatAckMsg failure_callback_url = [%s]\n",csPtr));
        ...
        strcat((char*)csBuf,MY_OLN_TOKEN);
...
/* pay_amt*/
	/*if (GetField_Double(hIn, "display_amt", &dTmp)) */
	if (GetField_Double(hIn, "deposit_amt", &dTmp)) {
DEBUGLOG(("FormatAckMsg:: pay amt = [%f]\n",dTmp));
                sprintf((char*)csTmp,"%ld",double2long(dTmp));
DEBUGLOG(("FormatAckMsg:: pay amt = [%s]\n",csTmp));
...
DEBUGLOG(("FormatAckMsg:: pay_amt is missing!!!\n"));
        }

/* pay_ccy */
        if (GetField_CString(hIn,"display_ccy",&csPtr)){
DEBUGLOG(("FormatAckMsg:: pay_ccy = [%s]\n",csPtr));
                strcat((char*)outMsg,"pay_ccy");
                strcat((char*)outMsg,MY_OLN_FIELD_TOKEN);
                strcat((char*)outMsg,csPtr);
                strcat((char*)outMsg,MY_OLN_TOKEN);
        }
        else {
DEBUGLOG(("FormatAckMsg:: pay_ccy is missing!!!\n"));
        }

/* deposit_amt*/
	if (GetField_Double(hIn, "deposit_amt", &dTmp)) {
DEBUGLOG(("FormatAckMsg:: deposit amt = [%f]\n",dTmp));
                sprintf((char*)csTmp,"%ld",double2long(dTmp));
DEBUGLOG(("FormatAckMsg:: deposit amt = [%s]\n",csTmp));
                strcat((char*)outMsg,"deposit_amt");
                strcat((char*)outMsg,MY_OLN_FIELD_TOKEN);
                strcat((char*)outMsg,(char*)csTmp);
                strcat((char*)outMsg,MY_OLN_TOKEN);
        }
        else {
DEBUGLOG(("FormatAckMsg:: deposit_amt is missing!!!\n"));
        }

/* deposit_ccy*/
        if (GetField_CString(hIn,"deposit_ccy",&csPtr)){
DEBUGLOG(("FormatAckMsg:: deposit_ccy = [%s]\n",csPtr));
                strcat((char*)outMsg,"deposit_ccy");
                strcat((char*)outMsg,MY_OLN_FIELD_TOKEN);
                strcat((char*)outMsg,csPtr);
                strcat((char*)outMsg,MY_OLN_TOKEN);
        }
        else {
DEBUGLOG(("FormatAckMsg:: deposit_ccy is missing!!!\n"));
        }
...
/* bank_deposit_datetime */
        if (GetField_CString(hIn,"bank_deposit_datetime",&csPtr)){
DEBUGLOG(("FormatAckMsg:: bank_deposit_datetime = [%s]\n",csPtr));
                strcat((char*)outMsg,"bank_deposit_date");
                strcat((char*)outMsg,MY_OLN_FIELD_TOKEN);
                strcat((char*)outMsg,csPtr);
                strcat((char*)outMsg,MY_OLN_TOKEN);
        }
...
/* pay_ccy */
        if (GetField_CString(hIn,"display_ccy",&csVal)) {
DEBUGLOG(("BuildAckAuthData pay_ccy = [%s]\n",csVal));
                strcat(csData,csVal);
        }
/* pay_amt */
        /*if (GetField_Double(hIn,"display_amt",&dTmp)) */
...
DEBUGLOG(("BuildAckAuthData PAY AMOUNT = [%f]\n",dTmp));
                sprintf((char*)csTmp,"%ld",double2long(dTmp));
DEBUGLOG(("BuildAckAuthData pay_amt = [%s]\n",csTmp));
                strcat(csData,csTmp);
                FREE_ME(csTmp);
        }
/* deposit_ccy */
        if (GetField_CString(hIn,"deposit_ccy",&csVal)) {
DEBUGLOG(("BuildAckAuthData deposit_ccy = [%s]\n",csVal));
                strcat(csData,csVal);
        }
/* deposit_amt */
        if (GetField_Double(hIn,"deposit_amt",&dTmp)) {
...
DEBUGLOG(("BuildAckAuthData DEPOSIT AMOUNT = [%f]\n",dTmp));
                sprintf((char*)csTmp,"%ld",double2long(dTmp));
DEBUGLOG(("BuildAckAuthData deposit_amt = [%s]\n",csTmp));
                strcat(csData,csTmp);
                FREE_ME(csTmp);
        }
...
/* service_fee_ccy */
        if (GetField_CString(hIn,"service_fee_ccy",&csVal)) {
DEBUGLOG(("BuildAckAuthData service_fee_ccy = [%s]\n",csVal));
                strcat(csData,csVal);
        }
...
DEBUGLOG(("BuildAckAuthData SERVICE FEE = [%f]\n",dTmp));
                sprintf((char*)csTmp,"%ld",double2long(dTmp));
DEBUGLOG(("BuildAckAuthData service_fee = [%s]\n",csTmp));
                strcat(csData,csTmp);
                FREE_ME(csTmp);
        } else {
		strcat(csData, "0");
DEBUGLOG(("BuildAckAuthData service_fee = 0\n"));
	}
