...
-------------------------------                    ------------            --------------
Init Version					   2017/03/21		   David Wong
...
/* 1 inputCharset */
		...
		strcat((char*)outMsg,MY_OPP_FIELD_TOKEN);
		strcat((char*)outMsg,"1"); //UTF-8
		strcat((char*)outMsg,MY_OPP_TOKEN);
...
/* 2 pickupUrl */
		if (GetField_CString(hIn,"return_url_only",&csTmp)) {
			...
			strcat((char*)outMsg,MY_OPP_TOKEN);
DEBUGLOG(("FormatMsg:: pickupUrl = [%sfe/]\n",csTmp));
		}
		else {
			iRet = PD_ERR;
...
/* 5 language */
		...
		strcat((char*)outMsg,MY_OPP_FIELD_TOKEN);
		strcat((char*)outMsg,"1"); //Simplified Chinese
		strcat((char*)outMsg,MY_OPP_TOKEN);
...
/* 24 payType */
		if (GetField_CString(hIn, "bank_code", &csTmp)) {
			...
			if (iOpt == PD_FALSE && iOpt != INT_ERR) {
				strcat((char*)outMsg, MY_OPP_BANKCARD); //vnet
DEBUGLOG(("FormatMsg:: payType = [%s]\n", MY_OPP_BANKCARD));
				iIsVnet = PD_TRUE;
			} else if (iOpt && iOpt != INT_ERR) {
				strcat((char*)outMsg, MY_OPP_MOBILE); //ec
DEBUGLOG(("FormatMsg:: payType = [%s]\n", MY_OPP_MOBILE));
			}
			strcat((char*)outMsg, MY_OPP_TOKEN);
		}
...
DEBUGLOG(("DATA = [%s][%d]\n",inMsg,inLen));
	...
	if (Str2Cls(hRec,(char *)inMsg,MY_OPP_TOKEN, MY_OPP_FIELD_TOKEN) == PD_OK) {
/* 1 merchantId */
...
/* 1 inputCharset */
	...
	strcat((char*)csBuf,MY_OPP_FIELD_TOKEN);
	strcat((char*)csBuf,"1"); //UTF-8
	strcat((char*)csBuf,MY_OPP_TOKEN);
...
/* 5 language */
	...
	strcat((char*)csBuf,MY_OPP_FIELD_TOKEN);
	strcat((char*)csBuf,"1"); //Simplified Chinese
	strcat((char*)csBuf,MY_OPP_TOKEN);
...
/* 24 payType */
	if (GetField_CString(hIn, "bank_code", &csPtr)) {
		...
		if (iOpt == PD_FALSE && iOpt != INT_ERR) {
			strcat((char*)csBuf, MY_OPP_BANKCARD); //vnet
DEBUGLOG(("BuildAuthData:: payType = [%s]\n", MY_OPP_BANKCARD));
			iIsVnet = PD_TRUE;
		} else if (iOpt && iOpt != INT_ERR) {
			strcat((char*)csBuf, MY_OPP_MOBILE); //ec
DEBUGLOG(("BuildAuthData:: payType = [%s]\n", MY_OPP_MOBILE));
		}
		strcat((char*)csBuf, MY_OPP_TOKEN);
	}
