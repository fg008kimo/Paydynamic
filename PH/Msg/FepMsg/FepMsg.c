...
DEBUGLOG(("FormatMsg:: uid not found!!!\n"));
	}
...
/* pdt */
		if (GetField_CString(hRec,"pdt",&csPtr)) {
			...
			sprintf(csFundinDate,"%.*s%.*s%.*s%.*s%.*s%.*s",PD_ACC_YEAR_LEN,csPtr,2,&csPtr[PD_ACC_YEAR_LEN +1],2,&csPtr[PD_ACC_YEAR_LEN +4],2,&csPtr[PD_ACC_YEAR_LEN +7],2,&csPtr[PD_ACC_YEAR_LEN +10],2,&csPtr[PD_ACC_YEAR_LEN +13]);
/*
			memcpy(&csFundinDate[0],csPtr,PD_DATE_LEN);
			memcpy(&csFundinDate[PD_DATE_LEN],&csPtr[PD_DATE_LEN +1],2);
			memcpy(&csFundinDate[PD_DATE_LEN +1 +2 ],&csPtr[PD_DATE_LEN +1 +2 +1],2);
			memcpy(&csFundinDate[PD_DATE_LEN +1 +2 +1 + 2],&csPtr[PD_DATE_LEN +1 +2 +1 +2 +1],2);
			csFundinDate[PD_DATETIME_LEN] = '\0';
*/

			PutField_CString(hOut,"fundin_date",csFundinDate);
			PutField_CString(hOut,"txn_date",csFundinDate);
DEBUGLOG(("BreakDownMsg:: pdt (fundin_date)= [%s]\n",csPtr));
...
DEBUGLOG(("BuildAuthData:: return_url_only missing\n"));
	}
...
//sr
        if (GetField_CString(hIn,"sr",&csPtr)) {
...
DEBUGLOG(("BuildRspAuthData:: fcode = [%s]\n",csPtr));
		...
		strcat(csDATA,"TRANSACTION");
DEBUGLOG(("BuildRspAuthData:: fcode missing put default TRANSACTION\n"));
	}
//sc
