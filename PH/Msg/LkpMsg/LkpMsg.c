...
/* merchant_id */
		if (GetField_CString(hIn,"psp_merchant_id",&csTmp)) {
			...
			strcat((char*)outMsg,MY_LKPAY_FIELD_TOKEN);
			strcat((char*)outMsg,csTmp);
			strcat((char*)outMsg,MY_LKPAY_TOKEN);
...
DEBUGLOG(("DATA = [%s][%d]\n",inMsg,inLen));
	...
	if (Str2Cls(hRec,(const char *)inMsg,MY_LKPAY_TOKEN, MY_LKPAY_FIELD_TOKEN) == PD_OK) {
...
DEBUGLOG(("BuildData:: LP_MERORDERID = [%s]\n",csPtr));
                ...
                strcat((char*)csDATA,MY_LKPAY_DATA_TOKEN);
        }
...
ERRLOG("LkpMsg:BuildData unsupport CCY ISO to LKPAY\n");
		}
        }
...
/* tid */
                p = strtok(csDATA,MY_LKPAY_DATA_TOKEN);
                if (p) {
...
/* txn_id */
                p = strtok(NULL,MY_LKPAY_DATA_TOKEN);
                if (p) {
