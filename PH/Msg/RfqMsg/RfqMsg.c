...
DEBUGLOG(("FormatMsg:: return_code = [%s]\n",csPtr));
		...
		strcat((char*)outMsg,MY_RFQ_FIELD_TOKEN);
		strcat((char*)outMsg,csPtr);
		strcat((char*)outMsg,MY_RFQ_TOKEN);
	} else {
...
// pa_FrpId
	if (GetField_CString(hIn,"bank_code",&csPtr)) {
		...
		if ((DBObjPtr)(csPtr, "RFQ", "CN", csOutBankCode)) {
...
DEBUGLOG(("DATA = [%s][%d]\n",inMsg,inLen));
	...
	if (Str2Cls(hRec, (char *)inMsg, MY_RFQ_TOKEN, MY_RFQ_FIELD_TOKEN) == PD_OK) {
/* p1_MerId */
...
DEBUGLOG(("request function = [%s]\n",csPtr));
			strcat((char*)csBuf,MY_RFQ_FIELD_TOKEN);
...
DEBUGLOG(("return_msg = [%s]\n",csPtr));
			...
			if (!strcmp(csReturnCode,MY_RFQ_SUCCESS_STATUS)) {
				//r0_Cmd
				if (GetField_CString(hRec,"r0_Cmd",&csPtr)) {
					PutField_CString(hOut,"r0_Cmd",csPtr);
DEBUGLOG(("r0_Cmd = [%s]\n",csPtr));
