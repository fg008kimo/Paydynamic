...
// Country
	if (GetField_CString(hIn,"txn_country",&csPtr)) {
		json_object_object_add(jobj, "Country", json_object_new_string(csPtr));
DEBUGLOG(("Country = [%s]\n", csPtr));
	}
	else {
		json_object_object_add(jobj, "Country", json_object_new_string(MY_ZPE_COUNTRY));
...
DEBUGLOG(("return_msg = [%s]\n",csPtr));
			...
			//if (!strcmp(csReturnCode,MY_ZPE_SUCCESS_STATUS)) {

				//Merchant_ID
				if (GetField_CString(hRec,"Merchant_ID",&csPtr)) {
					PutField_CString(hOut,"Merchant_ID",csPtr); //for BuildRspAuth
DEBUGLOG(("Merchant_ID = [%s]\n",csPtr));
				}
				...
				if (GetField_CString(hRec,"Merchant_Ref",&csPtr)) {
					...
					PutField_CString(hOut,"Merchant_Ref",csPtr); //for BuildRspAuth
DEBUGLOG(("Merchant_Ref:ret_txn_id = [%s]\n",csPtr));
				}

				//Mer_txn_date
				if (GetField_CString(hRec,"Mer_txn_date",&csPtr)) {
					PutField_CString(hOut,"Mer_txn_date",csPtr); //for BuildRspAuth
DEBUGLOG(("Mer_txn_date = [%s]\n",csPtr));
				}

				//Currency
				if (GetField_CString(hRec,"Currency",&csPtr)) {
					PutField_CString(hOut,"Currency",csPtr); //for BuildRspAuth
DEBUGLOG(("Currency = [%s]\n",csPtr));
				}

				//-----?? Amount (psp will return 10 (integer) when amount is 10.00, 10.1 when amount is 10.10)
				//-----?? char csAmt[PD_TMP_BUF_LEN];
				if (GetField_CString(hRec,"Amount",&csPtr)) {
					//-----?? need pad zero at the back?
					//-----?? PutField_CString(hOut,"amount",csAmt);
					PutField_CString(hOut,"amount",csPtr);
					PutField_CString(hOut,"Amount",csPtr); //for BuildRspAuth
...
DEBUGLOG(("Redirect_URL = [%s]\n",csPtr));
				...
				//Order_Ref
				if (GetField_CString(hRec,"Order_Ref",&csPtr)) {
					PutField_CString(hOut,"Order_Ref",csPtr); //for BuildRspAuth
DEBUGLOG(("Order_Ref:tid = [%s]\n",csPtr));
				}
