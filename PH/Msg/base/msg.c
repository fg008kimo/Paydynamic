...
void json_parse_array( json_object *jobj, hash_t* hOut,char *csKey,char* key) {
	...
	json_bool  tmpJBool;
DEBUGLOG(("json_parse_array start!\n"));

        json_object *jarray = jobj; /*Simply get the array*/
        if(key) {
                //jarray = json_object_object_get(jobj, key); // Getting the array if it is a key value pair
                tmpJBool = json_object_object_get_ex(jobj, key, &jarray); /*Getting the array if it is a key value pair*/
        }
...
DEBUGLOG(("*parse_array\n"));
			...
			PutField_CString(hOut, csNewObj, json_object_get_string(jvalue));
DEBUGLOG(("string [%s] = [%s]\n", csNewObj, json_object_get_string(jvalue)));
		}
...
DEBUGLOG(("ParseJ:: supported type,[%d] array [%s]\n",type, key));
