...
Add SengPay					   2012/04/10		   Cody Chan
Add mobile_no on BreakDown Msg			   2013/01/22		   Virginia Yun
Add FEP FormatMsg				   2013/01/28		   Virginia Yun
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
DEBUGLOG(("FormatMsg:: psp_merchant_id = [%s]\n",csPtr));
							strcat(csTmpBuf,"MID");
							strcat(csTmpBuf,MY_XPAY_FIELD_TOKEN );
							strcat(csTmpBuf,csPtr);
							strcat(csTmpBuf,MY_XPAY_TOKEN);
						}
						else {
...
DEBUGLOG(("FormatMsg:: outmsg in clear text = [%s]\n",csTmpBuf));
                                                strcat((char*)outMsg,"redirect_url");
                                                strcat((char*)outMsg,"=");
                                                csBuf = (char*) malloc (PD_MAX_BUFFER +1);
                                                ...
                                                strcat((char*)outMsg,"ret_status=0");
                                                strcat((char*)outMsg,MY_XPAY_TOKEN);
						if (GetField_CString(hIn,"post_method",&csPtr)) {
...
DEBUGLOG(("XpyMsg:FormatMsg ret = [%d]\n",iRet));
			}
                       	return iRet;
                }
...
DEBUGLOG(("FormatAckMsg:: tid = [%s]\n",csPtr));
		strcat((char*)outMsg,"PSPTID");
        	strcat((char*)outMsg,MY_XPAY_FIELD_TOKEN);
               	strcat((char*)outMsg,csPtr);
               	strcat((char*)outMsg,MY_XPAY_TOKEN);	
	}
...
/* mobile_no */
	p = GetField((const unsigned char*)csBuf,"mobile_no",MY_XPAY_TOKEN,MY_XPAY_FIELD_TOKEN);
	if(p) {
		//PutField_CString(hOut,"mobile_no",p);
		PutField_CString(hOut,"customer_tel",p);
DEBUGLOG(("BreakDownMsg customer_tel = [%s]\n",p));
	}
...
/* language */
		...
		if (p) {
			if(!strcmp(csTxnCountry,PD_TAIWAN))
				PutField_CString(hOut,"language",PD_LANG_TW);
			else
...
DEBUGLOG(("BuildData:: ORDERID is missing!!!\n"));
	}
...
DEBUGLOG(("BuildData:: MERORDERID is missing!!!\n"));
	}
...
DEBUGLOG(("BuildData:: MERDATE is missing!!!\n"));
	}
...
DEBUGLOG(("BuildData:: BANKBILLNO is missing!!!\n"))
	}
/* fundin_date */
...
DEBUGLOG(("BuildData:: BANKTIME is missing!!!\n"));
	}
...
DEBUGLOG(("BuildData:: LANG is missing!!!\n"));
	}
	if (cArInd == PD_ACCEPT) {
