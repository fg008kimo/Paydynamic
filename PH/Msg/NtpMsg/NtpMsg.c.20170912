...
-------------------------------                    ------------            --------------
Init Version					   2016/12/22              David Wong
*/
...
/* business_id */
		if (GetField_CString(hIn,"psp_merchant_id",&csTmp)) {
			...
			strcat((char*)outMsg,MY_NTP_FIELD_TOKEN);
			strcat((char*)outMsg,csTmp);
			strcat((char*)outMsg,MY_NTP_TOKEN);
...
DEBUGLOG(("DATA = [%s][%d]\n",inMsg,inLen));
	...
	if (Str2Cls(hRec,(char *)inMsg,MY_NTP_TOKEN, MY_NTP_FIELD_TOKEN) == PD_OK) {
...
/* orderDetails */
	strcat((char*)csBuf,PD_ORDER_DETAILS);
DEBUGLOG(("BuildAuthData:: orderDetails = [%s]\n",PD_ORDER_DETAILS));
...
/* org_code */
	if (GetField_CString(hIn,"bank_code",&csPtr)) {
		...
		if(iOpt==PD_FALSE && iOpt!=INT_ERR){
			strcat((char*)csBuf,csPtr);
DEBUGLOG(("BuildAuthData:: org_code = [%s]\n",csPtr));
		}
	}
/*
	strcat((char*)csBuf,"TestBank");
DEBUGLOG(("BuildAuthData:: org_code = [TestBank]\n"));
*/
