...
DEBUGLOG(("FormatMsg:: request function  = [%s]\n",csPtr));
			strcat((char*)csBuf,MY_TPM_FIELD_TOKEN);
...
DEBUGLOG(("FormatMsg:: outMsg = [%s]\n",outMsg));
		...
		//strcat((char*)outMsg,MY_TPM_URL_END_TOKEN);
...
DEBUGLOG(("FormatMsg:: ver = [%s]\n",csPtr));
		...
		strcat((char*)outMsg,MY_TPM_FIELD_TOKEN);
		strcat((char*)outMsg,csPtr);
		strcat((char*)outMsg,MY_TPM_TOKEN);
	}
	else {
...
DEBUGLOG(("DATA = [%s][%d]\n",inMsg,inLen));
        ...
        if (Str2Cls(hRec,(char*)inMsg,MY_TPM_TOKEN, MY_TPM_FIELD_TOKEN) == PD_OK) {
...
DEBUGLOG(("BuildAuthData:: bank_type = [%s]\n",csPtr));
		...
		strcat(csDATA,MY_TPM_FIELD_TOKEN);
		//strcat(csDATA,csPtr);
		strcat(csDATA,"0");
		strcat(csDATA,MY_TPM_TOKEN);
	}
	else {
...
DEBUGLOG(("BuildRspAuthData:: purchase_alias is missing\n"));
        }
...
/* 1 input_charset*/
        ...
        strcat((char*)csBuf,MY_TPM_TOKEN);
