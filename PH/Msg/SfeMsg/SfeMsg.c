...
Support json_type_int                              2018/02/15              David Wong
Support cardholderPhoneNo                          2018/02/27              David Wong
Support Vnet                                       2018/03/02              David Wong
change url_method, not use special redirect	   2018/03/29		   LokMan Chow
*/
...
DEBUGLOG(("transId = [%s]\n", MY_SFE_TRANS_ID_VNET));
				iIsVnet = PD_TRUE;
				json_object_object_add(jobj, "transId", json_object_new_string(MY_SFE_TRANS_ID_VNET));
...
// bankCode
		if (iIsVnet) {
			if (GetField_CString(hIn, "bank_code", &csTmp)) {
DEBUGLOG(("bankCode = [%s]\n", csTmp));
				json_object_object_add(jobj, "bankCode", json_object_new_string(csTmp));
			}
			else {
				iRet = PD_ERR;
DEBUGLOG(("***bank_code is missing\n"));
...
DEBUGLOG(("url_method = [%s]\n", csMethod));
		...
		if(!strcmp(csMethod,PD_DEFAULT_METHOD))
...
// transId
	...
	strcat((char*)csBuf, MY_SFE_FIELD_TOKEN);
	if (iIsVnet) {
		strcat((char*)csBuf, MY_SFE_TRANS_ID_VNET);
DEBUGLOG(("transId = [%s]\n", MY_SFE_TRANS_ID_VNET));
	}
	else {
