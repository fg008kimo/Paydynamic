...
Add "add_user","update_user"			   2015/07/03		   Dirk Wong
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
DEBUGLOG(("initReplyFromRequest()txn_code = [%s]\n",csPtr));
        ...
        if (GetField_CString(hRequest,"code",&csPtr)) {
DEBUGLOG(("initReplyFromRequest()code = [%s]\n",csPtr));
                PutField_CString(hResponse,"code",csPtr);
        }
