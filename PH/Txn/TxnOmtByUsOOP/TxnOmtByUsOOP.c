		...
		hash_t* hResponse)
{
	...
	if (GetField_CString(hContext, "txn_code", &csApiCode)) {
DEBUGLOG(("Authorize:: API code = [%s]\n", csApiCode));
	}
...
ERRLOG("TxnOmtByUsOOP::DBOLTransaction:Update Failed Ret [%d]\n", iRet);
	...
	if (iRet == PD_OK) {
		PutField_CString(hContext, "txn_code", csApiCode);
		PutField_CString(hContext, "mini_txn_type", PD_MI_TXN_TYPE_SWEEP_IN);
		PutField_Int(hContext, "need_add_mi_txn_log", PD_FALSE);

DEBUGLOG(("Authorize:: Call TxnOmtByUs2MiniMMM:Authorize()\n"));
		TxnObjPtr = CreateObj(TxnPtr, "TxnOmtByUs2MiniMMM", "Authorize");
		iRet = (unsigned long)((*TxnObjPtr)(hContext, hRequest, hResponse));
		if (iRet != PD_OK) {
DEBUGLOG(("Authorize:: TxnOmtByUs2MiniMMM:Authorize() Failed Ret [%d]\n", iRet));
ERRLOG("TxnOmtByUsPTO:Authorize:: TxnOmtByUs2MiniMMM:Authorize() Failed Ret [%d]\n", iRet);
		}
	}
