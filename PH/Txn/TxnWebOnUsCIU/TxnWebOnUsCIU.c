/*
Partnerdelight (c)2013. All rights reserved. No part of this software may be reproduced in any form without written permission
of an authorized representative of Partnerdelight.

Change Description                                 Change Date             Change By
-------------------------------                    ------------            --------------
Init Version                                       2013/08/19              Cody Chan
Add  GetTotalMatchRec Checking		  	   2013/09/24              Virginia Yun
Add  VP Allow List                                 2013/10/09              Virginia Yun
add vp group id					   2013/10/16		   LokMan Chow
*/

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <ctype.h>
#include "common.h"
#include "utilitys.h"
#include "ObjPtr.h"
#include "internal.h"
#include "TxnWebOnUsCIU.h"
#include "myrecordset.h"
#include <curl/curl.h>
#include "queue_utility.h"
#include "mq_db.h"

char cDebug;

OBJPTR(DB);
void TxnWebOnUsCIU(char    cdebug)
{
        cDebug = cdebug;
}

int     Authorize(hash_t* hContext,
                        const hash_t* hRequest,
                        hash_t* hResponse)
{
        int     iRet = PD_OK;
	char*	csPtr;
	hash_t  *hTxn;
	hash_t  *hGpTxn;

	char	*csClientId;
	int	iTxnGpId = -1;

	int	iTotalCnt = -1;

DEBUGLOG(("TxnWebOnUsCIU Authorize\n"));

	hTxn = (hash_t*)  malloc (sizeof(hash_t));
        hash_init(hTxn,0);

	hGpTxn = (hash_t*)  malloc (sizeof(hash_t));
        hash_init(hGpTxn,0);

/* magic num */
	if (GetField_CString(hRequest,"magic_num",&csPtr)) {
DEBUGLOG(("TxnWebOnUsCIU magic_num = [%s]\n",csPtr));
		PutField_CString(hTxn,"magic_num",csPtr);
	}
	else {
DEBUGLOG(("TxnWebOnUsCIU magic_num is missing\n"));
		iRet = INT_FORMAT_ERR;
	}

/* magic word */
	if (GetField_CString(hRequest,"magic_word",&csPtr) && iRet == PD_OK) {
DEBUGLOG(("TxnWebOnUsCIU magic_word = [%s]\n",csPtr));
		PutField_CString(hTxn,"magic_word",csPtr);
	}
	else {
DEBUGLOG(("TxnWebOnUsCIU magic_word is missing\n"));
		iRet = INT_FORMAT_ERR;
	}

/* customer_id */
	if (GetField_CString(hRequest,"customer_id",&csPtr) && iRet == PD_OK) {
DEBUGLOG(("TxnWebOnUsCIU customer_id = [%s]\n",csPtr));
		PutField_CString(hTxn,"customer_id",csPtr);
	}
	else {
DEBUGLOG(("TxnWebOnUsCIU customer_id is missing\n"));
		iRet = INT_FORMAT_ERR;
	}

/* key_id */
	if (GetField_CString(hRequest,"key_id",&csClientId) && iRet == PD_OK) {
DEBUGLOG(("TxnWebOnUsCIU key_id = [%s]\n",csClientId));
		PutField_CString(hTxn,"client_id",csClientId);
	}
	else {
DEBUGLOG(("TxnWebOnUsCIU key_id is missing\n"));
		iRet = INT_FORMAT_ERR;
	}

/* vp_list - optional*/
/*	if (GetField_CString(hRequest,"vp_list",&csPtr) && iRet == PD_OK) {
		iAllowList = atoi(csPtr);
DEBUGLOG(("TxnWebOnUsCIU allow list = [%d]\n", iAllowList));
		if (iAllowList != PD_TRUE && iAllowList != PD_FALSE) {
			iRet = INT_FORMAT_ERR;
		} else {
			PutField_Int(hTxn,"allow_list",iAllowList);
		}
	}
*/
	if(GetField_CString(hRequest,"vp_txn_gp_id",&csPtr)  && iRet == PD_OK){
DEBUGLOG(("TxnWebOnUsCIU vp_txn_gp_id = [%s]\n",csPtr));
		PutField_Int(hTxn,"txn_gp_id",atoi(csPtr));
		iTxnGpId = atoi(csPtr);
	}
	else {
DEBUGLOG(("TxnWebOnUsCIU txn_gp_id is missing\n"));
		iRet = INT_FORMAT_ERR;
	}


	if (iRet == PD_OK) {
DEBUGLOG(("TxnWebOnUsCIU GetTotalMatchRec\n")); 
		DBObjPtr = CreateObj(DBPtr,"DBClientMagic","GetTotalMatchRec");
		if (((unsigned long)(*DBObjPtr)(hTxn, &iTotalCnt)) == PD_OK) {
DEBUGLOG(("TxnWebOnUsCIU count [%d]!\n", iTotalCnt));
			if (iTotalCnt != 0 && iTotalCnt != 3) {
DEBUGLOG(("TxnWebOnUsCIU count Invalid!\n"));
				iRet = INT_INVALID_MAGIC_NUM;
				PutField_Int(hContext, "internal_error", iRet);
			}
		}
	}

	PutField_Int(hTxn, "disabled", PD_FALSE);
	PutField_CString(hTxn, "create_user", PD_UPDATE_USER);

	if (iRet == PD_OK) {
DEBUGLOG(("TxnWebOnUsCIU Check Group Exists!\n"));
		DBObjPtr = CreateObj(DBPtr,"DBClientMagicGpStatus","ChkGpRecExists");
		if ( (unsigned long)(*DBObjPtr)(csClientId, iTxnGpId) != PD_FOUND) {

DEBUGLOG(("TxnWebOnUsCIU Group NOT Exists [%s] [%d]!\n", csClientId, iTxnGpId));
			PutField_CString(hGpTxn, "client_id", csClientId);
			PutField_Int(hGpTxn, "txn_gp_id", iTxnGpId);
			PutField_Char(hGpTxn, "status", PD_VP_OPEN);
			PutField_CString(hGpTxn, "create_user", PD_UPDATE_USER);

			DBObjPtr = CreateObj(DBPtr,"DBClientMagicGpStatus","Add");
			iRet = (unsigned long)(*DBObjPtr)(hGpTxn);
DEBUGLOG(("TxnWebOnUsCIU Group Created!\n"));
		}
	}



	if (iRet == PD_OK) {
DEBUGLOG(("TxnWebOnUsCIU Call DBClientMagic::ADD\n"));
		DBObjPtr = CreateObj(DBPtr,"DBClientMagic","Add");
		iRet = (unsigned long)(*DBObjPtr)(hTxn);
	}

	hash_destroy(hGpTxn);
        FREE_ME(hGpTxn);

	hash_destroy(hTxn);
        FREE_ME(hTxn);

DEBUGLOG(("TxnWebOnUsCIU Authorize iRet = [%d]\n",iRet));
	return iRet;
}
