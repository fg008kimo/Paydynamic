...
-------------------------------                    ------------            --------------
Init Version                                       2016/11/30              David Wong
*/
...
OBJPTR(DB);
OBJPTR(BO);
int     UpdatePreWCPRespTxnLog(hash_t *hContext,
                	hash_t* hResponse);

int     AddWCPTxnLog(hash_t *hContext,
                const hash_t* hRequest);

void TxnWebOnUs2WCP(char    cdebug)
{
        cDebug = cdebug;
}
int     AddWCPTxnLog(hash_t *hContext,
...

{
	...
	hash_t	*hTxn;
	hash_t	*hRec;
DEBUGLOG(("TxnWebOnUs2WCP::UpdateRespTxnLog()\n"));
...
DEBUGLOG(("UpdateTxnLog  iRet = [%d] after Insert TxnQrRequestLog\n",iRet));
		...
		int	iResult;
        	...
        	iRet = (unsigned long)(*DBObjPtr)(PD_CHANNEL_WECHATPAY,&iResult,csTmpStatus);
...
DEBUGLOG(("process_resp_txn()\n"));
	...
	PutField_CString(hContext,"psp_channel_code",PD_CHANNEL_WECHATPAY);
	PutField_CString(hContext,"return_psp_channel",PD_CHANNEL_WECHATPAY);
	...
	GetField_CString(hRequest,"psp_name",&csPspName);
      	MsgObjPtr = CreateObj(MsgPtr,"WcpMsg","BreakDownInitRspMsg");

        if ((*MsgObjPtr)(hResponse,inMsg->msg,inMsg->len) != PD_OK)  {
		iRet = INT_BREAKDOWN_ERR;
		PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("process_resp_txn:BreakDown WcpMsg Error\n"));
	}
//	else {
		// testing
...
DEBUGLOG(("process_resp_txn: returned return_code = [%s]\n", csTmp));
			...
			if (!strcmp(csTmp, "SUCCESS")) {
				// get mid
				if (!GetField_CString(hResponse, "mid", &csTmp)) {
DEBUGLOG(("process_resp_txn: cannot get mid!!!\n"));
				...
				if (GetField_CString(hResponse, "order_id", &csTmp)) {
...
DEBUGLOG(("process_resp_txn: cannot get order_id!!!\n"));
					PutField_CString(hResponse, "return_code", PD_FAIL_STATUS);
					iSendAlert = PD_TRUE;
					// PD_MISS_TXN_SEQ
					sprintf(csCmd, "send_init_req_error.sh \"%s\" \"%s\" \"%s\"", PD_INVALID_FIELD, csPspId, csPspName);
				...
				if (GetField_CString(hResponse, "order_amount", &csTmp)) {
...
DEBUGLOG(("process_resp_txn: cannot get order_amount!!!\n"));
					PutField_CString(hResponse, "return_code", PD_FAIL_STATUS);
					iSendAlert = PD_TRUE;
					// PD_MISS_TXN_AMT
					sprintf(csCmd, "send_init_req_error.sh \"%s\" \"%s\" \"%s\"", PD_INVALID_FIELD, csPspId, csPspName);
...
DEBUGLOG(("process_resp_txn() call Update\n"));
		iRet = UpdatePreWCPRespTxnLog(hContext,hResponse);
...
DEBUGLOG(("TxnWebOnUs2WCP: Authroize()\n"));
	if (!GetField_CString(hContext,"org_txn_code",&csTxnCode))
		GetField_CString(hContext,"txn_code",&csTxnCode);
DEBUGLOG(("TxbWebOnUs2WCP: txn_code = [%s]\n",csTxnCode));

	if (strcmp(csTxnCode,PD_WITHDRAW_BATCH_TXN_CODE) && strcmp(csTxnCode,PD_INITIAL_TXN_CODE)) //if none batch txn
		iRet = AddWCPTxnLog(hContext,hRequest);

	if (iRet == PD_OK) {

//version
		PutField_CString((hash_t*)hRequest,"version","2.0");

//sign_type
		PutField_CString((hash_t*)hRequest,"sign_type","MD5");
...
DEBUGLOG(("TxbWebOnUs2WCP: time_redirect = [%d]\n",iTmp));
	        ...
	        if (!GetField_CString(hContext,"from_txn_seq",&csTxnSeq)) 
       		 	...
       		 	if ((*BOObjPtr)(hContext,hRequest,hRequest) == PD_OK) {
       		 		...
       		 		MsgObjPtr = CreateObj(MsgPtr,"WcpMsg","FormatInitMsg");
...
DEBUGLOG(("TxnWebOnUs2WCP: after process_resp_err_txn iret = [%d]\n",iRet));
					}
					else {
						msg->mtext[iRecvLen] = '\0';
DEBUGLOG(("TxnWebOnUs2WCP:  recv len = [%d]\n",iRecvLen));
DEBUGLOG(("TxnWebOnUs2WCP:  recv = [%s]\n",&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN));
						memcpy(h_msg->msg,&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN);
						iRet = process_resp_txn(hContext,h_msg,hRequest);
DEBUGLOG(("TxnWebOnUs2WCP: after process_resp_txn iret = [%d]\n",iRet));
					}
                		}
        			FREE_ME(msg);
        		}     
...
DEBUGLOG(("TxnWebOnUs2WCP: BuildData Error\n"));
		}
	}
...
DEBUGLOG(("TxnWebOnUs2WCP: noraml exit iret = [%d]\n",iRet));
	return iRet;
}
