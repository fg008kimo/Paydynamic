...
int	GetVoidBatchInfo(const char* csTxnId, hash_t* hBatchInfo, hash_t *hContext);
...
DEBUGLOG(("+++++++++++++++++++++++++++++++++++++++++++++++++++ Get Void Batch Info ++++++++++++++++++++++++++++++++++++++++++++++++++++\n"));

		iRet = GetVoidBatchInfo(csTxnSeq, hOrgBatchInfo, hContext);
		if (iRet == PD_OK) {
...
DEBUGLOG(("++++++++++++++++++++++++++++++++++++++++++++++++++ Get Void Batch Txn Id +++++++++++++++++++++++++++++++++++++++++++++++++++\n"));
...
DEBUGLOG(("++++++++++++++++++++++++++++++++++++++++++++++++ Get Void Batch Stmt Txn Id ++++++++++++++++++++++++++++++++++++++++++++++++\n"));
...
//DEBUGLOG(("Authorize: [%d]txn_id = [%s]\n",iCnt,csTmpStmtTxnSeq));
						...
						if (iRet == PD_OK) {
							...
							iPutToBatch = IsPutStmtTxnIdToBatch(csTmpStmtTxnSeq, hTmpStmtTxnRec, hBaidTxnRec, hStmtTxnContext); 
...
int     GetVoidBatchInfo(const char* csTxnSeq, hash_t *hBatchInfo, hash_t *hContext)
{
        int     iRet = PD_OK;
	int	iDtlRet = PD_TRUE;
...
DEBUGLOG(("GetVoidBatchInfo:: Call DBOLBatchTxnRelation:: GetBatchIdByRTAscOrder:: batch_id = [%s]\n",csTmpBatchId));
...
DEBUGLOG(("GetVoidBatchInfo:: Call DBOLBatchTxnRelation:: GetBatchIdByRTAscOrder:: txn_type = [%s]\n", csTxnType));
                                }
...
DEBUGLOG(("GetVoidBatchInfo:: Call DBOLBatchTxnRelation:: GetBatchIdByRTAscOrder:: select_batch_id = [%s]\n", csSelectBatchId));
                              	}
...
DEBUGLOG(("GetVoidBatchInfo:: Call DBOLBatchTxnRelation:: GetBatchIdByRTAscOrder:: txn_type = [%s]\n", csTxnType));
		...
		if (iNumOfBatchId == 0) {
DEBUGLOG(("GetVoidBatchInfo:: Call DBOLBatchTxnRelation:: GetBatchIdByRTAscOrder:: No Batch Found!!!\n"));
             	} else if (iNumOfBatchId == 1) {
DEBUGLOG(("GetVoidBatchInfo:: Call DBOLBatchTxnRelation:: GetBatchIdByRTAscOrder:: Single Batch Found!!!\n"));
             	} else if (iNumOfBatchId > 1) {
DEBUGLOG(("GetVoidBatchInfo:: Call DBOLBatchTxnRelation:: GetBatchIdByRTAscOrder:: Multi-Batch Found!!!\n"));
            	}
	} else {
            	iRet = INT_ERR;
            	PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("GetVoidBatchInfo:: Call DBOLBatchTxnRelation:: GetBatchIdByRTAscOrder Invalid Txn Id!!!\n"));
ERRLOG("TxnOmtByUsRBT:: Call DBOLBatchTxnRelation:: GetBatchIdByRTAscOrder Invalid Txn Id!!!\n");
    	}
		
	// Check txn type
	if (iRet == PD_OK) {	
//DEBUGLOG(("GetVoidBatchInfo:: txn_type = [%s]\n",csTxnType));	
...
//DEBUGLOG(("GetVoidBatchInfo:: org_batch_type = [%c]\n", cTmpBatchType));

					PutField_Char(hBatchInfo,"org_batch_sub_type",cTmpBatchSubType);
//DEBUGLOG(("GetVoidBatchInfo:: org_batch_sub_type = [%c]\n", cTmpBatchSubType));
				
					PutField_CString(hBatchInfo,"org_batch_id",csTmpBatchId);
//DEBUGLOG(("GetVoidBatchInfo:: org_batch_id = [%s]\n", csTmpBatchId));
				}
...
DEBUGLOG(("GetVoidBatchInfo:: [%d]batch_id = [%s]\n",iRecCnt,csTmpBatchId));
				...
				// First Record
				if (iRecCnt == 0) {
DEBUGLOG(("GetVoidBatchInfo:: [%d]first_record\n",iRecCnt));
                                        ...
                                        if (iDtlRet == PD_FALSE) {
DEBUGLOG(("GetVoidBatchInfo:: txn_id not allow to void/cancel\n"));
				...
				// Last Record
				if (!hBatchRec) {
DEBUGLOG(("GetVoidBatchInfo:: [%d]last_record\n",iRecCnt));
...
DEBUGLOG(("GetVoidBatchInfo:: iRet = [%d]\n",iRet));
        return iRet;
}
...
DEBUGLOG(("CheckVoidTxnIdValidProviderCharge:: GetBatchTypeInfo, cBatchType = [%c]\n", cBatchType));
			}

/* batch_sub_type */
			if (GetField_Char(hBatchTypeInfo,"batch_sub_type",&cBatchSubType)) {
DEBUGLOG(("CheckVoidTxnIdValidProviderCharge:: GetBatchTypeInfo, cBatchSubType = [%c]\n", cBatchSubType));
			}
		}

/* batch_id */
DEBUGLOG(("CheckVoidTxnIdValidProviderCharge:: GetBatchTypeInfo, csBatchId = [%s]\n", csTmpBatchId));
				...
				// Check if Batch Sub Type is Undo
					...
					// Check is void txn code
						...
						if (iDtlRet == PD_TRUE) {
							...
							if (iDtlRet == PD_FOUND) {
DEBUGLOG(("CheckVoidTxnIdValidProviderCharge:: next_batch_id = [%s], next_txn_id = [%s]\n", csNextBatchId, csNextTxnId));
							...
							while ((iBatchSubTypeUndoRet == PD_TRUE) && (iRet != INT_ERR)) {
								...
								if (iDtlRet == PD_FOUND) {
DEBUGLOG(("CheckVoidTxnIdValidProviderCharge:: prev_batch_id = [%s], prev_txn_id = [%s]\n", csPrevBatchId, csPrevTxnId));
...
DEBUGLOG(("CheckVoidTxnIdValidProviderCharge:: GetBatchTypeInfo, cPrevBatchType = [%c]\n", cPrevBatchType));
                        							}

/* batch_sub_type */
                        							if (GetField_Char(hBatchTypeInfo,"batch_sub_type",&cPrevBatchSubType)) {
DEBUGLOG(("CheckVoidTxnIdValidProviderCharge:: GetBatchTypeInfo, cPrevBatchSubType = [%c]\n", cPrevBatchSubType));
                        							}
                							}
...
DEBUGLOG(("CheckVoidTxnIdValidProviderCharge:: tmp_batch_id = [%s], last_txn_id = [%s]\n", csTmpBatchId, csLastTxnId));
...
DEBUGLOG(("FindVoidBatchIdProviderCharge:: batch_id = [%s], txn_id = [%s], found_batch_id = [%s], found_txn_id = [%s]\n", csBatchId, csTxnId, csFoundBatchId, csFoundTxnId));
...
/* batch_type */
        		if (GetField_Char(hBatchTypeInfo,"batch_type",&cBatchType)) {
DEBUGLOG(("FindVoidBatchIdProviderCharge:: GetBatchTypeInfo, cBatchType = [%c]\n", cBatchType));
        		}

/* batch_sub_type */
        		if (GetField_Char(hBatchTypeInfo,"batch_sub_type",&cBatchSubType)) {
DEBUGLOG(("FindVoidBatchIdProviderCharge:: GetBatchTypeInfo, cBatchSubType = [%c]\n", cBatchSubType));
        		}
		}
...
DEBUGLOG(("FindVoidBatchIdProviderCharge:: found_batch_id = [%s], found_txn_id = [%s]\n", csFoundBatchId, csFoundTxnId));
...
DEBUGLOG(("FindVoidBatchIdProviderCharge:: GetBatchTypeInfo, cFoundBatchType = [%c]\n", cFoundBatchType));
                        					}

/* batch_sub_type */
                        					if (GetField_Char(hBatchTypeInfo,"batch_sub_type",&cFoundBatchSubType)) {
DEBUGLOG(("FindVoidBatchIdProviderCharge:: GetBatchTypeInfo, cFoundBatchSubType = [%c]\n", cFoundBatchSubType));
                        					}
                					}
...
/* batch_id */
DEBUGLOG(("GetLastTxnIdByRTDescOrder:: GetBatchTypeInfo, batch_id = [%s]\n",csBatchId));
...
//DEBUGLOG(("GetTxnHeaderInfo:: Call DBOLTransaction:: csTxnId = [%s]\n",csTxnId));
