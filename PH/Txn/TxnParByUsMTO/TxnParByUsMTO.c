...
void TxnParByUsMTO(char    cdebug)
{
        cDebug = cdebug;
}
...
ERRLOG("TxnParByUsMTO::Authorize::txnid not found!!\n");
...
ERRLOG("TxnParByUsMTO::Authorize::txn_code not found!!\n");
                iRet=INT_ERR;
...
ERRLOG("TxnParByUsMTO::Authorize::merchant_id not found!!\n");
...
ERRLOG("TxnParByUsMTO::Authorize::serivce_code not found!!\n");
...
ERRLOG("TxnParByUsMTO:Authorize::invalid service code [%s]\n",csService);
...
ERRLOG("TxnParByUsMTO::Authorize::ccy not found!!\n");
...
ERRLOG("TxnParByUsMTO::Authorize::txn_amt not found!!\n");
...
ERRLOG("TxnParByUsMTO::Authorize::approval_date not found!!\n");
                        iRet=INT_INVALID_TXN;
...
ERRLOG("TxnParByUsMTO::BOFee::GetTxnFee Failed Ret [%d]\n", iRet);
			iRet = INT_ERR;
                } 	
		else {
...
DEBUGLOG(("Authorize::BOFee:GetTxnFee src_txn_fee_ccy [%s]\n", csTmp));
	...
	//Update Balance
        if(iRet==PD_OK){
DEBUGLOG(("Authorize::Call BOParAdjustment:ProcessPartyBalance\n"));
                BOObjPtr = CreateObj(BOPtr,"BOParAdjustment","ProcessPartyBalance");
		iRet = (unsigned long)((*BOObjPtr)(hContext));

		if (iRet != PD_OK) {
DEBUGLOG(("Authorize::BOParAdjustment:ProcessPartyBalance Failed Ret [%d]\n", iRet));
ERRLOG("TxnParByUsMTO::BOParADjustment::ProcesspartyBalance Failed Ret [%d]\n", iRet);
		} else {	
...
DEBUGLOG(("Authorize::BOParAdjustment:ProcessPartyBalance merchant_open_bal [%lf]\n", dTmp));
					PutField_Double(hContext, "open_bal", dTmp);
				}

				if (GetField_Double(hContext, "merchant_open_bal_settlement", &dTmp)) {
DEBUGLOG(("Authorize::BOParAdjustment:ProcessPartyBalance merchant_open_bal_settlement [%lf]\n", dTmp));
...
DEBUGLOG(("Authorize::BOParAdjustment:ProcessPartyBalance Failed to get dc_ind\n"));
ERRLOG("TxnParByUsMTO::BOParAdjustment::ProcesspartyBalance Failed to get dc_ind\n");
				iRet = PD_ERR;
			}
...
DEBUGLOG(("TxnParByUsMTO Normal Exit() iRet = [%d]\n",iRet));
        return iRet;
}
