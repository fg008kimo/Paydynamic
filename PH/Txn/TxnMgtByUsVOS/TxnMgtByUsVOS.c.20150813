...
-------------------------------                    ------------            --------------
Init Version                                       2013/03/08              LokMan Chow
dual control void				   2013/05/06		   LokMan Chow
*/
...
void TxnMgtByUsVOS(char    cdebug)
{
        cDebug = cdebug;
}
...
DEBUGLOG(("TxnMgtByUsVOS: Authroize()\n"));
	if (GetField_CString(hRequest,"org_txn_seq",&csOrgTxnSeq)) {
DEBUGLOG(("TxnMgtByUsVOS: org txn seq = [%s]\n",csOrgTxnSeq));
...
DEBUGLOG(("TxnMgtByUsVOS: update_user = [%s]\n",csUpdateUser));
	}
...
DEBUGLOG(("GetTxnHeader::txn_code = [%s]\n",csOrgTxnCode));
                                ...
                                if (GetField_CString(hRec,"org_txn_id",&csOrgOSTSeq)) {
...
ERRLOG("TxnMgtByUsVOS:Authorize::GetTxnHeader::not found record!!\n");
                        iRet=INT_NOT_RECORD;
                        PutField_Int(hContext,"internal_error",iRet);
                }
	}

	if(!strcmp(csOrgTxnCode,PD_OFFLINE_SETTLE_TO)){
...
DEBUGLOG(("Authorize::Call DBSebBalance:Add failed\n"));
		...
		if(iRet==PD_OK){
			//find the related OST and update the sub-status to Approved 
			//csOrgOSTSeq
			hash_init(hTxn,0);
			PutField_CString(hTxn,"txn_seq",csOrgOSTSeq);
			PutField_CString(hTxn,"sub_status",PD_APPROVED);
DEBUGLOG(("Authorize::Call DBTransaction:Update OST[%s]\n",csOrgOSTSeq));
			DBObjPtr = CreateObj(DBPtr,"DBTransaction","Update");
			iRet = (unsigned long)(*DBObjPtr)(hTxn);
DEBUGLOG(("Authorize::DBTransaction:Update iRet = [%d]\n",iRet));
		}
	}
	else if(!strcmp(csOrgTxnCode,PD_OFFLINE_SETTLE_FROM)){
//////do nothing
		iRet = PD_OK;
	}
	else{
