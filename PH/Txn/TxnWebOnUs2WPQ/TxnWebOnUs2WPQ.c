...
OBJPTR(DB);
OBJPTR(BO);
int     UpdatePreWPQRespTxnLog(hash_t *hContext,
                	hash_t* hResponse);

int     AddWPQTxnLog(hash_t *hContext,
                const hash_t* hRequest);

void TxnWebOnUs2WPQ(char    cdebug)
{
        cDebug = cdebug;
}
int     AddWPQTxnLog(hash_t *hContext,
...

{
	...
	hash_t	*hTxn;
	hash_t	*hRec;
DEBUGLOG(("TxnWebOnUs2WPQ::UpdateRespTxnLog()\n"));
...
DEBUGLOG(("UpdateTxnLog  iRet = [%d] after Insert TxnQrRequestLog\n",iRet));
		...
		int	iResult;
        	...
        	iRet = (unsigned long)(*DBObjPtr)(PD_CHANNEL_WPAY_QR,&iResult,csTmpStatus);
...
DEBUGLOG(("process_resp_txn()\n"));
	...
	PutField_CString(hContext,"psp_channel_code",PD_CHANNEL_WPAY_QR);
	PutField_CString(hContext,"return_psp_channel",PD_CHANNEL_WPAY_QR);
	...
	GetField_CString(hRequest,"psp_name",&csPspName);
      	MsgObjPtr = CreateObj(MsgPtr,"WpqMsg","BreakDownInitRspMsg");

        if ((*MsgObjPtr)(hResponse,inMsg->msg,inMsg->len) != PD_OK)  {
		iRet = INT_BREAKDOWN_ERR;
		PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("process_resp_txn:BreakDown WpqMsg Error\n"));
	}
//	else {
		// testing
...
DEBUGLOG(("process_resp_txn() call Update\n"));
		iRet = UpdatePreWPQRespTxnLog(hContext,hResponse);
...
DEBUGLOG(("TxbWebOnUs2WPQ: time_redirect = [%d]\n",iTmp));
			...
			PutField_Int((hash_t*)hRequest,"is_init_req",PD_TRUE);
       		 	...
       		 	if ((*BOObjPtr)(hContext,hRequest,hRequest) == PD_OK) {
       		 		...
       		 		MsgObjPtr = CreateObj(MsgPtr,"WpqMsg","FormatInitMsg");
...
DEBUGLOG(("TxnWebOnUs2WPQ: after process_resp_err_txn iret = [%d]\n",iRet));
					}
					else {
						msg->mtext[iRecvLen] = '\0';
DEBUGLOG(("TxnWebOnUs2WPQ:  recv len = [%d]\n",iRecvLen));
DEBUGLOG(("TxnWebOnUs2WPQ:  recv = [%s]\n",&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN));
						memcpy(h_msg->msg,&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN);
						iRet = process_resp_txn(hContext,h_msg,hRequest);
DEBUGLOG(("TxnWebOnUs2WPQ: after process_resp_txn iret = [%d]\n",iRet));
					}
                		}
        			FREE_ME(msg);
        		}     
...
DEBUGLOG(("TxnWebOnUs2WPQ: BuildData Error\n"));
		}
	}
...
DEBUGLOG(("TxnWebOnUs2WPQ: noraml exit iret = [%d]\n",iRet));
	return iRet;
}
