...
int	CheckOrgBatchValid(hash_t* hContext, hash_t* hRequest, hash_t* hBatchRec);
int     PrcoessRspDeliveryOut(hash_t* hContext, hash_t* hRequest, hash_t* hOrgTxnRec, hash_t* hTxnRec);
int	GetRspDeliveryOutTxnInfo(hash_t* hTxnRec);
int     FormRspDeliveryOutTxn(hash_t* hContext, hash_t* hRequest, hash_t* hOrgTxnRec, hash_t* hTxnRec);
int	ProcessRspDeliveryOutTxn(hash_t* hContext, hash_t* hRequest, hash_t* hTxnRec);
...
int     ProcessBatch(hash_t* hBatchRec, hash_t* hRspDeliInTxnRec, hash_t* hRspDeliOutTxnRec, hash_t* hOpbFundInTxnRec, hash_t* hRspAmtDiffTxnRec);
int     AddBatchDetail(hash_t* hBatchRec, hash_t* hTxnRec);
int	UpdateBatch(hash_t* hBatchRec, hash_t* hRspDeliInTxnRec, hash_t* hRspDeliOutTxnRec);
int	ProcessACR(hash_t* hContext, hash_t* hRequest, hash_t* hRspDeliInTxnRec, hash_t* hOpbFundInTxnRec, hash_t* hBatchRec);
...
DEBUGLOG(("Authorize::org_txn_id = [%s]\n",csOrgTxnId));
		PutField_CString(hRspDeliInTxnRec,"txn_seq",csOrgTxnId);
		//PutField_CString(hRspDeliOutTxnRec,"org_txn_seq",csOrgTxnId);
		PutField_CString(hBatchRec,"org_txn_seq",csOrgTxnId);
...
DEBUGLOG(("Authorize::product_code = [%s]\n",csTmp));
             	PutField_CString(hRspDeliOutTxnRec,"product_code",csTmp);
...
DEBUGLOG(("Authorize::remark = [%s]\n",csTmp));
		PutField_CString(hRspDeliOutTxnRec,"remark",csTmp);
...
DEBUGLOG(("Authorize::mini_txn_type = [%s]\n",csTmp));
                PutField_CString(hRspDeliOutTxnRec,"mini_txn_type",csTmp);
...
DEBUGLOG(("Authorize::add_user = [%s]\n",csTmp));
                PutField_CString(hRspDeliOutTxnRec,"add_user",csTmp);
		...
		PutField_CString(hRspDeliOutTxnRec,"update_user",csTmp);
...
/*
 * 
 * Lock RSP Delivery In Txn	
 *
 */	
	if (iRet == PD_OK) {
DEBUGLOG(("Authorize:: Call DBTransaction:MatchTxnStatusforUpdate [%s]\n", csOrgTxnId));
                DBObjPtr = CreateObj(DBPtr, "DBTransaction", "MatchTxnStatusforUpdate");
                iDtlRet = (unsigned long)(*DBObjPtr)(csOrgTxnId,PD_COMPLETE);
                if (iDtlRet == PD_NOT_FOUND) {
                       	iRet = INT_INVALID_TXN;
			PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("Authorize::call DBTransaction MatchTxnStatusforUpdate::Invalid Txn Id!!!\n"));
ERRLOG("TxnMinByUsDLI::Authorize::call DBTransaction MatchTxnStatusforUpdate::Invalid Txn Id!!!\n");
                }
	}

/*
 *
 * Check Org Batch Valid:       1. Org Batch Process Type
 *                              2. Org Batch Id
 *				3. Org Batch Header
 *                              4. Org Batch Detail - Txn Header (TxnCode, SubStatus, Status, ArInd)
 *       
 */
        if (iRet == PD_OK) {
                iRet = CheckOrgBatchValid(hContext, hRequest, hBatchRec);
                if (iRet != PD_OK) {
                        PutField_Int(hContext,"internal_error",iRet);
                }
        }
...
/*
 *
 * Generate a New Txn:          1. RSP Delivery Out Txn     	- Debit Rsp Available Balance
 *                                                              - prev grp txn id
 *                                                              - Sub Status
 *  
 */
	if (iRet == PD_OK) { 
		iRet = PrcoessRspDeliveryOut(hContext, hRequest, hRspDeliInTxnRec, hRspDeliOutTxnRec);
	}

/*
 *
 * Generate a New Txn:          1. OPB Fund In Txn		- Credit Op Bank Balance
 *                                                              - prev grp txn id
 *                                                              - Sub Status	
 *  
 */
...
/*
 *
 * Generate a New Txn:          1. RSP Amt Diff Txn   		- Credit/Debit AR Balance
 * 								- prev grp txn id
 * 								- Sub Status
 *  
 */
...
/*
 *  
 * Generate a New Batch:        1. RSP Delivery Out Txn
 *                              2. OPB Fund In Txn
 *                              3. RSP Amt Diff Txn
 *                              4. RSP Delivery In Txn
 *                              5, RSP Settlement Txn
 *                              6. PSP Settlement Txn
 *
 */
	if (iRet == PD_OK) {
		iRet = ProcessBatch(hBatchRec, hRspDeliInTxnRec, hRspDeliOutTxnRec, hOpbFundInTxnRec, hRspAmtDiffTxnRec);
	}

/*
 *  
 * Update New Batch Txn:        1. RSP Delivery Out Txn		- Update Sub Status
 *                              2. OPB Fund In Txn		- Update Sub Status
 *                              3. RSP Amt Diff Txn		- Update Sub Status
 ...
 */
	if (iRet == PD_OK) {
		iRet = UpdateBatch(hBatchRec, hRspDeliInTxnRec, hRspDeliOutTxnRec);
	}

/*
 *
 * Update ACR:                  1. Sum of OPB Fund In Txn       - Credit ACR Balance
 *
 */
	if (iRet == PD_OK) {
                iRet = ProcessACR(hContext, hRequest, hRspDeliInTxnRec, hOpbFundInTxnRec, hBatchRec);
        }
...
/* txn_seq */
        if (GetField_CString(hBatchRec,"org_txn_seq",&csOrgTxnId)) {
DEBUGLOG(("CheckOrgBatchValid::org_txn_id = [%s]\n",csOrgTxnId));
        } else {
DEBUGLOG(("CheckOrgBatchValid::org_txn_id not found!!\n"));
ERRLOG("TxnMinByUsDLI::CheckOrgBatchValid::org_txn_id not found!!\n");
                iRet = INT_INVALID_TXN;
        }

	// Get Org Batch Id
	if (iRet == PD_OK) {
DEBUGLOG(("CheckOrgBatchValid::Call DBMiBatchDetail::GetLastBatchDtByTxnId:: org_txn_id [%s]\n",csOrgTxnId));
           	DBObjPtr = CreateObj(DBPtr,"DBMiBatchDetail","GetLastBatchDtByTxnId");
                iDtlRet = (unsigned long)(*DBObjPtr)(csOrgTxnId,hOrgBatchRec);
               	if (iDtlRet == PD_FOUND) {
			
/* process_type */
			if (GetField_CString(hOrgBatchRec,"process_type",&csOrgProcessType)) {
DEBUGLOG(("CheckOrgBatchValid::process_type = [%s]\n",csOrgProcessType));
   
				// Compare process type - PD_MI_TXN_TYPE_DELIV_IN
				if (strcmp(csOrgProcessType, PD_MI_TXN_TYPE_DELIV_IN)) {
DEBUGLOG(("CheckOrgBatchValid::process_type invalid!!\n"));
ERRLOG("TxnMinByUsDLI::CheckOrgBatchValid::process_type invalid!!\n");
                        		iRet = INT_ERR;		
				}
			}      
...
/* batch_id */
			if (iRet == PD_OK) {
                  	      	if (GetField_CString(hOrgBatchRec,"batch_id",&csOrgBatchId)) {
DEBUGLOG(("CheckOrgBatchValid::batch_id = [%s]\n",csOrgBatchId));
                                	PutField_CString(hBatchRec,"org_batch_id",csOrgBatchId);
                        	}
			}

		} else {
DEBUGLOG(("CheckOrgBatchValid::Call DBMiBatchDetail::GetLastBatchDtByTxnId Failed\n"));
                        iRet = INT_MI_BATCH_ID_NOT_FOUND;
		}
	}		

	// Check Org Batch Header
	if (iRet == PD_OK) {

DEBUGLOG(("CheckOrgBatchValid::Call DBMiBatchHeader:: GetBatchHeader\n"));
...
DEBUGLOG(("CheckOrgBatchValid::status = [%c]\n",cStatus));

				// Compare Status - PD_MI_BATCH_STATUS_INOPERATIVE
                                if (cStatus == PD_MI_BATCH_STATUS_INOPERATIVE) {
DEBUGLOG(("CheckOrgBatchValid::batch status is inoperative\n"));
ERRLOG("TxnMinByUsDLI::CheckOrgBatchValid::batch status is inoperative\n");
                                        iRet = INT_ERR;
                                }				
                        }
                } else {
DEBUGLOG(("CheckOrgBatchValid:::Call DBMiBatchHeader:: GetBatchHeader Failed\n"));
ERRLOG("TxnMinByUsDLI::CheckOrgBatchValid:::Call DBMiBatchHeader:: GetBatchHeader Failed\n");
                        iRet = INT_ERR;
                }
	}

	// Check Org Batch Detail - Txn Header
	if (iRet == PD_OK) {

		// Get Batch Detail
DEBUGLOG(("CheckOrgBatchValid::Call DBMiBatchDetail::GetAllDetailByBatchId:: org_batch_id [%s]\n",csOrgBatchId));
...
DEBUGLOG(("CheckOrgBatchValid::txn_id = [%s]\n",csTxnId));
                                     	PutField_CString(hOrgBatchRec,"txn_id",csTxnId);

					// Get Txn Header
DEBUGLOG(("CheckOrgBatchValid::Call DBTransaction::GetTxnHeader:: txn_id [%s]\n",csTxnId));
...
DEBUGLOG(("CheckOrgBatchValid::txn_code = [%s]\n",csTxnCode));
                                                          	PutField_CString(hOrgBatchRec,"txn_code",csTxnCode);
                                                      	}

/* sub_status */
                                                   	if (GetField_CString(hTxnHdRec,"sub_status",&csSubStatus)) {
DEBUGLOG(("CheckOrgBatchValid::sub_status = [%s]\n",csSubStatus));
                                                            	PutField_CString(hOrgBatchRec,"sub_status",csSubStatus);
                                                     	}
...
ERRLOG("TxnMinByUsDLI::CheckBatchValid:: sub_status invalid\n");
                                        				iRet = INT_ERR;
                                        				break;
                                				}
                        				}
                        				...
                        				{
                                				// Compare SubStatus - PD_DELIVERING (129)
...
ERRLOG("TxnMinByUsDLI::CheckBatchValid::txn_code invalid\n");
                                				iRet = INT_ERR;
                                				break;        
                        				}

/* status */
							if (GetField_Char(hTxnHdRec,"status",&cStatus)) {
DEBUGLOG(("CheckOrgBatchValid::status = [%c]\n",cStatus));
                                                                PutField_Char(hOrgBatchRec,"status",cStatus);

								// Compare Status - PD_COMPLETE ('C')
								if (cStatus != PD_COMPLETE) {
DEBUGLOG(("CheckBatchValid:: status invalid\n"));
ERRLOG("TxnMinByUsDLI::CheckBatchValid::status invalid\n");
                                                                        iRet = INT_ERR;
                                                                        break;
								}
                                                        }
...
/* ar_ind */
							if (GetField_Char(hTxnHdRec,"ar_ind",&cArInd)) {
DEBUGLOG(("CheckOrgBatchValid::ar_ind = [%c]\n",cArInd));
                                                                PutField_Char(hOrgBatchRec,"ar_ind",cArInd);

								// Compare ArInd - PD_ACCEPT ('A')
								if (cArInd != PD_ACCEPT) {
DEBUGLOG(("CheckBatchValid:: ar_ind invalid\n"));
ERRLOG("TxnMinByUsDLI::CheckBatchValid::ar_ind invalid\n");
                                                                        iRet = INT_ERR;
                                                                        break;
                                                                }
                                                        }
...
DEBUGLOG(("CheckOrgBatchValid::Call DBTransaction::GetTxnHeader Failed\n"));
ERRLOG("TxnMinByUsDLI::CheckOrgBatchValid::Call DBTransaction::GetTxnHeader Failed\n");
                                             	iRet = INT_ERR;
                                    	}
                            	}
...
DEBUGLOG(("CheckOrgBatchValid::Call DBMiBatchDetail::GetAllDetailByBatchId Failed\n"));
ERRLOG("TxnMinByUsDLI::CheckOrgBatchValid::Call DBMiBatchDetail::GetAllDetailByBatchId Failed\n");
                    	iRet = INT_ERR;
          	}
        }
...
DEBUGLOG(("CheckOrgBatchValid Normal Exit() iRet = [%d]\n",iRet));
        return iRet;
}
...
DEBUGLOG(("===== PrcoessRspDeliveryOut ===== START!!\n"));

	// Lock Org Txn + Get Org Txn
	iRet = GetRspDeliveryOutTxnInfo(hOrgTxnRec);
	if (iRet == INT_ERR) {
DEBUGLOG(("PrcoessRspDeliveryOut::GetRspDeliveryOutTxnInfo Failed\n"));
ERRLOG("TxnMinByUsDLI::PrcoessRspDeliveryOut::GetRspDeliveryOutTxnInfo Failed\n");
         	iRet = INT_INVALID_TXN;
            	PutField_Int(hContext,"internal_error",iRet);
       	}
	...
	// Form New Txn
	if (iRet == PD_OK) {
		iRet = FormRspDeliveryOutTxn(hContext, hRequest, hOrgTxnRec, hTxnRec);
	}	

	// Create Txn + Update Balance (with add approval_date, approval_timestamp)
	if (iRet == PD_OK) {
		iRet = ProcessRspDeliveryOutTxn(hContext, hRequest, hTxnRec);
	}

DEBUGLOG(("PrcoessRspDeliveryOut:: iRet [%d]\n", iRet));
DEBUGLOG(("===== PrcoessRspDeliveryOut ===== END !!\n"));	
...
DEBUGLOG(("GetRspDeliveryOutTxnInfo::txn_country = [%s]\n",csTmp));
                                        ...
                                        PutField_CString(hTxnRec,"opb_txn_country",csTmp);
                                }
...
DEBUGLOG(("GetRspDeliveryOutTxnInfo::Call TxnMiDetail:GetTxnMiDetail: txn_country = [%s]\n", csTmp));
                                	PutField_CString(hTxnRec,"txn_country",csTmp);
                                	PutField_CString(hTxnRec,"opb_mi_country",csTmp);
				}
...
DEBUGLOG(("GetRspDeliveryOutTxnInfo::Call TxnMiDetail:GetTxnMiDetail: remit_rate = [%f]\n", dTmp));
...
DEBUGLOG(("FormRspDeliveryOutTxn:: txn_seq [%s]\n", csTmp));
        }
...
DEBUGLOG(("FormRspDeliveryOutTxn (TxnLog):: channel_code [%s]\n", csTmp));
                	PutField_CString(hTxnRec, "channel_code", csTmp);
        	}		

/* service_code */
                if (GetField_CString(hOrgTxnRec, "service_code", &csTmp)) {
DEBUGLOG(("FormRspDeliveryOutTxn (TxnLog):: service_code [%s]\n", csTmp));
                        PutField_CString(hTxnRec, "service_code", csTmp);
                }

/* status */
                PutField_Char(hTxnRec,"status",PD_PROCESSING);
DEBUGLOG(("FormRspDeliveryOutTxn (TxnLog):: status [%c]\n", PD_PROCESSING));

/* txn_code */
                PutField_CString(hTxnRec, "txn_code", PD_MI_TXN_CODE_RSP_DELIVERY_OUT);
DEBUGLOG(("FormRspDeliveryOutTxn (TxnLog):: txn_code [%s]\n", PD_MI_TXN_CODE_RSP_DELIVERY_OUT));

/* process_code */
                PutField_CString(hTxnRec,"process_code", PD_PROCESS_CODE_DEF);
DEBUGLOG(("FormRspDeliveryOutTxn (TxnLog):: process_code [%s]\n", PD_PROCESS_CODE_DEF));

/* process_type */
                PutField_CString(hTxnRec,"process_type", PD_PROCESS_TYPE_DEF);
DEBUGLOG(("FormRspDeliveryOutTxn (TxnLog):: process_type [%s]\n", PD_PROCESS_TYPE_DEF));

/* response_code */
                PutField_CString(hTxnRec, "response_code", "0");
DEBUGLOG(("FormRspDeliveryOutTxn (TxnLog):: response_code [%s]\n", "0"));

/* internal_code */
                PutField_Int(hTxnRec, "internal_code", 0);
DEBUGLOG(("FormRspDeliveryOutTxn (TxnLog):: internal_code [%d]\n", 0));

/* host_posting_date */
                if (GetField_CString(hContext,"PHDATE",&csTmp)) {
DEBUGLOG(("FormRspDeliveryOutTxn (TxnLog):: PHDATE [%s]\n", csTmp));
                        PutField_CString(hTxnRec,"PHDATE",csTmp);
                }

/* transmission_datetime */
                if (GetField_CString(hContext, "local_tm_date", &csTmp)) {
DEBUGLOG(("FormRspDeliveryOutTxn (TxnLog):: local_tm_date [%s]\n", csTmp));
...
DEBUGLOG(("FormRspDeliveryOutTxn (TxnLog):: local_tm_time [%s]\n", csTmp));
...
DEBUGLOG(("FormRspDeliveryOutTxn (TxnLog):: net_ccy = [%s]\n",csTmp));
                  	PutField_CString(hTxnRec,"net_ccy",csTmp);
...
DEBUGLOG(("FormRspDeliveryOutTxn (TxnLog):: net_amt = [%lf]\n",dTmp));
                      	PutField_Double(hTxnRec,"net_amt",dTmp);
               	}

/* txn_amt */
            	if (GetField_Double(hOrgTxnRec,"entity_txn_amount",&dTmp)) {
DEBUGLOG(("FormRspDeliveryOutTxn (TxnLog):: txn_amt = [%lf]\n",dTmp));
                   	PutField_Double(hTxnRec,"txn_amt",dTmp);
             	}

/* ex_supplier */
		 PutField_Char(hTxnRec,"ex_supplier",PD_INT_EX);
DEBUGLOG(("FormRspDeliveryOutTxn (TxnLog):: ex_supplier [%c]\n", PD_INT_EX));

/* ex_rate */
		PutField_Double(hTxnRec,"ex_rate",1);
DEBUGLOG(("FormRspDeliveryOutTxn (TxnLog):: ex_rate = [%lf]\n",1));
...
DEBUGLOG(("FormRspDeliveryOutTxn (TxnLog):: txn_country = [%s]\n", csTmp));
                        PutField_CString(hTxnRec, "txn_country", csTmp);
                }

/* opb_txn_country */
                if (GetField_CString(hOrgTxnRec, "opb_txn_country", &csTmp)) {
DEBUGLOG(("FormRspDeliveryOutTxn (TxnLog):: opb_txn_country = [%s]\n", csTmp));
                        PutField_CString(hContext, "opb_txn_country", csTmp);
                }
	}
...
DEBUGLOG(("FormRspDeliveryOutTxn:: entity_id = [%s]\n", csTmp));
                    	PutField_CString(hTxnRec,"entity_id",csTmp);
            	}

/* party_type */
             	if (GetField_CString(hOrgTxnRec,"party_type",&csTmp)) {
DEBUGLOG(("FormRspDeliveryOutTxn:: party_type = [%s]\n", csTmp));
                      	PutField_CString(hTxnRec,"party_type",csTmp);
               	}

/* party_id */
                if (GetField_CString(hOrgTxnRec,"party_id",&csTmp)) {
DEBUGLOG(("FormRspDeliveryOutTxn:: party_id = [%s]\n", csTmp));
                       	PutField_CString(hTxnRec,"party_id",csTmp);
             	}

/* txn_ccy */
            	if (GetField_CString(hOrgTxnRec,"entity_ccy",&csTmp)) {
DEBUGLOG(("FormRspDeliveryOutTxn:: txn_ccy = [%s]\n", csTmp));
                     	PutField_CString(hTxnRec,"txn_ccy",csTmp);
...
/* txn_country */
            	if (GetField_CString(hOrgTxnRec,"txn_country",&csTmp)) {
DEBUGLOG(("FormRspDeliveryOutTxn:: txn_country = [%s]\n", csTmp));
                      	PutField_CString(hTxnRec,"txn_country",csTmp);
                      	PutField_CString(hTxnRec,"country",csTmp);
              	}

/* opb_mi_country */
                if (GetField_CString(hOrgTxnRec,"opb_mi_country",&csTmp)) {
DEBUGLOG(("FormRspDeliveryOutTxn:: opb_mi_country = [%s]\n", csTmp));
                        PutField_CString(hContext, "opb_mi_country", csTmp);
                }

/* txn_product */
           	if (GetField_CString(hTxnRec,"product_code",&csTmp)) {
DEBUGLOG(("FormRspDeliveryOutTxn:: txn_product = [%s]\n", csTmp));
                      	PutField_CString(hTxnRec,"txn_product",csTmp);
             	}

/* txn_date */
                if (GetField_CString(hContext,"PHDATE",&csTmp)) {
DEBUGLOG(("FormRspDeliveryOutTxn:: txn_date = [%s]\n", csTmp));
                        PutField_CString(hTxnRec,"txn_date",csTmp);
                }

/* report_date */
                if (GetField_CString(hContext,"PHDATE",&csTmp)) {
DEBUGLOG(("FormRspDeliveryOutTxn:: report_date = [%s]\n", csTmp));
                        PutField_CString(hTxnRec,"report_date",csTmp);
                }
...
DEBUGLOG(("FormRspDeliveryOutTxn:: remittance_slip_date = [%s]\n", csTmp));
                      	PutField_CString(hTxnRec,"remittance_slip_date",csTmp);
              	}

/* converted_ccy */
              	if (GetField_CString(hOrgTxnRec,"converted_ccy",&csTmp)) {
DEBUGLOG(("FormRspDeliveryOutTxn::  converted_ccy = [%s]\n", csTmp));
                    	PutField_CString(hTxnRec,"converted_ccy",csTmp);
             	}

/* converted_amount */
              	if (GetField_Double(hOrgTxnRec,"converted_amount",&dTmp)) {
DEBUGLOG(("FormRspDeliveryOutTxn:: converted_amount = [%f]\n", dTmp));
                  	PutField_Double(hTxnRec,"converted_amount",dTmp);
              	}

/* remit_rate */
          	if (GetField_Double(hOrgTxnRec,"remit_rate",&dTmp)) {
DEBUGLOG(("FormRspDeliveryOutTxn:: remit_rate = [%f]\n", dTmp));
                   	PutField_Double(hTxnRec,"remit_rate",dTmp);
             	}

/* prev_grp_txn_id */
		if (GetField_CString(hOrgTxnRec,"txn_seq",&csTmp)) {
DEBUGLOG(("FormRspDeliveryOutTxn:: prev_grp_txn_id = [%s]\n", csTmp));
                        PutField_CString(hTxnRec,"prev_grp_txn_id",csTmp);
                        PutField_CString(hContext,"prev_grp_txn_id",csTmp);
                }

/* entity_ccy */
              	if (GetField_CString(hOrgTxnRec,"entity_ccy",&csTmp)) {
DEBUGLOG(("FormRspDeliveryOutTxn:: entity_ccy = [%s]\n", csTmp));
                       	PutField_CString(hTxnRec,"entity_ccy",csTmp);
            	}

/* entity_txn_amount */
           	if (GetField_Double(hOrgTxnRec,"entity_txn_amount",&dTmp)) {
DEBUGLOG(("FormRspDeliveryOutTxn:: entity_txn_amount = [%f]\n", dTmp));
                 	PutField_Double(hTxnRec,"entity_txn_amount",dTmp);
             	}

/* actual_cost */
                PutField_Double(hTxnRec,"actual_cost",0.0);
DEBUGLOG(("FormRspDeliveryOutTxn:: actual_cost = [%f]\n", 0.0));

/* acr_prorata */
               	PutField_Int(hTxnRec,"acr_prorata",PD_FALSE);
DEBUGLOG(("FormRspDeliveryOutTxn:: acr_prorata = [%d]\n", PD_FALSE));
	}

DEBUGLOG(("FormRspDeliveryOutTxn Normal Exit() iRet = [%d]\n",iRet));
        return iRet;
}
...
/* txn_seq */
        if (GetField_CString(hTxnRec, "txn_seq", &csTxnId)) {
DEBUGLOG(("ProcessRspDeliveryOutTxn:: txn_seq [%s]\n", csTxnId));
        }
...
DEBUGLOG(("ProcessRspDeliveryOutTxn::do_logging [%d]\n", iTmp));
				...
				// Update TxnHeader
DEBUGLOG(("ProcessRspDeliveryOutTxn::Call DBTransaction:: Update\n"));
              			DBObjPtr = CreateObj(DBPtr,"DBTransaction","Update");
               			iRet = (unsigned long)(*DBObjPtr)(hTxnRec);
             			if (iRet != PD_OK) {
DEBUGLOG(("ProcessRspDeliveryOutTxn::Call DBTransaction:: Update Failed\n"));
                  			iRet = INT_ERR;
              			}
			}
		}
...
DEBUGLOG(("ProcessRspDeliveryOutTxn::Call DBTransaction:: AddDetail\n"));
                                        DBObjPtr = CreateObj(DBPtr,"DBTransaction","AddDetail");
                                        iRet = (unsigned long)(*DBObjPtr)(hTxnRec);
                                        if (iRet != PD_OK) {
DEBUGLOG(("ProcessRspDeliveryOutTxn::Call DBTransaction:: Add Failed\n"));
                                                iRet = INT_ERR;
                                        }
                                }
...
DEBUGLOG(("ProcessRspDeliveryOutTxn::Call DBMiEntityBalAcct:: GetEntityBalAcct\n"));
...
DEBUGLOG(("ProcessRspDeliveryOutTxn::Call DBMiEntityBalAcct:: status = [%s]\n", csStatus));

                                // Check Entity Bal Acct Status
                                if (!strcmp(csStatus, PD_MI_ENTITY_BAL_ACCT_STATUS_CLOSE)) {
DEBUGLOG(("ProcessRspDeliveryOutTxn::Call DBMiEntityBalAcct:: GetEntityBalAcct Disabled\n"));
ERRLOG("TxnMinByUsDLI::ProcessRspDeliveryOutTxn::Call DBMiEntityBalAcct:: GetEntityBalAcct Disabled\n");
                                        iRet = INT_MMS_ENTITY_BAL_ACCT_DISABLED;
                                }
                        }

                } else {
DEBUGLOG(("ProcessRspDeliveryOutTxn::Call DBMiEntityBalAcct:: GetEntityBalAcct Not Found\n"));
ERRLOG("TxnMinByUsDLI::ProcessRspDeliveryOutTxn::Call DBMiEntityBalAcct:: GetEntityBalAcct Not Found\n");
...
DEBUGLOG(("ProcessRspDeliveryOutTxn::batch_mode = [%c]\n",PD_ONLINE));

        	        // amt_type
        	        PutField_CString(hTxnRec, "amt_type", PD_DR);
DEBUGLOG(("ProcessRspDeliveryOutTxn::amt_type = [%s]\n",PD_DR));

                	// bal_type
                	PutField_Char(hTxnRec, "bal_type", PD_MI_ENTITY_POOL_ACCT_BAL);
DEBUGLOG(("ProcessRspDeliveryOutTxn::bal_type = [%c]\n",PD_MI_ENTITY_POOL_ACCT_BAL));	
		
			// UpdateEntityBalance
DEBUGLOG(("ProcessRspDeliveryOutTxn::Call BOMiEntityBalance:: UpdateEntityBalance\n"));
                	BOObjPtr = CreateObj(BOPtr, "BOMiEntityBalance", "UpdateEntityBalance");
                	iRet = (unsigned long)(*BOObjPtr)(hTxnRec,hTxnRec);
                	if (iRet == PD_OK) {
DEBUGLOG(("ProcessRspDeliveryOutTxn::Call BOMiEntityBalance:: UpdateEntityBalance Success\n"));

                        	if (GetField_CString(hTxnRec, "approval_date", &csApprovalDate)) {
DEBUGLOG(("ProcessRspDeliveryOutTxn::Call BOMiEntityBalance:: approval_date [%s]\n", csApprovalDate));
                        	}

                        	if (GetField_CString(hTxnRec, "approval_timestamp", &csApprovalTimestamp))  {
DEBUGLOG(("ProcessRspDeliveryOutTxn::Call BOMiEntityBalance:: approval_timestamp [%s]\n", csApprovalTimestamp));
                        	}

                        	// Open Balance
                        	if (GetField_Double(hTxnRec, "open_acct_bal", &dTmp)) {
DEBUGLOG(("ProcessRspDeliveryOutTxn::Call BOMiEntityBalance:: open_acct_bal = [%f]\n", dTmp));
                        	}

                        	if (GetField_Double(hTxnRec, "open_intransit", &dTmp)) {
DEBUGLOG(("ProcessRspDeliveryOutTxn::Call BOMiEntityBalance:: open_intransit = [%f]\n", dTmp));
                        	}

                        	if (GetField_Double(hTxnRec, "open_ar_bal", &dTmp)) {
DEBUGLOG(("ProcessRspDeliveryOutTxn::Call BOMiEntityBalance:: open_ar_bal = [%f]\n", dTmp));
                        	}

                        	// Close Balance
                        	if (GetField_Double(hTxnRec, "acct_bal", &dTmp)) {
DEBUGLOG(("ProcessRspDeliveryOutTxn::Call BOMiEntityBalance:: closing acct_bal = [%f]\n", dTmp));
                        	}

                        	if (GetField_Double(hTxnRec, "intransit", &dTmp)) {
DEBUGLOG(("ProcessRspDeliveryOutTxn::Call BOMiEntityBalance:: closing intransit = [%f]\n", dTmp));
                        	}

                        	if (GetField_Double(hTxnRec, "ar_bal", &dTmp)) {
DEBUGLOG(("ProcessRspDeliveryOutTxn::Call BOMiEntityBalance:: closing ar_bal = [%f]\n", dTmp));
                        	}

                	} else {
DEBUGLOG(("ProcessRspDeliveryOutTxn::Call BOMiEntityBalance:: UpdateEntityBalance Failed\n"));
ERRLOG("TxnMinByUsDLI::ProcessRspDeliveryOutTxn::Call BOMiEntityBalance:: UpdateEntityBalance Failed\n");
                	}
		}	
	}
...
DEBUGLOG(("ProcessRspDeliveryOutTxn::do_logging [%d]\n", iTmp));
                        ...
                        if (iTmp != PD_ADD_LOG) {
                                ...
                                // Add TxnMiDetail
                                if (iRet == PD_OK) {
DEBUGLOG(("ProcessRspDeliveryOutTxn::Call DBTxnMiDetail:: Add\n"));
                                        DBObjPtr = CreateObj(DBPtr,"DBTxnMiDetail","Add");
                                        iRet = (unsigned long)(*DBObjPtr)(hTxnRec);
                                        if (iRet != PD_OK) {
DEBUGLOG(("ProcessRspDeliveryOutTxn::Call DBTxnMiDetail:: Add Failed\n"));
                                                iRet = INT_ERR;
                                        }
                                }
                        }
...
DEBUGLOG(("ProcessRspDeliveryOutTxn::do_logging [%d]\n", iTmp));
                        ...
                        if (iTmp != PD_ADD_LOG) {
                                ...
                                // Add MiTxnLog
                                if (iRet == PD_OK) {
DEBUGLOG(("ProcessRspDeliveryOutTxn::Call DBTransaction:: AddMiTxnLog\n"));
                                        DBObjPtr = CreateObj(DBPtr,"DBTransaction","AddMiTxnLog");
                                        iRet = (unsigned long)(*DBObjPtr)(csTxnId);
                                        if (iRet != PD_OK) {
DEBUGLOG(("ProcessRspDeliveryOutTxn::Call DBTransaction:: AddMiTxnLog Failed\n"));
                                                iRet = INT_ERR;
                                        }
                                }
                        }
...
DEBUGLOG(("ProcessRspDeliveryOutTxn::status [%c]\n", PD_COMPLETE));

/* ar_ind */
                PutField_Char(hTxnRec,"ar_ind",PD_ACCEPT);
DEBUGLOG(("ProcessRspDeliveryOutTxn::ar_ind [%c]\n", PD_ACCEPT));

/* sub_status */
                PutField_CString(hTxnRec,"sub_status",PD_APPROVED);
DEBUGLOG(("ProcessRspDeliveryOutTxn::sub_status [%s]\n", PD_APPROVED));

/* approval_date */
                PutField_CString(hTxnRec, "approval_date", csApprovalDate);
DEBUGLOG(("ProcessRspDeliveryOutTxn::approval_date [%s]\n", csApprovalDate));

/* approval_timestamp */
                PutField_CString(hTxnRec, "approval_timestamp", csApprovalTimestamp);
DEBUGLOG(("ProcessRspDeliveryOutTxn::approval_timestamp [%s]\n", csApprovalTimestamp));
...
DEBUGLOG(("ProcessRspDeliveryOutTxn Normal Exit() iRet = [%d]\n",iRet));
        return iRet;
}
...
DEBUGLOG(("ProcessOpbFundIn::[%s] = [NULL]\n",csTag));
					RemoveField_CString(hTxnRec,"deliver_date");
				}
...
DEBUGLOG(("ProcessOpbFundInTxn:: Call MGTChannel: AddTxnLog Failed\n"));
	...
	if (iRet == PD_OK) {
		...
		// Check Entity Balance Acct Exist
DEBUGLOG(("ProcessOpbFundInTxn::Call DBMiEntityBalAcct:: GetEntityBalAcct\n"));
                DBObjPtr = CreateObj(DBPtr, "DBMiEntityBalAcct", "GetEntityBalAcct");
                iDtlRet = (unsigned long)(*DBObjPtr)(hTxnRec,hTxnRec);
                if (iDtlRet == PD_FOUND) {
...
/* status */
                        if (GetField_CString(hTxnRec, "status", &csStatus)) {
DEBUGLOG(("ProcessOpbFundInTxn::Call DBMiEntityBalAcct:: status = [%s]\n", csStatus));

                                // Check Entity Bal Acct Status
                                if (!strcmp(csStatus, PD_MI_ENTITY_BAL_ACCT_STATUS_CLOSE)) {
DEBUGLOG(("ProcessOpbFundInTxn::Call DBMiEntityBalAcct:: GetEntityBalAcct Disabled\n"));
ERRLOG("TxnMinByUsDLI::ProcessOpbFundInTxn::Call DBMiEntityBalAcct:: GetEntityBalAcct Disabled\n");
                                        iRet = INT_MMS_ENTITY_BAL_ACCT_DISABLED;
                                }
                        }

                } else {
DEBUGLOG(("ProcessOpbFundInTxn::Call DBMiEntityBalAcct:: GetEntityBalAcct Not Found\n"));
ERRLOG("TxnMinByUsDLI::ProcessOpbFundInTxn::Call DBMiEntityBalAcct:: GetEntityBalAcct Not Found\n");
                        iRet = INT_MMS_ENTITY_BAL_ACCT_NOT_FOUND;
                }
...
/* opb_txn_country */
		if (GetField_CString(hContext, "opb_txn_country", &csTmp)) {
DEBUGLOG(("ProcessOpbFundInTxn:: opb_txn_country = [%s]\n", csTmp));
			PutField_CString(hTxnRec, "txn_country", csTmp);
               	}	
...
/* opb_mi_country */
                if (GetField_CString(hContext, "opb_mi_country", &csTmp)) {
DEBUGLOG(("ProcessOpbFundInTxn:: opb_mi_country = [%s]\n", csTmp));
                        PutField_CString(hTxnRec, "txn_country", csTmp);
                }
...
/* txn_date */
		if (GetField_CString(hContext,"PHDATE",&csTmp)) {
DEBUGLOG(("ProcessOpbFundInTxn:: txn_date = [%s]\n", csTmp));
			PutField_CString(hTxnRec,"txn_date",csTmp);
		}		
...
DEBUGLOG(("ProcessOpbFundInTxn::deliver_date = [%s]\n",csTmp));
			...
			RemoveField_CString(hTxnRec,"report_date");
		}
...
/* prev_grp_txn_id */
                if (GetField_CString(hContext,"prev_grp_txn_id",&csTmp)) {
DEBUGLOG(("ProcessOpbFundInTxn::prev_grp_txn_id = [%s]\n", csTmp));
                        PutField_CString(hTxnRec,"prev_grp_txn_id",csTmp);
                }

/* actual_cost */
                PutField_Double(hTxnRec,"actual_cost",0.0);
DEBUGLOG(("ProcessOpbFundInTxn::actual_cost = [%f]\n", 0.0));
...
/* entity_id */
                if (GetField_CString(hOrgTxnRec,"entity_id",&csTmp)) {
DEBUGLOG(("FormRspAmtDiffTxn:: entity_id = [%s]\n", csTmp));
                        PutField_CString(hTxnRec,"entity_id",csTmp);
                }

/* party_type */
                if (GetField_CString(hOrgTxnRec,"party_type",&csTmp)) {
DEBUGLOG(("FormRspAmtDiffTxn:: party_type = [%s]\n", csTmp));
                        PutField_CString(hTxnRec,"party_type",csTmp);
                }

/* party_id */
                if (GetField_CString(hOrgTxnRec,"party_id",&csTmp)) {
DEBUGLOG(("FormRspAmtDiffTxn:: party_id = [%s]\n", csTmp));
                        PutField_CString(hTxnRec,"party_id",csTmp);
                }

/* txn_ccy */
		if (GetField_CString(hTxnRec,"amtdiff_ccy",&csTmp)) {
DEBUGLOG(("FormRspAmtDiffTxn:: txn_ccy = [%s]\n", csTmp));
                        PutField_CString(hTxnRec,"txn_ccy",csTmp);
                        PutField_CString(hTxnRec,"ccy",csTmp);
                }
...
/* txn_country */
                if (GetField_CString(hOrgTxnRec,"txn_country",&csTmp)) {
DEBUGLOG(("FormRspAmtDiffTxn:: txn_country = [%s]\n", csTmp));
                        PutField_CString(hTxnRec,"txn_country",csTmp);
                        PutField_CString(hTxnRec,"country",csTmp);
                }
...
/* txn_product */
                if (GetField_CString(hTxnRec,"product_code",&csTmp)) {
DEBUGLOG(("FormRspAmtDiffTxn:: txn_product = [%s]\n", csTmp));
                        PutField_CString(hTxnRec,"txn_product",csTmp);
                }
...
/* prev_grp_txn_id */
                if (GetField_CString(hContext,"prev_grp_txn_id",&csTmp)) {
DEBUGLOG(("FormRspAmtDiffTxn:: prev_grp_txn_id = [%s]\n", csTmp));
                        PutField_CString(hTxnRec,"prev_grp_txn_id",csTmp);
                }

/* entity_ccy */
		if (GetField_CString(hTxnRec,"amtdiff_ccy",&csTmp)) {
DEBUGLOG(("FormRspAmtDiffTxn:: entity_ccy = [%s]\n", csTmp));
                        PutField_CString(hTxnRec,"entity_ccy",csTmp);
                }

/* entity_txn_amount */
		if (GetField_Double(hTxnRec,"amtdiff_amt",&dTmp)) {
DEBUGLOG(("FormRspAmtDiffTxn: entity_txn_amount = [%f]\n", dTmp));
                        PutField_Double(hTxnRec,"entity_txn_amount",dTmp);
                }

/* actual_cost */
                PutField_Double(hTxnRec,"actual_cost",0.0);
DEBUGLOG(("FormRspAmtDiffTxn:: actual_cost = [%f]\n", 0.0));

/* acr_prorata */
                PutField_Int(hTxnRec,"acr_prorata",PD_FALSE);
DEBUGLOG(("FormRspAmtDiffTxn:: acr_prorata = [%d]\n", PD_FALSE));
        }
...
/* txn_seq */
        if (GetField_CString(hRspDeliInTxnRec,"txn_seq",&csTxnId)) {
DEBUGLOG(("ProcessBatch::txn_id = [%s]\n",csTxnId));
        } else {
DEBUGLOG(("ProcessBatch::txn_id not found!!\n"));
ERRLOG("TxnMinByUsDLI::ProcessBatch::txn_id not found!!\n");
                iRet = INT_INVALID_TXN;
        }

/* org_batch_id */
	if (GetField_CString(hBatchRec,"org_batch_id",&csOrgBatchId)) {
DEBUGLOG(("ProcessBatch::org_batch_id = [%s]\n",csOrgBatchId));
        } else {
DEBUGLOG(("ProcessBatch::org_batch_id not found!!\n"));
ERRLOG("TxnMinByUsDLI::ProcessBatch::org_batch_id not found!!\n");
                iRet = INT_MI_BATCH_ID_NOT_FOUND;
        }
...
DEBUGLOG(("ProcessBatch::Call DBMiBatchDetail::GetAllDetailByBatchId:: org_batch_id [%s]\n",csOrgBatchId));
                DBObjPtr = CreateObj(DBPtr,"DBMiBatchDetail","GetAllDetailByBatchId");
...
DEBUGLOG(("ProcessBatch::Call DBMiBatchDetail::GetAllDetailByBatchId:: txn_id = [%s]\n", csTmp));
                                }				
	
				hOrgBatchRec = RecordSet_GetNext(rOrgBatchRecordSet);
			}
		} else {
DEBUGLOG(("ProcessBatch::Call DBMiBatchDetail::GetAllDetailByBatchId Failed\n"));
                        iRet = INT_ERR;
		}
	}
	...
	// Generate Batch Id
	if (iRet == PD_OK) {
DEBUGLOG(("ProcessBatch::Call TxnSeq:: GetNextMiActionBatchSeq\n"));
...
DEBUGLOG(("ProcessBatch::Call TxnSeq:: GetNextMiActionBatchSeq: [%s]\n",csBatchId));
	}
	...
	// Add Batch Header
       	if (iRet == PD_OK) {
           	...
           	// process_type
             	PutField_CString(hBatchRec,"process_type",PD_MI_TXN_TYPE_DELIV_OUT);

              	// status
               	PutField_Char(hBatchRec,"status",PD_MI_BATCH_STATUS_NORMAL);

DEBUGLOG(("ProcessBatch::Call MiBatchHeader:: Add\n"));
            	DBObjPtr = CreateObj(DBPtr, "DBMiBatchHeader", "Add");
             	iRet = (unsigned long)(*DBObjPtr)(hBatchRec);
             	if (iRet != PD_OK) {
                     	iRet = INT_ERR;
DEBUGLOG(("ProcessBatch::Call MiBatchHeader:: Add Failed\n"));
            	}
      	}
...
DEBUGLOG(("ProcessBatch::seq = [%d]\n", iSeq));
		...
		// RSP Amt Diff
                if (iRet == PD_OK) {
DEBUGLOG(("ProcessBatch::RSP Amt Diff\n"));
...
DEBUGLOG(("ProcessBatch::seq = [%d]\n", iSeq));
		...
		if (iRet == PD_OK) {
DEBUGLOG(("ProcessBatch::PSP Settlement, RSP Settlement and RSP Delivery Out\n"));
...
/* txn_id */
                              		if (GetField_CString(hOrgBatchRec, "txn_id", &csTmp)) {
DEBUGLOG(("ProcessBatch::batch_txn_id = [%s]\n", csTmp));
						PutField_CString(hBatchRec,"txn_id",csTmp);
					}
...
DEBUGLOG(("ProcessBatch::Call MiBatchRelation: Add \n"));
              	DBObjPtr = CreateObj(DBPtr, "DBMiBatchRelation", "Add");
              	iRet = (unsigned long)(*DBObjPtr)(hBatchRec);
              	if (iRet != PD_OK) {
                    	iRet = INT_ERR;
DEBUGLOG(("ProcessBatch::Call MiBatchRelation:: Add Failed\n"));
		}
	}	
...
DEBUGLOG(("ProcessBatch Normal Exit() iRet = [%d]\n",iRet));
        return iRet;
}
...
/* party_type */
       	if (GetField_CString(hTxnRec,"party_type",&csPartyType)) {
...
DEBUGLOG(("UpdateBatch:: upate_user [%s]\n", csTmp));
                PutField_CString(hTxnHeader, "update_user", csTmp);
                PutField_CString(hTxnMiDetail, "update_user", csTmp);
        }

/* batch_id */
        if (GetField_CString(hBatchRec,"batch_id",&csBatchId)) {
DEBUGLOG(("UpdateBatch::batch_id = [%s]\n",csBatchId));
        } else {
DEBUGLOG(("UpdateBatch::batch_id not found!!\n"));
ERRLOG("TxnMinByUsDLI::UpdateBatch::batch_id not found!!\n");
                iRet = INT_ERR;
        }

/* txn_seq (delivery in) */
    	if (GetField_CString(hRspDeliInTxnRec,"txn_seq",&csRspDeliOutTxnId)) {
DEBUGLOG(("UpdateBatch:: txn_seq (delivery in) = [%s]\n", csRspDeliOutTxnId));
            	PutField_CString(hTxnMiDetail,"txn_seq",csRspDeliOutTxnId);
    	}

/* txn_seq (delivery out) */
     	if (GetField_CString(hRspDeliOutTxnRec, "txn_seq", &csTmp)) {
DEBUGLOG(("UpdateBatch:: next_grp_txn_id [%s]\n", csTmp));
...
DEBUGLOG(("UpdateBatch::Call DBMiBatchDetail::GetAllDetailByBatchId:: batch_id [%s]\n",csBatchId));
                DBObjPtr = CreateObj(DBPtr,"DBMiBatchDetail","GetAllDetailByBatchId");
...
DEBUGLOG(("UpdateBatch::Call DBMiBatchDetail::GetAllDetailByBatchId:: oper_ind = [%c]\n", cOperInd));

/* txn_id */
                                                if (GetField_CString(hRec, "txn_id", &csTxnId)) {
DEBUGLOG(("UpdateBatch:: txn_id [%s]\n", csTxnId));
							PutField_CString(hTxnHeader, "txn_seq", csTxnId);

/* sub_status */
        						PutField_CString(hTxnHeader, "sub_status", PD_DELIVERED);
DEBUGLOG(("UpdateBatch:: sub_status [%s]\n", PD_DELIVERED));						

							// Update Txn Header	
							if (iRet == PD_OK) {
DEBUGLOG(("UpdateBatch::Call DBTransaction:: Update\n"));
                                        			DBObjPtr = CreateObj(DBPtr,"DBTransaction","Update");
                                        			iRet = (unsigned long)(*DBObjPtr)(hTxnHeader);
                                        			if (iRet != PD_OK) {
DEBUGLOG(("UpdateBatch::Call DBTransaction:: Update Failed\n"));
                                                			iRet = INT_ERR;
                                        			}	
							}
							...
							// Update TxnMiDetail
							if (iRet == PD_OK) {
								if (!strcmp(csTxnId, csRspDeliOutTxnId)) {
DEBUGLOG(("UpdateBatch::Call DBTxnMiDetail:: Update\n"));
							                DBObjPtr = CreateObj(DBPtr,"DBTxnMiDetail","Update");
                							iRet = (unsigned long)(*DBObjPtr)(hTxnMiDetail);
                							if (iRet != PD_OK) {
DEBUGLOG(("UpdateBatch::Call DBTxnMiDetail:: Update Failed\n"));
                        							iRet = INT_ERR;
                							}
...
DEBUGLOG(("UpdateBatch::Call DBMiBatchDetail::GetAllDetailByBatchId Failed\n"));
                        iRet = INT_ERR;
                }
        }
...
DEBUGLOG(("UpdateBatch Normal Exit() iRet = [%d]\n",iRet));
        return iRet;
}
...
/* net_ccy */
       		if(GetField_CString(hRspDeliInTxnRec,"net_ccy",&csFrCcy)){
...
/* txn_amt */
       		if(GetField_Double(hRspDeliInTxnRec,"txn_amt",&dTmp)){
