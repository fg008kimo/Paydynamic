...
DEBUGLOG(("Authorize::bal_type = [%c]\n", cBalType));
			...
			PutField_Char(hTxn, "bal_type", PD_MERCH_AVA_SETT_POOL);
DEBUGLOG(("Authorize::bal_type = [%c] (default)\n", PD_MERCH_AVA_SETT_POOL));
		}
...
DEBUGLOG(("Authorize:: [%c] (default) apply_fee [%d]\n", cPartyType, iTmp));
		...
		//PutField_Char(hTxn, "bal_type", PD_PSP_AVAILABLE_POOL);
//DEBUGLOG(("Authorize::bal_type = [%c] (default)\n", PD_PSP_AVAILABLE_POOL));
	}
