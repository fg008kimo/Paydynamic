...
-------------------------------                    ------------            --------------
Init Version                                       2018/12/14              David Wong
*/
...
OBJPTR(DB);
OBJPTR(BO);
int     UpdatePreXDQRespTxnLog(hash_t *hContext,
                	hash_t* hResponse);

int     AddXDQTxnLog(hash_t *hContext,
                const hash_t* hRequest);

void TxnWebOnUs2XDQ(char    cdebug)
{
        cDebug = cdebug;
}
int     AddXDQTxnLog(hash_t *hContext,
...

{
	...
	hash_t	*hTxn;
	hash_t	*hRec;
DEBUGLOG(("TxnWebOnUs2XDQ::UpdateRespTxnLog()\n"));
...
DEBUGLOG(("UpdateTxnLog  iRet = [%d] after Insert TxnQrRequestLog\n",iRet));
		...
		int	iResult;
        	...
        	iRet = (unsigned long)(*DBObjPtr)(PD_CHANNEL_XJ_DBAO_QR,&iResult,csTmpStatus);
...
DEBUGLOG(("process_resp_txn()\n"));
	...
	PutField_CString(hContext,"psp_channel_code",PD_CHANNEL_XJ_DBAO_QR);
	PutField_CString(hContext,"return_psp_channel",PD_CHANNEL_XJ_DBAO_QR);
	...
	GetField_CString(hRequest,"psp_name",&csPspName);
      	MsgObjPtr = CreateObj(MsgPtr,"XdqMsg","BreakDownInitRspMsg");

        if ((*MsgObjPtr)(hResponse,inMsg->msg,inMsg->len) != PD_OK)  {
		iRet = INT_BREAKDOWN_ERR;
		PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("process_resp_txn:BreakDown XdqMsg Error\n"));
	}
//	else {
		// testing
...
DEBUGLOG(("process_resp_txn() call Update\n"));
		iRet = UpdatePreXDQRespTxnLog(hContext,hResponse);
...
DEBUGLOG(("TxbWebOnUs2XDQ: time_redirect = [%d]\n",iTmp));
			...
			PutField_Int((hash_t*)hRequest,"is_init_req",PD_TRUE);
       		 	BOObjPtr = CreateObj(BOPtr,"BOSecurity","GeneratePspSign");
       		 	if ((*BOObjPtr)(hContext,hRequest,hRequest) == PD_OK) {
			
       		 		MsgObjPtr = CreateObj(MsgPtr,"XdqMsg","FormatInitMsg");
...
DEBUGLOG(("TxnWebOnUs2XDQ: after process_resp_err_txn iret = [%d]\n",iRet));
					}
					else {
						msg->mtext[iRecvLen] = '\0';
DEBUGLOG(("TxnWebOnUs2XDQ:  recv len = [%d]\n",iRecvLen));
DEBUGLOG(("TxnWebOnUs2XDQ:  recv = [%s]\n",&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN));
						memcpy(h_msg->msg,&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN);
						iRet = process_resp_txn(hContext,h_msg,hRequest);
DEBUGLOG(("TxnWebOnUs2XDQ: after process_resp_txn iret = [%d]\n",iRet));
					}
                		}
        			FREE_ME(msg);
        		}     
...
DEBUGLOG(("TxnWebOnUs2XDQ: BuildData Error\n"));
		}
	}
...
DEBUGLOG(("TxnWebOnUs2XDQ: noraml exit iret = [%d]\n",iRet));
	return iRet;
}
