...
-------------------------------                    ------------            --------------
Init Version                                       2017/04/05              Elvis Wong
Add baid for DBOLTxnPspDetail(Update)              2019/06/11              Elvis Wong
*/
...
DEBUGLOG(("Authorize::Call DBOLPayoutRequest:GetOLPayoutRequest by org_txn_seq = [%s]\n", csOrgTxnSeq));
...
DEBUGLOG(("Authorize::Call DBOLPayoutRequest:GetOLPayoutRequest::status = [%d]\n", iStatus));
...
DEBUGLOG(("Authorize::Call DBOLPayoutRequest:GetOLPayoutRequest::status invalid!!\n"));
...
DEBUGLOG(("Authorize::Call DBOLPayoutRequest:GetOLPayoutRequest not found record for [%s]\n",csOrgTxnSeq));
ERRLOG("TxnOmtByUsVPI::Authorize::Call DBOLPayoutRequest:GetOLPayoutRequest not found!!\n");
	...
	if (iRet == PD_OK) {
		...
		if (iRefundParty == PD_REFUND_PAYOUT_TXN_GENERATED) {
			...
			// Get Generated Txn
DEBUGLOG(("Authorize::Call DBOLPayoutGeneratedFileDT:GetApiFileDetailByUploadTxnId by org_txn_seq = [%s]\n", csOrgTxnSeq));
...
DEBUGLOG(("GetFileDetailByUploadTxnId::generate_txn_cnt = [%d]\n",iUnMatchedCnt));
				...
				// no record
				if (iCnt == 0) {
DEBUGLOG(("Authorize::DBOLPayoutGeneratedFileDT:GetApiFileDetailByUploadTxnId Txn Not Generated. Cannot Void!!!\n"));
ERRLOG("TxnOmtByUsVPI::Authorize::DBOLPayoutGeneratedFileDT:GetApiFileDetailByUploadTxnId Txn Not Generated. Cannot Void!!!\n");
                                	iRet = INT_INVALID_TXN;
                                	PutField_Int(hContext,"internal_error",iRet);
                        	}
			} else {
DEBUGLOG(("Authorize::DBOLPayoutGeneratedFileDT:GetApiFileDetailByUploadTxnId Error!!!\n"));
ERRLOG("TxnOmtByUsVPI::Authorize::DBOLPayoutGeneratedFileDT:GetApiFileDetailByUploadTxnId Error!!!\n");
                                iRet = INT_ERR;
                                PutField_Int(hContext,"internal_error",iRet);
			}
		}
...
DEBUGLOG(("Authorize::Call GetTxnInfo by org_txn_seq = [%s]\n", csOrgTxnSeq));
		iRet = GetTxnInfo(csOrgTxnSeq, hContext, hData);
		if (iRet == PD_OK) {

/* status */
                	if (GetField_Char(hData, "status", &cStatus)) {
DEBUGLOG(("Authorize::GetTxnInfo::status = [%c]\n",cStatus));

 				// PD_COMPLETE - 'C'
                         	if (cStatus != PD_COMPLETE) {
DEBUGLOG(("Authorize::GetTxnInfo::status invalid!!\n"));
...
DEBUGLOG(("Authorize::GetTxnInfo::ar_ind = [%c]\n",cARInd));

				// PD_ACCEPT = 'A'
                               	if (cARInd != PD_ACCEPT) {
DEBUGLOG(("Authorize::GetTxnInfo::ar_ind invalid!!\n"));
...
DEBUGLOG(("Authorize::GetTxnInfo::sub_status = [%s]\n",csSubStatus));
 				...
 				// PD_APPROVED_AND_SENT - "128"
                            	...
                            	{
DEBUGLOG(("Authorize::GetTxnInfo::sub_status invalid!!\n"));
...
DEBUGLOG(("Authorize::GetTxnInfo::process_code = [%s]\n",csTmp));
...
DEBUGLOG(("Authorize::GetTxnInfo::process_type = [%s]\n",csTmp));
...
DEBUGLOG(("Authorize::GetTxnInfo::merchant_id = [%s]\n",csTmp));
...
DEBUGLOG(("Authorize::GetTxnInfo::merchant_ref = [%s]\n",csTmp));
                           	PutField_CString(hContext,"merchant_ref",csTmp);
                       	}

/* client_id */
                     	if (GetField_CString(hData,"client_id",&csTmp)) {
DEBUGLOG(("Authorize::GetTxnInfo::client_id = [%s]\n",csTmp));
...
DEBUGLOG(("Authorize::GetTxnInfo::service_code = [%s]\n",csTmp));
...
DEBUGLOG(("Authorize::GetTxnInfo::txn_amt = [%lf]\n",dTmp));
...
DEBUGLOG(("Authorize::GetTxnInfo::net_amt = [%lf]\n",dTmp));
                   	}

/* net_ccy */
                  	if (GetField_CString(hData,"net_ccy",&csTmp)) {
DEBUGLOG(("Authorize::GetTxnInfo::net_ccy = [%s]\n",csTmp));
                           	PutField_CString(hContext,"net_ccy",csTmp);
                     	}

/* txn_ccy */
                        if (GetField_CString(hData,"txn_ccy",&csTmp)) {
DEBUGLOG(("Authorize::GetTxnInfo::txn_ccy = [%s]\n",csTmp));
...
DEBUGLOG(("Authorize::GetTxnInfo::txn_country = [%s]\n",csTmp));
...
DEBUGLOG(("Authorize::GetTxnInfo::bank_name = [%s]\n",csTmp));
				PutField_CString(hContext,"bank_name",csTmp);
			}

/* branch */
			if (GetField_CString(hData,"branch",&csTmp)) {
DEBUGLOG(("Authorize::GetTxnInfo::branch = [%s]\n",csTmp));
...
DEBUGLOG(("Authorize::GetTxnInfo::account_id = [%s]\n",csTmp));
				PutField_CString(hContext,"account_id",csTmp);
			}	

/* account_name */
			if (GetField_CString(hData,"account_name",&csTmp)) {
DEBUGLOG(("Authorize::GetTxnInfo::account_name = [%s]\n",csTmp));
...
DEBUGLOG(("Authorize::GetTxnInfo::host_posting_date = [%s]\n",csOrgPostDate));
...
ERRLOG("TxnOmtByUsVPI::Authorize::GetTxnInfo:: not found record!!\n");
	...
	if (iRet == PD_OK) {
DEBUGLOG(("Authorize::Call DBOLTransaction:AddDetail for txn_seq = [%s]\n", csTxnSeq));
      		DBObjPtr = CreateObj(DBPtr,"DBOLTransaction","AddDetail");
         	if ((unsigned long)(*DBObjPtr)(hContext) != PD_OK) {
DEBUGLOG(("Authorize::DBOLTransaction:AddDetail Failed!!\n"));
ERRLOG("TxnOmtByUsVPI::Authorize::DBOLTransaction::AddDetail Failed!!\n");
                    	iRet = INT_ERR;
            	}
        }
...
/* amount */
                                	if (GetField_Double(hRec,"amount",&dTmp)) {
                                	        dPreFee += dTmp;
DEBUGLOG(("Authorize::Call DBOLTxnElements::GetFeeChgDetailByType::previous fee = [%lf]\n",dTmp));
                                	}
...
DEBUGLOG(("Authorize::Call BOFee:GetTxnFee::net_amt = [%f]\n",dTmp));
...
DEBUGLOG(("Authorize::Call BOFee:GetTxnFee::real net_amt = [%f]\n",(dTmp+dPreFee)));
                        	}

/* src_txn_fee_ccy */
				if (GetField_CString(hContext,"src_txn_fee_ccy",&csTmp)) {
DEBUGLOG(("Authorize::Call BOFee:GetTxnFee::src_txn_fee_ccy = [%s]\n",csTmp));
                                        PutField_CString(hTxnElement,"org_txn_ccy",csTmp);
                                }

/* src_txn_fee */
                                if(GetField_Double(hContext,"src_txn_fee",&dTmp)){
DEBUGLOG(("Authorize::Call BOFee:GetTxnFee::src_txn_fee = [%f]\n",dTmp));
...
DEBUGLOG(("Authorize::Call BOOLTxnElements::AddAvaForPOElement\n"));
...
ERRLOG("TxnOmtByUsVPI:Authorize::BOFee:GetTxnFee Error!!\n");
                        	iRet = INT_ERR;
                	}
        	}
	}
...
DEBUGLOG(("Authorize::Call DBOLPayoutRequest:Update for org_txn_seq = [%s]\n", csOrgTxnSeq));
                DBObjPtr = CreateObj(DBPtr,"DBOLPayoutRequest","Update");
                if ((*DBObjPtr)(hOrgTxn) != PD_OK) {
DEBUGLOG(("Authorize::Call DBOLPayoutRequest:Update Failed!!\n"));
...
DEBUGLOG(("Authorize::Call DBOLTransaction:Update for org_txn_seq = [%s]\n", csOrgTxnSeq));
...
DEBUGLOG(("Authorize:: generate_txn_cnt = [%d]\n",iGenTxnCnt));
		...
		if (iGenTxnCnt == 0) {
DEBUGLOG(("Authorize:: ***********************\n"));
DEBUGLOG(("Authorize:: ** Void Approved Txn **\n"));
DEBUGLOG(("Authorize:: ***********************\n"));
...
DEBUGLOG(("Authorize::Call BOOLBalance:UpdatePayoutAmount\n"));
...
DEBUGLOG(("Authorize::Call BOOLBalance:UpdatePayoutAmount Error!!\n"));
ERRLOG("TxnOmtByUsVPI:Authorize::Call BOOLBalance:UpdatePayoutAmount Error!!\n");
			...
			if (iRet == PD_OK) {
DEBUGLOG(("Authorize::Call DBOLTransaction:UpdateDetail for txn_seq = [%s]\n", csTxnSeq));
                                DBObjPtr = CreateObj(DBPtr,"DBOLTransaction","UpdateDetail");
                                if ((unsigned long)(*DBObjPtr)(hBalance) != PD_OK) {
DEBUGLOG(("Authorize::Call DBOLTransaction:UpdateDetail Error!!\n"));
...
DEBUGLOG(("Authorize::Call DBOLTransaction:Update for txn_seq = [%s]\n", csTxnSeq));
                              	DBObjPtr = CreateObj(DBPtr,"DBOLTransaction","Update");
                             	if ((unsigned long)(*DBObjPtr)(hContext) != PD_OK) {
DEBUGLOG(("Authorize::Call DBOLTransaction:Update Error!!\n"));
...
DEBUGLOG(("Authorize:: ************************\n"));
DEBUGLOG(("Authorize:: ** Void Generated Txn **\n"));
DEBUGLOG(("Authorize:: ************************\n"));
...
DEBUGLOG(("Authorize:: local_tm_time = [%s]\n",csTmp));
			...
			if (iRet == PD_OK) {
DEBUGLOG(("Authorize::Call isAllowProcess:: generate_txn_cnt = [%d]\n", iGenTxnCnt));
				iRet = isAllowProcess(hContext);
				if (iRet != PD_OK) {
DEBUGLOG(("Authorize::Call isAllowProcess:: Failure!!\n"));
				}
			}
...
DEBUGLOG(("Authorize:: version_no = [%d][%d]\n", i, iVersionNo));				
...
DEBUGLOG(("Authorize::Call DBOLPayoutGeneratedFileDT::UpdateByGenId\n"));
							DBObjPtr = CreateObj(DBPtr,"DBOLPayoutGeneratedFileDT","UpdateByGenId");
                                                	if ((unsigned long)(*DBObjPtr)(hOrgGenTxn) != PD_OK) {
DEBUGLOG(("Authorize::Call DBOLPayoutGeneratedFileDT::UpdateByGenId Error!!\n"));
...
/* txn_amt */
                                                	if (GetField_Double(hData,"txn_amt",&dTmp)) {
DEBUGLOG(("Authorize::GetTxnInfo::txn_amt = [%d][%lf]\n",i,dTmp));
...
DEBUGLOG(("Authorize::GetTxnInfo::display_amt = [%d][%lf]\n",i,dTmp));
...
/* txn_ccy */
							if (GetField_CString(hData,"txn_ccy",&csTmp)) {
DEBUGLOG(("Authorize::GetTxnInfo::txn_ccy = [%d][%s]\n",i,csTmp));
                                                        	PutField_CString(hGenTxn,"txn_ccy",csTmp);
                                                	}

/* txn_country */
                                                	if (GetField_CString(hData,"txn_country",&csTmp)) {
DEBUGLOG(("Authorize::GetTxnInfo::txn_country = [%d][%s]\n",i,csTmp));
                                                        	PutField_CString(hGenTxn,"txn_country",csTmp);
                                                	}

/* psp_txn_ccy */
							if (GetField_CString(hData,"psp_txn_ccy",&csTmp)) {
DEBUGLOG(("Authorize::GetTxnInfo::psp_txn_ccy = [%d][%s]\n",i,csTmp));
...
DEBUGLOG(("Authorize::GetTxnInfo::psp_id = [%d][%s]\n",i,csTmp));
...
DEBUGLOG(("Authorize::GetTxnInfo::baid = [%d][%s]\n",i,csTmp));
...
/* bank_code */
							if (GetField_CString(hData, "bank_code", &csTmp)) {
DEBUGLOG(("Authorize::GetTxnInfo::bank_code = [%d][%s]\n",i,csTmp));
                                                        	PutField_CString(hTxnPspDetail, "bank_code", csTmp);
                                                	}

/* psp_txn_amount */
                                                	if (GetField_Double(hData,"psp_txn_amount",&dTmp)) {
DEBUGLOG(("Authorize::GetTxnInfo::psp_txn_amount = [%d][%s]\n",i,csTmp));
...
DEBUGLOG(("Authorize::GetTxnInfo::cost = [%d][%lf]\n",i,dTmp));
...
DEBUGLOG(("Authorize::GetTxnInfo::grp_tracking_txn_seq = [%d][%s]\n",i,csTmp));
...
DEBUGLOG(("Authorize::GetTxnInfo::tracking_txn_seq = [%d][%s]\n",i,csTmp));
                                                        	sprintf(csTag, "tracking_txn_seq_%d", i);
                                                        	PutField_CString(hBalance, csTag, csTmp);
                                                	}
						}
					} else {
DEBUGLOG(("Authorize::GetTxnInfo:: not found record for [%d][%s]\n",i,csOrgGenTxnSeq));
ERRLOG("TxnOmtByUsVPI::Authorize::GetTxnInfo:: not found record!!\n");
                        			iRet = INT_NOT_RECORD;
						break;
					}
...
DEBUGLOG(("Authorize::Call OMTChannel:AddTxnLog for gen_txn_seq = [%d][%s]\n", i, csGenTxnSeq));
                                                ChannelObjPtr = CreateObj(ChannelPtr,"OMTChannel","AddTxnLog");
                                                if ((unsigned long)(*ChannelObjPtr)(hGenTxn,hGenTxn) != PD_OK) {
DEBUGLOG(("Authorize::Call OMTChannel:AddTxnLog Failed!!\n"));
ERRLOG("TxnOmtByUsVPI:Authorize::Call OMTChannel:AddTxnLog Failed!!\n");
					...
					if (iRet == PD_OK) {
DEBUGLOG(("Authorize::Call DBOLTransaction:AddDetail for gen_txn_seq = [%d][%s]\n", i, csGenTxnSeq));
						DBObjPtr = CreateObj(DBPtr,"DBOLTransaction","AddDetail");
...
DEBUGLOG(("Authorize::Call DBOLTxnPspDetail:Add for gen_txn_seq = [%d][%s]\n", i, csGenTxnSeq));
...
DEBUGLOG(("Authorize::Call BOOLTxnElements:VoidOrgTxnElements Failed!!\n"));
...
DEBUGLOG(("Authorize::Call DBOLPayoutGeneratedFileDT::UpdateByGenId for org_gen_txn_seq = [%d][%s]\n", i, csOrgGenTxnSeq));
                                           	DBObjPtr = CreateObj(DBPtr,"DBOLPayoutGeneratedFileDT","UpdateByGenId");
                                            	if ((unsigned long)(*DBObjPtr)(hOrgGenTxn) != PD_OK) {
DEBUGLOG(("Authorize::Call DBOLPayoutGeneratedFileDT::UpdateByGenId Error!!\n"));
ERRLOG("TxnOmtByUsVPI:Authorize::Call DBOLPayoutGeneratedFileDT::UpdateByGenId Error!!\n");
...
DEBUGLOG(("Authorize:: /**************************************** loop for generated txn: end: [%d][%d] ****************************************/\n", i, iGenTxnCnt));
				}
			}
...
DEBUGLOG(("Authorize::Call UpdatePayoutGenerateTxn:: succ_gen_txn_cnt = [%d]\n", iSuccGenTxnCnt));
                        		iRet = UpdatePayoutGenerateTxn(hSuccGenTxn);
                        		if (iRet != PD_OK) {
DEBUGLOG(("Authorize::Call UpdatePayoutGenerateTxn:: Failure!!\n"));
                        		}
                		}
			}
...
DEBUGLOG(("Authorize:: /**************************************** loop for success generated txn: start: [%d][%d] ****************************************/\n", i, iSuccGenTxnCnt));
...
DEBUGLOG(("Authorize:: org_psp_id = [%d][%s]\n", i, csTmpPsp));
...
DEBUGLOG(("Authorize:: org_dst_net_amt = [%d][%lf]\n", i, dTmp));
...
DEBUGLOG(("Authorize:: org_baid = [%d][%s]\n", i, csTmp));
...
DEBUGLOG(("Authorize::Call DBOLBankAcctId::GetBaidByProviderCatAcctType\n"));
...
DEBUGLOG(("Authorize::Call DBOLBankAcctId::GetBaidByProviderCatAcctType Failed!!\n"));
...
/* precal_fee */
					sprintf(csTag,"precal_fee_%d",i);
                        		if (GetField_Double(hBalance,csTag,&dTmp)) {
DEBUGLOG(("Authorize:: precal_fee = [%d][%lf]\n", i, dTmp));
...
DEBUGLOG(("Authorize:: party_type = [%d][%c]\n", i, PD_TYPE_PSP));
                                		} else {
DEBUGLOG(("Authorize:: version 1, skip party_type\n"));
                                        		continue;
                                		}
                        		} else {
                                		if (iVersionNo != 1) {
...
/* merchant_open_bal */
							if (GetField_Double(hBalance,"merchant_open_bal",&dTmp)) {
DEBUGLOG(("Authorize:: merchant_open_bal = [%d][%d]\n", i, dTmp));
                                        			PutField_Double(hBalance,"open_bal",dTmp);
                                			}

/* merchant_open_bal_settlement */
                                			if (GetField_Double(hBalance,"merchant_open_bal_settlement",&dTmp)) {
DEBUGLOG(("Authorize:: merchant_open_bal_settlement = [%d][%d]\n", i, dTmp));
                                        			PutField_Double(hBalance,"open_bal_settlement",dTmp);
                                			}

/* approval_date */
                                			if (GetField_CString(hBalance,"approval_date",&csTmp)) {
DEBUGLOG(("Authorize:: approval_date = [%d][%s]\n", i, csTmp));
...
/* approval_timestamp */
                                			if (GetField_CString(hBalance,"approval_timestamp",&csTmp)) {
DEBUGLOG(("Authorize:: approval_timestamp = [%d][%s]\n", i, csTmp));
                                        			PutField_CString(hGenTxn,"approval_timestamp",csTmp);
                                        			PutField_CString(hContext,"approval_timestamp",csTmp);
                                			}
						} else {
DEBUGLOG(("Authorize::Call BOOLBalance::UpdatePayoutAmount Failed\n"));
...
DEBUGLOG(("Authorize::Call DBOLTransaction:UpdateDetail for txn_seq = [%d][%s]\n", i, csTxnSeq));
                                        		DBObjPtr = CreateObj(DBPtr,"DBOLTransaction","UpdateDetail");
                                        		if ((unsigned long)(*DBObjPtr)(hBalance) != PD_OK) {
DEBUGLOG(("Authorize::Call DBOLTransaction:UpdateDetail Failed!!\n"));
...
DEBUGLOG(("Authorize::Call DBOLTransaction:Update for txn_seq = [%d][%s]\n", i, csTxnSeq));
                                                	DBObjPtr = CreateObj(DBPtr,"DBOLTransaction","Update");
                                                	if ((unsigned long)(*DBObjPtr)(hContext) != PD_OK) {
...
DEBUGLOG(("Authorize:: gen_txn_seq = [%d][%s]\n", i, csTmp));
...
DEBUGLOG(("Authorize:: net_ccy = [%d][%s]\n", i, csTmp));
...
DEBUGLOG(("Authorize:: txn_amt = [%d][%lf]\n", i, dTmp));
...
DEBUGLOG(("Authorize:: psp_total_hold = [%d][%lf]\n", i, dTmp));
...
DEBUGLOG(("Authorize:: fail_at_provider = [%d][%d]\n", i, iProviderFail));
                                		}

/* grp_tracking_txn_seq */
                                		sprintf(csTag,"grp_tracking_txn_seq_%d",i);
                                		if (GetField_CString(hBalance, csTag, &csTmp)){
DEBUGLOG(("Authorize:: grp_tracking_txn_seq = [%d][%s]\n", i, csTmp));
...
/* tracking_txn_seq */
                                		sprintf(csTag,"tracking_txn_seq_%d",i);
                                		if (GetField_CString(hBalance, csTag, &csTmp)){
DEBUGLOG(("Authorize:: tracking_txn_seq = [%d][%s]\n", i, csTmp));
...
DEBUGLOG(("Authorize::Call DBOLTxnPspDetail:Update for gen_txn_seq = [%d][%s]\n", i, csTmpTxnSeq));
...
DEBUGLOG(("Authorize::Call DBOLTxnPspDetail:Update Failed!!\n"));
...
DEBUGLOG(("Authorize::Call DBOLTransaction:Update for gen_txn_seq = [%d][%s]\n", i, csTmpTxnSeq));
                                       	 		DBObjPtr = CreateObj(DBPtr,"DBOLTransaction","Update");
                                        		if ((unsigned long)(*DBObjPtr)(hGenTxn) != PD_OK) {
DEBUGLOG(("Authorize::Call DBOLTransaction:Update Failed!!\n"));
ERRLOG("TxnOmtByUsVPI:Authorize::Call DBOLTransaction:Update Failed!!\n");
...
DEBUGLOG(("Authorize::Call DBOLTxnRelation::Add [%d][P1:%s] ---- %c ----> [P2:%s]\n", i, csP1TxnId, PD_REL_VOID, csP2TxnId));
                                        		DBObjPtr = CreateObj(DBPtr,"DBOLTxnRelation","Add");
                                        		if ((unsigned long)(*DBObjPtr)(hGenTxn) != PD_OK) {
DEBUGLOG(("Authorize::Call DBOLTxnRelation::Add Failed!!\n"));
...
DEBUGLOG(("Authorize:: /**************************************** loop for success generated txn: end: [%d][%d] ****************************************/\n", i, iSuccGenTxnCnt));
				}	
			}
...
//DEBUGLOG(("GetTxnInfo (TxnPspDeatil)::psp_id = [%s]\n",csTmp));
...
DEBUGLOG(("isAllowProcess::Call DBOLPayoutReturn:CheckPayoutRtnAllow::unrecon_cnt[%d][%d] total[%d][%d]\n", i, iUnReconCnt, i, iTotalUnReconCnt));
                                }

/* recon_cnt */
                                if (GetField_Int(hGenTxn, "recon_cnt", &iReconCnt)) {
                                        iTotalReconCnt = iTotalReconCnt + iReconCnt;
DEBUGLOG(("isAllowProcess::Call DBOLPayoutReturn:CheckPayoutRtnAllow::recon_cnt[%d][%d] total[%d][%d]\n", i, iReconCnt, i, iTotalReconCnt));
...
DEBUGLOG(("isAllowProcess::Call DBOLPayoutReturn:CheckPayoutRtnAllow::level[%d][%d]\n", i, iLevel));
...
DEBUGLOG(("isAllowProcess::Call DBOLPayoutReturn:CheckPayoutRtnAllow::fail_at_provider[%d][%d]\n", i, PD_TRUE));
...
DEBUGLOG(("isAllowProcess::Call DBOLPayoutReturn:CheckPayoutRtnAllow::Not Pass to continue...\n"));
				iRet = INT_INVALID_TXN;
                                break;
                        }
                }
...
DEBUGLOG(("UpdatePayoutGenerateTxn:Call DBOLPayoutReturn::GetPayoutRtnList::get txn_seq [%d][%s]\n", i, csTmpTxnSeq));
                                        }

/* txn_seq_value */
                                        sprintf(csTag, "txn_seq_value_%s", csTmpTxnSeq);
                                        if (!GetField_CString(hPayoutRtnList, csTag, &csTmpTxnSeqVal)) {
DEBUGLOG(("UpdatePayoutGenerateTxn:Call DBOLPayoutReturn::GetPayoutRtnList::add txn_seq into list [%d[%s]\n", i, csTmpTxnSeq));
...
DEBUGLOG(("UpdatePayoutGenerateTxn:Call DBOLPayoutReturn::GetPayoutRtnList::get recon_status [%d][%c]\n", i, cReconStatus));
...
DEBUGLOG(("UpdatePayoutGenerateTxn: txn_cnt [%d]\n", iTxnCnt));
...
DEBUGLOG(("UpdatePayoutGenerateTxn: prepare Updat update_user [%d][%s]\n", i, csTmp));		
                                        PutField_CString(hOrgGenTxn,"update_user",csTmp);
                                }
