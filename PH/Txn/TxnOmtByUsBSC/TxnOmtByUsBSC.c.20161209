...
Change GetDtlByBankAcct to GetBankAcctIdDtl	   2015/02/11		   Elvis Wong
*/
...
/* Lock file_id */
	if (iRet == PD_OK) {
DEBUGLOG(("Authorize() call DBOLStatement::GetFileIdForUpdateNoWait()\n"));
		DBObjPtr = CreateObj(DBPtr, "DBOLStatement", "GetFileIdForUpdateNoWait");
		iTmpRet = (unsigned long)(*DBObjPtr)(csFileId);
		if (iTmpRet != PD_OK) {
			iIsAlreadyLocked = PD_TRUE;
			iRet = INT_ERR;
			PutField_Int(hContext, "internal_error", iRet);
DEBUGLOG(("Authorize() call DBOLStatement::GetFileIdForUpdateNoWait() FAILURE!!!\n"));
ERRLOG("TxnOmtByUsBSC::Authorize() call DBOLStatement::GetFileIdForUpdateNoWait() FAILURE!!!\n");
		}
	}
                  	...
                  	DBObjPtr = CreateObj(DBPtr, "DBOLBankAcctId", "GetBankAcctIdDtl");
                      	if ((unsigned long)(*DBObjPtr)(csBAID, hRec) != PD_OK) {
                          	iRet = INT_INVALID_BAID;
                             	PutField_Int(hContext, "internal_error", iRet);
DEBUGLOG(("Authorize() call DBOLBankAcctId::GetBankAcctIdDtl() FAILURE!!!\n"));
ERRLOG("TxnOmtByUsBSC::Authorize() call DBOLBankAcctId::GetBankAcctIdDtl() FAILURE!!!\n");
                  	} else {
                             	if (!GetField_CString(hRec,"org_baid",&csTmp)) {
                                 	iRet = INT_ERR;
DEBUGLOG(("Authorize() call DBOLBankAcctId::GetBankAcctIdDtl() baid NOT FOUND!!!\n"));
                           	}
				...
				if (iRet == PD_OK) {
					if (GetField_CString(hRec,"psp_id",&csPspId)) {
						PutField_CString(hContext, "psp_id", csPspId);
                                               	...
                                               	iRet = INT_ERR;
DEBUGLOG(("Authorize() call DBOLBankAcctId::GetBankAcctIdDtl() psp_id NOT FOUND!!!\n"));
					}
                               	}
                    	}			
		}
...
DEBUGLOG(("Authorize() iDtlRet = [%d]\n", iDtlRet));

					if (WIFEXITED(iDtlRet)) {
						int status = WEXITSTATUS(iDtlRet);
DEBUGLOG(("Authorize() status = [%d]\n", status));
						if (status == SUCCESS) {
