...
-------------------------------                    ------------            --------------
Init Version                                       2014/07/09              David Wong
*/
...
void TxnOmtByUsORR(char cdebug)
{
	cDebug = cdebug;
}
...
DEBUGLOG(("net_ccy = [%s]\n", csTmp));
						...
						PutField_CString((hash_t*)hContext, "net_ccy", csTmp);
					}
/* reserve amount */
...
DEBUGLOG(("new txn id [%s]\n", csTxnSeq));
			...
			if (iRet == PD_OK) {
				BOObjPtr = CreateObj(BOPtr, "BOOLBalance", "GetTotalReserveForUpdate");
				iRet = (unsigned long)BOObjPtr(hContext, csTxnCountry, csTxnCcy, csService, csMerchantId, &dBalance);
			}

			if (iRet == PD_OK) {
DEBUGLOG(("total reserved amount = [%f]\n", dBalance));
				if (dBalance < dReserveAmt) {
					iRet = INT_INSUFFICIENT_FUND;
				} else {
					dBalance = dBalance - dReserveAmt;
DEBUGLOG(("new total reserved amount = [%f]\n", dBalance));
					BOObjPtr = CreateObj(BOPtr, "BOOLBalance", "ReleaseMerchantResAmount");
...
/* amount type */
			...
			if (iRet == PD_OK) {
DEBUGLOG(("Try to update org txn's reserved release date\n"));
				DBObjPtr = CreateObj(DBPtr, "DBOLTransaction", "UpdateReleaseReserveDate");
				iRet = (unsigned long)DBObjPtr(csOrgTxnSeq, csPostingDate, PD_UPDATE_USER);
			}
		}
	}
