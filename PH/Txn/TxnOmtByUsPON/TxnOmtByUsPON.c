...
-------------------------------                    ------------            --------------
Initial Version					   2014/01/09		   LokMan Chow
*/
...
void TxnOmtByUsPON(char    cdebug)
{
        cDebug = cdebug;
}
...
ERRLOG("TxnOmtByUsPON:Authorize::file_id not found!!\n");
                        iRet=INT_ERR;
                }
	}
...
ERRLOG("TxnOmtByUsPON:Authorize::batch_id not found!!\n");
                        iRet=INT_ERR;
                }
	}
...
ERRLOG("TxnOmtByUsPON::Authorize::total_cnt not found\n");
			iRet = INT_ERR;
		}
...
ERRLOG("TxnOmtByUsPON::Authorize::[%s] not found\n", csTag);
                                	iDtlRet = INT_ERR;
				}
...
DEBUGLOG(("Authorize::call BOOLPayout->HandleCancelGenerate\n"));
			BOObjPtr = CreateObj(BOPtr,"BOOLPayout","HandleCancelGenerate");
			iRet = (unsigned long)(*BOObjPtr)(hContext,hRequest,hResponse);
		}
	}
...
DEBUGLOG(("Authorize::call BOOLPayout->HandleCancelUpload\n"));
					BOObjPtr = CreateObj(BOPtr,"BOOLPayout","HandleCancelUpload");
					iRet = (unsigned long)(*BOObjPtr)(hContext,hRequest,hResponse);
				}
				else if(iStatus == PAYOUT_MASTER_TRANSACTION_CONFIRMED){
					if(cParty==PD_TYPE_MERCHANT){
DEBUGLOG(("Authorize::BOOLPayout:HandleCancelConfirm Merchant cannot cancel confirmed\n"));
					}
					else{
						BOObjPtr = CreateObj(BOPtr,"BOOLPayout","HandleCancelConfirm");
						iRet = (unsigned long)(*BOObjPtr)(hContext,hRequest,hResponse);
					}
				}
