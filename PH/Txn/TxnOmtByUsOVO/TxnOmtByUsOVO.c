...
-------------------------------                    ------------            --------------
Init Version                                       2016/07/08              David Wong
*/
...
void TxnOmtByUsOVO(char cdebug)
{
	cDebug = cdebug;
}
...
ERRLOG("TxnOmtByUsOVO:: Authroize:: from_txn_seq not found\n");
...
ERRLOG("TxnOmtByUsOVO:: Authroize:: org_txn_seq not found\n");
		iRet = INT_INVALID_TXN;
		PutField_Int(hContext, "internal_error", iRet);
	}
...
ERRLOG("TxnOmtByUsOVO:: Authroize:: to_txn_seq not found\n");
		iRet = INT_INVALID_TXN;
		PutField_Int(hContext, "internal_error", iRet);
	}
...
ERRLOG("TxnOmtByUsOVO:: Authroize:: return_mfee not found\n");
		iRet = INT_INVALID_TXN;
		PutField_Int(hContext, "internal_error", iRet);
	}
...
ERRLOG("TxnOmtByUsOVO:: Authroize:: return_to_mfee not found\n");
		iRet = INT_INVALID_TXN;
		PutField_Int(hContext, "internal_error", iRet);
	}
...
ERRLOG("TxnOmtByUsOVO:: Authorize:: remark not found\n");
		iRet = INT_INVALID_TXN;
		PutField_Int(hContext, "internal_error", iRet);
	}
...
// lock FROM record
	if (iRet == PD_OK) {
DEBUGLOG(("Authorize:: (FROM) Call DBOLTransaction:MatchRespTxn\n"));
...
ERRLOG("TxnOmtByUsOVO:: Authroize:: cannot lock FROM record\n");
			iRet = INT_INVALID_TXN;
			PutField_Int(hContext, "internal_error", iRet);
		}
	}
...
// lock TO record
	if (iRet == PD_OK) {
DEBUGLOG(("Authorize:: (TO) Call DBOLTransaction:MatchRespTxn\n"));
		DBObjPtr = CreateObj(DBPtr, "DBOLTransaction", "MatchRespTxn");
		iTmpRet = (unsigned long)(*DBObjPtr)(csOrgTxnSeqTo, PD_COMPLETE);
		if (iTmpRet == PD_NOT_FOUND) {
DEBUGLOG(("Authroize:: cannot lock TO record\n"));
ERRLOG("TxnOmtByUsOVO:: Authroize:: cannot lock TO record\n");
			iRet = INT_INVALID_TXN;
			PutField_Int(hContext, "internal_error", iRet);
		}
	}
...
DEBUGLOG(("Authorize:: (FROM) Call DBOLMerchantBalance:GetOpenBalanceForUpdate\n"));
...
DEBUGLOG(("Authorize:: (FROM) Call DBOLMerchantBalance:GetOpenBalanceForUpdate FAILED\n"));
ERRLOG("TxnOmtByUsOVO:: Authorize:: (FROM) Call DBOLMerchantBalance:GetOpenBalanceForUpdate FAILED\n");
			iRet = INT_ERR;
		}
	}
...
DEBUGLOG(("Authorize:: (FROM) Call DBOLTxnElements:GetAllFeeChgDetail\n"));
...
DEBUGLOG(("Authorize:: (FROM) Call DBOLTxnElements:GetAllFeeChgDetail FAILED\n"));
ERRLOG("TxnOmtByUsOVO:: Authorize:: (FROM) Call DBOLTxnElements:GetAllFeeChgDetail FAILED\n");
			iRet = INT_ERR;
		}
	}
...
// process Void FROM txn elements and balance
	if (iRet == PD_OK) {
DEBUGLOG(("Authorize:: (FROM) Call BOOLBalance:ProcessTransferAvaPayout\n"));
		BOObjPtr = CreateObj(BOPtr, "BOOLBalance", "ProcessTransferAvaPayout");
		iRet = (unsigned long)(*BOObjPtr)(hVoidTxnEle);
		if (iRet != PD_OK) {
DEBUGLOG(("Authorize:: (FROM) Call BOOLBalance:ProcessTransferAvaPayout FAILED\n"));
ERRLOG("TxnOmtByUsOVO:: Authorize:: (FROM) Call BOOLBalance:ProcessTransferAvaPayout FAILED\n");
			iRet = INT_ERR;
		}
	}
...
DEBUGLOG(("Authorize:: (FROM) Call DBOLTransaction:AddDetail\n"));
		DBObjPtr = CreateObj(DBPtr, "DBOLTransaction", "AddDetail");
		iRet = (unsigned long)(*DBObjPtr)(hVoidTxnEle);
	}

	if (iRet == PD_OK) {
DEBUGLOG(("Authorize:: (FROM) Call DBOLTransaction:UpdateDetail\n"));
...
DEBUGLOG(("Authorize:: (TO) Call DBOLMerchantBalance:GetOpenBalanceForUpdate\n"));
		DBObjPtr = CreateObj(DBPtr, "DBOLMerchantBalance", "GetOpenBalanceForUpdate");
...
DEBUGLOG(("Authorize:: (TO) Call DBOLMerchantBalance:GetOpenBalanceForUpdate FAILED\n"));
ERRLOG("TxnOmtByUsOVO:: Authorize:: (TO) Call DBOLMerchantBalance:GetOpenBalanceForUpdate FAILED\n");
			iRet = INT_ERR;
		}
	}
...
// prepare Void TO txn id
	if (iRet == PD_OK) {
DEBUGLOG(("Authorize:: (TO) Call DBOLTxnSeq:GetNextOmtTxnSeq\n"));
...
// create Void TO txn_header & txn_detail
	if (iRet == PD_OK) {
		...
		PutField_CString(hOrgTxnTo, "channel_code", PD_CHANNEL_OMT);
		PutField_CString(hOrgTxnTo, "txn_code", PD_OFL_VOID_AVA_PAYOUT_REQ_TF_TO);
...
DEBUGLOG(("Authorize:: (TO) Call OMTChannel:AddTxnLog\n"));
...
DEBUGLOG(("Authorize:: (TO) Call DBOLTxnElements:GetAllFeeChgDetail\n"));
		DBObjPtr = CreateObj(DBPtr, "DBOLTxnElements", "GetAllFeeChgDetail");
...
DEBUGLOG(("Authorize:: (TO) Call DBOLTxnElements:GetAllFeeChgDetail FAILED\n"));
ERRLOG("Authorize:: (TO) Call DBOLTxnElements:GetAllFeeChgDetail FAILED\n");
			iRet = INT_ERR;
		}
	}

	if (iRet == PD_OK) {
DEBUGLOG(("Authorize:: (TO) Call DBOLMerchantBalance:GetRemainPOBalance\n"));
...
DEBUGLOG(("Authorize:: (TO) Call DBOLMerchantBalance:GetRemainPOBalance FAILED\n"));
ERRLOG("TxnOmtByUsOVO:: Authorize:: (TO) Call DBOLMerchantBalance:GetRemainPOBalance FAILED\n");
			iRet = INT_ERR;
		}
...
ERRLOG("TxnOmtByUsOVO:: Authorize:: insufficient fund [%s] [%s] [%s] [%lf] [%lf]\n", csMid, csNetCcy, csService, dAvaPOBal, dNetAmount);
...
ERRLOG("TxnOmtByUsOVO:: Authorize:: TO merchant_net_float problem [%lf]\n", dTmp);
				iRet = INT_ERR;
			}
		}
	}
...
// process Void TO txn elements and balance
	if (iRet == PD_OK) {
DEBUGLOG(("Authorize:: (TO) Call BOOLBalance:ProcessTransferAvaPayout\n"));
		BOObjPtr = CreateObj(BOPtr, "BOOLBalance", "ProcessTransferAvaPayout");
		iRet = (unsigned long)(*BOObjPtr)(hVoidTxnToEle);
		if (iRet != PD_OK) {
DEBUGLOG(("Authorize:: (TO) Call BOOLBalance:ProcessTransferAvaPayout FAILED\n"));
ERRLOG("TxnOmtByUsOVO:: Authorize:: (TO) Call BOOLBalance:ProcessTransferAvaPayout FAILED\n");
			iRet = INT_ERR;
		}
	}
...
DEBUGLOG(("Authorize:: (Org FROM) Call DBOLTransaction:Update\n"));
...
DEBUGLOG(("Authorize:: (ORG TO) Call DBOLTransaction:Update\n"));
		DBObjPtr = CreateObj(DBPtr,"DBOLTransaction","Update");
		iRet = (unsigned long)(*DBObjPtr)(hTmp);
	}
...
ERRLOG("TxnOmtByUsOVO:: GetTxnInfo:: GetTxnHeader: cannot found record\n");
			iRet = INT_NOT_RECORD;
		}
	}
...
ERRLOG("TxnOmtByUsOVO:: GetTxnInfo:: GetTxnDetail: cannot found record\n");
			iRet = INT_NOT_RECORD;
		}
	}
