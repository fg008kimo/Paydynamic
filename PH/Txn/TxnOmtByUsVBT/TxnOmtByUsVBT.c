...
Add handling of deposit match pair                 2014/04/15              David Wong
*/
...
DEBUGLOG(("Authorize:: (hRequest) remark = [%s]\n", csRemark));
	} else {
		if (GetField_CString(hContext, "remark", &csRemark)) {
DEBUGLOG(("Authorize:: (hContext) remark = [%s]\n", csRemark));
		} else {
...
DEBUGLOG(("Authorize::call DBOLBAIDTxn:: pid [%s]\n", csPid));
...
DEBUGLOG(("Authorize::call DBOLBAIDTxn:: posted_ind [%d]\n", iPostedInd));
			...
			//*** if change transaction type and stmt ref is null then not allow void
			//*** if void sms and stmt ref is not null then not allow void
...
ERRLOG("TxnOmtByUsVBT::Authorize::call DBOLStatement GetStmtDtl [%s] FAILURE!!!\n", csStmtTxnID);
				...
				if (iConvertType) {
					if (!GetField_CString(hRec, "stmt_ref", &csTmp)) {
						iRet = INT_INVALID_TXN;
						PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("Authorize::call DBOLStatement stmt ref not found for CTT\n"));
ERRLOG("TxnOmtByUsVBT::Authorize::call DBOLStatement stmt ref not found for CTT!\n");
					}
				} else {
					if (GetField_CString(hRec, "stmt_ref", &csTmp)) {
						iRet = INT_INVALID_TXN;
						PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("Authorize::call DBOLStatement stmt ref found for VBT\n"));
ERRLOG("TxnOmtByUsVBT::Authorize::call DBOLStatement stmt ref found for VBT!\n");
					}
				}
				...
				if (!GetField_CString(hRec, "amt_type", &csAmtType)) {
					iRet = INT_INVALID_TXN;
					PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("Authorize::call DBOLStatement amt type not found\n"));
ERRLOG("TxnOmtByUsVBT::Authorize::call DBOLStatement amt type not found!\n");
				}
			}
...
DEBUGLOG(("Authorize::call DBOLPspDetail:: GetPspDetail()\n"));
			...
			if ((unsigned long)(*DBObjPtr)(csPid, hTmp) != PD_OK) {
				iRet = INT_INVALID_TXN;
				PutField_Int(hContext, "internal_error", iRet);

DEBUGLOG(("Authorize::call DBOLPspDetail:: GetPspDetail() failed\n"));
ERRLOG("TxnOmtByUsVBT::Authorize::call DBOLPspDetail::GetPspDetail() failed\n");
			} else {
				// get bank_acct_type
				GetField_CString(hTmp, "bank_acct_type", &csBankAcctType);
			}
...
DEBUGLOG(("Authorize::call DBOLBAIDTxnCode:: GetDetail()\n"));
			...
			if ((unsigned long)(*DBObjPtr)(csOrgBAIDTxnCode, csBankAcctType, hTmp) != PD_FOUND) {
				iRet = INT_INVALID_TXN;
				PutField_Int(hContext, "internal_error", iRet);

DEBUGLOG(("Authorize::call DBOLBAIDTxnCode:: GetDetail() failed\n"));
ERRLOG("TxnOmtByUsVBT::Authorize::call DBOLBAIDTxnCode:: GetDetail() failed\n");
			} else {
				// get update_bal
				GetField_Int(hTmp, "update_bal", &iUpdateBal);
			}
		}
	}
...
DEBUGLOG(("Authorize() call BOOLDepositMatch::HandleDepositMatchPair()\n"));
							BOObjPtr = CreateObj(BOPtr, "BOOLDepositMatch", "HandleDepositMatchPair");
							if (iConvertType) {
								iRet = (unsigned long)(*BOObjPtr)(csOrgDstTxnSeq, "0000000000000000", 'C', csUser);
							} else {
								iRet = (unsigned long)(*BOObjPtr)(csOrgDstTxnSeq, "0000000000000000", 'V', csUser);
							}

							if (iRet == PD_OK) {
								PutField_Int(hRec, "multi_match_ind", 0);
							}
						}
					}
				}
				...
				if (iRet == PD_OK && iUpdateBal) {
					// txn_seq
					PutField_CString(hBal, "org_txn_seq", csOrgDstTxnSeq);
...
ERRLOG("TxnOmtByUsVBT::Authorize() call DBBankDesc::GetBankCountry() failed\n");
						} else {
							PutField_CString(hBal, "org_txn_country", csTxnCountry);
						}
					}

					// txn_ccy
					if (GetField_CString(hOrgTxn, "txn_ccy", &csTxnCcy)) {
						PutField_CString(hBal, "org_txn_ccy", csTxnCcy);
						PutField_CString(hBal, "org_dst_txn_ccy", csTxnCcy);
					}
					...
					// baid
					if (GetField_CString(hOrgTxn, "baid", &csBaid)) {
						PutField_CString(hBal, "org_baid", csBaid);
					}

					// txn_amount
					if (GetField_Double(hOrgTxn, "txn_amt", &dTmp)) {
						PutField_Double(hBal, "org_dst_net_amt", dTmp);
					}

					// lock BAID bal
DEBUGLOG(("Authorize() call DBOLBAIDBal::GetBalanceForUpdate()\n"));
					DBObjPtr = CreateObj(DBPtr, "DBOLBAIDBal", "GetBalanceForUpdate");
					if ((unsigned long)(*DBObjPtr)(csBaid, csTxnCountry, csTxnCcy, hBal) != PD_OK) {
						iRet = INT_ERR;
...
ERRLOG("TxnOmtByUsACN::Authorize() call DBOLBAIDBal: GetBalanceForUpdate() failed!!\n");
						...
						GetField_Double(hBal, "balance", &dTmp);
						PutField_Double(hOrgTxn, "open_bal", dTmp);
						RemoveField_Double(hBal, "balance");
						RemoveField_Double(hBal, "total_hold");
					}

DEBUGLOG(("Authorize() call BOOLBalance::UpdateTxnAmount()\n"));
					BOObjPtr = CreateObj(BOPtr, "BOOLBalance", "UpdateTxnAmount");
					if ((unsigned long)(*BOObjPtr)(hBal) != PD_OK) {
						iRet = INT_ERR;
...
ERRLOG("TxnOmtByUsACN::Authorize() call BOOLBalance::UpdateTxnAmount() failed\n");
						...
						GetField_Double(hBal, "total_hold", &dTmp);
						PutField_Double(hOrgTxn, "total_hold", dTmp);
						GetField_Double(hBal, "balance", &dTmp);
						PutField_Double(hOrgTxn, "balance", dTmp);
					}
				}
...
ERRLOG("TxnOmtByUsVBT::Authorize() call DBOLBAIDTxn::Update() failed\n");
		...
		if (iRet == PD_OK)  {
			...
			if (csRemark != NULL) {
				PutField_CString(hVoidBAIDTxn, "remark", csRemark);
			}
...
int FormVoidTxn(const char *csVoidTxnSeq, const char *csOrgDstTxnSeq, hash_t *hContext, hash_t *hOrgTxn, hash_t *hVoidTxn)
{
	...
	if (GetField_Double(hOrgTxn, "txn_amount", &dTxnAmt)) {
                PutField_Double(hVoidTxn, "txn_amount", dTxnAmt);
	}
        if (GetField_CString(hOrgTxn, "bank_code", &csTmp)) {
	...
	if (GetField_Double(hOrgTxn, "total_hold", &dTmp)) {
		PutField_Double(hVoidTxn, "total_hold", dTmp);
	}
	if (GetField_Double(hOrgTxn, "balance", &dTmp)) {
		PutField_Double(hVoidTxn, "bal", dTmp);
	}
