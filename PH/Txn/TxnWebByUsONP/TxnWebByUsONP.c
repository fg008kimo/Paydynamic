...
-------------------------------                    ------------            --------------
Init Version                                       2012/03/27		   LokMan Chow
*/
...
OBJPTR(DB);
void TxnWebByUsONP(char    cdebug)
{
        cDebug = cdebug;
}
...
DEBUGLOG(("TxnWebByUsONP batch_id = [%s]\n",csBatchId));
				...
				if ((*DBObjPtr)(hUpd) == PD_OK) {
					...
					if(!strcmp(csChannel,PD_CHANNEL_TWV)){
						sprintf(csCmd,"send_twv_payout.sh %s %s >/dev/null 2>&1",csBatchId,csSeq);
					}
					else if(!strcmp(csChannel,PD_CHANNEL_HPAY)){
						sprintf(csCmd,"send_hpy_payout.sh %s %s >/dev/null 2>&1",csBatchId,csSeq);
					}
