                        ...
                        hash_t* hResponse)
{
        ...
        char    csService[PD_SERVICE_CODE_LEN+1];
	char	cPartyType;
	char	cType;
...
DEBUGLOG(("Authorize::country = [%s]\n",csTmp));
		sprintf(csCountry,"%s",csTmp);
		PutField_CString(hContext, "country", csTmp);
	}
	else {
		sprintf(csCountry,"000");
...
DEBUGLOG(("Authorize::service_code = [%s]\n",csTmp));
		sprintf(csService,"%s",csTmp);
		PutField_CString(hContext, "service", csTmp);
        }
        else {
		sprintf(csService,"000");
...
DEBUGLOG(("Authorize::psp_type = [%c]\n",cType));
                PutField_Char(hContext, "psp_type", cType);
        }
        else {
		cType = '1';
...
DEBUGLOG(("Authorize::TxnCounters:ResetReqCounters Fail\n"));
	...
	if(iRet==PD_OK){
		...
		sprintf(csCmd,"reset_limit_counter.sh %s %s %c %s",csCountry,csService,cType,csUser);
		system(csCmd);
		FREE_ME(csCmd);
	}
