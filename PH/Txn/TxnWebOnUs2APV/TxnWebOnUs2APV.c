...
-------------------------------                    ------------            --------------
Init Version                                       2020/09/11              [WMC]
Support 2 initial request                          2020/09/16              [WCS]
*/
...
int UpdatePreAPVRespTxnLog(hash_t *hContext,
		hash_t *hResponse);


int AddAPVTxnLog(hash_t *hContext,
		const hash_t *hRequest);


void TxnWebOnUs2APV(char cdebug)
{
	cDebug = cdebug;
}
		...
		hash_t *hResponse)
{
	...
	hash_t *hTxn;
	hash_t *hRec;
DEBUGLOG(("TxnWebOnUs2APV::UpdateRespTxnLog()\n"));
...
DEBUGLOG(("UpdateTxnLog RESP:: redirectUrl = [%s]\n", csPtr));
		PutField_CString(hPspDetail, "deposit_url", csPtr);
		PutField_CString(hContext, "code_url", csPtr);
	}

/* tid */
	if (GetField_CString(hResponse, "tid", &csPtr)) {
DEBUGLOG(("UpdateTxnLog RESP:: tid = [%s]\n", csPtr));
		PutField_CString(hPspDetail, "tid", csPtr);
	}
...
DEBUGLOG(("UpdateTxnLog iRet = [%d] after Update TxnPspDetail\n", iRet));
	...
	if (iRet == PD_OK) {
		...
		iRet = (unsigned long)(*DBObjPtr)(PD_CHANNEL_ALIPAY_VNET, &iResult, csTmpStatus);
...
DEBUGLOG(("process_resp_txn()\n"));
	...
	PutField_CString(hContext, "psp_channel_code", PD_CHANNEL_ALIPAY_VNET);
	PutField_CString(hContext, "return_psp_channel", PD_CHANNEL_ALIPAY_VNET);
	GetField_CString(hContext, "psp_id", &csPspId);
	GetField_CString(hRequest, "psp_name", &csPspName);

	MsgObjPtr = CreateObj(MsgPtr, "ApvMsg", "BreakDownInitRspMsg");
	if ((*MsgObjPtr)(hResponse, inMsg->msg, inMsg->len) != PD_OK) {
		iRet = INT_BREAKDOWN_ERR;
		PutField_Int(hContext, "internal_error", iRet);
DEBUGLOG(("BreakDown ApvMsg Error\n"));
	}

	if (GetField_CString(hResponse, "return_code", &csTmp)) {
DEBUGLOG(("returned return_code = [%s]\n", csTmp));

		if (!strcmp(csTmp, PD_SUCCESS_STATUS)) {
			// verify returned txn_id
			if (GetField_CString(hResponse, "requestId", &csTmp)) {
DEBUGLOG(("returned txn_id = [%s]\n", csTmp));
				if (GetField_CString(hRequest, "order_num", &csTxnSeq)) {
DEBUGLOG(("sent txn_id = [%s]\n", csTxnSeq));
				}
				if (strcmp(csTmp, csTxnSeq)) {
DEBUGLOG(("txn_id not match!!!\n"));
...
DEBUGLOG(("PSP return FAIL!!!\n"));
			PutField_CString(hResponse, "return_code", PD_FAIL_STATUS);
			iSendAlert = PD_TRUE;
			if (GetField_CString(hResponse, "return_msg", &csTmp)) {
DEBUGLOG(("returned return_msg = [%s]\n", csTmp));
...
DEBUGLOG(("cannot get return_code!!!\n"));
		PutField_CString(hResponse, "return_code", PD_FAIL_STATUS);
		iSendAlert = PD_TRUE;
		sprintf(csCmd, "send_init_req_error.sh \"%s\" \"%s\" \"%s\"", PD_INVALID_FIELD, csPspId, csPspName);
		goto skipped;
...
DEBUGLOG(("process_resp_txn() call Update\n"));
	iRet = UpdatePreAPVRespTxnLog(hContext, hResponse);
...
int process_token_resp_txn(hash_t *hContext,
		const hex_t *inMsg,
		hash_t *hRequest)
{
	int iRet = PD_OK;
...
DEBUGLOG(("process_token_resp_txn()\n"));
	...
	MsgObjPtr = CreateObj(MsgPtr, "ApvMsg", "BreakDownGetTokenRspMsg");
	if ((*MsgObjPtr)(hResponse, inMsg->msg, inMsg->len) != PD_OK) {
		iRet = INT_BREAKDOWN_ERR;
		PutField_Int(hContext, "internal_error", iRet);
DEBUGLOG(("BreakDown ApvMsg Error\n"));
...
DEBUGLOG(("returned return_code = [%s]\n", csTmp));
		...
		if (!strcmp(csTmp, PD_SUCCESS_STATUS)) {
			// get token
			if (!GetField_CString(hResponse, "token", &csTmp)) {
DEBUGLOG(("cannot get token!!!\n"));
				PutField_CString(hResponse, "return_code", PD_FAIL_STATUS);
				iSendAlert = PD_TRUE;
				sprintf(csCmd, "send_init_req_error.sh \"%s\" \"%s\" \"%s\"", PD_INVALID_FIELD, csPspId, csPspName);
				goto skipped;
...
DEBUGLOG(("token = [%s]\n", csTmp));
				PutField_CString(hRequest, "token", csTmp);
			}
		} else {
DEBUGLOG(("PSP return FAIL!!!\n"));
			PutField_CString(hResponse, "return_code", PD_FAIL_STATUS);
...
DEBUGLOG(("process_token_resp_txn: exit iret = [%d]\n", iRet));
	return iRet;
}
...
DEBUGLOG(("TxnWebOnUs2APV: after process_resp_err_txn iret = [%d]\n", iRet));
				}
				else {
					msg->mtext[iRecvLen] = '\0';
DEBUGLOG(("TxnWebOnUs2APV:  recv len = [%d]\n", iRecvLen));
DEBUGLOG(("TxnWebOnUs2APV:  recv = [%s]\n", &msg->mtext[MQ_HEADER_LEN]));
					memcpy(h_msg->msg, &msg->mtext[MQ_HEADER_LEN], iRecvLen - MQ_HEADER_LEN);
					iRet = process_token_resp_txn(hContext, h_msg, hRequest);
DEBUGLOG(("TxnWebOnUs2APV: after process_token_resp_txn iret = [%d]\n", iRet));

					if (iCont) {
						MsgObjPtr = CreateObj(MsgPtr, "ApvMsg", "FormatInitMsg");
						h_msg = (hex_t*) malloc (sizeof(hex_t));
						if ((*MsgObjPtr)(hRequest, h_msg->msg, &h_msg->len) == PD_OK) {
							lKey = GetMQKey((const unsigned char *)"PSPPREQQ");
							lRspKey = GetMQKey((const unsigned char *)"PSPPRSPQ");
...
DEBUGLOG(("TxnWebOnUs2APV: after process_resp_txn iret = [%d]\n", iRet));
								}
							}
						}
						else {
...
DEBUGLOG(("TxnWebOnUs2APV: noraml exit iret = [%d]\n", iRet));
	return iRet;
}
