...
void TxnMgtByUsTNI(char    cdebug)
{
        cDebug = cdebug;
}
...
ERRLOG("TxnMgtByUsTNI::Authorize() enc_txn_seq is missing\n");
...
/* cDeviceType */
	if (iRet == PD_OK){
                if (GetField_CString(hRequest,"device_type",&csTmp)) {
			cDeviceType = csTmp[0];
DEBUGLOG(("Authorize() device_type = [%c]\n",cDeviceType));
                } else {
			iRet = INT_ERR;
DEBUGLOG(("Authorize() device_type is missing\n"));
ERRLOG("TxnMgtByUsTNI::Authorize() device_type is missing\n");
                	PutField_Int(hContext,"internal_error",iRet);
                }
        }

	// Set Expire Time for Deposit Transaction
	if (iRet == PD_OK) {
...
DEBUGLOG(("Authorize() Filter by selected PID\n"));
			...
			//get NG bank by csPspId
			BOObjPtr = CreateObj(BOPtr,"BOBank","GetNGBankByPID");
                	iRet = (unsigned long)(BOObjPtr)(csSelectedPID,csOrgServiceCode,csOrgTxnCountry,cDeviceType,hResponse);
...
DEBUGLOG(("Authorize() Get Psp ccy [%s]\n",csDstCcy));
			...
			if(iRet==PD_OK){
DEBUGLOG(("Authorize() BOBank Call BOBank:GetAvailableNGBank\n"));
				BOObjPtr = CreateObj(BOPtr,"BOBank","GetAvailableNGBank");
				iRet=(unsigned long)(BOObjPtr)(csOrgChannelCode,csOrgServiceCode,csOrgTxnCountry,
							csOrgMerchantId,csOrgDateTime,csTxnCcy,cDeviceType,dTxnAmt,hResponse);
			}
...
ERRLOG("TxnMgtByUsTNI: Authorize() DBTmpCalAmount:Add failed!!!\n");
					}
				}
			}
		}
...
ERRLOG("TxnMgtByUsTNI: Authorize() DBTransaction:Update Failed!!!!\n");
			}

			FREE_ME(hCon);

			if(iRet == PD_OK){
				DBObjPtr = CreateObj(DBPtr,"DBTxnBrowserInfo","UpdateDeviceType");
                        	if ((DBObjPtr)(csTxnSeq,cDeviceType) != PD_OK) {	
					iRet = INT_ERR;
                        		PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("Authorize() DBTxnBrowserInfo:UpdateTxnBrowserInfo Failed!!!!\n"));
ERRLOG("TxnMgtByUsTNI: Authorize() DBTxnBrowserInfo:UpdateTxnBrowserInfo Failed!!!!\n");
                        	}
			}		
		}
	}
