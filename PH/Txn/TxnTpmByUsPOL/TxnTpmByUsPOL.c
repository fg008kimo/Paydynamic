...
OBJPTR(DB);
OBJPTR(Channel);
void TxnTpmByUsPOL(char    cdebug)
{
        cDebug = cdebug;
}
...
DEBUGLOG(("TxnTpmByUsPOL() call TxnTpmOnUsPOL\n"));
     		TxnObjPtr = CreateObj(TxnPtr,"TxnTpmByUsINQ","Authorize");
       		iRet = (unsigned long)(*TxnObjPtr)(hContext,hRequest,hResponse);
		if (GetField_CString(hResponse,"success_code",&csSuccessCode)) {
DEBUGLOG(("TxnTpmByUsPOL::Authorize() success_code = [%s]\n",csSuccessCode));
        	}
	}

DEBUGLOG(("TxnTpmByUsPOL() exit = [%d]\n",iRet));
	return	iRet;
}
