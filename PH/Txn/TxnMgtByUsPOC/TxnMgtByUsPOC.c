...
pass to batch job, return msg immediately	   2014/03/13		   LokMan Chow
Add commit before calling background job	   2018/12/20		   David Wong
*/
...
ERRLOG("TxnMgtByUsPOC:Authorize::user not found!!\n");
	}
...
ERRLOG("TxnMgtByUsPOC:Authorize::the file is not for confirm!!\n");
		}
	}
...
DEBUGLOG(("Authorize::commit before calling background job\n"));
			TxnCommit();
		}
	}

	if(iRet == PD_OK){
		sprintf(csCmd,"payout_confirm.exec -b %s -u %s &",csBatchId,csTmp);
		system(csCmd);
	}
/*
	if(iRet == PD_OK){
