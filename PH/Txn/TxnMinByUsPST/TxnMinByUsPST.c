		  ...
		  hash_t* hResponse)
{
	...
	hash_t  *hTmpRec = (hash_t*) malloc (sizeof(hash_t));
        hash_init(hTmpRec,0);
...
/* psp_id */
		...
		// Get PSP Business Type
DEBUGLOG(("Authorize::Call PspDetail:: GetPspDetail\n"));
                DBObjPtr = CreateObj(DBPtr, "DBPspDetail", "GetPspDetail");
                iRet = (unsigned long)(*DBObjPtr)(csPspId,hTmpRec);
                if (iRet == PD_OK) {
DEBUGLOG(("Authorize::Call PspDetail:: GetPspDetail Success\n"));

/* psp_type */
                    	if (GetField_Char(hTmpRec, "psp_type", &cTmp)) {
DEBUGLOG(("Authorize::Call PspDetail:: GetPspDetail:: psp_type = [%c]\n", cTmp));
                           	PutField_Char(hData,"business_type",cTmp);
                      	} else {
DEBUGLOG(("Authorize::Call PspDetail:: GetPspDetail psp_type not found\n"));
ERRLOG("TxnMinByUsPST::Authorize::Call PspDetail:: GetPspDetail psp_type not found\n");
                             	iRet = INT_ERR;
                    	}

		} else {
DEBUGLOG(("Authorize::Call PspDetail:: GetPspDetail Failed\n"));
ERRLOG("TxnMinByUsPST::Authorize::Call PspDetail:: GetPspDetail Failed\n");
                        iRet = INT_ERR;
                }		
