...
-------------------------------                    ------------            --------------
Init Version                                       2017/01/11              David Wong
*/
...
OBJPTR(DB);
OBJPTR(BO);
int     UpdatePreAWCRespTxnLog(hash_t *hContext,
                	hash_t* hResponse);

int     AddAWCTxnLog(hash_t *hContext,
                const hash_t* hRequest);

void TxnWebOnUs2AWC(char    cdebug)
{
        cDebug = cdebug;
}
int     AddAWCTxnLog(hash_t *hContext,
...

{
	...
	hash_t	*hTxn;
	hash_t	*hRec;
DEBUGLOG(("TxnWebOnUs2AWC::UpdateRespTxnLog()\n"));
...
DEBUGLOG(("UpdateTxnLog  iRet = [%d] after Insert TxnQrRequestLog\n",iRet));
		...
		int	iResult;
        	...
        	iRet = (unsigned long)(*DBObjPtr)(PD_CHANNEL_AIPWECHAT,&iResult,csTmpStatus);
...
DEBUGLOG(("process_resp_txn()\n"));
	...
	PutField_CString(hContext,"psp_channel_code",PD_CHANNEL_AIPWECHAT);
	PutField_CString(hContext,"return_psp_channel",PD_CHANNEL_AIPWECHAT);
	...
	GetField_CString(hRequest,"psp_name",&csPspName);
      	MsgObjPtr = CreateObj(MsgPtr,"AwcMsg","BreakDownInitRspMsg");

        if ((*MsgObjPtr)(hResponse,inMsg->msg,inMsg->len) != PD_OK)  {
		iRet = INT_BREAKDOWN_ERR;
		PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("process_resp_txn:BreakDown AwcMsg Error\n"));
	}
//	else {
		// testing
...
DEBUGLOG(("process_resp_txn: returned return_code = [%s]\n", csTmp));
			...
			if (!strcmp(csTmp, "SUCCESS")) {
				// check trxstatus
				if (GetField_CString(hResponse, "trxstatus", &csTmp)) {
DEBUGLOG(("process_resp_txn: trxstatus = [%s]\n", csTmp));
					if (strcmp(csTmp, "0000")) {
DEBUGLOG(("process_resp_txn: trxstatus fail\n"));
				...
				if (GetField_CString(hResponse, "reqsn", &csTmp)) {
...
DEBUGLOG(("process_resp_txn() call Update\n"));
		iRet = UpdatePreAWCRespTxnLog(hContext,hResponse);
...
DEBUGLOG(("TxbWebOnUs2AWC: time_redirect = [%d]\n",iTmp));
	        ...
	        if (!GetField_CString(hContext,"from_txn_seq",&csTxnSeq)) 
       		 	...
       		 	if ((*BOObjPtr)(hContext,hRequest,hRequest) == PD_OK) {
       		 		...
       		 		MsgObjPtr = CreateObj(MsgPtr,"AwcMsg","FormatInitMsg");
...
DEBUGLOG(("TxnWebOnUs2AWC: after process_resp_err_txn iret = [%d]\n",iRet));
					}
					else {
						msg->mtext[iRecvLen] = '\0';
DEBUGLOG(("TxnWebOnUs2AWC:  recv len = [%d]\n",iRecvLen));
DEBUGLOG(("TxnWebOnUs2AWC:  recv = [%s]\n",&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN));
						memcpy(h_msg->msg,&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN);
						iRet = process_resp_txn(hContext,h_msg,hRequest);
DEBUGLOG(("TxnWebOnUs2AWC: after process_resp_txn iret = [%d]\n",iRet));
					}
                		}
        			FREE_ME(msg);
        		}     
...
DEBUGLOG(("TxnWebOnUs2AWC: BuildData Error\n"));
		}
	}
...
DEBUGLOG(("TxnWebOnUs2AWC: noraml exit iret = [%d]\n",iRet));
	return iRet;
}
