...
-------------------------------                    ------------            --------------
Init Version                                       2010/10/22              [GOD]
encrypt type not hard code			   2011/01/27		   [MSN]
Add Fee and Pay Method				   2011/11/28		   [GOD]
Add checking for ignore_nack			   2013/01/30		   [MSN] 
Add customer_id					   2013/08/23		   [WCS]
Chk if succ or fail cb url not found               2013/11/06              [SWK]
Not allow to send callback if void or Approved Refund
                                                   2014/10/07              [SWK]
Add fields in Response Message
	- paid_ccy
	- paid_amt				   2020/11/18		   [WMC] 
*/
                        ...
                        hash_t* hResponse)
{
	...
	recordset_t     *rMerchantSet;
        ...
        hash_init(hRsp,0);
	hPspPaidAmt = (hash_t*) malloc (sizeof(hash_t));
        hash_init(hPspPaidAmt,0);
...
DEBUGLOG(("TxnWebByUsACK batch_txn_seq = [%s]\n",csBatchTxnSeq));
		}
		else {
...
DEBUGLOG(("TxnWebByUsACK new_txn_seq = [%s]\n",csTxnSeq));
		}
/* end ack log */
...
DEBUGLOG(("TxnWebByUsACK:process_code= [%s]\n",csOrgProcessCode));
					if(!strcmp(csOrgProcessCode,PD_PROCESS_CODE_DSP))
						...
						PutField_CString(hRsp,"process_type",csOrgProcessCode);
				}
...
DEBUGLOG(("TxnWebByUsACK:internal_error= [%d]\n",iTmp));
				...
				if (GetField_CString(hRec, "sub_status", &csPtr)) {
DEBUGLOG(("TxnWebByUsACK:sub_status= [%s]\n",csPtr));
					if ( !strcmp(csPtr, PD_VOID)  ||
                                             !strcmp(csPtr, PD_UNDO)) {
...
/* merch_detail success_callback_url */
		if (GetField_CString(hContext,"merchant_success_callback_url",&csPtr)) {
DEBUGLOG(("BOMerchant: GetMerchantDetail - success_callback_url = [%s]\n",csPtr));
			PutField_CString(hRsp,"success_callback_url",csPtr);
		}
/* merch_detail failure_callback_url */
		if (GetField_CString(hContext,"merchant_failure_callback_url",&csPtr)) {
DEBUGLOG(("BOMerchant: GetMerchantDetail - failure_callback_url = [%s]\n",csPtr));
			PutField_CString(hRsp,"failure_callback_url",csPtr);
		}
	}
	if (iRet == PD_OK ) {
        	recordset_init(rRecordSet,0);
        	...
        	if ((*DBObjPtr)(csOrgTxnSeq,rRecordSet) == PD_OK) {
                	...
                	while (hRec) {
/* customer_id */
				if (GetField_CString(hRec,"customer_id",&csValue)) {
DEBUGLOG(("TxnWebByUsACK:cust_id= [%s]\n",csValue));
					PutField_CString(hRsp,"process_code",PD_PROCESS_CODE_VP_ACK);

					PutField_CString(hRsp,"customer_id",csValue);
					RemoveField_CString(hRsp,"merchant_ref");
					RemoveField_CString(hRsp,"transmission_datetime");
				} else {
...
DEBUGLOG(("TxnWebByUsACK:pay_method is missing!!!\n"));
				}
/* bank code */
...
DEBUGLOG(("TxnWebByUsACK: unable to get success_callback_url!!!\n")); 
				iRet = INT_SUCC_CB_URL_NOT_FOUND;
			}
		} else {
			if (!GetField_CString(hRsp,"failure_callback_url",&csPtr)) {
DEBUGLOG(("TxnWebByUsACK: unable to get failure_callback_url!!!\n")); 
				iRet = INT_SUCC_CB_URL_NOT_FOUND;
			}
		}
...
DEBUGLOG(("TxnWebByUsACK:error_code= [%s]\n",csValue));
				...
				if (GetField_CString(hRec,"psp_id",&csValue)) {
DEBUGLOG(("TxnWebByUsACK:psp_id= [%s]\n",csValue));
                                        PutField_CString(hPspPaidAmt,"org_psp_id",csValue);
                                }
				if (GetField_CString(hRec,"txn_ccy",&csPspTxnCcy)) {
//DEBUGLOG(("TxnWebByUsACK:txn_ccy= [%s]\n",csPspTxnCcy));
                                }
				if (GetField_Double(hRec,"txn_amt",&dTxnAmt)) {
//DEBUGLOG(("TxnWebByUsACK:txn_amt= [%lf]\n",dTxnAmt));
                                       	PutField_Double(hPspPaidAmt,"org_dst_net_amt",dTxnAmt);
                                }
				if (GetField_Double(hRec,"paid_amount",&dPaidAmt)) {
//DEBUGLOG(("TxnWebByUsACK:paid_amt= [%lf]\n",dPaidAmt));
                                        PutField_Double(hPspPaidAmt,"paid_amt",dPaidAmt);
                                }	
                                hRec = RecordSet_GetNext(rRecordSet);
                        }
                }
	...
	if (iRet == PD_OK) {
		BOObjPtr = CreateObj(BOPtr,"BOPspPaidAmount","IsAllowAmtDiff");
               	if ((unsigned long)(*BOObjPtr)(hPspPaidAmt) == PD_TRUE) {
			BOObjPtr = CreateObj(BOPtr,"BOPspPaidAmount","IsInAmtDiffRange");
                        if ((unsigned long)(*BOObjPtr)(hPspPaidAmt) == PD_TRUE) {
DEBUGLOG(("TxnWebByUsACK:org_dst_txn_ccy= [%s]\n",csPspTxnCcy));
DEBUGLOG(("TxnWebByUsACK:paid_amt= [%lf]\n",dPaidAmt));
	               		PutField_CString(hRsp,"paid_ccy",csPspTxnCcy);
        	      		PutField_Double(hRsp,"paid_amt",dPaidAmt);
			}
		}
	}
...
/*
				rMsg = (struct msg_t*)malloc(sizeof(struct msg_t)+MAX_MSG_SIZE);
                        	rMsg->mtype  = ctol((const unsigned char *)csOrgTxnSeq,strlen(csOrgTxnSeq));
				int iRecvLen = 0;
				int iBufLen = 0;
				char    outBuf[PD_TMP_MSG_BUF_LEN +1];
				char	cStatus;
	
DEBUGLOG(("TxnWebByAck: try to recv from ACKRSPQ [%ld][%ld]\n",lRspKey,rMsg->mtype));
                		iRet = MQRecv(lRspKey,rMsg,&iRecvLen,10);
				if (iRet == PD_OK) {
					memcpy(outBuf,&rMsg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN);
                                	iBufLen = iRecvLen - MQ_HEADER_LEN;
					outBuf[iBufLen] = '\0';
DEBUGLOG(("TxnWebByUsACK: Responsed from ACK GW = [%s]\n",outBuf));
					iRet = outBuf[0] - '0';
DEBUGLOG(("TxnWebByUsACK: Responsed from ACK GW = [%d]\n",iRet));
					if (iRet == PD_OK) 
						cStatus = PD_COMPLETE;
					else 
						cStatus = PD_REJECT;
					...
					cStatus = PD_REJECT;
DEBUGLOG(("TxnWebByUsACK: Recv timeout\n"));
				...
				DBObjPtr = CreateObj(DBPtr,"DBTxnAckLog","Update");
				if ((*DBObjPtr)(hAck) != PD_OK) {
DEBUGLOG(("DBTxnAckLog: Update Failed\n"));
					iRet = INT_ERR;
				}
*/
			}
