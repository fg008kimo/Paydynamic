...
OBJPTR(DB);
OBJPTR(BO);
int     UpdatePreCOQRespTxnLog(hash_t *hContext,
                	hash_t* hResponse);

int     AddCOQTxnLog(hash_t *hContext,
                const hash_t* hRequest);

void TxnWebOnUs2COQ(char    cdebug)
{
        cDebug = cdebug;
}
int     AddCOQTxnLog(hash_t *hContext,
...

{
	...
	hash_t	*hTxn;
	hash_t	*hRec;
DEBUGLOG(("TxnWebOnUs2COQ::UpdateRespTxnLog()\n"));
...
DEBUGLOG(("UpdateTxnLog RESP:: return_code = [%s]\n",csStatus))
		...
		if (!strcmp(csStatus, PD_COQ_SUCCESS_STATUS))
			PutField_CString(hResponse,"return_code",PD_SUCCESS_STATUS);
	}
	else {
...
DEBUGLOG(("UpdateTxnLog  iRet = [%d] after Insert TxnQrRequestLog\n",iRet));
		...
		int	iResult;
        	...
        	iRet = (unsigned long)(*DBObjPtr)(PD_CHANNEL_C123PAY_QR,&iResult,csTmpStatus);
...
DEBUGLOG(("process_resp_txn()\n"));
	...
	PutField_CString(hContext,"psp_channel_code",PD_CHANNEL_C123PAY_QR);
	PutField_CString(hContext,"return_psp_channel",PD_CHANNEL_C123PAY_QR);
	...
	GetField_CString(hRequest,"psp_name",&csPspName);
      	MsgObjPtr = CreateObj(MsgPtr,"CoqMsg","BreakDownInitRspMsg");

        if ((*MsgObjPtr)(hResponse,inMsg->msg,inMsg->len) != PD_OK)  {
		iRet = INT_BREAKDOWN_ERR;
		PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("process_resp_txn:BreakDown CoqMsg Error\n"));
	}
//	else {
		// testing
...
DEBUGLOG(("process_resp_txn: returned return_code = [%s]\n", csTmp));
			...
			if (!strcmp(csTmp, PD_COQ_SUCCESS_STATUS)) {
...
DEBUGLOG(("process_resp_txn() call Update\n"));
		iRet = UpdatePreCOQRespTxnLog(hContext,hResponse);
...
DEBUGLOG(("TxbWebOnUs2COQ: time_redirect = [%d]\n",iTmp));
	        ...
	        if (!GetField_CString(hContext,"from_txn_seq",&csTxnSeq)) 
       		 	...
       		 	if ((*BOObjPtr)(hContext,hRequest,hRequest) == PD_OK) {
       		 		...
       		 		MsgObjPtr = CreateObj(MsgPtr,"CoqMsg","FormatInitMsg");
...
DEBUGLOG(("TxnWebOnUs2COQ: after process_resp_err_txn iret = [%d]\n",iRet));
					}
					else {
						msg->mtext[iRecvLen] = '\0';
DEBUGLOG(("TxnWebOnUs2COQ:  recv len = [%d]\n",iRecvLen));
DEBUGLOG(("TxnWebOnUs2COQ:  recv = [%s]\n",&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN));
						memcpy(h_msg->msg,&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN);
						iRet = process_resp_txn(hContext,h_msg,hRequest);
DEBUGLOG(("TxnWebOnUs2COQ: after process_resp_txn iret = [%d]\n",iRet));
					}
                		}
        			FREE_ME(msg);
        		}     
...
DEBUGLOG(("TxnWebOnUs2COQ: BuildData Error\n"));
		}
	}
...
DEBUGLOG(("TxnWebOnUs2COQ: noraml exit iret = [%d]\n",iRet));
	return iRet;
}
