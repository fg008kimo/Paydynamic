...
void TxnMgtByUsPMD(char cdebug)
{
	cDebug = cdebug;
}
...
ERRLOG("TxnMgtByUsPMD:: Authorize:: org_txn_seq not found!!\n");
...
ERRLOG("TxnMgtByUsPMD:: Authorize:: add_user not found!!\n");
...
ERRLOG("TxnMgtByUsPMD:Authorize:: TxnMgtByUs2MiniMMM:Authorize() Failed Ret [%d]\n", iRet);
		}
	}

DEBUGLOG(("TxnMgtByUsPMD Normal Exit() iRet = [%d]\n", iRet));
	return iRet;
}
