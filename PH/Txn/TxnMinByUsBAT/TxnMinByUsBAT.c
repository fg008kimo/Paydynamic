		  ...
		  hash_t* hResponse)
{
	...
	char	*csToCcy = NULL;
	char	*csToBankCountry= NULL;
	char	*csToPartyId = NULL;
	char	*csToBaseCcy = NULL;
	double	dToAmt=0.0;
...
/* is_acr_bank */
	if(GetField_Int(hContext,"is_acr_bank",&iIsACRBank)){
		PutField_Int(hFrContext,"is_acr_bank",iIsACRBank);
DEBUGLOG(("Authorize::is_acr_bank = [%d]\n",iIsACRBank));
	}

/* bank_base_country */
	if(GetField_CString(hContext,"bank_base_country",&csBankCountry)){
		PutField_CString(hFrContext,"txn_country",csBankCountry);
DEBUGLOG(("Authorize::bank_base_country = [%s]\n",csBankCountry));
	}

/* bank_base_ccy*/
	if(GetField_CString(hContext,"bank_base_ccy",&csBaseCcy)){
...
/* bank_base_country */
	if(GetField_CString(hContext,"to_bank_base_country",&csToBankCountry)){
		PutField_CString(hToContext,"txn_country",csToBankCountry);
DEBUGLOG(("Authorize::to_bank_base_country = [%s]\n",csToBankCountry));
	}

/* bank_base_ccy*/
	if(GetField_CString(hContext,"to_bank_base_ccy",&csToBaseCcy)){
DEBUGLOG(("Authorize::to_bank_base_ccy = [%s]\n",csToBaseCcy));
	}
...
ERRLOG("TxnMinByUsBAT:Authorize::Same entity_id From[%s], To[%s]\n",csEntityId,csToEntityId);
	...
	//check ccy
	if(iRet == PD_OK){
		if(strcmp(csBaseCcy,csFrCcy)){
			iRet = INT_CURRENCY_ID_NOT_MATCH;
DEBUGLOG(("Authorize::Currency not match: From[%s], Base[%s]\n",csFrCcy,csBaseCcy));
ERRLOG("TxnMinByUsBAT:Authorize::Currency not match: From[%s],Base [%s]\n",csFrCcy,csBaseCcy);
		}
		if(strcmp(csToBaseCcy,csToCcy)){
			iRet = INT_CURRENCY_ID_NOT_MATCH;
DEBUGLOG(("Authorize::Currency not match: To[%s], Base[%s]\n",csToCcy,csBaseCcy));
ERRLOG("TxnMinByUsBAT:Authorize::Currency not match: To[%s],Base [%s]\n",csToCcy,csBaseCcy);
		}
	}
...
//Update txn_header, insert txn_mi_detail
	if(iRet == PD_OK){
		if(iDoLogging!=PD_ADD_LOG) {
			...
			PutField_Double(hToContext,"txn_amt",dFrAmt);
			PutField_CString(hToContext, "txn_country", csCountry);
			PutField_CString(hToContext, "txn_ccy", csFrCcy);
...
DEBUGLOG(("AddBatchDetail::Call MiBatchDetail: Add (%s)\n",csTmpTxnSeq));
