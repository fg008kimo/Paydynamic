...
Enhance support batchid input			   2016/04/27		   Elvis Wong
*/
...
#define PD_PAYOUT_GROUP_MODE_BY_BATCH   	'B'
#define PD_PAYOUT_GROUP_MODE_BY_MERCHANT    	'M'
                        ...
                        hash_t* hResponse)
{
	...
	//Default mode: by mid (original)
	cMode = PD_PAYOUT_GROUP_MODE_BY_MERCHANT;
...
DEBUGLOG(("Authorize: batch_id[%s]\n",csTmp));
		...
		cMode = PD_PAYOUT_GROUP_MODE_BY_BATCH;
...
DEBUGLOG(("Authorize: PHDATE [%s]\n",csDate));
	...
	// Check Payout Group Mode
	if ((cMode == PD_PAYOUT_GROUP_MODE_BY_BATCH) || (cMode == PD_PAYOUT_GROUP_MODE_BY_MERCHANT)) { 

		if (cMode == PD_PAYOUT_GROUP_MODE_BY_BATCH){
DEBUGLOG(("Authorize::Call DBOLMerchantUploadFileDetail:GetDetailForPayoutGrpByBatchId\n"));
        		DBObjPtr = CreateObj(DBPtr,"DBOLMerchantUploadFileDetail","GetDetailForPayoutGrpByBatchId");			
			iDtlRet = (unsigned long)(*DBObjPtr)(hTxn,rRecordSet);
			if (iDtlRet != PD_OK){
                                iRet = INT_ERR;
                                PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("Authorize::DBOLMerchantUploadFileDetail:GetDetailForPayoutGrpByBatchId Failed!!!\n"));
                        }
		} else if (cMode == PD_PAYOUT_GROUP_MODE_BY_MERCHANT){
