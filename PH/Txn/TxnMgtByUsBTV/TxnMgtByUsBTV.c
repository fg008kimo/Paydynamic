...
-------------------------------                    ------------            --------------
Init Version                                       2012/11/27              Virginia Yun
return txn fee 
and ex rate should not include markup		   2014/02/11		   LokMan Chow
*/
...
void TxnMgtByUsBTV(char    cdebug)
{
        cDebug = cdebug;
}
...
ERRLOG("TxnMgtByUsBTV:Authorize() merchant_id is missing\n");
	}
...
ERRLOG("TxnMgtByUSBTP:Authorize() to_merchant_id is missing\n");
	}
...
ERRLOG("TxnMgtByUsBTV:Authorize() service is missing\n");
	}
...
ERRLOG("TxnMgtByUsBTV:Authorize() to_service is missing\n");
	}
...
ERRLOG("TxnMgtByUsBTV:Authorize() txn_ccy is missing\n");
	}
...
ERRLOG("TxnMgtByUsBTV:Authorize() dst_txn_ccy is missing\n");
	}
...
ERRLOG("TxnMgtByUsBTV:Authorize() from_country is missing\n");
	}
...
ERRLOG("TxnMgtByUsBTV:Authorize() to_country is missing\n");
	}
...
ERRLOG("TxnMgtByUsBTV:Authorize() txn amount is missing\n");
	}
	...
	if (dTxnAmt <= 0 ) {
		...
		PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("Authorize::() txn_amt is missing\n"));
ERRLOG("TxnMgtByUSBTP:Authorize() txn amount is missing\n");
	}
...
ERRLOG("TxnMgtByUSBTP:Authorize() req_txn_code is missing\n");
	}

	if (strcmp(csEnqTxnCode, PD_AVA_PAYOUT_REQ_TF_FROM) &&
            strcmp(csEnqTxnCode, PD_MERCHANT_BAL_TFF)) {
		iRet = INT_CODE_ERROR;
		PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("Authorize::() unexpected req_txn_code\n"));
ERRLOG("TxnMgtByUSBTP:Authorize() unexpected req_txn_code\n");
	}
...
ERRLOG("TxnMgtByUsBTV:Authorize() difference country not allow\n");
			PutField_Int(hContext,"internal_error",iRet);
		}
	}
/* check same account */
...
ERRLOG("TxnMgtByUsBTV:Authorize() same account transfer\n");
		}
	}
...
DEBUGLOG(("Authorize:: Call Exchange\n"));
		...
		if (!strcmp(csEnqTxnCode, PD_AVA_PAYOUT_REQ_TF_FROM)){
			PutField_CString(hContext,"txn_code", PD_AVA_PAYOUT_REQ_TF_TO);
		} 
		if(!strcmp(csEnqTxnCode, PD_MERCHANT_BAL_TFF)) {
			PutField_CString(hContext,"txn_code", PD_MERCHANT_BAL_TFT);
		}
...
DEBUGLOG(("TxnMgtByUsBTV Normal Exit() iRet = [%d]\n",iRet));
	return iRet;
}
