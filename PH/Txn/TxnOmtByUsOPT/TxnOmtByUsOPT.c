...
Handle Settlement Date				   2015/03/26              Virginia Yun
*/
...
ERRLOG("TxnOmtByUsOPT::Authorize::baid not found!!\n");
	...
	if  (GetField_CString(hContext, "baid_psp_id", &csPspID)) {
		...
		hash_init(hPspRec, 0);
DEBUGLOG(("Authorize:: check PSP bank acct type!\n"));

		DBObjPtr = CreateObj(DBPtr,"DBOLPspDetail","GetPspDetail");
		iRet = (unsigned long)(*DBObjPtr)(csPspID,hPspRec);
                if (iRet == PD_OK) {

			if (GetField_CString(hPspRec, "bank_acct_type", &csTmp)) {
				if (!strcmp(csTmp, PD_NATURE_INTERMEDIATE) ||
				    !strcmp(csTmp, PD_NATURE_PAYOUT)) {
DEBUGLOG(("Authorize:: check PSP bank acct type [%s]!\n", csTmp));
				}
				else {
DEBUGLOG(("Authorize::GetPspDetail bank_acct_type not allow!!\n"));
ERRLOG("TxnOmtByUsOPT::Authorize::GetPspDetail bank_acct_type not allow!!\n");
					iRet=INT_INVALID_BAID;
					PutField_Int(hContext,"internal_error",iRet);
				}
			} else {
DEBUGLOG(("Authorize::GetPspDetail bank_acct_type failed!!\n"));
ERRLOG("TxnOmtByUsOPT::Authorize::GetPspDetail bank_acct_type failed!!\n");
				iRet=INT_INVALID_BAID;
				PutField_Int(hContext,"internal_error",iRet);
			}
		} else {
DEBUGLOG(("Authorize::GetPspDetail Failed!!\n"));
ERRLOG("TxnOmtByUsOPT::Authorize::GetPspDetail failed!!\n");
			iRet=INT_INVALID_BAID;
			PutField_Int(hContext,"internal_error",iRet);
		}
...
DEBUGLOG(("Authorize::baid_category = [%s]\n",csTmp));
		if (!strcmp(csTmp, PD_BAID_CAT_IQ) ||
		    !strcmp(csTmp, PD_BAID_CAT_INTERNAL)) {
DEBUGLOG(("Authorize::baid_category allow!\n"));
		} else {
DEBUGLOG(("Authorize::baid category not allow!!\n"));
ERRLOG("TxnOmtByUsOPT::Authorize::baid category not allow!!\n");
                	iRet=INT_INVALID_BAID;
                	PutField_Int(hContext,"internal_error",iRet);
		}
	}
...
DEBUGLOG(("Authorize::PHDATE = [%s]\n",csTmp));
                    ...
                    PutField_CString(hContext, "settlement_date", csTmp); // for update OLTxnPspDetail
                }
...
DEBUGLOG(("Authorize::Call DBOLTxnPspDetail:Add\n"));
        ...
        if(iRet==PD_OK){
		PutField_CString(hContext, "tracking_txn_seq", csTxnId);
