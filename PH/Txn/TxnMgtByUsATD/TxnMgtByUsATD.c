...
add flag for merchant_txn_display		   2013/06/20		   David Wong
	& psp_txn_display
add adjustment nature type			   2014/06/30		   Dirk Wong
*/	
...
ERRLOG("TxnMgtByUsATD::Authorize::nature not found!!\n");
		...
		if (cPartyType == PD_TYPE_MERCHANT) {
			...
			PutField_Int(hTxn,"merchant_txn_display",PD_TRUE);
		}
		else {
		...
		if (cPartyType == PD_TYPE_PSP)
			PutField_Int(hTxn,"psp_txn_display",PD_TRUE);
		else
