...
void TxnOmtByUsORF(char    cdebug)
{
        cDebug = cdebug;
}
...
ERRLOG("TxnOmtByUsORF:Authorize() txn amount is missing\n");
	}
...
ERRLOG("TxnOmtByUsORF:Authorize() txn_country is missing\n");
	}
*/
...
ERRLOG("TxnOmtByUsORF:Authorize() merchant_id is missing\n");
	}
...
ERRLOG("TxnOmtByUsORF:Authorize() to_merchant_id is missing\n");
	}
...
ERRLOG("TxnOmtByUsORF:Authorize() service is missing\n");
	}
...
ERRLOG("TxnOmtByUsORF:Authorize() to_service is missing\n");
	}
...
ERRLOG("TxnOmtByUsORF:Authorize() txn_ccy is missing\n");
	}
...
ERRLOG("TxnOmtByUsORF:Authorize() dst_txn_ccy is missing\n");
	}
/* from country */
...
ERRLOG("TxnOmtOnUSORF:Authorize() difference country not allow\n");
                        PutField_Int(hContext,"internal_error",iRet);
                }
        }
...
ERRLOG("TxnOmtByUsORF:Authorize() difference service_code not allow\n");
		}
	}
*/
...
ERRLOG("TxnOmtByUsORF:Authorize() same account transfer\n");
		}
	}
...
DEBUGLOG(("TxnOmtByUsORF merchant_remain_aval_po = [%lf]\n",dAvaPOBal));
			}
...
ERRLOG("TxnOmtByUsORF::Authorize GetRemainPOBalance FAILED\n");
		}
	}
...
DEBUGLOG(("Authorize() BOOLMerchant->GetMerchantTxnInfo = [%d]\n",iRet));
		...
		/* for update country */
		if (iRet == PD_OK) {
DEBUGLOG(("Authorize() ORF UpdateTxnDetailLog\n"));
...
/* host_posting_date */
		...
		PutField_CString(hNewContext,"txn_code", PD_OFL_AVA_PAYOUT_REQ_TF_TO);
...
DEBUGLOG(("Result_Code = %s\n",csBuf));
			...
			if (cParty == PD_TYPE_MERCHANT) {
				PutField_CString(hNewContext,"sub_status",PD_MERCHANT_REQUESTED);
				PutField_CString(hContext,"sub_status",PD_MERCHANT_REQUESTED);
			} else {
				PutField_CString(hNewContext,"sub_status",PD_ADMIN_REQUESTED);
...
DEBUGLOG(("TxnOmtByUsORF Normal Exit() iRet = [%d]\n",iRet));
	return iRet;
}
