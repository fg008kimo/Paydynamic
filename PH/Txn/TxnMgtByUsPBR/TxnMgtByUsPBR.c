...
DEBUGLOG(("Authorize::total_txn_amt = [%lf]\n",dTmp));
		...
		if(GetField_Double(hContext,"total_paid_amt",&dTmp)){
                        PutField_Double(hContext,"paid_amt",dTmp);
			PutField_Double(hContext,"paid_amount",dTmp);
...
DEBUGLOG(("Authorize::Call DBTxnPspDetail:Update\n"));
		if(GetField_CString(hContext,"PHDATE",&csTmp)){
			...
			if(dInputFee>0.0){
				PutField_Double(hPsp,"service_fee",dInputFee);
			}
			else{
				if(GetField_Double(hContext,"total_txn_fee",&dTmp)){
...
DEBUGLOG(("TxnPspDetail::Update: psp_total_hold = [%f]\n",dTmp));
			...
			if(iRet==PD_OK){
                                ...
                                for(i=1; i<=iTxn; i++){
                                        ...
                                        PutField_CString(hPsp,"recon_date","");
                                        PutField_Int(hPsp,"recon_ind",PD_FALSE);
...
DEBUGLOG(("TxnPspDetail:Update Failed\n"));
	...
	if(iRet==PD_OK){
		DBObjPtr = CreateObj(DBPtr,"DBPspReconHistory","Delete");
		if((unsigned long) ((*DBObjPtr)(csTxnSeq))!=PD_OK){
			iRet = INT_ERR;
DEBUGLOG(("PspReconHistory:Delete\n"));
		}
	}
