...
void TxnMgtByUsPOI(char    cdebug)
{
        cDebug = cdebug;
}
...
DEBUGLOG(("Authorize: merchant_group = [%c]\n",csCharSet[pos]));
		...
		sprintf(csPayoutFile,"%s/input/on/%s",getenv("OUTPUT_PAYOUT"),csFileName);
	}
	else{
		iRet = INT_ERR;
		PutField_Int(hContext,"internal_error",iRet);
...
DEBUGLOG(("Authorize:: Process payout file\n"));
			BOObjPtr = CreateObj(BOPtr,"BOPayout","ProcessPayoutRecords");
...
DEBUGLOG(("Authorize:: Process payout file result = [%d]\n",iRet));
	...
	if(iRet == PD_OK){
		...
		if(GetField_CString(hTxn,"batch_id",&csTmp)){
			...
			sprintf(csNewPath,"%s/input/on/%s",getenv("OUTPUT_PAYOUT"),csFileRename);
...
DEBUGLOG(("Authorize:: payout file cannot be removed!!!!\n"));
	...
	FREE_ME(hTxn);
DEBUGLOG(("TxnMgtByUsPOI Normal Exit() iRet = [%d]\n",iRet));
        return iRet;
}
