...
-------------------------------                    ------------            --------------
Init Version                                       2018/03/19              David Wong
*/
...
OBJPTR(DB);
OBJPTR(BO);
int     UpdatePreRFQRespTxnLog(hash_t *hContext,
                	hash_t* hResponse);

int     AddRFQTxnLog(hash_t *hContext,
                const hash_t* hRequest);

void TxnWebOnUs2RFQ(char    cdebug)
{
        cDebug = cdebug;
}
int     AddRFQTxnLog(hash_t *hContext,
...

{
	...
	hash_t	*hTxn;
	hash_t	*hRec;
DEBUGLOG(("TxnWebOnUs2RFQ::UpdateRespTxnLog()\n"));
...
DEBUGLOG(("UpdateTxnLog  iRet = [%d] after Insert TxnQrRequestLog\n",iRet));
		...
		int	iResult;
        	...
        	iRet = (unsigned long)(*DBObjPtr)(PD_CHANNEL_RFPAY_QR,&iResult,csTmpStatus);
...
DEBUGLOG(("process_resp_txn()\n"));
	...
	PutField_CString(hContext,"psp_channel_code",PD_CHANNEL_RFPAY_QR);
	PutField_CString(hContext,"return_psp_channel",PD_CHANNEL_RFPAY_QR);
	...
	GetField_CString(hRequest,"psp_name",&csPspName);
      	MsgObjPtr = CreateObj(MsgPtr,"RfqMsg","BreakDownInitRspMsg");

        if ((*MsgObjPtr)(hResponse,inMsg->msg,inMsg->len) != PD_OK)  {
		iRet = INT_BREAKDOWN_ERR;
		PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("process_resp_txn:BreakDown RfqMsg Error\n"));
	}
//	else {
		// testing
...
DEBUGLOG(("process_resp_txn() call Update\n"));
		iRet = UpdatePreRFQRespTxnLog(hContext,hResponse);
...
DEBUGLOG(("TxbWebOnUs2RFQ: time_redirect = [%d]\n",iTmp));
	        ...
	        if (!GetField_CString(hContext,"from_txn_seq",&csTxnSeq)) 
       		 	...
       		 	if ((*BOObjPtr)(hContext,hRequest,hRequest) == PD_OK) {
       		 		...
       		 		MsgObjPtr = CreateObj(MsgPtr,"RfqMsg","FormatInitMsg");
...
DEBUGLOG(("TxnWebOnUs2RFQ: after process_resp_err_txn iret = [%d]\n",iRet));
					}
					else {
						msg->mtext[iRecvLen] = '\0';
DEBUGLOG(("TxnWebOnUs2RFQ:  recv len = [%d]\n",iRecvLen));
DEBUGLOG(("TxnWebOnUs2RFQ:  recv = [%s]\n",&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN));
						memcpy(h_msg->msg,&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN);
						iRet = process_resp_txn(hContext,h_msg,hRequest);
DEBUGLOG(("TxnWebOnUs2RFQ: after process_resp_txn iret = [%d]\n",iRet));
					}
                		}
        			FREE_ME(msg);
        		}     
...
DEBUGLOG(("TxnWebOnUs2RFQ: BuildData Error\n"));
		}
	}
...
DEBUGLOG(("TxnWebOnUs2RFQ: noraml exit iret = [%d]\n",iRet));
	return iRet;
}
