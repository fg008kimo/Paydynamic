...
OBJPTR(DB);
OBJPTR(BO);
int     UpdatePreDPQRespTxnLog(hash_t *hContext,
                	hash_t* hResponse);

int     AddDPQTxnLog(hash_t *hContext,
                const hash_t* hRequest);

void TxnWebOnUs2DPQ(char    cdebug)
{
        cDebug = cdebug;
}
int     AddDPQTxnLog(hash_t *hContext,
...

{
	...
	hash_t	*hTxn;
	hash_t	*hRec;
DEBUGLOG(("TxnWebOnUs2DPQ::UpdateRespTxnLog()\n"));
...
DEBUGLOG(("UpdateTxnLog  iRet = [%d] after Insert TxnQrRequestLog\n",iRet));
		...
		int	iResult;
        	...
        	iRet = (unsigned long)(*DBObjPtr)(PD_CHANNEL_DINPAY_QR,&iResult,csTmpStatus);
...
DEBUGLOG(("process_resp_txn()\n"));
	...
	PutField_CString(hContext,"psp_channel_code",PD_CHANNEL_DINPAY_QR);
	PutField_CString(hContext,"return_psp_channel",PD_CHANNEL_DINPAY_QR);
	...
	GetField_CString(hRequest,"psp_name",&csPspName);
      	MsgObjPtr = CreateObj(MsgPtr,"DpqMsg","BreakDownInitRspMsg");

        if ((*MsgObjPtr)(hResponse,inMsg->msg,inMsg->len) != PD_OK)  {
		iRet = INT_BREAKDOWN_ERR;
		PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("process_resp_txn:BreakDown DpqMsg Error\n"));
	}
//	else {
		// testing
...
DEBUGLOG(("process_resp_txn: returned return_code = [%s]\n", csTmp));
			...
			if (!strcmp(csTmp, PD_SUCCESS_STATUS)) {
				// verify result_code
				if (GetField_CString(hResponse, "result_code", &csTmp)) {
DEBUGLOG(("process_resp_txn: result_code = [%s]\n", csTmp));
					if (csTmp[0] != '0') {
DEBUGLOG(("process_resp_txn: get qr code url failed!!!\n"));
...
DEBUGLOG(("process_resp_txn: cannot get result_code!!!\n"));
					PutField_CString(hResponse, "return_code", PD_FAIL_STATUS);
					iSendAlert = PD_TRUE;
					sprintf(csCmd, "send_init_req_error.sh \"%s\" \"%s\" \"%s\"", PD_INVALID_FIELD, csPspId, csPspName);
					goto skipped;
				...
				if (GetField_CString(hResponse, "order_no", &csTmp)) {
...
DEBUGLOG(("process_resp_txn: cannot get order_no!!!\n"));
					PutField_CString(hResponse, "return_code", PD_FAIL_STATUS);
					iSendAlert = PD_TRUE;
					// PD_MISS_TXN_SEQ
					sprintf(csCmd, "send_init_req_error.sh \"%s\" \"%s\" \"%s\"", PD_INVALID_FIELD, csPspId, csPspName);
...
DEBUGLOG(("process_resp_txn: returned signature = [%s]\n", csTmp));
					PutField_Int(hResponse, "isInitReq", PD_TRUE);
...
DEBUGLOG(("process_resp_txn() call Update\n"));
		iRet = UpdatePreDPQRespTxnLog(hContext,hResponse);
...
DEBUGLOG(("TxbWebOnUs2DPQ: time_redirect = [%d]\n",iTmp));
	        ...
	        if (!GetField_CString(hContext,"from_txn_seq",&csTxnSeq)) 
       		 	...
       		 	if ((*BOObjPtr)(hContext,hRequest,hRequest) == PD_OK) {
       		 		...
       		 		MsgObjPtr = CreateObj(MsgPtr,"DpqMsg","FormatInitMsg");
...
DEBUGLOG(("TxnWebOnUs2DPQ: after process_resp_err_txn iret = [%d]\n",iRet));
					}
					else {
						msg->mtext[iRecvLen] = '\0';
DEBUGLOG(("TxnWebOnUs2DPQ:  recv len = [%d]\n",iRecvLen));
DEBUGLOG(("TxnWebOnUs2DPQ:  recv = [%s]\n",&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN));
						memcpy(h_msg->msg,&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN);
						iRet = process_resp_txn(hContext,h_msg,hRequest);
DEBUGLOG(("TxnWebOnUs2DPQ: after process_resp_txn iret = [%d]\n",iRet));
					}
                		}
        			FREE_ME(msg);
        		}     
...
DEBUGLOG(("TxnWebOnUs2DPQ: BuildData Error\n"));
		}
	}
...
DEBUGLOG(("TxnWebOnUs2DPQ: noraml exit iret = [%d]\n",iRet));
	return iRet;
}
