...
int	CheckStatus(const char* csTxnSeq, const char* csModeType);
...
DEBUGLOG(("Authorize::() [%s] = [%s]\n",csTag,csOrgTxnId));
					...
					if(CheckStatus(csOrgTxnId, csMode)!=PD_OK){
...
int	CheckStatus(const char* csTxnSeq, const char* csModeType)
{
	...
	char	cArInd;
	char	*csSubStatus;
	char	cReconStatus = PD_UNRECONCILED;
...
DEBUGLOG(("Authorize::recon status = [%c]\n",cReconStatus));
			}
...
DEBUGLOG(("Authorize::Valid txn[%s]: [%c][%c][%s]\n",csTxnSeq,cStatus,cArInd,csSubStatus));
			...
			if (!strcmp(csModeType,PD_DELIVER_FROM)) {
				if (cReconStatus == PD_RECONCILED) {
DEBUGLOG(("Authorize::Valid recon status [%s]: [%c]\n",csTxnSeq,cReconStatus));
					iRet = PD_OK;
				}
				else {
DEBUGLOG(("Authorize::Invalid txn[%s]: recon status[%c]\n",csTxnSeq,cReconStatus));
				}
			} else {	
				iRet = PD_OK;
			}
