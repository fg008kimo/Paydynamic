...
Add version number checking for different handling 2015/03/20              David Wong
Add bank_code for Return Payout(PSP)               2015/05/04              Virginia Yun
Add CheckPayoutReconMulti                          2015/06/24              Virginia Yun
Change UpdatePayoutGenerateTxn in batch searching  2015/08/12              Virginia Yun
                        ...
                        hash_t* hResponse)
{
	...
	int	iRefundParty = 0;
	int iVersionNo;
	hash_t  *hRec, *hOrgTxn, *hIn, *hPspDetail, *hBalance, *hPsp, *hElement, *hTxnDetail, *hGenPOTxn;
	...
	hGenPOTxn = (hash_t*)  malloc (sizeof(hash_t));
        hash_init(hGenPOTxn,0);
...
DEBUGLOG(("org txn seq = [%s]\n",csOrgTxnSeq));
	...
	if(GetField_CString(hContext,"update_user",&csTmp)){
                ...
                PutField_CString(hGenPOTxn,"update_user",csTmp);
...
DEBUGLOG(("GetFileDetailByUploadTxnId version_no = [%d]\n",iVersionNo));
				}
...
DEBUGLOG(("Call DBOLTransaction:(xxxxx) [%d] fail_at_provider\n", i));
			if (GetField_Int(hContext, csTag, &iTmp)) {
DEBUGLOG(("Call DBOLTransaction:(xxxxx) fail_at_provider_%d = [%d]\n", i, iTmp));
			}
*/
...
DEBUGLOG(("GetTxnHeader::found record = [%s]\n",csOrgGenTxnSeq));
					...
					while (hRec) {
						...
						PutField_Int(hPsp, "db_commit", PD_FALSE);
...
DEBUGLOG(("GetTxnDetail::found record = [%s]\n",csOrgGenTxnSeq));
						...
						if (GetField_Int(hTxnDetail, "charge_rule_id", &iTmp)) {
							PutField_Int(hPspDetail, "charge_rule_id", iTmp);
							//sprintf(csTag, "charge_rule_id_%d", i);
							//PutField_Int(hBalance, csTag, iTmp);
						}
						if (GetField_Char(hTxnDetail, "charge_period_type", &cTmp)) {
							PutField_Char(hPspDetail, "charge_period_type", cTmp);
							//sprintf(csTag, "charge_period_type_%d", i);
							//PutField_Char(hBalance, csTag, cTmp);
						}
						if (GetField_Double(hTxnDetail, "pre_cal_charge", &dTmp)) {
							PutField_Double(hPspDetail, "pre_cal_charge", dTmp);
							//sprintf(csTag, "precal_charge_%d", i);
							//PutField_Double(hBalance, csTag, dTmp);
						}

						PutField_Int(hPspDetail, "charge_txn_created", PD_FALSE);


						if (GetField_CString(hTxnDetail, "grp_tracking_txn_seq", &csTmp)){
							sprintf(csTag, "grp_tracking_txn_seq_%d", i);
							PutField_CString(hBalance, csTag, csTmp);
						}

						if (GetField_CString(hTxnDetail, "tracking_txn_seq", &csTmp)){
							sprintf(csTag, "tracking_txn_seq_%d", i);
							PutField_CString(hBalance, csTag, csTmp);
						}

						if (GetField_CString(hTxnDetail, "bank_code", &csTmp)) {
...
DEBUGLOG(("GetDetail:: VoidOrgTxnElements Failed!!!\n"));
				...
				if(iRet==PD_OK){
					...
					if (GetField_Int(hContext, csTag, &iTmp)) {
						...
						if (iTmp == PD_TRUE) {
DEBUGLOG(("Call DBOLTransaction:(Org) recon_status to [%c]\n", PD_RECON_NOT_REQ));
					...
					*/
					...
					PutField_CString(hGenPOTxn, csTag, csOrgGenTxnSeq);

					PutField_Int(hGenPOTxn, "gen_po_cnt", i+1);

					/* Start Remark on 2015/08/12  
DEBUGLOG(("Call DBOLTransaction:Update (Org)\n"));
					iRet = UpdatePayoutGenerateTxn(hOrgTxn);

					if (iRet != PD_OK) {
						break;
					}
                                           end Remark on 2015/08/12 */
...
DEBUGLOG(("DBOLTransaction:UpdateDetail Failed\n"));
		...
		if (GetField_Int(hGenPOTxn, "gen_po_cnt", &iPOTxnCnt)) {
DEBUGLOG(("Before UpdatePayoutGenerateTxn count = [%d]\n", iPOTxnCnt));
			iRet = UpdatePayoutGenerateTxn(hGenPOTxn);
			if (iRet != PD_OK) {
DEBUGLOG(("After UpdatePayoutGenerateTxn FAILED!!\n"));
			}
		}
...
DEBUGLOG(("CheckReconMulti::Call DBOLPayoutReturn:CheckPayoutReconMulti\n"));
			DBObjPtr = CreateObj(DBPtr,"DBOLPayoutReturn","CheckPayoutReconMulti");
			iRet = (unsigned long)(*DBObjPtr)(csOrgGenTxnSeq);
			if (iRet != PD_OK) {
DEBUGLOG(("CheckReconMulit:: Not Pass to continue....\n"));
				break;
			}


DEBUGLOG(("CheckAllow::Call DBOLPayoutReturn:CheckPayoutRtnAllow\n"));
			DBObjPtr = CreateObj(DBPtr,"DBOLPayoutReturn","CheckPayoutRtnAllow");
			iRet = (unsigned long)(*DBObjPtr)(csOrgGenTxnSeq, hPsp);

			if (iRet == PD_OK) {
DEBUGLOG(("CheckAllow::Call DBOLPayoutReturn:CheckPayoutRtnAllow Result OK\n"));

				if (GetField_Int(hPsp, "recon_cnt", &iReconCnt)) {
					iTotalReconCnt = iTotalReconCnt + iReconCnt;
DEBUGLOG(("CheckAllow::recon_cnt[%d] total[%d]\n", iTmp, iTotalReconCnt));

					sprintf(csTag, "gen_recon_cnt_%d", i);
					PutField_Int(hContext, csTag, iReconCnt);
				}

				if (GetField_Int(hPsp, "unrecon_cnt", &iTmp)) {
					iTotalUnReconCnt = iTotalUnReconCnt + iTmp;
DEBUGLOG(("CheckAllow::unrecon_cnt[%d] total[%d]\n", iTmp, iTotalUnReconCnt));
				}

				if (GetField_Int(hPsp, "level", &iLevel)) {
DEBUGLOG(("CheckAllow::level[%d]\n", iLevel));
				...
				if (iLevel == 1 && iReconCnt  == 0) {
					//sprintf(csTag, "fail_at_provider_%d", i);
DEBUGLOG(("CheckAllow::[%d] fail_at_provider TRUE [%d]\n", i, PD_TRUE));
					PutField_Int(hContext, csTag, PD_TRUE);
				} else {
DEBUGLOG(("CheckAllow::[%d] fail_at_provider FALSE [%d]\n", i, PD_FALSE));
...
int	UpdatePayoutGenerateTxn(hash_t *hUpdateTxn)
{
	...
	hash_t	*hTxnList;
	hTxnList = (hash_t*)  malloc (sizeof(hash_t));
	hash_init(hTxnList, 0);
...
DEBUGLOG(("UpdatePayoutGenerateTxn:START!\n"));


	if (GetField_Int(hUpdateTxn, "gen_po_cnt", &iTxnCnt)) {
DEBUGLOG(("UpdatePayoutGenerateTxn:cnt [%d]!\n", iTxnCnt));

		iTxnListCnt = 0;
	
		for (i = 0; i < iTxnCnt; i++) {
			sprintf(csTag, "txn_seq_%d", i);
			GetField_CString(hUpdateTxn, csTag, &csTxnSeq);
DEBUGLOG(("UpdatePayoutGenerateTxn:[%d] txn_seq [%s]!\n", i, csTxnSeq));
...
DEBUGLOG(("UpdatePayoutGenerateTxn: get txn_seq [%s]!\n", csTmp));
					}

					sprintf(csTag, "txn_seq_value_%s", csTmp);

					if (!GetField_CString(hTxnList, csTag, &csTmpSeq)) {
DEBUGLOG(("UpdatePayoutGenerateTxn: add txn_seq into list [%s]!\n", csTmp));
						PutField_CString(hTxnList, csTag, csTmp);
			
						sprintf(csTag, "txn_seq_%d", iTxnListCnt);
						PutField_CString(hTxnList, csTag, csTmp);


						if (GetField_Char(hRec, "recon_status", &cTmp)) {
							sprintf(csTag, "recon_status_%d", iTxnListCnt);
							PutField_Char(hTxnList, csTag, cTmp);
						}

						PutField_Int(hTxnList, "txn_cnt", iTxnListCnt+1);
		...
		if (GetField_Int(hTxnList, "txn_cnt", &iTmp)) {
DEBUGLOG(("UpdatePayoutGenerateTxn: total list cnt [%d]\n", iTmp));
			for (i = 0; i < iTmp; i++) {
				hash_init(hOrgTxn, 0);

				sprintf(csTag, "txn_seq_%d", i);

				GetField_CString(hTxnList, csTag, &csTmp);
DEBUGLOG(("UpdatePayoutGenerateTxn: prepare Update txn_seq [%s]!\n", csTmp));
				PutField_CString(hOrgTxn, "txn_seq", csTmp);

				PutField_Char(hOrgTxn,"status",PD_REVERSED);
				PutField_CString(hOrgTxn,"sub_status",PD_VOID);

				if (GetField_CString(hUpdateTxn,"update_user",&csTmp)) {
					PutField_CString(hOrgTxn,"update_user",csTmp);
				}

				sprintf(csTag, "recon_status_%d", i);
				if (GetField_Char(hTxnList, csTag, &cTmp)) {
					if (cTmp == PD_UNRECONCILED) {
DEBUGLOG(("UpdatePayoutGenerateTxn: Update org recon_status [%c] to [%c]!\n", cTmp, PD_RECON_NOT_REQ));
						PutField_Char(hOrgTxn,"recon_status", PD_RECON_NOT_REQ);
					}
				}
				...
				DBObjPtr = CreateObj(DBPtr,"DBOLTransaction","Update");
				if((unsigned long) ((*DBObjPtr)(hOrgTxn))!=PD_OK){
DEBUGLOG(("UpdatePayoutGenerateTxn: Update OLTransaction ERROR!\n"));
					iRet = INT_INVALID_TXN;
					break;
				}
...
DEBUGLOG(("UpdatePayoutGenerateTxn: FAILED!\n"));
			iRet = INT_INVALID_TXN;
		}
	}

DEBUGLOG(("UpdatePayoutGenerateTxn:END!\n"));
