...
void TxnMgtByUsTFF(char    cdebug)
{
        cDebug = cdebug;
}
...
ERRLOG("TxnMgtOnUSTFF:Authorize() txn amount is missing\n");
	}
...
ERRLOG("TxnMgtOnUSTFF:Authorize() txn_country is missing\n");
	}
*/
...
ERRLOG("TxnMgtOnUSTFF:Authorize() merchant_id is missing\n");
	}
...
ERRLOG("TxnMgtOnUSTFF:Authorize() to_merchant_id is missing\n");
	}
...
ERRLOG("TxnMgtOnUSTFF:Authorize() service is missing\n");
	}
...
ERRLOG("TxnMgtOnUSTFF:Authorize() to_service is missing\n");
	}
...
ERRLOG("TxnMgtOnUSTFF:Authorize() txn_ccy is missing\n");
	}
...
ERRLOG("TxnMgtOnUSTFF:Authorize() dst_txn_ccy is missing\n");
	}
...
ERRLOG("TxnMgtOnUSTFF:Authorize() only allow same ccy and same service transfer\n");
		}
	}
*/
...
ERRLOG("TxnMgtOnUSTFF:Authorize() same account transfer\n");
		}
	}
*/
...
ERRLOG("TxnMgtOnUSTFF:Authorize() difference service_code not allow\n");
			PutField_Int(hContext,"internal_error",iRet);
		}
	}
*/
...
ERRLOG("TxnMgtByUsTFF::Authorize insufficient fund [%s] [%s] [%s] [%s] [%lf]\n",csCountry,csFromCcy,csFromService,csFromMid,dFromBalance);
			}
		}
...
DEBUGLOG(("TxnMgtByUsTFF:: Call BOTxnElements:AddTxnAmtElement\n"));
			BOObjPtr = CreateObj(BOPtr,"BOTxnElements","AddTxnAmtElement");
			iRet = (unsigned long)(*BOObjPtr)(hContext);
DEBUGLOG(("TxnMgtByUsTFF:: ret from AddTxnAmtElements = [%d]\n",iRet));
		}
		if (iRet == PD_OK) {
			PutField_CString(hContext,"src_txn_fee_ccy",csFromCcy);
			PutField_Double(hContext,"src_txn_fee",dTotalSrcFee);
DEBUGLOG(("TxnMgtByUsTFF:: Call BOTxnElements:AddTxnFeeElement\n"));
			BOObjPtr = CreateObj(BOPtr,"BOTxnElements","AddTxnFeeElements");
			iRet = (unsigned long)(*BOObjPtr)(hContext);
DEBUGLOG(("TxnMgtByUsTFF:: BOTxnElements: AddTxnFeeElements result = [%d]\n",iRet));
		}
...
DEBUGLOG(("TxnMgtByUsTFF:: ret from AddTxnAmtElements = [%d]\n",iRet));
		...
		if (iRet == PD_OK) {
			if (dMarkupAmt > 0.0) {
DEBUGLOG(("TxnMgtByUsTFF:: Prepare Call BOTxnElements:AddMarkupAmtElement\n"));
...
DEBUGLOG(("TxnMgtByUsTFF:: Call BOTxnElements:AddMarkupAmtElement\n"));
			BOObjPtr = CreateObj(BOPtr,"BOTxnElements","AddMarkupAmtElement");
			iRet = (unsigned long)(*BOObjPtr)(hContext);
DEBUGLOG(("TxnMgtByUsTFF:: BOTxnElements: AddMarkupAmtElement result = [%d]\n",iRet));
			}
		}
...
DEBUGLOG(("TxnMgtByUsTFF Normal Exit() iRet = [%d]\n",iRet));
	return iRet;
}
