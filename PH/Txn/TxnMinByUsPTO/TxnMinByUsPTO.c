...
DEBUGLOG(("Authorize:: Call DBPspDetail::GetPspDetail()\n"));
		...
		if (iRet != PD_OK) {
DEBUGLOG(("Authorize:: Call DBPspDetail::GetPspDetail() failed!!\n"));
ERRLOG("TxnMinByUsPTO::Authorize:: Call DBPspDetail::GetPspDetail() failed!!\n");
			iRet = INT_ERR;
		} else {
			if (GetField_Char(hTmp, "psp_type", &cPspType)) {
DEBUGLOG(("Authorize:: psp_type = [%c]\n", cPspType));
			} else {
DEBUGLOG(("Authorize:: psp_type not found!!\n"));
ERRLOG("TxnMinByUsPTO::Authorize:: psp_type not found!!\n");
				iRet = INT_ERR;
				PutField_Int(hContext, "internal_error", iRet);
			}
		}
...
//		PutField_Char(hTxn, "business_type", PD_BUSINESS_TYPE_GAMING);
		PutField_Char(hTxn, "business_type", cPspType);
...
DEBUGLOG(("Authorize:: psp product_code not found!!\n"));
ERRLOG("TxnMinByUsPTO::Authorize:: psp product_code not found!!\n");
				iRet = INT_ERR;
				PutField_Int(hContext, "internal_error", iRet);
                        }
		}
