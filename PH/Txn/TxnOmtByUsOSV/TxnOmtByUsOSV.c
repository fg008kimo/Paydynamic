...
-------------------------------                    ------------            --------------
Init Version                                       2016/05/12		   Dirk Wong
...
void TxnOmtByUsOSV(char    cdebug)
{
        cDebug = cdebug;
}
...
DEBUGLOG(("TxnOmtByUsOSV: Authroize()\n"));
...
ERRLOG("TxnOmtByUsOSV::Authorize::remark not found!!\n");
...
ERRLOG("TxnOmtByUsOSV::Authorize: INVALID org_sub_status = [%s]\n",csTmp);
			}
		} else {
DEBUGLOG(("Authorize: OrgSubStatus not found!!\n"));
ERRLOG("TxnOmtByUsOSV::Authorize: OrgSubStatus not found!!\n");
			iRet = INT_ERR;
		}
	}
