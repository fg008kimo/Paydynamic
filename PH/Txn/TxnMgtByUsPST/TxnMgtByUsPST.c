...
Add TxnMgtByUs2MiniMMM (Mini-MMM)		   2015/11/13		   Elvis Wong 
Bug fix, set dbcommit to false			   2016/02/03		   Dirk Wong
*/
...
//                PutField_CString(hContext,"remark",csTmp);
		  PutField_CString(hContext,"mi_remark",csTmp);		
        }
...
DEBUGLOG(("Authorize::Call DBTxnPspDetail:Update\n"));
	...
	// check if Mini-MMM mode is on
        if (iRet==PD_OK) {

/* cost_txn_seq */
		if (GetField_CString(hNewTxn,"txn_seq",&csTmp)) {
DEBUGLOG(("Authorize::cost_txn_seq = [%s]\n",csTmp));
                        PutField_CString(hContext,"cost_txn_seq",csTmp);
                } 
...
ERRLOG("TxnMgtByUsPST::Authorize::entity_id not found!!\n");
...
ERRLOG("TxnMgtByUsPST::Authorize::Call TxnMgtByUs2MiniMMM:Authorize FAILED, ret [%d]\n",iRet);
                }
        }
