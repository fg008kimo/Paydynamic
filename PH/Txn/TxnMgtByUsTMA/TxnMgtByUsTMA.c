...
Revised for PRD290                                 2020/11/24              [ZBL]
*/
...
/* batch_mode */
	GetField_Int(hContext, "batch_mode", &iBatchMode);

/* con_paid_amt */
	if (iBatchMode == PD_FALSE) {
		int iPaidAmtCheck = PD_FALSE;

		if (GetField_CString(hRequest, "con_paid_amt", &csPtr)) {
			char *csTmp = NULL;
			csTmp = (char*)strchr(csPtr, '.');

			if (csTmp == NULL) {
				iPaidAmtCheck = is_numeric(csPtr);

				if (iPaidAmtCheck != PD_FALSE) {
					if (sscanf(csPtr, "%lf", &dPaidAmt) == PD_TRUE) {
DEBUGLOG((" con_paid_amt = [%lf]\n", dPaidAmt));
					}
					else
						iPaidAmtCheck = PD_FALSE;
				}
			}
			else {
				if (sscanf(csPtr, "%lf", &dPaidAmt) == PD_TRUE) {
DEBUGLOG((" con_paid_amt = [%lf]\n", dPaidAmt));
					iPaidAmtCheck = PD_TRUE;
				}
			}

			if (iPaidAmtCheck == PD_FALSE) {
DEBUGLOG(("Invalid con_paid_amt!!\n"));
ERRLOG("TxnMgtByUsTMA::Authorize() Invalid con_paid_amt!!\n");

				iRet = INT_INVALID_PAID_AMOUNT;
				PutField_Int(hContext, "internal_error", iRet);
			}
		}
	}
...
/* PRD290 PSP Actual Amount From Deposit Callback */
	if (iRet == PD_OK) {
		// Check PSP is allowed to pay different amount or not if batch_mode is False (Transaction manual approval)
		if (iBatchMode == PD_FALSE) {
			int iIsAllowAmtDiff = PD_FALSE;
			int iIsInAmtDiffRange = PD_FALSE;

DEBUGLOG(("Call BOPspPaidAmount::IsAllowAmtDiff()\n"));
			BOObjPtr = CreateObj(BOPtr, "BOPspPaidAmount", "IsAllowAmtDiff");
			iIsAllowAmtDiff = (unsigned long)(*BOObjPtr)(hContext);

			if (iIsAllowAmtDiff == PD_TRUE) {
DEBUGLOG(("Call BOPspPaidAmount::IsAllowAmtDiff() True\n"));

				PutField_Double(hContext, "paid_amt", dPaidAmt);

DEBUGLOG(("Call BOPspPaidAmount::IsInAmtDiffRange()\n"));
				BOObjPtr = CreateObj(BOPtr, "BOPspPaidAmount", "IsInAmtDiffRange");
				iIsInAmtDiffRange = (unsigned long)(*BOObjPtr)(hContext);

				if (iIsInAmtDiffRange == PD_TRUE) {
DEBUGLOG(("Call BOPspPaidAmount::IsInAmtDiffRange() True\n"));
				}
				else {
DEBUGLOG(("Call BOPspPaidAmount::IsInAmtDiffRange() False\n"));
ERRLOG("TxnMgtByUsTMA::Authorize() Amount difference not in range!!\n");
...
DEBUGLOG(("Call BOPspPaidAmount::IsAllowAmtDiff() False\n"));

				// Assumed comsumer paid amount is same as transaction amount if PSP is not allowed to pay different amount
				if (GetField_Double(hContext, "org_dst_net_amt", &dPtr))
					PutField_Double(hContext, "paid_amt", dPtr);
				else {
DEBUGLOG((" org_dst_net_amt is missing!!\n"));
ERRLOG("TxnMgtByUsTMA::Authorize() org_dst_net_amt is missing!!\n");
			...
			// Assumed comsumer paid amount is same as transaction amount if batch_mode is True (Transaction manual approval by batch)
			if (GetField_Double(hContext, "org_dst_net_amt", &dPtr))
				PutField_Double(hContext, "paid_amt", dPtr);
			else {
DEBUGLOG((" org_dst_net_amt is missing!!\n"));
