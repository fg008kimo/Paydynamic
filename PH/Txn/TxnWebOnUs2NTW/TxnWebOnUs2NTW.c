...
OBJPTR(DB);
OBJPTR(BO);
int     UpdatePreNTWRespTxnLog(hash_t *hContext,
                	hash_t* hResponse);

int     AddNTWTxnLog(hash_t *hContext,
                const hash_t* hRequest);

void TxnWebOnUs2NTW(char    cdebug)
{
        cDebug = cdebug;
}
int     AddNTWTxnLog(hash_t *hContext,
...

{
	...
	hash_t	*hTxn;
	hash_t	*hRec;
DEBUGLOG(("TxnWebOnUs2NTW::UpdateRespTxnLog()\n"));
...
DEBUGLOG(("UpdateTxnLog  iRet = [%d] after Insert TxnQrRequestLog\n",iRet));
		...
		int	iResult;
        	...
        	iRet = (unsigned long)(*DBObjPtr)(PD_CHANNEL_N2P_AIPWECHAT,&iResult,csTmpStatus);
...
DEBUGLOG(("process_resp_txn()\n"));
	...
	PutField_CString(hContext,"psp_channel_code",PD_CHANNEL_N2P_AIPWECHAT);
	PutField_CString(hContext,"return_psp_channel",PD_CHANNEL_N2P_AIPWECHAT);
	...
	GetField_CString(hRequest,"psp_name",&csPspName);
      	MsgObjPtr = CreateObj(MsgPtr,"NtwMsg","BreakDownInitRspMsg");

        if ((*MsgObjPtr)(hResponse,inMsg->msg,inMsg->len) != PD_OK)  {
		iRet = INT_BREAKDOWN_ERR;
		PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("process_resp_txn:BreakDown NtwMsg Error\n"));
	}
//	else {
		// testing
...
DEBUGLOG(("process_resp_txn() call Update\n"));
		iRet = UpdatePreNTWRespTxnLog(hContext,hResponse);
...
DEBUGLOG(("TxbWebOnUs2NTW: time_redirect = [%d]\n",iTmp));
	        ...
	        if (!GetField_CString(hContext,"from_txn_seq",&csTxnSeq)) 
       		 	...
       		 	if ((*BOObjPtr)(hContext,hRequest,hRequest) == PD_OK) {
       		 		...
       		 		MsgObjPtr = CreateObj(MsgPtr,"NtwMsg","FormatInitMsg");
...
DEBUGLOG(("TxnWebOnUs2NTW: after process_resp_err_txn iret = [%d]\n",iRet));
					}
					else {
						msg->mtext[iRecvLen] = '\0';
DEBUGLOG(("TxnWebOnUs2NTW:  recv len = [%d]\n",iRecvLen));
DEBUGLOG(("TxnWebOnUs2NTW:  recv = [%s]\n",&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN));
						memcpy(h_msg->msg,&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN);
						iRet = process_resp_txn(hContext,h_msg,hRequest);
DEBUGLOG(("TxnWebOnUs2NTW: after process_resp_txn iret = [%d]\n",iRet));
					}
                		}
        			FREE_ME(msg);
        		}     
...
DEBUGLOG(("TxnWebOnUs2NTW: BuildData Error\n"));
		}
	}
...
DEBUGLOG(("TxnWebOnUs2NTW: noraml exit iret = [%d]\n",iRet));
	return iRet;
}
