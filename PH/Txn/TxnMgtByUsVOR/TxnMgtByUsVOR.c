...
void TxnMgtByUsVOR(char    cdebug)
{
        cDebug = cdebug;
}
...
ERRLOG("TxnMgtByUsVOR:Authorize::txnid not found!!\n");
...
ERRLOG("TxnMgtByUsVOR::Authorize() txn_amt Invalid\n");
                        }
                }
        }
...
ERRLOG("TxnMgtByUsVOR::Authorize: MMS Mode is ON, Txn unvoidable!!!!!!!!\n");
						iRet=INT_ERR;
					}
				}
			}
...
ERRLOG("TxnMgtByUsVOR::Authorize: The two txn_seq are not in a pair!!!!!!!!\n");
					}
				}
			}
...
/*Void Deposit*/
				if(!strcmp(csTxnCode,PD_DEPOSIT_TXN_CODE)||
					...
					if (iDepositChargeback == PD_TRUE) {
						PutField_CString(hContext,"new_txn_code",PD_DEPOSIT_CHARGEBACK_WITH_FEE);
					} else {
						PutField_CString(hContext,"new_txn_code",PD_DEPOSIT_VOID_WITH_FEE);
                			}
					sprintf(csHandler,"TxnMgtByUs%s",PD_DEPOSIT_VOID_WITH_FEE);
				}
/*Payout*/
...
DEBUGLOG(("TxnMgtByUsVOR Create Txn Object [%s]\n",csHandler));
	...
	FREE_ME(rRecordSet);
	//FREE_ME(hOptContext);
DEBUGLOG(("TxnMgtByUsVOR Normal Exit() iRet = [%d]\n",iRet));
	return iRet;
}
...
ERRLOG("TxnMgtByUsVOR:GetTxnHeader::Invalid status!!\n");
				}
				else{
...
ERRLOG("TxnMgtByUsVOR:Authorize::GetTxnHeader::Invalid ar_ind!!\n");
						}
					}
				}
			}
...
//ERRLOG("TxnMgtByUsVOR:Authorize::GetTxnHeader::not found record!!\n");
		iRet=INT_NOT_RECORD;
		//PutField_Int(hContext,"internal_error",iRet);
	}
...
ERRLOG("TxnMgtByUsVOR:Authorize::un-voidable txn!!\n");
		}
	}
