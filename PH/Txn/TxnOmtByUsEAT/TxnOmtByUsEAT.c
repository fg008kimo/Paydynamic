...
void TxnOmtByUsEAT(char    cdebug)
{
	cDebug = cdebug;
}
...
DEBUGLOG(("TxnOmtByUsEAT::Authorize\n"));
...
ERRLOG("TxnOmtByUsEAT::Authorize::entity_type not found!!\n");
...
ERRLOG("TxnOmtByUsEAT::Authorize::amt_type not found!!\n");
...
DEBUGLOG(("Authorize::desc = [%s]\n",csTmp));
		...
		PutField_CString(hMapTxnElement, "desc", csTmp);
	}
	else {
DEBUGLOG(("Authorize::desc not found!!\n"));
ERRLOG("TxnOmtByUsEAT::Authorize::desc not found!!\n");
...
ERRLOG("TxnOmtByUsEAT::Authorize::nature not found!!\n");
...
ERRLOG("TxnOmtByUsEAT::Authorize::bal_type not found!!\n");
...
ERRLOG("TxnOmtByUsEAT::Authorize::action not found!!\n");
	...
	PutField_Int(hTxn,"mi_ofl_fe_display",PD_TRUE);
...
DEBUGLOG(("Authorize::Call BOOLMiAdjustment:GetCurrentCode\n"));
		BOObjPtr = CreateObj(BOPtr,"BOOLMiAdjustment","GetCurrentCode");
		if((unsigned long)((*BOObjPtr)(hTxn) != PD_OK)){
DEBUGLOG(("Authorize::GetCurrentCode Failed\n"));
ERRLOG("TxnOmtByUsEAT::Authorize::GetCurrentCode Failed\n");
                        iRet=INT_CODE_ERROR;
...
DEBUGLOG(("Authorize:: call SystemParameter:FindCode prefix\n"));
				...
				if ((unsigned long)(DBObjPtr)(PD_MI_OL_RSP_ADJ_CODE_PREFIX,csValueTmp) == FOUND) {
					cTmp = csValueTmp[0];
...
/* Validate to process while update or delete */
	if (iRet == PD_OK) {
DEBUGLOG(("Authorize::Call BOOLMiAdjustment:ValidateProcess\n"));
                BOObjPtr = CreateObj(BOPtr,"BOOLMiAdjustment","ValidateProcess");	
                if((unsigned long)((*BOObjPtr)(hTxn) != PD_OK)){
DEBUGLOG(("Authorize::validateProcess Failed\n"));
/*ERRLOG("TxnOmtByUsEAT::Authorize::ValidateProcess Failed\n");*/
...
DEBUGLOG(("Authorize::TagAction = [%s]\n", csTagAction));
	                DBObjPtr = CreateObj(DBPtr,"DBMiOLAdjustmentType", csTagAction);
...
DEBUGLOG(("TxnOmtByUsEAT Normal Exit() iRet = [%d]\n",iRet));
	return iRet;
}
