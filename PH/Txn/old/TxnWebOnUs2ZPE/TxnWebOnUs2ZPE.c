...
OBJPTR(DB);
OBJPTR(BO);
int     UpdatePreZPERespTxnLog(hash_t *hContext,
                	hash_t* hResponse);

int     AddZPETxnLog(hash_t *hContext,
                const hash_t* hRequest);

void TxnWebOnUs2ZPE(char    cdebug)
{
        cDebug = cdebug;
}
int     AddZPETxnLog(hash_t *hContext,
...

{
	...
	hash_t	*hTxn;
	hash_t	*hRec;
DEBUGLOG(("TxnWebOnUs2ZPE::UpdateRespTxnLog()\n"));
...
DEBUGLOG(("UpdateTxnLog  iRet = [%d] after Update TxnPspDetail\n",iRet));
		...
		int	iResult;
        	...
        	iRet = (unsigned long)(*DBObjPtr)(PD_CHANNEL_ZPAY_EC,&iResult,csTmpStatus);
...
DEBUGLOG(("process_resp_txn()\n"));
	...
	PutField_CString(hContext,"psp_channel_code",PD_CHANNEL_ZPAY_EC);
	PutField_CString(hContext,"return_psp_channel",PD_CHANNEL_ZPAY_EC);
	...
	GetField_CString(hRequest,"psp_name",&csPspName);
      	MsgObjPtr = CreateObj(MsgPtr,"ZpeMsg","BreakDownInitRspMsg");

        if ((*MsgObjPtr)(hResponse,inMsg->msg,inMsg->len) != PD_OK)  {
		iRet = INT_BREAKDOWN_ERR;
		PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("process_resp_txn:BreakDown ZpeMsg Error\n"));
	}
//	else {
...
DEBUGLOG(("process_resp_txn() call Update\n"));
		iRet = UpdatePreZPERespTxnLog(hContext,hResponse);
	...
	if (iRet == PD_OK) {
	        ...
	        if (!GetField_CString(hContext,"from_txn_seq",&csTxnSeq)) 
       		 	...
       		 	if ((*BOObjPtr)(hContext,hRequest,hRequest) == PD_OK) {
       		 		...
       		 		MsgObjPtr = CreateObj(MsgPtr,"ZpeMsg","FormatInitMsg");
...
DEBUGLOG(("TxnWebOnUs2ZPE: after process_resp_err_txn iret = [%d]\n",iRet));
					}
					else {
						msg->mtext[iRecvLen] = '\0';
DEBUGLOG(("TxnWebOnUs2ZPE:  recv len = [%d]\n",iRecvLen));
DEBUGLOG(("TxnWebOnUs2ZPE:  recv = [%s]\n",&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN));
						memcpy(h_msg->msg,&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN);
						iRet = process_resp_txn(hContext,h_msg,hRequest);
DEBUGLOG(("TxnWebOnUs2ZPE: after process_resp_txn iret = [%d]\n",iRet));
					}
                		}
        			FREE_ME(msg);
        		}     
...
DEBUGLOG(("TxnWebOnUs2ZPE: BuildData Error\n"));
		}
	}

DEBUGLOG(("TxnWebOnUs2ZPE: noraml exit iret = [%d]\n",iRet));
	return iRet;
}
