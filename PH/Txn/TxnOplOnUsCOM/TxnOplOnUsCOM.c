...
-------------------------------                    ------------            --------------
Init Version (Clone from OLN)                      2017/03/13              [SWK]
Check Support Version				   2017/06/02              [SWK]
Revise parameters checking (Mantis 3047)	   2021/01/28		   [MSN]
*/
...
#define SUPPORT_VERSION_LIST  1

char	iSupportVersion[SUPPORT_VERSION_LIST]={1};
...
DEBUGLOG(("Authorize() transmission_datetime = [%s]\n",csTmp));
			if(check_valid_datetime(csTmp)!=PD_OK){
				iRet = INT_INVALID_TRASMISSION_TIME;
				PutField_Int(hContext,"internal_error",iRet);
...
DEBUGLOG(("Authorize() version_no =[%d]\n",iTmp));
			...
			int	pos = 0;
			int     iIsSupportVer = PD_FALSE;

			while(PD_TRUE) {

				if (pos >= SUPPORT_VERSION_LIST) {
					break; 
				}

				if (iTmp == iSupportVersion[pos]) {
					iIsSupportVer = PD_TRUE;
					break;
				} else{
					pos++;
				}
			...
			if (iIsSupportVer == PD_FALSE) {
                        	iRet = INT_INVALID_VERSION;
	                        PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("Authorize() version_no invalid \n"));
ERRLOG("TxnOplOnUsCOM::Authorize() version_no invalid \n");
			}
