...
To create cust_deposit_datetime for non-365        2014/08/27              Virginia Yun
*/
...
/* cust_deposit_datetime */
	if (GetField_CString(hRequest,"process_code",&csProcessCode)) {
		...
		/* Only for ODI, not for ODD */
		if (!strcmp(csProcessCode, PD_PROCESS_CODE_ODI)) {
                        char csDateTime[PD_DATETIME_LEN + 1];
                        strcpy(csDateTime,getdatetime());
                        csDateTime[PD_YYYYMMDDHHMM_LEN] = '\0';
                        PutField_CString(hContext,"cust_deposit_datetime",csDateTime);
                        PutField_Int(hContext,"cust_deposit_dt_prov",PD_FALSE);
