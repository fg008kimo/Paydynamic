		  ...
		  hash_t* hResponse)
{
	...
	hash_t *hMiEntityBalAcct;
	hMiEntityBalAcct = (hash_t*) malloc (sizeof(hash_t));
	hash_init(hMiEntityBalAcct,0);
...
DEBUGLOG(("Authorize::entity_id = [%s]\n",csEntityId));
                ...
                PutField_CString(hMiEntityBalAcct,"entity_id",csEntityId);
        }
	else {
...
/* txn_country */
	if(GetField_CString(hRequest,"txn_country",&csTmp)){
                ...
                PutField_CString(hMiEntityBalAcct,"country",csTmp);
...
DEBUGLOG(("Authorize::txn_ccy= [%s]\n",csTmp));
                ...
                PutField_CString(hMiEntityBalAcct,"ccy",csTmp);
        }
        else{
...
ERRLOG("TxnMgtByUsDLO::Authorize::total_cnt not found!!\n");
	...
	if (iRet == PD_OK) {
		hash_t *hRSP = (hash_t*) malloc (sizeof(hash_t));
		hash_init(hRSP,0);

		//Call DBMiEntityRsp::GetRSPInfoByEntityId
DEBUGLOG(("Authorize:: Call DBMiEntityRsp:GetRSPInfoByEntityId\n"));
		DBObjPtr = CreateObj(DBPtr,"DBMiEntityRsp","GetRSPInfoByEntityId");
		iDtlRet = ((unsigned long)(*DBObjPtr)(csEntityId,hRSP));

		if (iDtlRet != PD_OK) {
DEBUGLOG(("Authorize:: Call DBMiEntityRsp:GetRSPInfoByEntityId FAILED, iRet [%d]\n",iRet));
ERRLOG("TxnMgtByUsDLO:Authorize:: Call DBMiEntityRsp:GetRSPInfoByEntityId FAILED, iRet [%d]\n",iRet);
			iRet = iDtlRet;
			PutField_Int(hContext,"internal_error",iRet);
		} else {
			if (GetField_CString(hRSP,"status",&csTmp)) {
DEBUGLOG(("Authorize::  status = [%s]\n",csTmp));
				if (strcmp(csTmp,PD_MI_ENTITY_BAL_ACCT_STATUS_OPEN)) {
DEBUGLOG(("Authorize::  status NOT ALLOW!\n"));
ERRLOG("TxnMgtByUsDLO:Authorize::  status NOT ALLOW!\n");
					iRet = INT_ERR;
					PutField_Int(hContext,"internal_error",iRet);
				}
			}
	...
	if (iRet == PD_OK) {
		...
		//Call DBMiEntityBalAcct::GetEntityBalAcct
DEBUGLOG(("Authorize:: Call DBMiEntityBalAcct::GetEntityBalAcct\n"));
		DBObjPtr = CreateObj(DBPtr,"DBMiEntityBalAcct","GetEntityBalAcct");
		iDtlRet = ((unsigned long)(*DBObjPtr)(hMiEntityBalAcct,hTmp));

		if (iDtlRet != PD_FOUND) {
DEBUGLOG(("Authorize:: Call DBMiEntityBalAcct:GetEntityBalAcct FAILED, iRet [%d]\n",iRet));
ERRLOG("TxnMgtByUsDLO:Authorize:: Call DBMiEntityBalAcct:GetEntityBalAcct FAILED, iRet [%d]\n",iRet);
			iRet = iDtlRet;
			PutField_Int(hContext,"internal_error",iRet);
		} else {
...
ERRLOG("TxnMgtByUsDLO::Authorize::Call TxnMgtByUs2MiniMMM:Authorize FAILED, ret [%d]\n",iRet);
	...
	hash_destroy(hMiEntityBalAcct);
	FREE_ME(hMiEntityBalAcct);
