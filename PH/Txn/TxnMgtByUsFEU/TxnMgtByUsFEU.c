...
Support to get FE URL with New UI		   2013/10/03		   Virginia Yun
*/
...
DEBUGLOG(("Authorize ::language = [%s]\n",csLanguage));
				...
				if (GetField_CString(hRec, "customer_tag", &csCustomerTag)) {
DEBUGLOG(("Authorize ::customer_tag =[%s]\n", csCustomerTag));
					PutField_CString(hContext, "customer_tag", csCustomerTag);
				}
