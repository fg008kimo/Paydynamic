...
-------------------------------                    ------------            --------------
Init Version                                       2016/07/04              Elvis Wong
*/
...
void TxnOmtByUsVIM(char    cdebug)
{
        cDebug = cdebug;
}
...
ERRLOG("TxnOmtByUsVIM::Authorize::merchant_id not found!!\n");
...
ERRLOG("TxnOmtByUsVIM::Authorize::service_code not found!!\n");
...
ERRLOG("TxnOmtByUsVIM::Authorize::remark not found!!\n");
...
DEBUGLOG(("Authorize::return_mfee = [%d]\n",iTmp));
		PutField_Int(hContext,"return_mfee",iTmp);
        }
...
/* markup_amt */
			if (GetField_Double(hTxn, "markup_amt", &dTmp)) {
DEBUGLOG(("Authorize::markup_amt = [%lf]\n",dTmp));
                	        PutField_Double(hContext, "markup_amt", dTmp);
                	}

/* markup_rate */
                	if (GetField_Double(hTxn, "markup_rate", &dTmp)) {
DEBUGLOG(("Authorize::markup_rate = [%lf]\n",dTmp));
...
DEBUGLOG(("Authorize::ex_rate = [%lf]\n",dTmp));
	...
	if (iRet == PD_OK){
DEBUGLOG(("Authorize::Call BOOLBalance: ProcessFundinPayout\n"));
                BOObjPtr = CreateObj(BOPtr,"BOOLBalance","ProcessFundinPayout");
                iRet = (unsigned long)((*BOObjPtr)(hContext));
	}

	// Update OL Txn Header (Org Txn Seq)
	if (iRet == PD_OK) {
DEBUGLOG(("Authorize::Call Update Org OLTransaction\n"));
...
DEBUGLOG(("TxnOmtByUsVIM Normal Exit() iRet = [%d]\n",iRet));
	return iRet;
}
...
ERRLOG("TxnOmtByUsVIM:Authorize::GetTxnHeader::not found record!!\n");
			iRet=INT_NOT_RECORD;
		}
...
ERRLOG("TxnOmtByUsVIM::GetTxnInfo::GetTxnDetail::not found record!!\n");
			iRet=INT_NOT_RECORD;
		}
	...
	// Get OL Txn Elements (Txn Amount)
        if (iRet == PD_OK) {
		recordset_init(rRecordSet,0);

DEBUGLOG(("GetTxnInfo::Call DBOLTxnElements:GetFeeChgDetailByType [%s]\n", PD_ELEMENT_TXN_AMT));
...
ERRLOG("TxnOmtByUsVIM::GetTxnInfo::GetFeeChgDetailByType:: [%s] not found record for [%s]!!\n", PD_ELEMENT_TXN_AMT, csTxnSeq);
			iRet=INT_NOT_RECORD;
		}
...
ERRLOG("TxnOmtByUsVIM::GetTxnInfo::GetFeeChgDetailByType:: [%s] not found record for [%s]!!\n",  PD_ELEMENT_TXN_FEE , csTxnSeq);
	...
	// Get OL Txn Elements (Markup Amount)
        if (iRet == PD_OK) {
                recordset_init(rRecordSet,0);

DEBUGLOG(("GetTxnInfo::Call DBOLTxnElements:GetFeeChgDetailByType [%s]\n",  PD_ELEMENT_MARKUP_AMT ));
                DBObjPtr = CreateObj(DBPtr,"DBOLTxnElements","GetFeeChgDetailByType");
                if ((*DBObjPtr)(csTxnSeq,  PD_ELEMENT_MARKUP_AMT , PD_TYPE_CUSTOMER, rRecordSet) == PD_OK) {
DEBUGLOG(("GetTxnInfo::GetFeeChgDetailByType::[%s] found record = [%s]\n", PD_ELEMENT_MARKUP_AMT, csTxnSeq));
                        hRec = RecordSet_GetFirst(rRecordSet);
			while (hRec) {
/* amount */
				if (GetField_Double(hRec, "amount", &dTmp)) {
...
DEBUGLOG(("GetTxnInfo::GetFeeChgDetailByType::[%s] dMarkupAmt = [%lf], dMarkupRate = [%lf]\n", PD_ELEMENT_MARKUP_AMT , dMarkupAmt , dMarkupRate));

			// markup_amt
			PutField_Double(hTxn, "markup_amt", dMarkupAmt);

			// markup_rate
			PutField_Double(hTxn, "markup_rate", dMarkupRate);
		}
                else{
DEBUGLOG(("GetTxnInfo::GetFeeChgDetailByType:: [%s] not found record for [%s]\n", PD_ELEMENT_MARKUP_AMT , csTxnSeq));
ERRLOG("TxnOmtByUsVIM::GetTxnInfo::GetFeeChgDetailByType:: [%s] not found record for [%s]!!\n",  PD_ELEMENT_MARKUP_AMT , csTxnSeq);
