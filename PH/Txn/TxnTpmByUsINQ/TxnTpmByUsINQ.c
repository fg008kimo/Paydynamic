...
-------------------------------                    ------------            --------------
Init Version                                       2015/03/04              LokMan Chow
*/
...
#include "ObjPtr.h"
#include "internal.h"
#include "TxnTpmByUsINQ.h"
#include "myrecordset.h"
#include <curl/curl.h>
#include "queue_utility.h"
#include "mq_db.h"

#define	PD_SUPPORTED_PSP_CHANNEL 	"TPM"
...
void TxnTpmByUsINQ(char    cdebug)
{
        cDebug = cdebug;
}
...
DEBUGLOG(("GetTxnPspByPspId - psp_key = [%s]\n",csPtr));
	...
	if (iRet == PD_OK) {
		char*   csValueTmp;
                ...
                if ((unsigned long)(DBObjPtr)("TPM_INQ_URL",csValueTmp) == FOUND) {
...
DEBUGLOG(("GetTxnPsp - url_method = [%s]\n",csPtr));
	...
	if (iRet == PD_OK) {
		BOObjPtr = CreateObj(BOPtr,"BOSecurity","GeneratePspSignINQ");
			...
			MsgObjPtr = CreateObj(MsgPtr,"TpmMsg","FormatInqMsg");
...
DEBUGLOG(("BreakDownInqRespMsg\n"));
	p = strstr((char*)csMsg,"<transaction_id>");
	if(p){
		p = GetXmlField((char*)csMsg, "<transaction_id>",'<');
		if (p) {
			PutField_CString(hMsg,"tid",p);
DEBUGLOG(("transaction_id [%s]\n",p));
		}
	}
	p = strstr((char*)csMsg,"<trade_state>");
	if(p){
		stat = GetXmlField((char*)csMsg, "<trade_state>",'<');
		if (stat) {
			PutField_CString(hMsg,"status",stat);
DEBUGLOG(("trade_state [%s]\n",stat));
		}
	}
	p = strstr((char*)csMsg,"<total_fee>");
	if(p){
		p = GetXmlField((char*)csMsg, "<total_fee>",'<');
		if (p) {
			PutField_CString(hMsg,"txn_amt",p);
DEBUGLOG(("total_fee [%s]\n",p));
		}
	}
	p = strstr((char*)csMsg,"<time_end>");
	if(p){
		p = GetXmlField((char*)csMsg, "<time_end>",'<');
		if (p) {
			PutField_CString(hMsg,"time_end",p);
DEBUGLOG(("time_end [%s]\n",p));
		}
	}

	p = strstr((char*)csMsg,"<retmsg>");
	if(p){
		retmsg = GetXmlField((char*)csMsg, "<retmsg>",'<');
		if (retmsg) {
DEBUGLOG(("retmsg [%s]\n",retmsg));
		}
	}
	p = strstr((char*)csMsg,"<retcode>");
	if(p){
		ret = GetXmlField((char*)csMsg, "<retcode>",'<');
		if (ret) {
DEBUGLOG(("retcode [%s]\n",ret));
		}
	}

	p = strstr((char*)csMsg,"<partner>");
	if(p){
		p = GetXmlField((char*)csMsg, "<partner>",'<');
		if (p) {
			PutField_CString(hMsg,"mid",p);
DEBUGLOG(("partner [%s]\n",p));
		}
	}
...
DEBUGLOG(("out_trade_no [%s]\n",p));
	...
	p = strstr((char*)csMsg,"<bank_type>");
	if(p){
		p = GetXmlField((char*)csMsg, "<bank_type>",'<');
		if (p) {
			PutField_CString(hMsg,"ret_bank_code",p);
DEBUGLOG(("bank_type [%s]\n",p));
		}
	}
	p = strstr((char*)csMsg,"<bank_billno>");
	if(p){
		p = GetXmlField((char*)csMsg, "<bank_billno>",'<');
		if (p) {
			PutField_CString(hMsg,"bank_bill_no",p);
DEBUGLOG(("bank_billno [%s]\n",p));
		}
	}
...
DEBUGLOG(("not a success response\n"));
		...
		PutField_CString(hMsg,"ver","2.0");
		PutField_CString(hMsg,"charset","UTF-8");
		PutField_CString(hMsg,"fee_type","1");
	}
