...
PRD067 Add txn_ex_detail			   2017/04/28		   David Wong
*/
...
ERRLOG("TxnMgtByUsVPM::Authorize::remark not found!!\n");
...
DEBUGLOG(("Authorize::Call MGTChannel:UpdateTxnDetailLog\n"));
	...
	// Add txn_ex_detail
	if (iRet == PD_OK) {
DEBUGLOG(("Authorize::Call DBTxnExDetail:Get\n"));
		DBObjPtr = CreateObj(DBPtr, "DBTxnExDetail", "Get");
		if ((unsigned long)((*DBObjPtr)(csOrgTxnSeq, hTxnExDt)) != PD_OK) {
			iRet = INT_ERR;
		}

		if (GetField_CString(hContext, "txn_seq", &csTmp)) {
			PutField_CString(hTxnExDt, "txn_seq", csTmp);
		}

DEBUGLOG(("Authorize::Call DBTxnExDetail:Add\n"));
...
ERRLOG("TxnMgtByUsVPM::GetTxnInfo::GetFeeChgDetailByType:: [%s] not found record for [%s]!!\n",  PD_ELEMENT_TXN_FEE , csTxnSeq);
	...
	// Get Txn Elements (Markup Amount)
        if (iRet == PD_OK) {
                recordset_init(rRecordSet,0);
