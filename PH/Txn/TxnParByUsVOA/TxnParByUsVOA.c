...
-------------------------------                    ------------            --------------
Init Version                                       2012/12/28              Virginia Yun
*/
...
void TxnParByUsVOA(char    cdebug)
{
        cDebug = cdebug;
}
...
DEBUGLOG(("TxnParByUsVOA: Authroize()\n"));
...
//******************
	if (GetField_Int(hContext, "exists_poa", &iPOAExists)) {
DEBUGLOG(("Authorize:: POA Exists [%d]\n", iPOAExists));
	}
...
DEBUGLOG(("Authorize::Call DBMerchantUploadFileDetail:GetDetailByTxnId\n"));
	...
	if(iRet==PD_OK){
			...
			if(GetField_Int(hRec,"status",&iStatus)){
				/* not check status for parallel_run
				if(iStatus!=PAYOUT_MASTER_TRANSACTION_GENERATED &&
				   iStatus != PAYOUT_MASTER_TRANSACTION_APPROVED){ // this for parallel run only!!!
...
ERRLOG("TxnParByUsVOA:Authorize::GetTxnHeader::not found record!!\n");
                        iRet=INT_NOT_RECORD;
                        PutField_Int(hContext,"internal_error",iRet);
                }
		} 
...
//*************
		// iPOAExists not exists!!	
DEBUGLOG(("POA Txn not exists!\n"));		

			if (GetField_CString(hRequest, "merchant_id", &csTmp)) {
DEBUGLOG(("POA Not Exists:: merchant_id [%s]\n", csTmp));
				PutField_CString(hBalance, "org_merchant_id", csTmp);
				PutField_CString(hContext, "merchant_id", csTmp);
			}
			if (GetField_CString(hContext, "merchant_ref", &csTmp)) {
DEBUGLOG(("POA Not Exists:: merchant_ref [%s]\n", csTmp));
			}
			if (GetField_CString(hRequest, "client_id", &csTmp)) {
DEBUGLOG(("POA Not Exists:: client_id [%s]\n", csTmp));
				PutField_CString(hRequest, "merchant_client_id", csTmp);
				PutField_CString(hContext, "client_id", csTmp);
				PutField_CString(hContext, "merchant_client_id", csTmp);
			}
			if (GetField_CString(hRequest, "service_code", &csTmp)) {
DEBUGLOG(("POA Not Exists:: service_code [%s]\n", csTmp));
				PutField_CString(hBalance, "org_service_code", csTmp);
				PutField_CString(hContext, "service_code", csTmp);
			}
			if (GetField_Double(hContext, "txn_amt", &dTmp)) {
DEBUGLOG(("POA Not Exists:: txn_amt [%lf]\n", dTmp));
				PutField_Double(hContext, "org_txn_amt", dTmp);
			}
			if (GetField_CString(hContext, "net_ccy", &csTmp)) {
DEBUGLOG(("POA Not Exists:: net_ccy [%s]\n", csTmp));
			}
...
ERRLOG("TxnParByUsVOA:Authorize::GetTxnDetail:: not found record!!\n");
                        iRet = INT_NOT_RECORD;
                        PutField_Int(hContext,"internal_error",iRet);
                }
		} else {
//*************
			// POA Txn Record not exists
DEBUGLOG(("POA Txn not exists!\n"));		
			if (GetField_CString(hContext, "txn_ccy", &csTmp)) {
DEBUGLOG(("POA not exists:: txn_ccy [%s]\n", csTmp));
				PutField_CString(hBalance, "org_txn_ccy", csTmp);
				PutField_CString(hRequest, "txn_ccy", csTmp);
			}
			if (GetField_CString(hRequest, "txn_country", &csTmp)) {
DEBUGLOG(("POA not exists:: txn_country [%s]\n", csTmp));
				PutField_CString(hBalance, "org_txn_country", csTmp);
				PutField_CString(hContext, "txn_country", csTmp);
			}
			if (GetField_CString(hContext, "bank_name", &csTmp)) {
DEBUGLOG(("POA not exists:: bank_name [%s]\n", csTmp));
				PutField_CString(hBalance, "bank_name", csTmp);
			}
			if (GetField_CString(hContext, "branch", &csTmp)) {
DEBUGLOG(("POA not exists:: branch[%s]\n", csTmp));
				PutField_CString(hBalance, "branch", csTmp);
			}
			if (GetField_CString(hContext, "account_id", &csTmp)) {
DEBUGLOG(("POA not exists:: account_id [%s]\n", csTmp));
				PutField_CString(hBalance, "account_id", csTmp);
			}
			if (GetField_CString(hContext, "account_name", &csTmp)) {
DEBUGLOG(("POA not exists:: account_name [%s]\n", csTmp));
				PutField_CString(hBalance, "account_name", csTmp);
			}
...
ERRLOG("TxnParByUsVOA:Authorize::BOFee:GetTxnFee Error\n");
                }
		if(iReturnFee==PD_TRUE)
...
ERRLOG("TxnParByUsVOA::DBPayoutGeneratedFileDT Update Failed!!!\n");
						break;
					}
				}
...
/**********/
PutField_Int(hContext,"generate_txn_cnt",1);
...
DEBUGLOG(("Call BOBalance:UpdatePayoutAmount()\n"));
			...
			*/
			if (GetField_Char(hContext, "party_type", &cTmp)) {
				PutField_Char(hBalance,"party_type",cTmp);
			}
...
DEBUGLOG(("TxnParByUsVOA: Authroize:: BOBalance:UpdatePayoutAmount() result = [%d]\n",iRet));
		}
	}
