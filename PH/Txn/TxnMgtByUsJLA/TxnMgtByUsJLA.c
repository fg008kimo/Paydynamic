/*
Partnerdelight (c)2010. All rights reserved. No part of this software may be reproduced in any form without written permission
of an authorized representative of Partnerdelight.

Change Description                                 Change Date             Change By
-------------------------------                    ------------            --------------
Init Version                                       2011/05/18              Simon Fung
*/

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "common.h"
#include "utilitys.h"
#include "ObjPtr.h"
#include "internal.h"
#include "TxnMgtByUsJLA.h"
#include "myrecordset.h"

char cDebug;
OBJPTR(DB);
OBJPTR(BO);

void TxnMgtByUsJLA(char    cdebug)
{
        cDebug = cdebug;
}

int     Authorize(hash_t* hContext,
                        const hash_t* hRequest,
                        hash_t* hResponse)
{
  int     iRet = PD_OK;
	char*	csTmp = NULL;
	hash_t	*hJnlHdr;

	hJnlHdr = (hash_t*) malloc (sizeof(hash_t));
	hash_init(hJnlHdr,0);

DEBUGLOG(("TxnMgtByUsJLA::Authorize()\n"));

		// jnl_id //
	if (GetField_CString(hRequest,"jnl_id",&csTmp)) {
DEBUGLOG(("TxnMgtByUsJLA::Authorize(): jnl_id = [%s]\n",csTmp));
		PutField_CString(hJnlHdr,"jnl_id",csTmp);
	}
	
		// approve_user //
	if (GetField_CString(hRequest,"approve_user",&csTmp)) {
DEBUGLOG(("TxnMgtByUsJLA::Authorize(): approve_user = [%s]\n",csTmp));
		PutField_CString(hJnlHdr,"approve_user",csTmp);
	}
	
	// Approve Journal Header
	BOObjPtr = CreateObj(DBPtr,"BOCrrJnl","Approve");
	iRet = (unsigned long) ((*BOObjPtr)(hJnlHdr));		

	if(iRet==PD_OK) {
DEBUGLOG(("TxnMgtByUsJLA::Authorize->Approve Success\n"));		
	} else {
DEBUGLOG(("TxnMgtByUsJLA::Authorize->Approve Failed\n"));	
	}
	
	if (iRet != PD_OK)
		PutField_Int(hContext,"internal_error",INT_JL_INVALID_MSG_FORMAT);	

	hash_destroy(hJnlHdr);
	FREE_ME(hJnlHdr);
DEBUGLOG(("TxnMgtByUsJLA::Authorize() Normal exit iret = [%d]\n",iRet));	
	return iRet;
}
