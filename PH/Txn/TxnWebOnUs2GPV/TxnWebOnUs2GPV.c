...
-------------------------------                    ------------            --------------
Init Version                                       2018/04/24              David Wong
*/
...
int UpdatePreGPVRespTxnLog(hash_t *hContext,
		hash_t *hResponse);


int AddGPVTxnLog(hash_t *hContext,
		const hash_t *hRequest);


void TxnWebOnUs2GPV(char cdebug)
{
	cDebug = cdebug;
}
		...
		hash_t *hResponse)
{
	...
	hash_t *hRec;
	hash_t *hInfo;
DEBUGLOG(("TxnWebOnUs2GPV::UpdateRespTxnLog()\n"));
...
DEBUGLOG(("UpdateTxnLog RESP:: txn_seq = [%s]\n", csTxnSeq));
		...
		PutField_CString(hInfo, "txn_id", csTxnSeq);
	}
...
/* redirectUrl */
/*
	if (GetField_CString(hResponse, "redirectUrl", &csPtr)) {
DEBUGLOG(("UpdateTxnLog RESP:: redirectUrl = [%s]\n", csPtr));
		PutField_CString(hPspDetail, "deposit_url", csPtr);
		PutField_CString(hContext, "redirect_url", csPtr);
	}
*/

/* credential */
	if (GetField_CString(hResponse, "credential", &csPtr)) {
DEBUGLOG(("UpdateTxnLog RESP:: credential = [%s]\n", csPtr));
		// insert into txn_supp_info due to value too large
		PutField_CString(hInfo, "info_type", PD_PSP_REDIRECT_URL);
		PutField_CString(hInfo, "info_value", csPtr);
		PutField_CString(hInfo, "add_user", PD_UPDATE_USER);
		DBObjPtr = CreateObj(DBPtr, "DBTxnSuppInfo", "Add");
		iTmpRet = (unsigned long)(*DBObjPtr)(hInfo);
DEBUGLOG(("UpdateTxnLog RESP:: iTmpRet = [%d] after insert TxnSuppInfo\n", iTmpRet));

		PutField_CString(hContext, "credential", csPtr);
	}
...
DEBUGLOG(("UpdateTxnLog iRet = [%d] after Update TxnPspDetail\n", iRet));
	...
	if (iRet == PD_OK) {
		...
		iRet = (unsigned long)(*DBObjPtr)(PD_CHANNEL_GOLDENPAY_VNET, &iResult, csTmpStatus);
...
DEBUGLOG(("process_resp_txn()\n"));
	...
	PutField_CString(hContext, "psp_channel_code", PD_CHANNEL_GOLDENPAY_VNET);
	PutField_CString(hContext, "return_psp_channel", PD_CHANNEL_GOLDENPAY_VNET);
	...
	MsgObjPtr = CreateObj(MsgPtr, "GpvMsg", "BreakDownInitRspMsg");
	if ((*MsgObjPtr)(hResponse, inMsg->msg, inMsg->len) != PD_OK) {
		iRet = INT_BREAKDOWN_ERR;
		PutField_Int(hContext, "internal_error", iRet);
DEBUGLOG(("process_resp_txn:BreakDown GpvMsg Error\n"));
	}
//	else {
...
DEBUGLOG(("process_resp_txn: cannot get requestId!!!\n"));
					PutField_CString(hResponse, "return_code", PD_FAIL_STATUS);
					iSendAlert = PD_TRUE;
					sprintf(csCmd, "send_init_req_error.sh \"%s\" \"%s\" \"%s\"", PD_INVALID_FIELD, csPspId, csPspName);
					goto skipped;
...
*/
				...
				// get credential
				if (!GetField_CString(hResponse, "credential", &csTmp)) {
DEBUGLOG(("process_resp_txn: cannot get credential!!!\n"));
					PutField_CString(hResponse, "return_code", PD_FAIL_STATUS);
					iSendAlert = PD_TRUE;
					sprintf(csCmd, "send_init_req_error.sh \"%s\" \"%s\" \"%s\"", PD_INVALID_FIELD, csPspId, csPspName);
					goto skipped;
...
DEBUGLOG(("process_resp_txn() call Update\n"));
		iRet = UpdatePreGPVRespTxnLog(hContext, hResponse);
//	}
...
DEBUGLOG(("TxbWebOnUs2GPV: txn_code = [%s]\n", csTxnCode));
	...
	if (iRet == PD_OK) {
		...
		if ((*BOObjPtr)(hContext, hRequest, hRequest) == PD_OK) {
			MsgObjPtr = CreateObj(MsgPtr, "GpvMsg", "FormatInitMsg");
...
DEBUGLOG(("TxnWebOnUs2GPV: after process_resp_err_txn iret = [%d]\n", iRet));
					}
					else {
						msg->mtext[iRecvLen] = '\0';
DEBUGLOG(("TxnWebOnUs2GPV:  recv len = [%d]\n", iRecvLen));
DEBUGLOG(("TxnWebOnUs2GPV:  recv = [%s]\n", &msg->mtext[MQ_HEADER_LEN]));
						memcpy(h_msg->msg, &msg->mtext[MQ_HEADER_LEN], iRecvLen - MQ_HEADER_LEN);
						iRet = process_resp_txn(hContext, h_msg, hRequest);
DEBUGLOG(("TxnWebOnUs2GPV: after process_resp_txn iret = [%d]\n", iRet));
					}
				}
				FREE_ME(msg);
			}
...
DEBUGLOG(("TxnWebOnUs2GPV: BuildData Error\n"));
		}
	}

DEBUGLOG(("TxnWebOnUs2GPV: noraml exit iret = [%d]\n", iRet));
	return iRet;
}
