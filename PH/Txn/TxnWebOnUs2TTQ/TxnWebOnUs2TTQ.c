...
OBJPTR(DB);
OBJPTR(BO);
int     UpdatePreTTQRespTxnLog(hash_t *hContext,
                	hash_t* hResponse);

int     AddTTQTxnLog(hash_t *hContext,
                const hash_t* hRequest);

void TxnWebOnUs2TTQ(char    cdebug)
{
        cDebug = cdebug;
}
int     AddTTQTxnLog(hash_t *hContext,
...

{
	...
	hash_t	*hTxn;
	hash_t	*hRec;
DEBUGLOG(("TxnWebOnUs2TTQ::UpdateRespTxnLog()\n"));
...
DEBUGLOG(("UpdateTxnLog RESP:: return_code = [%s]\n",csStatus))
		...
		if (!strcmp(csStatus, PD_TTQ_SUCCESS_STATUS))
...
DEBUGLOG(("UpdateTxnLog  iRet = [%d] after Insert TxnQrRequestLog\n",iRet));
		...
		int	iResult;
        	...
        	iRet = (unsigned long)(*DBObjPtr)(PD_CHANNEL_TTPAY_QR,&iResult,csTmpStatus);
...
DEBUGLOG(("process_resp_txn()\n"));
	...
	PutField_CString(hContext,"psp_channel_code",PD_CHANNEL_TTPAY_QR);
	PutField_CString(hContext,"return_psp_channel",PD_CHANNEL_TTPAY_QR);
	...
	GetField_CString(hRequest,"psp_name",&csPspName);
      	MsgObjPtr = CreateObj(MsgPtr,"TtqMsg","BreakDownInitRspMsg");

        if ((*MsgObjPtr)(hResponse,inMsg->msg,inMsg->len) != PD_OK)  {
		iRet = INT_BREAKDOWN_ERR;
		PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("process_resp_txn:BreakDown TtqMsg Error\n"));
	}
//	else {
...
DEBUGLOG(("process_resp_txn: returned return_code = [%s]\n", csTmp));
			...
			if (!strcmp(csTmp, PD_TTQ_SUCCESS_STATUS)) {
...
DEBUGLOG(("process_resp_txn() call Update\n"));
		iRet = UpdatePreTTQRespTxnLog(hContext,hResponse);
...
DEBUGLOG(("TxbWebOnUs2TTQ: time_redirect = [%d]\n",iTmp));
	        ...
	        if (!GetField_CString(hContext,"from_txn_seq",&csTxnSeq)) 
       		 	...
       		 	if ((*BOObjPtr)(hContext,hRequest,hRequest) == PD_OK) {
       		 		...
       		 		MsgObjPtr = CreateObj(MsgPtr,"TtqMsg","FormatInitMsg");
...
DEBUGLOG(("TxnWebOnUs2TTQ: after process_resp_err_txn iret = [%d]\n",iRet));
					}
					else {
						msg->mtext[iRecvLen] = '\0';
DEBUGLOG(("TxnWebOnUs2TTQ:  recv len = [%d]\n",iRecvLen));
DEBUGLOG(("TxnWebOnUs2TTQ:  recv = [%s]\n",&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN));
						memcpy(h_msg->msg,&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN);
						iRet = process_resp_txn(hContext,h_msg,hRequest);
DEBUGLOG(("TxnWebOnUs2TTQ: after process_resp_txn iret = [%d]\n",iRet));
					}
                		}
        			FREE_ME(msg);
        		}     
...
DEBUGLOG(("TxnWebOnUs2TTQ: BuildData Error\n"));
		}
	}
...
DEBUGLOG(("TxnWebOnUs2TTQ: noraml exit iret = [%d]\n",iRet));
	return iRet;
}
