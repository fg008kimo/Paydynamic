...
-------------------------------                    ------------            --------------
Init Version                                       2019/07/19              David Wong
*/
...
OBJPTR(DB);
OBJPTR(BO);
int     UpdatePreXYQRespTxnLog(hash_t *hContext,
                	hash_t* hResponse);

int     AddXYQTxnLog(hash_t *hContext,
                const hash_t* hRequest);

void TxnWebOnUs2XYQ(char    cdebug)
{
        cDebug = cdebug;
}
int     AddXYQTxnLog(hash_t *hContext,
...

{
	...
	hash_t	*hTxn;
	hash_t	*hRec;
DEBUGLOG(("TxnWebOnUs2XYQ::UpdateRespTxnLog()\n"));
...
DEBUGLOG(("UpdateTxnLog  iRet = [%d] after Insert TxnQrRequestLog\n",iRet));
		...
		int	iResult;
        	...
        	iRet = (unsigned long)(*DBObjPtr)(PD_CHANNEL_ZXPAY_QR,&iResult,csTmpStatus);
...
DEBUGLOG(("process_resp_txn()\n"));
	...
	PutField_CString(hContext,"psp_channel_code",PD_CHANNEL_ZXPAY_QR);
	PutField_CString(hContext,"return_psp_channel",PD_CHANNEL_ZXPAY_QR);
	...
	GetField_CString(hRequest,"psp_name",&csPspName);
      	MsgObjPtr = CreateObj(MsgPtr,"XyqMsg","BreakDownInitRspMsg");

        if ((*MsgObjPtr)(hResponse,inMsg->msg,inMsg->len) != PD_OK)  {
		iRet = INT_BREAKDOWN_ERR;
		PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("process_resp_txn:BreakDown XyqMsg Error\n"));
	}
//	else {
		// testing
...
DEBUGLOG(("process_resp_txn() call Update\n"));
		iRet = UpdatePreXYQRespTxnLog(hContext,hResponse);
...
DEBUGLOG(("TxbWebOnUs2XYQ: time_redirect = [%d]\n",iTmp));
			...
			PutField_Int((hash_t*)hRequest,"is_init_req",PD_TRUE);
       		 	...
       		 	if ((*BOObjPtr)(hContext,hRequest,hRequest) == PD_OK) {
       		 		...
       		 		MsgObjPtr = CreateObj(MsgPtr,"XyqMsg","FormatInitMsg");
...
DEBUGLOG(("TxnWebOnUs2XYQ: after process_resp_err_txn iret = [%d]\n",iRet));
					}
					else {
						msg->mtext[iRecvLen] = '\0';
DEBUGLOG(("TxnWebOnUs2XYQ:  recv len = [%d]\n",iRecvLen));
DEBUGLOG(("TxnWebOnUs2XYQ:  recv = [%s]\n",&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN));
						memcpy(h_msg->msg,&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN);
						iRet = process_resp_txn(hContext,h_msg,hRequest);
DEBUGLOG(("TxnWebOnUs2XYQ: after process_resp_txn iret = [%d]\n",iRet));
					}
                		}
        			FREE_ME(msg);
        		}     
...
DEBUGLOG(("TxnWebOnUs2XYQ: BuildData Error\n"));
		}
	}
...
DEBUGLOG(("TxnWebOnUs2XYQ: noraml exit iret = [%d]\n",iRet));
	return iRet;
}
