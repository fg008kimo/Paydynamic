...
OBJPTR(DB);
OBJPTR(BO);
int     UpdatePreESQRespTxnLog(hash_t *hContext,
                	hash_t* hResponse);

int     AddESQTxnLog(hash_t *hContext,
                const hash_t* hRequest);

void TxnWebOnUs2ESQ(char    cdebug)
{
        cDebug = cdebug;
}
int     AddESQTxnLog(hash_t *hContext,
...

{
	...
	hash_t	*hTxn;
	hash_t	*hRec;
DEBUGLOG(("TxnWebOnUs2ESQ::UpdateRespTxnLog()\n"));
...
DEBUGLOG(("UpdateTxnLog RESP:: return_code = [%s]\n",csStatus))
		...
		if (!strcmp(csStatus, PD_ESQ_SUCCESS_STATUS))
...
DEBUGLOG(("UpdateTxnLog  iRet = [%d] after Insert TxnQrRequestLog\n",iRet));
		...
		int	iResult;
        	...
        	iRet = (unsigned long)(*DBObjPtr)(PD_CHANNEL_ESKY_QR,&iResult,csTmpStatus);
...
DEBUGLOG(("process_resp_txn()\n"));
	...
	PutField_CString(hContext,"psp_channel_code",PD_CHANNEL_ESKY_QR);
	PutField_CString(hContext,"return_psp_channel",PD_CHANNEL_ESKY_QR);
	...
	GetField_CString(hRequest,"psp_name",&csPspName);
      	MsgObjPtr = CreateObj(MsgPtr,"EsqMsg","BreakDownInitRspMsg");

        if ((*MsgObjPtr)(hResponse,inMsg->msg,inMsg->len) != PD_OK)  {
		iRet = INT_BREAKDOWN_ERR;
		PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("process_resp_txn:BreakDown EsqMsg Error\n"));
	}
//	else {
...
DEBUGLOG(("process_resp_txn: returned return_code = [%s]\n", csTmp));
			...
			if (!strcmp(csTmp, PD_ESQ_SUCCESS_STATUS)) {
...
DEBUGLOG(("process_resp_txn() call Update\n"));
		iRet = UpdatePreESQRespTxnLog(hContext,hResponse);
...
DEBUGLOG(("TxbWebOnUs2ESQ: time_redirect = [%d]\n",iTmp));
	        ...
	        if (!GetField_CString(hContext,"from_txn_seq",&csTxnSeq)) 
       		 	...
       		 	if ((*BOObjPtr)(hContext,hRequest,hRequest) == PD_OK) {
       		 		...
       		 		MsgObjPtr = CreateObj(MsgPtr,"EsqMsg","FormatInitMsg");
...
DEBUGLOG(("TxnWebOnUs2ESQ: after process_resp_err_txn iret = [%d]\n",iRet));
					}
					else {
						msg->mtext[iRecvLen] = '\0';
DEBUGLOG(("TxnWebOnUs2ESQ:  recv len = [%d]\n",iRecvLen));
DEBUGLOG(("TxnWebOnUs2ESQ:  recv = [%s]\n",&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN));
						memcpy(h_msg->msg,&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN);
						iRet = process_resp_txn(hContext,h_msg,hRequest);
DEBUGLOG(("TxnWebOnUs2ESQ: after process_resp_txn iret = [%d]\n",iRet));
					}
                		}
        			FREE_ME(msg);
        		}     
...
DEBUGLOG(("TxnWebOnUs2ESQ: BuildData Error\n"));
		}
	}
...
DEBUGLOG(("TxnWebOnUs2ESQ: noraml exit iret = [%d]\n",iRet));
	return iRet;
}
