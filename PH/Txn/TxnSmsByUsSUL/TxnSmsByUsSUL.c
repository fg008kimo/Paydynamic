...
-------------------------------                    ------------            --------------
Init Version                                       2013/09/11              [WCS]
Use txn engine to create txn                       2015/01/04              [WCS]
Fix INT_ERR core dump                              2021/05/14              [WCS]
*/
...
DEBUGLOG(("Authorize:: call DBOLStatement::AddSmsDetail() failed\n"));
	...
	if (iRet == PD_OK) {
		...
		// txn_code
		/* GetField_CString(hContext, "txn_code", &csBaidTxnCode); */
		GetField_CString(hText, "first_baid_txn_code", &csBaidTxnCode);
		PutField_CString(hRec, "txn_code", csBaidTxnCode);
...
/*
	...
	if (iRet == PD_OK) {
		...
		GetField_CString(hText, "next_engine_txn_code", &csTmp);

DEBUGLOG(("Authorize:: Call DBOLTxnEngineTxnCodeGrp::GetTxnGroup()\n"));
		DBObjPtr = CreateObj(DBPtr, "DBOLTxnEngineTxnCodeGrp", "GetTxnGroup");
		iTmpRet = (unsigned long)(*DBObjPtr)(csTmp, hRec);
		if (iTmpRet != PD_OK) {
DEBUGLOG(("Authorize:: call DBOLTxnEngineTxnCodeGrp::GetTxnGroup() failed\n"));
			PutField_CString(hText, "req_status", "FAILED");
			iRet = PD_ERR;
		} else {
			GetField_CString(hRec, "txn_grp", &csTmp);
DEBUGLOG(("Authorize:: txn_grp = [%s]\n", csTmp));
...
*/
	...
	// call txn engine
	if (iRet == PD_OK) {
		GetField_CString(hText, "next_engine_action", &csTmp);
		PutField_CString(hRec, "activity", csTmp);
		GetField_CString(hText, "next_engine_txn_code", &csTmp);
		PutField_CString(hRec, "bank_stmt_type", csTmp);
		GetField_CString(hText, "next_engine_recon_type", &csTmp);
		PutField_CString(hRec, "recon_type", csTmp);
		PutField_CString(hRec, "trigger_type", PD_TRIGGER_SYSTEM);
		PutField_CString(hRec, "input_channel", PD_CHANNEL_SMS);
		PutField_Int(hRec, "baid_txn_cnt", 1);
		PutField_CString(hRec, "baid_txn_id_0", csBaidTxnSeq);
