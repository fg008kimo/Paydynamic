...
/* bal_over_ind */
        if (iRet == PD_OK) {
                if (GetField_CString(hRequest, "bal_over_ind", &csTmp)) {
                        iBalOverInd = atoi(csTmp);
DEBUGLOG(("Authorize::bal_over_ind [%d]\n", iBalOverInd));
			PutField_Int(hFrBAIDDetails,"bal_over_ind",iBalOverInd);
                } else {
DEBUGLOG(("Authorize::bal_over_ind not found!!\n"));
ERRLOG("TxnOiaByUsCIA::Authorize::bal_over_ind not found!!\n");
