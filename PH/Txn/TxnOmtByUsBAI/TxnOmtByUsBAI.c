...
-------------------------------                    ------------            --------------
Init Version                                       2014/01/29              [SWK]
Handle auto-generated BAID                         2014/05/05              [SWK]
Handle BAID name & category changes		   2014/09/19		   [WWK]
Use Init Bal as the BAID open bal                  2015/01/12              [SWK]
when closing BAID, check any unknown stmt here	   2015/03/25		   [MSN] 
Use pre-assigned BAID if provided                  2015/05/26              [WCS]
Add sub provider & modify BAID name generation     2015/06/19              [WCS] 
Add txn creation for new baid initial balance      2015/11/12              [WCS]
Add new field apply_depost_cost into BAID creation 2018/06/25		   [ETN]
Enable auto upload bank stmt for POA and ITM BAID  2018/09/28		   [WCS]
Handle db_commit in OLTransaction:Add for AFU      2020/06/16		   [MIC]
*/
                        ...
                        hash_t* hResponse)
{
	...
	char	*csApplyDepositCost = NULL;
	int	iApplyDepositCost = PD_FALSE;
...
ERRLOG("TxnOmtByUsBAI::Authorize::psp id not found!!\n");
			...
			if (GetField_CString(hRequest, "sub_provider_id", &csSubProviderId)) {
DEBUGLOG(("Authorize:: sub_provider_id [%s]\n", csSubProviderId));
				PutField_CString(hTxn, "sub_provider_id", csSubProviderId);
			}

			if (GetField_CString(hRequest, "init_bal", &csTmp)) {
DEBUGLOG(("Authorize:: init_bal [%s]\n", csTmp));
				dTmp = string2doublewithsign((const unsigned char *) csTmp);
...
DEBUGLOG(("Authorize:: Bank Acct Currency [%s]\n", csBankCurrency));
						...
						if (GetField_CString(hOrgTxn, "owner_name", &csBankAcctOwnerName)) {
DEBUGLOG(("Authorize:: Bank Acct Owner Name [%s]\n", csBankAcctOwnerName));
						}
					}
				}
			}
...
DEBUGLOG(("Authorize::Call DBBankDesc:GetBankAbbrevName\n"));
					DBObjPtr = CreateObj(DBPtr, "DBBankDesc", "GetBankAbbrevName");
					if ((unsigned long)(*DBObjPtr)(csBankCode, hTmp) != PD_FOUND) {
DEBUGLOG(("Authorize::Call DBBankDesc:GetBankAbbrevName Failed\n"));
ERRLOG("TxnOmtByUsBAI::Authorize::Call DBBankDesc:GetBankAbbrevName Failed\n");
					} else {
						if (GetField_CString(hTmp, "bank_abbrev_name", &csBankAbbrevName)) {
DEBUGLOG(("Authorize:: Bank Abbrev Name [%s]\n", csBankAbbrevName));
						}
					}
				}
			}
...
DEBUGLOG(("Authorize:: PSP Ccy [%s]\n", csPIDCurrency)); 
			...
			/* Get Bank Acct Type Code for BAID Name */
			if (iRet == PD_OK) {
				if (csPIDAcctType != NULL) {
					hash_destroy(hTmp);
					hash_init(hTmp, 0);

DEBUGLOG(("Authorize::Call DBOLDefBankAcctType:GetBaidNameCode\n"));
					DBObjPtr = CreateObj(DBPtr, "DBOLDefBankAcctType", "GetBaidNameCode");
					if ((unsigned long)(*DBObjPtr)(csPIDAcctType, hTmp) != PD_FOUND) {
DEBUGLOG(("Authorize::Call DBOLDefBankAcctType:GetBaidNameCode Failed\n"));
ERRLOG("TxnOmtByUsBAI::Authorize::Call DBOLDefBankAcctType:GetBaidNameCode Failed\n");
					} else {
						if (GetField_CString(hTmp, "baid_name_code", &csBaidNameCode)) {
DEBUGLOG(("Authorize:: BAID Name Bank Acct Type Code [%s]\n", csBaidNameCode));
						}
					}
				}
			}
...
DEBUGLOG(("Authorize::Call DBPspMaster:GetPspMaster\n"));
				DBObjPtr = CreateObj(DBPtr, "DBPspMaster", "GetPspMaster");
				if ((unsigned long)(*DBObjPtr)(csClientId, hTmp) != PD_OK) {
DEBUGLOG(("Authorize::Call DBPspMaster:GetPspMaster Failed\n"));
ERRLOG("TxnOmtByUsBAI::Authorize::Call DBPspMaster:GetPspMaster Failed\n");
				} else {
					if (!GetField_CString(hTmp, "client_name", &csClientName)) {
DEBUGLOG(("Authorize::Provider Not Found\n"));
ERRLOG("TxnOmtByUsBAI::Authorize::Provider Not Found\n");
						iRet = INT_CLIENT_ID_NOT_FOUND;
						PutField_Int(hContext, "internal_error", iRet);
					} else {
DEBUGLOG(("Authorize:: Provider Name [%s]\n", csClientName));
					}
				}
			}
			...
			/* Get Sub Provider */
			if (iRet == PD_OK) {
				if (csSubProviderId != NULL) {
					hash_destroy(hTmp);
					hash_init(hTmp, 0);

DEBUGLOG(("Authorize::Call DBOLDefSubProvider:GetSubProviderName\n"));
					DBObjPtr = CreateObj(DBPtr, "DBOLDefSubProvider", "GetSubProviderName");
					if ((unsigned long)(*DBObjPtr)(csSubProviderId, hTmp) != PD_FOUND) {
DEBUGLOG(("Authorize::Call DBOLDefSubProvider:GetSubProviderName Failed\n"));
ERRLOG("TxnOmtByUsBAI::Authorize::Call DBOLDefSubProvider:GetSubProviderName Failed\n");
					} else {
						if (GetField_CString(hTmp, "sub_provider_name", &csSubProviderName)) {
DEBUGLOG(("Authorize:: Sub Provider Name [%s]\n", csSubProviderName));
						}
					}
				}
			}
...
*/
				if (GetField_CString(hBaidCategoryDtl,"cat_name",&csBankAbbrevName)) {
DEBUGLOG(("Authorize::DBOLDefBaidCategory::GetBaidCategoryDtl cat_name = [%s]\n",csBankAbbrevName));
				}
			}

			/* baid_name */
			if (csSubProviderName != NULL) {
				sprintf(csBaidName, "%s_%s", csClientName, csSubProviderName);
			} else {
				sprintf(csBaidName, "%s", csClientName);
			}
			if (csBankAbbrevName != NULL) {
				sprintf(csBaidName, "%s_%s_%s", csBaidName, csBaidNameCode, csBankAbbrevName);
			} else {
				sprintf(csBaidName, "%s_%s", csBaidName, csBaidNameCode);
			}
			if ((csBankAcctOwnerName != NULL) && !(strcmp(csCategory, PD_BAID_CATEGORY_DEFAULT))) {
				sprintf(csBaidName, "%s_%s_%06d", csBaidName, csBankAcctOwnerName, iCodeInNum);
			} else {
				sprintf(csBaidName, "%s_%06d", csBaidName, iCodeInNum);
			}
...
ERRLOG("TxnOmtByUsBAI::(Update) Authorize::Call DBOLBankAcctId:GetDtlByBankAcct Found OPEN BAID !!!!\n");
	...
	// PRD130
	// For deposit type bank accout user can choose to apply deposit cost or not upon creation
	if (iRet == PD_OK) {
/* apply_deposit_cost */                                                                                        
		if (csBankCode != NULL)
			if (!strcmp(csBankCode, PD_SYS_BANK_CODE))
				goto skip_apply_deposit_cost;

		if (cAction == PD_ACTION_ADD &&
		    !strcmp(csBankAcctType, PD_NATURE_DEPOSIT) &&
		    GetField_CString(hRequest, "apply_deposit_cost", &csApplyDepositCost)) {
DEBUGLOG(("Authorize()::apply_deposit_cost = [%s]\n",csApplyDepositCost));                                                
			iApplyDepositCost = atoi(csApplyDepositCost); 
		} 
		else if (cAction == PD_ACTION_UPDATE &&
			 GetField_CString(hRequest, "apply_deposit_cost", &csApplyDepositCost)) {
DEBUGLOG(("Authorize()::apply_deposit_cost = [%s]\n",csApplyDepositCost));
...
skip_apply_deposit_cost:
	PutField_Int(hTxn, "apply_deposit_cost", iApplyDepositCost);
...
DEBUGLOG(("Authorize::DBOLBankAcctId:Update Succ\n"));
	...
	// Enable auto upload bank stmt for POA and ITM BAID
...
DEBUGLOG(("acct_type = [%s]\n", csBankAcctType));
			if (strcmp(csBankCode, PD_SYS_BANK_CODE)) {
				if (!strcmp(csBankAcctType, PD_NATURE_DEPOSIT) || !strcmp(csBankAcctType, PD_NATURE_PAYOUT) || !strcmp(csBankAcctType, PD_NATURE_INTERMEDIATE)) {
					hash_destroy(hTmp);
					hash_init(hTmp, 0);
					PutField_CString(hTmp, "int_bank_code", csBankCode);
					PutField_CString(hTmp, "bank_acct_num", csAcctNum);
					PutField_CString(hTmp, "acct_type", csBankAcctType);
					if (!strcmp(csBankAcctType, PD_NATURE_DEPOSIT)) {
						PutField_Int(hTmp, "disabled", PD_TRUE);
					} else {
						PutField_Int(hTmp, "disabled", PD_FALSE);
					}
					GetField_CString(hRequest, "add_user", &csTmp);
					PutField_CString(hTmp, "update_user", csTmp);
DEBUGLOG(("Authorize:: Call DBOLAutoUploadBankDetail: UpdateDisabled\n"));
...
ERRLOG("TxnOmtByUsBAI:: Authorize:: Call DBOLAutoUploadBankDetail: UpdateDisabled Failed\n");
					}
				}
			}
		}
...
DEBUGLOG(("Authorize:: Handle Init Bal [%s] [%s] [%s]\n", csBAID, csBaidCountry, csPIDCurrency));
				if (dInitAmt < 0.0) {
					...
					dInitAmt = dInitAmt * (-1.0);
				} else if (dInitAmt >= 0.0) {
					cAmtType = PD_IND_CREDIT;
					PutField_Char(hBalTxn, "amt_type", cAmtType);
				}
	
				PutField_Double(hBalTxn, "txn_amt", dInitAmt);
...
DEBUGLOG(("Authorize::Call DBOLTxnSeq:: New Txn Seq [%s]\n", csBalTxnSeq));
					...
					//if (GetField_Double(hTxn, "init_bal", &dInitAmt)) {
						PutField_Double(hBalTxn, "txn_amt", dInitAmt);
						PutField_Double(hBalTxn, "display_amt", dInitAmt);
						PutField_Double(hBalTxn, "txn_amount", dInitAmt);
						PutField_Double(hBalTxn, "net_amt", dInitAmt);
						//if (cAmtType == PD_IND_CREDIT) {
							PutField_Double(hBalTxn, "amount", dInitAmt);
						//} else {
						//	PutField_Double(hBalTxn, "amount", dInitAmt * (-1.0));
						//}
					//}
					...
					// for adding txn header
					if (GetField_CString(hContext, "channel_code", &csTmp)) {
						PutField_CString(hBalTxn, "channel_code", csTmp);
					}
					PutField_Char(hBalTxn, "status", PD_TO_PSP);
					if (cAmtType == PD_IND_CREDIT) {
						PutField_CString(hBalTxn, "txn_code", PD_NEW_BAID_INIT_BAL_POS);
					} else {
						PutField_CString(hBalTxn, "txn_code", PD_NEW_BAID_INIT_BAL_NEG);
					}
					PutField_CString(hBalTxn, "process_type", PD_PROCESS_TYPE_DEF);
					PutField_CString(hBalTxn, "process_code", PD_PROCESS_CODE_DEF);
					if (GetField_CString(hContext, "PHDATE", &csTmp)) {
						PutField_CString(hBalTxn, "host_posting_date", csTmp);
					}
					PutField_Int(hBalTxn, "internal_code", 0);
					PutField_CString(hBalTxn, "response_code", "0");
					if (GetField_CString(hContext, "local_tm_date", &csTmp)) {
						PutField_CString(hBalTxn, "local_tm_date", csTmp);
						strcpy(csTxnDateTime, csTmp);
						PutField_CString(hBalTxn, "transmission_datetime", csTxnDateTime);
						PutField_CString(hBalTxn, "tm_date", csTmp);
					}
					if (GetField_CString(hContext, "local_tm_time", &csTmp)) {
						PutField_CString(hBalTxn, "local_tm_time", csTmp);
						strcat(csTxnDateTime, csTmp);
						PutField_CString(hBalTxn, "transmission_datetime", csTxnDateTime);
						PutField_CString(hBalTxn, "tm_time", csTmp);
					}
					PutField_Char(hBalTxn, "recon_status", PD_RECON_NOT_REQ);

					// for adding txn psp detail
					tNow = time(0);
					tStruct = localtime(&tNow);
					strftime(csBuf, sizeof(csBuf), "%Y%m%d", tStruct);
					PutField_CString(hBalTxn, "txn_date", csBuf);
					strftime(csBuf, sizeof(csBuf), "%H%M%S", tStruct);
					PutField_CString(hBalTxn, "txn_time", csBuf);
					if (csBankCode != NULL) {
						PutField_CString(hBalTxn, "bank_code", csBankCode);
					}
					if (csAcctNum != NULL) {
						PutField_CString(hBalTxn, "bank_acct_num", csAcctNum);
					}
					PutField_Int(hBalTxn, "txn_hold_ind", PD_FALSE);
					PutField_Int(hBalTxn, "sys_match_ind", PD_TRUE);

					// update txn header
					PutField_CString(hBalTxn, "net_ccy", csPIDCurrency);
					PutField_Char(hBalTxn, "ex_supplier", PD_INT_EX);
					PutField_Double(hBalTxn, "ex_rate", 1.0);
					DBObjPtr = CreateObj(DBPtr, "DBSystemControl", "GetApprovalDT");
					(*DBObjPtr)(hBalTxn);

					// for adding txn elements
					PutField_CString(hBalTxn, "txn_element_type", PD_ELEMENT_TXN_AMT);
					PutField_CString(hBalTxn, "ccy", csPIDCurrency);
					if (cAmtType == PD_IND_CREDIT) {
						PutField_CString(hBalTxn, "amount_type", PD_CR);
					} else {
						PutField_CString(hBalTxn, "amount_type", PD_DR);
					}
					PutField_Char(hBalTxn, "party_type", PD_TYPE_PSP);
					
					if (GetField_Int(hContext,"db_commit",&iCommit)) {
						PutField_Int(hBalTxn,"db_commit", iCommit);
					}
...
DEBUGLOG(("Authorize::Call DBOLTransaction:: Add Failed!!\n"));
						...
						PutField_Char(hBalTxn, "status", PD_COMPLETE);
						PutField_Char(hBalTxn, "ar_ind", PD_ACCEPT);
						PutField_CString(hBalTxn, "sub_status", PD_APPROVED);
...
DEBUGLOG(("Authorize::Call DBOLTxnElements:: Add\n"));
								...
								iRet = (unsigned long)(*DBObjPtr)(hBalTxn);
DEBUGLOG(("Authorize::Call DBOLTxnElements:: Return [%d]\n", iRet));
