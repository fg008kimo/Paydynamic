...
Add payout_provider_fail tag			   2015/01/22		   Virginia Yun
Add return void merchant payout txn_id             2015/08/12              Virginia Yun
Modify for void merchant settlement		   2016/05/12		   Dirk Wong
Add return_to_mfee & call DBDefVoidTxnConfig       2016/06/23              David Wong
Add return void bal-transfer: inter payout	   2016/09/22		   Dirk Wong
Add Txn Code: Void Payout API			   2017/04/07		   Elvis Wong
*/
...
ERRLOG("TxnOmtByUsVOT:Authorize::txnid not found!!\n");
...
ERRLOG("TxnOmtByUsVOT::Authorize() txn_amt Invalid\n");
			}
		}
	}
*/
...
ERRLOG("TxnOmtByUsVOT::Authorize: MMS Mode is ON, Txn unvoidable!!!!!!!!\n");
						iRet=INT_ERR;
					}
				}
			}
...
DEBUGLOG(("Authorize:: org_posting_date = [%s]\n", csOrgPostDate));
...
DEBUGLOG(("Authorize:: same_date_cancel = [%d]\n", PD_TRUE));
...
DEBUGLOG(("Authorize:: skip BOOLTxnElements:VoidOrgTxnElements\n"));
					}
				}
...
/*Void Payout*/
				if(!strcmp(csTxnCode,PD_OL_PAYOUT_APPROVE)){
					...
					/*
					if (GetField_Int(hRequest, "po_provider_fail", &iTmp)) {
DEBUGLOG(("Authorize:: po_provider_fail [%d]\n", iTmp));
						PutField_Int(hContext, "po_provider_fail", iTmp);
					} else {
						PutField_Int(hContext, "po_provider_fail", PD_TRUE);
					}
					*/
...
*/				}
DEBUGLOG(("TxnOmtByUsVOT Create Txn Object [%s]\n",csHandler));
	...
	FREE_ME(rRecordSet);
	//FREE_ME(hOptContext);
DEBUGLOG(("TxnOmtByUsVOT Normal Exit() iRet = [%d]\n",iRet));
	return iRet;
}
...
ERRLOG("TxnOmtByUsVOT:GetTxnHeader::Invalid status!!\n");
				}
				else{
...
ERRLOG("TxnOmtByUsVOT:Authorize::GetTxnHeader::Invalid ar_ind!!\n");
						}
					}
				}
			}
...
//ERRLOG("TxnOmtByUsVOT:Authorize::GetTxnHeader::not found record!!\n");
		iRet=INT_NOT_RECORD;
		//PutField_Int(hContext,"internal_error",iRet);
	}
...
ERRLOG("TxnOmtByUsVOT:Authorize::un-voidable txn!!\n");
		}
	}
