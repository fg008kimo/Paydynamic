...
void TxnOmtByUsATF(char    cdebug)
{
        cDebug = cdebug;
}
...
ERRLOG("TxnOmtByUsATF:Authorize() txn seq is missing\n");
        }
...
ERRLOG("TxnOmtByUsATF:Authorize() txn seq to is missing\n");
	}
...
DEBUGLOG(("Authorize::call DBOLTransaction->MatchRespTxn\n"));
                ...
                int iChk = (unsigned long)(*DBObjPtr)(csTxnSeq,PD_PROCESSING);
...
DEBUGLOG(("Authorize::call DBOLTransaction->MatchRespTxn\n"));
                DBObjPtr = CreateObj(DBPtr,"DBOLTransaction","MatchRespTxn");
                int iChk = (unsigned long)(*DBObjPtr)(csTxnSeqTo,PD_PROCESSING);
                if(iChk==PD_NOT_FOUND){
                        iRet = INT_INVALID_TXN;
                        PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("Authorize:: The Transaction is not for approval\n"));
...
DEBUGLOG(("Authorize::(FROM) GetTxnInfo\n"));
		...
		if (iRet != PD_OK) {
			PutField_Int(hContext,"internal_error",iRet);
			if(iRet== INT_INVALID_TXN || iRet == INT_ALREADY_APPROVED){
DEBUGLOG(("Authorize::GetTxnInfo (From party) iRet = [%d]\n"));
			}
...
DEBUGLOG(("Authorize::GetTxnInfo (From party) service_code [%s]\n", csFromService));
			}
		}
	}
	// Check Merchant Account - From
	if (iRet == PD_OK) {
DEBUGLOG(("Authorize() (FROM) BOOLMerchant->GetMerchantTxnInfo\n"));
		BOObjPtr = CreateObj(BOPtr,"BOOLMerchant","GetMerchantTxnInfo");
		iRet = (unsigned long)(*BOObjPtr)(hFrContext,hFrReq);
DEBUGLOG(("Authorize() (FROM) BOOLMerchant->GetMerchantTxnInfo = [%d]\n",iRet));

		if (iRet != PD_OK) {
			PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("Authorize::() (FROM) Merchant GetMerchantTxnInfo FAILED\n"));
ERRLOG("TxnOmtByUsATF:Authorize() (FROM) Merchant GetMerchantTxnInfo FAILED\n");
...
DEBUGLOG(("Authorize::GetTxnInfo (TO) service_code [%s]\n", csToService));
			}
		}
	}
...
DEBUGLOG(("Authorize() (TO) BOOLMerchant->GetMerchantTxnInfo\n"));
		BOObjPtr = CreateObj(BOPtr,"BOOLMerchant","GetMerchantTxnInfo");
		iRet = (unsigned long)(*BOObjPtr)(hToContext,hToReq);
DEBUGLOG(("Authorize() (TO) BOOLMerchant->GetMerchantTxnInfo = [%d]\n",iRet));

		if (iRet != PD_OK) {
			PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("Authorize::() (TO) Merchant GetMerchantTxnInfo FAILED\n"));
ERRLOG("TxnOmtByUsATF:Authorize() (TO) Merchant GetMerchantTxnInfo FAILED\n");
...
ERRLOG("TxnOmtByUsATF:Authorize() the inpuuted seq is not in a pair\n");
			}
		}
		else {
			iRet = INT_TXN_ID_NOT_FOUND;
...
ERRLOG("TxnOmtByUsATF:Authorize() org_txn_idis missing on TO information\n");
		}
	}
...
DEBUGLOG(("Authorize() (FROM) GetTxnFee\n",iRet));
		...
		PutField_CString(hFrContext, "txn_code", PD_OFL_AVA_PAYOUT_REQ_TF_FROM);
...
DEBUGLOG(("Authorize net amount (from Fee) = [%lf]\n",dSrcNetAmt));
	...
	// Get To TxnFee 
	if (iRet == PD_OK) {
		PutField_CString(hToContext, "txn_code", PD_OFL_AVA_PAYOUT_REQ_TF_TO);
		//PutField_CString(hToContext, "txn_code", PD_OFL_AVA_PAYOUT_REQ_TF_FROM);
...
DEBUGLOG(("Authorize() BOExchange->GetExchangeInfo ex_rate[%f]\n", dExRate));
			...
			if (GetField_Double(hToContext, "inter_rate", &dInterRate)) {
DEBUGLOG(("Authorize() BOExchange->GetExchangeInfo inter_rate[%f]\n", dInterRate));
			}
...
DEBUGLOG(("Authorize() BOExchange->GetExchangeInfo markup amt [%lf] ccy [%s]!\n", dMarkupAmt, csDstNetCcy));
		...
		*/
		...
		if (iRet == PD_OK) {
			...
			if (GetField_CString(hFrContext, "merchant_id", &csTmp)) {
				PutField_CString(hToReq, "merchant_id", csTmp);
			}

			if (GetField_CString(hToContext, "merchant_client_id", &csClientID)) {
DEBUGLOG(("Authorize() (TO) Org Clinet ID [%s]\n", csClientID)); 

				if (GetField_CString(hFrContext, "merchant_client_id", &csTmp)) {
					PutField_CString(hToContext, "merchant_client_id", csTmp);
				}
			}

			PutField_CString(hToReq, "service_code", csFromService);

DEBUGLOG(("Authorize() (TO) GetTxnFee\n")); 
			BOObjPtr = CreateObj(BOPtr,"BOFee","GetTxnFee");
			iRet = (unsigned long)(*BOObjPtr)(hToContext,hToReq);
DEBUGLOG(("Authorize() (TO) GetTxnFee [%d]\n",iRet));

			if (GetField_CString(hToContext, "merchant_id", &csTmp)) {
				PutField_CString(hToReq, "merchant_id", csTmp);
			}

			PutField_CString(hToContext, "merchant_client_id", csClientID);
			PutField_CString(hToReq, "service_code", csToService);
...
ERRLOG("TxnOmtByUsATF::Authorize From GetOpenBalanceForUpdate FAILED\n");
		}
	}
...
ERRLOG("TxnOmtByUsATF::Authorize (FROM) GetRemainPOBalance FAILED\n");
		}
		...
		if (iRet == PD_OK) {
			if (dSrcNetAmt > dFromAvaPOBal) {
				...
				PutField_Int(hContext,"internal_error",iRet);
				PutField_CString(hFrContext,"remark","Insufficient Availiable For Payout Amount");

DEBUGLOG(("Authorize insufficient fund [%s] [%s] [%s] [%lf] [%lf]\n", csFromMid, csFromCcy,csFromService, dFromAvaPOBal, dSrcNetAmt));
ERRLOG("TxnOmtByUsATF::Authorize insufficient fund [%s] [%s] [%s] [%lf] [%lf]\n",csFromMid,csFromCcy,csFromService,dFromAvaPOBal, dSrcNetAmt);
			}
		}
	}

	if (iRet == PD_OK) {	
		if (GetField_Double(hFrContext, "merchant_net_float", &dTmp)) {
			if (dTmp < 0.0) {
DEBUGLOG(("Authorize merchant_net_float problem [%lf]\n", dTmp));
				iRet = INT_ERR;
			}
		}
	}
...
DEBUGLOG(("Authorize() (FROM) BOOLBalance.ProcessTransferAvaPO\n"));
		BOObjPtr = CreateObj(BOPtr,"BOOLBalance","ProcessTransferAvaPayout");
		iRet = (unsigned long)(*BOObjPtr)(hFrContext);

		if (iRet != PD_OK) {
DEBUGLOG(("Authorize() (FROM) BOOLBalance.ProcessTransferAvaPO FAILED\n"));
ERRLOG("TxnOmtByUsATF: Authorize() (FROM) BOOLBalance.ProcessTransferAvaPO FAILED\n");
		}
...
DEBUGLOG(("Authorize() (FROM) OMTChannel.UpdateTxnDetailLog\n"));
	...
	if (iRet == INT_INSUFFICIENT_FUND) {
		// Update Detail Remark 
DEBUGLOG(("Authorize() (FROM) OMTChannel.UpdateTxnDetailLog\n"));
		ChannelObjPtr = CreateObj(ChannelPtr,"OMTChannel","UpdateTxnDetailLog");
		if((unsigned long)(*ChannelObjPtr)(hFrContext,hFrReq,hResponse)!=PD_OK){
DEBUGLOG(("Authorize() (FROM) OMTChannel.UpdateTxnDetailLog update remark failed\n"));
		}
	}
...
ERRLOG("TxnOmtByUsATF::Authorize To GetOpenBalanceForUpdate FAILED\n");
		}
	}
...
DEBUGLOG(("Authorize (TO) !! Add [%lf] [%lf] [%lf] total [%lf] rate [%lf]\n", dOrgAddFundin, dOrgAddResvPO, dOrgAddPrevFt, (dOrgAddFundin + dOrgAddResvPO + dOrgAddPrevFt), dExRate));

		dRemainTxnFee = dTotalDstFee + dMarkupAmt;

DEBUGLOG(("Authorize (TO) !! dRemainTxnFee [%lf]\n", dRemainTxnFee));
...
DEBUGLOG(("Authorize() (TO) OMTChannel.UpdateTxnDetailLog\n"));
		ChannelObjPtr = CreateObj(ChannelPtr,"OMTChannel","UpdateTxnDetailLog");
...
DEBUGLOG(("Authorize() (TO) OMTChannel.UpdateTxnLog\n"));
...
DEBUGLOG(("Authorize() (TO) OMTChannel.UpdateTxnLog (as FROM error)\n"));
			ChannelObjPtr = CreateObj(ChannelPtr,"OMTChannel","UpdateTxnLog");
			iTmpRet = (unsigned long)(*ChannelObjPtr)(hToContext,hToReq,hResponse);	
			if (iTmpRet != PD_OK) {
				iRet = PD_ERR;
...
DEBUGLOG(("TxnOmtByUsATF Normal Exit() iRet = [%d]\n",iRet));
	return iRet;
}
...
ERRLOG("TxnOmtByUsATF:GetTxnHeader::Invalid status!!\n");
					} 
					else {
						if (GetField_Char(hRec, "ar_ind", &cTmp)) {
							iRet = INT_INVALID_TXN;
DEBUGLOG(("GetTxnHeader::Invalid ar_ind [%c]\n",cTmp));
ERRLOG("TxnOmtByUsATF:GetTxnHeader::Invalid ar_ind!!\n");
						}
					}
				}		
...
ERRLOG("TxnOmtByUsATF:Authorize::GetTxnHeader::not found record!!\n");
			iRet=INT_NOT_RECORD;
		}
...
ERRLOG("TxnOmtByUsATF::GetTxnInfo::GetTxnDetail::not found record!!\n");
			iRet=INT_NOT_RECORD;
		}
                RecordSet_Destroy(rRecordSet);
	}
