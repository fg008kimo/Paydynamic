...
-------------------------------                    ------------            --------------
Init Version                                       2016/07/27              Dirk Wong
PRD023 - Add key type (DSI/POA)			   2017/03/13		   Dirk Wong
*/
...
void TxnOmtByUsTMK(char cdebug)
{
	cDebug = cdebug;
}
		...
		hash_t* hResponse)
{
	...
	//char	*csMD5KeyName = PD_PTK_KEY_NAME;
	//char	*csSHA1KeyName = PD_SHA1_KEY_NAME;
	char	csMD5KeyName[PD_KEY_NAME_LEN+1];
	char	csSHA1KeyName[PD_KEY_NAME_LEN+1];
...
ERRLOG("TxnOmtByUsTMK:: Authorize:: merchant_id not found!!\n");
		}

//API: type
		//Set default as DSI Key
		memset(csMD5KeyName,0,sizeof(csMD5KeyName));
		memset(csSHA1KeyName,0,sizeof(csSHA1KeyName));
		strcpy(csMD5KeyName,PD_PTK_KEY_NAME);
		strcpy(csSHA1KeyName,PD_SHA1_KEY_NAME);

		if (GetField_CString(hRequest, "type", &csType)) {
			if (!strcmp(csType,PD_NATURE_DEPOSIT)) {
DEBUGLOG(("Authorize() type = [%s]\n", csType));
			} else if (!strcmp(csType,PD_NATURE_PAYOUT)) {
				strcpy(csMD5KeyName,(char*)PD_MOK_KEY_NAME);
				strcpy(csSHA1KeyName,PD_POK_KEY_NAME);
DEBUGLOG(("Authorize() type = [%s]\n", csType));
			} else {
				iRet = INT_KEY_TYPE_NOT_FOUND;
				PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("Authorize:: invalid type!!\n"));    
ERRLOG("TxnOmtByUsTMK:: Authorize:: invalid type!!\n");
			}
		} else {
			//Default Type = "DSI"
			PutField_CString(hContext,"type",PD_NATURE_DEPOSIT);
DEBUGLOG(("Authorize:: type = [%s] (API not found, use DEFAULT)!!\n",PD_NATURE_DEPOSIT));
		}
...
ERRLOG("TxnOmtByUsTMK::Authorize() user NOT FOUND!!!\n");
		}
...
//Call DBMerchKeys:CheckAvailableForGenSTK
			...
			if (iRet != PD_OK) {
				...
				PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("Authorize() merchant not available for update Despoit key!!!\n"));
ERRLOG("TxnOmtByUsTMK::Authorize() merchant not available for update Deposit key!!!\n");
			}
		} else if (!strcmp(csType,PD_NATURE_PAYOUT)) {

//Call DBMerchKeys:CheckAvailableForGenPOAKey
			DBObjPtr = CreateObj(DBPtr, "DBMerchKeys", "CheckAvailableForGenPOAKey");
			iRet = (unsigned long)(*DBObjPtr)(csMerchantId);

			if (iRet != PD_OK) {
				iRet = INT_ITT_NOT_NEW_MERCHANT;
				PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("Authorize() merchant not available for update Payout key!!!\n"));
ERRLOG("TxnOmtByUsTMK::Authorize() merchant not available for update Payout key!!!\n");
			}
		}
	}
...
/*********************************************************
 * 1) Update PTK Key (Deposit) / Create MOK Key (Payout) *
 * *******************************************************/

	if (iRet == PD_OK && !strcmp(csType,PD_NATURE_DEPOSIT)) {

DEBUGLOG(("Authorize() - Update PTK\n"))
...
ERRLOG("TxnOmtByUsTMK: Call DBMerchKeys:GetMerchantKey() key_value NOT FOUND!!!\n");
	                        }
...
ERRLOG("TxnOmtByUsTMK: Call DBMerchKeys:GetMerchantKey() effect_date NOT FOUND!!!\n");
	                        }
...
ERRLOG("TxnOmtByUsTMK: Call DBMerchKeys:GetMerchantKey() FAILED!!!\n");
		}
	...
	if (iRet == PD_OK) {	
		...
		if (!strcmp(csType,PD_NATURE_PAYOUT)) {
DEBUGLOG(("Authorize() - Create MOK\n"))
		}
...
//key
		...
		PutField_CString(hMerchKeys,"key",csMD5KeyName);
//DEBUGLOG(("Authorize() - Update:: key = [%s]\n",PD_PTK_KEY_NAME));
DEBUGLOG(("Authorize():: key = [%s]\n",csMD5KeyName));
...
DEBUGLOG(("Authorize():: effect_date = [%s]\n",(char*)csEffectDate));
		...
		sprintf(csTag,"%s||%s",csMerchantId,(char*)csMD5KeyName);
		if (!memcmp(csType,PD_NATURE_DEPOSIT,3)) {
...
ERRLOG("TxnOmtByUsTMK Call DBMerchKeys:UpdateKey FAILED!!!\n");
			}

		} else if (!memcmp(csType,PD_NATURE_PAYOUT,3)) {
			AddTaskLog(hITTLog,iLogBatchSeq,PD_ITT_ACTION_INSERT_MOK_KEY,PD_ITT_LOG_INSERT,"MERCH_KEYS","","merchant_id",csMerchantId,"");
			AddTaskLog(hITTLog,iLogBatchSeq,PD_ITT_ACTION_INSERT_MOK_KEY,PD_ITT_LOG_INSERT,"MERCH_KEYS","","key",PD_MOK_KEY_NAME,"");
			AddTaskLog(hITTLog,iLogBatchSeq,PD_ITT_ACTION_INSERT_MOK_KEY,PD_ITT_LOG_INSERT,"MERCH_KEYS","","key_value",csTmpKey,"");
			AddTaskLog(hITTLog,iLogBatchSeq,PD_ITT_ACTION_INSERT_MOK_KEY,PD_ITT_LOG_INSERT,"MERCH_KEYS","","effect_date",csEffectDate,"");
...
ERRLOG("TxnOmtByUsTMK Call DBMerchKeys:Add FAILED!!!\n");
			}
		}
...
//End Update PTK/MOK Key



/**************************************************
 * 2) create STK Key (Deposit) / POK Key (Payout) *
 * ************************************************/
...
DEBUGLOG(("Authorize() - Create POK Key\n"));
		}
...
//key
		PutField_CString(hMerchKeys,"key",csSHA1KeyName);
...
DEBUGLOG(("Authorize() - Insert:: effect_date = [%s]\n",(char*)csEffectDate));
		...
		if (!strcmp(csType,PD_NATURE_DEPOSIT)) {
			...
			AddTaskLog(hITTLog,iLogBatchSeq,PD_ITT_ACTION_INSERT_POK_KEY,PD_ITT_LOG_INSERT,"MERCH_KEYS","","merchant_id",csMerchantId,"");
			AddTaskLog(hITTLog,iLogBatchSeq,PD_ITT_ACTION_INSERT_POK_KEY,PD_ITT_LOG_INSERT,"MERCH_KEYS","","key_value",csTmpKey,"");
			AddTaskLog(hITTLog,iLogBatchSeq,PD_ITT_ACTION_INSERT_POK_KEY,PD_ITT_LOG_INSERT,"MERCH_KEYS","","key",PD_POK_KEY_NAME,"");
			AddTaskLog(hITTLog,iLogBatchSeq,PD_ITT_ACTION_INSERT_POK_KEY,PD_ITT_LOG_INSERT,"MERCH_KEYS","","effect_date",csEffectDate,"");
		}
