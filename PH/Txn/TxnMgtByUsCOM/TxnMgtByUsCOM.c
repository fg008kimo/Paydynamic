...
-------------------------------                    ------------            --------------
Init Version  					   2011/06/23		   [GOD]
txn_amt not 0 or 0.0				   2012/12/21		   [MSN]
...
/* paid_amt */
	if(GetField_CString(hRequest,"paidamt",&csPtr) && iRet == PD_OK){
                iCheck = PD_FALSE;
                p = (char*)strchr(csPtr, '.');
                if (p == NULL){
                        iCheck = is_numeric(csPtr);
...
DEBUGLOG(("Authorize() paid_amt = [%f]\n",dTmp));
                                }
				/*
                                if(!(dTmp > 0.0)){
                                        iRet =  INT_INVALID_PAY_AMOUNT;
                                        PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("Authorize()paid_amt Invalid[%s]\n",csPtr));
ERRLOG("TxnMgtByUsCOM::Authorize() paid_amt Invalid\n");
                                }*/
                        }
                }
                else{
...
DEBUGLOG(("TxnMgtOnUsCOM::Authorize() [%s] doesn't support decimal\n",csTxnCcy));
                                lTmp = (long) dTmp;
                                if (dTmp > lTmp) {
                                        iRet = INT_UNSUPPORTED_PAID_AMOUNT;
                                        PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("TxnMgtOnUsCOM::Authorize() unsupported paid amount [%f]\n",dTmp));
ERRLOG("TxnMgtOnUsCOM::Authorize() unsupported paid amount [%f]\n",dTmp);
                                }
                                else if(lTmp==0l){
                                        iRet =  INT_INVALID_PAID_AMOUNT;
                                        PutField_Int(hContext,"internal_error",iRet);
