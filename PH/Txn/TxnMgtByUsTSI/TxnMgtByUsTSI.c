...
-------------------------------                    ------------            --------------
Init Version                                       2011/08/04              [GOD]
Filter by selected_pid if any			   2013/06/05		   [GOD]
Add psp_key_id into response			   2015/03/16		   [GOD]
Support CC/DC card type option			   2016/09/28		   [MSN]
Support WeChatPay when ResendToPsp		   2016/12/09		   [MSN]
Add Txn Expire Checking				   2017/07/03		   [MSN]
Support device type option			   2017/10/12		   [WMC]				
Support New Phase of Vmobile Customer Segment      2017/11/23              [WMC]
Support PSP channel CPE resend to PSP		   2018/01/10		   [WCS]
Add PayLink-WellPay				   2018/08/13		   [WCS]
Add EASYPAY-ALIPAY VNET				   2020/09/21		   [WCS]
Add ip_region_code				   2021/01/18		   [WMC]
Handle multiple IPs				   2021/06/04		   [WCS]
Add customer_tag				   2021/07/12		   [WCS]
*/
...
DEBUGLOG(("Authorize() Normal flow...\n"));
		...
		if(iRet == PD_OK){
			...
			PutField_CString(hCon,"ip_region",csIpRegionCode);
...
DEBUGLOG(("ResendToPsp::GetTxnDetail -customer_tag = [%s]\n",csPtr));
