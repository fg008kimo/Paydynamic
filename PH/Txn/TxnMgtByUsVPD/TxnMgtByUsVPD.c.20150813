...
void TxnMgtByUsVPD(char    cdebug)
{
        cDebug = cdebug;
}
...
DEBUGLOG(("TxnMgtByUsVPD: Authroize()\n"));
	if (GetField_CString(hRequest,"org_txn_seq",&csOrgTxnSeq)) {
DEBUGLOG(("TxnMgtByUsVPD: org txn seq = [%s]\n",csOrgTxnSeq));
...
DEBUGLOG(("TxnMgtByUsVPD: update_user = [%s]\n",csUpdateUser));
	}
...
ERRLOG("TxnMgtByUsVPD:Authorize::GetTxnHeader::not found record!!\n");
                        iRet=INT_NOT_RECORD;
                        PutField_Int(hContext,"internal_error",iRet);
                }
	}

	if(!strcmp(csOrgTxnCode,PD_PSP_DELIVER_FROM)){
		if(iRet==PD_OK){
...
DEBUGLOG(("Authorize::Call DBTransaction:Update PST [%s]\n",csTmp));
...
DEBUGLOG(("Authorize::DBTransaction:Update iRet = [%d]\n",iRet));
	...
	else if(!strcmp(csOrgTxnCode,PD_PSP_DELIVER_TO)){
...
DEBUGLOG(("Authorize::Call DBSebBalance:Add failed\n"));
		...
		if(iRet==PD_OK){
			...
			PutField_CString(hTxn,"sub_status",PD_APPROVED);
DEBUGLOG(("Authorize::Call DBTransaction:Update PSF [%s]\n",csOrgPSFSeq));
			DBObjPtr = CreateObj(DBPtr,"DBTransaction","Update");
			iRet = (unsigned long)(*DBObjPtr)(hTxn);
DEBUGLOG(("Authorize::DBTransaction:Update iRet = [%d]\n",iRet));
		}
		if(iRet==PD_OK){
			...
			DBObjPtr = CreateObj(DBPtr,"DBTxnPspDetail","GetTxnPspDetailByDlvId");
			if((unsigned long)(*DBObjPtr)(csOrgPSFSeq,rRecordSet)==PD_OK){
				hRec = RecordSet_GetFirst(rRecordSet);
				while(hRec && iRet==PD_OK){
					if (GetField_CString(hRec,"txn_id",&csTmp)){
						PutField_CString(hTxn,"txn_seq",csTmp);
