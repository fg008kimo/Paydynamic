...
Test for Mini-MMMM Set Balance Pool		   2015/11/02		   Elvis Wong
*/
...
// Update Balance
	if (iRet == PD_OK) {
		...
		if (!GetField_CString(hFrContext, "pool", &csTmp)) {
...
// Update Balance
	if (iRet == PD_OK) {
		...
		if (!GetField_CString(hToContext, "pool", &csTmp)) {
...
DEBUGLOG(("Authorize::baid_category (from) = [%s]\n",csFrBaidCategory));
				...
				// Set Balance Pool
				if (!strcmp(csFrBaidCategory, PD_BAID_CAT_BIT)) {
					PutField_CString(hFrContext, "pool", PD_INTRANSIT_POOL);				
					PutField_CString(hToContext, "pool", PD_INTRANSIT_POOL);				
				}
  			}
