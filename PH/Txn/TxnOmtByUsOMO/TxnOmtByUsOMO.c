...
-------------------------------                    ------------            --------------
Init Version                                       2014/01/28              Virginia Yun
*/
...
void TxnOmtByUsOMO(char    cdebug)
{
        cDebug = cdebug;
}
                        ...
                        hash_t* hResponse)
{
	...
	int	    iTmp;
	double      dTmp =0.0;
	int	iTransferByAvaiSett = PD_FALSE;
...
ERRLOG("TxnOmtByUsOMO::Authorize::txnid not found!!\n");
...
ERRLOG("TxnOmtByUsOMO::Authorize::txn_code not found!!\n");
                iRet=INT_ERR;
...
ERRLOG("TxnOmtByUsOMO::Authorize::merchant_id not found!!\n");
...
ERRLOG("TxnOmtByUsOMO::Authorize::serivce_code not found!!\n");
...
ERRLOG("TxnOmtByUsOMO::Authorize::country not found!!\n");
...
ERRLOG("TxnOmtByUsOMO::Authorize::ccy not found!!\n");
...
ERRLOG("TxnOmtByUsOMO::Authorize::txn_amt not found!!\n");
...
DEBUGLOG(("Authorize::Call BOFee:GetTxnFee\n"));
		...
		if (GetField_CString(hContext, "txn_code", &csCode)) {
DEBUGLOG(("Authorize::BOFee: txn_code [%s]\n", csCode));
		}
...
ERRLOG("TxnOmtByUsOMO::BOFee::GetTxnFee Failed Ret [%d]\n", iRet);
			iRet = INT_ERR;
                } 	
		else {
			if (GetField_Double(hContext, "src_txn_fee", &dTmp)) {
				PutField_Double(hContext, "net_amt", dAmt+dTmp);
...
DEBUGLOG(("Authorize::BOFee:GetTxnFee src_txn_fee_ccy [%s]\n", csTmp));
	...
	if ((unsigned long)(DBObjPtr)("OFL_BAL_TRF_AVAI_SETT",csValueTmp) == FOUND) {
		if(strcmp(csValueTmp,"Y")==0)
			iTransferByAvaiSett = PD_TRUE;
	}
	FREE_ME(csValueTmp);

DEBUGLOG(("Authorize::Balance Transfer by Available Settlement = [%d]\n", iTransferByAvaiSett));

	//Update Balance
	if(iRet==PD_OK && iTransferByAvaiSett==PD_FALSE){
DEBUGLOG(("Authorize::Call BOOLBalance:UpdateAFPOFloat\n"));
                BOObjPtr = CreateObj(BOPtr,"BOOLBalance","UpdateAFPOFloat");
		iRet = (unsigned long)((*BOObjPtr)(hContext));
		if (iRet != PD_OK) {
DEBUGLOG(("Authorize::BOOLBalance:UpdateAFPOFloat Failed Ret [%d]\n", iRet));
ERRLOG("TxnOmtByUsOMO::BOOLBalance::UpdateAFPOFloat Failed Ret [%d]\n", iRet);
                } else { 
			if (GetField_Double(hContext, "merchant_open_bal", &dTmp)) {
DEBUGLOG(("Authorize::BOOLBalance:UpdateAFPOFloat merchant_open_bal [%lf]\n", dTmp));
				PutField_Double(hContext, "open_bal", dTmp);
			}

			if (GetField_Double(hContext, "merchant_open_bal_settlement", &dTmp)) {
DEBUGLOG(("Authorize::BOOLBalance:UpdateAFPOFloat merchant_open_bal_settlement [%lf]\n", dTmp));
        ...
        if(iRet==PD_OK && iTransferByAvaiSett==PD_TRUE){
...
ERRLOG("TxnOmtByUsOMO::BOOLADjustment::ProcesspartyBalance Failed Ret [%d]\n", iRet);
		} else {	
...
ERRLOG("TxnOmtByUsOMO::BOOLADjustment::ProcesspartyBalance Failed to get dc_ind\n");
				iRet = PD_ERR;
			}
...
DEBUGLOG(("TxnOmtByUsOMO Normal Exit() iRet = [%d]\n",iRet));
        return iRet;
}
