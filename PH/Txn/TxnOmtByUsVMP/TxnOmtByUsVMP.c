...
-------------------------------                    ------------            --------------
Init Version                                       2016/09/22              Dirk Wong
*/
...
void 	TxnOmtByUsVMP(char    cdebug)
{
        cDebug = cdebug;
}
...
ERRLOG("TxnOmtByUsVMP::Authorize::merchant_id not found!!\n");
...
ERRLOG("TxnOmtByUsVMP::Authorize::service_code not found!!\n");
...
ERRLOG("TxnOmtByUsVMP::Authorize::remark not found!!\n");
...
DEBUGLOG(("Authorize::txn_ccy = [%s]\n",csTmp));
	...
	if (iRet == PD_OK) {
		...
		// bal_type
		PutField_Char(hContext, "bal_type", PD_MERCH_AVA_PO_POOL);
	}
...
DEBUGLOG(("Authorize::Call BOOLTxnElements:VoidDynamicMerchantElements\n"));
                BOObjPtr = CreateObj(BOObjPtr,"BOOLTxnElements","VoidDynamicMerchantElements");
...
DEBUGLOG(("TxnOmtByUsVMP Normal Exit() iRet = [%d]\n",iRet));
	return iRet;
}
...
ERRLOG("TxnOmtByUsVMP:Authorize::GetTxnHeader::not found record!!\n");
                        iRet=INT_NOT_RECORD;
                }
...
ERRLOG("TxnOmtByUsVMP::GetTxnInfo::GetTxnDetail::not found record!!\n");
			iRet=INT_NOT_RECORD;
		}
	...
	// Get OL Txn Elements (Float Movement) 
        if (iRet == PD_OK) {
		recordset_init(rRecordSet,0);
...
DEBUGLOG(("GetTxnInfo::Call DBOLTxnElements:GetFeeChgDetailByType [%s]\n", PD_ELEMENT_MFLT_AMT));
		...
		if ((*DBObjPtr)(csTxnSeq, PD_ELEMENT_MFLT_AMT, cPartyType, rRecordSet) == PD_OK) {
DEBUGLOG(("GetTxnInfo::GetFeeChgDetailByType::[%s] found record = [%s]\n",PD_ELEMENT_MFLT_AMT, csTxnSeq));
			hRec = RecordSet_GetFirst(rRecordSet);
			while (hRec) {
/* amount */
...
ERRLOG("TxnOmtByUsVMP::GetTxnInfo::GetFeeChgDetailByType:: [%s] not found record for [%s]!!\n", PD_ELEMENT_MFLT_AMT, csTxnSeq);
			iRet=INT_NOT_RECORD;
		}
	...
	// Get OL Txn Elements (Fundin Movement) 
        if (iRet == PD_OK) {
		recordset_init(rRecordSet,0);

DEBUGLOG(("GetTxnInfo::Call DBOLTxnElements:GetFeeChgDetailByType [%s]\n", PD_ELEMENT_MFIN_AMT));
		DBObjPtr = CreateObj(DBPtr,"DBOLTxnElements","GetFeeChgDetailByType");
		if ((*DBObjPtr)(csTxnSeq, PD_ELEMENT_MFIN_AMT, cPartyType, rRecordSet) == PD_OK) {
DEBUGLOG(("GetTxnInfo::GetFeeChgDetailByType::[%s] found record = [%s]\n",PD_ELEMENT_MFIN_AMT, csTxnSeq));
			hRec = RecordSet_GetFirst(rRecordSet);
			while (hRec) {
/* amount */
				if (GetField_Double(hRec, "amount", &dTmp)) {
...
ERRLOG("TxnOmtByUsVMP::GetTxnInfo::GetFeeChgDetailByType:: [%s] not found record for [%s]!!\n", PD_ELEMENT_MFIN_AMT, csTxnSeq);
	...
	// Get OL Txn Elements (Payout Reserved Movement) 
        if (iRet == PD_OK) {
		recordset_init(rRecordSet,0);

DEBUGLOG(("GetTxnInfo::Call DBOLTxnElements:GetFeeChgDetailByType [%s]\n", PD_ELEMENT_MPOR_AMT));
		DBObjPtr = CreateObj(DBPtr,"DBOLTxnElements","GetFeeChgDetailByType");
		if ((*DBObjPtr)(csTxnSeq, PD_ELEMENT_MPOR_AMT, cPartyType, rRecordSet) == PD_OK) {
DEBUGLOG(("GetTxnInfo::GetFeeChgDetailByType::[%s] found record = [%s]\n",PD_ELEMENT_MPOR_AMT, csTxnSeq));
			hRec = RecordSet_GetFirst(rRecordSet);
			while (hRec) {
/* amount */
				if (GetField_Double(hRec, "amount", &dTmp)) {
