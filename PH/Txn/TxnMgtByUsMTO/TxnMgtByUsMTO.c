...
not debit available settlement,
	debit AFPO float instead		   2013/02/18		   LokMan Chow
...
void TxnMgtByUsMTO(char    cdebug)
{
        cDebug = cdebug;
}
...
ERRLOG("TxnMgtByUsMTO::Authorize::txnid not found!!\n");
...
ERRLOG("TxnMgtByUsMTO::Authorize::txn_code not found!!\n");
                iRet=INT_ERR;
...
ERRLOG("TxnMgtByUsMTO::Authorize::merchant_id not found!!\n");
...
ERRLOG("TxnMgtByUsMTO::Authorize::serivce_code not found!!\n");
...
ERRLOG("TxnMgtByUsMTO:Authorize::invalid service code [%s]\n",csService);
...
ERRLOG("TxnMgtByUsMTO::Authorize::country not found!!\n");
...
ERRLOG("TxnMgtByUsMTO::Authorize::ccy not found!!\n");
...
ERRLOG("TxnMgtByUsMTO::Authorize::txn_amt not found!!\n");
...
ERRLOG("TxnMgtByUsMTO::Authorize::approval_date not found!!\n");
                        iRet=INT_INVALID_TXN;
...
ERRLOG("TxnMgtByUsMTO::BOFee::GetTxnFee Failed Ret [%d]\n", iRet);
			iRet = INT_ERR;
                } 	
		else {
...
DEBUGLOG(("Authorize::BOFee:GetTxnFee src_txn_fee_ccy [%s]\n", csTmp));
	...
	if ((unsigned long)(DBObjPtr)("BAL_TRF_AVAI_SETT",csValueTmp) == FOUND) {
...
DEBUGLOG(("Authorize::Call BOBalance:UpdateAFPOFloat\n"));
                BOObjPtr = CreateObj(BOPtr,"BOBalance","UpdateAFPOFloat");
		iRet = (unsigned long)((*BOObjPtr)(hContext));
		if (iRet != PD_OK) {
DEBUGLOG(("Authorize::BOBalance:UpdateAFPOFloat Failed Ret [%d]\n", iRet));
ERRLOG("TxnMgtByUsMTO::BOBalance::UpdateAFPOFloat Failed Ret [%d]\n", iRet);
                } else { 
			if (GetField_Double(hContext, "merchant_open_bal", &dTmp)) {
DEBUGLOG(("Authorize::BOBalance:UpdateAFPOFloat merchant_open_bal [%lf]\n", dTmp));
				PutField_Double(hContext, "open_bal", dTmp);
			}

			if (GetField_Double(hContext, "merchant_open_bal_settlement", &dTmp)) {
DEBUGLOG(("Authorize::BOBalance:UpdateAFPOFloat merchant_open_bal_settlement [%lf]\n", dTmp));
...
DEBUGLOG(("TxnMgtByUsMTO Normal Exit() iRet = [%d]\n",iRet));
        return iRet;
}
