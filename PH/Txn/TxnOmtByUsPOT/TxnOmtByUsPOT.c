...
void TxnOmtByUsPOT(char    cdebug)
{
        cDebug = cdebug;
}
...
DEBUGLOG(("Authorize::call DBOLMerchantUploadFileDetail->MatchRespTxn_ForUpdate\n"));
        DBObjPtr = CreateObj(DBPtr,"DBOLMerchantUploadFileDetail","MatchRespTxn_ForUpdate");
...
DEBUGLOG(("Authorize::call BOOLPayout->UpdateDetailByTxn\n"));
		BOObjPtr = CreateObj(BOPtr,"BOOLPayout","UpdateDetailByTxn");
		iRet = (unsigned long)(*BOObjPtr)(hContext,hRequest);
	}
...
DEBUGLOG(("TxnOmtByUsPOT Normal Exit() iRet = [%d]\n",iRet));
	return iRet;
}
