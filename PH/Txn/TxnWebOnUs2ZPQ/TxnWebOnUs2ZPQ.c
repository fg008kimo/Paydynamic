...
OBJPTR(DB);
OBJPTR(BO);
int     UpdatePreZPQRespTxnLog(hash_t *hContext,
                	hash_t* hResponse);

int     AddZPQTxnLog(hash_t *hContext,
                const hash_t* hRequest);

void TxnWebOnUs2ZPQ(char    cdebug)
{
        cDebug = cdebug;
}
int     AddZPQTxnLog(hash_t *hContext,
...

{
	...
	hash_t	*hTxn;
	hash_t	*hRec;
DEBUGLOG(("TxnWebOnUs2ZPQ::UpdateRespTxnLog()\n"));
...
DEBUGLOG(("UpdateTxnLog  iRet = [%d] after Insert TxnQrRequestLog\n",iRet));
		...
		int	iResult;
        	...
        	iRet = (unsigned long)(*DBObjPtr)(PD_CHANNEL_ZPAY_QR,&iResult,csTmpStatus);
...
DEBUGLOG(("process_resp_txn()\n"));
	...
	PutField_CString(hContext,"psp_channel_code",PD_CHANNEL_ZPAY_QR);
	PutField_CString(hContext,"return_psp_channel",PD_CHANNEL_ZPAY_QR);
	...
	GetField_CString(hRequest,"psp_name",&csPspName);
      	MsgObjPtr = CreateObj(MsgPtr,"ZpqMsg","BreakDownInitRspMsg");

        if ((*MsgObjPtr)(hResponse,inMsg->msg,inMsg->len) != PD_OK)  {
		iRet = INT_BREAKDOWN_ERR;
		PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("process_resp_txn:BreakDown ZpqMsg Error\n"));
	}
//	else {
		// testing
...
DEBUGLOG(("process_resp_txn() call Update\n"));
		iRet = UpdatePreZPQRespTxnLog(hContext,hResponse);
...
DEBUGLOG(("TxbWebOnUs2ZPQ: time_redirect = [%d]\n",iTmp));
	        ...
	        if (!GetField_CString(hContext,"from_txn_seq",&csTxnSeq)) 
       		 	...
       		 	if ((*BOObjPtr)(hContext,hRequest,hRequest) == PD_OK) {
       		 		...
       		 		MsgObjPtr = CreateObj(MsgPtr,"ZpqMsg","FormatInitMsg");
...
DEBUGLOG(("TxnWebOnUs2ZPQ: after process_resp_err_txn iret = [%d]\n",iRet));
					}
					else {
						msg->mtext[iRecvLen] = '\0';
DEBUGLOG(("TxnWebOnUs2ZPQ:  recv len = [%d]\n",iRecvLen));
DEBUGLOG(("TxnWebOnUs2ZPQ:  recv = [%s]\n",&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN));
						memcpy(h_msg->msg,&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN);
						iRet = process_resp_txn(hContext,h_msg,hRequest);
DEBUGLOG(("TxnWebOnUs2ZPQ: after process_resp_txn iret = [%d]\n",iRet));
					}
                		}
        			FREE_ME(msg);
        		}     
...
DEBUGLOG(("TxnWebOnUs2ZPQ: BuildData Error\n"));
		}
	}
...
DEBUGLOG(("TxnWebOnUs2ZPQ: noraml exit iret = [%d]\n",iRet));
	return iRet;
}
