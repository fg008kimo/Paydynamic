...
OBJPTR(DB);
OBJPTR(BO);
int     UpdatePreUNWRespTxnLog(hash_t *hContext,
                	hash_t* hResponse);

int     AddUNWTxnLog(hash_t *hContext,
                const hash_t* hRequest);

void TxnWebOnUs2UNW(char    cdebug)
{
        cDebug = cdebug;
}
...

{
	...
	hash_t	*hTxn;
	hash_t	*hRec;
DEBUGLOG(("TxnWebOnUs2UNW::UpdateRespTxnLog()\n"));
...
DEBUGLOG(("UpdateTxnLog  iRet = [%d] after Insert TxnQrRequestLog\n",iRet));
		...
		int	iResult;
        	...
        	iRet = (unsigned long)(*DBObjPtr)(PD_CHANNEL_UNIPAY_WECHAT,&iResult,csTmpStatus);
...
DEBUGLOG(("process_resp_txn()\n"));
	...
	PutField_CString(hContext,"psp_channel_code",PD_CHANNEL_UNIPAY_WECHAT);
	PutField_CString(hContext,"return_psp_channel",PD_CHANNEL_UNIPAY_WECHAT);
	...
	GetField_CString(hRequest,"psp_name",&csPspName);
      	MsgObjPtr = CreateObj(MsgPtr,"UnwMsg","BreakDownInitRspMsg");

        if ((*MsgObjPtr)(hResponse,inMsg->msg,inMsg->len) != PD_OK)  {
		iRet = INT_BREAKDOWN_ERR;
		PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("process_resp_txn:BreakDown UnwMsg Error\n"));
	}
//	else {
		// testing
...
DEBUGLOG(("process_resp_txn: returned return_code = [%s]\n", csTmp));
			...
			if (!strcmp(csTmp, PD_SUCCESS_STATUS) ||
			    !strcmp(csTmp, PD_SUCCESS_STATUS_2)) {
...
DEBUGLOG(("process_resp_txn() call Update\n"));
		iRet = UpdatePreUNWRespTxnLog(hContext,hResponse);
...
DEBUGLOG(("TxbWebOnUs2UNW: time_redirect = [%d]\n",iTmp));
	        ...
	        if (!GetField_CString(hContext,"from_txn_seq",&csTxnSeq)) 
       		 	...
       		 	if ((*BOObjPtr)(hContext,hRequest,hRequest) == PD_OK) {
       		 		...
       		 		MsgObjPtr = CreateObj(MsgPtr,"UnwMsg","FormatInitMsg");
...
DEBUGLOG(("TxnWebOnUs2UNW: after process_resp_err_txn iret = [%d]\n",iRet));
					}
					else {
						msg->mtext[iRecvLen] = '\0';
DEBUGLOG(("TxnWebOnUs2UNW:  recv len = [%d]\n",iRecvLen));
DEBUGLOG(("TxnWebOnUs2UNW:  recv = [%s]\n",&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN));
						memcpy(h_msg->msg,&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN);
						iRet = process_resp_txn(hContext,h_msg,hRequest);
DEBUGLOG(("TxnWebOnUs2UNW: after process_resp_txn iret = [%d]\n",iRet));
					}
                		}
        			FREE_ME(msg);
        		}     
...
DEBUGLOG(("TxnWebOnUs2UNW: BuildData Error\n"));
		}
	}
...
DEBUGLOG(("TxnWebOnUs2UNW: noraml exit iret = [%d]\n",iRet));
	return iRet;
}
