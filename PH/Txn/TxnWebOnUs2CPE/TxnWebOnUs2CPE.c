...
-------------------------------                    ------------            --------------
Init Version                                       2017/12/18              David Wong
*/
...
int UpdatePreCPERespTxnLog(hash_t *hContext,
		hash_t* hResponse);

int AddCPETxnLog(hash_t *hContext,
		const hash_t* hRequest);


void TxnWebOnUs2CPE(char cdebug)
{
	cDebug = cdebug;
}
		...
		hash_t* hResponse)
{
	...
	hash_t	*hRec;
	hash_t	*hInfo;
DEBUGLOG(("TxnWebOnUs2CPE::UpdateRespTxnLog()\n"));
...
/* pay_url */
	if (GetField_CString(hResponse, "pay_url", &csPtr)) {
DEBUGLOG(("UpdateTxnLog RESP:: pay_url = [%s]\n", csPtr))
...
DEBUGLOG(("UpdateTxnLog iRet = [%d] after Update TxnPspDetail\n", iRet));
	...
	if (iRet == PD_OK) {
		...
		iRet = (unsigned long)(*DBObjPtr)(PD_CHANNEL_COALAPAY_EC, &iResult, csTmpStatus);
...
DEBUGLOG(("process_resp_txn()\n"));
	...
	PutField_CString(hContext, "psp_channel_code", PD_CHANNEL_COALAPAY_EC);
	PutField_CString(hContext, "return_psp_channel", PD_CHANNEL_COALAPAY_EC);
	...
	MsgObjPtr = CreateObj(MsgPtr, "CpeMsg", "BreakDownInitRspMsg");
	if ((*MsgObjPtr)(hResponse, inMsg->msg, inMsg->len) != PD_OK) {
		iRet = INT_BREAKDOWN_ERR;
		PutField_Int(hContext, "internal_error", iRet);
DEBUGLOG(("process_resp_txn: BreakDown CpeMsg Error\n"));
	}
//	else {
		if (GetField_CString(hResponse, "return_code", &csTmp)) {
DEBUGLOG(("process_resp_txn: returned status = [%s]\n", csTmp));

			if (!strcmp(csTmp, PD_OK_STATUS)) {
				// verify returned txn_id
				if (GetField_CString(hResponse, "app_id", &csTmp)) {
...
DEBUGLOG(("process_resp_txn: cannot get app_id!!!\n"));
					PutField_CString(hResponse, "return_code", PD_FAIL_STATUS);
					iSendAlert = PD_TRUE;
					sprintf(csCmd, "send_init_req_error.sh \"%s\" \"%s\" \"%s\"", PD_INVALID_FIELD, csPspId, csPspName);
					goto skipped;
...
DEBUGLOG(("process_resp_txn: cannot get amount!!!\n"));
				...
				// get redirect_url
				if (!GetField_CString(hResponse, "pay_url", &csTmp)) {
DEBUGLOG(("process_resp_txn: cannot get pay_url!!!\n"));
					PutField_CString(hResponse, "return_code", PD_FAIL_STATUS);
					iSendAlert = PD_TRUE;
					sprintf(csCmd, "send_init_req_error.sh \"%s\" \"%s\" \"%s\"", PD_INVALID_FIELD, csPspId, csPspName);
					goto skipped;
...
DEBUGLOG(("process_resp_txn() call Update\n"));
		iRet = UpdatePreCPERespTxnLog(hContext, hResponse);
//	}
	FREE_ME(csCmd);
...
DEBUGLOG(("process_resp_err_txn()\n"));
	...
	PutField_CString(hContext, "psp_channel_code", PD_CHANNEL_COALAPAY_EC);
	PutField_CString(hResponse, "status", PD_FAIL_STATUS);
	GetField_CString(hContext, "psp_id", &csPspId);
	GetField_CString(hRequest, "psp_name", &csPspName);
...
DEBUGLOG(("TxbWebOnUs2CPE: txn_code = [%s]\n", csTxnCode));
	...
	if (iRet == PD_OK) {
		...
		if ((*BOObjPtr)(hContext, hRequest, hRequest) == PD_OK) {
			...
			MsgObjPtr = CreateObj(MsgPtr, "CpeMsg", "FormatInitMsg");
...
DEBUGLOG(("TxnWebOnUs2CPE: after process_resp_err_txn iret = [%d]\n", iRet));
					}
					else {
						msg->mtext[iRecvLen] = '\0';
DEBUGLOG(("TxnWebOnUs2CPE: recv len = [%d]\n", iRecvLen));
DEBUGLOG(("TxnWebOnUs2CPE: recv = [%s]\n", &msg->mtext[MQ_HEADER_LEN], iRecvLen - MQ_HEADER_LEN));
						memcpy(h_msg->msg, &msg->mtext[MQ_HEADER_LEN], iRecvLen - MQ_HEADER_LEN);
						iRet = process_resp_txn(hContext, h_msg, hRequest);
DEBUGLOG(("TxnWebOnUs2CPE: after process_resp_txn iret = [%d]\n", iRet));
					}
				}
				FREE_ME(msg);
			}
...
DEBUGLOG(("TxnWebOnUs2CPE: BuildData Error\n"));
		}
	}

DEBUGLOG(("TxnWebOnUs2CPE: noraml exit iret = [%d]\n", iRet));
	return iRet;
}
