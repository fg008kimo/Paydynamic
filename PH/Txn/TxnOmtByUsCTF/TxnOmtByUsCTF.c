...
void TxnOmtByUsCTF(char    cdebug)
{
        cDebug = cdebug;
}
...
ERRLOG("TxnOmtByUsCTF:Authorize() txn seq is missing\n");
        }
...
ERRLOG("TxnOmtByUsCTF:Authorize() txn seq to is missing\n");
	}
...
DEBUGLOG(("Authorize::GetTxnInfo (From party) Failed!\n"));
			...
			if (GetField_CString(hFrContext, "txn_code", &csTmp)) {
				if (strcmp(csTmp, PD_OFL_AVA_PAYOUT_REQ_TF_FROM))  {
					iRet = INT_INVALID_TXN;
					PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("Authorize::() txn_code [%s] is not expected\n", csTmp));
ERRLOG("TxnOmtByUsCTF:Authorize() txn code [%s] is not expected\n", csTmp);
				}
			}
		}
...
DEBUGLOG(("Authorize::GetTxnInfo (To party) Failed!\n"));
			...
			if (GetField_CString(hFrContext, "txn_code", &csTmp)) {
				if (strcmp(csTmp, PD_OFL_AVA_PAYOUT_REQ_TF_TO))  {
					iRet = INT_INVALID_TXN;
					PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("Authorize::() txn_code [%s] is not expected\n", csTmp));
ERRLOG("TxnOmtByUsCTF:Authorize() txn code [%s] is not expected\n", csTmp);
...
ERRLOG("TxnOmtByUsCTF:Authorize() the inpuuted seq is not in a pair\n");
			}
		}
		else {
			iRet = INT_INVALID_TXN;
...
ERRLOG("TxnOmtByUsCTF:Authorize() org_txn_idis missing on TO information\n");
		}
	}
...
DEBUGLOG(("TxnOmtByUsCTF Normal Exit() iRet = [%d]\n",iRet));
	return iRet;
}
...
ERRLOG("TxnOmtByUsCTF:GetTxnHeader::Invalid status!!\n");
					} 
					else {
						if (GetField_Char(hRec, "ar_ind", &cTmp)) {
							iRet = INT_INVALID_TXN;
DEBUGLOG(("GetTxnHeader::Invalid ar_ind [%c]\n",cTmp));
ERRLOG("TxnOmtByUsCTF:GetTxnHeader::Invalid ar_ind!!\n");
						}
					}
				}		
...
ERRLOG("TxnOmtByUsCTF:Authorize::GetTxnHeader::not found record!!\n");
			iRet=INT_NOT_RECORD;
		}
...
int	UpdateHeaderLog(const hash_t *hContext, const hash_t *hRequest)
{
	...
	if (iTmp != PD_NO_LOG) {
		...
		if (iRet == PD_OK) {
DEBUGLOG(("Authorize:UpdateHeaderLog:Call OMTChannel:UpdateTxnLog\n"));
...
DEBUGLOG(("Authorize:UpdateHeaderLog:Call OMTChannel:UpdateTxnLog FAILED\n"));
ERRLOG("TxnOmtByUsCTF:Authorize() UpdateHeaderLog Call OMTChannel:UpdateTxnLog FAILED\n");
				iRet = INT_ERR;
			}
		}
