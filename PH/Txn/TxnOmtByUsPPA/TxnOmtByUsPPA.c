...
-------------------------------                    ------------            --------------
Init Version                                       2014/02/05              LokMan Chow
*/
...
void TxnOmtByUsPPA(char    cdebug)
{
        cDebug = cdebug;
}
...
ERRLOG("TxnOmtByUsPPA:Authorize::bank_name not found!!\n");
			iRet=INT_ERR;
		}
...
ERRLOG("TxnOmtByUsPPA::Authorize() total_amt Invalid\n");
			}
		}
        }
...
ERRLOG("TxnOmtByUsPPA:Authorize::batch_id not found!!\n");
			iRet=INT_ERR;
		}
	}
...
ERRLOG("TxnOmtByUsPPA:Authorize::total_amt not found!!\n");
			iRet=INT_ERR;
		}
        }
...
ERRLOG("TxnOmtByUsPPA::Authorize() %s Invalid[%s]\n",csTag,csTmp);
				}
			}
			else{
DEBUGLOG(("Authorize:: %s not found\n",csTag));
ERRLOG("TxnOmtByUsPPA::Authorize:: %s not found\n",csTag);
				iRet = INT_ERR;
			}
		}
...
ERRLOG("TxnOmtByUsPPA::Authorize::total_cnt not found\n");
                        iRet = INT_ERR;
                }
...
ERRLOG("TxnOmtByUsPPA::Authorize::Invalid input ind!!!\n");
                        iRet = INT_ERR;
	}
...
DEBUGLOG(("Authorize::DBOLMerchantUploadFileDetail:GetNextPreApproveId\n"));
		DBObjPtr = CreateObj(DBPtr,"DBOLMerchantUploadFileDetail","GetNextPreApproveId");
		if((unsigned long) ((*DBObjPtr)(&iApproveId))!=PD_OK){
			iRet = INT_ERR;
			PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("Authorize::DBOLMerchantUploadFileDetail:GetNextPreApproveId Failed\n"));
		}
		else{
...
DEBUGLOG(("BOOLPayout::DBOLMerchantUploadFileDetail:UpdateDetailByBatchId Failed\n"));
	...
	if(iRet==PD_OK){
		DBObjPtr = CreateObj(DBPtr,"DBOLMerchantUploadFileDetail","ResumeUnusedPreApproveId");
		if ((*DBObjPtr)(iApproveId) != PD_OK) {
			iRet = INT_ERR;
DEBUGLOG(("BOOLPayout::DBOLMerchantUploadFileDetail:ResumeUnusedPreApproveId Failed\n"));
			PutField_Int(hContext,"internal_error",iRet);
		}
	}


DEBUGLOG(("TxnOmtByUsPPA Normal Exit() iRet = [%d]\n",iRet));
	return iRet;
}
