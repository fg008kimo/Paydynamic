...
Handle pre-defined paypage bank list               2014/08/27              Virginia Yun
*/
...
//DEBUGLOG(("Called DBOLBankAccts\n"));
		...
		DBObjPtr = CreateObj(DBPtr,"DBOLRulePaypageBank","GetBankList");
                if ((DBObjPtr)(csOrgMerchantId,csClientId,rRecordSet) == PD_OK) {
DEBUGLOG(("Called DBOLRulePaypageBank\n"));
			int i = 0;
			char 	*csTag;
...
DEBUGLOG(("Bank Code = [%s]\n",csPtr));
					if (GetField_Int(hRec, "display_order", &iTmp)) {
...
DEBUGLOG(("Call DBOLRulePaypageBank Error\n"));
			iRet = PD_ERR;
		}
...
DEBUGLOG(("bank code cnt = [%d]\n",iBankCodeCnt));
						...
						sprintf(csTag, "bank_order_%d", iBankCodeCnt);
						PutField_Int(hResponse,csTag, PD_OTHER_BANK_ORDER);
