...
-------------------------------                    ------------            --------------
Init Version                                       2017/01/03              David Wong
*/
...
OBJPTR(DB);
OBJPTR(BO);
int     UpdatePreXJPRespTxnLog(hash_t *hContext,
                	hash_t* hResponse);

int     AddXJPTxnLog(hash_t *hContext,
                const hash_t* hRequest);

void TxnWebOnUs2XJP(char    cdebug)
{
        cDebug = cdebug;
}
int     AddXJPTxnLog(hash_t *hContext,
...

{
	...
	hash_t	*hTxn;
	hash_t	*hRec;
DEBUGLOG(("TxnWebOnUs2XJP::UpdateRespTxnLog()\n"));
...
DEBUGLOG(("UpdateTxnLog RESP:: status not found\n"));
		csTmpStatus=strdup(" ");
	}
...
DEBUGLOG(("UpdateTxnLog  iRet = [%d] after Update TxnPspDetail\n",iRet));
		...
		int	iResult;
        	...
        	iRet = (unsigned long)(*DBObjPtr)(PD_CHANNEL_XJP,&iResult,csTmpStatus);
...
DEBUGLOG(("process_resp_txn()\n"));
	...
	PutField_CString(hContext,"psp_channel_code",PD_CHANNEL_XJP);
	PutField_CString(hContext,"return_psp_channel",PD_CHANNEL_XJP);
	...
	GetField_CString(hRequest,"psp_name",&csPspName);
      	MsgObjPtr = CreateObj(MsgPtr,"XjpMsg","BreakDownInitRspMsg");

        if ((*MsgObjPtr)(hResponse,inMsg->msg,inMsg->len) != PD_OK)  {
		iRet = INT_BREAKDOWN_ERR;
		PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("process_resp_txn:BreakDown XjpMsg Error\n"));
	}
//	else {
		// testing
		//RemoveField_CString(hResponse, "status");

		if (GetField_CString(hResponse, "status", &csTmp)) {
DEBUGLOG(("process_resp_txn: returned status = [%s]\n", csTmp));

			if (!strcmp(csTmp, "REDIRECT")) {
				// get mid
				if (!GetField_CString(hResponse, "merchantId", &csTmp)) {
DEBUGLOG(("process_resp_txn: cannot get merchantId!!!\n"));
...
DEBUGLOG(("process_resp_txn: cannot get requestId!!!\n"));
				...
				// get redirect_url
				if (!GetField_CString(hResponse, "redirectUrl", &csTmp)) {
DEBUGLOG(("process_resp_txn: cannot get redirectUrl!!!\n"));
					PutField_CString(hResponse, "status", PD_FAIL_STATUS);
					iSendAlert = PD_TRUE;
					sprintf(csCmd, "send_init_req_error.sh \"%s\" \"%s\" \"%s\"", PD_INVALID_FIELD, csPspId, csPspName);
					goto skipped;
...
DEBUGLOG(("process_resp_txn: cannot get status!!!\n"));
			PutField_CString(hResponse, "status", PD_FAIL_STATUS);
			iSendAlert = PD_TRUE;
			sprintf(csCmd, "send_init_req_error.sh \"%s\" \"%s\" \"%s\"", PD_INVALID_FIELD, csPspId, csPspName);
			goto skipped;
...
DEBUGLOG(("process_resp_txn() call Update\n"));
		iRet = UpdatePreXJPRespTxnLog(hContext,hResponse);
//	}
	FREE_ME(csCmd);
	...
	if (iRet == PD_OK) {
	        ...
	        if (!GetField_CString(hContext,"from_txn_seq",&csTxnSeq)) 
       		 	...
       		 	if ((*BOObjPtr)(hContext,hRequest,hRequest) == PD_OK) {
       		 		...
       		 		MsgObjPtr = CreateObj(MsgPtr,"XjpMsg","FormatInitMsg");
...
DEBUGLOG(("TxnWebOnUs2XJP: after process_resp_err_txn iret = [%d]\n",iRet));
					}
					else {
						msg->mtext[iRecvLen] = '\0';
DEBUGLOG(("TxnWebOnUs2XJP:  recv len = [%d]\n",iRecvLen));
DEBUGLOG(("TxnWebOnUs2XJP:  recv = [%s]\n",&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN));
						memcpy(h_msg->msg,&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN);
						iRet = process_resp_txn(hContext,h_msg,hRequest);
DEBUGLOG(("TxnWebOnUs2XJP: after process_resp_txn iret = [%d]\n",iRet));
					}
                		}
        			FREE_ME(msg);
        		}     
...
DEBUGLOG(("TxnWebOnUs2XJP: BuildData Error\n"));
		}
	}

DEBUGLOG(("TxnWebOnUs2XJP: noraml exit iret = [%d]\n",iRet));
	return iRet;
}
