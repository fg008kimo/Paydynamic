...
void TxnOmtByUsOPA(char    cdebug)
{
        cDebug = cdebug;
}
...
DEBUGLOG(("Authorize::call TxnOmtByUsPPA\n"));
		TxnObjPtr = CreateObj(TxnPtr,"TxnOmtByUsPPA","Authorize");
		iRet = (unsigned long)(*TxnObjPtr)(hContext,hRequest,hResponse);
DEBUGLOG(("Authorize::TxnOmtByUsPPA iRet = [%d]\n",iRet));
DEBUGLOG(("TxnOmtByUsOPA Normal Exit()\n"));
		return iRet;
	}
...
ERRLOG("TxnOmtByUsOPA:Authorize::bank_name not found!!\n");
			iRet=INT_ERR;
		}
...
ERRLOG("TxnOmtByUsOPA::Authorize() total_amt Invalid\n");
			}
		}
        }
...
ERRLOG("TxnOmtByUsOPA:Authorize::batch_id not found!!\n");
			iRet=INT_ERR;
		}
	}
...
ERRLOG("TxnOmtByUsOPA:Authorize::total_amt not found!!\n");
			iRet=INT_ERR;
		}
        }
...
ERRLOG("TxnOmtByUsOPA::Authorize() %s Invalid[%s]\n",csTag,csTmp);
				}
				else{
					if(i==0){
						dMinAmt = dTmp;
...
ERRLOG("TxnOmtByUsOPA::Authorize:: %s not found\n",csTag);
				iRet = INT_ERR;
			}
		}
...
ERRLOG("TxnOmtByUsOPA::Authorize::total_cnt not found\n");
                        iRet = INT_ERR;
                }
...
/*Selected Transactions*/
	else if(cInputInd == PD_BY_SELECTED_TXN){
		if(GetField_CString(hRequest,"batch_id",&csTmp)){
			PutField_CString(hContext,"tmp_batch_id",csTmp);
DEBUGLOG(("Authorize::tmp batch_id= [%s]\n",csTmp));
		}
		else{
DEBUGLOG(("Authorize::tmp batch_id not found!!\n"));
ERRLOG("TxnMgtByUsPOA:Authorize::tmp batch_id not found!!\n");
			iRet=INT_ERR;
		}
...
ERRLOG("TxnOmtByUsOPA::Authorize::Invalid input ind!!!\n");
                        iRet = INT_ERR;
	}
...
DEBUGLOG(("Authorize::call BOOLPayout->GetPreApproveRecord\n"));
				BOObjPtr = CreateObj(BOPtr,"BOOLPayout","GetPreApproveRecord");
...
DEBUGLOG(("Authorize::merchant_fundin_po = [%lf]\n",dTmp));
	...
	if(iRet==PD_OK){
DEBUGLOG(("Authorize::DBOLMerchantUploadFileDetail:GetNextApproveId\n"));
		DBObjPtr = CreateObj(DBPtr,"DBOLMerchantUploadFileDetail","GetNextApproveId");
		if((unsigned long) ((*DBObjPtr)(&iApproveId))!=PD_OK){
			iRet = INT_ERR;
			PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("Authorize::DBOLMerchantUploadFileDetail:GetNextApproveId Failed\n"));
		}
		else{
...
DEBUGLOG(("Authorize::batch last seq = [%d]\n",iLastSeq));
			...
			if(cInputInd == PD_BY_SELECTED_TXN){
				...
				iBatchCnt = 1;
				PutField_CString(hContext,"org_batch_id_0",csBatchId);
			}
...
ERRLOG("TxnOmtByUsOPA:Authorize::the file is not for approve!!\n");
					}
					else{
						hash_init(hTxn,0);
...
ERRLOG("Authorize::DBOLMerchantUploadFileHeader:Update Failed\n");
			...
			if(iRet==PD_OK){
				if(cInputInd == PD_BY_SELECTED_TXN){
					iProcess = 1;
					GetField_CString(hContext,"tmp_batch_id",&csBatchId);
				}
				else{
...
DEBUGLOG(("Authorize:: [%d]record for each process\n",iPCnt));
				for(iTh = 0; iTh<iProcess; iTh++){
					...
					sprintf(csCmd,"offline_payout_approve.exec -b %s -n %d -p %c -m %c -k %s -t %.2f -i %.2f -x %.2f -l %d -v %d -a %d -u %s &",csBatchId,iPCnt,cMode,cInputInd,csBankName,dTotalAmt,dMinAmt,dMaxAmt,iLastSeq,iPreApproveId,iApproveId,csUser);
DEBUGLOG(("Authorize:: csCmd = [%s]\n",csCmd));
DEBUGLOG(("Authorize:: offline_payout_approve.exec batch[%s], count[%d], mode[%c], type[%c], bank_name[%s], total_amt[%.2f], min_amt[%.2f], max_amt[%.2f], last_seq[%d], pre_approve_id[%d], approve_id[%d], user[%s]\n",csBatchId,iPCnt,cMode,cInputInd,csBankName,dTotalAmt,dMinAmt,dMaxAmt,iLastSeq,iPreApproveId,iApproveId,csUser));
					system(csCmd);
			...
			for(i=0;i<iCnt;i++){
				...
				if(!iSkip){
DEBUGLOG(("Authorize::call TxnOmtByUsPOT\n"));
...
DEBUGLOG(("TxnOmtByUsOPA Normal Exit() iRet = [%d]\n",iRet));
	return iRet;
}
