                        ...
                        hash_t* hResponse)
{
	...
	recordset_t     *rRelTxnRecordSet;
	rRelTxnRecordSet = (recordset_t *)malloc (sizeof(recordset_t));
	recordset_init(rRelTxnRecordSet,0);
...
DEBUGLOG(("Authorize::Call TxnMmcByUsARH\n"));
					TxnObjPtr = CreateObj(TxnPtr, "TxnMmcByUsARH","Authorize");
					if((unsigned long)(*TxnObjPtr)(hContext,hAutoRev,hResponse)!=PD_OK){
DEBUGLOG(("Authorize::call TxnMmcByUsARH Failed\n"));
					}
					break;
...
DEBUGLOG(("Authorize: MmsRelatedTxn...prev_txn_seq [%s]\n", csRelatedPrevTxnSeq));
							PutField_CString(hRelatedTxn, "prev_txn_seq", csRelatedPrevTxnSeq);
						}

						if (GetField_CString(hRec, "org_dtl_txnid", &csTmp)) {
DEBUGLOG(("Authorize: MmsRelatedTxn...prev_dtl_txn_seq [%s]\n", csTmp));
							PutField_CString(hRelatedTxn, "prev_dtl_txn_seq", csTmp);
						}

						if (GetField_CString(hContext, "new_txn_seq", &csRelatedTxnSeq)) {
DEBUGLOG(("Authorize: MmsRelatedTxn...txn_seq [%s]\n", csRelatedTxnSeq));
							PutField_CString(hRelatedTxn, "txn_seq", csRelatedTxnSeq);
						}

						if (GetField_CString(hContext, "dtl_txn_seq", &csTmp)) { // User AIN Return dtl_txn_seq
DEBUGLOG(("Authorize: MmsRelatedTxn...dtl_txn_seq [%s]\n", csTmp));
							PutField_CString(hRelatedTxn, "dtl_txn_seq", csTmp);
						}

						if (strcmp(csRelatedPrevTxnSeq, csRelatedTxnSeq)) {
DEBUGLOG(("Authorize::RecordSet_Add (RelatedTxn)\n"));

							RecordSet_Add(rRelTxnRecordSet,hRelatedTxn); // for further handling Dst 
...
DEBUGLOG(("DBMmsRelatedTxn:Add iRet[%d]\n",iRet));
	...
	if(iRet==PD_OK){
		...
		while(hRec){
			...
			if (iTmpCnt == 1) {
				if (GetField_CString(hContext, "dtl_txn_seq", &csSourceDtlTxnSeq)) {
DEBUGLOG(("Authorize:: Dst last source dtl_txn_seq [%s]\n", csSourceDtlTxnSeq));
				}
			}

			if (iGroup == PD_FALSE) {
				// if already having a new txn_seq 
				csRelatedTxnSeq = NULL;
				csRelatedPrevTxnSeq = NULL;

				if (GetField_CString(hContext, "new_txn_seq", &csRelatedTxnSeq)) {
DEBUGLOG(("Authorize:: Dst new_txn_seq [%s]\n", csRelatedTxnSeq)); 
				}

				if(GetField_CString(hContext,"org_txnid", &csRelatedPrevTxnSeq)){
DEBUGLOG(("Authorize:: Dst org_txn_seq [%s]\n", csRelatedPrevTxnSeq)); 
				}

				if (strcmp(csRelatedTxnSeq, csRelatedPrevTxnSeq)) {
					RemoveField_CString(hContext,"dtl_txn_seq");
				}	
				else {
					PutField_CString(hContext, "dtl_txn_seq", csSourceDtlTxnSeq);
				}
			}
...
/***** TESTING ******/
//PutField_Char(hContext, "test_case", PD_TRUE);
...
DEBUGLOG(("Authorize::End to Call %s:Authorize\n", csTxnObjName));
			...
			if ((iRet != PD_OK) ||  GetField_Char(hResponse, "auto_reverse", &cTmp)){
DEBUGLOG(("Authorize::auto_reverse [%c]\n", cTmp));
				/////Call Handler to reverse previous success txn
DEBUGLOG(("Authorize::Call TxnMmcByUsARH\n"));
...
/***** TESTING ******/
//PutField_Char(hContext, "test_case", PD_TRUE);
//RemoveField_Char(hContext, "test_case");
				TxnObjPtr = CreateObj(TxnPtr, "TxnMmcByUsARH","Authorize");
				if((unsigned long)(*TxnObjPtr)(hContext,hAutoRev,hResponse)!=PD_OK){
DEBUGLOG(("Authorize::call TxnMmcByUsARH Failed\n"));
				}
			...
			if (iRet == PD_OK && iGroup == PD_FALSE) {
				hRelatedTxn = (hash_t*)  malloc (sizeof(hash_t));
				hash_init(hRelatedTxn,0);

				// only exists while iGroup = PD_TRUE and iTotalSrc > 1
				hTmpRec = RecordSet_GetFirst(rRelTxnRecordSet);
				while (hTmpRec) {			

					if (GetField_CString(hTmpRec, "txn_seq", &csTmp)) {
DEBUGLOG(("Authorize:Dst MmsRelatedTxn.......... prev_txn_seq [%s]\n", csTmp));
						PutField_CString(hRelatedTxn, "prev_txn_seq", csTmp);
					}

					if (GetField_CString(hTmpRec,"dtl_txn_seq",&csTmp)) {
DEBUGLOG(("Authorize:Dst MmsRelatedTxn.......... prev_dtl_txn_seq [%s]\n", csTmp));
						PutField_CString(hRelatedTxn, "prev_dtl_txn_seq", csTmp);
					}

					if (GetField_CString(hContext,"new_txn_seq",&csTmp)) {
						PutField_CString(hRelatedTxn, "txn_seq", csTmp);
DEBUGLOG(("Authorize:Dst MmsRelatedTxn.......... txn_seq [%s]\n", csTmp));
					}

					if (GetField_CString(hContext,"dtl_txn_seq",&csTmp)) {
						PutField_CString(hRelatedTxn, "dtl_txn_seq", csTmp);
DEBUGLOG(("Authorize:Dst MmsRelatedTxn.......... dtl_txn_seq [%s]\n", csTmp));
					}

DEBUGLOG(("Authorize::Dst Call DBMmsRelatedTxn:Add\n"));
					DBObjPtr = CreateObj(DBPtr,"DBMmsRelatedTxn","Add");
					iRet = (unsigned long) ((*DBObjPtr)(hRelatedTxn));
DEBUGLOG(("Dst DBMmsRelatedTxn:Add iRet[%d]\n",iRet));
					if (iRet != PD_OK) {
						break;	
					}

					hTmpRec = RecordSet_GetNext(rRelTxnRecordSet);
				}
...
//Update Prev Txn status -> A
	if(iRet==PD_OK ||
	   !GetField_Char(hResponse, "auto_reverse", &cTmp)){
		hash_t *hOrgTxn;
...
// Update Header 
	if (iRet == PD_OK ||
	    !GetField_Char(hResponse, "auto_reverse", &cTmp)) {
		hash_t *hHeaderRec;
		hHeaderRec = (hash_t*)  malloc (sizeof(hash_t));
		hash_init(hHeaderRec,0);
		...
		while (hRec && iRet == PD_OK) {
			if (GetField_CString(hRec,"org_txnid",&csRelatedPrevTxnSeq)){
DEBUGLOG(("Authorize: TxnHeader...org_txn_id [%s]\n", csRelatedPrevTxnSeq));
			}

			if (GetField_CString(hContext, "new_txn_seq", &csRelatedTxnSeq)) {
DEBUGLOG(("Authorize: TxnHeader...new_txn_seq [%s]\n", csRelatedPrevTxnSeq));
			}
		
			if (strcmp(csRelatedPrevTxnSeq, csRelatedTxnSeq)) {
				PutField_CString(hHeaderRec, "txn_seq", csRelatedPrevTxnSeq);
				PutField_Char(hHeaderRec, "status", PD_COMPLETE);
				PutField_Char(hHeaderRec, "ar_ind", PD_ACCEPT);

DEBUGLOG(("Authorize::Call DBMmsTransaction:UpdateHeader\n"));
				DBObjPtr = CreateObj(DBPtr,"DBMmsTransaction","UpdateHeader");
				if ((unsigned long)(*DBObjPtr)(hHeaderRec) != PD_OK) {
DEBUGLOG(("DBMmsTransaction:UpdateHeader Failed!!!\n"));
	...
	FREE_ME(hRelatedTxn);
	RecordSet_Destroy(rRelTxnRecordSet);
	FREE_ME(rRelTxnRecordSet);
