                        ...
                        hash_t* hResponse)
{
	...
	char*	csOrgTxnSeq;
	char*	csOrgDtlTxnSeq;
	//char*	csBatchLastDtlTxnSeq = NULL;
	char	cTmp;
	char	*csTmp = NULL;
	double	dAmt = 0.0;
	hash_t  *hRec;
...
DEBUGLOG(("TxnMmcByUsVTX: txn_seq = [%s]\n",csOrgTxnSeq));
		PutField_CString(hTxn, "host_ref", csOrgTxnSeq);
	}

	if(GetField_CString(hRequest,"dtl_txn_seq",&csOrgDtlTxnSeq)){
DEBUGLOG(("TxnMmcByUsVTX: dtl_txn_seq = [%s]\n",csOrgDtlTxnSeq));
		PutField_CString(hTxn, "host_dtl_ref", csOrgDtlTxnSeq);
	}

/*
	if(GetField_CString(hContext,"batch_last_dtl_txn_seq",&csBatchLastDtlTxnSeq)){
DEBUGLOG(("TxnMmcByUsVTX: batch last dtl_txn_seq = [%s]\n",csBatchLastDtlTxnSeq));
	}
*/
...
DEBUGLOG(("Authorize::Put dtl_txn_seq [%s]\n", csOrgDtlTxnSeq));
	
		PutField_CString(hContext, "dtl_txn_seq", csOrgDtlTxnSeq);

		BOObjPtr = CreateObj(BOPtr,"BOMmsTask","NewTask");
                iRet = (unsigned long)((*BOObjPtr)(hContext, hTxn, hResponse));

//DEBUGLOG(("Authorize::Put dtl_txn_seq [%s]\n", csBatchLastDtlTxnSeq));

		//PutField_CString(hContext, "dtl_txn_seq", csBatchLastDtlTxnSeq);
