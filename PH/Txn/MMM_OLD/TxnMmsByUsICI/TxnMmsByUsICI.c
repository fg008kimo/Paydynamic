...
void TxnMmsByUsICI(char    cdebug)
{
	cDebug = cdebug;
}
...
ERRLOG("TxnMmsByUsICI::Authorize::business_type not found!!\n");
                iRet = INT_ERR;
...
ERRLOG("TxnMmsByUsICI::Authorize::DBMerchDetail:GetMerchantByType Failed!!\n");
		iRet = INT_ERR;
	}
	else {
		int	iTmp = 0;
...
DEBUGLOG(("Authorize::DBMerchDetail [%d] merchant_id = [%s]\n", iTmp, csTmp));
...
DEBUGLOG(("Authorize::DBMerchDetail [%d] merchant_name = [%s]\n", iTmp, csTmp));
...
DEBUGLOG(("TxnMmsByUsICI Normal Exit() iRet = [%d]\n",iRet));
	return iRet;
}
