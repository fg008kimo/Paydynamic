                        ...
                        hash_t* hResponse)
{
	...
	hash_t *hToNodes;
        hToNodes = (hash_t*)  malloc (sizeof(hash_t));
        hash_init(hToNodes,0);

	if(GetField_CString(hContext,"txn_seq",&csTmp)){
DEBUGLOG(("Authorize::txn_seq = [%s]\n",csTmp));
		PutField_CString(hToNodes, "client_ref", csTmp);
	}
...
DEBUGLOG(("Authorize::transmission_datetime = [%s]\n",csDate));
		PutField_CString(hToNodes, "transmission_datetime", csDate);
	}
...
ERRLOG("TxnMmcByUsIPM::Authorize::query_type not found!!\n");
	...
	if (GetField_CString(hContext, "mms_node_id", &csMmsNodeId)) {
DEBUGLOG(("Authorize::mms_node_id = [%s]\n",csMmsNodeId));
		PutField_CString(hTxn, "mms_node_id", csMmsNodeId);
	}
...
DEBUGLOG(("Authorize::business_type = [%c]\n",cBusinessType));
		...
		PutField_Char(hToNodes, "business_type", cBusinessType);
	}
	else {
...
DEBUGLOG(("Authorize::DBMmsIntantPMAcctDt:Delete Succ!!\n"));
			...
			if(iRet==PD_OK){
//request from nodes
				PutField_CString(hToNodes,"process_type", PD_MMS_PROCESS_INQ);
				if(cPartyType == PD_TYPE_MERCHANT){
					PutField_CString(hToNodes, "process_code", PD_MMS_INQ_MERCH_ID);
                                }
                                else if(cPartyType == PD_TYPE_PSP){
                                        PutField_CString(hToNodes,"process_code", PD_MMS_INQ_PSP_ID);
                                }
				...
				if((unsigned long)(*DBObjPtr)(PD_MMS_CLIENT,rRecordSet)==PD_FOUND){
					hRec = RecordSet_GetFirst(rRecordSet);
						...
						if(GetField_CString(hRec,"node_id",&csNodeId)){
							PutField_CString(hToNodes,"mms_node_id",csNodeId);
DEBUGLOG(("Authorize::TxnMmsByUs2Node: node_id [%s]\n",csNodeId));
						}

						if(GetField_CString(hRec,"mms_key",&csTmp)){
							PutField_CString(hToNodes,"mms_key",csTmp);
DEBUGLOG(("Authorize::TxnMmsByUs2Node: mms_key [%s]\n",csTmp));
						}
DEBUGLOG(("Authorize::Call TxnMmsByUs2Node: Authorize\n"));
                                		TxnObjPtr = CreateObj(TxnPtr, "TxnMmsByUs2Node","Authorize");
                                		iRet = (unsigned long)(*TxnObjPtr)(hContext,hToNodes,hFromNodes);
...
//.********Insert Detail ********//
									...
									PutField_CString(hDt,"mms_node_id",csNodeId);
...
// Multiple
			if (GetField_CString(hRequest,"last_id", &csLastId)) {
				...
				PutField_CString(hTxn, "last_party_id", csLastId);
...
DEBUGLOG(("Authorize::Call MmsInstantPMAcctDt: GetNextPartyId\n"));
                                ...
                                iTmpRet = (unsigned long)((*DBObjPtr)(hTxn, &csStartPartyId));
