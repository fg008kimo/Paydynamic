...
ERRLOG("TxnMmcByUsIBI::Authorize::server node_id not found!!\n");
		iRet = INT_ERR;
...
DEBUGLOG(("Authorize::GetNextBankId Succ [%s]\n", csStartBankId));
			PutField_CString(hContext, "start_bank_id", csStartBankId);
                }
...
DEBUGLOG(("Authorize::GetBankDetail [%d] mms_node_id= [%s]\n", iTmp, csMmcId));
...
DEBUGLOG(("Authorize::GetBankDetailbyRange [%d] bank_acct = [%s]\n", iTmp, csTmp));
							...
							sprintf(csTag, "mms_node_id_%d", iTmp);
							PutField_CString(hResponse, csTag, csMmcId);
DEBUGLOG(("Authorize::GetBankDetailbyRange [%d] mms_node_id= [%s]\n", iTmp, csMmcId));
