...
void TxnMmsByUsTFC(char    cdebug)
{
	cDebug = cdebug;
}
...
DEBUGLOG(("Authorize::org_txn_seq = [%s]\n",csOrgSeq));
		...
		PutField_CString(hTxn, "txn_seq", csOrgSeq);
	}
...
ERRLOG("TxnMmsByUsTFC::Authorize::txn_amt not found!!\n");
...
ERRLOG("TxnMmsByUsTFC::Authorize::ccy not found!!\n");
...
ERRLOG("TxnMmsByUsTFC::Authorize::service_code not found!!\n");
...
DEBUGLOG(("Authorize::Call BOMmsBalance: TxnTypeHandler\n"));
		...
		if (iRet != PD_OK) {
DEBUGLOG(("Authorize::TFC TxnTypeHandler Failed\n"));
ERRLOG("TxnMmsByUsTFC::Authorize::TFC TxnTypeHandler Failed\n");
                }
        }
...
DEBUGLOG(("TxnMmsByUsTFC Normal Exit() iRet = [%d]\n",iRet));
	return iRet;
}
