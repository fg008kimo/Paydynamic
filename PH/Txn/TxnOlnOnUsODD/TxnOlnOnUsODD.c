...
DEBUGLOG(("TxnOlnOnUsODD: deposit_flow = [%c]\n", cTmp));
        		...
        		if (iRet != PD_OK) {
                        	if (GetField_Int(hTxn,"internal_error",&iTmpIntErr)) {
					if (iTmpIntErr != PD_OK) {

DEBUGLOG(("TxnOlnOnUsODD: GetRequestBankInfo FAILED\n"));
ERRLOG("TxnOlnOnUsODD: GetRequestBankInfo FAILED\n");

						PutField_Int(hContext, "internal_error", iTmpIntErr);
					}
				}
			}
        	}
