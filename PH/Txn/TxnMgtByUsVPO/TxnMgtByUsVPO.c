...
Include approved void				   2013/01/10		   LokMan Chow
*/
                        ...
                        hash_t* hResponse)
{
	...
	int	iRefundParty = 0;
	hash_t  *hRec, *hOrgTxn, *hIn, *hPspDetail, *hBalance, *hPsp, *hElement;
...
DEBUGLOG(("TxnMgtByUsVPO: Void Payout Fee Return [%d]\n",iReturnFee));
	}
*/
...
ERRLOG("TxnMgtByUsVPO:Authorize::GetTxnHeader::not found record!!\n");
                        iRet=INT_NOT_RECORD;
                        PutField_Int(hContext,"internal_error",iRet);
                }
	}
...
ERRLOG("TxnMgtByUsVPO:Authorize::GetTxnDetail:: not found record!!\n");
                        iRet = INT_NOT_RECORD;
                        PutField_Int(hContext,"internal_error",iRet);
                }
        }
...
DEBUGLOG(("Authorize::deposit (void) real net amt=[%f]\n",(dTmp+dPreFee)));
			...
			if(iSameDate==PD_FALSE){
				...
				BOObjPtr = CreateObj(BOPtr,"BOTxnElements","AddAvaForPOElement");
...
ERRLOG("TxnMgtByUsVPO:Authorize::BOFee:GetTxnFee Error\n");
                }
        }
...
ERRLOG("TxnMgtByUsVPO::DBPayoutGeneratedFileDT Update Failed!!!\n");
						break;
					}
				}
