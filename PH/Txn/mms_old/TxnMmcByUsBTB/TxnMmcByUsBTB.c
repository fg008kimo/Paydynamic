...
void TxnMmcByUsBTB(char    cdebug)
{
	cDebug = cdebug;
}
...
ERRLOG("TxnMmcByUsBTB::Authorize::txnid not found!!\n");
...
ERRLOG("TxnMmcByUsBTB::Authorize::dtl_txnid not found!!\n");
                iRet=INT_INVALID_TXN;
	}
*/
...
ERRLOG("TxnMmcByUsBTB::Authorize::txn_amt not found!!\n");
...
ERRLOG("TxnMmcByUsBTB::Authorize::ccy not found!!\n");
...
ERRLOG("TxnMmcByUsBTB::Authorize::bank_charge_refund not found!!\n");
        }
*/
...
ERRLOG("TxnMmcByUsBTB::Authorize::from_node not found!!\n");
                iRet = PD_ERR;
                PutField_Int(hContext,"internal_error",iRet);
                }
        }
...
ERRLOG("TxnMmcByUsBTB::Authorize::to_node not found!!\n");
                iRet = PD_ERR;
                PutField_Int(hContext,"internal_error",iRet);
                }
        }
...
/* ??????????????
If isd_ind == SOURCE 
	Debit(-) Bank Balance :txn_amt
	Debit(-) Bank Balance :processing_cost
   else 
...
DEBUGLOG(("Authorize::Call BOMmsBalance: TxnTypeHandler\n"));
		...
		if (iRet != PD_OK) {
DEBUGLOG(("Authorize::BTBHandler Failed\n"));
ERRLOG("TxnMmcByUsBTB::Authorize::BTBHandler Failed\n");
                }
        }
*/

        if (iRet == PD_OK) { // no need send 2 nodes
DEBUGLOG(("Authorize::Call BOMmsBalance: TxnTypeHandler\n"));
                BOObjPtr = CreateObj(BOPtr,"BOMmsBalance","TxnTypeHandler");
                iRet = (unsigned long)((*BOObjPtr)(hTxn));
...
ERRLOG("TxnMmcByUsBTB::Authorize::TxnTypeHandler Failed\n");
                }
                else {
...
DEBUGLOG(("TxnMmcByUsBTB Normal Exit() iRet = [%d]\n",iRet));
	return iRet;
}
