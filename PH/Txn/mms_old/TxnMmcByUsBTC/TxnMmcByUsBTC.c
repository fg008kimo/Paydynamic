...
-------------------------------                    ------------            --------------
Init Version                                       2012/02/21              Virginia Yun
*/
...
void TxnMmcByUsBTC(char    cdebug)
{
	cDebug = cdebug;
}
...
ERRLOG("TxnMmcByUsBTC::Authorize::txnid not found!!\n");
...
ERRLOG("TxnMmcByUsBTC::Authorize::dtl_txnid not found!!\n");
                iRet=INT_INVALID_TXN;
	}
*/
...
ERRLOG("TxnMmcByUsBTC::Authorize::host_ref not found!!\n");
                iRet=INT_INVALID_TXN;
...
ERRLOG("TxnMmcByUsBTC::Authorize::host_dtl_ref not found!!\n");
                iRet=INT_INVALID_TXN;
...
DEBUGLOG(("Authorize::bank_id = [%s]\n",csTmp));
		...
		PutField_CString(hTxn,"src_party_type",PD_MMS_PARTY_BANK);
        }
...
ERRLOG("TxnMmcByUsBTC::Authorize::txn_amt not found!!\n");
...
ERRLOG("TxnMmcByUsBTC::Authorize::ccy not found!!\n");
...
ERRLOG("TxnMmcByUsBTC::Authorize::bank_charge_refund not found!!\n");
        }
*/
...
ERRLOG("TxnMmcByUsBTC::Authorize::from_node not found!!\n");
                iRet = PD_ERR;
                PutField_Int(hContext,"internal_error",iRet);
                }
        }
...
ERRLOG("TxnMmcByUsBTC::Authorize::to_node not found!!\n");
                iRet = PD_ERR;
                PutField_Int(hContext,"internal_error",iRet);
                }
...
ERRLOG("TxnMmcByUsCTB::Authorize::service_code not found!!\n");
                	iRet = PD_ERR;
	                PutField_Int(hContext,"internal_error",iRet);
		}
        }
...
ERRLOG("TxnMmcByUsBTC::Authorize::server node_id not found!!\n");
                iRet = INT_ERR;
...
/**************************************/
/* Call BO for Cal MB or Bank Bal      */
/**************************************/
/* ??????????????
If isd_ind == SOURCE 
	Debit(-) Merch Balance :txn_amt
	Debit(-) Merch Balance :processing_cost
   else 
	Credit(+) Bank Balance : txn_amt 
	Debit (-) Bank Balance : bank_charge
*/
/**************************************/
...
DEBUGLOG(("Authorize::Call BOMmsBalance: TxnTypeHandler\n"));
		...
		if (iRet != PD_OK) {
DEBUGLOG(("Authorize::CTBHandler Failed\n"));
ERRLOG("TxnMmcByUsBTC::Authorize::CTBHandler Failed\n");
                }
        }
...
ERRLOG("TxnMmcByUsBTC::Authorize::TxnTypeHandler Failed\n");
                        }
                        else {
			...
			PutField_CString(hTxn,"process_type", PD_MMS_PROCESS_TYPE);
                        PutField_CString(hTxn, "process_code", PD_MMS_TRANSFER_TO_MERCH);
...
ERRLOG("TxnMmcByUsBTC::Authorize::TxnMmsByUs2Node Failed\n");
                                                        }
                                                        else {
...
DEBUGLOG(("TxnMmcByUsBTC Normal Exit() iRet = [%d]\n",iRet));
	return iRet;
}
