...
Add alert for out queue error                      2015/06/17              Virginia Yun
*/
...
OBJPTR(DB);
OBJPTR(BO);
int     UpdatePreFEPRespTxnLog(hash_t *hContext,
                	hash_t* hResponse);

int     AddFEPTxnLog(hash_t *hContext,
                const hash_t* hRequest);

void TxnWebOnUs2FEP(char    cdebug)
{
        cDebug = cdebug;
}
int     AddFEPTxnLog(hash_t *hContext,
...

{
	...
	hash_t	*hTxn;
	hash_t	*hRec;
DEBUGLOG(("TxnWebOnUs2FEP::UpdateRespTxnLog()\n"));
...
DEBUGLOG(("UpdateTxnLog  iRet = [%d] after Update TxnPspDetail\n",iRet));
		...
		int	iResult;
        	...
        	iRet = (unsigned long)(*DBObjPtr)(PD_CHANNEL_51EPAY,&iResult,csTmpStatus);
...
DEBUGLOG(("process_resp_txn()\n"));
	...
	PutField_CString(hContext,"psp_channel_code",PD_CHANNEL_51EPAY);
...
DEBUGLOG(("process_resp_txn() call Update\n"));
		iRet = UpdatePreFEPRespTxnLog(hContext,hResponse);
	}
...
//#ifdef _SIMULATOR_
	...
	if (iRet == PD_OK && iSIM) {
		...
		PutField_CString(hResponse,"notice_status","OK");
		PutField_CString(hResponse,"sc","SIM00000001");
		PutField_Int(hResponse,"use_mobile_simulator",iSIM);
		return UpdatePreFEPRespTxnLog(hContext,hResponse);
	}
//#endif
	if (iRet == PD_OK) {
        if (!GetField_CString(hContext,"from_txn_seq",&csTxnSeq)) 
        	...
        	if ((*BOObjPtr)(hContext,hRequest,hRequest) == PD_OK) {
       	 		...
       	 		MsgObjPtr = CreateObj(MsgPtr,"FepMsg","FormatMsg");
        		h_msg = (hex_t*) malloc (sizeof(hex_t));
        		if ((*MsgObjPtr)(hRequest,h_msg->msg,&h_msg->len) == PD_OK) {
        
               			lKey = GetMQKey((const unsigned char *)"FEPPREQQ");
                		lRspKey = GetMQKey((const unsigned char *)"FEPPRSPQ");
...
DEBUGLOG(("Sent Error\n"));
ERRLOG("Fatal Error !!! TxnWebOnUs2FEP:: send Error to Queue...\n");

					char *csCmd=(char*) malloc(PD_MAX_FILE_LEN  +1);
					sprintf(csCmd,"send_fep_outq_error.sh");
					system(csCmd);
					FREE_ME(csCmd);
...
DEBUGLOG(("recv Error\n"));
ERRLOG("Fatal Error !!! TxnWebOnUs2FEP:: Recv Error to Queue...\n");
...
DEBUGLOG(("TxnWebOnUs2FEP: after process_resp_err_txn iret = [%d]\n",iRet));
					}
					else {
						msg->mtext[iRecvLen] = '\0';
DEBUGLOG(("TxnWebOnUs2FEP:  recv len = [%d]\n",iRecvLen));
DEBUGLOG(("TxnWebOnUs2FEP:  recv = [%s]\n",&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN));
						memcpy(h_msg->msg,&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN);
						iRet = process_resp_txn(hContext,h_msg);
DEBUGLOG(("TxnWebOnUs2FEP: after process_resp_txn iret = [%d]\n",iRet));
					}
                		}
        			FREE_ME(msg);
        		}     
...
DEBUGLOG(("TxnWebOnUs2FEP: BuildData Error\n"));
		}
	}

DEBUGLOG(("TxnWebOnUs2FEP: noraml exit iret = [%d]\n",iRet));
	return iRet;
}
