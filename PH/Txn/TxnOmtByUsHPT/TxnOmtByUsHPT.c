		...
		hash_t* hResponse)
{
	...
	char	*csBankStatTxnId;

	hash_t *hTxn = (hash_t*) malloc (sizeof(hash_t));
	hash_init(hTxn,0);

	hash_t *hBaidTxn = (hash_t*) malloc (sizeof(hash_t));
...
/* Get Stmt Txn Id */
	if (iRet == PD_OK) {
DEBUGLOG(("Authorize() call OLStmtTxnRelation:: GetStmtTxnIdByTxnId()\n"));
		DBObjPtr = CreateObj(DBPtr, "DBOLStmtTxnRelation", "GetStmtTxnIdByTxnId");
		iTmpRet = (unsigned long)(*DBObjPtr)(csTxnSeq, csStmtTxnId);
		if (iTmpRet != PD_FOUND) {
			iRet = INT_INVALID_TXN;
			PutField_Int(hContext, "internal_error", iRet);
DEBUGLOG(("Authorize() call OLStmtTxnRelation:: GetTxnIdByStmtTxnId() FAILURE!!!\n"));
ERRLOG("TxnOmtByUsHPT::Authorize() call OLStmtTxnRelation:: GetTxnIdByStmtTxnId() FAILURE!!!\n");
		} else {
...
DEBUGLOG(("Authorize() call DBOLBAIDTxn:: MatchRespTxn()\n"));
		...
		if ((unsigned long)(*DBObjPtr)(csStmtTxnId,PD_COMPLETE) != PD_FOUND) {
			iRet = INT_INVALID_TXN;
			PutField_Int(hContext, "internal_error", iRet);
DEBUGLOG(("Authorize() call DBOLBAIDTxn:: MatchRespTxn() FAILURE!!!\n"));
ERRLOG("TxnOmtByUsHPT::Authorize() call DBOLBAIDTxn:: MatchRespTxn() FAILURE!!!\n");
		}
	}
...
DEBUGLOG(("Authorize() call DBOLBAIDTxn:: GetBaidTxn()\n"));
		...
		if ((unsigned long)(*DBObjPtr)(csStmtTxnId, hBaidTxn) != PD_OK) {
			iRet = INT_ERR;
DEBUGLOG(("Authorize() call DBOLBAIDTxn:: GetBaidTxn() FAILURE!!!\n"));
ERRLOG("TxnOmtByUsHPT::Authorize() call DBOLBAIDTxn:: GetBaidTxn() FAILURE!!!\n");
		} else {
			// csBankStatTxnId
			if (GetField_CString(hBaidTxn, "stat_txn_id", &csBankStatTxnId)) {
DEBUGLOG(("Authorize() call DBOLBAIDTxn:: GetBaidTxn() stat_txn_id = [%s]\n", csBankStatTxnId));
				PutField_CString(hTxn, "stat_txn_id", csBankStatTxnId);
			}
		}
...
/* Update BAID Txn  */
	if (iRet == PD_OK) {
		PutField_CString(hBaidTxn,"txn_seq",csStmtTxnId);
		PutField_CString(hBaidTxn,"update_user",csUser);
DEBUGLOG(("Authorize() call DBOLBAIDTxn:: Update()\n"));
		...
		if ((unsigned long)(*DBObjPtr)(hBaidTxn) != PD_OK) {
			iRet = INT_ERR;
DEBUGLOG(("Authorize() call DBOLBAIDTxn:: Update() FAILURE!!!\n"));
ERRLOG("TxnOmtByUsHPT::Authorize() call DBOLBAIDTxn:: Update() FAILURE!!!\n");
		}
	}
