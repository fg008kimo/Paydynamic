...
OBJPTR(DB);
OBJPTR(BO);
int     UpdatePreSFQRespTxnLog(hash_t *hContext,
                	hash_t* hResponse);

int     AddSFQTxnLog(hash_t *hContext,
                const hash_t* hRequest);

void TxnWebOnUs2SFQ(char    cdebug)
{
        cDebug = cdebug;
}
int     AddSFQTxnLog(hash_t *hContext,
...

{
	...
	hash_t	*hTxn;
	hash_t	*hRec;
DEBUGLOG(("TxnWebOnUs2SFQ::UpdateRespTxnLog()\n"));
...
DEBUGLOG(("UpdateTxnLog  iRet = [%d] after Insert TxnQrRequestLog\n",iRet));
		...
		int	iResult;
        	...
        	iRet = (unsigned long)(*DBObjPtr)(PD_CHANNEL_SHANFU_QR,&iResult,csTmpStatus);
...
DEBUGLOG(("process_resp_txn()\n"));
	...
	PutField_CString(hContext,"psp_channel_code",PD_CHANNEL_SHANFU_QR);
	PutField_CString(hContext,"return_psp_channel",PD_CHANNEL_SHANFU_QR);
	...
	GetField_CString(hRequest,"psp_name",&csPspName);
      	MsgObjPtr = CreateObj(MsgPtr,"SfqMsg","BreakDownInitRspMsg");

        if ((*MsgObjPtr)(hResponse,inMsg->msg,inMsg->len) != PD_OK)  {
		iRet = INT_BREAKDOWN_ERR;
		PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("process_resp_txn:BreakDown SfqMsg Error\n"));
	}
//	else {
		// testing
...
DEBUGLOG(("process_resp_txn() call Update\n"));
		iRet = UpdatePreSFQRespTxnLog(hContext,hResponse);
...
DEBUGLOG(("TxbWebOnUs2SFQ: time_redirect = [%d]\n",iTmp));
	        ...
	        if (!GetField_CString(hContext,"from_txn_seq",&csTxnSeq)) 
       		 	...
       		 	if ((*BOObjPtr)(hContext,hRequest,hRequest) == PD_OK) {
       		 		...
       		 		MsgObjPtr = CreateObj(MsgPtr,"SfqMsg","FormatInitMsg");
...
DEBUGLOG(("TxnWebOnUs2SFQ: after process_resp_err_txn iret = [%d]\n",iRet));
					}
					else {
						msg->mtext[iRecvLen] = '\0';
DEBUGLOG(("TxnWebOnUs2SFQ:  recv len = [%d]\n",iRecvLen));
DEBUGLOG(("TxnWebOnUs2SFQ:  recv = [%s]\n",&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN));
						memcpy(h_msg->msg,&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN);
						iRet = process_resp_txn(hContext,h_msg,hRequest);
DEBUGLOG(("TxnWebOnUs2SFQ: after process_resp_txn iret = [%d]\n",iRet));
					}
                		}
        			FREE_ME(msg);
        		}     
...
DEBUGLOG(("TxnWebOnUs2SFQ: BuildData Error\n"));
		}
	}
...
DEBUGLOG(("TxnWebOnUs2SFQ: noraml exit iret = [%d]\n",iRet));
	return iRet;
}
