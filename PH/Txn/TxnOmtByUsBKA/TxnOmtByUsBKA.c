...
-------------------------------                    		------------            --------------
Init Version                                       		2013/09/09              [SWK]
Add Sim Cards Mgt				   		2013/12/19		[STP]
Amend for sys_switch_enabled                       		2014/01/17              [SWK]
Amend for province                                 		2014/05/09              [SWK]
Modify Sim Card Management			   		2014/09/01		[WWK]
Add bank_acct_source				   		2015/01/28		[WWK]
Check SimCard Status before associate		   		2015/02/04		[WWK]
Add DBOLBankAccts(ChkDepositBankAcctsUnique)	   		2015/02/26		[WMC]
Add DBOLBankAccts(ChkNotDepositBankAcctsUnique)
Add sub_provider				   		2015/02/27		[WWK]
Change DBOLBankAccts(ChkDepositBankAcctsUnique)	   		2015/02/27              [WMC]
...
Add key_expired_datetime		  	   		2015/03/12              [WMC]
Add init_provider_id                               		2018/12/07              [MIC]     
Add 
   DBOLAutoUploadBankDetail(UpdateDisabled)	   		2019/03/05		[WMC]
   DBOLAutoUploadAcctStatus(ChkBankAcctStatusSetEnable)
   DBOLAutoUploadAcctStatus(ChkBankAcctStatusSetDisable)	
Add override condition for disable sms_stmt, sys_switch		2019/06/10		[ING]
Change Action "Update"						2020/06/18		[WMC]
Add disable support_sms_stmt                                    2021/04/21              [MIC] 
when not yet frozen, not yet sunset bank acct status            
*/
...
DEBUGLOG(("Authorize::DBOLBankAccts:ChkNumOfDepositBankAccts NOT DUPLICATE\n"));
...
ERRLOG("TxnOmtByUsBKA:: Authorize:: DBOLBankAccts:GetBankAccts Failed\n");
                                                ...
                                                if (GetField_CString(hBankAcct, "status_type", &csTmp)) {
                                                        if (!strcmp(csTmp, PD_BANK_ACCT_STATUS_DISPOSED)) {
DEBUGLOG(("Authorize:DBOLBankAccts: GetBankAccts org status type disposed!\n"));
                                                                iRet = INT_BANK_ACCT_NOT_FOUND;
                                                                PutField_Int(hContext,"internal_error",iRet);
                                                        } else {
DEBUGLOG(("Authorize:DBOLBankAccts: GetBankAccts org status type not disposed, OK for Update!\n"));
							}
                                                } else {
DEBUGLOG(("Authorize:DBOLBankAccts: GetBankAccts status type not found!\n"));
ERRLOG("TxnOmtByUsBKA:: Authorize:: DBOLBankAccts:GetBankAccts status type not found!\n");
                                                        iRet = PD_ERR;
                                                        PutField_Int(hContext,"internal_error",iRet);
                                                }
                                        }
...
// Update support_sms_stmt and sys_switch_enabled 
				if (iRet == PD_OK) {
					if (csStatusType != NULL) {
						if (	(!strcmp(csStatusType, PD_BANK_ACCT_STATUS_FROZEN)) || 
							(!strcmp(csStatusType, PD_BANK_ACCT_STATUS_HOLD))   || 
							(!strcmp(csStatusType, PD_BANK_ACCT_STATUS_SUNSET)) || 
							(!strcmp(csStatusType, PD_BANK_ACCT_STATUS_SUSPEND))||
							(!strcmp(csStatusType, PD_BANK_ACCT_STATUS_NOT_YET_SUNSET))||
							(!strcmp(csStatusType, PD_BANK_ACCT_STATUS_NOT_YET_FROZEN))
							 
						) 
						{

								PutField_Int(hTxn, "support_sms_stmt", PD_FALSE);      
						}
		
						if (!strcmp(csStatusType, PD_BANK_ACCT_STATUS_DISPOSED)) {
							PutField_Int(hTxn, "sys_switch_enabled", PD_FALSE);
						}
					}
				}
				...
				if (iRet == PD_OK) {
					...
					if ((*DBObjPtr)(csIntBankCode, csBankAcctNum, hBankAcctId) == PD_OK) {
						...
						if (iRenewBankAcct == PD_TRUE) {
							iRet = INT_EXISTING_ASSOCIATED_BAID_OPEN;
                                               		PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("Authorize:: existing associated BAID open, not allow to process!\n"));
ERRLOG("TxnOmtByUsBKA:: Authorize:: existing associated BAID open, not allow to process!\n");
						} else {
							if (GetField_CString(hBankAcct, "acct_type", &csOrgAcctType)) {
DEBUGLOG(("Authorize:: Org Acct Type [%s]\n", csOrgAcctType));
							} else {
DEBUGLOG(("Authorize:: Org Acct Type [NULL]\n"));
							}
							
							if (csAcctType != NULL && csOrgAcctType != NULL) {
								if (strcmp(csAcctType, csOrgAcctType)) {
									iRet = INT_BANK_ACCT_NATURE_NOT_MATCH;
                                                                	PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("Authorize:: bank account nature not match to associated BAID nature, not allow to process!\n"));
ERRLOG("TxnOmtByUsBKA:: Authorize:: bank account nature not match to associated BAID nature, not allow to process!\n");
                                                                }
							} else {
								if ((csAcctType == NULL && csOrgAcctType != NULL) ||
...
/* OLAutoUploadBankDetail */
	if (iRet == PD_OK) {
                if ((cAction == PD_ACTION_UPDATE) && (iRenewBankAcct == PD_FALSE)) {
			if ((csAcctType != NULL) && (csOrgStatusType != NULL) && (csStatusType != NULL)) {
