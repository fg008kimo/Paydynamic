...
Update to support offline phase 2                  2015/07/13              David Wong
*/
...
/* sender_baid_name */
	if (iRet == PD_OK) {
                if (GetField_CString(hRequest, "sender_baid_name", &csTmp)) {
DEBUGLOG(("Authorize() sender_baid_name = [%s]\n", csTmp));
                }
        }
/* recipient_baid_name */
        if (iRet == PD_OK) {
                if (GetField_CString(hRequest, "recipient_baid_name", &csTmp)) {
DEBUGLOG(("Authorize() recipient_baid_name = [%s]\n", csTmp));
                }
	}
/* client_name */
        if (iRet == PD_OK) {
...
DEBUGLOG(("Authorize() call DBOLTxnSeq::GetNextOdiFileSeq() file_id = [%s]\n",csFileId));
		...
		PutField_Char(hRec,"status",PD_STMT_APPROVED);
		PutField_Int(hRec,"file_count",1);
...
/* Add Statement Detail */
	if (iRet == PD_OK) {
		...
		if (GetField_CString(hRequest,"sender_baid_name",&csTmp)) {
                        PutField_CString(hRec,"sender_baid_name",csTmp);
                }
		if (GetField_CString(hRequest,"recipient_baid_name",&csTmp)) {
                        PutField_CString(hRec,"recipient_baid_name",csTmp);
                }
		if (GetField_CString(hRequest,"client_name",&csTmp)) {
                        PutField_CString(hRec,"client_name",csTmp);
                }
...
DEBUGLOG(("Authorize() call DBOLStatement::UpdateTxnId()\n"));
		...
		if ((unsigned long)(*DBObjPtr)(hRec) != PD_OK) {
			iRet = INT_ERR;
DEBUGLOG(("TxnOmtByUsBSS::Authorize() call DBOLStatement::UpdateTxnId() FAILURE!!!\n"));
ERRLOG("TxnOmtByUsBSS::Authorize() call DBOLStatement::UpdateTxnId() FAILURE!!!\n");
		}
	}
	...
	if (iRet == PD_OK) {
		// activity
		PutField_CString(hRec, "activity", "RECON");

		// bank_stmt_type
		PutField_CString(hRec, "bank_stmt_type", "DEPOSIT");

		// recon_type
		PutField_CString(hRec, "recon_type", "SAME");
...
DEBUGLOG(("Authorize() baid_txn_id_0 = [%s]\n", csBAIDTxnId));
...
DEBUGLOG(("Authorize() call BOOLTxnEngine::DoAction()\n"));
		...
		if (iTmpRet != PD_OK) {
			iRet = INT_ERR;
DEBUGLOG(("Authorize() call BOOLTxnEngine::DoAction() FAILURE!!!\n"));
ERRLOG("TxnOmtByUsBSS::Authorize() call BOOLTxnEngine::DoAction() FAILURE!!!\n");
		}
	}
