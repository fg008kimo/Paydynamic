...
BAID handling 					   2014/12/29		   Virginia Yun
*/
...
void TxnOmtByUsCOM(char    cdebug)
{
        cDebug = cdebug;
}
...
ERRLOG("TxnOmtByUsCOM::Authorize() merchant_id Invalid\n");
		}
	}
/* to merchant id */
...
ERRLOG("TxnOmtByUsCOM::Authorize()  to_merchant_id Invalid\n");
		}
		FREE_ME(hReq);
	}
...
DEBUGLOG(("Authorize() baid from request[%s]\n",csBaid));

		hash_t *hBaidDtl;
		hBaidDtl = (hash_t*)malloc(sizeof(hash_t));
		hash_init(hBaidDtl, 0);

DEBUGLOG(("Authorize() OLBankAcctId GetBankAcctIdDtl\n"));


		DBObjPtr = CreateObj(DBPtr,"DBOLBankAcctId","GetBankAcctIdDtl");
                iRet = (unsigned long)(*DBObjPtr)(csBaid, hBaidDtl); 

		if(iRet!=PD_OK){
			iRet = INT_INVALID_BAID;
                       	PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("Authorize() baid Invalid[%s]\n",csPtr));
ERRLOG("TxnOmtByUsCOM::Authorize() baid Invalid\n");
		}

		if (!GetField_CString(hBaidDtl, "org_baid", &csPtr)) {
			iRet = INT_INVALID_BAID;
                       	PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("Authorize() baid Invalid[%s]\n",csPtr));
ERRLOG("TxnOmtByUsCOM::Authorize() baid Invalid\n");
		...
		if (GetField_CString(hBaidDtl, "psp_id", &csPtr)) {
DEBUGLOG(("Authorize() baid [%s] psp_id [%s]\n", csBaid, csPtr));
			PutField_CString(hContext, "baid_psp_id", csPtr);
		}

		if (GetField_CString(hBaidDtl, "status", &csPtr)) {
DEBUGLOG(("Authorize() baid [%s] status [%s]\n", csBaid, csPtr));
			PutField_CString(hContext, "baid_status", csPtr);
		}

		if (GetField_CString(hBaidDtl, "int_bank_code", &csPtr)) {
DEBUGLOG(("Authorize() baid [%s] int_bank_code [%s]\n", csBaid, csPtr));
			PutField_CString(hContext, "baid_bank_code", csPtr);
		}

		if (GetField_CString(hBaidDtl, "bank_acct_num", &csPtr)) {
DEBUGLOG(("Authorize() baid [%s] bank_acct_num [%s]\n", csBaid, csPtr));
			PutField_CString(hContext, "baid_bank_acct_num", csPtr);
		}

		if (GetField_CString(hBaidDtl, "category", &csPtr)) {
DEBUGLOG(("Authorize() baid [%s] category [%s]\n", csBaid, csPtr));
			PutField_CString(hContext, "baid_category", csPtr);
		}
...
ERRLOG("TxnOmtByUsCOM:Authorize() Currency code invalid\n",csTxnCcy);
                }
        }
...
ERRLOG("TxnOmtByUsCOM::Authorize() txn_amt Invalid\n");
				}*/
			}
		}
		else{
...
ERRLOG("TxnOmtByUsCOM::Authorize() service_fee Invalid\n");
		}
	}
/*
	if(GetField_CString(hRequest,"txnamt",&csPtr) && iRet == PD_OK){       
...
ERRLOG("TxnOmtByUsCOM:Authorize() to Currency code invalid\n",csPtr);
                }
		FREE_ME(csTxnCcy);
        }
...
ERRLOG("TxnOmtByUsCOM:Authorize() invalid country [%s]\n",csTxnCountry);
                }
	}
...
ERRLOG("TxnOmtByUsCOM:Authorize() invalid service code [%s]\n",csPtr);
		}
/*
		else{
...
ERRLOG("TxnOmtByUsCOM:Authorize() invalid To service code [%s]\n",csPtr);
                	}
		}
*/
        }
