...
-------------------------------                    ------------            --------------
Init Version                                       2018/03/28              LokMan Chow
*/
...
OBJPTR(DB);
OBJPTR(BO);
int     UpdatePrePSVRespTxnLog(hash_t *hContext,
                	hash_t* hResponse);

int     AddPSVTxnLog(hash_t *hContext,
                const hash_t* hRequest);

void TxnWebOnUs2PSV(char    cdebug)
{
        cDebug = cdebug;
}
int     AddPSVTxnLog(hash_t *hContext,
...

{
	...
	hash_t	*hTxn;
	hash_t	*hRec;
DEBUGLOG(("TxnWebOnUs2PSV::UpdateRespTxnLog()\n"));
...
DEBUGLOG(("UpdateTxnLog  iRet = [%d] after Update TxnPspDetail\n",iRet));
		...
		int	iResult;
        	...
        	iRet = (unsigned long)(*DBObjPtr)(PD_CHANNEL_PAYESCAPE_VNET,&iResult,csTmpStatus);
...
DEBUGLOG(("process_resp_txn()\n"));
	...
	PutField_CString(hContext,"psp_channel_code",PD_CHANNEL_PAYESCAPE_VNET);
	PutField_CString(hContext,"return_psp_channel",PD_CHANNEL_PAYESCAPE_VNET);
	...
	GetField_CString(hRequest,"psp_name",&csPspName);
      	MsgObjPtr = CreateObj(MsgPtr,"PsvMsg","BreakDownInitRspMsg");

        if ((*MsgObjPtr)(hResponse,inMsg->msg,inMsg->len) != PD_OK)  {
		iRet = INT_BREAKDOWN_ERR;
		PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("process_resp_txn:BreakDown PsvMsg Error\n"));
	}
//	else {

		if (GetField_Int(hResponse, "ret", &iRetCode)) {
DEBUGLOG(("process_resp_txn: returned status = [%d]\n", iRetCode));

			if (iRetCode == 1) {

				// get redirect_url
				if (!GetField_CString(hResponse, "redirect_url", &csTmp)) {
DEBUGLOG(("process_resp_txn: cannot get redirect_url!!!\n"));
...
DEBUGLOG(("process_resp_txn() call Update\n"));
		iRet = UpdatePrePSVRespTxnLog(hContext,hResponse);
//	}
	FREE_ME(csCmd);
	...
	if (iRet == PD_OK) {
	        ...
	        if (!GetField_CString(hContext,"from_txn_seq",&csTxnSeq)) 
       		 	...
       		 	if ((*BOObjPtr)(hContext,hRequest,hRequest) == PD_OK) {
       		 		...
       		 		MsgObjPtr = CreateObj(MsgPtr,"PsvMsg","FormatInitMsg");
...
DEBUGLOG(("TxnWebOnUs2PSV: after process_resp_err_txn iret = [%d]\n",iRet));
					}
					else {
						msg->mtext[iRecvLen] = '\0';
DEBUGLOG(("TxnWebOnUs2PSV:  recv len = [%d]\n",iRecvLen));
DEBUGLOG(("TxnWebOnUs2PSV:  recv = [%s]\n",&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN));
						memcpy(h_msg->msg,&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN);
						iRet = process_resp_txn(hContext,h_msg,hRequest);
DEBUGLOG(("TxnWebOnUs2PSV: after process_resp_txn iret = [%d]\n",iRet));
					}
                		}
        			FREE_ME(msg);
        		}     
...
DEBUGLOG(("TxnWebOnUs2PSV: BuildData Error\n"));
		}
	}

DEBUGLOG(("TxnWebOnUs2PSV: noraml exit iret = [%d]\n",iRet));
	return iRet;
}
