...
OBJPTR(DB);
OBJPTR(BO);
int     UpdatePreXJWRespTxnLog(hash_t *hContext,
                	hash_t* hResponse);

int     AddXJWTxnLog(hash_t *hContext,
                const hash_t* hRequest);

void TxnWebOnUs2XJW(char    cdebug)
{
        cDebug = cdebug;
}
int     AddXJWTxnLog(hash_t *hContext,
...

{
	...
	hash_t	*hTxn;
	hash_t	*hRec;
DEBUGLOG(("TxnWebOnUs2XJW::UpdateRespTxnLog()\n"));
...
DEBUGLOG(("UpdateTxnLog  iRet = [%d] after Insert TxnQrRequestLog\n",iRet));
		...
		int	iResult;
        	...
        	iRet = (unsigned long)(*DBObjPtr)(PD_CHANNEL_XJ_CHWECHAT,&iResult,csTmpStatus);
...
DEBUGLOG(("process_resp_txn()\n"));
	...
	PutField_CString(hContext,"psp_channel_code",PD_CHANNEL_XJ_CHWECHAT);
	PutField_CString(hContext,"return_psp_channel",PD_CHANNEL_XJ_CHWECHAT);
	...
	GetField_CString(hRequest,"psp_name",&csPspName);
      	MsgObjPtr = CreateObj(MsgPtr,"XjwMsg","BreakDownInitRspMsg");

        if ((*MsgObjPtr)(hResponse,inMsg->msg,inMsg->len) != PD_OK)  {
		iRet = INT_BREAKDOWN_ERR;
		PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("process_resp_txn:BreakDown XjwMsg Error\n"));
	}
//	else {
		// testing
...
DEBUGLOG(("process_resp_txn() call Update\n"));
		iRet = UpdatePreXJWRespTxnLog(hContext,hResponse);
...
DEBUGLOG(("TxbWebOnUs2XJW: time_redirect = [%d]\n",iTmp));
	        ...
	        if (!GetField_CString(hContext,"from_txn_seq",&csTxnSeq)) 
       		 	...
       		 	if ((*BOObjPtr)(hContext,hRequest,hRequest) == PD_OK) {
       		 		...
       		 		MsgObjPtr = CreateObj(MsgPtr,"XjwMsg","FormatInitMsg");
...
DEBUGLOG(("TxnWebOnUs2XJW: after process_resp_err_txn iret = [%d]\n",iRet));
					}
					else {
						msg->mtext[iRecvLen] = '\0';
DEBUGLOG(("TxnWebOnUs2XJW:  recv len = [%d]\n",iRecvLen));
DEBUGLOG(("TxnWebOnUs2XJW:  recv = [%s]\n",&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN));
						memcpy(h_msg->msg,&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN);
						iRet = process_resp_txn(hContext,h_msg,hRequest);
DEBUGLOG(("TxnWebOnUs2XJW: after process_resp_txn iret = [%d]\n",iRet));
					}
                		}
        			FREE_ME(msg);
        		}     
...
DEBUGLOG(("TxnWebOnUs2XJW: BuildData Error\n"));
		}
	}
...
DEBUGLOG(("TxnWebOnUs2XJW: noraml exit iret = [%d]\n",iRet));
	return iRet;
}
