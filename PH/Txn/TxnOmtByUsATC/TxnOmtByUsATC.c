...
void TxnOmtByUsATC(char cdebug)
{
	cDebug = cdebug;
}
...
ERRLOG("TxnOmtByUsATC: Authorize: baid_txn_id not found\n");
		iRet = PD_ERR;
	}
...
/* statement transaction id */
		PutField_CString(hReq, "baid_txn_id", csBaidTxnId);

/* Request Host Queue */
		lKey = GetMQKey((const unsigned char *)"ATCREQQ");

		h_msg = (hex_t*) malloc (sizeof(hex_t));
		MsgObjPtr = CreateObj(MsgPtr, "AtcMsg", "FormatMsg");
...
DEBUGLOG(("Authorize: h_msg->msg = [%s] h_msg->len = [%d]\n", h_msg->msg, h_msg->len));
			...
			msg->mtype = ctol((const unsigned char *)csBaidTxnId, strlen(csBaidTxnId));
...
DEBUGLOG(("Authorize: Request Queue = [ATCREQQ]\n"));
			if (MQSend(lKey, msg, iSendLen) != MQ_OK) {
DEBUGLOG(("Authorize: Sent Error\n"));
ERRLOG("TxnOmtByUsATC: Authorize: Sent Error\n");
				iRet = PD_ERR;
			}
			FREE_ME(msg);
		} else {
DEBUGLOG(("Authorize: call AtcMsg:FormatMsg() failed\n"));
ERRLOG("TxnOmtByUsATC: Authorize: call AtcMsg:FormatMsg() failed\n");
			iRet = PD_ERR;
		}
		FREE_ME(h_msg);
