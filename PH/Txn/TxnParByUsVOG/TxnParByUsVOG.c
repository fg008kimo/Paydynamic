...
-------------------------------                    ------------            --------------
Init Version                                       2013/01/10              Virginia Yun
*/
...
#include "ObjPtr.h"
#include "internal.h"
#include "TxnParByUsVOG.h"
#include "myrecordset.h"
#include "queue_utility.h"
#include "mq_db.h"
#include "../src/batch/parallel_run/pr_par_funct.h"
...
void TxnParByUsVOG(char    cdebug)
{
        cDebug = cdebug;
}
...
//	int	iStatus;
	...
	int	iRefundParty = 0;
	hash_t  *hRec, *hOrgTxn, *hIn, *hPspDetail, *hBalance, *hPsp;
...
DEBUGLOG(("TxnParByUsVOG: Authroize()\n"));
/*
	if(GetField_Int(hContext,"VPFEE",&iReturnFee)){
DEBUGLOG(("TxnParByUsVOG: Void Payout Fee Return [%d]\n",iReturnFee));
	}
*/
...
********************************************* */
	...
	GetField_Int(hContext, "pog_record_exists", &iPOGExists);

////////////Finding POG
	if(iRet==PD_OK && iRefundParty==2){

		if (iPOGExists == PD_TRUE) {
...
DEBUGLOG(("Authorize::DBPayoutGeneratedFileDT:GetFileDetailByUploadTxnId Txn Not Generated. Cannot Void!!!\n"));
			...
			// Get From Reversed Record Table
DEBUGLOG(("Authorize:: Call prpar2_GetReversedFileDeatil as not exists pog record\n"));
			recordset_init(rRecordSet, 0);

			DBObjPtr = CreateObj(DBPtr, "ParPOReversedDT", "GetReversedFileDetail");
			if ((*DBObjPtr)(hContext, rRecordSet) == PD_OK) {
                                i = 0;
                                hRec = RecordSet_GetFirst(rRecordSet);
                                while (hRec) {
					DBObjPtr = CreateObj(DBPtr,"DBTxnSeq","GetNextPayoutTxnSeq");
					strcpy(csTxnID, (*DBObjPtr)());
...
ERRLOG("TxnParByUsVOG:Authorize::GetTxnHeader::not found record!!\n");
                        iRet=INT_NOT_RECORD;
                        PutField_Int(hContext,"internal_error",iRet);
                }
	}
...
ERRLOG("TxnParByUsVOG:Authorize::GetTxnDetail:: not found record!!\n");
                        iRet = INT_NOT_RECORD;
                        PutField_Int(hContext,"internal_error",iRet);
                }
        }
...
ERRLOG("TxnParByUsVOG:Authorize::BOFee:GetTxnFee Error\n");
                }
		if(iReturnFee==PD_TRUE)
...
ERRLOG("TxnParByUsVOG::DBPayoutGeneratedFileDT Update Failed!!!\n");
						break;
					}
				}
...
DEBUGLOG(("GetDetail:: VoidOrgTxnElements Failed!!!\n"));
				...
				if(iRet==PD_OK){
					...
					PutField_CString(hOrgTxn, "status", "PD_REVERSED");
...
DEBUGLOG(("DBTransaction:UpdateDetail Failed\n"));
				...
				// POG Record NOT Exists!!!
				PutField_CString(hPsp, "channel_code", PD_CHANNEL_MGT);
				PutField_CString(hPsp, "process_code", PD_PROCESS_CODE_DEF);
				PutField_CString(hPsp, "process_type", PD_PROCESS_TYPE_DEF);
...
DEBUGLOG(("POG Record NOT Exists:: Call MGTChannel:AddTxnLog\n"));
				ChannelObjPtr = CreateObj(ChannelPtr,"MGTChannel","AddTxnLog");
				if((unsigned long) ((*ChannelObjPtr)(hPsp,hPsp))!=PD_OK){
					iRet = INT_ERR;
DEBUGLOG(("POG Record NOT Exists:: MGTChannel:AddTxnLog Failed\n"));
				}
				PutField_Int(hPsp,"do_logging",PD_FALSE);
				...
				if (iRet == SUCCESS) {
					...
					if (GetField_CString(hContext, "psp_country", &csTmp)) {
						PutField_CString(hPsp, "txn_country", csTmp);
						PutField_CString(hBalance, "org_txn_country", csTmp);
					}

DEBUGLOG(("POG Record NOT Exists:: Call DBTransaction:AddDetail\n"));
					DBObjPtr = CreateObj(DBPtr,"DBTransaction","AddDetail");
					if((unsigned long) ((*DBObjPtr)(hPsp))!=PD_OK){
						iRet = INT_ERR;
DEBUGLOG(("POG Record NOT Exists:: DBTransaction:AddDetail Failed\n"));
					}
				}
				...
				if (iRet == SUCCESS) {
					...
					if (GetField_CString(hContext, "vog_psp_id", &csTmp)) {
						PutField_CString(hPspDetail, "psp_id", csTmp);
						PutField_CString(hBalance, "org_psp_id", csTmp);
					}
					...
					if (GetField_CString(hContext, "service_fee", &dTmp)) {
							sprintf(csTag,"precal_fee_%d",i);
							if(iReturnPspFee == PD_FALSE){
								dTmp = 0.0;
							}
...
DEBUGLOG(("Authorize::Call DBTransaction:UpdateDetail txn_seq [%s]\n", csTxnSeq));
					DBObjPtr = CreateObj(DBPtr,"DBTransaction","UpdateDetail");
					if((unsigned long) ((*DBObjPtr)(hBalance))!=PD_OK){
						iRet = INT_ERR;
DEBUGLOG(("Authorize::DBTransaction:UpdateDetail Failed\n"));
...
DEBUGLOG(("TxnParByUsVOG: Authroize:: BOBalance:UpdatePayoutAmount() result = [%d]\n",iRet));
		}
	}
