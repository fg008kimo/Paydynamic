...
-------------------------------                    ------------            --------------
Init Version                                       2017/09/28              David Wong
*/
...
OBJPTR(DB);
OBJPTR(BO);
int     UpdatePreXJQRespTxnLog(hash_t *hContext,
                	hash_t* hResponse);

int     AddXJQTxnLog(hash_t *hContext,
                const hash_t* hRequest);

void TxnWebOnUs2XJQ(char    cdebug)
{
        cDebug = cdebug;
}
int     AddXJQTxnLog(hash_t *hContext,
...

{
	...
	hash_t	*hTxn;
	hash_t	*hRec;
DEBUGLOG(("TxnWebOnUs2XJQ::UpdateRespTxnLog()\n"));
...
DEBUGLOG(("UpdateTxnLog  iRet = [%d] after Insert TxnQrRequestLog\n",iRet));
		...
		int	iResult;
        	...
        	iRet = (unsigned long)(*DBObjPtr)(PD_CHANNEL_XJ_JINHAI_QR,&iResult,csTmpStatus);
...
DEBUGLOG(("process_resp_txn()\n"));
	...
	PutField_CString(hContext,"psp_channel_code",PD_CHANNEL_XJ_JINHAI_QR);
	PutField_CString(hContext,"return_psp_channel",PD_CHANNEL_XJ_JINHAI_QR);
	...
	GetField_CString(hRequest,"psp_name",&csPspName);
      	MsgObjPtr = CreateObj(MsgPtr,"XjqMsg","BreakDownInitRspMsg");

        if ((*MsgObjPtr)(hResponse,inMsg->msg,inMsg->len) != PD_OK)  {
		iRet = INT_BREAKDOWN_ERR;
		PutField_Int(hContext,"internal_error",iRet);
DEBUGLOG(("process_resp_txn:BreakDown XjqMsg Error\n"));
	}
//	else {
		// testing
...
DEBUGLOG(("process_resp_txn() call Update\n"));
		iRet = UpdatePreXJQRespTxnLog(hContext,hResponse);
...
DEBUGLOG(("TxbWebOnUs2XJQ: time_redirect = [%d]\n",iTmp));
	        ...
	        if (!GetField_CString(hContext,"from_txn_seq",&csTxnSeq)) 
       		 	...
       		 	if ((*BOObjPtr)(hContext,hRequest,hRequest) == PD_OK) {
       		 		...
       		 		MsgObjPtr = CreateObj(MsgPtr,"XjqMsg","FormatInitMsg");
...
DEBUGLOG(("TxnWebOnUs2XJQ: after process_resp_err_txn iret = [%d]\n",iRet));
					}
					else {
						msg->mtext[iRecvLen] = '\0';
DEBUGLOG(("TxnWebOnUs2XJQ:  recv len = [%d]\n",iRecvLen));
DEBUGLOG(("TxnWebOnUs2XJQ:  recv = [%s]\n",&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN));
						memcpy(h_msg->msg,&msg->mtext[MQ_HEADER_LEN],iRecvLen - MQ_HEADER_LEN);
						iRet = process_resp_txn(hContext,h_msg,hRequest);
DEBUGLOG(("TxnWebOnUs2XJQ: after process_resp_txn iret = [%d]\n",iRet));
					}
                		}
        			FREE_ME(msg);
        		}     
...
DEBUGLOG(("TxnWebOnUs2XJQ: BuildData Error\n"));
		}
	}
...
DEBUGLOG(("TxnWebOnUs2XJQ: noraml exit iret = [%d]\n",iRet));
	return iRet;
}
