...
re-order txn_code checking &call BOTxnElement	   2012/02/06		   LokMan Chow
Add Txn COde - Deposit Chargeback		   2015/12/17		   Elvis Wong
Add return_to_mfee & call DBDefVoidTxnConfig       2016/06/23              David Wong
*/
...
DEBUGLOG(("Authorize::return_to_merchantfee = [%d]\n",iReturnToMerchFee));
	}
	else{
		PutField_Int(hRequest,"return_to_mfee",PD_FALSE);
DEBUGLOG(("Authorize::return_to_merchantfee (default)= [%d]\n",PD_FALSE));
	}
...
DEBUGLOG(("Authorize::return_pspfee (default)= [%d]\n",PD_FALSE));
	...
	if(GetField_CString(hRequest,"is_deposit_chargeback",&csTmp)){
                iDepositChargeback = atoi(csTmp);
		PutField_Int(hContext,"is_deposit_chargeback",iDepositChargeback);
DEBUGLOG(("Authorize::deposit_chargeback = [%d]\n",iDepositChargeback));
        }
...
ERRLOG("TxnMgtByUsVOT::Authorize() txn_amt Invalid\n");
			}
		}
	}
...
DEBUGLOG(("Authorize::Call DBMerchantUploadFileDetail:GetDetailByTxnId\n"));
			...
			if ((*DBObjPtr)(csTxnSeq,rRecordSet) == PD_OK) {
				...
				while (hRec) {
					iFound = PD_TRUE;
					PutField_Char(hContext,"merch_psp_ind",PD_TYPE_MERCHANT);
...
DEBUGLOG(("handle_PayoutIndvCancel::Call DBPayoutGeneratedFileDT:GetDetailByGenId\n"));
				...
				if((unsigned long)(*DBObjPtr)(csTxnSeq,rRecordSet)==PD_OK) {
					...
					while (hRec) {
						iFound = PD_TRUE;
						PutField_Char(hContext,"merch_psp_ind",PD_TYPE_PSP);
						if(GetField_Int(hRec,"status",&iTmp)){
DEBUGLOG(("status = [%d]\n",iTmp));
...
ERRLOG("TxnMgtByUsVOT::Authorize: MMS Mode is ON, Txn unvoidable!!!!!!!!\n");
						iRet=INT_ERR;
					}
				}
			}
...
ERRLOG("TxnMgtByUsVOT::Authorize: The two txn_seq are not in a pair!!!!!!!!\n");
					}
				}
			}
...
/*
				if(!strcmp(csTxnCode,PD_PAYOUT_APPROVE) ||
				   !strcmp(csTxnCode,PD_WITHDRAW_BATCH_TXN_CODE)){
					BOObjPtr = CreateObj(BOPtr,"BOPayout","isVoidWithFee");
					iRet = (unsigned long)(*BOObjPtr)(hContext);
DEBUGLOG(("Authorize:: BOPayout:isVoidWithFee iRet = [%d]\n",iRet));
				}
*/
				if(!strcmp(csTxnCode,PD_PAYOUT_APPROVE)){
				...
				if (iTmpRet == FOUND) {
DEBUGLOG(("Authorize:: csValue = [%s]\n", csValue));
					if (csValue[0] == PD_NO) {
						iSkipVoidOrgTxnElements = PD_TRUE;
DEBUGLOG(("Authorize:: skip BOTxnElements:VoidOrgTxnElements\n"));
					}
				}
...
DEBUGLOG(("Authorize:: BOTxnElements:VoidOrgTxnElements iRet = [%d]\n",iRet));
				...
				if(!strcmp(csTxnCode,PD_MERCHANT_BAL_TFF)){
					BOObjPtr = CreateObj(BOPtr,"BOTxnElements","VoidOrgTxnElements");
					iRet = (unsigned long)(*BOObjPtr)(hOptContext,hRequest);
...
/*Void Deposit*/
				if(!strcmp(csTxnCode,PD_DEPOSIT_TXN_CODE)||
					...
					if (iDepositChargeback == PD_TRUE) {
                                                PutField_CString(hContext,"new_txn_code",PD_DEPOSIT_CHARGEBACK);
					} else {
						PutField_CString(hContext,"new_txn_code",PD_DEPOSIT_VOID);
					}
                			sprintf(csHandler,"TxnMgtByUs%s",PD_DEPOSIT_VOID);
				}
/*Void Payout*/
					...
					PutField_Int(hContext,"do_logging",PD_NO_LOG);
                			sprintf(csHandler,"TxnMgtByUs%s",PD_PAYOUT_VOID);
				}
...
/*Void Merchant Bal Transfer To Other System */
					...
					PutField_CString(hContext, "new_txn_code", PD_MERCH_VOID_BAL_TRF_OTH_SYS);
                			sprintf(csHandler,"TxnMgtByUs%s",PD_MERCH_VOID_BAL_TRF_OTH_SYS);
				}
/*Void PSP Bal Transfer To Other System */
					...
					PutField_CString(hContext, "new_txn_code", PD_PSP_VOID_BAL_TRF_OTH_SYS);
                			sprintf(csHandler,"TxnMgtByUs%s",PD_PSP_VOID_BAL_TRF_OTH_SYS);
				}
/*Void Intra Merchant Bal Transfer*/
					...
					PutField_CString(hContext,"new_txn_code",PD_MERCHANT_BAL_TFF_VOID);
                			sprintf(csHandler,"TxnMgtByUs%s",PD_MERCHANT_BAL_TFF_VOID);
				}
/*Void Balance Transfer Out - Intra Payout*/
				else if(!strcmp(csTxnCode,PD_AVA_PAYOUT_REQ_TF_FROM)){
					PutField_CString(hContext,"new_txn_code",PD_VOID_AVA_PAYOUT_REQ_TF_FROM);
					sprintf(csHandler,"TxnMgtByUs%s",PD_VOID_AVA_PAYOUT_REQ_TF_FROM);
				}
/*Void Psp Bal Transfer*/
				else if(!strcmp(csTxnCode,"PBU")){
					PutField_CString(hContext,"new_txn_code","PBR");
                			sprintf(csHandler,"TxnMgtByUs%s","PBR");
				}
/* Fund-in Payout for Merchant */
					...
					PutField_CString(hContext,"new_txn_code",PD_FUND_IN_PAYOUT_MERCHANT_VOID);
                			sprintf(csHandler,"TxnMgtByUs%s", PD_FUND_IN_PAYOUT_MERCHANT_VOID);
				}
/* Fund-in Payout for PSP */
					...
					PutField_CString(hContext,"new_txn_code",PD_FUND_IN_PAYOUT_PSP_VOID);
                			sprintf(csHandler,"TxnMgtByUs%s", PD_FUND_IN_PAYOUT_PSP_VOID);
				}
/* Fixed Amount HoldBack */
					...
					sprintf(csHandler,"TxnMgtByUs%s", PD_RELEASE_FIXED_AMT_HB);
				}
...
/*Void Adjustment*/
					if (GetField_Int(hContext, "is_adj_txn_code", &iTmp)) {
						if (iTmp == PD_TRUE) {
                					sprintf(csHandler,"TxnMgtByUs%s","VAD");
						}
					}
				}
DEBUGLOG(("TxnMgtByUsVOT Create Txn Object [%s]\n",csHandler));
